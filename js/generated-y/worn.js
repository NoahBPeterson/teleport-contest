...
export function* mon_break_armor(mon, polyspot) {
    ...
    if (breakarm(mdat)) {
        ...
        if ((otmp = (yield* which_armor(mon, 2n))) !== null && (cptr.ldI16o(otmp, $obj_otyp) != NHC.MUMMY_WRAPPING || !(humanoid(mdat) && cptr.ld1uo((mdat), $permonst_msize) >= NHM.MZ_SMALL && cptr.ld1uo((mdat), $permonst_msize) <= NHM.MZ_HUGE && !noncorporeal(mdat) && cptr.ld1so((mdat), $permonst_mlet) != NHC.S_CENTAUR && !cptr.eq((mdat), cptr.add(mons, NHC.PM_WINGED_GARGOYLE, 96)) && !cptr.eq((mdat), cptr.add(mons, NHC.PM_MARILITH, 96))))) {
        ...
        if ((otmp = (yield* which_armor(mon, 32n))) !== null) {
            if (vis) {
                if (is_whirly(cptr.ldPtro(mon, $monst_data)))
                    ...
                    (yield* pline_mon(mon, __sl61, (yield* s_suffix((yield* Monnam(mon)))), verysmall(mdat) ? __sl62 : __sl63, ppronoun));
            }
            (yield* m_lose_armor(mon, otmp, polyspot));
        }
    }
