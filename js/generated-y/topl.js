// YIELDABLE BUILD — generated by tools/c2js/yieldify.mjs from js/generated/topl.js
// Do not edit by hand, and do not score from this directory: the synchronous
// build in js/generated/ is the one the judge runs. Coloured functions here
// are generators; `yield*` marks every call that can reach a keystroke read.
import * as Y from '../yield-rt.js';
// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/win/tty/topl.c
// Input sha256: d355faebfb41d9e61e784f1ef29050a55c5ca45cd066f2f47688c6b7b7cee566
// Transpiler: tools/c2js c2js emit v1+batch

import { i16, schar, uchar } from '../cmachine.js';
import * as cptr from '../cptr.js';
import * as NHM from './nhmacro.js';
import * as FLD from './nhfield.js';
import { BASE_WINDOW, defmorestr, docorner, end_glyphout, g_putch, ttyDisplay, tty_clear_nhwindow, tty_curs, wins } from './wintty.js';
import { WIN_MESSAGE, flags, gs, gt, iflags, program_state, quitchars, svx, yn_number } from './decl.js';
import { windowprocs } from './windows.js';
import { morc, xwaitforspace } from './getline.js';
import { backsp, cl_end, home, nomux_putch, standoutbeg, standoutend, tty_nhbell } from './termcap.js';
import { alloc } from './alloc.js';
import { panic } from './end.js';
import { custompline, dumplogmsg, nhassert_failed } from './pline.js';
import { digit, eos, lowc } from './hacklib.js';
import { key2txt, readchar } from './cmd.js';
import { erase_char } from './unixtty.js';

// string literals (C char* uses decay to CPtr into these static buffers)
const __sl0 = cptr.lit("Message History");
const __sl1 = cptr.lit("");
const __sl2 = cptr.lit("\x1b ");
const __sl3 = cptr.lit("You die");
const __sl4 = cptr.lit("  ");
const __sl5 = cptr.lit("Putsym window MESSAGE nonexistent");
const __sl6 = cptr.lit("ttyDisplay->curx > 0");
const __sl7 = cptr.lit("/Users/noahpeterson/Documents/Projects/teleport-contest-research/original-contest-to-fork/nethack-c/recorder/win/tty/topl.c");
const __sl8 = cptr.lit("\b \b");
const __sl9 = cptr.lit(" [%s]");
const __sl10 = cptr.lit(" (%c)");
const __sl11 = cptr.lit(" ");
const __sl12 = cptr.lit("%s");
const __sl13 = cptr.lit("%s ");
const __sl14 = cptr.lit("#");
const __sl15 = cptr.lit("#%ld");
const __sl16 = cptr.lit("%s%s");
const __sl17 = cptr.lit("ttyDisplay == NULL || ttyDisplay->toplin != TOPLINE_NEED_MORE");

/** C ref: topl.c:20 @returns {CInt} */
export function* tty_doprev_message() {
    let cw = cptr.ldPtro(wins, WIN_MESSAGE.v, 8);
    let prevmsg_win;
    let i;
    if ((cptr.ld1so(iflags, FLD.instance_flags_prevmsg_window) != 115) && !cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_inread)) {
        if (cptr.ld1so(iflags, FLD.instance_flags_prevmsg_window) == 102) {
            prevmsg_win = (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_create_nhwindow))(NHM.NHW_MENU)));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(prevmsg_win, 0, __sl0)));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(prevmsg_win, 0, __sl1)));
            cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.ldI64o(cw, FLD.WinDesc_maxrow));
            i = Number(BigInt.asIntN(32, cptr.ldI64o(cw, FLD.WinDesc_maxcol)));
            do {
                if (cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), i, 8) && strcmp(cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), i, 8), __sl1))
                    (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(prevmsg_win, 0, cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), i, 8))));
                i = Number(BigInt.asIntN(32, (BigInt(((i + 1) | 0)) % cptr.ldI64o(cw, FLD.WinDesc_rows))));
            } while (BigInt(i) != cptr.ldI64o(cw, FLD.WinDesc_maxcol));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(prevmsg_win, 0, cptr.add(gt, FLD.instance_globals_t_toplines))));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_display_nhwindow))(prevmsg_win, 1)));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_destroy_nhwindow))(prevmsg_win)));
        } else if (cptr.ld1so(iflags, FLD.instance_flags_prevmsg_window) == 99) {
            do {
                morc.v = 0;
                if (cptr.ldI64o(cw, FLD.WinDesc_maxcol) == cptr.ldI64o(cw, FLD.WinDesc_maxrow)) {
                    cptr.st1o(ttyDisplay, FLD.DisplayDesc_dismiss_more, 16);
                    (yield* redotoplin(cptr.add(gt, FLD.instance_globals_t_toplines)));
                    (cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.ldI64o(cw, FLD.WinDesc_maxcol) + -1n)) - (-1n);
                    if (cptr.ldI64o(cw, FLD.WinDesc_maxcol) < 0n)
                        cptr.stI64o(cw, FLD.WinDesc_maxcol, BigInt.asIntN(64, cptr.ldI64o(cw, FLD.WinDesc_rows) - 1n));
                    if (!cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), cptr.ldI64o(cw, FLD.WinDesc_maxcol), 8))
                        cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.ldI64o(cw, FLD.WinDesc_maxrow));
                } else if (cptr.ldI64o(cw, FLD.WinDesc_maxcol) == (BigInt.asIntN(64, cptr.ldI64o(cw, FLD.WinDesc_maxrow) - 1n))) {
                    cptr.st1o(ttyDisplay, FLD.DisplayDesc_dismiss_more, 16);
                    (yield* redotoplin(cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), cptr.ldI64o(cw, FLD.WinDesc_maxcol), 8)));
                    (cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.ldI64o(cw, FLD.WinDesc_maxcol) + -1n)) - (-1n);
                    if (cptr.ldI64o(cw, FLD.WinDesc_maxcol) < 0n)
                        cptr.stI64o(cw, FLD.WinDesc_maxcol, BigInt.asIntN(64, cptr.ldI64o(cw, FLD.WinDesc_rows) - 1n));
                    if (!cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), cptr.ldI64o(cw, FLD.WinDesc_maxcol), 8))
                        cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.ldI64o(cw, FLD.WinDesc_maxrow));
                } else {
                    prevmsg_win = (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_create_nhwindow))(NHM.NHW_MENU)));
                    (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(prevmsg_win, 0, __sl0)));
                    (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(prevmsg_win, 0, __sl1)));
                    cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.ldI64o(cw, FLD.WinDesc_maxrow));
                    i = Number(BigInt.asIntN(32, cptr.ldI64o(cw, FLD.WinDesc_maxcol)));
                    do {
                        if (cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), i, 8) && strcmp(cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), i, 8), __sl1))
                            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(prevmsg_win, 0, cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), i, 8))));
                        i = Number(BigInt.asIntN(32, (BigInt(((i + 1) | 0)) % cptr.ldI64o(cw, FLD.WinDesc_rows))));
                    } while (BigInt(i) != cptr.ldI64o(cw, FLD.WinDesc_maxcol));
                    (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(prevmsg_win, 0, cptr.add(gt, FLD.instance_globals_t_toplines))));
                    (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_display_nhwindow))(prevmsg_win, 1)));
                    (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_destroy_nhwindow))(prevmsg_win)));
                }
            } while (morc.v == 16);
            cptr.st1o(ttyDisplay, FLD.DisplayDesc_dismiss_more, 0);
        } else {
            morc.v = 0;
            prevmsg_win = (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_create_nhwindow))(NHM.NHW_MENU)));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(prevmsg_win, 0, __sl0)));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(prevmsg_win, 0, __sl1)));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(prevmsg_win, 0, cptr.add(gt, FLD.instance_globals_t_toplines))));
            cptr.stI64o(cw, FLD.WinDesc_maxcol, BigInt.asIntN(64, cptr.ldI64o(cw, FLD.WinDesc_maxrow) - 1n));
            if (cptr.ldI64o(cw, FLD.WinDesc_maxcol) < 0n)
                cptr.stI64o(cw, FLD.WinDesc_maxcol, BigInt.asIntN(64, cptr.ldI64o(cw, FLD.WinDesc_rows) - 1n));
            do {
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(prevmsg_win, 0, cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), cptr.ldI64o(cw, FLD.WinDesc_maxcol), 8))));
                (cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.ldI64o(cw, FLD.WinDesc_maxcol) + -1n)) - (-1n);
                if (cptr.ldI64o(cw, FLD.WinDesc_maxcol) < 0n)
                    cptr.stI64o(cw, FLD.WinDesc_maxcol, BigInt.asIntN(64, cptr.ldI64o(cw, FLD.WinDesc_rows) - 1n));
                if (!cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), cptr.ldI64o(cw, FLD.WinDesc_maxcol), 8))
                    cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.ldI64o(cw, FLD.WinDesc_maxrow));
            } while (cptr.ldI64o(cw, FLD.WinDesc_maxcol) != cptr.ldI64o(cw, FLD.WinDesc_maxrow));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_display_nhwindow))(prevmsg_win, 1)));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_destroy_nhwindow))(prevmsg_win)));
            cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.ldI64o(cw, FLD.WinDesc_maxrow));
            cptr.st1o(ttyDisplay, FLD.DisplayDesc_dismiss_more, 0);
        }
    } else if (cptr.ld1so(iflags, FLD.instance_flags_prevmsg_window) == 115) {
        cptr.st1o(ttyDisplay, FLD.DisplayDesc_dismiss_more, 16);
        do {
            morc.v = 0;
            if (cptr.ldI64o(cw, FLD.WinDesc_maxcol) == cptr.ldI64o(cw, FLD.WinDesc_maxrow))
                (yield* redotoplin(cptr.add(gt, FLD.instance_globals_t_toplines)));
            else if (cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), cptr.ldI64o(cw, FLD.WinDesc_maxcol), 8))
                (yield* redotoplin(cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), cptr.ldI64o(cw, FLD.WinDesc_maxcol), 8)));
            (cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.ldI64o(cw, FLD.WinDesc_maxcol) + -1n)) - (-1n);
            if (cptr.ldI64o(cw, FLD.WinDesc_maxcol) < 0n)
                cptr.stI64o(cw, FLD.WinDesc_maxcol, BigInt.asIntN(64, cptr.ldI64o(cw, FLD.WinDesc_rows) - 1n));
            if (!cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), cptr.ldI64o(cw, FLD.WinDesc_maxcol), 8))
                cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.ldI64o(cw, FLD.WinDesc_maxrow));
        } while (morc.v == 16);
        cptr.st1o(ttyDisplay, FLD.DisplayDesc_dismiss_more, 0);
    }
    return 0;
}

/** C ref: topl.c:122 — @param {CPtr} str */
function* redotoplin(str) {
    let otoplin = cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_toplin);
    (yield* home());
    if (!cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_topl_utf8)) {
        if (cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_mixed) && (cptr.ld1s(str) & 128)) {
            g_putch(cptr.ld1s(cptr.postinc(() => str, (v) => { str = v; })));
            (cptr.stI16o(ttyDisplay, FLD.DisplayDesc_curx, cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_curx) + 1)) - (1);
        }
        end_glyphout();
    }
    (yield* putsyms(str));
    (yield* cl_end());
    cptr.stI32o(ttyDisplay, FLD.DisplayDesc_toplin, NHM.TOPLINE_NEED_MORE);
    if (cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_cury) && otoplin != NHM.TOPLINE_SPECIAL_PROMPT)
        (yield* more());
}

/** C ref: topl.c:145 — @param {CPtr} str */
export function* show_topl(str) {
    let cw = cptr.ldPtro(wins, WIN_MESSAGE.v, 8);
    if ((cptr.ldI32(cw) & 5) != NHM.WIN_STOP) {
        cptr.stI32(cw, cptr.ldI32(cw) & -6);
        if (cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_cury) && cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_toplin) == NHM.TOPLINE_NON_EMPTY)
            (yield* tty_clear_nhwindow(WIN_MESSAGE.v));
        cptr.stI64o(cw, FLD.WinDesc_curx, cptr.stI64o(cw, FLD.WinDesc_cury, 0n));
        (yield* home());
        (yield* cl_end());
        (yield* addtopl(str));
        if (cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_cury) && cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_toplin) != NHM.TOPLINE_SPECIAL_PROMPT)
            cptr.stI32o(ttyDisplay, FLD.DisplayDesc_toplin, NHM.TOPLINE_NON_EMPTY);
    }
}

/** C ref: topl.c:170 */
export function* remember_topl() {
    let cw = cptr.ldPtro(wins, WIN_MESSAGE.v, 8);
    let idx = Number(BigInt.asIntN(32, cptr.ldI64o(cw, FLD.WinDesc_maxrow)));
    let len = Number(BigInt.asUintN(32, BigInt.asUintN(64, cptr.strlen(cptr.add(gt, FLD.instance_globals_t_toplines)) + 1n)));
    if ((cptr.ldI32(cw) & NHM.WIN_LOCKHISTORY) || !cptr.ld1so(gt, FLD.instance_globals_t_toplines))
        return;
    if (len > cptr.ldI16o(cptr.ldPtro(cw, FLD.WinDesc_datlen), idx, 2)) {
        if (cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), idx, 8))
            cptr.free(cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), idx, 8));
        len = (len + ((8 - ((len & 7) >>> 0)) >>> 0)) | 0;
        cptr.stPtro(cptr.ldPtro(cw, FLD.WinDesc_data), idx, (yield* alloc(len)), 8);
        cptr.stI16o(cptr.ldPtro(cw, FLD.WinDesc_datlen), idx, i16(len), 2);
    }
    void cptr.strcpy(cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), idx, 8), cptr.add(gt, FLD.instance_globals_t_toplines));
    if (!cptr.ldI32o(program_state, FLD.sinfo_in_checkpoint)) {
        cptr.st1o(gt, FLD.instance_globals_t_toplines, 0);
        cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.stI64o(cw, FLD.WinDesc_maxrow, BigInt(((idx + 1) | 0)) % cptr.ldI64o(cw, FLD.WinDesc_rows)));
    }
}

/** C ref: topl.c:194 — @param {CPtr} s */
export function* addtopl(s) {
    let cw = cptr.ldPtro(wins, WIN_MESSAGE.v, 8);
    (yield* tty_curs(BASE_WINDOW, Number(BigInt.asIntN(32, BigInt.asIntN(64, cptr.ldI64o(cw, FLD.WinDesc_curx) + 1n))), Number(BigInt.asIntN(32, cptr.ldI64o(cw, FLD.WinDesc_cury)))));
    (yield* putsyms(s));
    (yield* cl_end());
    cptr.stI32o(ttyDisplay, FLD.DisplayDesc_toplin, NHM.TOPLINE_NEED_MORE);
}

/** C ref: topl.c:205 */
export function* more() {
    let cw = cptr.ldPtro(wins, WIN_MESSAGE.v, 8);
    if (cptr.ld1so(iflags, FLD.instance_flags_debug_fuzzer))
        return;
    if (cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_inmore))
        return;
    (cptr.stI32o(ttyDisplay, FLD.DisplayDesc_inmore, cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_inmore) + 1)) - (1);
    if (cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_toplin)) {
        (yield* tty_curs(BASE_WINDOW, Number(BigInt.asIntN(32, BigInt.asIntN(64, cptr.ldI64o(cw, FLD.WinDesc_curx) + 1n))), Number(BigInt.asIntN(32, cptr.ldI64o(cw, FLD.WinDesc_cury)))));
        if (cptr.ldI64o(cw, FLD.WinDesc_curx) >= BigInt(((cptr.ldI32o(gt, FLD.instance_globals_t_tc_gbl_data + FLD.tc_gbl_data_tc_CO) - 8) | 0)))
            (yield* topl_putsym(10));
    }
    if (cptr.ld1so(flags, FLD.flag_standout))
        standoutbeg();
    (yield* putsyms(cptr.decay(defmorestr)));
    if (cptr.ld1so(flags, FLD.flag_standout))
        standoutend();
    (yield* xwaitforspace(__sl2));
    if (morc.v == 27) {
        if (!(cptr.ldI32(cw) & NHM.WIN_NOSTOP))
            cptr.stI32(cw, cptr.ldI32(cw) | NHM.WIN_STOP);
    }
    if (cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_toplin) && cptr.ldI64o(cw, FLD.WinDesc_cury)) {
        (yield* docorner(1, Number(BigInt.asIntN(32, BigInt.asIntN(64, cptr.ldI64o(cw, FLD.WinDesc_cury) + 1n))), 0));
        cptr.stI64o(cw, FLD.WinDesc_curx, cptr.stI64o(cw, FLD.WinDesc_cury, 0n));
        (yield* home());
    } else if (morc.v == 27) {
        cptr.stI64o(cw, FLD.WinDesc_curx, cptr.stI64o(cw, FLD.WinDesc_cury, 0n));
        (yield* home());
        (yield* cl_end());
    }
    cptr.stI32o(ttyDisplay, FLD.DisplayDesc_toplin, NHM.TOPLINE_EMPTY);
    cptr.stI32o(ttyDisplay, FLD.DisplayDesc_inmore, 0);
}

/** C ref: topl.c:251 — @param {CPtr} bp */
export function* update_topl(bp) {
    let tl;
    let otl;
    let n0;
    let notdied = 1;
    let cw = cptr.ldPtro(wins, WIN_MESSAGE.v, 8);
    let skip = schar(((cptr.ldI32(cw) & 5) == NHM.WIN_STOP));
    n0 = Number(BigInt.asIntN(32, cptr.strlen(bp)));
    if ((cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_toplin) == NHM.TOPLINE_NEED_MORE || skip) && cptr.ldI64o(cw, FLD.WinDesc_cury) == 0n && ((((n0 + Number(BigInt.asIntN(32, cptr.strlen(cptr.add(gt, FLD.instance_globals_t_toplines))))) | 0) + 3) | 0) < ((cptr.ldI32o(gt, FLD.instance_globals_t_tc_gbl_data + FLD.tc_gbl_data_tc_CO) - 8) | 0) && (notdied = cptr.strncmp(bp, __sl3, 7n)) != 0) {
        void cptr.strcat(cptr.add(gt, FLD.instance_globals_t_toplines), __sl4);
        void cptr.strcat(cptr.add(gt, FLD.instance_globals_t_toplines), bp);
        cptr.stI64o(cw, FLD.WinDesc_curx, cptr.ldI64o(cw, FLD.WinDesc_curx) + 2n);
        if (!skip)
            (yield* addtopl(bp));
        return;
    } else if (!skip) {
        if (cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_toplin) == NHM.TOPLINE_NEED_MORE) {
            (yield* more());
        } else if (cptr.ldI64o(cw, FLD.WinDesc_cury)) {
            (yield* docorner(1, Number(BigInt.asIntN(32, BigInt.asIntN(64, cptr.ldI64o(cw, FLD.WinDesc_cury) + 1n))), 0));
            cptr.stI64o(cw, FLD.WinDesc_curx, cptr.stI64o(cw, FLD.WinDesc_cury, 0n));
        }
    }
    (yield* remember_topl());
    void __builtin___strncpy_chk(cptr.add(gt, FLD.instance_globals_t_toplines), bp, 300n, __builtin_object_size(cptr.add(gt, FLD.instance_globals_t_toplines), 1));
    cptr.st1o2(gt, 299, 1, FLD.instance_globals_t_toplines, 0);
    for (tl = cptr.add(gt, FLD.instance_globals_t_toplines); n0 >= cptr.ldI32o(gt, FLD.instance_globals_t_tc_gbl_data + FLD.tc_gbl_data_tc_CO); ) {
        otl = tl;
        for (tl = cptr.add(tl, (cptr.ldI32o(gt, FLD.instance_globals_t_tc_gbl_data + FLD.tc_gbl_data_tc_CO) - 1) | 0); !cptr.eq(tl, otl); tl = cptr.add(tl, -1))
            if (cptr.ld1s(tl) == 32)
                break;
        if (cptr.eq(tl, otl)) {
            tl = cptr.strchr(otl, 32);
            if (!tl)
                break;
        }
        cptr.st1(cptr.postinc(() => tl, (v) => { tl = v; }), 10);
        n0 = Number(BigInt.asIntN(32, cptr.strlen(tl)));
    }
    if (!notdied)
        cptr.stI32(cw, cptr.ldI32(cw) & -2), skip = 0;
    if (!skip)
        (yield* redotoplin(cptr.add(gt, FLD.instance_globals_t_toplines)));
}

/** C ref: topl.c:305 — @param {CInt} c */
function* topl_putsym(c) {
    let cw = cptr.ldPtro(wins, WIN_MESSAGE.v, 8);
    if (cw === null)
        (yield* panic(__sl5));
    switch (c) {
        case 8:
        if (cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_curx) == 0 && cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_cury) > 0)
            (yield* tty_curs(BASE_WINDOW, cptr.ldI32o(gt, FLD.instance_globals_t_tc_gbl_data + FLD.tc_gbl_data_tc_CO), (cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_cury) - 1) | 0));
        backsp();
        void ((!!(cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_curx) > 0)) || ((yield* nhassert_failed(__sl6, __sl7, 317)), 0) ? 1 : 0);
        (cptr.stI16o(ttyDisplay, FLD.DisplayDesc_curx, cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_curx) + -1)) - (-1);
        cptr.stI64o(cw, FLD.WinDesc_curx, BigInt(cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_curx)));
        return;
        case 10:
        (yield* cl_end());
        cptr.stI16o(ttyDisplay, FLD.DisplayDesc_curx, 0);
        (cptr.stI16o(ttyDisplay, FLD.DisplayDesc_cury, cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_cury) + 1)) - (1);
        cptr.stI64o(cw, FLD.WinDesc_cury, BigInt(cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_cury)));
        break;
        default:
        if (cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_curx) == ((cptr.ldI32o(gt, FLD.instance_globals_t_tc_gbl_data + FLD.tc_gbl_data_tc_CO) - 1) | 0))
            (yield* topl_putsym(10));
        (cptr.stI16o(ttyDisplay, FLD.DisplayDesc_curx, cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_curx) + 1)) - (1);
    }
    cptr.stI64o(cw, FLD.WinDesc_curx, BigInt(cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_curx)));
    if (cptr.ldI64o(cw, FLD.WinDesc_curx) == 0n)
        (yield* cl_end());
    void putchar(c);
    {
        let svx = cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_curx);
        (cptr.stI16o(ttyDisplay, FLD.DisplayDesc_curx, cptr.ldI16o(ttyDisplay, FLD.DisplayDesc_curx) + -1)) - (-1);
        nomux_putch(c);
        cptr.stI16o(ttyDisplay, FLD.DisplayDesc_curx, i16(svx));
    }
}

/** C ref: topl.c:355 — @param {CPtr} str */
export function* putsyms(str) {
    while (cptr.ld1s(str))
        (yield* topl_putsym(cptr.ld1s(cptr.postinc(() => str, (v) => { str = v; }))));
}

/** C ref: topl.c:362 — @param {CInt} n */
function* removetopl(n) {
    while (n-- > 0)
        (yield* putsyms(__sl8));
}

/** C ref: topl.c:373 — @param {CPtr} query @param {CPtr} resp @param {CInt} def @returns {CInt} */
export function* tty_yn_function(query, resp, def) {
    let q;
    let rtmp = new Uint8Array(40);
    let digit_ok;
    let allow_num;
    let preserve_case = 0;
    let cw = cptr.ldPtro(wins, WIN_MESSAGE.v, 8);
    let doprev = 0;
    let prompt = new Uint8Array(256);
    __lbl_clean_up: {
        yn_number.v = 0n;
        if (cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_toplin) == NHM.TOPLINE_NEED_MORE && (cptr.ldI32(cw) & 5) != NHM.WIN_STOP)
            (yield* more());
        cptr.stI32(cw, cptr.ldI32(cw) & -6);
        cptr.stI32o(ttyDisplay, FLD.DisplayDesc_toplin, NHM.TOPLINE_SPECIAL_PROMPT);
        (cptr.stI32o(ttyDisplay, FLD.DisplayDesc_inread, cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_inread) + 1)) - (1);
        if (resp) {
            let rb;
            let respbuf = new Uint8Array(128);
            allow_num = schar((cptr.strchr(resp, 35) !== null));
            void cptr.strcpy(cptr.decay(respbuf), resp);
            for (rb = cptr.decay(respbuf); cptr.ld1s(rb); rb = cptr.add(rb, 1))
                if (65 <= cptr.ld1s(rb) && cptr.ld1s(rb) <= 90) {
                    preserve_case = 1;
                    break;
                }
            if ((rb = cptr.strchr(cptr.decay(respbuf), 27)) !== null)
                cptr.st1(rb, 0);
            void __builtin___strncpy_chk(cptr.decay(prompt), query, 127n, __builtin_object_size(cptr.decay(prompt), 1));
            cptr.st1o(cptr.decay(prompt), 127, 0, 1);
            void cptr.sprintf(eos(cptr.decay(prompt)), __sl9, cptr.decay(respbuf));
            if (def)
                void cptr.sprintf(eos(cptr.decay(prompt)), __sl10, def);
            void cptr.strcat(cptr.decay(prompt), __sl11);
            (yield* custompline(6, __sl12, cptr.decay(prompt)));
        } else {
            void cptr.sprintf(cptr.decay(prompt), __sl13, query);
            (yield* custompline(6, __sl12, cptr.decay(prompt)));
            q = (yield* readchar());
            break __lbl_clean_up;
        }
        do {
            q = (yield* readchar());
            if (!preserve_case)
                q = lowc(q);
            if (q == 16) {
                if (cptr.ld1so(iflags, FLD.instance_flags_prevmsg_window) != 115) {
                    let sav = cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_inread);
                    cptr.stI32o(ttyDisplay, FLD.DisplayDesc_inread, 0);
                    void (yield* tty_doprev_message());
                    cptr.stI32o(ttyDisplay, FLD.DisplayDesc_inread, sav);
                    (yield* tty_clear_nhwindow(WIN_MESSAGE.v));
                    cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.ldI64o(cw, FLD.WinDesc_maxrow));
                    (yield* addtopl(cptr.decay(prompt)));
                } else {
                    if (!doprev)
                        void (yield* tty_doprev_message());
                    void (yield* tty_doprev_message());
                    doprev = 1;
                }
                q = 0;
                continue;
            } else if (doprev) {
                (yield* tty_clear_nhwindow(WIN_MESSAGE.v));
                cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.ldI64o(cw, FLD.WinDesc_maxrow));
                doprev = 0;
                (yield* addtopl(cptr.decay(prompt)));
                q = 0;
                continue;
            }
            digit_ok = schar((allow_num && digit(q) ? 1 : 0));
            if (q == 27) {
                if (cptr.strchr(resp, 113))
                    q = 113;
                else if (cptr.strchr(resp, 110))
                    q = 110;
                else
                    q = def;
                break;
            } else if (cptr.strchr(cptr.decay(quitchars), q)) {
                q = def;
                break;
            }
            if (!cptr.strchr(resp, q) && !digit_ok) {
                tty_nhbell();
                q = 0;
            } else if (q == 35 || digit_ok) {
                let z;
                let digit_string = new Uint8Array(2);
                let n_len = 0;
                let value = 0n;
                (yield* addtopl(__sl14)), n_len++;
                cptr.st1o(cptr.decay(digit_string), 1, 0, 1);
                if (q != 35) {
                    cptr.st1o(cptr.decay(digit_string), 0, q, 1);
                    (yield* addtopl(cptr.decay(digit_string))), n_len++;
                    value = BigInt(((q - 48) | 0));
                    q = 35;
                }
                do {
                    z = (yield* readchar());
                    if (!preserve_case)
                        z = lowc(z);
                    if (digit(z)) {
                        let dgt = BigInt(((z - 48) | 0));
                        value = (((value) < 922337203685477580n || ((value) == 922337203685477580n && (dgt) <= 7n)) ? BigInt.asIntN(64, BigInt.asIntN(64, (value) * 10n) + (dgt)) : -1n);
                        if (value < 0n)
                            break;
                        cptr.st1o(cptr.decay(digit_string), 0, z, 1);
                        (yield* addtopl(cptr.decay(digit_string))), n_len++;
                    } else if (z == 121 || cptr.strchr(cptr.decay(quitchars), z)) {
                        if (z == 27)
                            value = -1n;
                        z = 10;
                    } else if (z == erase_char || z == 8) {
                        if (n_len <= 1) {
                            value = -1n;
                            break;
                        } else {
                            value /= 10n;
                            (yield* removetopl(1)), n_len--;
                        }
                    } else {
                        value = -1n;
                        tty_nhbell();
                        break;
                    }
                } while (z != 10);
                if (value > 0n)
                    yn_number.v = value;
                else if (value == 0n)
                    q = 110;
                else {
                    (yield* removetopl(n_len)), n_len = 0;
                    q = 0;
                }
            }
        } while (!q);
    }
    if (yn_number.v)
        void cptr.sprintf(cptr.decay(rtmp), __sl15, yn_number.v);
    else
        void key2txt(uchar(q), cptr.decay(rtmp));
    void cptr.sprintf(cptr.add(gt, FLD.instance_globals_t_toplines), __sl16, cptr.decay(prompt), cptr.decay(rtmp));
    (yield* dumplogmsg(cptr.add(gt, FLD.instance_globals_t_toplines)));
    (cptr.stI32o(ttyDisplay, FLD.DisplayDesc_inread, cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_inread) + -1)) - (-1);
    cptr.stI32o(ttyDisplay, FLD.DisplayDesc_toplin, NHM.TOPLINE_NON_EMPTY);
    if (cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_intr))
        (cptr.stI32o(ttyDisplay, FLD.DisplayDesc_intr, cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_intr) + -1)) - (-1);
    if (cptr.ldI64o(cptr.ldPtro(wins, WIN_MESSAGE.v, 8), FLD.WinDesc_cury))
        (yield* tty_clear_nhwindow(WIN_MESSAGE.v));
    return q;
}

/** C ref: topl.c:562 — char ** */
let snapshot_mesgs = null;

/** C ref: topl.c:567 — @param {CInt} purge */
function* msghistory_snapshot(purge) {
    let mesg;
    let i;
    let inidx;
    let outidx;
    let cw;
    if (WIN_MESSAGE.v == -1 || !cptr.ldPtro(wins, WIN_MESSAGE.v, 8))
        return;
    cw = cptr.ldPtro(wins, WIN_MESSAGE.v, 8);
    (yield* remember_topl());
    if (!purge)
        cptr.stI32(cw, cptr.ldI32(cw) | NHM.WIN_LOCKHISTORY);
    snapshot_mesgs = (yield* alloc(Number(BigInt.asUintN(32, BigInt.asUintN(64, BigInt.asUintN(64, (BigInt.asIntN(64, cptr.ldI64o(cw, FLD.WinDesc_rows) + 1n))) * 8n)))));
    outidx = 0;
    inidx = Number(BigInt.asIntN(32, cptr.ldI64o(cw, FLD.WinDesc_maxrow)));
    for (i = 0; BigInt(i) < cptr.ldI64o(cw, FLD.WinDesc_rows); ++i) {
        cptr.stPtro(snapshot_mesgs, i, null, 8);
        mesg = cptr.ldPtro(cptr.ldPtro(cw, FLD.WinDesc_data), inidx, 8);
        if (mesg && cptr.ld1s(mesg)) {
            cptr.stPtro(snapshot_mesgs, outidx++, mesg, 8);
            if (purge) {
                cptr.stPtro(cptr.ldPtro(cw, FLD.WinDesc_data), inidx, null, 8);
                cptr.stI16o(cptr.ldPtro(cw, FLD.WinDesc_datlen), inidx, 0, 2);
            }
        }
        inidx = Number(BigInt.asIntN(32, (BigInt(((inidx + 1) | 0)) % cptr.ldI64o(cw, FLD.WinDesc_rows))));
    }
    cptr.stPtro(snapshot_mesgs, cptr.ldI64o(cw, FLD.WinDesc_rows), null, 8);
    if (purge)
        cptr.stI64o(cw, FLD.WinDesc_maxcol, cptr.stI64o(cw, FLD.WinDesc_maxrow, 0n));
}

/** C ref: topl.c:613 — @param {CInt} purged */
function free_msghistory_snapshot(purged) {
    if (snapshot_mesgs) {
        if (purged) {
            let i;
            for (i = 0; cptr.ldPtro(snapshot_mesgs, i, 8); ++i)
                cptr.free(cptr.ldPtro(snapshot_mesgs, i, 8));
        }
        cptr.free(snapshot_mesgs), snapshot_mesgs = null;
        if (!purged)
            cptr.stI32(cptr.ldPtro(wins, WIN_MESSAGE.v, 8), cptr.ldI32(cptr.ldPtro(wins, WIN_MESSAGE.v, 8)) & -3);
    }
}

let __static_tty_getmsghistory_nxtidx = 0; /** C ref: topl.c:647 — int (function-static) */

/** C ref: topl.c:645 — @param {CInt} init @returns {CPtr} */
export function* tty_getmsghistory(init) {
    let nextmesg;
    let result = null;
    if (init) {
        (yield* msghistory_snapshot(0));
        __static_tty_getmsghistory_nxtidx = 0;
    }
    if (snapshot_mesgs) {
        nextmesg = cptr.ldPtro(snapshot_mesgs, __static_tty_getmsghistory_nxtidx++, 8);
        if (nextmesg) {
            result = nextmesg;
        } else {
            free_msghistory_snapshot(0);
        }
    }
    return result;
}

let __static_tty_putmsghistory_initd = 0; /** C ref: topl.c:687 — signed char (function-static) */

/** C ref: topl.c:685 — @param {CPtr} msg @param {CInt} restoring_msghist */
export function* tty_putmsghistory(msg, restoring_msghist) {
    let idx;
    if (restoring_msghist && !__static_tty_putmsghistory_initd) {
        (yield* msghistory_snapshot(1));
        __static_tty_putmsghistory_initd = 1;
        cptr.stI32o(gs, FLD.instance_globals_s_saved_pline_index, 0);
    }
    if (msg) {
        if (ttyDisplay && cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_toplin) == NHM.TOPLINE_NEED_MORE)
            cptr.stI32o(ttyDisplay, FLD.DisplayDesc_toplin, NHM.TOPLINE_NON_EMPTY);
        (yield* remember_topl());
        void cptr.strcpy(cptr.add(gt, FLD.instance_globals_t_toplines), msg);
        (yield* dumplogmsg(cptr.add(gt, FLD.instance_globals_t_toplines)));
    } else if (snapshot_mesgs) {
        void ((!!(cptr.eq(ttyDisplay, (null)) || cptr.ldI32o(ttyDisplay, FLD.DisplayDesc_toplin) != NHM.TOPLINE_NEED_MORE)) || ((yield* nhassert_failed(__sl17, __sl7, 720)), 0) ? 1 : 0);
        for (idx = 0; cptr.ldPtro(snapshot_mesgs, idx, 8); ++idx) {
            (yield* remember_topl());
            void cptr.strcpy(cptr.add(gt, FLD.instance_globals_t_toplines), cptr.ldPtro(snapshot_mesgs, idx, 8));
            (yield* dumplogmsg(cptr.add(gt, FLD.instance_globals_t_toplines)));
        }
        free_msghistory_snapshot(1);
        __static_tty_putmsghistory_initd = 0;
    }
}

// --- BEGIN c2js reset block (tools/c2js/resetify.mjs) — do not edit ---
// 3 bindings: 0 rebound+refilled, 3 rebound, 0 refilled.
// S/P are supplied by js/generated-y/__reset.js so this module needs no new import.
let __c2js_rs = null;
export function __captureState(S) { __c2js_rs = [S(snapshot_mesgs), S(__static_tty_getmsghistory_nxtidx), S(__static_tty_putmsghistory_initd)]; }
export function __resetState(P) {
    const r = __c2js_rs;
    if (r === null) throw new Error("topl.js: __resetState before __captureState");
    snapshot_mesgs = P(r[0]);
    __static_tty_getmsghistory_nxtidx = P(r[1]);
    __static_tty_putmsghistory_initd = P(r[2]);
}
// --- END c2js reset block ---
