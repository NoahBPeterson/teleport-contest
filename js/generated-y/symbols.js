...
export function* match_sym(buf) {
    ...
    while (cptr.ldI32(sp)) {
        if ((len >= cptr.strlen(cptr.ldPtro(sp, 8))) && !(yield* strncmpi(buf, cptr.ldPtro(sp, 8), Number(BigInt.asIntN(32, len)))))
            return sp;
        sp = cptr.add(sp, 1, 16);
    }
    for (i = 0; i < 10; ++i) {
    ...
    if (res && cptr.ldPtro(gs, 952)) {
        ...
        for (sl = cptr.ldPtro(gs, 952); sl; sl = cptr.ldPtr(sl)) {
            ...
            if (cptr.ldPtro(sl, 8)) {
                ...
                if (symset_name && !(yield* strncmpi((cptr.ldPtro(sl, 8)), (symset_name), -1)))
                    defindx = cptr.ldI32(any);
