        ...
        if (!Protection_from_shape_changers() && !(rng_log_enabled() ? (rng_log_set_caller(__sl0, 17, __sl1), rn2((yield* night()) ? (cptr.ldI32o(flags, FLD.flag_moonphase) == NHM.FULL_MOON ? 3 : 30) : (cptr.ldI32o(flags, FLD.flag_moonphase) == NHM.FULL_MOON ? 10 : 50))) : rn2((yield* night()) ? (cptr.ldI32o(flags, FLD.flag_moonphase) == NHM.FULL_MOON ? 3 : 30) : (cptr.ldI32o(flags, FLD.flag_moonphase) == NHM.FULL_MOON ? 10 : 50)))) {
            ...
            if (!Deaf() && !canseemon(mon)) {
                ...
                if (howler) {
                    ...
                    (yield* wake_nearto(cptr.ldI16o(mon, FLD.monst_mx), cptr.ldI16o(mon, FLD.monst_my), 16));
                }
            }
        }
...
export function* new_were(mon) {
    ...
    (yield* healmon(mon, (((cptr.ldI32o(mon, FLD.monst_mhpmax) - cptr.ldI32o(mon, FLD.monst_mhp)) | 0) / 4) | 0, 0));
    ...
    if (cptr.ld1so(svc, FLD.context_info_mon_moving) && !(cptr.ldI32o(mon, FLD.monst_mpeaceful) & 1) && (yield* onscary(cptr.ldI16o(mon, FLD.monst_mux), cptr.ldI16o(mon, FLD.monst_muy), mon)) && monnear(mon, cptr.ldI16o(mon, FLD.monst_mux), cptr.ldI16o(mon, FLD.monst_muy)))
...
export function* were_summon(ptr, yours, visible, genbuf) {
    ...
    for (i = (rng_log_enabled() ? (rng_log_set_caller(__sl0, 155, __sl9), rnd(5)) : rnd(5)); i > 0; i--) {
        ...
        mtmp = (yield* makemon(cptr.add(mons, typ, 96), cptr.ldI16(u), cptr.ldI16o(u, FLD.you_uy), NHM.NO_MM_FLAGS));
        if (mtmp) {
            total++;
            if (canseemon(mtmp))
...
export function* you_were() {
    ...
    if (controllable_poly) {
        ...
        if (!(yield* paranoid_query(schar((((cptr.ldI32o(flags, FLD.flag_paranoia_bits) & NHM.PARANOID_WERECHANGE) >>> 0) != 0)), cptr.decay(qbuf))))
            return;
    } else if ((yield* monster_nearby())) {
        return;
    }
    (cptr.stI64o(gw, FLD.instance_globals_w_were_changes, cptr.ldI64o(gw, FLD.instance_globals_w_were_changes) + 1n)) - (1n);
    void (yield* polymon(cptr.ldI32o(u, FLD.you_ulycn)));
}
