// YIELDABLE BUILD — generated by tools/c2js/yieldify.mjs from js/generated/strutil.js
// Do not edit by hand, and do not score from this directory: the synchronous
// build in js/generated/ is the one the judge runs. Coloured functions here
// are generators; `yield*` marks every call that can reach a keystroke read.
// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/src/strutil.c
// Input sha256: 7a9d406919c3711b8ec3ee9fb8618b2af8f546b6d6d2a192a5962a2de313ce1e
// Transpiler: tools/c2js c2js emit v1+batch

import { schar } from '../cmachine.js';
import * as cptr from '../cptr.js';
import * as FLD from './nhfield.js';
import { alloc } from './alloc.js';
import { panic } from './end.js';
import { lowc } from './hacklib.js';

// string literals (C char* uses decay to CPtr into these static buffers)
const __sl0 = cptr.lit("%s:%d string too long");

/** C ref: strutil.c:9 — @param {CPtr} strbuf */
export function strbuf_init(strbuf) {
    cptr.stPtro(strbuf, FLD.strbuf_t_str, null);
    cptr.stI32(strbuf, 0);
}

/** C ref: strutil.c:17 — @param {CPtr} strbuf @param {CPtr} str */
export function* strbuf_append(strbuf, str) {
    let len = (Number(BigInt.asIntN(32, cptr.strlen(str))) + 1) | 0;
    (yield* strbuf_reserve(strbuf, (len + (cptr.ldPtro(strbuf, FLD.strbuf_t_str) ? Number(BigInt.asIntN(32, cptr.strlen(cptr.ldPtro(strbuf, FLD.strbuf_t_str)))) : 0)) | 0));
    void cptr.strcat(cptr.ldPtro(strbuf, FLD.strbuf_t_str), str);
}

/** C ref: strutil.c:28 — @param {CPtr} strbuf @param {CInt} len */
export function* strbuf_reserve(strbuf, len) {
    if (cptr.eq(cptr.ldPtro(strbuf, FLD.strbuf_t_str), (null))) {
        cptr.stPtro(strbuf, FLD.strbuf_t_str, cptr.add(strbuf, FLD.strbuf_t_buf));
        cptr.st1o(cptr.ldPtro(strbuf, FLD.strbuf_t_str), 0, 0);
        cptr.stI32(strbuf, 256);
    }
    if (len > cptr.ldI32(strbuf)) {
        let oldbuf = cptr.ldPtro(strbuf, FLD.strbuf_t_str);
        cptr.stI32(strbuf, (len + 256) | 0);
        cptr.stPtro(strbuf, FLD.strbuf_t_str, (yield* alloc(cptr.ldI32(strbuf) >>> 0)));
        void cptr.strcpy(cptr.ldPtro(strbuf, FLD.strbuf_t_str), oldbuf);
        if (!cptr.eq(oldbuf, cptr.add(strbuf, FLD.strbuf_t_buf)))
            cptr.free(oldbuf);
    }
}

/** C ref: strutil.c:49 — @param {CPtr} strbuf */
export function strbuf_empty(strbuf) {
    if (!cptr.eq(cptr.ldPtro(strbuf, FLD.strbuf_t_str), (null)) && !cptr.eq(cptr.ldPtro(strbuf, FLD.strbuf_t_str), cptr.add(strbuf, FLD.strbuf_t_buf)))
        cptr.free(cptr.ldPtro(strbuf, FLD.strbuf_t_str));
    strbuf_init(strbuf);
}

/** C ref: strutil.c:58 — @param {CPtr} strbuf */
export function* strbuf_nl_to_crlf(strbuf) {
    if (cptr.ldPtro(strbuf, FLD.strbuf_t_str)) {
        let len = Number(BigInt.asIntN(32, cptr.strlen(cptr.ldPtro(strbuf, FLD.strbuf_t_str))));
        let count = 0;
        let cp = cptr.ldPtro(strbuf, FLD.strbuf_t_str);
        while (cptr.ld1s(cp))
            if (cptr.ld1s(cptr.postinc(() => cp, (v) => { cp = v; })) == 10)
                count++;
        if (count) {
            (yield* strbuf_reserve(strbuf, (((len + count) | 0) + 1) | 0));
            for (cp = cptr.add(cptr.add(cptr.ldPtro(strbuf, FLD.strbuf_t_str), len), count); count; cp = cptr.add(cp, -1))
                if ((cptr.st1(cp, cptr.ld1so(cp, -count))) == 10) {
                    cptr.st1(cptr.predec(() => cp, (v) => { cp = v; }), 13);
                    --count;
                }
        }
    }
}

/** C ref: strutil.c:82 — @param {CPtr} str @param {CPtr} file @param {CInt} line @returns {CUInt} */
export function* Strlen_(str, file, line) {
    let p;
    let len;
    for (p = str, len = 0n; len < 32767n; ++len)
        if (cptr.ld1s(cptr.postinc(() => p, (v) => { p = v; })) == 0)
            break;
    if (len == 32767n)
        (yield* panic(__sl0, file, line));
    return Number(BigInt.asUintN(32, len));
}

/** C ref: strutil.c:105 — @param {CPtr} patrn @param {CPtr} strng @param {CInt} ci @param {CPtr} sk @returns {CInt} */
function* pmatch_internal(patrn, strng, ci, sk) {
    let s;
    let p;
    __lbl_pmatch_top: while (true) {
        if (!sk) {
            s = cptr.ld1s(cptr.postinc(() => strng, (v) => { strng = v; }));
            p = cptr.ld1s(cptr.postinc(() => patrn, (v) => { patrn = v; }));
        } else {
            do {
                s = cptr.ld1s(cptr.postinc(() => strng, (v) => { strng = v; }));
            } while (cptr.strchr(sk, s));
            do {
                p = cptr.ld1s(cptr.postinc(() => patrn, (v) => { patrn = v; }));
            } while (cptr.strchr(sk, p));
        }
        if (!p)
            return schar((s == 0));
        else if (p == 42)
            return schar(((!cptr.ld1s(patrn) || (yield* pmatch_internal(patrn, cptr.add(strng, -(1)), ci, sk))) ? 1 : (s ? (yield* pmatch_internal(cptr.add(patrn, -(1)), strng, ci, sk)) : 0)));
        else if ((ci ? lowc(p) != lowc(s) : p != s) && (p != 63 || !s))
            return 0;
        else
            continue __lbl_pmatch_top;
        break __lbl_pmatch_top;
    }
}

/** C ref: strutil.c:145 — @param {CPtr} patrn @param {CPtr} strng @returns {CInt} */
export function* pmatch(patrn, strng) {
    return (yield* pmatch_internal(patrn, strng, 0, null));
}

/** C ref: strutil.c:152 — @param {CPtr} patrn @param {CPtr} strng @returns {CInt} */
export function* pmatchi(patrn, strng) {
    return (yield* pmatch_internal(patrn, strng, 1, null));
}
