// YIELDABLE BUILD — generated by tools/c2js/yieldify.mjs from js/generated/ldebug.js
// Do not edit by hand, and do not score from this directory: the synchronous
// build in js/generated/ is the one the judge runs. Coloured functions here
// are generators; `yield*` marks every call that can reach a keystroke read.
// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/lib/lua-5.4.8/src/ldebug.c
// Input sha256: 06d065d1f26c29546cc74b61a50267cb70567ae538691f190bf15ca5cd29d308
// Transpiler: tools/c2js c2js emit v1+batch

import { schar, u32div, uchar } from '../cmachine.js';
import * as cptr from '../cptr.js';
import * as NHC from './nhconst.js';
import * as FLD from './nhfield.js';
import { luaF_getlocalname } from './lfunc.js';
import { luaO_chunkid, luaO_pushfstring, luaO_pushvfstring } from './lobject.js';
import { luaH_new, luaH_setint } from './ltable.js';
import { luaP_opmodes } from './lopcodes.js';
import { luaT_objtypename } from './ltm.js';
import { luaV_tointegerns } from './lvm.js';
import { luaD_callnoyield, luaD_hook, luaD_hookcall, luaD_throw } from './ldo.js';
import { luaC_step } from './lgc.js';

// string literals (C char* uses decay to CPtr into these static buffers)
const __sl0 = cptr.lit("?");
const __sl1 = cptr.lit("(vararg)");
const __sl2 = cptr.lit("(temporary)");
const __sl3 = cptr.lit("(C temporary)");
const __sl4 = cptr.lit("=[C]");
const __sl5 = cptr.lit("C");
const __sl6 = cptr.lit("=?");
const __sl7 = cptr.lit("main");
const __sl8 = cptr.lit("Lua");
const __sl9 = cptr.lit("");
const __sl10 = cptr.lit("constant");
const __sl11 = cptr.lit("_ENV");
const __sl12 = cptr.lit("global");
const __sl13 = cptr.lit("field");
const __sl14 = cptr.lit("integer index");
const __sl15 = cptr.lit("method");
const __sl16 = cptr.lit("for iterator");
const __sl17 = cptr.lit("metamethod");
const __sl18 = cptr.lit("hook");
const __sl19 = cptr.lit("__gc");
const __sl20 = cptr.lit(" (%s '%s')");
const __sl21 = cptr.lit("attempt to %s a %s value%s");
const __sl22 = cptr.lit("call");
const __sl23 = cptr.lit("bad 'for' %s (number expected, got %s)");
const __sl24 = cptr.lit("concatenate");
const __sl25 = cptr.lit("number%s has no integer representation");
const __sl26 = cptr.lit("attempt to compare two %s values");
const __sl27 = cptr.lit("attempt to compare %s with %s");
const __sl28 = cptr.lit("%s:%d: %s");

/** C ref: ldebug.c:40 — char[6] */
const strlocal = cptr.bytes("local");

/** C ref: ldebug.c:41 — char[8] */
const strupval = cptr.bytes("upvalue");

/** C ref: ldebug.c:44 — @param {CPtr} ci @returns {CInt} */
function currentpc(ci) {
    (void 0);
    return (((Number(BigInt.asIntN(32, ((cptr.diff((cptr.ldPtro(ci, FLD.CallInfo_u)), cptr.ldPtro((cptr.ldPtro((((((((cptr.ldPtr(((((cptr.ldPtr((ci)))))))))))))), FLD.LClosure_p)), FLD.Proto_code)) / 4n))))) - 1) | 0);
}

/** C ref: ldebug.c:63 — @param {CPtr} f @param {CInt} pc @param {CPtr} basepc @returns {CInt} */
function getbaseline(f, pc, basepc) {
    if (cptr.ldI32o(f, FLD.Proto_sizeabslineinfo) == 0 || pc < cptr.ldI32o(cptr.ldPtro(f, FLD.Proto_abslineinfo), 0, 8)) {
        cptr.stI32(basepc, -1);
        return cptr.ldI32o(f, FLD.Proto_linedefined);
    } else {
        let i = ((u32div((((pc)) >>> 0), 128) - 1) >>> 0) | 0;
        (void 0);
        while (((i + 1) | 0) < cptr.ldI32o(f, FLD.Proto_sizeabslineinfo) && pc >= cptr.ldI32o(cptr.ldPtro(f, FLD.Proto_abslineinfo), (i + 1) | 0, 8))
            i++;
        cptr.stI32(basepc, cptr.ldI32o(cptr.ldPtro(f, FLD.Proto_abslineinfo), i, 8));
        return cptr.ldI32o2(cptr.ldPtro(f, FLD.Proto_abslineinfo), i, 8, FLD.AbsLineInfo_line);
    }
}

/** C ref: ldebug.c:86 — @param {CPtr} f @param {CInt} pc @returns {CInt} */
export function luaG_getfuncline(f, pc) {
    if (cptr.eq(cptr.ldPtro(f, FLD.Proto_lineinfo), (null)))
        return -1;
    else {
        let basepc = cptr.box(0);
        let baseline = getbaseline(f, pc, basepc);
        while (basepc.v++ < pc) {
            (void 0);
            baseline = (baseline + cptr.ld1uo(cptr.ldPtro(f, FLD.Proto_lineinfo), basepc.v)) | 0;
        }
        return baseline;
    }
}

/** C ref: ldebug.c:101 — @param {CPtr} ci @returns {CInt} */
function getcurrentline(ci) {
    return luaG_getfuncline(cptr.ldPtro((((((((cptr.ldPtr(((((cptr.ldPtr((ci)))))))))))))), FLD.LClosure_p), currentpc(ci));
}

/** C ref: ldebug.c:117 — @param {CPtr} ci */
function settraps(ci) {
    for (; !cptr.eq(ci, (null)); ci = cptr.ldPtro(ci, FLD.CallInfo_previous))
        if ((!(cptr.ldU16o((ci), FLD.CallInfo_callstatus) & 2)))
            cptr.stI32o(ci, FLD.CallInfo_u + 8, 1);
}

/** C ref: ldebug.c:134 — @param {CPtr} L @param {CPtr} func @param {CInt} mask @param {CInt} count */
export function lua_sethook(L, func, mask, count) {
    if (func === (null) || mask == 0) {
        mask = 0;
        func = null;
    }
    cptr.stPtro(L, FLD.lua_State_hook, func);
    cptr.stI32o(L, FLD.lua_State_basehookcount, count);
    (cptr.stI32o(L, FLD.lua_State_hookcount, cptr.ldI32o(L, FLD.lua_State_basehookcount)));
    cptr.stI32o(L, FLD.lua_State_hookmask, (uchar(((mask)))));
    if (mask)
        settraps(cptr.ldPtro(L, FLD.lua_State_ci));
}

/** C ref: ldebug.c:148 — @param {CPtr} L @returns {*} */
export function lua_gethook(L) {
    return cptr.ldPtro(L, FLD.lua_State_hook);
}

/** C ref: ldebug.c:153 — @param {CPtr} L @returns {CInt} */
export function lua_gethookmask(L) {
    return cptr.ldI32o(L, FLD.lua_State_hookmask);
}

/** C ref: ldebug.c:158 — @param {CPtr} L @returns {CInt} */
export function lua_gethookcount(L) {
    return cptr.ldI32o(L, FLD.lua_State_basehookcount);
}

/** C ref: ldebug.c:163 — @param {CPtr} L @param {CInt} level @param {CPtr} ar @returns {CInt} */
export function lua_getstack(L, level, ar) {
    let status;
    let ci;
    if (level < 0)
        return 0;
    (void 0);
    for (ci = cptr.ldPtro(L, FLD.lua_State_ci); level > 0 && !cptr.eq(ci, cptr.add(L, FLD.lua_State_base_ci)); ci = cptr.ldPtro(ci, FLD.CallInfo_previous))
        level--;
    if (level == 0 && !cptr.eq(ci, cptr.add(L, FLD.lua_State_base_ci))) {
        status = 1;
        cptr.stPtro(ar, FLD.lua_Debug_i_ci, ci);
    } else
        status = 0;
    (void 0);
    return status;
}

/** C ref: ldebug.c:180 — @param {CPtr} p @param {CInt} uv @returns {CPtr} */
function upvalname(p, uv) {
    let s = (cptr.ldPtro(cptr.ldPtro(p, FLD.Proto_upvalues), uv, 16));
    if (cptr.eq(s, (null)))
        return __sl0;
    else
        return (cptr.add((s), FLD.TString_contents));
}

/** C ref: ldebug.c:187 — @param {CPtr} ci @param {CInt} n @param {CPtr} pos @returns {CPtr} */
function findvararg(ci, n, pos) {
    if (cptr.ld1uo(cptr.ldPtro(((((((cptr.ldPtr(((((cptr.ldPtr(ci)))))))))))), FLD.LClosure_p), FLD.Proto_is_vararg)) {
        let nextra = cptr.ldI32o(ci, FLD.CallInfo_u + 12);
        if (n >= -nextra) {
            cptr.stPtr(pos, cptr.add(cptr.add(cptr.ldPtr(ci), -(nextra), 16), -(((n + 1) | 0)), 16));
            return __sl1;
        }
    }
    return null;
}

/** C ref: ldebug.c:199 — @param {CPtr} L @param {CPtr} ci @param {CInt} n @param {CPtr} pos @returns {CPtr} */
export function luaG_findlocal(L, ci, n, pos) {
    let base = cptr.add(cptr.ldPtr(ci), 1, 16);
    let name = null;
    if ((!(cptr.ldU16o((ci), FLD.CallInfo_callstatus) & 2))) {
        if (n < 0)
            return findvararg(ci, n, pos);
        else
            name = luaF_getlocalname(cptr.ldPtro((((((((cptr.ldPtr(((((cptr.ldPtr((ci)))))))))))))), FLD.LClosure_p), n, currentpc(ci));
    }
    if (cptr.eq(name, (null))) {
        let limit = (cptr.eq(ci, cptr.ldPtro(L, FLD.lua_State_ci))) ? cptr.ldPtro(L, FLD.lua_State_top) : cptr.ldPtr(cptr.ldPtro(ci, FLD.CallInfo_next));
        if (cptr.diff(limit, base) / 16n >= BigInt(n) && n > 0) {
            name = (!(cptr.ldU16o((ci), FLD.CallInfo_callstatus) & 2)) ? __sl2 : __sl3;
        } else
            return null;
    }
    if (pos)
        cptr.stPtr(pos, cptr.add(base, ((n - 1) | 0), 16));
    return name;
}

/** C ref: ldebug.c:223 — @param {CPtr} L @param {CPtr} ar @param {CInt} n @returns {CPtr} */
export function* lua_getlocal(L, ar, n) {
    let name;
    (void 0);
    if (cptr.eq(ar, (null))) {
        if (!((cptr.ld1uo(((((cptr.add(cptr.ldPtro(L, FLD.lua_State_top), -(1), 16))))), FLD.TValue_tt_)) == 70))
            name = null;
        else
            name = luaF_getlocalname(cptr.ldPtro(((((((cptr.ldPtr(((((cptr.add(cptr.ldPtro(L, FLD.lua_State_top), -(1), 16)))))))))))), FLD.LClosure_p), n, 0);
    } else {
        let pos = cptr.box(null);
        name = luaG_findlocal(L, cptr.ldPtro(ar, FLD.lua_Debug_i_ci), n, pos);
        if (name) {
            {
                let io1 = (((cptr.ldPtro(L, FLD.lua_State_top))));
                let io2 = (((pos.v)));
                cptr.memcpy(io1, io2, 8);
                (cptr.st1o((io1), FLD.TValue_tt_, (cptr.ld1uo(io2, FLD.TValue_tt_))));
                (void L, (void 0));
                (void 0);
            }
            ;
            {
                cptr.postinc(() => cptr.ldPtro(L, FLD.lua_State_top), (v) => { cptr.stPtro(L, FLD.lua_State_top, v); }, 16);
                (void L, (void 0));
            }
            ;
        }
    }
    (void 0);
    return name;
}

/** C ref: ldebug.c:245 — @param {CPtr} L @param {CPtr} ar @param {CInt} n @returns {CPtr} */
export function* lua_setlocal(L, ar, n) {
    let pos = cptr.box(null);
    let name;
    (void 0);
    name = luaG_findlocal(L, cptr.ldPtro(ar, FLD.lua_Debug_i_ci), n, pos);
    if (name) {
        {
            let io1 = (((pos.v)));
            let io2 = (((cptr.add(cptr.ldPtro(L, FLD.lua_State_top), -(1), 16))));
            cptr.memcpy(io1, io2, 8);
            (cptr.st1o((io1), FLD.TValue_tt_, (cptr.ld1uo(io2, FLD.TValue_tt_))));
            (void L, (void 0));
            (void 0);
        }
        ;
        cptr.postdec(() => cptr.ldPtro(L, FLD.lua_State_top), (v) => { cptr.stPtro(L, FLD.lua_State_top, v); }, 16);
    }
    (void 0);
    return name;
}

/** C ref: ldebug.c:259 — @param {CPtr} ar @param {CPtr} cl */
function funcinfo(ar, cl) {
    if (!(!cptr.eq((cl), (null)) && cptr.ld1uo((cl), FLD.CClosure_tt) == 6)) {
        cptr.stPtro(ar, FLD.lua_Debug_source, __sl4);
        cptr.stU64o(ar, FLD.lua_Debug_srclen, (BigInt.asUintN(64, 5n / 1n - 1n)));
        cptr.stI32o(ar, FLD.lua_Debug_linedefined, -1);
        cptr.stI32o(ar, FLD.lua_Debug_lastlinedefined, -1);
        cptr.stPtro(ar, FLD.lua_Debug_what, __sl5);
    } else {
        let p = cptr.ldPtro(cl, FLD.LClosure_p);
        if (cptr.ldPtro(p, FLD.Proto_source)) {
            cptr.stPtro(ar, FLD.lua_Debug_source, (cptr.add((cptr.ldPtro(p, FLD.Proto_source)), FLD.TString_contents)));
            cptr.stU64o(ar, FLD.lua_Debug_srclen, (cptr.ld1uo((cptr.ldPtro(p, FLD.Proto_source)), FLD.TString_shrlen) != 255 ? BigInt(cptr.ld1uo((cptr.ldPtro(p, FLD.Proto_source)), FLD.TString_shrlen) >>> 0) : cptr.ldU64o((cptr.ldPtro(p, FLD.Proto_source)), FLD.TString_u)));
        } else {
            cptr.stPtro(ar, FLD.lua_Debug_source, __sl6);
            cptr.stU64o(ar, FLD.lua_Debug_srclen, (BigInt.asUintN(64, 3n / 1n - 1n)));
        }
        cptr.stI32o(ar, FLD.lua_Debug_linedefined, cptr.ldI32o(p, FLD.Proto_linedefined));
        cptr.stI32o(ar, FLD.lua_Debug_lastlinedefined, cptr.ldI32o(p, FLD.Proto_lastlinedefined));
        cptr.stPtro(ar, FLD.lua_Debug_what, (cptr.ldI32o(ar, FLD.lua_Debug_linedefined) == 0) ? __sl7 : __sl8);
    }
    luaO_chunkid(cptr.add(ar, FLD.lua_Debug_short_src), cptr.ldPtro(ar, FLD.lua_Debug_source), cptr.ldU64o(ar, FLD.lua_Debug_srclen));
}

/** C ref: ldebug.c:285 — @param {CPtr} p @param {CInt} currentline @param {CInt} pc @returns {CInt} */
function nextline(p, currentline, pc) {
    if (cptr.ld1uo(cptr.ldPtro(p, FLD.Proto_lineinfo), pc) != -128)
        return (currentline + cptr.ld1uo(cptr.ldPtro(p, FLD.Proto_lineinfo), pc)) | 0;
    else
        return luaG_getfuncline(p, pc);
}

/** C ref: ldebug.c:293 — @param {CPtr} L @param {CPtr} f */
function* collectvalidlines(L, f) {
    if (!(!cptr.eq((f), (null)) && cptr.ld1uo((f), FLD.CClosure_tt) == 6)) {
        (cptr.st1o((((cptr.ldPtro(L, FLD.lua_State_top)))), FLD.TValue_tt_, 0));
        {
            cptr.postinc(() => cptr.ldPtro(L, FLD.lua_State_top), (v) => { cptr.stPtro(L, FLD.lua_State_top, v); }, 16);
            (void L, (void 0));
        }
        ;
    } else {
        let p = cptr.ldPtro(f, FLD.LClosure_p);
        let currentline = cptr.ldI32o(p, FLD.Proto_linedefined);
        let t = (yield* luaH_new(L));
        {
            let io = (((cptr.ldPtro(L, FLD.lua_State_top))));
            let x_ = (t);
            cptr.stPtr(((io)), ((((x_)))));
            (cptr.st1o((io), FLD.TValue_tt_, 69));
            (void L, (void 0));
        }
        ;
        {
            cptr.postinc(() => cptr.ldPtro(L, FLD.lua_State_top), (v) => { cptr.stPtro(L, FLD.lua_State_top, v); }, 16);
            (void L, (void 0));
        }
        ;
        if (!cptr.eq(cptr.ldPtro(p, FLD.Proto_lineinfo), (null))) {
            let i;
            let v = cptr.alloc(16);
            (cptr.st1o((v), FLD.TValue_tt_, 17));
            if (!cptr.ld1uo(p, FLD.Proto_is_vararg))
                i = 0;
            else {
                (void 0);
                currentline = nextline(p, currentline, 0);
                i = 1;
            }
            for (; i < cptr.ldI32o(p, FLD.Proto_sizelineinfo); i++) {
                currentline = nextline(p, currentline, i);
                (yield* luaH_setint(L, t, BigInt(currentline), v));
            }
        }
    }
}

/** C ref: ldebug.c:324 — @param {CPtr} L @param {CPtr} ci @param {CPtr} name @returns {CPtr} */
function getfuncname(L, ci, name) {
    if (!cptr.eq(ci, (null)) && !(cptr.ldU16o(ci, FLD.CallInfo_callstatus) & 32))
        return funcnamefromcall(L, cptr.ldPtro(ci, FLD.CallInfo_previous), name);
    else
        return null;
}

/** C ref: ldebug.c:332 — @param {CPtr} L @param {CPtr} what @param {CPtr} ar @param {CPtr} f @param {CPtr} ci @returns {CInt} */
function auxgetinfo(L, what, ar, f, ci) {
    let status = 1;
    for (; cptr.ld1s(what); what = cptr.add(what, 1)) {
        switch (cptr.ld1s(what)) {
            case 83:
            {
                funcinfo(ar, f);
                break;
            }
            case 108:
            {
                cptr.stI32o(ar, FLD.lua_Debug_currentline, (ci && (!(cptr.ldU16o((ci), FLD.CallInfo_callstatus) & 2))) ? getcurrentline(ci) : -1);
                break;
            }
            case 117:
            {
                cptr.st1o(ar, FLD.lua_Debug_nups, uchar(((cptr.eq(f, (null))) ? 0 : cptr.ld1uo(f, FLD.CClosure_nupvalues))));
                if (!(!cptr.eq((f), (null)) && cptr.ld1uo((f), FLD.CClosure_tt) == 6)) {
                    cptr.st1o(ar, FLD.lua_Debug_isvararg, 1);
                    cptr.st1o(ar, FLD.lua_Debug_nparams, 0);
                } else {
                    cptr.st1o(ar, FLD.lua_Debug_isvararg, schar(cptr.ld1uo(cptr.ldPtro(f, FLD.LClosure_p), FLD.Proto_is_vararg)));
                    cptr.st1o(ar, FLD.lua_Debug_nparams, cptr.ld1uo(cptr.ldPtro(f, FLD.LClosure_p), FLD.Proto_numparams));
                }
                break;
            }
            case 116:
            {
                cptr.st1o(ar, FLD.lua_Debug_istailcall, schar(((ci) ? cptr.ldU16o(ci, FLD.CallInfo_callstatus) & 32 : 0)));
                break;
            }
            case 110:
            {
                cptr.stPtro(ar, FLD.lua_Debug_namewhat, getfuncname(L, ci, cptr.add(ar, FLD.lua_Debug_name)));
                if (cptr.eq(cptr.ldPtro(ar, FLD.lua_Debug_namewhat), (null))) {
                    cptr.stPtro(ar, FLD.lua_Debug_namewhat, __sl9);
                    cptr.stPtro(ar, FLD.lua_Debug_name, null);
                }
                break;
            }
            case 114:
            {
                if (cptr.eq(ci, (null)) || !(cptr.ldU16o(ci, FLD.CallInfo_callstatus) & 256))
                    cptr.stI16o(ar, FLD.lua_Debug_ftransfer, cptr.stI16o(ar, FLD.lua_Debug_ntransfer, 0));
                else {
                    cptr.stI16o(ar, FLD.lua_Debug_ftransfer, cptr.ldU16o(ci, FLD.CallInfo_u2));
                    cptr.stI16o(ar, FLD.lua_Debug_ntransfer, cptr.ldU16o(ci, FLD.CallInfo_u2 + 2));
                }
                break;
            }
            case 76:
            case 102:
            break;
            default:
            status = 0;
        }
    }
    return status;
}

/** C ref: ldebug.c:388 — @param {CPtr} L @param {CPtr} what @param {CPtr} ar @returns {CInt} */
export function* lua_getinfo(L, what, ar) {
    let status;
    let cl;
    let ci;
    let func;
    (void 0);
    if (cptr.ld1s(what) == 62) {
        ci = null;
        func = ((cptr.add(cptr.ldPtro(L, FLD.lua_State_top), -(1), 16)));
        (void L, (void 0));
        what = cptr.add(what, 1);
        cptr.postdec(() => cptr.ldPtro(L, FLD.lua_State_top), (v) => { cptr.stPtro(L, FLD.lua_State_top, v); }, 16);
    } else {
        ci = cptr.ldPtro(ar, FLD.lua_Debug_i_ci);
        func = ((cptr.ldPtr(ci)));
        (void 0);
    }
    cl = (((cptr.ld1uo(((func)), FLD.TValue_tt_)) == 70) || ((cptr.ld1uo(((func)), FLD.TValue_tt_)) == 102)) ? ((((((cptr.ldPtr(((func))))))))) : null;
    status = auxgetinfo(L, what, ar, cl, ci);
    if (cptr.strchr(what, 102)) {
        {
            let io1 = (((cptr.ldPtro(L, FLD.lua_State_top))));
            let io2 = (func);
            cptr.memcpy(io1, io2, 8);
            (cptr.st1o((io1), FLD.TValue_tt_, (cptr.ld1uo(io2, FLD.TValue_tt_))));
            (void L, (void 0));
            (void 0);
        }
        ;
        {
            cptr.postinc(() => cptr.ldPtro(L, FLD.lua_State_top), (v) => { cptr.stPtro(L, FLD.lua_State_top, v); }, 16);
            (void L, (void 0));
        }
        ;
    }
    if (cptr.strchr(what, 76))
        (yield* collectvalidlines(L, cl));
    (void 0);
    return status;
}

/** C ref: ldebug.c:426 — @param {CInt} pc @param {CInt} jmptarget @returns {CInt} */
function filterpc(pc, jmptarget) {
    if (pc < jmptarget)
        return -1;
    else
        return pc;
}

/** C ref: ldebug.c:436 — @param {CPtr} p @param {CInt} lastpc @param {CInt} reg @returns {CInt} */
function findsetreg(p, lastpc, reg) {
    let pc;
    let setreg = -1;
    let jmptarget = 0;
    if ((cptr.ld1uo(cptr.decay(luaP_opmodes), ((((((cptr.ldI32o(cptr.ldPtro(p, FLD.Proto_code), lastpc, 4)) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0))), 1) & 128))
        lastpc--;
    for (pc = 0; pc < lastpc; pc++) {
        let i = cptr.ldI32o(cptr.ldPtro(p, FLD.Proto_code), pc, 4);
        let op = ((((((i) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0)));
        let a = (((((((i) >>> 7) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0));
        let change;
        switch (op) {
            case NHC.OP_LOADNIL:
            {
                let b = ((((((((i) >>> 16) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)));
                change = (a <= reg && reg <= ((a + b) | 0) ? 1 : 0);
                break;
            }
            case NHC.OP_TFORCALL:
            {
                change = (reg >= ((a + 2) | 0));
                break;
            }
            case NHC.OP_CALL:
            case NHC.OP_TAILCALL:
            {
                change = (reg >= a);
                break;
            }
            case NHC.OP_JMP:
            {
                let b = (((((((((i) >>> 7) & (((~(((~0) << 25) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)) - 16777215) | 0);
                let dest = (((pc + 1) | 0) + b) | 0;
                if (dest <= lastpc && dest > jmptarget)
                    jmptarget = dest;
                change = 0;
                break;
            }
            default:
            change = ((cptr.ld1uo(cptr.decay(luaP_opmodes), op, 1) & 8) && reg == a ? 1 : 0);
            break;
        }
        if (change)
            setreg = filterpc(pc, jmptarget);
    }
    return setreg;
}

/** C ref: ldebug.c:485 — @param {CPtr} p @param {CInt} index @param {CPtr} name @returns {CPtr} */
function kname(p, index, name) {
    let kvalue = cptr.add(cptr.ldPtro(p, FLD.Proto_k), index, 16);
    if ((((((cptr.ld1uo(((kvalue)), FLD.TValue_tt_))) & 15)) == 4)) {
        cptr.stPtr(name, (cptr.add((((((((cptr.ldPtr(((kvalue)))))))))), FLD.TString_contents)));
        return __sl10;
    } else {
        cptr.stPtr(name, __sl0);
        return null;
    }
}

/** C ref: ldebug.c:498 — @param {CPtr} p @param {CPtr} ppc @param {CInt} reg @param {CPtr} name @returns {CPtr} */
function basicgetobjname(p, ppc, reg, name) {
    let pc = cptr.ldI32(ppc);
    cptr.stPtr(name, luaF_getlocalname(p, (reg + 1) | 0, pc));
    if (cptr.ldPtr(name))
        return cptr.decay(strlocal);
    cptr.stI32(ppc, pc = findsetreg(p, pc, reg));
    if (pc != -1) {
        let i = cptr.ldI32o(cptr.ldPtro(p, FLD.Proto_code), pc, 4);
        let op = ((((((i) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0)));
        switch (op) {
            case NHC.OP_MOVE:
            {
                let b = ((((((((i) >>> 16) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)));
                if (b < (((((((i) >>> 7) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)))
                    return basicgetobjname(p, ppc, b, name);
                break;
            }
            case NHC.OP_GETUPVAL:
            {
                cptr.stPtr(name, upvalname(p, ((((((((i) >>> 16) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)))));
                return cptr.decay(strupval);
            }
            case NHC.OP_LOADK:
            return kname(p, ((((((((i) >>> 15) & (((~(((~0) << 17) >>> 0)) << 0) >>> 0)) >>> 0)) | 0))), name);
            case NHC.OP_LOADKX:
            return kname(p, ((((((((cptr.ldI32o(cptr.ldPtro(p, FLD.Proto_code), (pc + 1) | 0, 4)) >>> 7) & (((~(((~0) << 25) >>> 0)) << 0) >>> 0)) >>> 0)) | 0))), name);
            default:
            break;
        }
    }
    return null;
}

/** C ref: ldebug.c:532 — @param {CPtr} p @param {CInt} pc @param {CInt} c @param {CPtr} name */
function rname(p, pc, c, name) {
    pc = cptr.box(pc);
    let what = basicgetobjname(p, pc, c, name);
    if (!(what && cptr.ld1s(what) == 99))
        cptr.stPtr(name, __sl0);
}

/** C ref: ldebug.c:542 — @param {CPtr} p @param {CInt} pc @param {CUInt} i @param {CPtr} name */
function rkname(p, pc, i, name) {
    let c = ((((((((i) >>> 24) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)));
    if (((((((((i) >>> 15) & (((~(((~0) << 1) >>> 0)) << 0) >>> 0)) >>> 0)) | 0))))
        kname(p, c, name);
    else
        rname(p, pc, c, name);
}

/** C ref: ldebug.c:558 — @param {CPtr} p @param {CInt} pc @param {CUInt} i @param {CInt} isup @returns {CPtr} */
function isEnv(p, pc, i, isup) {
    pc = cptr.box(pc);
    let t = ((((((((i) >>> 16) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)));
    let name = cptr.box(0);
    if (isup)
        name.v = upvalname(p, t);
    else {
        let what = basicgetobjname(p, pc, t, name);
        if (!cptr.eq(what, cptr.decay(strlocal)) && !cptr.eq(what, cptr.decay(strupval)))
            name.v = null;
    }
    return (name.v && strcmp(name.v, __sl11) == 0) ? __sl12 : __sl13;
}

/** C ref: ldebug.c:575 — @param {CPtr} p @param {CInt} lastpc @param {CInt} reg @param {CPtr} name @returns {CPtr} */
function getobjname(p, lastpc, reg, name) {
    lastpc = cptr.box(lastpc);
    let kind = basicgetobjname(p, lastpc, reg, name);
    if (!cptr.eq(kind, (null)))
        return kind;
    else if (lastpc.v != -1) {
        let i = cptr.ldI32o(cptr.ldPtro(p, FLD.Proto_code), lastpc.v, 4);
        let op = ((((((i) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0)));
        switch (op) {
            case NHC.OP_GETTABUP:
            {
                let k = ((((((((i) >>> 24) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)));
                kname(p, k, name);
                return isEnv(p, lastpc.v, i, 1);
            }
            case NHC.OP_GETTABLE:
            {
                let k = ((((((((i) >>> 24) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)));
                rname(p, lastpc.v, k, name);
                return isEnv(p, lastpc.v, i, 0);
            }
            case NHC.OP_GETI:
            {
                cptr.stPtr(name, __sl14);
                return __sl13;
            }
            case NHC.OP_GETFIELD:
            {
                let k = ((((((((i) >>> 24) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)));
                kname(p, k, name);
                return isEnv(p, lastpc.v, i, 0);
            }
            case NHC.OP_SELF:
            {
                rkname(p, lastpc.v, i, name);
                return __sl15;
            }
            default:
            break;
        }
    }
    return null;
}

/** C ref: ldebug.c:620 — @param {CPtr} L @param {CPtr} p @param {CInt} pc @param {CPtr} name @returns {CPtr} */
function funcnamefromcode(L, p, pc, name) {
    let tm = 0;
    let i = cptr.ldI32o(cptr.ldPtro(p, FLD.Proto_code), pc, 4);
    switch (((((((i) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0)))) {
        case NHC.OP_CALL:
        case NHC.OP_TAILCALL:
        return getobjname(p, pc, (((((((i) >>> 7) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)), name);
        case NHC.OP_TFORCALL:
        {
            cptr.stPtr(name, __sl16);
            return __sl16;
        }
        case NHC.OP_SELF:
        case NHC.OP_GETTABUP:
        case NHC.OP_GETTABLE:
        case NHC.OP_GETI:
        case NHC.OP_GETFIELD:
        tm = NHC.TM_INDEX;
        break;
        case NHC.OP_SETTABUP:
        case NHC.OP_SETTABLE:
        case NHC.OP_SETI:
        case NHC.OP_SETFIELD:
        tm = NHC.TM_NEWINDEX;
        break;
        case NHC.OP_MMBIN:
        case NHC.OP_MMBINI:
        case NHC.OP_MMBINK:
        {
            tm = ((((((((((i) >>> 24) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)))));
            break;
        }
        case NHC.OP_UNM:
        tm = NHC.TM_UNM;
        break;
        case NHC.OP_BNOT:
        tm = NHC.TM_BNOT;
        break;
        case NHC.OP_LEN:
        tm = NHC.TM_LEN;
        break;
        case NHC.OP_CONCAT:
        tm = NHC.TM_CONCAT;
        break;
        case NHC.OP_EQ:
        tm = NHC.TM_EQ;
        break;
        case NHC.OP_LT:
        case NHC.OP_LTI:
        case NHC.OP_GTI:
        tm = NHC.TM_LT;
        break;
        case NHC.OP_LE:
        case NHC.OP_LEI:
        case NHC.OP_GEI:
        tm = NHC.TM_LE;
        break;
        case NHC.OP_CLOSE:
        case NHC.OP_RETURN:
        tm = NHC.TM_CLOSE;
        break;
        default:
        return null;
    }
    cptr.stPtr(name, cptr.add((cptr.add((cptr.ldPtro2((cptr.ldPtro(L, FLD.lua_State_l_G)), tm, 8, FLD.global_State_tmname)), FLD.TString_contents)), 2));
    return __sl17;
}

/** C ref: ldebug.c:664 — @param {CPtr} L @param {CPtr} ci @param {CPtr} name @returns {CPtr} */
function funcnamefromcall(L, ci, name) {
    if (cptr.ldU16o(ci, FLD.CallInfo_callstatus) & 8) {
        cptr.stPtr(name, __sl0);
        return __sl18;
    } else if (cptr.ldU16o(ci, FLD.CallInfo_callstatus) & 128) {
        cptr.stPtr(name, __sl19);
        return __sl17;
    } else if ((!(cptr.ldU16o((ci), FLD.CallInfo_callstatus) & 2)))
        return funcnamefromcode(L, cptr.ldPtro((((((((cptr.ldPtr(((((cptr.ldPtr((ci)))))))))))))), FLD.LClosure_p), currentpc(ci), name);
    else
        return null;
}

/** C ref: ldebug.c:690 — @param {CPtr} ci @param {CPtr} o @returns {CInt} */
function instack(ci, o) {
    let pos;
    let base = cptr.add(cptr.ldPtr(ci), 1, 16);
    for (pos = 0; cptr.cmp(cptr.add(base, pos, 16), cptr.ldPtro(ci, FLD.CallInfo_top)) < 0; pos++) {
        if (cptr.eq(o, ((cptr.add(base, pos, 16)))))
            return pos;
    }
    return -1;
}

/** C ref: ldebug.c:706 — @param {CPtr} ci @param {CPtr} o @param {CPtr} name @returns {CPtr} */
function getupvalname(ci, o, name) {
    let c = (((((((cptr.ldPtr(((((cptr.ldPtr((ci))))))))))))));
    let i;
    for (i = 0; i < cptr.ld1uo(c, FLD.LClosure_nupvalues); i++) {
        if (cptr.eq(cptr.ldPtro(cptr.ldPtro2(c, i, 8, FLD.LClosure_upvals), FLD.UpVal_v), o)) {
            cptr.stPtr(name, upvalname(cptr.ldPtro(c, FLD.LClosure_p), i));
            return cptr.decay(strupval);
        }
    }
    return null;
}

/** C ref: ldebug.c:720 — @param {CPtr} L @param {CPtr} kind @param {CPtr} name @returns {CPtr} */
function* formatvarinfo(L, kind, name) {
    if (cptr.eq(kind, (null)))
        return __sl9;
    else
        return (yield* luaO_pushfstring(L, __sl20, kind, name));
}

/** C ref: ldebug.c:732 — @param {CPtr} L @param {CPtr} o @returns {CPtr} */
function* varinfo(L, o) {
    let ci = cptr.ldPtro(L, FLD.lua_State_ci);
    let name = cptr.box(null);
    let kind = null;
    if ((!(cptr.ldU16o((ci), FLD.CallInfo_callstatus) & 2))) {
        kind = getupvalname(ci, o, name);
        if (!kind) {
            let reg = instack(ci, o);
            if (reg >= 0)
                kind = getobjname(cptr.ldPtro((((((((cptr.ldPtr(((((cptr.ldPtr((ci)))))))))))))), FLD.LClosure_p), currentpc(ci), reg, name);
        }
    }
    return (yield* formatvarinfo(L, kind, name.v));
}

/** C ref: ldebug.c:751 — @param {CPtr} L @param {CPtr} o @param {CPtr} op @param {CPtr} extra */
function* typeerror(L, o, op, extra) {
    let t = (yield* luaT_objtypename(L, o));
    (yield* luaG_runerror(L, __sl21, op, t, extra));
}

/** C ref: ldebug.c:762 — @param {CPtr} L @param {CPtr} o @param {CPtr} op */
export function* luaG_typeerror(L, o, op) {
    (yield* typeerror(L, o, op, (yield* varinfo(L, o))));
}

/** C ref: ldebug.c:772 — @param {CPtr} L @param {CPtr} o */
export function* luaG_callerror(L, o) {
    let ci = cptr.ldPtro(L, FLD.lua_State_ci);
    let name = cptr.box(null);
    let kind = funcnamefromcall(L, ci, name);
    let extra = kind ? (yield* formatvarinfo(L, kind, name.v)) : (yield* varinfo(L, o));
    (yield* typeerror(L, o, __sl22, extra));
}

/** C ref: ldebug.c:781 — @param {CPtr} L @param {CPtr} o @param {CPtr} what */
export function* luaG_forerror(L, o, what) {
    (yield* luaG_runerror(L, __sl23, what, (yield* luaT_objtypename(L, o))));
}

/** C ref: ldebug.c:787 — @param {CPtr} L @param {CPtr} p1 @param {CPtr} p2 */
export function* luaG_concaterror(L, p1, p2) {
    if ((((((cptr.ld1uo(((p1)), FLD.TValue_tt_))) & 15)) == 4) || (((((cptr.ld1uo(((p1)), FLD.TValue_tt_))) & 15)) == 3))
        p1 = p2;
    (yield* luaG_typeerror(L, p1, __sl24));
}

/** C ref: ldebug.c:793 — @param {CPtr} L @param {CPtr} p1 @param {CPtr} p2 @param {CPtr} msg */
export function* luaG_opinterror(L, p1, p2, msg) {
    if (!(((((cptr.ld1uo(((p1)), FLD.TValue_tt_))) & 15)) == 3))
        p2 = p1;
    (yield* luaG_typeerror(L, p2, msg));
}

/** C ref: ldebug.c:804 — @param {CPtr} L @param {CPtr} p1 @param {CPtr} p2 */
export function* luaG_tointerror(L, p1, p2) {
    let temp = cptr.box(0n);
    if (!luaV_tointegerns(p1, temp, NHC.F2Ieq))
        p2 = p1;
    (yield* luaG_runerror(L, __sl25, (yield* varinfo(L, p2))));
}

/** C ref: ldebug.c:812 — @param {CPtr} L @param {CPtr} p1 @param {CPtr} p2 */
export function* luaG_ordererror(L, p1, p2) {
    let t1 = (yield* luaT_objtypename(L, p1));
    let t2 = (yield* luaT_objtypename(L, p2));
    if (strcmp(t1, t2) == 0)
        (yield* luaG_runerror(L, __sl26, t1));
    else
        (yield* luaG_runerror(L, __sl27, t1, t2));
}

/** C ref: ldebug.c:823 — @param {CPtr} L @param {CPtr} msg @param {CPtr} src @param {CInt} line @returns {CPtr} */
export function* luaG_addinfo(L, msg, src, line) {
    let buff = new Uint8Array(60);
    if (src)
        luaO_chunkid(cptr.decay(buff), (cptr.add((src), FLD.TString_contents)), (cptr.ld1uo((src), FLD.TString_shrlen) != 255 ? BigInt(cptr.ld1uo((src), FLD.TString_shrlen) >>> 0) : cptr.ldU64o((src), FLD.TString_u)));
    else {
        cptr.st1o(cptr.decay(buff), 0, 63, 1);
        cptr.st1o(cptr.decay(buff), 1, 0, 1);
    }
    return (yield* luaO_pushfstring(L, __sl28, cptr.decay(buff), line, msg));
}

/** C ref: ldebug.c:835 — @param {CPtr} L */
export function* luaG_errormsg(L) {
    if (cptr.ldI64o(L, FLD.lua_State_errfunc) != 0n) {
        let errfunc = ((cptr.add((((cptr.ldPtro(L, FLD.lua_State_stack)))), (cptr.ldI64o(L, FLD.lua_State_errfunc)))));
        (void 0);
        {
            let io1 = (((cptr.ldPtro(L, FLD.lua_State_top))));
            let io2 = (((cptr.add(cptr.ldPtro(L, FLD.lua_State_top), -(1), 16))));
            cptr.memcpy(io1, io2, 8);
            (cptr.st1o((io1), FLD.TValue_tt_, (cptr.ld1uo(io2, FLD.TValue_tt_))));
            (void L, (void 0));
            (void 0);
        }
        ;
        {
            let io1 = (((cptr.add(cptr.ldPtro(L, FLD.lua_State_top), -(1), 16))));
            let io2 = (((errfunc)));
            cptr.memcpy(io1, io2, 8);
            (cptr.st1o((io1), FLD.TValue_tt_, (cptr.ld1uo(io2, FLD.TValue_tt_))));
            (void L, (void 0));
            (void 0);
        }
        ;
        cptr.postinc(() => cptr.ldPtro(L, FLD.lua_State_top), (v) => { cptr.stPtro(L, FLD.lua_State_top, v); }, 16);
        (yield* luaD_callnoyield(L, cptr.add(cptr.ldPtro(L, FLD.lua_State_top), -(2), 16), 1));
    }
    (yield* luaD_throw(L, 2));
}

/** C ref: ldebug.c:848 — @param {CPtr} L @param {CPtr} fmt */
export function* luaG_runerror(L, fmt, ...__va) {
    let ci = cptr.ldPtro(L, FLD.lua_State_ci);
    let msg;
    let argp;
    {
        if (cptr.ldI64o((cptr.ldPtro(L, FLD.lua_State_l_G)), FLD.global_State_GCdebt) > 0n) {
            void 0;
            (yield* luaC_step(L));
            void 0;
        }
        ;
        (void 0);
    }
    ;
    argp = cptr.vaList(__va);
    msg = (yield* luaO_pushvfstring(L, fmt, argp));
    argp = null;
    if ((!(cptr.ldU16o((ci), FLD.CallInfo_callstatus) & 2))) {
        (yield* luaG_addinfo(L, msg, cptr.ldPtro(cptr.ldPtro((((((((cptr.ldPtr(((((cptr.ldPtr((ci)))))))))))))), FLD.LClosure_p), FLD.Proto_source), getcurrentline(ci)));
        {
            let io1 = (((cptr.add(cptr.ldPtro(L, FLD.lua_State_top), -(2), 16))));
            let io2 = (((cptr.add(cptr.ldPtro(L, FLD.lua_State_top), -(1), 16))));
            cptr.memcpy(io1, io2, 8);
            (cptr.st1o((io1), FLD.TValue_tt_, (cptr.ld1uo(io2, FLD.TValue_tt_))));
            (void L, (void 0));
            (void 0);
        }
        ;
        cptr.postdec(() => cptr.ldPtro(L, FLD.lua_State_top), (v) => { cptr.stPtro(L, FLD.lua_State_top, v); }, 16);
    }
    (yield* luaG_errormsg(L));
}

/** C ref: ldebug.c:873 — @param {CPtr} p @param {CInt} oldpc @param {CInt} newpc @returns {CInt} */
function changedline(p, oldpc, newpc) {
    if (cptr.eq(cptr.ldPtro(p, FLD.Proto_lineinfo), (null)))
        return 0;
    if (((newpc - oldpc) | 0) < 64) {
        let delta = 0;
        let pc = oldpc;
        for (; ; ) {
            let lineinfo = cptr.ld1uo(cptr.ldPtro(p, FLD.Proto_lineinfo), ++pc);
            if (lineinfo == -128)
                break;
            delta = (delta + lineinfo) | 0;
            if (pc == newpc)
                return (delta != 0);
        }
    }
    return (luaG_getfuncline(p, oldpc) != luaG_getfuncline(p, newpc));
}

/** C ref: ldebug.c:902 — @param {CPtr} L @returns {CInt} */
export function* luaG_tracecall(L) {
    let ci = cptr.ldPtro(L, FLD.lua_State_ci);
    let p = cptr.ldPtro((((((((cptr.ldPtr(((((cptr.ldPtr((ci)))))))))))))), FLD.LClosure_p);
    cptr.stI32o(ci, FLD.CallInfo_u + 8, 1);
    if (cptr.eq(cptr.ldPtro(ci, FLD.CallInfo_u), cptr.ldPtro(p, FLD.Proto_code))) {
        if (cptr.ld1uo(p, FLD.Proto_is_vararg))
            return 0;
        else if (!(cptr.ldU16o(ci, FLD.CallInfo_callstatus) & 64))
            (yield* luaD_hookcall(L, ci));
    }
    return 1;
}

/** C ref: ldebug.c:928 — @param {CPtr} L @param {CPtr} pc @returns {CInt} */
export function* luaG_traceexec(L, pc) {
    let ci = cptr.ldPtro(L, FLD.lua_State_ci);
    let mask = uchar(cptr.ldI32o(L, FLD.lua_State_hookmask));
    let p = cptr.ldPtro((((((((cptr.ldPtr(((((cptr.ldPtr((ci)))))))))))))), FLD.LClosure_p);
    let counthook;
    if (!(mask & 12)) {
        cptr.stI32o(ci, FLD.CallInfo_u + 8, 0);
        return 0;
    }
    pc = cptr.add(pc, 1, 4);
    cptr.stPtro(ci, FLD.CallInfo_u, pc);
    counthook = (mask & 8) && (cptr.stI32o(L, FLD.lua_State_hookcount, cptr.ldI32o(L, FLD.lua_State_hookcount) + -1) == 0) ? 1 : 0;
    if (counthook)
        (cptr.stI32o(L, FLD.lua_State_hookcount, cptr.ldI32o(L, FLD.lua_State_basehookcount)));
    else if (!(mask & 4))
        return 1;
    if (cptr.ldU16o(ci, FLD.CallInfo_callstatus) & 64) {
        cptr.stI16o(ci, FLD.CallInfo_callstatus, cptr.ldU16o(ci, FLD.CallInfo_callstatus) & -65);
        return 1;
    }
    if (!((cptr.ld1uo(cptr.decay(luaP_opmodes), ((((((cptr.ldI32((cptr.add(cptr.ldPtro(ci, FLD.CallInfo_u), -(1), 4)))) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0))), 1) & 32) && ((((((((cptr.ldI32((cptr.add(cptr.ldPtro(ci, FLD.CallInfo_u), -(1), 4)))) >>> 16) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0))) == 0))
        cptr.stPtro(L, FLD.lua_State_top, cptr.ldPtro(ci, FLD.CallInfo_top));
    if (counthook)
        (yield* luaD_hook(L, 3, -1, 0, 0));
    if (mask & 4) {
        let oldpc = (cptr.ldI32o(L, FLD.lua_State_oldpc) < cptr.ldI32o(p, FLD.Proto_sizecode)) ? cptr.ldI32o(L, FLD.lua_State_oldpc) : 0;
        let npci = (((Number(BigInt.asIntN(32, ((cptr.diff((pc), cptr.ldPtro((p), FLD.Proto_code)) / 4n))))) - 1) | 0);
        if (npci <= oldpc || changedline(p, oldpc, npci)) {
            let newline = luaG_getfuncline(p, npci);
            (yield* luaD_hook(L, 2, newline, 0, 0));
        }
        cptr.stI32o(L, FLD.lua_State_oldpc, npci);
    }
    if (cptr.ld1uo(L, FLD.lua_State_status) == 1) {
        if (counthook)
            cptr.stI32o(L, FLD.lua_State_hookcount, 1);
        cptr.stI16o(ci, FLD.CallInfo_callstatus, cptr.ldU16o(ci, FLD.CallInfo_callstatus) | 64);
        (yield* luaD_throw(L, 1));
    }
    return 1;
}

// --- BEGIN c2js reset block (tools/c2js/resetify.mjs) — do not edit ---
// 2 bindings: 0 rebound+refilled, 0 rebound, 2 refilled.
// S/P are supplied by js/generated-y/__reset.js so this module needs no new import.
let __c2js_rs = null;
export function __captureState(S) { __c2js_rs = [S(strlocal), S(strupval)]; }
export function __resetState(P) {
    const r = __c2js_rs;
    if (r === null) throw new Error("ldebug.js: __resetState before __captureState");
    P(r[0]);
    P(r[1]);
}
// --- END c2js reset block ---
