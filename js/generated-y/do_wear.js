// YIELDABLE BUILD — generated by tools/c2js/yieldify.mjs from js/generated/do_wear.js
// Do not edit by hand, and do not score from this directory: the synchronous
// build in js/generated/ is the one the judge runs. Coloured functions here
// are generators; `yield*` marks every call that can reach a keystroke read.
// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/src/do_wear.c
// Input sha256: cf33ad9f5b18bcdfffd09d4e99867590802735a9ae2d34b9eb2b64880fa459c7
// Transpiler: tools/c2js c2js emit v1+batch

import { schar, u32mod, uchar } from '../cmachine.js';
import * as cptr from '../cptr.js';
import * as NHC from './nhconst.js';
import * as NHM from './nhmacro.js';
import * as FLD from './nhfield.js';
import { ARM_BONUS, WrappingAllowed, bimanual, cant_drown, cantweararm, is_boots, is_cloak, is_corrodeable, is_crackable, is_damageable, is_flimsy, is_gloves, is_helmet, is_metallic, is_shield, is_shirt, is_suit, is_sword, touch_petrifies } from './nhmacrofn.js';
import { BInvis, BLevitation, BStealth, Blind, Blind_telepat, Breathless, Detect_monsters, EInvis, ESleepy, EStealth, Fast, Flying, Glib, HFast, HFumbling, HInvis, HLevitation, HProtection, HSee_invisible, HSleepy, HStealth, Hallucination, Invis, Invisible, Levitation, ParanoidRemove, Protection_from_shape_changers, Punished, See_invisible, Slimed, Stone_resistance, Strangled, Swimming, ULEFTY, URIGHTY, Unblind_telepat, Unchanging, Underwater, Upolyd, Very_fast } from './nhprop.js';
import { c_color_names, c_common_strings, cg, disp, flags, ga, gi, gm, gn, gu, gw, gy, iflags, program_state, rightleftchars, svc, svd, u, uamul, uarm, uarmc, uarmf, uarmg, uarmh, uarms, uarmu, uball, ublindf, uleft, uquiver, uright, uskin, uswapwep, uwep } from './decl.js';
import { Tobjnam, Yname2, an, ansimpleoname, boots_simple_name, cloak_simple_name, corpse_xname, doname, erosion_matters, gloves_simple_name, helm_simple_name, killer_xname, makeplural, makesingular, obj_is_pname, otense, safe_typename, shield_simple_name, shirt_simple_name, simpleonames, suit_simple_name, the, thesimpleoname, vtense, xname, yname } from './objnam.js';
import { body_part, change_sex, float_vs_flight, livelog_newform, poly_gender } from './polyself.js';
import { There, You, You_cant, You_feel, Your, impossible, pline, pline_The, urgent_pline } from './pline.js';
import { carrying_stoning_corpse, getobj, ggetobj, is_worn, prinv, silly_thing, update_inventory, useup, wearing_armor } from './invent.js';
import { discover_object, observe_object } from './o_init.js';
import { objects } from './objects.js';
import { hcolor, hliquid, obj_pmname, x_monnam } from './do_name.js';
import { nomul, spoteffects, unmul } from './hack.js';
import { incr_itimeout, make_glib, make_hallucinated, make_slimed, self_invis_message, toggle_blindness } from './potion.js';
import { rn2, rnd, rng_log_enabled, rng_log_set_caller } from './rnd.js';
import { drown, erode_obj, float_down, float_up, instapetrify, selftouch } from './trap.js';
import { racial_exception, setnotworn, setworn, which_armor } from './worn.js';
import { is_lava, is_pool, is_pool_or_lava } from './dbridge.js';
import { has_ceiling, on_level, surface } from './dungeon.js';
import { newsym, see_monsters, set_mimic_blocking } from './display.js';
import { acurr, change_luck, extremeattr, uchangealign } from './attrib.js';
import { curse, is_flammable, is_rottable, set_bknown } from './mkobj.js';
import { mons } from './monst.js';
import { nh_snprintf, sgn, strsubst } from './hacklib.js';
import { remove_worn_item } from './steal.js';
import { add_valid_menu_class, encumber_msg, is_worn_by_type, menu_class_present, query_category, query_objlist, u_safe_from_fatal_corpse } from './pickup.js';
import { condtests } from './botl.js';
import { artifact_light, retouch_object } from './artifact.js';
import { begin_burn, end_burn } from './timeout.js';
import { arti_light_description } from './light.js';
import { region_danger } from './region.js';
import { canletgo, dropx, trycall } from './do.js';
import { breakarm, can_be_strangled, num_horns, sliparm } from './mondata.js';
import { empty_handed, setuqwep, setuswapwep, setuwep, welded } from './wield.js';
import { rescham, restartcham } from './mon.js';
import { set_bc } from './ball.js';
import { gulp_blnd_check } from './mhitu.js';
import { cmdq_clear, cmdq_peek, cmdq_pop, paranoid_ynq, set_occupation, yn_function } from './cmd.js';
import { amulet } from './wizard.js';
import { panic } from './end.js';
import { weapon_descr } from './weapon.js';
import { obj_resists } from './zap.js';
import { stop_occupation } from './allmain.js';
import { shk_your } from './shk.js';

// struct field offsets used below, bound at module scope so V8 folds them
// (values from ./nhfield.js, which is the whole table)
const $Role_mnum = FLD.Role_mnum, $_cmd_queue_key = FLD._cmd_queue_key,
    $c_common_strings_c_something = FLD.c_common_strings_c_something,
    $condtests_t_enabled = FLD.condtests_t_enabled, $context_info_mon_moving = FLD.context_info_mon_moving,
    $context_info_takeoff = FLD.context_info_takeoff, $d_level_dlevel = FLD.d_level_dlevel,
    $dgn_topology_d_air_level = FLD.dgn_topology_d_air_level,
    $dgn_topology_d_qstart_level = FLD.dgn_topology_d_qstart_level,
    $dgn_topology_d_water_level = FLD.dgn_topology_d_water_level, $flag_female = FLD.flag_female,
    $flag_menu_style = FLD.flag_menu_style, $flag_paranoia_bits = FLD.flag_paranoia_bits,
    $flag_verbose = FLD.flag_verbose, $instance_flags_in_lava_effects = FLD.instance_flags_in_lava_effects,
    $instance_globals_i_initial_don = FLD.instance_globals_i_initial_don,
    $instance_globals_i_invent = FLD.instance_globals_i_invent,
    $instance_globals_i_item_action_in_progress = FLD.instance_globals_i_item_action_in_progress,
    $instance_globals_m_multi = FLD.instance_globals_m_multi,
    $instance_globals_m_multi_reason = FLD.instance_globals_m_multi_reason,
    $instance_globals_n_nomovemsg = FLD.instance_globals_n_nomovemsg,
    $instance_globals_saved_d_dungeon_topology = FLD.instance_globals_saved_d_dungeon_topology,
    $instance_globals_u_urole = FLD.instance_globals_u_urole,
    $instance_globals_w_wasinwater = FLD.instance_globals_w_wasinwater,
    $instance_globals_y_youmonst = FLD.instance_globals_y_youmonst, $monst_data = FLD.monst_data,
    $obj_bknown = FLD.obj_bknown, $obj_corpsenm = FLD.obj_corpsenm, $obj_cursed = FLD.obj_cursed,
    $obj_dknown = FLD.obj_dknown, $obj_in_use = FLD.obj_in_use, $obj_known = FLD.obj_known,
    $obj_lamplit = FLD.obj_lamplit, $obj_o_id = FLD.obj_o_id, $obj_oclass = FLD.obj_oclass,
    $obj_oeroded = FLD.obj_oeroded, $obj_oeroded2 = FLD.obj_oeroded2, $obj_oerodeproof = FLD.obj_oerodeproof,
    $obj_otyp = FLD.obj_otyp, $obj_owornmask = FLD.obj_owornmask, $obj_quan = FLD.obj_quan,
    $obj_spe = FLD.obj_spe, $obj_unpaid = FLD.obj_unpaid, $objclass_oc_big = FLD.objclass_oc_big,
    $objclass_oc_charged = FLD.objclass_oc_charged, $objclass_oc_delay = FLD.objclass_oc_delay,
    $objclass_oc_material = FLD.objclass_oc_material, $objclass_oc_name_known = FLD.objclass_oc_name_known,
    $objclass_oc_oc1 = FLD.objclass_oc_oc1, $objclass_oc_oprop = FLD.objclass_oc_oprop,
    $objclass_oc_subtyp = FLD.objclass_oc_subtyp, $permonst_ac = FLD.permonst_ac,
    $permonst_mflags1 = FLD.permonst_mflags1, $permonst_mlet = FLD.permonst_mlet,
    $permonst_msize = FLD.permonst_msize, $prop_blocked = FLD.prop_blocked,
    $prop_intrinsic = FLD.prop_intrinsic, $sinfo_restoring = FLD.sinfo_restoring,
    $takeoff_info_cancelled_don = FLD.takeoff_info_cancelled_don,
    $takeoff_info_delay = FLD.takeoff_info_delay, $takeoff_info_disrobing = FLD.takeoff_info_disrobing,
    $takeoff_info_what = FLD.takeoff_info_what, $you_abon = FLD.you_abon, $you_acurr = FLD.you_acurr,
    $you_atemp = FLD.you_atemp, $you_twoweap = FLD.you_twoweap, $you_uac = FLD.you_uac,
    $you_ualign = FLD.you_ualign, $you_ualignbase = FLD.you_ualignbase, $you_ublessed = FLD.you_ublessed,
    $you_udaminc = FLD.you_udaminc, $you_uhandedness = FLD.you_uhandedness, $you_uhitinc = FLD.you_uhitinc,
    $you_uinwater = FLD.you_uinwater, $you_umonnum = FLD.you_umonnum, $you_umonster = FLD.you_umonster,
    $you_uprops = FLD.you_uprops, $you_uroleplay = FLD.you_uroleplay, $you_uspellprot = FLD.you_uspellprot,
    $you_usteed = FLD.you_usteed, $you_uswallow = FLD.you_uswallow, $you_utrap = FLD.you_utrap,
    $you_utraptype = FLD.you_utraptype, $you_uy = FLD.you_uy, $you_uz = FLD.you_uz;

// string literals (C char* uses decay to CPtr into these static buffers)
const __s_were_wearing_s = cptr.lit("were wearing %s.");
const __s_around_your_s = cptr.lit(" around your %s");
const __s_are_now_wearing_s_s = cptr.lit("are now wearing %s%s.");
const __s_move_very_quietly = cptr.lit("move very quietly.");
const __s_float_imperceptibly = cptr.lit("float imperceptibly.");
const __s_walk_very_quietly = cptr.lit("walk very quietly.");
const __s_s_s_are_noisy = cptr.lit("%s%s are noisy.");
const __s_and = cptr.lit("and ");
const __s_sure = cptr.lit("sure");
const __s_empty = cptr.lit("");
const __s_that_monsters_s_have_difficulty = cptr.lit("that monsters%s have difficulty pinpointing your location.");
const __s_no_longer = cptr.lit(" no longer");
const __s_yourself_speed_up_s = cptr.lit("yourself speed up%s.");
const __s_a_bit_more = cptr.lit(" a bit more");
const __s_do_wear_c = cptr.lit("do_wear.c");
const __s_boots_on = cptr.lit("Boots_on");
const __s_yourself_slow_down_s = cptr.lit("yourself slow down%s.");
const __s_a_bit = cptr.lit(" a bit");
const __s_can_s = cptr.lit("can %s!");
const __s_no_longer_see_through_yourself = cptr.lit("no longer see through yourself");
const __s_suddenly_you_can_s_yourself = cptr.lit("Suddenly you can%s yourself.");
const __s_see_through = cptr.lit(" see through");
const __s_not_see = cptr.lit("not see");
const __s_s_very_tightly = cptr.lit("%s very tightly.");
const __s_fit = cptr.lit("fit");
const __s_can_s__2 = cptr.lit("can %s.");
const __s_see_through_yourself = cptr.lit("see through yourself");
const __s_no_longer_see_yourself = cptr.lit("no longer see yourself");
const __s_suddenly_you_can_s = cptr.lit("Suddenly you can %s.");
const __s_s_for_a_moment = cptr.lit("%s for a moment.");
const __s_vibrate = cptr.lit("vibrate");
const __s_s_s_for_a_moment = cptr.lit("%s %s for a moment.");
const __s_glow = cptr.lit("glow");
const __s_my_brain_hurts = cptr.lit("My brain hurts!");
const __s_pct_s_dot = cptr.lit("%s.");
const __s_like_sitting_in_a_corner = cptr.lit("like sitting in a corner");
const __s_giddy = cptr.lit("giddy");
const __s_gloves_on = cptr.lit("Gloves_on");
const __s_s_s_in_your_bare_s = cptr.lit("%s %s in your bare %s.");
const __s_now_wield = cptr.lit("now wield");
const __s_are_wielding = cptr.lit("are wielding");
const __s_s_s = cptr.lit("%s %s");
const __s_removing = cptr.lit("removing");
const __s_losing = cptr.lit("losing");
const __s_set_of = cptr.lit("set of ");
const __s_resistance_timing_out = cptr.lit("resistance timing out");
const __s_wielding_corpse = cptr.lit("wielding_corpse");
const __s_s_while_wielding_s = cptr.lit("%s while wielding %s");
const __s_speed_up_s = cptr.lit("speed up%s.");
const __s_slow_down = cptr.lit("slow down.");
const __s_s_s_to_shine_s = cptr.lit("%s %s to shine %s!");
const __s_begin = cptr.lit("begin");
const __s_s_shining = cptr.lit("%s shining.");
const __s_stop = cptr.lit("stop");
const __s_are_no_longer_bothered_by_the_poison_gas = cptr.lit("are no longer bothered by the poison gas.");
const __s_are_suddenly_very_s = cptr.lit("are suddenly very %s!");
const __s_feminine = cptr.lit("feminine");
const __s_masculine = cptr.lit("masculine");
const __s_don_t_feel_like_yourself = cptr.lit("don't feel like yourself.");
const __s_amulet_disintegrates = cptr.lit("amulet disintegrates!");
const __s_it_constricts_your_throat = cptr.lit("It constricts your throat!");
const __s_amulet_on = cptr.lit("Amulet_on");
const __s_are_now_in_flight = cptr.lit("are now in flight.");
const __s_suddenly_inhale_an_unhealthy_amount_of_s = cptr.lit("suddenly inhale an unhealthy amount of %s!");
const __s_water = cptr.lit("water");
const __s_are_breathing_poison_gas = cptr.lit("are breathing poison gas!");
const __s_s_is_no_longer_constricted = cptr.lit("%s is no longer constricted!");
const __s_can_breathe_more_easily = cptr.lit("can breathe more easily!");
const __s_stop_flying = cptr.lit("stop flying");
const __s_land = cptr.lit("land");
const __s_suddenly_you_are_transparent_but_there = cptr.lit("Suddenly you are transparent, but there!");
const __s_strange_i_didn_t_know_you_had_that_ring = cptr.lit("Strange... I didn't know you had that ring.");
const __s_suddenly_you_cannot_see_yourself = cptr.lit("Suddenly you cannot see yourself.");
const __s_body_seems_to_unfade_s = cptr.lit("body seems to unfade%s.");
const __s_completely = cptr.lit(" completely");
const __s_dot2 = cptr.lit("..");
const __s_see_any_more = cptr.lit("see any more.");
const __s_for_the_first_time_in_your_life_you_can = cptr.lit("For the first time in your life, you can see!");
const __s_can_see = cptr.lit("can see!");
const __s_blindf_off_without_eyewear = cptr.lit("Blindf_off without eyewear?");
const __s_still_cannot_see = cptr.lit("still cannot see.");
const __s_see_anything_now = cptr.lit("see anything now!");
const __s_can_see_again = cptr.lit("can see again.");
const __s_you_stop_s_s = cptr.lit("You stop %s %s.");
const __s_putting_on = cptr.lit("putting on");
const __s_taking_off = cptr.lit("taking off");
const __s_are_not_wearing_that = cptr.lit("are not wearing that.");
const __s_and__2 = cptr.lit(" and ");
const __s_armor_or_accessory_off = cptr.lit("armor_or_accessory_off");
const __s_without_taking_off_your_s_first = cptr.lit(" without taking off your %s first");
const __s_it_s_embedded = cptr.lit("; it's embedded");
const __s_take_that_off_s = cptr.lit("take that off%s.");
const __s_removing_strange_accessory_s = cptr.lit("removing strange accessory: %s");
const __s_s_merged_with_your_skin = cptr.lit("%s merged with your skin!");
const __s_dragon_scales_are = cptr.lit("dragon scales are");
const __s_dragon_scale_mail_is = cptr.lit("dragon scale mail is");
const __s_not_wearing_any_armor_or_accessories = cptr.lit("Not wearing any armor or accessories.");
const __s_take_off = cptr.lit("take off");
const __s_not_wearing_any_accessories_or_armor = cptr.lit("Not wearing any accessories or armor.");
const __s_remove = cptr.lit("remove");
const __s_cursed_without_otmp = cptr.lit("cursed without otmp");
const __s_despite_your_slippery_s_you_can_t = cptr.lit("Despite your slippery %s, you can't.");
const __s_can_t_s_cursed = cptr.lit("can't.  %s cursed.");
const __s_they_are = cptr.lit("They are");
const __s_it_is = cptr.lit("It is");
const __s_disrobing = cptr.lit("disrobing");
const __s_taking_off_unknown_armor_d_d_delay_d = cptr.lit("Taking off unknown armor (%d: %d), delay %d");
const __s_armoroff = cptr.lit("armoroff");
const __s_you_finish_taking_off_your_s = cptr.lit("You finish taking off your %s.");
const __s_taking_off_unknown_armor_d_d_no_delay = cptr.lit("Taking off unknown armor (%d: %d), no delay");
const __s_are_already_wearing_s_c = cptr.lit("are already wearing %s%c");
const __s_wear_s_because_you_re_wearing_s_there = cptr.lit("wear %s because you're wearing %s there already.");
const __s_can_t_wear_any_armor_in_your_current = cptr.lit("can't wear any armor in your current form.");
const __s_s_will_not_fit_on_your_body = cptr.lit("%s will not fit on your body.");
const __s_cannot_do_that_while_holding_your_s = cptr.lit("cannot do that while holding your %s.");
const __s_s_won_t_fit_over_your_horn_s = cptr.lit("%s won't fit over your horn%s.");
const __s_s = cptr.lit("s");
const __s_cannot_wear_a_shield_while_wielding_a = cptr.lit("cannot wear a shield while wielding a two-handed %s.");
const __s_cannot_wear_a_shield_while_wielding_two = cptr.lit("cannot wear a shield while wielding two weapons.");
const __s_have_no_feet = cptr.lit("have no feet...");
const __s_have_too_many_hooves_to_wear_s = cptr.lit("have too many hooves to wear %s.");
const __s_s_is_trapped = cptr.lit("%s is trapped!");
const __s_s_are_stuck_in_the_s = cptr.lit("%s are stuck in the %s!");
const __s_s_is_attached_to_the_buried_ball = cptr.lit("%s is attached to the buried ball!");
const __s_cannot_wear_gloves_over_your_s = cptr.lit("cannot wear gloves over your %s.");
const __s_s_are_too_slippery_to_pull_on_s = cptr.lit("%s are too slippery to pull on %s.");
const __s_wear_that_over_your_s = cptr.lit("wear that over your %s.");
const __s_cannot_wear_armor_over_a_s = cptr.lit("cannot wear armor over a %s.");
const __s_some_armor = cptr.lit("some armor");
const __s_wear = cptr.lit("wear");
const __s_narrowly_avoid_losing_all_chance_at = cptr.lit("narrowly avoid losing all chance at your goal.");
const __s_are_suddenly_overcome_with_shame_and = cptr.lit("are suddenly overcome with shame and change your mind.");
const __s_cannot_make_the_ring_stick_to_your_body = cptr.lit("cannot make the ring stick to your body.");
const __s_are_no_more_s_s_to_fill = cptr.lit("are no more %s%s to fill.");
const __s_ring = cptr.lit("ring-");
const __s_which_s_s_right_or_left = cptr.lit("Which %s%s, Right or Left?");
const __s_s_are_too_slippery_to_remove_so_you = cptr.lit("%s are too slippery to remove, so you cannot put on the ring.");
const __s_cannot_remove_your_s_to_put_on_the_ring = cptr.lit("cannot remove your %s to put on the ring.");
const __s_cannot_free_your_weapon_s_to_put_on_the = cptr.lit("cannot free your weapon %s to put on the ring.");
const __s_an_amulet = cptr.lit("an amulet");
const __s_have_no_head_to_wear_s_on = cptr.lit("have no head to wear %s on.");
const __s_s_is_already_covered_by_a_towel = cptr.lit("%s is already covered by a towel.");
const __s_lenses = cptr.lit("lenses");
const __s_a_blindfold = cptr.lit("a blindfold");
const __s_some_lenses = cptr.lit("some lenses");
const __s_wear_that = cptr.lit("wear that!");
const __s_wearing_armor_not_worn_as_armor_08lx = cptr.lit("wearing armor not worn as armor? [%08lx]");
const __s_dressing_up = cptr.lit("dressing up");
const __s_you_finish_your_dressing_maneuver = cptr.lit("You finish your dressing maneuver.");
const __s_putting_on_unexpected_type_of_accessory = cptr.lit("putting on unexpected type of accessory: %s");
const __s_don_t_even_bother = cptr.lit("Don't even bother.");
const __s_are_already_wearing_a_full_complement = cptr.lit("are already wearing a full complement of armor.");
const __s_s_s_are_full_and_you_re_already_wearing = cptr.lit("%s%s are full, and you're already wearing an amulet and %s.");
const __s_put_on = cptr.lit("put on");
const __s_s_off_your_s = cptr.lit("%s off your %s.");
const __s_rings_slip = cptr.lit("rings slip");
const __s_ring_slips = cptr.lit("ring slips");
const __s_left = cptr.lit("left ");
const __s_right = cptr.lit("right ");
const __s_s_s_s_from_your_s_s = cptr.lit("%s %s%s from your %s%s.");
const __s_also = cptr.lit("also ");
const __s_slip = cptr.lit("slip");
const __s_food = cptr.lit("food");
const __s_s_s_s_s_s_from_your_s_s = cptr.lit("%s %s%s %s%s from your %s%s.");
const __s_corpse = cptr.lit("corpse");
const __s_the = cptr.lit("The");
const __s_your = cptr.lit("Your");
const __s_other = cptr.lit("other ");
const __s_some_armor__2 = cptr.lit("some_armor");
const __s_stuck_ring_neither_left_nor_right = cptr.lit("stuck_ring: neither left nor right?");
const __s_ring_is_stuck = cptr.lit("ring is stuck.");
const __s_free_a_weapon_s = cptr.lit("free a weapon %s");
const __s_take_off_your_s_s = cptr.lit("take off your %s%s");
const __s_slippery = cptr.lit("slippery ");
const __s_cannot_s_to_remove_the_ring = cptr.lit("cannot %s to remove the ring.");
const __s_are_unable_to_take_off_your_s_while = cptr.lit("are unable to take off your %s while wielding that %s.");
const __s_s_s_are_too_slippery_to_take_off = cptr.lit("%s %s are too slippery to take off.");
const __s_bear_trap_prevents_you_from_pulling = cptr.lit("bear trap prevents you from pulling your %s out.");
const __s_are_stuck_in_the_s_and_cannot_pull_your = cptr.lit("are stuck in the %s, and cannot pull your %s out.");
const __s_remove_your_s = cptr.lit("remove your %s");
const __s_release_your_s = cptr.lit("release your %s");
const __s_cannot_s_to_take_off_s = cptr.lit("cannot %s to take off %s.");
const __s_select_off_s = cptr.lit("select_off: %s???");
const __s_are_no_longer_wielding_either_weapon = cptr.lit("are no longer wielding either weapon.");
const __s_are_s = cptr.lit("are %s.");
const __s_sno_longer_s = cptr.lit("%sno longer %s.");
const __s_are = cptr.lit("are ");
const __s_wielding_two_weapons_at_once = cptr.lit("wielding two weapons at once");
const __s_have_a_second_weapon_readied = cptr.lit("have a second weapon readied");
const __s_no_longer_have_ammunition_readied = cptr.lit("no longer have ammunition readied.");
const __s_do_takeoff_taking_off_lx = cptr.lit("do_takeoff: taking off %lx");
const __s_finish_s = cptr.lit("finish %s.");
const __s_take_off_taking_off_lx = cptr.lit("take_off: taking off %lx");
const __s_better_not_take_that_off = cptr.lit("better_not_take_that_off");
const __s_take_off_your_s_despite_carrying_a_dead = cptr.lit("Take off your %s despite carrying a dead %s?");
const __s_continue_s = cptr.lit("continue %s.");
const __s_are_not_wearing_anything = cptr.lit("are not wearing anything.");
const __s_disarming = cptr.lit("disarming");
const __s_what_type_of_things_do_you_want_to_take = cptr.lit("What type of things do you want to take off?");
const __s_what_do_you_want_to_take_off = cptr.lit("What do you want to take off?");
const __s_is_nothing_else_you_can_remove_or = cptr.lit("is nothing else you can remove or unwield.");
const __s_your_s_crumbles_and_turns_to_dust = cptr.lit("Your %s crumbles and turns to dust!");
const __s_your_s_s_to_dust_and_s_to_the_s = cptr.lit("Your %s %s to dust and %s to the %s!");
const __s_turn = cptr.lit("turn");
const __s_fall = cptr.lit("fall");
const __s_your_s_crumbles_into_tiny_threads_and = cptr.lit("Your %s crumbles into tiny threads and falls apart!");
const __s_your_s_turns_to_dust_and_is_blown_away = cptr.lit("Your %s turns to dust and is blown away!");
const __s_your_s_vanish = cptr.lit("Your %s vanish!");
const __s_your_s_disintegrate = cptr.lit("Your %s disintegrate!");
const __s_your_s_crumbles_away = cptr.lit("Your %s crumbles away!");
const __s_you = cptr.lit("You");
const __s_destroy_arm = cptr.lit("destroy_arm");

/** C ref: do_wear.c:8 — char[13] */
const see_yourself = cptr.bytes("see yourself");

/** C ref: do_wear.c:9 — char[24] */
const unknown_type = cptr.bytes("Unknown type of %s (%d)");

/** C ref: do_wear.c:10 — char[6] */
const c_armor = cptr.bytes("armor");

/** C ref: do_wear.c:10 — char[5] */
const c_suit = cptr.bytes("suit");

/** C ref: do_wear.c:11 — char[6] */
const c_shirt = cptr.bytes("shirt");

/** C ref: do_wear.c:11 — char[6] */
const c_cloak = cptr.bytes("cloak");

/** C ref: do_wear.c:12 — char[7] */
const c_gloves = cptr.bytes("gloves");

/** C ref: do_wear.c:12 — char[6] */
const c_boots = cptr.bytes("boots");

/** C ref: do_wear.c:13 — char[7] */
const c_helmet = cptr.bytes("helmet");

/** C ref: do_wear.c:13 — char[7] */
const c_shield = cptr.bytes("shield");

/** C ref: do_wear.c:14 — char[7] */
const c_weapon = cptr.bytes("weapon");

/** C ref: do_wear.c:14 — char[6] */
const c_sword = cptr.bytes("sword");

/** C ref: do_wear.c:15 — char[4] */
const c_axe = cptr.bytes("axe");

/** C ref: do_wear.c:15 — char[5] */
const c_that_ = cptr.bytes("that");

/** C ref: do_wear.c:17 — long[15] */
const takeoff_order = cptr.alloc(15 * 8);
cptr.stI64o(takeoff_order, 0, 524288n);
cptr.stI64o(takeoff_order, 8, 256n);
cptr.stI64o(takeoff_order, 16, 8n);
cptr.stI64o(takeoff_order, 24, 16n);
cptr.stI64o(takeoff_order, 32, 131072n);
cptr.stI64o(takeoff_order, 40, 262144n);
cptr.stI64o(takeoff_order, 48, 2n);
cptr.stI64o(takeoff_order, 56, 4n);
cptr.stI64o(takeoff_order, 64, 65536n);
cptr.stI64o(takeoff_order, 72, 1n);
cptr.stI64o(takeoff_order, 80, 64n);
cptr.stI64o(takeoff_order, 88, 32n);
cptr.stI64o(takeoff_order, 96, 1024n);
cptr.stI64o(takeoff_order, 104, 512n);
cptr.stI64o(takeoff_order, 112, 0n);

/** C ref: do_wear.c:60 — @param {CInt} check_gloves @returns {CPtr<char>} */
export function* fingers_or_gloves(check_gloves) {
    return ((check_gloves && uarmg.v) ? (yield* gloves_simple_name(uarmg.v)) : (yield* makeplural((yield* body_part(NHC.FINGER)))));
}

/** C ref: do_wear.c:68 — @param {CPtr<struct obj>} otmp */
export function* off_msg(otmp) {
    if (cptr.ld1so(flags, $flag_verbose))
        (yield* You(__s_were_wearing_s, (yield* doname(otmp))));
}

/** C ref: do_wear.c:76 — @param {CPtr<struct obj>} otmp */
function* on_msg(otmp) {
    if ((cptr.ldI64o(otmp, $obj_owornmask) & 458752n) != 0n || ((cptr.ldI64o(otmp, $obj_owornmask) & 524288n) != 0n && !cptr.ld1so(flags, $flag_verbose))) {
        (yield* prinv((null), otmp, 0n));
        return;
    }
    if (cptr.ld1so(flags, $flag_verbose)) {
        let how = new Uint8Array(256);
        let otmp_name = (yield* xname(otmp));
        cptr.st1o(cptr.decay(how), 0, 0, 1);
        if (cptr.ldI16o(otmp, $obj_otyp) == NHC.TOWEL)
            void cptr.sprintf(cptr.decay(how), __s_around_your_s, (yield* body_part(NHC.HEAD)));
        (yield* You(__s_are_now_wearing_s_s, obj_is_pname(otmp) ? (yield* the(otmp_name)) : (yield* an(otmp_name)), cptr.decay(how)));
    }
}

/** C ref: do_wear.c:107 — @param {CPtr<struct obj>} obj @param {CLongLong} oldprop @param {CInt} on */
function* toggle_stealth(obj, oldprop, on) {
    if (on ? cptr.ld1so(gi, $instance_globals_i_initial_don) : cptr.ld1so(svc, $context_info_takeoff + $takeoff_info_cancelled_don))
        return;
    if (!oldprop && !HStealth() && !BStealth()) {
        if (cptr.ldI16o(obj, $obj_otyp) == NHC.RIN_STEALTH)
            (yield* learnring(obj, 1));
        else
            (yield* discover_object((cptr.ldI16o(obj, $obj_otyp)), 1, 1, 1));
        if (on) {
            if (!is_boots(obj))
                (yield* You(__s_move_very_quietly));
            else if (Levitation() || Flying())
                (yield* You(__s_float_imperceptibly));
            else
                (yield* You(__s_walk_very_quietly));
        } else {
            let riding = schar((!cptr.eq(cptr.ldPtro(u, $you_usteed), (null))));
            (yield* You(__s_s_s_are_noisy, riding ? __s_and : __s_sure, riding ? (yield* x_monnam(cptr.ldPtro(u, $you_usteed), NHM.ARTICLE_YOUR, (null), 12, 0)) : __s_empty));
        }
    }
}

/** C ref: do_wear.c:148 — @param {CPtr<struct obj>} obj @param {CLongLong} oldprop @param {CInt} on */
export function* toggle_displacement(obj, oldprop, on) {
    if (on ? cptr.ld1so(gi, $instance_globals_i_initial_don) : cptr.ld1so(svc, $context_info_takeoff + $takeoff_info_cancelled_don))
        return;
    if (!oldprop && !(cptr.ldI64o2(u, NHC.DISPLACED, 24, $you_uprops + $prop_intrinsic)) && !(cptr.ldI64o2(u, NHC.DISPLACED, 24, $you_uprops + $prop_blocked)) && ((!Blind() && !(cptr.ldI32o(u, $you_uswallow) & 1) && !Invisible()) || (Unblind_telepat() || (Blind_telepat() && Blind()) || Detect_monsters()))) {
        if (obj)
            (yield* discover_object((cptr.ldI16o(obj, $obj_otyp)), 1, 1, 1));
        (yield* You_feel(__s_that_monsters_s_have_difficulty, on ? __s_empty : __s_no_longer));
    }
}

/** C ref: do_wear.c:187 @returns {CInt} */
export function* Boots_on() {
    let oldprop = cptr.ldI64o2(u, cptr.ld1uo2(objects, cptr.ldI16o(uarmf.v, $obj_otyp), 120, $objclass_oc_oprop), 24, $you_uprops) & -33n;
    switch (cptr.ldI16o(uarmf.v, $obj_otyp)) {
        case NHC.LOW_BOOTS:
        case NHC.IRON_SHOES:
        case NHC.HIGH_BOOTS:
        case NHC.JUMPING_BOOTS:
        case NHC.KICKING_BOOTS:
        break;
        case NHC.WATER_WALKING_BOOTS:
        if ((cptr.ldI32o(u, $you_uinwater) & 1))
            (yield* spoteffects(1));
        if (cptr.ld1uo(gw, $instance_globals_w_wasinwater)) {
            if (!(cptr.ldI32o(u, $you_uinwater) & 1))
                (yield* discover_object(NHC.WATER_WALKING_BOOTS, 1, 1, 1));
            cptr.st1o(gw, $instance_globals_w_wasinwater, 0);
        }
        break;
        case NHC.SPEED_BOOTS:
        if (!oldprop && !(HFast() & 16777215n)) {
            (yield* discover_object((cptr.ldI16o(uarmf.v, $obj_otyp)), 1, 1, 1));
            (yield* You_feel(__s_yourself_speed_up_s, (oldprop || HFast()) ? __s_a_bit_more : __s_empty));
        }
        break;
        case NHC.ELVEN_BOOTS:
        (yield* toggle_stealth(uarmf.v, oldprop, 1));
        break;
        case NHC.FUMBLE_BOOTS:
        if (!oldprop && !(HFumbling() & -16777216n))
            incr_itimeout(cptr.add(cptr.add(cptr.add(u, $you_uprops), NHC.FUMBLING, 24), $prop_intrinsic), (rng_log_enabled() ? (rng_log_set_caller(__s_do_wear_c, 233, __s_boots_on), rnd(20)) : rnd(20)));
        break;
        case NHC.LEVITATION_BOOTS:
        if (!oldprop && !HLevitation() && !(BLevitation() & 67108864n)) {
            cptr.stI32o(uarmf.v, $obj_known, 1);
            cptr.st1(disp, 1);
            (yield* discover_object((cptr.ldI16o(uarmf.v, $obj_otyp)), 1, 1, 1));
            (yield* float_up());
            if (Levitation())
                (yield* spoteffects(0));
        } else {
            float_vs_flight();
        }
        break;
        default:
        (yield* impossible(cptr.decay(unknown_type), cptr.decay(c_boots), cptr.ldI16o(uarmf.v, $obj_otyp)));
    }
    if (uarmf.v && !(cptr.ldI32o(uarmf.v, $obj_known) & 1)) {
        cptr.stI32o(uarmf.v, $obj_known, 1);
        (yield* update_inventory());
    }
    return 0;
}

/** C ref: do_wear.c:262 @returns {CInt} */
export function* Boots_off() {
    let otmp = uarmf.v;
    let otyp = cptr.ldI16o(otmp, $obj_otyp);
    let oldprop = cptr.ldI64o2(u, cptr.ld1uo2(objects, otyp, 120, $objclass_oc_oprop), 24, $you_uprops) & -33n;
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) & (-33n));
    (yield* setworn(null, 32n));
    switch (otyp) {
        case NHC.SPEED_BOOTS:
        if (!Very_fast() && !cptr.ld1so(svc, $context_info_takeoff + $takeoff_info_cancelled_don)) {
            (yield* discover_object((otyp), 1, 1, 1));
            (yield* You_feel(__s_yourself_slow_down_s, Fast() ? __s_a_bit : __s_empty));
        }
        break;
        case NHC.WATER_WALKING_BOOTS:
        if ((is_pool(cptr.ldI16(u), cptr.ldI16o(u, $you_uy)) || is_lava(cptr.ldI16(u), cptr.ldI16o(u, $you_uy))) && !Levitation() && !Flying() && !(((cptr.ldU64o((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_mflags1) & 16n) != 0n) && has_ceiling(cptr.add(u, $you_uz))) && !cptr.ld1so(svc, $context_info_takeoff + $takeoff_info_cancelled_don) && !cptr.ldI32o(iflags, $instance_flags_in_lava_effects)) {
            (yield* discover_object((otyp), 1, 1, 1));
            (yield* spoteffects(1));
        }
        break;
        case NHC.ELVEN_BOOTS:
        (yield* toggle_stealth(otmp, oldprop, 0));
        break;
        case NHC.FUMBLE_BOOTS:
        if (!oldprop && !(HFumbling() & -16777216n))
            cptr.stI64o2(u, NHC.FUMBLING, 24, $you_uprops + $prop_intrinsic, cptr.stI64o2(u, NHC.FUMBLING, 24, $you_uprops, 0n));
        break;
        case NHC.LEVITATION_BOOTS:
        if (!oldprop && !HLevitation() && !(BLevitation() & 67108864n) && !cptr.ld1so(svc, $context_info_takeoff + $takeoff_info_cancelled_don)) {
            if (!cptr.ldI32o(iflags, $instance_flags_in_lava_effects))
                void (yield* float_down(0n, 0n));
            (yield* discover_object((otyp), 1, 1, 1));
        } else {
            float_vs_flight();
        }
        break;
        case NHC.LOW_BOOTS:
        case NHC.IRON_SHOES:
        case NHC.HIGH_BOOTS:
        case NHC.JUMPING_BOOTS:
        case NHC.KICKING_BOOTS:
        break;
        default:
        (yield* impossible(cptr.decay(unknown_type), cptr.decay(c_boots), otyp));
    }
    cptr.st1o(svc, $context_info_takeoff + $takeoff_info_cancelled_don, 0);
    return 0;
}

/** C ref: do_wear.c:326 @returns {CInt} */
function* Cloak_on() {
    let oldprop = cptr.ldI64o2(u, cptr.ld1uo2(objects, cptr.ldI16o(uarmc.v, $obj_otyp), 120, $objclass_oc_oprop), 24, $you_uprops) & -3n;
    switch (cptr.ldI16o(uarmc.v, $obj_otyp)) {
        case NHC.ORCISH_CLOAK:
        case NHC.DWARVISH_CLOAK:
        case NHC.CLOAK_OF_MAGIC_RESISTANCE:
        case NHC.ROBE:
        case NHC.LEATHER_CLOAK:
        break;
        case NHC.CLOAK_OF_PROTECTION:
        (yield* discover_object((cptr.ldI16o(uarmc.v, $obj_otyp)), 1, 1, 1));
        break;
        case NHC.ELVEN_CLOAK:
        (yield* toggle_stealth(uarmc.v, oldprop, 1));
        break;
        case NHC.CLOAK_OF_DISPLACEMENT:
        (yield* toggle_displacement(uarmc.v, oldprop, 1));
        break;
        case NHC.MUMMY_WRAPPING:
        if ((HInvis() || EInvis()) && !Blind()) {
            (yield* newsym(cptr.ldI16(u), cptr.ldI16o(u, $you_uy)));
            (yield* You(__s_can_s, See_invisible() ? __s_no_longer_see_through_yourself : cptr.decay(see_yourself)));
        }
        break;
        case NHC.CLOAK_OF_INVISIBILITY:
        if (!oldprop && !HInvis() && !Blind()) {
            (yield* discover_object((cptr.ldI16o(uarmc.v, $obj_otyp)), 1, 1, 1));
            (yield* newsym(cptr.ldI16(u), cptr.ldI16o(u, $you_uy)));
            (yield* pline(__s_suddenly_you_can_s_yourself, See_invisible() ? __s_see_through : __s_not_see));
        }
        break;
        case NHC.OILSKIN_CLOAK:
        (yield* pline(__s_s_very_tightly, (yield* Tobjnam(uarmc.v, __s_fit))));
        break;
        case NHC.ALCHEMY_SMOCK:
        cptr.stI64o2(u, NHC.ACID_RES, 24, $you_uprops, cptr.ldI64o2(u, NHC.ACID_RES, 24, $you_uprops) | 2n);
        break;
        default:
        (yield* impossible(cptr.decay(unknown_type), cptr.decay(c_cloak), cptr.ldI16o(uarmc.v, $obj_otyp)));
    }
    if (uarmc.v && !(cptr.ldI32o(uarmc.v, $obj_known) & 1)) {
        cptr.stI32o(uarmc.v, $obj_known, 1);
        (yield* update_inventory());
    }
    return 0;
}

/** C ref: do_wear.c:383 @returns {CInt} */
export function* Cloak_off() {
    let otmp = uarmc.v;
    let otyp = cptr.ldI16o(otmp, $obj_otyp);
    let oldprop = cptr.ldI64o2(u, cptr.ld1uo2(objects, otyp, 120, $objclass_oc_oprop), 24, $you_uprops) & -3n;
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) & (-3n));
    (yield* setworn(null, 2n));
    switch (otyp) {
        case NHC.ORCISH_CLOAK:
        case NHC.DWARVISH_CLOAK:
        case NHC.CLOAK_OF_PROTECTION:
        case NHC.CLOAK_OF_MAGIC_RESISTANCE:
        case NHC.OILSKIN_CLOAK:
        case NHC.ROBE:
        case NHC.LEATHER_CLOAK:
        break;
        case NHC.ELVEN_CLOAK:
        (yield* toggle_stealth(otmp, oldprop, 0));
        break;
        case NHC.CLOAK_OF_DISPLACEMENT:
        (yield* toggle_displacement(otmp, oldprop, 0));
        break;
        case NHC.MUMMY_WRAPPING:
        if (Invis() && !Blind()) {
            (yield* newsym(cptr.ldI16(u), cptr.ldI16o(u, $you_uy)));
            (yield* You(__s_can_s__2, See_invisible() ? __s_see_through_yourself : __s_no_longer_see_yourself));
        }
        break;
        case NHC.CLOAK_OF_INVISIBILITY:
        if (!oldprop && !HInvis() && !Blind()) {
            (yield* discover_object(NHC.CLOAK_OF_INVISIBILITY, 1, 1, 1));
            (yield* newsym(cptr.ldI16(u), cptr.ldI16o(u, $you_uy)));
            (yield* pline(__s_suddenly_you_can_s, See_invisible() ? __s_no_longer_see_through_yourself : cptr.decay(see_yourself)));
        }
        break;
        case NHC.ALCHEMY_SMOCK:
        cptr.stI64o2(u, NHC.ACID_RES, 24, $you_uprops, cptr.ldI64o2(u, NHC.ACID_RES, 24, $you_uprops) & (-3n));
        break;
        default:
        (yield* impossible(cptr.decay(unknown_type), cptr.decay(c_cloak), otyp));
    }
    return 0;
}

/** C ref: do_wear.c:434 @returns {CInt} */
function* Helmet_on() {
    switch (cptr.ldI16o(uarmh.v, $obj_otyp)) {
        case NHC.FEDORA:
        if ((cptr.ldI16o(gu, $instance_globals_u_urole + $Role_mnum) == NHC.PM_ARCHEOLOGIST))
            change_luck(1);
        break;
        case NHC.HELMET:
        case NHC.DENTED_POT:
        case NHC.ELVEN_LEATHER_HELM:
        case NHC.DWARVISH_IRON_HELM:
        case NHC.ORCISH_HELM:
        case NHC.HELM_OF_TELEPATHY:
        break;
        case NHC.HELM_OF_CAUTION:
        (yield* see_monsters());
        break;
        case NHC.HELM_OF_BRILLIANCE:
        (yield* adj_abon(uarmh.v, cptr.ld1so(uarmh.v, $obj_spe)));
        break;
        case NHC.CORNUTHAUM:
        cptr.st1o2(u, NHC.A_CHA, 1, $you_abon, cptr.ld1so2(u, NHC.A_CHA, 1, $you_abon) + ((cptr.ldI16o(gu, $instance_globals_u_urole + $Role_mnum) == NHC.PM_WIZARD) ? 1 : -1));
        cptr.st1(disp, 1);
        (yield* discover_object((cptr.ldI16o(uarmh.v, $obj_otyp)), 1, 1, 1));
        break;
        case NHC.HELM_OF_OPPOSITE_ALIGNMENT:
        cptr.stI32o(uarmh.v, $obj_known, 1);
        (yield* uchangealign((cptr.ld1so(u, $you_ualign) != NHM.A_NEUTRAL) ? -cptr.ld1so(u, $you_ualign) : ((u32mod(cptr.ldI32o(uarmh.v, $obj_o_id), 2)) ? -1 : NHM.A_LAWFUL), NHC.A_CG_HELM_ON));
        // @FallThrough
        ;
        case NHC.DUNCE_CAP:
        if (uarmh.v && !(cptr.ldI32o(uarmh.v, $obj_cursed) & 1)) {
            if (Blind())
                (yield* pline(__s_s_for_a_moment, (yield* Tobjnam(uarmh.v, __s_vibrate))));
            else
                (yield* pline(__s_s_s_for_a_moment, (yield* Tobjnam(uarmh.v, __s_glow)), hcolor(cptr.ldPtr(c_color_names))));
            (yield* curse(uarmh.v));
            if (Blind())
                (yield* set_bknown(uarmh.v, 0));
            else if ((cptr.ldI16o(gu, $instance_globals_u_urole + $Role_mnum) == NHC.PM_CLERIC))
                (yield* set_bknown(uarmh.v, 1));
            else if ((cptr.ldI32o(uarmh.v, $obj_bknown) & 1))
                (yield* update_inventory());
        }
        cptr.st1(disp, 1);
        if (Hallucination()) {
            (yield* pline(__s_my_brain_hurts));
        } else if (uarmh.v && cptr.ldI16o(uarmh.v, $obj_otyp) == NHC.DUNCE_CAP) {
            (yield* You_feel(__s_pct_s_dot, (acurr(NHC.A_INT)) <= (((((cptr.ld1so2(u, NHC.A_INT, 1, $you_acurr)) + (cptr.ld1so2(u, NHC.A_INT, 1, $you_abon))) | 0) + (cptr.ld1so2(u, NHC.A_INT, 1, $you_atemp))) | 0) ? __s_like_sitting_in_a_corner : __s_giddy));
        } else {
            (yield* discover_object(NHC.HELM_OF_OPPOSITE_ALIGNMENT, 1, 1, 1));
        }
        break;
        default:
        (yield* impossible(cptr.decay(unknown_type), cptr.decay(c_helmet), cptr.ldI16o(uarmh.v, $obj_otyp)));
    }
    if (uarmh.v && !(cptr.ldI32o(uarmh.v, $obj_known) & 1)) {
        cptr.stI32o(uarmh.v, $obj_known, 1);
        (yield* update_inventory());
    }
    return 0;
}

/** C ref: do_wear.c:518 @returns {CInt} */
export function* Helmet_off() {
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) & (-5n));
    switch (cptr.ldI16o(uarmh.v, $obj_otyp)) {
        case NHC.FEDORA:
        if ((cptr.ldI16o(gu, $instance_globals_u_urole + $Role_mnum) == NHC.PM_ARCHEOLOGIST))
            change_luck(-1);
        break;
        case NHC.HELMET:
        case NHC.DENTED_POT:
        case NHC.ELVEN_LEATHER_HELM:
        case NHC.DWARVISH_IRON_HELM:
        case NHC.ORCISH_HELM:
        break;
        case NHC.DUNCE_CAP:
        cptr.st1(disp, 1);
        break;
        case NHC.CORNUTHAUM:
        if (!cptr.ld1so(svc, $context_info_takeoff + $takeoff_info_cancelled_don)) {
            cptr.st1o2(u, NHC.A_CHA, 1, $you_abon, cptr.ld1so2(u, NHC.A_CHA, 1, $you_abon) + ((cptr.ldI16o(gu, $instance_globals_u_urole + $Role_mnum) == NHC.PM_WIZARD) ? -1 : 1));
            cptr.st1(disp, 1);
        }
        break;
        case NHC.HELM_OF_TELEPATHY:
        case NHC.HELM_OF_CAUTION:
        (yield* setworn(null, 4n));
        (yield* see_monsters());
        return 0;
        case NHC.HELM_OF_BRILLIANCE:
        if (!cptr.ld1so(svc, $context_info_takeoff + $takeoff_info_cancelled_don))
            (yield* adj_abon(uarmh.v, schar((-cptr.ld1so(uarmh.v, $obj_spe)))));
        break;
        case NHC.HELM_OF_OPPOSITE_ALIGNMENT:
        (yield* uchangealign(cptr.ld1so2(u, NHM.A_CURRENT, 1, $you_ualignbase), NHC.A_CG_HELM_OFF));
        break;
        default:
        (yield* impossible(cptr.decay(unknown_type), cptr.decay(c_helmet), cptr.ldI16o(uarmh.v, $obj_otyp)));
    }
    (yield* setworn(null, 4n));
    cptr.st1o(svc, $context_info_takeoff + $takeoff_info_cancelled_don, 0);
    return 0;
}

/** C ref: do_wear.c:568 — @param {CPtr<struct obj>} obj @returns {CInt} */
export function hard_helmet(obj) {
    if (!obj || !is_helmet(obj))
        return 0;
    return schar(((is_metallic(obj) || is_crackable(obj)) ? 1 : 0));
}

/** C ref: do_wear.c:576 @returns {CInt} */
function* Gloves_on() {
    let oldprop = cptr.ldI64o2(u, cptr.ld1uo2(objects, cptr.ldI16o(uarmg.v, $obj_otyp), 120, $objclass_oc_oprop), 24, $you_uprops) & -17n;
    switch (cptr.ldI16o(uarmg.v, $obj_otyp)) {
        case NHC.LEATHER_GLOVES:
        break;
        case NHC.GAUNTLETS_OF_FUMBLING:
        if (!oldprop && !(HFumbling() & -16777216n))
            incr_itimeout(cptr.add(cptr.add(cptr.add(u, $you_uprops), NHC.FUMBLING, 24), $prop_intrinsic), (rng_log_enabled() ? (rng_log_set_caller(__s_do_wear_c, 586, __s_gloves_on), rnd(20)) : rnd(20)));
        break;
        case NHC.GAUNTLETS_OF_POWER:
        (yield* discover_object((cptr.ldI16o(uarmg.v, $obj_otyp)), 1, 1, 1));
        cptr.st1(disp, 1);
        break;
        case NHC.GAUNTLETS_OF_DEXTERITY:
        (yield* adj_abon(uarmg.v, cptr.ld1so(uarmg.v, $obj_spe)));
        break;
        default:
        (yield* impossible(cptr.decay(unknown_type), cptr.decay(c_gloves), cptr.ldI16o(uarmg.v, $obj_otyp)));
    }
    if (!(cptr.ldI32o(uarmg.v, $obj_known) & 1)) {
        cptr.stI32o(uarmg.v, $obj_known, 1);
        (yield* update_inventory());
    }
    return 0;
}

/** C ref: do_wear.c:608 — @param {CPtr<struct obj>} obj @param {CPtr<struct obj>} how @param {CInt} voluntary */
export function* wielding_corpse(obj, how, voluntary) {
    if (!obj || cptr.ldI16o(obj, $obj_otyp) != NHC.CORPSE || uarmg.v)
        return;
    if (!cptr.eq(obj, uwep.v) && (!cptr.eq(obj, uswapwep.v) || !cptr.ld1so(u, $you_twoweap)))
        return;
    if (touch_petrifies(cptr.add(mons, cptr.ldI32o(obj, $obj_corpsenm), 96)) && !Stone_resistance()) {
        let kbuf = new Uint8Array(256);
        let hbuf = new Uint8Array(256);
        (yield* You(__s_s_s_in_your_bare_s, (how && is_gloves(how)) ? __s_now_wield : __s_are_wielding, (yield* corpse_xname(obj, null, NHM.CXN_ARTICLE)), (yield* makeplural((yield* body_part(NHC.HAND))))));
        if (how)
            void cptr.sprintf(cptr.decay(hbuf), __s_s_s, voluntary ? __s_removing : __s_losing, is_gloves(how) ? (yield* gloves_simple_name(how)) : strsubst((yield* simpleonames(how)), __s_set_of, __s_empty));
        else
            void cptr.strcpy(cptr.decay(hbuf), __s_resistance_timing_out);
        nh_snprintf(__s_wielding_corpse, 636, cptr.decay(kbuf), 256n, __s_s_while_wielding_s, cptr.decay(hbuf), (yield* killer_xname(obj)));
        (yield* instapetrify(cptr.decay(kbuf)));
        if (!Stone_resistance())
            (yield* remove_worn_item(obj, 0));
    }
}

/** C ref: do_wear.c:646 @returns {CInt} */
export function* Gloves_off() {
    let gloves = uarmg.v;
    let oldprop = cptr.ldI64o2(u, cptr.ld1uo2(objects, cptr.ldI16o(uarmg.v, $obj_otyp), 120, $objclass_oc_oprop), 24, $you_uprops) & -17n;
    let on_purpose = schar((!cptr.ld1so(svc, $context_info_mon_moving) && !(cptr.ldI32o(uarmg.v, $obj_in_use) & 1) ? 1 : 0));
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) & (-17n));
    switch (cptr.ldI16o(uarmg.v, $obj_otyp)) {
        case NHC.LEATHER_GLOVES:
        break;
        case NHC.GAUNTLETS_OF_FUMBLING:
        if (!oldprop && !(HFumbling() & -16777216n))
            cptr.stI64o2(u, NHC.FUMBLING, 24, $you_uprops + $prop_intrinsic, cptr.stI64o2(u, NHC.FUMBLING, 24, $you_uprops, 0n));
        break;
        case NHC.GAUNTLETS_OF_POWER:
        (yield* discover_object((cptr.ldI16o(uarmg.v, $obj_otyp)), 1, 1, 1));
        cptr.st1(disp, 1);
        break;
        case NHC.GAUNTLETS_OF_DEXTERITY:
        if (!cptr.ld1so(svc, $context_info_takeoff + $takeoff_info_cancelled_don))
            (yield* adj_abon(uarmg.v, schar((-cptr.ld1so(uarmg.v, $obj_spe)))));
        break;
        default:
        (yield* impossible(cptr.decay(unknown_type), cptr.decay(c_gloves), cptr.ldI16o(uarmg.v, $obj_otyp)));
    }
    (yield* setworn(null, 16n));
    cptr.st1o(svc, $context_info_takeoff + $takeoff_info_cancelled_don, 0);
    (yield* encumber_msg());
    if (Glib())
        (yield* make_glib(0));
    if (uwep.v && cptr.ldI16o(uwep.v, $obj_otyp) == NHC.CORPSE)
        (yield* wielding_corpse(uwep.v, gloves, on_purpose));
    if (cptr.ld1so(u, $you_twoweap) && uswapwep.v && cptr.ldI16o(uswapwep.v, $obj_otyp) == NHC.CORPSE)
        (yield* wielding_corpse(uswapwep.v, gloves, on_purpose));
    if (cptr.ld1so2(condtests, NHC.bl_bareh, 24, $condtests_t_enabled))
        cptr.st1(disp, 1);
    return 0;
}

/** C ref: do_wear.c:705 @returns {CInt} */
function* Shield_on() {
    switch (cptr.ldI16o(uarms.v, $obj_otyp)) {
        case NHC.SMALL_SHIELD:
        case NHC.SHIELD_OF_DRAIN_RESISTANCE:
        case NHC.SHIELD_OF_SHOCK_RESISTANCE:
        case NHC.ELVEN_SHIELD:
        case NHC.URUK_HAI_SHIELD:
        case NHC.ORCISH_SHIELD:
        case NHC.DWARVISH_ROUNDSHIELD:
        case NHC.LARGE_SHIELD:
        case NHC.SHIELD_OF_REFLECTION:
        break;
        default:
        (yield* impossible(cptr.decay(unknown_type), cptr.decay(c_shield), cptr.ldI16o(uarms.v, $obj_otyp)));
    }
    if (!(cptr.ldI32o(uarms.v, $obj_known) & 1)) {
        cptr.stI32o(uarms.v, $obj_known, 1);
        (yield* update_inventory());
    }
    return 0;
}

/** C ref: do_wear.c:733 @returns {CInt} */
export function* Shield_off() {
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) & (-9n));
    switch (cptr.ldI16o(uarms.v, $obj_otyp)) {
        case NHC.SMALL_SHIELD:
        case NHC.SHIELD_OF_DRAIN_RESISTANCE:
        case NHC.SHIELD_OF_SHOCK_RESISTANCE:
        case NHC.ELVEN_SHIELD:
        case NHC.URUK_HAI_SHIELD:
        case NHC.ORCISH_SHIELD:
        case NHC.DWARVISH_ROUNDSHIELD:
        case NHC.LARGE_SHIELD:
        case NHC.SHIELD_OF_REFLECTION:
        break;
        default:
        (yield* impossible(cptr.decay(unknown_type), cptr.decay(c_shield), cptr.ldI16o(uarms.v, $obj_otyp)));
    }
    (yield* setworn(null, 8n));
    return 0;
}

/** C ref: do_wear.c:759 @returns {CInt} */
function* Shirt_on() {
    switch (cptr.ldI16o(uarmu.v, $obj_otyp)) {
        case NHC.HAWAIIAN_SHIRT:
        case NHC.T_SHIRT:
        break;
        default:
        (yield* impossible(cptr.decay(unknown_type), cptr.decay(c_shirt), cptr.ldI16o(uarmu.v, $obj_otyp)));
    }
    if (!(cptr.ldI32o(uarmu.v, $obj_known) & 1)) {
        cptr.stI32o(uarmu.v, $obj_known, 1);
        (yield* update_inventory());
    }
    return 0;
}

/** C ref: do_wear.c:778 @returns {CInt} */
export function* Shirt_off() {
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) & (-65n));
    switch (cptr.ldI16o(uarmu.v, $obj_otyp)) {
        case NHC.HAWAIIAN_SHIRT:
        case NHC.T_SHIRT:
        break;
        default:
        (yield* impossible(cptr.decay(unknown_type), cptr.decay(c_shirt), cptr.ldI16o(uarmu.v, $obj_otyp)));
    }
    (yield* setworn(null, 64n));
    return 0;
}

/** C ref: do_wear.c:798 — @param {CPtr<struct obj>} otmp @param {CInt} puton @param {CInt} on_purpose */
function* dragon_armor_handling(otmp, puton, on_purpose) {
    if (!otmp)
        return;
    switch (cptr.ldI16o(otmp, $obj_otyp)) {
        case NHC.BLACK_DRAGON_SCALES:
        case NHC.BLACK_DRAGON_SCALE_MAIL:
        if (puton) {
            cptr.stI64o2(u, NHC.DRAIN_RES, 24, $you_uprops, cptr.ldI64o2(u, NHC.DRAIN_RES, 24, $you_uprops) | 1n);
        } else {
            cptr.stI64o2(u, NHC.DRAIN_RES, 24, $you_uprops, cptr.ldI64o2(u, NHC.DRAIN_RES, 24, $you_uprops) & (-2n));
        }
        break;
        case NHC.BLUE_DRAGON_SCALES:
        case NHC.BLUE_DRAGON_SCALE_MAIL:
        if (puton) {
            if (!Very_fast())
                (yield* You(__s_speed_up_s, Fast() ? __s_a_bit_more : __s_empty));
            cptr.stI64o2(u, NHC.FAST, 24, $you_uprops, cptr.ldI64o2(u, NHC.FAST, 24, $you_uprops) | 1n);
        } else {
            cptr.stI64o2(u, NHC.FAST, 24, $you_uprops, cptr.ldI64o2(u, NHC.FAST, 24, $you_uprops) & (-2n));
            if (!Very_fast() && !cptr.ld1so(svc, $context_info_takeoff + $takeoff_info_cancelled_don))
                (yield* You(__s_slow_down));
        }
        break;
        case NHC.GREEN_DRAGON_SCALES:
        case NHC.GREEN_DRAGON_SCALE_MAIL:
        if (puton) {
            cptr.stI64o2(u, NHC.SICK_RES, 24, $you_uprops, cptr.ldI64o2(u, NHC.SICK_RES, 24, $you_uprops) | 1n);
        } else {
            cptr.stI64o2(u, NHC.SICK_RES, 24, $you_uprops, cptr.ldI64o2(u, NHC.SICK_RES, 24, $you_uprops) & (-2n));
        }
        break;
        case NHC.RED_DRAGON_SCALES:
        case NHC.RED_DRAGON_SCALE_MAIL:
        if (puton) {
            cptr.stI64o2(u, NHC.INFRAVISION, 24, $you_uprops, cptr.ldI64o2(u, NHC.INFRAVISION, 24, $you_uprops) | 1n);
        } else {
            cptr.stI64o2(u, NHC.INFRAVISION, 24, $you_uprops, cptr.ldI64o2(u, NHC.INFRAVISION, 24, $you_uprops) & (-2n));
        }
        (yield* see_monsters());
        break;
        case NHC.GOLD_DRAGON_SCALES:
        case NHC.GOLD_DRAGON_SCALE_MAIL:
        void (yield* make_hallucinated(BigInt((!puton)), schar((cptr.ldI32o(program_state, $sinfo_restoring) ? 0 : 1)), 1n));
        break;
        case NHC.ORANGE_DRAGON_SCALES:
        case NHC.ORANGE_DRAGON_SCALE_MAIL:
        if (puton) {
            cptr.stI64o2(u, NHC.FREE_ACTION, 24, $you_uprops, cptr.ldI64o2(u, NHC.FREE_ACTION, 24, $you_uprops) | 1n);
        } else {
            cptr.stI64o2(u, NHC.FREE_ACTION, 24, $you_uprops, cptr.ldI64o2(u, NHC.FREE_ACTION, 24, $you_uprops) & (-2n));
        }
        break;
        case NHC.YELLOW_DRAGON_SCALES:
        case NHC.YELLOW_DRAGON_SCALE_MAIL:
        if (puton) {
            cptr.stI64o2(u, NHC.STONE_RES, 24, $you_uprops, cptr.ldI64o2(u, NHC.STONE_RES, 24, $you_uprops) | 1n);
        } else {
            cptr.stI64o2(u, NHC.STONE_RES, 24, $you_uprops, cptr.ldI64o2(u, NHC.STONE_RES, 24, $you_uprops) & (-2n));
            (yield* wielding_corpse(uwep.v, otmp, on_purpose));
            (yield* wielding_corpse(uswapwep.v, otmp, on_purpose));
        }
        break;
        case NHC.WHITE_DRAGON_SCALES:
        case NHC.WHITE_DRAGON_SCALE_MAIL:
        if (puton) {
            cptr.stI64o2(u, NHC.SLOW_DIGESTION, 24, $you_uprops, cptr.ldI64o2(u, NHC.SLOW_DIGESTION, 24, $you_uprops) | 1n);
        } else {
            cptr.stI64o2(u, NHC.SLOW_DIGESTION, 24, $you_uprops, cptr.ldI64o2(u, NHC.SLOW_DIGESTION, 24, $you_uprops) & (-2n));
        }
        break;
        default:
        break;
    }
}

/** C ref: do_wear.c:887 @returns {CInt} */
function* Armor_on() {
    if (!uarm.v)
        return 0;
    if (!(cptr.ldI32o(uarm.v, $obj_known) & 1)) {
        cptr.stI32o(uarm.v, $obj_known, 1);
        (yield* update_inventory());
    }
    (yield* dragon_armor_handling(uarm.v, 1, 1));
    if (artifact_light(uarm.v) && !(cptr.ldI32o(uarm.v, $obj_lamplit) & 1)) {
        (yield* begin_burn(uarm.v, 0));
        if (!Blind())
            (yield* pline(__s_s_s_to_shine_s, (yield* Yname2(uarm.v)), (yield* otense(uarm.v, __s_begin)), arti_light_description(uarm.v)));
    }
    return 0;
}

/** C ref: do_wear.c:909 @returns {CInt} */
export function* Armor_off() {
    let otmp = uarm.v;
    let was_arti_light = schar((otmp && (cptr.ldI32o(otmp, $obj_lamplit) & 1) | 0 && artifact_light(otmp) ? 1 : 0));
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) & (-2n));
    (yield* setworn(null, 1n));
    cptr.st1o(svc, $context_info_takeoff + $takeoff_info_cancelled_don, 0);
    if (was_arti_light && !artifact_light(otmp)) {
        (yield* end_burn(otmp, 0));
        if (!Blind())
            (yield* pline(__s_s_shining, (yield* Tobjnam(otmp, __s_stop))));
    }
    (yield* dragon_armor_handling(otmp, 0, 1));
    return 0;
}

/** C ref: do_wear.c:939 @returns {CInt} */
export function* Armor_gone() {
    let otmp = uarm.v;
    let was_arti_light = schar((otmp && (cptr.ldI32o(otmp, $obj_lamplit) & 1) | 0 && artifact_light(otmp) ? 1 : 0));
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) & (-2n));
    (yield* setnotworn(uarm.v));
    cptr.st1o(svc, $context_info_takeoff + $takeoff_info_cancelled_don, 0);
    if (was_arti_light && !artifact_light(otmp)) {
        (yield* end_burn(otmp, 0));
        if (!Blind())
            (yield* pline(__s_s_shining, (yield* Tobjnam(otmp, __s_stop))));
    }
    (yield* dragon_armor_handling(otmp, 0, 0));
    return 0;
}

/** C ref: do_wear.c:963 — @param {CPtr<struct obj>} amul */
function* Amulet_on(amul) {
    let on_msg_done = 0;
    (yield* remove_worn_item(amul, 0));
    (yield* setworn(amul, 65536n));
    switch (cptr.ldI16o(uamul.v, $obj_otyp)) {
        case NHC.AMULET_OF_ESP:
        case NHC.AMULET_OF_LIFE_SAVING:
        case NHC.AMULET_VERSUS_POISON:
        case NHC.AMULET_OF_REFLECTION:
        case NHC.FAKE_AMULET_OF_YENDOR:
        break;
        case NHC.AMULET_OF_MAGICAL_BREATHING:
        {
            let was_in_poison_gas;
            cptr.stI64o2(u, NHC.MAGICAL_BREATHING, 24, $you_uprops, cptr.ldI64o2(u, NHC.MAGICAL_BREATHING, 24, $you_uprops) & (-65537n));
            was_in_poison_gas = region_danger();
            cptr.stI64o2(u, NHC.MAGICAL_BREATHING, 24, $you_uprops, cptr.ldI64o2(u, NHC.MAGICAL_BREATHING, 24, $you_uprops) | 65536n);
            if (was_in_poison_gas) {
                (yield* discover_object(NHC.AMULET_OF_MAGICAL_BREATHING, 1, 1, 1));
                (yield* on_msg(uamul.v));
                on_msg_done = 1;
                (yield* You(__s_are_no_longer_bothered_by_the_poison_gas));
            }
            break;
        }
        case NHC.AMULET_OF_UNCHANGING:
        if (Slimed())
            (yield* make_slimed(0n, null));
        break;
        case NHC.AMULET_OF_CHANGE:
        {
            let call_it = 0;
            let new_sex;
            let orig_sex = poly_gender();
            if (!Unchanging())
                (yield* change_sex());
            new_sex = poly_gender();
            if (new_sex != orig_sex)
                (yield* discover_object(NHC.AMULET_OF_CHANGE, 1, 1, 1));
            (yield* on_msg(uamul.v));
            on_msg_done = 1;
            if (new_sex != orig_sex) {
                (yield* newsym(cptr.ldI16(u), cptr.ldI16o(u, $you_uy)));
                cptr.st1(disp, 1);
                (yield* You(__s_are_suddenly_very_s, cptr.ld1so(flags, $flag_female) ? __s_feminine : __s_masculine));
            } else {
                (yield* You(__s_don_t_feel_like_yourself));
                call_it = schar((((cptr.ldI32o(uamul.v, $obj_dknown) & 1) | 0) != 0));
            }
            (yield* livelog_newform(0, orig_sex, new_sex));
            (yield* pline_The(__s_amulet_disintegrates));
            if (call_it)
                (yield* trycall(uamul.v));
            (yield* useup(uamul.v));
            break;
        }
        case NHC.AMULET_OF_STRANGULATION:
        if ((yield* can_be_strangled(cptr.add(gy, $instance_globals_y_youmonst))) && !Strangled()) {
            (yield* discover_object(NHC.AMULET_OF_STRANGULATION, 1, 1, 1));
            cptr.stI64o2(u, NHC.STRANGLED, 24, $you_uprops + $prop_intrinsic, 6n);
            cptr.st1(disp, 1);
            (yield* on_msg(uamul.v));
            on_msg_done = 1;
            (yield* pline(__s_it_constricts_your_throat));
        }
        break;
        case NHC.AMULET_OF_RESTFUL_SLEEP:
        {
            let newnap = BigInt.asIntN(64, BigInt((rng_log_enabled() ? (rng_log_set_caller(__s_do_wear_c, 1048, __s_amulet_on), rnd(98)) : rnd(98))) + 2n);
            let oldnap = (HSleepy() & 16777215n);
            if (newnap < oldnap || oldnap == 0n)
                cptr.stI64o2(u, NHC.SLEEPY, 24, $you_uprops + $prop_intrinsic, (HSleepy() & -16777216n) | newnap);
            break;
        }
        case NHC.AMULET_OF_FLYING:
        float_vs_flight();
        if (Flying()) {
            let already_flying;
            cptr.stI64o2(u, NHC.FLYING, 24, $you_uprops, cptr.ldI64o2(u, NHC.FLYING, 24, $you_uprops) & (-65537n));
            already_flying = schar((!!Flying()));
            cptr.stI64o2(u, NHC.FLYING, 24, $you_uprops, cptr.ldI64o2(u, NHC.FLYING, 24, $you_uprops) | 65536n);
            if (!already_flying) {
                (yield* discover_object(NHC.AMULET_OF_FLYING, 1, 1, 1));
                (yield* on_msg(uamul.v));
                on_msg_done = 1;
                cptr.st1(disp, 1);
                (yield* You(__s_are_now_in_flight));
            }
        }
        break;
        case NHC.AMULET_OF_GUARDING:
        (yield* discover_object(NHC.AMULET_OF_GUARDING, 1, 1, 1));
        find_ac();
        break;
        case NHC.AMULET_OF_YENDOR:
        break;
    }
    if (!on_msg_done)
        (yield* on_msg(uamul.v));
}

/** C ref: do_wear.c:1090 */
export function* Amulet_off() {
    let amul = uamul.v;
    let mkn = 0;
    let early_off_msg = 0;
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) & (-65537n));
    switch (cptr.ldI16o(uamul.v, $obj_otyp)) {
        case NHC.AMULET_OF_ESP:
        (yield* setworn(null, 65536n));
        (yield* off_msg(amul));
        early_off_msg = 1;
        (yield* see_monsters());
        break;
        case NHC.AMULET_OF_LIFE_SAVING:
        case NHC.AMULET_VERSUS_POISON:
        case NHC.AMULET_OF_REFLECTION:
        case NHC.AMULET_OF_CHANGE:
        case NHC.AMULET_OF_UNCHANGING:
        case NHC.FAKE_AMULET_OF_YENDOR:
        break;
        case NHC.AMULET_OF_MAGICAL_BREATHING:
        (yield* setworn(null, 65536n));
        (yield* off_msg(amul));
        early_off_msg = 1;
        if (Underwater()) {
            if (!cant_drown(cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)) && !Swimming()) {
                (yield* You(__s_suddenly_inhale_an_unhealthy_amount_of_s, hliquid(__s_water)));
                mkn = 1;
                void (yield* drown());
            }
        }
        if (region_danger()) {
            (yield* You(__s_are_breathing_poison_gas));
            mkn = 1;
        }
        break;
        case NHC.AMULET_OF_STRANGULATION:
        (yield* setworn(null, 65536n));
        (yield* off_msg(amul));
        early_off_msg = 1;
        if (Strangled()) {
            cptr.stI64o2(u, NHC.STRANGLED, 24, $you_uprops + $prop_intrinsic, 0n);
            cptr.st1(disp, 1);
            if (Breathless())
                (yield* Your(__s_s_is_no_longer_constricted, (yield* body_part(NHC.NECK))));
            else
                (yield* You(__s_can_breathe_more_easily));
            mkn = 1;
        }
        break;
        case NHC.AMULET_OF_RESTFUL_SLEEP:
        (yield* setworn(null, 65536n));
        if (!ESleepy() && !(HSleepy() & -16777216n))
            cptr.stI64o2(u, NHC.SLEEPY, 24, $you_uprops + $prop_intrinsic, cptr.ldI64o2(u, NHC.SLEEPY, 24, $you_uprops + $prop_intrinsic) & (-16777216n));
        break;
        case NHC.AMULET_OF_FLYING:
        {
            let was_flying = schar((!!Flying()));
            (yield* setworn(null, 65536n));
            (yield* off_msg(amul));
            early_off_msg = 1;
            float_vs_flight();
            if (was_flying && !Flying()) {
                cptr.st1(disp, 1);
                (yield* You(__s_pct_s_dot, (is_pool_or_lava(cptr.ldI16(u), cptr.ldI16o(u, $you_uy)) || (((cptr.ldI16o((cptr.add(svd, $instance_globals_saved_d_dungeon_topology + $dgn_topology_d_water_level)), $d_level_dlevel) || cptr.ldI16((cptr.add(svd, $instance_globals_saved_d_dungeon_topology + $dgn_topology_d_water_level)))) && on_level(cptr.add(u, $you_uz), cptr.add(svd, $instance_globals_saved_d_dungeon_topology + $dgn_topology_d_water_level)))) || (((cptr.ldI16o((cptr.add(svd, $instance_globals_saved_d_dungeon_topology + $dgn_topology_d_air_level)), $d_level_dlevel) || cptr.ldI16((cptr.add(svd, $instance_globals_saved_d_dungeon_topology + $dgn_topology_d_air_level)))) && on_level(cptr.add(u, $you_uz), cptr.add(svd, $instance_globals_saved_d_dungeon_topology + $dgn_topology_d_air_level))))) ? __s_stop_flying : __s_land));
                mkn = 1;
                (yield* spoteffects(1));
            }
            break;
        }
        case NHC.AMULET_OF_GUARDING:
        find_ac();
        break;
        case NHC.AMULET_OF_YENDOR:
        break;
    }
    (yield* setworn(null, 65536n));
    if (!early_off_msg)
        (yield* off_msg(amul));
    if (mkn)
        (yield* discover_object((cptr.ldI16o(amul, $obj_otyp)), 1, 1, 1));
    return;
}

/** C ref: do_wear.c:1193 — @param {CPtr<struct obj>} ring @param {CInt} observed */
function* learnring(ring, observed) {
    let ringtype = cptr.ldI16o(ring, $obj_otyp);
    if (observed) {
        if ((cptr.ldI32o2(objects, ringtype, 120, $objclass_oc_name_known) & 1))
            (yield* observe_object(ring));
        else if ((cptr.ldI32o(ring, $obj_dknown) & 1))
            (yield* discover_object((ringtype), 1, 1, 1));
    }
    if ((cptr.ldI32o(ring, $obj_dknown) & 1) | 0 && (cptr.ldI32o2(objects, ringtype, 120, $objclass_oc_name_known) & 1) | 0) {
        if ((cptr.ldI32o2(objects, ringtype, 120, $objclass_oc_charged) & 1))
            cptr.stI32o(ring, $obj_known, 1);
        (yield* update_inventory());
    }
}

/** C ref: do_wear.c:1223 — @param {CPtr<struct obj>} obj @param {CInt} which @param {CInt} val */
function* adjust_attrib(obj, which, val) {
    let old_attrib;
    let observable;
    old_attrib = (acurr(which));
    cptr.st1o2(u, which, 1, $you_abon, cptr.ld1so2(u, which, 1, $you_abon) + val);
    observable = schar((old_attrib != (acurr(which))));
    if (observable || !extremeattr(which))
        (yield* learnring(obj, observable));
    cptr.st1(disp, 1);
}

/** C ref: do_wear.c:1242 — @param {CPtr<struct obj>} obj */
export function* Ring_on(obj) {
    let oldprop = cptr.ldI64o2(u, cptr.ld1uo2(objects, cptr.ldI16o(obj, $obj_otyp), 120, $objclass_oc_oprop), 24, $you_uprops);
    let observable;
    if (cptr.eq(obj, uwep.v))
        (yield* setuwep(null));
    else if (cptr.eq(obj, uswapwep.v))
        (yield* setuswapwep(null));
    else if (cptr.eq(obj, uquiver.v))
        (yield* setuqwep(null));
    if ((oldprop & 393216n) != 393216n)
        oldprop &= -393217n;
    switch (cptr.ldI16o(obj, $obj_otyp)) {
        case NHC.RIN_TELEPORTATION:
        case NHC.RIN_REGENERATION:
        case NHC.RIN_SEARCHING:
        case NHC.RIN_HUNGER:
        case NHC.RIN_AGGRAVATE_MONSTER:
        case NHC.RIN_POISON_RESISTANCE:
        case NHC.RIN_FIRE_RESISTANCE:
        case NHC.RIN_COLD_RESISTANCE:
        case NHC.RIN_SHOCK_RESISTANCE:
        case NHC.RIN_CONFLICT:
        case NHC.RIN_TELEPORT_CONTROL:
        case NHC.RIN_POLYMORPH:
        case NHC.RIN_POLYMORPH_CONTROL:
        case NHC.RIN_FREE_ACTION:
        case NHC.RIN_SLOW_DIGESTION:
        case NHC.RIN_SUSTAIN_ABILITY:
        break;
        case NHC.MEAT_RING:
        break;
        case NHC.RIN_STEALTH:
        (yield* toggle_stealth(obj, oldprop, 1));
        break;
        case NHC.RIN_WARNING:
        (yield* see_monsters());
        break;
        case NHC.RIN_SEE_INVISIBLE:
        (yield* set_mimic_blocking());
        (yield* see_monsters());
        if (Invis() && !oldprop && !HSee_invisible() && !Blind()) {
            (yield* newsym(cptr.ldI16(u), cptr.ldI16o(u, $you_uy)));
            (yield* pline(__s_suddenly_you_are_transparent_but_there));
            (yield* learnring(obj, 1));
        }
        break;
        case NHC.RIN_INVISIBILITY:
        if (!oldprop && !HInvis() && !BInvis() && !Blind()) {
            (yield* learnring(obj, 1));
            (yield* newsym(cptr.ldI16(u), cptr.ldI16o(u, $you_uy)));
            (yield* self_invis_message());
        }
        break;
        case NHC.RIN_LEVITATION:
        if (!oldprop && !HLevitation() && !(BLevitation() & 67108864n)) {
            (yield* float_up());
            (yield* learnring(obj, 1));
            if (Levitation())
                (yield* spoteffects(0));
        } else {
            float_vs_flight();
        }
        break;
        case NHC.RIN_GAIN_STRENGTH:
        (yield* adjust_attrib(obj, NHC.A_STR, cptr.ld1so(obj, $obj_spe)));
        break;
        case NHC.RIN_GAIN_CONSTITUTION:
        (yield* adjust_attrib(obj, NHC.A_CON, cptr.ld1so(obj, $obj_spe)));
        break;
        case NHC.RIN_ADORNMENT:
        (yield* adjust_attrib(obj, NHC.A_CHA, cptr.ld1so(obj, $obj_spe)));
        break;
        case NHC.RIN_INCREASE_ACCURACY:
        cptr.st1o(u, $you_uhitinc, cptr.ld1so(u, $you_uhitinc) + cptr.ld1so(obj, $obj_spe));
        break;
        case NHC.RIN_INCREASE_DAMAGE:
        cptr.st1o(u, $you_udaminc, cptr.ld1so(u, $you_udaminc) + cptr.ld1so(obj, $obj_spe));
        break;
        case NHC.RIN_PROTECTION_FROM_SHAPE_CHAN:
        (yield* rescham());
        break;
        case NHC.RIN_PROTECTION:
        observable = schar((cptr.ld1so(obj, $obj_spe) != 0));
        (yield* learnring(obj, observable));
        if (cptr.ld1so(obj, $obj_spe))
            find_ac();
        break;
    }
}

/** C ref: do_wear.c:1347 — @param {CPtr<struct obj>} obj @param {CInt} gone */
function* Ring_off_or_gone(obj, gone) {
    let mask = (cptr.ldI64o(obj, $obj_owornmask) & 393216n);
    let observable;
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) & BigInt.asIntN(64, ~mask));
    if (!(cptr.ldI64o2(u, cptr.ld1uo2(objects, cptr.ldI16o(obj, $obj_otyp), 120, $objclass_oc_oprop), 24, $you_uprops) & mask))
        (yield* impossible(__s_strange_i_didn_t_know_you_had_that_ring));
    if (gone)
        (yield* setnotworn(obj));
    else
        (yield* setworn(null, cptr.ldI64o(obj, $obj_owornmask)));
    switch (cptr.ldI16o(obj, $obj_otyp)) {
        case NHC.RIN_TELEPORTATION:
        case NHC.RIN_REGENERATION:
        case NHC.RIN_SEARCHING:
        case NHC.RIN_HUNGER:
        case NHC.RIN_AGGRAVATE_MONSTER:
        case NHC.RIN_POISON_RESISTANCE:
        case NHC.RIN_FIRE_RESISTANCE:
        case NHC.RIN_COLD_RESISTANCE:
        case NHC.RIN_SHOCK_RESISTANCE:
        case NHC.RIN_CONFLICT:
        case NHC.RIN_TELEPORT_CONTROL:
        case NHC.RIN_POLYMORPH:
        case NHC.RIN_POLYMORPH_CONTROL:
        case NHC.RIN_FREE_ACTION:
        case NHC.RIN_SLOW_DIGESTION:
        case NHC.RIN_SUSTAIN_ABILITY:
        case NHC.MEAT_RING:
        break;
        case NHC.RIN_STEALTH:
        (yield* toggle_stealth(obj, (EStealth() & BigInt.asIntN(64, ~mask)), 0));
        break;
        case NHC.RIN_WARNING:
        (yield* see_monsters());
        break;
        case NHC.RIN_SEE_INVISIBLE:
        if (!See_invisible()) {
            (yield* set_mimic_blocking());
            (yield* see_monsters());
        }
        if (Invisible() && !Blind()) {
            (yield* newsym(cptr.ldI16(u), cptr.ldI16o(u, $you_uy)));
            (yield* pline(__s_suddenly_you_cannot_see_yourself));
            (yield* learnring(obj, 1));
        }
        break;
        case NHC.RIN_INVISIBILITY:
        if (!Invis() && !BInvis() && !Blind()) {
            (yield* newsym(cptr.ldI16(u), cptr.ldI16o(u, $you_uy)));
            (yield* Your(__s_body_seems_to_unfade_s, See_invisible() ? __s_completely : __s_dot2));
            (yield* learnring(obj, 1));
        }
        break;
        case NHC.RIN_LEVITATION:
        if (!(BLevitation() & 67108864n)) {
            void (yield* float_down(0n, 0n));
            if (!Levitation())
                (yield* learnring(obj, 1));
        } else {
            float_vs_flight();
        }
        break;
        case NHC.RIN_GAIN_STRENGTH:
        (yield* adjust_attrib(obj, NHC.A_STR, -cptr.ld1so(obj, $obj_spe)));
        break;
        case NHC.RIN_GAIN_CONSTITUTION:
        (yield* adjust_attrib(obj, NHC.A_CON, -cptr.ld1so(obj, $obj_spe)));
        break;
        case NHC.RIN_ADORNMENT:
        (yield* adjust_attrib(obj, NHC.A_CHA, -cptr.ld1so(obj, $obj_spe)));
        break;
        case NHC.RIN_INCREASE_ACCURACY:
        cptr.st1o(u, $you_uhitinc, cptr.ld1so(u, $you_uhitinc) - cptr.ld1so(obj, $obj_spe));
        break;
        case NHC.RIN_INCREASE_DAMAGE:
        cptr.st1o(u, $you_udaminc, cptr.ld1so(u, $you_udaminc) - cptr.ld1so(obj, $obj_spe));
        break;
        case NHC.RIN_PROTECTION:
        observable = schar((cptr.ld1so(obj, $obj_spe) != 0));
        (yield* learnring(obj, observable));
        if (cptr.ld1so(obj, $obj_spe))
            find_ac();
        break;
        case NHC.RIN_PROTECTION_FROM_SHAPE_CHAN:
        if (!Protection_from_shape_changers())
            (yield* restartcham());
        break;
    }
}

/** C ref: do_wear.c:1449 — @param {CPtr<struct obj>} obj */
export function* Ring_off(obj) {
    (yield* Ring_off_or_gone(obj, 0));
}

/** C ref: do_wear.c:1455 — @param {CPtr<struct obj>} obj */
export function* Ring_gone(obj) {
    (yield* Ring_off_or_gone(obj, 1));
}

/** C ref: do_wear.c:1461 — @param {CPtr<struct obj>} otmp */
export function* Blindf_on(otmp) {
    let already_blind = schar(((cptr.ldI64o2(u, NHC.BLINDED, 24, $you_uprops + $prop_intrinsic) || cptr.ldI64o2(u, NHC.BLINDED, 24, $you_uprops)) && !cptr.ldI64o2(u, NHC.BLINDED, 24, $you_uprops + $prop_blocked) ? 1 : 0));
    let changed = 0;
    (yield* remove_worn_item(otmp, 0));
    (yield* setworn(otmp, 524288n));
    (yield* on_msg(otmp));
    if (Blind() && !already_blind) {
        changed = 1;
        if (cptr.ld1so(flags, $flag_verbose))
            (yield* You_cant(__s_see_any_more));
        if (Punished())
            (yield* set_bc(0));
    } else if (already_blind && !Blind()) {
        changed = 1;
        if (cptr.ld1so(u, $you_uroleplay)) {
            (yield* pline(__s_for_the_first_time_in_your_life_you_can));
            cptr.st1o(u, $you_uroleplay, 0);
        } else
            (yield* You(__s_can_see));
    }
    if (changed) {
        (yield* toggle_blindness());
    }
}

/** C ref: do_wear.c:1495 — @param {CPtr<struct obj>} otmp */
export function* Blindf_off(otmp) {
    let was_blind = schar(((cptr.ldI64o2(u, NHC.BLINDED, 24, $you_uprops + $prop_intrinsic) || cptr.ldI64o2(u, NHC.BLINDED, 24, $you_uprops)) && !cptr.ldI64o2(u, NHC.BLINDED, 24, $you_uprops + $prop_blocked) ? 1 : 0));
    let changed = 0;
    let nooffmsg = schar((!otmp));
    if (!otmp)
        otmp = ublindf.v;
    if (!otmp) {
        (yield* impossible(__s_blindf_off_without_eyewear));
        return;
    }
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) & (-524289n));
    (yield* setworn(null, cptr.ldI64o(otmp, $obj_owornmask)));
    if (!nooffmsg)
        (yield* off_msg(otmp));
    if (Blind()) {
        if (was_blind) {
            if (cptr.ldI16o(otmp, $obj_otyp) != NHC.LENSES)
                (yield* You(__s_still_cannot_see));
        } else {
            changed = 1;
            (yield* You_cant(__s_see_anything_now));
            if (Punished())
                (yield* set_bc(0));
        }
    } else if (was_blind) {
        if (!(yield* gulp_blnd_check())) {
            changed = 1;
            (yield* You(__s_can_see_again));
        }
    }
    if (changed) {
        (yield* toggle_blindness());
    }
}

/** C ref: do_wear.c:1539 — @param {CPtr<struct obj>} obj */
export function* set_wear(obj) {
    cptr.st1o(gi, $instance_globals_i_initial_don, schar((!obj)));
    if (!obj ? ublindf.v !== null : (cptr.eq(obj, ublindf.v)))
        void (yield* Blindf_on(ublindf.v));
    if (!obj ? uright.v !== null : (cptr.eq(obj, uright.v)))
        void (yield* Ring_on(uright.v));
    if (!obj ? uleft.v !== null : (cptr.eq(obj, uleft.v)))
        void (yield* Ring_on(uleft.v));
    if (!obj ? uamul.v !== null : (cptr.eq(obj, uamul.v)))
        void (yield* Amulet_on(uamul.v));
    if (!obj ? uarmu.v !== null : (cptr.eq(obj, uarmu.v)))
        void (yield* Shirt_on());
    if (!obj ? uarm.v !== null : (cptr.eq(obj, uarm.v)))
        void (yield* Armor_on());
    if (!obj ? uarmc.v !== null : (cptr.eq(obj, uarmc.v)))
        void (yield* Cloak_on());
    if (!obj ? uarmf.v !== null : (cptr.eq(obj, uarmf.v)))
        void (yield* Boots_on());
    if (!obj ? uarmg.v !== null : (cptr.eq(obj, uarmg.v)))
        void (yield* Gloves_on());
    if (!obj ? uarmh.v !== null : (cptr.eq(obj, uarmh.v)))
        void (yield* Helmet_on());
    if (!obj ? uarms.v !== null : (cptr.eq(obj, uarms.v)))
        void (yield* Shield_on());
    cptr.st1o(gi, $instance_globals_i_initial_don, 0);
}

/** C ref: do_wear.c:1574 — @param {CPtr<struct obj>} otmp @returns {CInt} */
export function donning(otmp) {
    let result = 0;
    if (doffing(otmp))
        result = 1;
    else if (cptr.eq(otmp, uarm.v))
        result = schar((cptr.ldPtr(ga) === Armor_on));
    else if (cptr.eq(otmp, uarmu.v))
        result = schar((cptr.ldPtr(ga) === Shirt_on));
    else if (cptr.eq(otmp, uarmc.v))
        result = schar((cptr.ldPtr(ga) === Cloak_on));
    else if (cptr.eq(otmp, uarmf.v))
        result = schar((cptr.ldPtr(ga) === Boots_on));
    else if (cptr.eq(otmp, uarmh.v))
        result = schar((cptr.ldPtr(ga) === Helmet_on));
    else if (cptr.eq(otmp, uarmg.v))
        result = schar((cptr.ldPtr(ga) === Gloves_on));
    else if (cptr.eq(otmp, uarms.v))
        result = schar((cptr.ldPtr(ga) === Shield_on));
    return result;
}

/** C ref: do_wear.c:1603 — @param {CPtr<struct obj>} otmp @returns {CInt} */
export function doffing(otmp) {
    let what = cptr.ldI64o(svc, $context_info_takeoff + $takeoff_info_what);
    let result = 0;
    if (cptr.eq(otmp, uarm.v))
        result = schar((cptr.ldPtr(ga) === Armor_off || what == 1n ? 1 : 0));
    else if (cptr.eq(otmp, uarmu.v))
        result = schar((cptr.ldPtr(ga) === Shirt_off || what == 64n ? 1 : 0));
    else if (cptr.eq(otmp, uarmc.v))
        result = schar((cptr.ldPtr(ga) === Cloak_off || what == 2n ? 1 : 0));
    else if (cptr.eq(otmp, uarmf.v))
        result = schar((cptr.ldPtr(ga) === Boots_off || what == 32n ? 1 : 0));
    else if (cptr.eq(otmp, uarmh.v))
        result = schar((cptr.ldPtr(ga) === Helmet_off || what == 4n ? 1 : 0));
    else if (cptr.eq(otmp, uarmg.v))
        result = schar((cptr.ldPtr(ga) === Gloves_off || what == 16n ? 1 : 0));
    else if (cptr.eq(otmp, uarms.v))
        result = schar((cptr.ldPtr(ga) === Shield_off || what == 8n ? 1 : 0));
    else if (cptr.eq(otmp, uamul.v))
        result = schar((what == 65536n));
    else if (cptr.eq(otmp, uleft.v))
        result = schar((what == 131072n));
    else if (cptr.eq(otmp, uright.v))
        result = schar((what == 262144n));
    else if (cptr.eq(otmp, ublindf.v))
        result = schar((what == 524288n));
    else if (cptr.eq(otmp, uwep.v))
        result = schar((what == 256n));
    else if (cptr.eq(otmp, uswapwep.v))
        result = schar((what == 1024n));
    else if (cptr.eq(otmp, uquiver.v))
        result = schar((what == 512n));
    return result;
}

/** C ref: do_wear.c:1645 — @param {CPtr<struct obj>} obj @param {CLongLong} slotmask */
export function cancel_doff(obj, slotmask) {
    if (!(cptr.ldI64o(svc, $context_info_takeoff) & 536870912n) && donning(obj))
        cancel_don();
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) & BigInt.asIntN(64, ~slotmask));
}

/** C ref: do_wear.c:1664 */
export function cancel_don() {
    cptr.st1o(svc, $context_info_takeoff + $takeoff_info_cancelled_don, schar((cptr.ldPtr(ga) === Cloak_on || cptr.ldPtr(ga) === Armor_on || cptr.ldPtr(ga) === Shirt_on || cptr.ldPtr(ga) === Helmet_on || cptr.ldPtr(ga) === Gloves_on || cptr.ldPtr(ga) === Boots_on || cptr.ldPtr(ga) === Shield_on ? 1 : 0)));
    cptr.stPtr(ga, null);
    cptr.stPtro(gn, $instance_globals_n_nomovemsg, null);
    cptr.stI64o(gm, $instance_globals_m_multi, 0n);
    cptr.stI32o(svc, $context_info_takeoff + $takeoff_info_delay, 0);
    cptr.stI64o(svc, $context_info_takeoff + $takeoff_info_what, 0n);
}

/** C ref: do_wear.c:1688 — @param {CPtr<struct obj>} stolenobj @returns {CInt} */
export function* stop_donning(stolenobj) {
    let buf = new Uint8Array(256);
    let otmp;
    let putting_on;
    let result = 0;
    for (otmp = cptr.ldPtro(gi, $instance_globals_i_invent); otmp; otmp = cptr.ldPtr(otmp))
        if ((cptr.ldI64o(otmp, $obj_owornmask) & 127n) && donning(otmp))
            break;
    if (!otmp)
        return 0;
    putting_on = schar((!doffing(otmp)));
    cancel_don();
    cptr.stPtr(ga, null);
    if (putting_on || !cptr.eq(otmp, stolenobj)) {
        void cptr.sprintf(cptr.decay(buf), __s_you_stop_s_s, putting_on ? __s_putting_on : __s_taking_off, (yield* thesimpleoname(otmp)));
    } else {
        cptr.st1o(cptr.decay(buf), 0, 0, 1);
        result = Number(BigInt.asIntN(32, (-cptr.ldI64o(gm, $instance_globals_m_multi))));
    }
    (yield* unmul(cptr.decay(buf)));
    if (putting_on)
        (yield* remove_worn_item(otmp, 0));
    return result;
}

/** C ref: do_wear.c:1729 — int */
let Narmorpieces = 0;

/** C ref: do_wear.c:1729 — int */
let Naccessories = 0;

/** C ref: do_wear.c:1733 — @param {CPtr<struct obj *>} which @param {CInt} accessorizing */
function count_worn_stuff(which, accessorizing) {
    let otmp;
    Narmorpieces = (Naccessories = 0);
    otmp = null;
    {
        if (uarmh.v) {
            Narmorpieces++;
            otmp = uarmh.v;
        }
    }
    {
        if (uarms.v) {
            Narmorpieces++;
            otmp = uarms.v;
        }
    }
    {
        if (uarmg.v) {
            Narmorpieces++;
            otmp = uarmg.v;
        }
    }
    {
        if (uarmf.v) {
            Narmorpieces++;
            otmp = uarmf.v;
        }
    }
    if (uarmc.v)
        {
            if (uarmc.v) {
                Narmorpieces++;
                otmp = uarmc.v;
            }
        }
    else if (uarm.v)
        {
            if (uarm.v) {
                Narmorpieces++;
                otmp = uarm.v;
            }
        }
    else if (uarmu.v)
        {
            if (uarmu.v) {
                Narmorpieces++;
                otmp = uarmu.v;
            }
        }
    if (!accessorizing)
        cptr.stPtr(which, otmp);
    otmp = null;
    {
        if (uleft.v) {
            Naccessories++;
            otmp = uleft.v;
        }
    }
    {
        if (uright.v) {
            Naccessories++;
            otmp = uright.v;
        }
    }
    {
        if (uamul.v) {
            Naccessories++;
            otmp = uamul.v;
        }
    }
    {
        if (ublindf.v) {
            Naccessories++;
            otmp = ublindf.v;
        }
    }
    if (accessorizing)
        cptr.stPtr(which, otmp);
}

/** C ref: do_wear.c:1771 — @param {CPtr<struct obj>} obj @returns {CInt} */
function* armor_or_accessory_off(obj) {
    if (!(cptr.ldI64o(obj, $obj_owornmask) & 983167n)) {
        (yield* You(__s_are_not_wearing_that));
        return NHM.ECMD_OK;
    }
    if (cptr.eq(obj, uskin.v) || ((cptr.eq(obj, uarm.v)) && uarmc.v) || ((cptr.eq(obj, uarmu.v)) && (uarmc.v || uarm.v))) {
        let why = new Uint8Array(128);
        let what = new Uint8Array(128);
        cptr.st1o(cptr.decay(why), 0, cptr.st1o(cptr.decay(what), 0, 0, 1), 1);
        if (!cptr.eq(obj, uskin.v)) {
            if (uarmc.v)
                void cptr.strcat(cptr.decay(what), cloak_simple_name(uarmc.v));
            if ((cptr.eq(obj, uarmu.v)) && uarm.v) {
                if (uarmc.v)
                    void cptr.strcat(cptr.decay(what), __s_and__2);
                void cptr.strcat(cptr.decay(what), suit_simple_name(uarm.v));
            }
            nh_snprintf(__s_armor_or_accessory_off, 1792, cptr.decay(why), 128n, __s_without_taking_off_your_s_first, cptr.decay(what));
        } else {
            void cptr.strcpy(cptr.decay(why), __s_it_s_embedded);
        }
        (yield* You_cant(__s_take_that_off_s, cptr.decay(why)));
        return NHM.ECMD_OK;
    }
    reset_remarm();
    void (yield* select_off(obj));
    if (!cptr.ldI64o(svc, $context_info_takeoff))
        return NHM.ECMD_OK;
    reset_remarm();
    if (cptr.ldI64o(obj, $obj_owornmask) & 127n) {
        void (yield* armoroff(obj));
    } else if (cptr.eq(obj, uright.v) || cptr.eq(obj, uleft.v)) {
        (yield* off_msg(obj));
        (yield* Ring_off(obj));
    } else if (cptr.eq(obj, uamul.v)) {
        (yield* Amulet_off());
    } else if (cptr.eq(obj, ublindf.v)) {
        (yield* Blindf_off(obj));
    } else {
        (yield* impossible(__s_removing_strange_accessory_s, (yield* safe_typename(cptr.ldI16o(obj, $obj_otyp)))));
        if (cptr.ldI64o(obj, $obj_owornmask))
            (yield* remove_worn_item(obj, 0));
    }
    return NHM.ECMD_TIME;
}

/** C ref: do_wear.c:1833 @returns {CInt} */
export function* dotakeoff() {
    let otmp = cptr.box(null);
    count_worn_stuff(otmp, 0);
    if (!Narmorpieces && !Naccessories) {
        if (uskin.v)
            (yield* pline_The(__s_s_merged_with_your_skin, cptr.ldI16o(uskin.v, $obj_otyp) >= NHC.GRAY_DRAGON_SCALES ? __s_dragon_scales_are : __s_dragon_scale_mail_is));
        else
            (yield* pline(__s_not_wearing_any_armor_or_accessories));
        return NHM.ECMD_OK;
    }
    if (Narmorpieces != 1 || ParanoidRemove() || cptr.ld1so(gi, $instance_globals_i_item_action_in_progress))
        otmp.v = (yield* getobj(__s_take_off, takeoff_ok, NHM.GETOBJ_NOFLAGS));
    if (!otmp.v)
        return NHM.ECMD_CANCEL;
    return (yield* armor_or_accessory_off(otmp.v));
}

/** C ref: do_wear.c:1862 @returns {CInt} */
export function* ia_dotakeoff() {
    let res;
    cptr.st1o(gi, $instance_globals_i_item_action_in_progress, 1);
    res = (yield* dotakeoff());
    cptr.st1o(gi, $instance_globals_i_item_action_in_progress, 0);
    return res;
}

/** C ref: do_wear.c:1874 @returns {CInt} */
export function* doremring() {
    let otmp = cptr.box(null);
    count_worn_stuff(otmp, 1);
    if (!Naccessories && !Narmorpieces) {
        (yield* pline(__s_not_wearing_any_accessories_or_armor));
        return NHM.ECMD_OK;
    }
    if (Naccessories != 1 || ParanoidRemove() || cmdq_peek(NHC.CQ_CANNED))
        otmp.v = (yield* getobj(__s_remove, remove_ok, NHM.GETOBJ_NOFLAGS));
    if (!otmp.v)
        return NHM.ECMD_CANCEL;
    return (yield* armor_or_accessory_off(otmp.v));
}

/** C ref: do_wear.c:1893 — @param {CPtr<struct obj>} otmp @returns {CInt} */
export function* cursed(otmp) {
    if (!otmp) {
        (yield* impossible(__s_cursed_without_otmp));
        return 0;
    }
    if ((cptr.eq(otmp, uwep.v)) ? (yield* welded(otmp)) : (cptr.ldI32o(otmp, $obj_cursed) & 1) | 0) {
        let use_plural = schar((is_boots(otmp) || is_gloves(otmp) || cptr.ldI16o(otmp, $obj_otyp) == NHC.LENSES || cptr.ldI64o(otmp, $obj_quan) > 1n ? 1 : 0));
        if (Glib() && (cptr.ldI32o(otmp, $obj_bknown) & 1) | 0 && (uarmg.v ? (cptr.eq(otmp, uwep.v)) : ((cptr.ldI64o(otmp, $obj_owornmask) & 393472n) != 0n)))
            (yield* pline(__s_despite_your_slippery_s_you_can_t, (yield* fingers_or_gloves(1))));
        else
            (yield* You(__s_can_t_s_cursed, use_plural ? __s_they_are : __s_it_is));
        (yield* set_bknown(otmp, 1));
        return 1;
    }
    return 0;
}

const __static_armoroff_offdelaybuf = new Uint8Array(60); /** C ref: do_wear.c:1922 — char[60] (function-static) */

/** C ref: do_wear.c:1920 — @param {CPtr<struct obj>} otmp @returns {CInt} */
export function* armoroff(otmp) {
    let delay = -cptr.ld1so2(objects, cptr.ldI16o(otmp, $obj_otyp), 120, $objclass_oc_delay);
    let what = null;
    if ((yield* cursed(otmp)))
        return 0;
    if (delay) {
        nomul(delay);
        cptr.stPtro(gm, $instance_globals_m_multi_reason, __s_disrobing);
        switch (cptr.ld1so2(objects, cptr.ldI16o(otmp, $obj_otyp), 120, $objclass_oc_subtyp)) {
            case NHC.ARM_SUIT:
            what = suit_simple_name(otmp);
            cptr.stPtr(ga, Armor_off);
            break;
            case NHC.ARM_SHIELD:
            what = shield_simple_name(otmp);
            cptr.stPtr(ga, Shield_off);
            break;
            case NHC.ARM_HELM:
            what = helm_simple_name(otmp);
            cptr.stPtr(ga, Helmet_off);
            break;
            case NHC.ARM_GLOVES:
            what = (yield* gloves_simple_name(otmp));
            cptr.stPtr(ga, Gloves_off);
            break;
            case NHC.ARM_BOOTS:
            what = (yield* boots_simple_name(otmp));
            cptr.stPtr(ga, Boots_off);
            break;
            case NHC.ARM_CLOAK:
            what = cloak_simple_name(otmp);
            cptr.stPtr(ga, Cloak_off);
            break;
            case NHC.ARM_SHIRT:
            what = shirt_simple_name(otmp);
            cptr.stPtr(ga, Shirt_off);
            break;
            default:
            (yield* impossible(__s_taking_off_unknown_armor_d_d_delay_d, cptr.ldI16o(otmp, $obj_otyp), cptr.ld1so2(objects, cptr.ldI16o(otmp, $obj_otyp), 120, $objclass_oc_subtyp), delay));
            break;
        }
        if (what) {
            nh_snprintf(__s_armoroff, 1970, cptr.decay(__static_armoroff_offdelaybuf), 60n, __s_you_finish_taking_off_your_s, what);
            cptr.stPtro(gn, $instance_globals_n_nomovemsg, cptr.decay(__static_armoroff_offdelaybuf));
        }
    } else {
        switch (cptr.ld1so2(objects, cptr.ldI16o(otmp, $obj_otyp), 120, $objclass_oc_subtyp)) {
            case NHC.ARM_SUIT:
            void (yield* Armor_off());
            break;
            case NHC.ARM_SHIELD:
            void (yield* Shield_off());
            break;
            case NHC.ARM_HELM:
            void (yield* Helmet_off());
            break;
            case NHC.ARM_GLOVES:
            void (yield* Gloves_off());
            break;
            case NHC.ARM_BOOTS:
            void (yield* Boots_off());
            break;
            case NHC.ARM_CLOAK:
            void (yield* Cloak_off());
            break;
            case NHC.ARM_SHIRT:
            void (yield* Shirt_off());
            break;
            default:
            (yield* impossible(__s_taking_off_unknown_armor_d_d_no_delay, cptr.ldI16o(otmp, $obj_otyp), cptr.ld1so2(objects, cptr.ldI16o(otmp, $obj_otyp), 120, $objclass_oc_subtyp)));
            break;
        }
        (yield* off_msg(otmp));
    }
    cptr.stI64o(svc, $context_info_takeoff, cptr.stI64o(svc, $context_info_takeoff + $takeoff_info_what, 0n));
    return 1;
}

/** C ref: do_wear.c:2011 — @param {CPtr<char>} cc */
function* already_wearing(cc) {
    (yield* You(__s_are_already_wearing_s_c, cc, (cptr.eq(cc, cptr.decay(c_that_))) ? 33 : 46));
}

/** C ref: do_wear.c:2017 — @param {CPtr<char>} cc1 @param {CPtr<char>} cc2 */
function* already_wearing2(cc1, cc2) {
    (yield* You_cant(__s_wear_s_because_you_re_wearing_s_there, cc1, cc2));
}

/** C ref: do_wear.c:2030 — @param {CPtr<struct obj>} otmp @param {CPtr<long>} mask @param {CInt} noisy @returns {CInt} */
export function* canwearobj(otmp, mask, noisy) {
    let err = 0;
    let which;
    if ((cptr.ld1uo((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_msize) < NHM.MZ_SMALL) || ((cptr.ldU64o((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_mflags1) & 8192n) != 0n)) {
        if (noisy)
            (yield* You(__s_can_t_wear_any_armor_in_your_current));
        return 0;
    }
    which = is_cloak(otmp) ? cptr.decay(c_cloak) : (is_shirt(otmp) ? cptr.decay(c_shirt) : (is_suit(otmp) ? cptr.decay(c_suit) : null));
    if (which && cantweararm(cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)) && (!cptr.eq(which, cptr.decay(c_cloak)) || ((cptr.ldI16o(otmp, $obj_otyp) != NHC.MUMMY_WRAPPING) ? cptr.ld1uo(cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data), $permonst_msize) != NHM.MZ_SMALL : !WrappingAllowed(cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)))) && (racial_exception(cptr.add(gy, $instance_globals_y_youmonst), otmp) < 1)) {
        if (noisy)
            (yield* pline_The(__s_s_will_not_fit_on_your_body, which));
        return 0;
    } else if (cptr.ldI64o(otmp, $obj_owornmask) & 127n) {
        if (noisy)
            (yield* already_wearing(cptr.decay(c_that_)));
        return 0;
    }
    if ((yield* welded(uwep.v)) && bimanual(uwep.v) && (is_suit(otmp) || is_shirt(otmp))) {
        if (noisy)
            (yield* You(__s_cannot_do_that_while_holding_your_s, is_sword(uwep.v) ? cptr.decay(c_sword) : cptr.decay(c_weapon)));
        return 0;
    }
    if (is_helmet(otmp)) {
        if (uarmh.v) {
            if (noisy)
                (yield* already_wearing((yield* an(helm_simple_name(uarmh.v)))));
            err++;
        } else if (Upolyd() && (num_horns(cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)) > 0) && !is_flimsy(otmp)) {
            if (noisy)
                (yield* pline_The(__s_s_won_t_fit_over_your_horn_s, helm_simple_name(otmp), (((num_horns(cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data))) == 1) ? __s_empty : __s_s)));
            err++;
        } else
            cptr.stI64(mask, 4n);
    } else if (is_shield(otmp)) {
        if (uarms.v) {
            if (noisy)
                (yield* already_wearing((yield* an(cptr.decay(c_shield)))));
            err++;
        } else if (uwep.v && bimanual(uwep.v)) {
            if (noisy)
                (yield* You(__s_cannot_wear_a_shield_while_wielding_a, is_sword(uwep.v) ? cptr.decay(c_sword) : ((cptr.ldI16o(uwep.v, $obj_otyp) == NHC.BATTLE_AXE) ? cptr.decay(c_axe) : cptr.decay(c_weapon))));
            err++;
        } else if (cptr.ld1so(u, $you_twoweap)) {
            if (noisy)
                (yield* You(__s_cannot_wear_a_shield_while_wielding_two));
            err++;
        } else
            cptr.stI64(mask, 8n);
    } else if (is_boots(otmp)) {
        if (uarmf.v) {
            if (noisy)
                (yield* already_wearing(cptr.decay(c_boots)));
            err++;
        } else if (Upolyd() && ((cptr.ldU64o((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_mflags1) & 524288n) != 0n)) {
            if (noisy)
                (yield* You(__s_have_no_feet));
            err++;
        } else if (Upolyd() && cptr.ld1so(cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data), $permonst_mlet) == NHC.S_CENTAUR) {
            if (noisy)
                (yield* You(__s_have_too_many_hooves_to_wear_s, cptr.decay(c_boots)));
            err++;
        } else if (cptr.ldI32o(u, $you_utrap) && (cptr.ldI32o(u, $you_utraptype) == NHC.TT_BEARTRAP || cptr.ldI32o(u, $you_utraptype) == NHC.TT_INFLOOR || cptr.ldI32o(u, $you_utraptype) == NHC.TT_LAVA || cptr.ldI32o(u, $you_utraptype) == NHC.TT_BURIEDBALL)) {
            if (cptr.ldI32o(u, $you_utraptype) == NHC.TT_BEARTRAP) {
                if (noisy)
                    (yield* Your(__s_s_is_trapped, (yield* body_part(NHC.FOOT))));
            } else if (cptr.ldI32o(u, $you_utraptype) == NHC.TT_INFLOOR || cptr.ldI32o(u, $you_utraptype) == NHC.TT_LAVA) {
                if (noisy)
                    (yield* Your(__s_s_are_stuck_in_the_s, (yield* makeplural((yield* body_part(NHC.FOOT)))), surface(cptr.ldI16(u), cptr.ldI16o(u, $you_uy))));
            } else {
                if (noisy)
                    (yield* Your(__s_s_is_attached_to_the_buried_ball, (yield* body_part(NHC.LEG))));
            }
            err++;
        } else
            cptr.stI64(mask, 32n);
    } else if (is_gloves(otmp)) {
        if (uarmg.v) {
            if (noisy)
                (yield* already_wearing(cptr.decay(c_gloves)));
            err++;
        } else if ((yield* welded(uwep.v))) {
            if (noisy)
                (yield* You(__s_cannot_wear_gloves_over_your_s, is_sword(uwep.v) ? cptr.decay(c_sword) : cptr.decay(c_weapon)));
            err++;
        } else if (Glib()) {
            if (noisy)
                (yield* Your(__s_s_are_too_slippery_to_pull_on_s, (yield* fingers_or_gloves(0)), (yield* gloves_simple_name(otmp))));
            err++;
        } else
            cptr.stI64(mask, 16n);
    } else if (is_shirt(otmp)) {
        if (uarm.v || uarmc.v || uarmu.v) {
            if (uarmu.v) {
                if (noisy)
                    (yield* already_wearing((yield* an(cptr.decay(c_shirt)))));
            } else {
                if (noisy)
                    (yield* You_cant(__s_wear_that_over_your_s, (uarm.v && !uarmc.v) ? cptr.decay(c_armor) : cloak_simple_name(uarmc.v)));
            }
            err++;
        } else
            cptr.stI64(mask, 64n);
    } else if (is_cloak(otmp)) {
        if (uarmc.v) {
            if (noisy)
                (yield* already_wearing((yield* an(cloak_simple_name(uarmc.v)))));
            err++;
        } else
            cptr.stI64(mask, 2n);
    } else if (is_suit(otmp)) {
        if (uarmc.v) {
            if (noisy)
                (yield* You(__s_cannot_wear_armor_over_a_s, cloak_simple_name(uarmc.v)));
            err++;
        } else if (uarm.v) {
            if (noisy)
                (yield* already_wearing(__s_some_armor));
            err++;
        } else
            cptr.stI64(mask, 1n);
    } else {
        if (noisy)
            (yield* silly_thing(__s_wear, otmp));
        err++;
    }
    return !err;
}

/** C ref: do_wear.c:2209 — @param {CPtr<struct obj>} obj @returns {CInt} */
function* accessory_or_armor_on(obj) {
    obj = cptr.box(obj);
    let mask = cptr.box(0n);
    let armor;
    let ring;
    let amulet;
    let eyewear;
    if (cptr.ldI64o(obj.v, $obj_owornmask) & 983167n) {
        (yield* already_wearing(cptr.decay(c_that_)));
        return NHM.ECMD_OK;
    }
    armor = schar((cptr.ld1so(obj.v, $obj_oclass) == NHC.ARMOR_CLASS));
    ring = schar((cptr.ld1so(obj.v, $obj_oclass) == NHC.RING_CLASS || cptr.ldI16o(obj.v, $obj_otyp) == NHC.MEAT_RING ? 1 : 0));
    amulet = schar((cptr.ld1so(obj.v, $obj_oclass) == NHC.AMULET_CLASS));
    eyewear = schar((cptr.ldI16o(obj.v, $obj_otyp) == NHC.BLINDFOLD || cptr.ldI16o(obj.v, $obj_otyp) == NHC.TOWEL || cptr.ldI16o(obj.v, $obj_otyp) == NHC.LENSES ? 1 : 0));
    if (armor) {
        if (!(yield* canwearobj(obj.v, mask, 1)))
            return NHM.ECMD_OK;
        if (cptr.ldI16o(obj.v, $obj_otyp) == NHC.HELM_OF_OPPOSITE_ALIGNMENT && cptr.ldI16((cptr.add(svd, $instance_globals_saved_d_dungeon_topology + $dgn_topology_d_qstart_level))) == cptr.ldI16o(u, $you_uz)) {
            if (cptr.ld1so2(u, NHM.A_CURRENT, 1, $you_ualignbase) == cptr.ld1so2(u, NHM.A_ORIGINAL, 1, $you_ualignbase))
                (yield* You(__s_narrowly_avoid_losing_all_chance_at));
            else
                (yield* You(__s_are_suddenly_overcome_with_shame_and));
            cptr.stI32o(u, $you_ublessed, 0);
            (yield* discover_object((cptr.ldI16o(obj.v, $obj_otyp)), 1, 1, 1));
            cptr.st1(disp, 1);
            return NHM.ECMD_TIME;
        }
    } else {
        if (ring) {
            let answer;
            let qbuf = new Uint8Array(128);
            let res = 0;
            if (((cptr.ldU64o((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_mflags1) & 24576n) == 24576n)) {
                (yield* You(__s_cannot_make_the_ring_stick_to_your_body));
                return NHM.ECMD_OK;
            }
            if (uleft.v && uright.v) {
                (yield* There(__s_are_no_more_s_s_to_fill, ((cptr.ldU64o((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_mflags1) & 131072n) != 0n) ? __s_ring : __s_empty, (yield* fingers_or_gloves(0))));
                return NHM.ECMD_OK;
            }
            if (uleft.v) {
                mask.v = 262144n;
            } else if (uright.v) {
                mask.v = 131072n;
            } else {
                do {
                    void cptr.sprintf(cptr.decay(qbuf), __s_which_s_s_right_or_left, ((cptr.ldU64o((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_mflags1) & 131072n) != 0n) ? __s_ring : __s_empty, (yield* body_part(NHC.FINGER)));
                    answer = (yield* yn_function(cptr.decay(qbuf), cptr.decay(rightleftchars), 0, 1));
                    switch (answer) {
                        case 0:
                        case 27:
                        return NHM.ECMD_OK;
                        case 108:
                        case 76:
                        mask.v = 131072n;
                        break;
                        case 114:
                        case 82:
                        mask.v = 262144n;
                        break;
                    }
                } while (!mask.v);
            }
            if (uarmg.v && Glib()) {
                (yield* Your(__s_s_are_too_slippery_to_remove_so_you, (yield* gloves_simple_name(uarmg.v))));
                return NHM.ECMD_TIME;
            }
            if (uarmg.v && (cptr.ldI32o(uarmg.v, $obj_cursed) & 1) | 0) {
                res = !(cptr.ldI32o(uarmg.v, $obj_bknown) & 1);
                (yield* set_bknown(uarmg.v, 1));
                (yield* You(__s_cannot_remove_your_s_to_put_on_the_ring, cptr.decay(c_gloves)));
                return res ? NHM.ECMD_TIME : NHM.ECMD_OK;
            }
            if (uwep.v) {
                res = !(cptr.ldI32o(uwep.v, $obj_bknown) & 1);
                if (((mask.v == 262144n && URIGHTY()) || (mask.v == 131072n && ULEFTY()) || bimanual(uwep.v)) && (yield* welded(uwep.v))) {
                    let hand = (yield* body_part(NHC.HAND));
                    if (bimanual(uwep.v))
                        hand = (yield* makeplural(hand));
                    (yield* You(__s_cannot_free_your_weapon_s_to_put_on_the, hand));
                    return res ? NHM.ECMD_TIME : NHM.ECMD_OK;
                }
            }
        } else if (amulet) {
            if (uamul.v) {
                (yield* already_wearing(__s_an_amulet));
                return NHM.ECMD_OK;
            }
        } else if (eyewear) {
            if (!((cptr.ldU64o((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_mflags1) & 32768n) == 0n)) {
                (yield* You(__s_have_no_head_to_wear_s_on, (yield* ansimpleoname(obj.v))));
                return NHM.ECMD_OK;
            }
            if (ublindf.v) {
                if (cptr.ldI16o(ublindf.v, $obj_otyp) == NHC.TOWEL)
                    (yield* Your(__s_s_is_already_covered_by_a_towel, (yield* body_part(NHC.FACE))));
                else if (cptr.ldI16o(ublindf.v, $obj_otyp) == NHC.BLINDFOLD) {
                    if (cptr.ldI16o(obj.v, $obj_otyp) == NHC.LENSES)
                        (yield* already_wearing2(__s_lenses, __s_a_blindfold));
                    else
                        (yield* already_wearing(__s_a_blindfold));
                } else if (cptr.ldI16o(ublindf.v, $obj_otyp) == NHC.LENSES) {
                    if (cptr.ldI16o(obj.v, $obj_otyp) == NHC.BLINDFOLD)
                        (yield* already_wearing2(__s_a_blindfold, __s_some_lenses));
                    else
                        (yield* already_wearing(__s_some_lenses));
                } else {
                    (yield* already_wearing(cptr.ldPtro(c_common_strings, $c_common_strings_c_something)));
                }
                return NHM.ECMD_OK;
            }
        } else {
            (yield* You_cant(__s_wear_that));
            return NHM.ECMD_OK;
        }
    }
    if (!(yield* retouch_object(obj, 0)))
        return NHM.ECMD_TIME;
    if (armor) {
        let delay;
        if (cptr.ldI64o(obj.v, $obj_owornmask) & 1792n)
            (yield* remove_worn_item(obj.v, 0));
        cptr.st1o(gw, $instance_globals_w_wasinwater, uchar((cptr.ldI32o(u, $you_uinwater) & 1)));
        (yield* setworn(obj.v, mask.v));
        if (cptr.eq(obj.v, uarm.v))
            cptr.stPtr(ga, Armor_on);
        else if (cptr.eq(obj.v, uarmh.v))
            cptr.stPtr(ga, Helmet_on);
        else if (cptr.eq(obj.v, uarmg.v))
            cptr.stPtr(ga, Gloves_on);
        else if (cptr.eq(obj.v, uarmf.v))
            cptr.stPtr(ga, Boots_on);
        else if (cptr.eq(obj.v, uarms.v))
            cptr.stPtr(ga, Shield_on);
        else if (cptr.eq(obj.v, uarmc.v))
            cptr.stPtr(ga, Cloak_on);
        else if (cptr.eq(obj.v, uarmu.v))
            cptr.stPtr(ga, Shirt_on);
        else
            (yield* panic(__s_wearing_armor_not_worn_as_armor_08lx, cptr.ldI64o(obj.v, $obj_owornmask)));
        delay = -cptr.ld1so2(objects, cptr.ldI16o(obj.v, $obj_otyp), 120, $objclass_oc_delay);
        if (delay) {
            nomul(delay);
            cptr.stPtro(gm, $instance_globals_m_multi_reason, __s_dressing_up);
            cptr.stPtro(gn, $instance_globals_n_nomovemsg, __s_you_finish_your_dressing_maneuver);
        } else {
            (yield* unmul(__s_empty));
            (yield* on_msg(obj.v));
        }
        cptr.stI64o(svc, $context_info_takeoff, cptr.stI64o(svc, $context_info_takeoff + $takeoff_info_what, 0n));
    } else {
        if (ring) {
            (yield* setworn(obj.v, mask.v));
            (yield* Ring_on(obj.v));
            if (is_worn(obj.v))
                (yield* on_msg(obj.v));
        } else if (amulet) {
            (yield* Amulet_on(obj.v));
        } else if (eyewear) {
            (yield* Blindf_on(obj.v));
        } else {
            (yield* impossible(__s_putting_on_unexpected_type_of_accessory, (yield* safe_typename(cptr.ldI16o(obj.v, $obj_otyp)))));
        }
    }
    return NHM.ECMD_TIME;
}

/** C ref: do_wear.c:2432 @returns {CInt} */
export function* dowear() {
    let otmp;
    if ((cptr.ld1uo((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_msize) < NHM.MZ_SMALL) || ((cptr.ldU64o((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_mflags1) & 8192n) != 0n)) {
        (yield* pline(__s_don_t_even_bother));
        return NHM.ECMD_OK;
    }
    if (uarm.v && uarmu.v && uarmc.v && uarmh.v && uarms.v && uarmg.v && uarmf.v && uleft.v && uright.v && uamul.v && ublindf.v) {
        (yield* You(__s_are_already_wearing_a_full_complement));
        return NHM.ECMD_OK;
    }
    otmp = (yield* getobj(__s_wear, wear_ok, NHM.GETOBJ_NOFLAGS));
    return otmp ? (yield* accessory_or_armor_on(otmp)) : NHM.ECMD_CANCEL;
}

/** C ref: do_wear.c:2454 @returns {CInt} */
export function* doputon() {
    let otmp;
    if (uleft.v && uright.v && uamul.v && ublindf.v && uarm.v && uarmu.v && uarmc.v && uarmh.v && uarms.v && uarmg.v && uarmf.v) {
        (yield* Your(__s_s_s_are_full_and_you_re_already_wearing, ((cptr.ldU64o((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_mflags1) & 131072n) != 0n) ? __s_ring : __s_empty, (yield* fingers_or_gloves(0)), (cptr.ldI16o(ublindf.v, $obj_otyp) == NHC.LENSES) ? __s_some_lenses : __s_a_blindfold));
        return NHM.ECMD_OK;
    }
    otmp = (yield* getobj(__s_put_on, puton_ok, NHM.GETOBJ_NOFLAGS));
    return otmp ? (yield* accessory_or_armor_on(otmp)) : NHM.ECMD_CANCEL;
}

/** C ref: do_wear.c:2473 */
export function find_ac() {
    let uac = cptr.ld1so2(mons, cptr.ldI32o(u, $you_umonnum), 96, $permonst_ac);
    if (uarm.v)
        uac = (uac - ARM_BONUS(uarm.v)) | 0;
    if (uarmc.v)
        uac = (uac - ARM_BONUS(uarmc.v)) | 0;
    if (uarmh.v)
        uac = (uac - ARM_BONUS(uarmh.v)) | 0;
    if (uarmf.v)
        uac = (uac - ARM_BONUS(uarmf.v)) | 0;
    if (uarms.v)
        uac = (uac - ARM_BONUS(uarms.v)) | 0;
    if (uarmg.v)
        uac = (uac - ARM_BONUS(uarmg.v)) | 0;
    if (uarmu.v)
        uac = (uac - ARM_BONUS(uarmu.v)) | 0;
    if (uleft.v && cptr.ldI16o(uleft.v, $obj_otyp) == NHC.RIN_PROTECTION)
        uac = (uac - cptr.ld1so(uleft.v, $obj_spe)) | 0;
    if (uright.v && cptr.ldI16o(uright.v, $obj_otyp) == NHC.RIN_PROTECTION)
        uac = (uac - cptr.ld1so(uright.v, $obj_spe)) | 0;
    if (uamul.v && cptr.ldI16o(uamul.v, $obj_otyp) == NHC.AMULET_OF_GUARDING)
        uac = (uac - 2) | 0;
    if (HProtection() & 117440512n)
        uac = (uac - cptr.ldI32o(u, $you_ublessed)) | 0;
    uac = (uac - cptr.ld1uo(u, $you_uspellprot)) | 0;
    if (Math.abs(uac) > NHM.AC_MAX)
        uac = Math.imul(sgn(uac), NHM.AC_MAX);
    if (uac != cptr.ld1so(u, $you_uac)) {
        cptr.st1o(u, $you_uac, schar(uac));
        cptr.st1(disp, 1);
    }
}

/** C ref: do_wear.c:2528 */
export function* glibr() {
    let otmp;
    let xfl = 0;
    let leftfall;
    let rightfall;
    let wastwoweap = 0;
    let otherwep = null;
    let thiswep;
    let which;
    let hand;
    leftfall = schar((uleft.v && !(cptr.ldI32o(uleft.v, $obj_cursed) & 1) && (!uwep.v || !((yield* welded(uwep.v)) && ULEFTY()) || !bimanual(uwep.v)) ? 1 : 0));
    rightfall = schar((uright.v && !(cptr.ldI32o(uright.v, $obj_cursed) & 1) && (!uwep.v || !((yield* welded(uwep.v)) && URIGHTY()) || !bimanual(uwep.v)) ? 1 : 0));
    if (!uarmg.v && (leftfall || rightfall) && !((cptr.ldU64o((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_mflags1) & 24576n) == 24576n)) {
        (yield* Your(__s_s_off_your_s, (leftfall && rightfall) ? __s_rings_slip : __s_ring_slips, (leftfall && rightfall) ? (yield* fingers_or_gloves(0)) : (yield* body_part(NHC.FINGER))));
        xfl++;
        if (leftfall) {
            otmp = uleft.v;
            (yield* Ring_off(uleft.v));
            (yield* dropx(otmp));
            cmdq_clear(NHC.CQ_CANNED);
        }
        if (rightfall) {
            otmp = uright.v;
            (yield* Ring_off(uright.v));
            (yield* dropx(otmp));
            cmdq_clear(NHC.CQ_CANNED);
        }
    }
    otmp = uswapwep.v;
    if (cptr.ld1so(u, $you_twoweap) && otmp) {
        otherwep = is_sword(otmp) ? cptr.decay(c_sword) : (yield* weapon_descr(otmp));
        if (cptr.ldI64o(otmp, $obj_quan) > 1n)
            otherwep = (yield* makeplural(otherwep));
        hand = (yield* body_part(NHC.HAND));
        which = URIGHTY() ? __s_left : __s_right;
        (yield* Your(__s_s_s_s_from_your_s_s, otherwep, xfl ? __s_also : __s_empty, (yield* otense(otmp, __s_slip)), which, hand));
        xfl++;
        wastwoweap = 1;
        (yield* setuswapwep(null));
        cmdq_clear(NHC.CQ_CANNED);
        if ((yield* canletgo(otmp, __s_empty)))
            (yield* dropx(otmp));
    }
    otmp = uwep.v;
    if (otmp && cptr.ldI16o(otmp, $obj_otyp) != NHC.AKLYS && !(yield* welded(otmp))) {
        let savequan = cptr.ldI64o(otmp, $obj_quan);
        thiswep = is_sword(otmp) ? cptr.decay(c_sword) : (yield* weapon_descr(otmp));
        if (otherwep && strcmp(thiswep, (yield* makesingular(otherwep))))
            otherwep = null;
        if (cptr.ldI64o(otmp, $obj_quan) > 1n) {
            if (!strcmp(thiswep, __s_food))
                cptr.stI64o(otmp, $obj_quan, 1n);
            else
                thiswep = (yield* makeplural(thiswep));
        }
        hand = (yield* body_part(NHC.HAND));
        which = __s_empty;
        if (bimanual(otmp)) {
            hand = (yield* makeplural(hand));
        } else if (wastwoweap) {
            which = URIGHTY() ? __s_right : __s_left;
        }
        (yield* pline(__s_s_s_s_s_s_from_your_s_s, !cptr.strncmp(thiswep, __s_corpse, 6n) ? __s_the : __s_your, otherwep ? __s_other : __s_empty, thiswep, xfl ? __s_also : __s_empty, (yield* otense(otmp, __s_slip)), which, hand));
        cptr.stI64o(otmp, $obj_quan, savequan);
        (yield* setuwep(null));
        cmdq_clear(NHC.CQ_CANNED);
        if ((yield* canletgo(otmp, __s_empty)))
            (yield* dropx(otmp));
    }
}

/** C ref: do_wear.c:2630 — @param {CPtr<struct monst>} victim @returns {CPtr<struct obj>} */
export function* some_armor(victim) {
    let otmph;
    let otmp;
    otmph = (cptr.eq(victim, cptr.add(gy, $instance_globals_y_youmonst))) ? uarmc.v : (yield* which_armor(victim, 2n));
    if (!otmph)
        otmph = (cptr.eq(victim, cptr.add(gy, $instance_globals_y_youmonst))) ? uarm.v : (yield* which_armor(victim, 1n));
    if (!otmph)
        otmph = (cptr.eq(victim, cptr.add(gy, $instance_globals_y_youmonst))) ? uarmu.v : (yield* which_armor(victim, 64n));
    otmp = (cptr.eq(victim, cptr.add(gy, $instance_globals_y_youmonst))) ? uarmh.v : (yield* which_armor(victim, 4n));
    if (otmp && (!otmph || !(rng_log_enabled() ? (rng_log_set_caller(__s_do_wear_c, 2641, __s_some_armor__2), rn2(4)) : rn2(4))))
        otmph = otmp;
    otmp = (cptr.eq(victim, cptr.add(gy, $instance_globals_y_youmonst))) ? uarmg.v : (yield* which_armor(victim, 16n));
    if (otmp && (!otmph || !(rng_log_enabled() ? (rng_log_set_caller(__s_do_wear_c, 2644, __s_some_armor__2), rn2(4)) : rn2(4))))
        otmph = otmp;
    otmp = (cptr.eq(victim, cptr.add(gy, $instance_globals_y_youmonst))) ? uarmf.v : (yield* which_armor(victim, 32n));
    if (otmp && (!otmph || !(rng_log_enabled() ? (rng_log_set_caller(__s_do_wear_c, 2647, __s_some_armor__2), rn2(4)) : rn2(4))))
        otmph = otmp;
    otmp = (cptr.eq(victim, cptr.add(gy, $instance_globals_y_youmonst))) ? uarms.v : (yield* which_armor(victim, 8n));
    if (otmp && (!otmph || !(rng_log_enabled() ? (rng_log_set_caller(__s_do_wear_c, 2650, __s_some_armor__2), rn2(4)) : rn2(4))))
        otmph = otmp;
    return otmph;
}

/** C ref: do_wear.c:2657 — @param {CPtr<struct obj>} ring @param {CInt} otyp @returns {CPtr<struct obj>} */
export function* stuck_ring(ring, otyp) {
    if (!cptr.eq(ring, uleft.v) && !cptr.eq(ring, uright.v)) {
        (yield* impossible(__s_stuck_ring_neither_left_nor_right));
        return null;
    }
    if (ring && cptr.ldI16o(ring, $obj_otyp) == otyp) {
        if (((cptr.ldU64o((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_mflags1) & 24576n) == 24576n) && uamul.v && cptr.ldI16o(uamul.v, $obj_otyp) == NHC.AMULET_OF_UNCHANGING && (cptr.ldI32o(uamul.v, $obj_cursed) & 1) | 0)
            return uamul.v;
        if ((yield* welded(uwep.v)) && ((cptr.eq(ring, ((((cptr.ldI32o(u, $you_uhandedness) & 1) | 0) == NHM.LEFT_HANDED) ? uleft.v : uright.v))) || bimanual(uwep.v)))
            return uwep.v;
        if (uarmg.v && (cptr.ldI32o(uarmg.v, $obj_cursed) & 1) | 0)
            return uarmg.v;
        if ((cptr.ldI32o(ring, $obj_cursed) & 1))
            return ring;
        if (uarmg.v && Glib())
            return uarmg.v;
    }
    return null;
}

/** C ref: do_wear.c:2687 @returns {CPtr<struct obj>} */
export function unchanger() {
    if (uamul.v && cptr.ldI16o(uamul.v, $obj_otyp) == NHC.AMULET_OF_UNCHANGING)
        return uamul.v;
    return null;
}

/** C ref: do_wear.c:2696 — @param {CPtr<struct obj>} otmp @returns {CInt} */
function* select_off(otmp) {
    let why;
    let buf = new Uint8Array(256);
    if (!otmp)
        return 0;
    cptr.st1(cptr.decay(buf), 0);
    if (cptr.eq(otmp, uright.v) || cptr.eq(otmp, uleft.v)) {
        let glibdummy = cptr.alloc(216);
        if (((cptr.ldU64o((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_mflags1) & 24576n) == 24576n)) {
            (yield* pline_The(__s_ring_is_stuck));
            return 0;
        }
        cptr.memcpy(glibdummy, cg, 216);
        why = null;
        if ((yield* welded(uwep.v)) && ((cptr.eq(otmp, ((((cptr.ldI32o(u, $you_uhandedness) & 1) | 0) == NHM.LEFT_HANDED) ? uleft.v : uright.v))) || bimanual(uwep.v))) {
            void cptr.sprintf(cptr.decay(buf), __s_free_a_weapon_s, (yield* body_part(NHC.HAND)));
            why = uwep.v;
        } else if (uarmg.v && ((cptr.ldI32o(uarmg.v, $obj_cursed) & 1) | 0 || Glib())) {
            void cptr.sprintf(cptr.decay(buf), __s_take_off_your_s_s, Glib() ? __s_slippery : __s_empty, (yield* gloves_simple_name(uarmg.v)));
            why = !Glib() ? uarmg.v : glibdummy;
        }
        if (why) {
            (yield* You(__s_cannot_s_to_remove_the_ring, cptr.decay(buf)));
            (yield* set_bknown(why, 1));
            return 0;
        }
    }
    if (cptr.eq(otmp, uarmg.v)) {
        if ((yield* welded(uwep.v))) {
            (yield* You(__s_are_unable_to_take_off_your_s_while, cptr.decay(c_gloves), is_sword(uwep.v) ? cptr.decay(c_sword) : cptr.decay(c_weapon)));
            (yield* set_bknown(uwep.v, 1));
            return 0;
        } else if (Glib()) {
            (yield* pline(__s_s_s_are_too_slippery_to_take_off, (cptr.ldI32o(uarmg.v, $obj_unpaid) & 1) | 0 ? __s_the : __s_your, (yield* gloves_simple_name(uarmg.v))));
            return 0;
        }
        if ((yield* better_not_take_that_off(otmp)))
            return 0;
    }
    if (cptr.eq(otmp, uarmf.v)) {
        if (cptr.ldI32o(u, $you_utrap) && cptr.ldI32o(u, $you_utraptype) == NHC.TT_BEARTRAP) {
            (yield* pline_The(__s_bear_trap_prevents_you_from_pulling, (yield* body_part(NHC.FOOT))));
            return 0;
        } else if (cptr.ldI32o(u, $you_utrap) && cptr.ldI32o(u, $you_utraptype) == NHC.TT_INFLOOR) {
            (yield* You(__s_are_stuck_in_the_s_and_cannot_pull_your, surface(cptr.ldI16(u), cptr.ldI16o(u, $you_uy)), (yield* makeplural((yield* body_part(NHC.FOOT))))));
            return 0;
        }
    }
    if (cptr.eq(otmp, uarm.v) || cptr.eq(otmp, uarmu.v)) {
        why = null;
        if (uarmc.v && (cptr.ldI32o(uarmc.v, $obj_cursed) & 1) | 0) {
            void cptr.sprintf(cptr.decay(buf), __s_remove_your_s, cloak_simple_name(uarmc.v));
            why = uarmc.v;
        } else if (cptr.eq(otmp, uarmu.v) && uarm.v && (cptr.ldI32o(uarm.v, $obj_cursed) & 1) | 0) {
            void cptr.sprintf(cptr.decay(buf), __s_remove_your_s, cptr.decay(c_suit));
            why = uarm.v;
        } else if ((yield* welded(uwep.v)) && bimanual(uwep.v)) {
            void cptr.sprintf(cptr.decay(buf), __s_release_your_s, is_sword(uwep.v) ? cptr.decay(c_sword) : ((cptr.ldI16o(uwep.v, $obj_otyp) == NHC.BATTLE_AXE) ? cptr.decay(c_axe) : cptr.decay(c_weapon)));
            why = uwep.v;
        }
        if (why) {
            (yield* You(__s_cannot_s_to_take_off_s, cptr.decay(buf), (yield* the((yield* xname(otmp))))));
            (yield* set_bknown(why, 1));
            return 0;
        }
    }
    if (cptr.eq(otmp, uquiver.v) || (cptr.eq(otmp, uswapwep.v) && !cptr.ld1so(u, $you_twoweap))) {
        ;
    } else {
        if ((yield* cursed(otmp)))
            return 0;
    }
    if (cptr.eq(otmp, uarm.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 1n);
    else if (cptr.eq(otmp, uarmc.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 2n);
    else if (cptr.eq(otmp, uarmf.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 32n);
    else if (cptr.eq(otmp, uarmg.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 16n);
    else if (cptr.eq(otmp, uarmh.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 4n);
    else if (cptr.eq(otmp, uarms.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 8n);
    else if (cptr.eq(otmp, uarmu.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 64n);
    else if (cptr.eq(otmp, uleft.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 131072n);
    else if (cptr.eq(otmp, uright.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 262144n);
    else if (cptr.eq(otmp, uamul.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 65536n);
    else if (cptr.eq(otmp, ublindf.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 524288n);
    else if (cptr.eq(otmp, uwep.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 256n);
    else if (cptr.eq(otmp, uswapwep.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 1024n);
    else if (cptr.eq(otmp, uquiver.v))
        cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 512n);
    else
        (yield* impossible(__s_select_off_s, (yield* doname(otmp))));
    return 0;
}

/** C ref: do_wear.c:2824 @returns {CPtr<struct obj>} */
function* do_takeoff() {
    let otmp = null;
    let was_twoweap = cptr.ld1so(u, $you_twoweap);
    let doff = cptr.add(svc, $context_info_takeoff);
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) | 536870912n);
    if (cptr.ldI64o(doff, $takeoff_info_what) == 256n) {
        if (!(yield* cursed(uwep.v))) {
            (yield* setuwep(null));
            if (was_twoweap)
                (yield* You(__s_are_no_longer_wielding_either_weapon));
            else
                (yield* You(__s_are_s, empty_handed()));
        }
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 1024n) {
        (yield* setuswapwep(null));
        (yield* You(__s_sno_longer_s, was_twoweap ? __s_are : __s_empty, was_twoweap ? __s_wielding_two_weapons_at_once : __s_have_a_second_weapon_readied));
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 512n) {
        (yield* setuqwep(null));
        (yield* You(__s_no_longer_have_ammunition_readied));
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 1n) {
        otmp = uarm.v;
        if (!(yield* cursed(otmp)))
            void (yield* Armor_off());
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 2n) {
        otmp = uarmc.v;
        if (!(yield* cursed(otmp)))
            void (yield* Cloak_off());
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 32n) {
        otmp = uarmf.v;
        if (!(yield* cursed(otmp)))
            void (yield* Boots_off());
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 16n) {
        otmp = uarmg.v;
        if (!(yield* cursed(otmp)))
            void (yield* Gloves_off());
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 4n) {
        otmp = uarmh.v;
        if (!(yield* cursed(otmp)))
            void (yield* Helmet_off());
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 8n) {
        otmp = uarms.v;
        if (!(yield* cursed(otmp)))
            void (yield* Shield_off());
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 64n) {
        otmp = uarmu.v;
        if (!(yield* cursed(otmp)))
            void (yield* Shirt_off());
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 65536n) {
        otmp = uamul.v;
        if (!(yield* cursed(otmp)))
            (yield* Amulet_off());
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 131072n) {
        otmp = uleft.v;
        if (!(yield* cursed(otmp)))
            (yield* Ring_off(uleft.v));
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 262144n) {
        otmp = uright.v;
        if (!(yield* cursed(otmp)))
            (yield* Ring_off(uright.v));
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 524288n) {
        if (!(yield* cursed(ublindf.v)))
            (yield* Blindf_off(ublindf.v));
    } else {
        (yield* impossible(__s_do_takeoff_taking_off_lx, cptr.ldI64o(doff, $takeoff_info_what)));
    }
    cptr.stI64o(svc, $context_info_takeoff, cptr.ldI64o(svc, $context_info_takeoff) & (-536870913n));
    return otmp;
}

/** C ref: do_wear.c:2900 @returns {CInt} */
function* take_off() {
    let i;
    let otmp;
    let doff = cptr.add(svc, $context_info_takeoff);
    if (cptr.ldI64o(doff, $takeoff_info_what)) {
        if (cptr.ldI32o(doff, $takeoff_info_delay) > 0) {
            (cptr.stI32o(doff, $takeoff_info_delay, cptr.ldI32o(doff, $takeoff_info_delay) + -1)) - (-1);
            return 1;
        }
        if ((otmp = (yield* do_takeoff())) !== null)
            (yield* off_msg(otmp));
        cptr.stI64(doff, cptr.ldI64(doff) & BigInt.asIntN(64, ~cptr.ldI64o(doff, $takeoff_info_what)));
        cptr.stI64o(doff, $takeoff_info_what, 0n);
    }
    for (i = 0; cptr.ldI64o(takeoff_order, i, 8); i++)
        if (cptr.ldI64(doff) & cptr.ldI64o(takeoff_order, i, 8)) {
            cptr.stI64o(doff, $takeoff_info_what, cptr.ldI64o(takeoff_order, i, 8));
            break;
        }
    otmp = null;
    cptr.stI32o(doff, $takeoff_info_delay, 0);
    if (cptr.ldI64o(doff, $takeoff_info_what) == 0n) {
        (yield* You(__s_finish_s, cptr.add(doff, $takeoff_info_disrobing)));
        return 0;
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 256n) {
        cptr.stI32o(doff, $takeoff_info_delay, 1);
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 1024n) {
        cptr.stI32o(doff, $takeoff_info_delay, 1);
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 512n) {
        cptr.stI32o(doff, $takeoff_info_delay, 1);
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 1n) {
        otmp = uarm.v;
        if (uarmc.v)
            cptr.stI32o(doff, $takeoff_info_delay, (cptr.ldI32o(doff, $takeoff_info_delay) + ((Math.imul(2, cptr.ld1so2(objects, cptr.ldI16o(uarmc.v, $obj_otyp), 120, $objclass_oc_delay)) + 1) | 0)) | 0);
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 2n) {
        otmp = uarmc.v;
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 32n) {
        otmp = uarmf.v;
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 16n) {
        otmp = uarmg.v;
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 4n) {
        otmp = uarmh.v;
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 8n) {
        otmp = uarms.v;
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 64n) {
        otmp = uarmu.v;
        if (uarm.v)
            cptr.stI32o(doff, $takeoff_info_delay, (cptr.ldI32o(doff, $takeoff_info_delay) + Math.imul(2, cptr.ld1so2(objects, cptr.ldI16o(uarm.v, $obj_otyp), 120, $objclass_oc_delay))) | 0);
        if (uarmc.v)
            cptr.stI32o(doff, $takeoff_info_delay, (cptr.ldI32o(doff, $takeoff_info_delay) + ((Math.imul(2, cptr.ld1so2(objects, cptr.ldI16o(uarmc.v, $obj_otyp), 120, $objclass_oc_delay)) + 1) | 0)) | 0);
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 65536n) {
        cptr.stI32o(doff, $takeoff_info_delay, 1);
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 131072n) {
        cptr.stI32o(doff, $takeoff_info_delay, 1);
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 262144n) {
        cptr.stI32o(doff, $takeoff_info_delay, 1);
    } else if (cptr.ldI64o(doff, $takeoff_info_what) == 524288n) {
        cptr.stI32o(doff, $takeoff_info_delay, 1);
    } else {
        (yield* impossible(__s_take_off_taking_off_lx, cptr.ldI64o(doff, $takeoff_info_what)));
        return 0;
    }
    if (otmp)
        cptr.stI32o(doff, $takeoff_info_delay, (cptr.ldI32o(doff, $takeoff_info_delay) + cptr.ld1so2(objects, cptr.ldI16o(otmp, $obj_otyp), 120, $objclass_oc_delay)) | 0);
    if (cptr.ldI32o(doff, $takeoff_info_delay) > 0)
        (cptr.stI32o(doff, $takeoff_info_delay, cptr.ldI32o(doff, $takeoff_info_delay) + -1)) - (-1);
    set_occupation(take_off, cptr.add(doff, $takeoff_info_disrobing), 0n);
    return 1;
}

/** C ref: do_wear.c:2990 — @param {CPtr<struct obj>} otmp @returns {CInt} */
function* better_not_take_that_off(otmp) {
    let corpse = carrying_stoning_corpse();
    let buf = new Uint8Array(256);
    if (corpse && !u_safe_from_fatal_corpse(corpse, (NHC.st_corpse | NHC.st_petrifies))) {
        nh_snprintf(__s_better_not_take_that_off, 3006, cptr.decay(buf), 256n, __s_take_off_your_s_despite_carrying_a_dead, (yield* gloves_simple_name(otmp)), (yield* obj_pmname(corpse)));
        return schar(((yield* paranoid_ynq(1, cptr.decay(buf), 0)) != 121));
    }
    return 0;
}

/** C ref: do_wear.c:3014 */
export function reset_remarm() {
    cptr.stI64o(svc, $context_info_takeoff + $takeoff_info_what, cptr.stI64o(svc, $context_info_takeoff, 0n));
    cptr.st1o2(svc, 0, 1, $context_info_takeoff + $takeoff_info_disrobing, 0);
}

/** C ref: do_wear.c:3022 @returns {CInt} */
export function* doddoremarm() {
    let result = 0;
    if (cptr.ldI64o(svc, $context_info_takeoff + $takeoff_info_what) || cptr.ldI64o(svc, $context_info_takeoff)) {
        (yield* You(__s_continue_s, cptr.add(svc, $context_info_takeoff + $takeoff_info_disrobing)));
        set_occupation(take_off, cptr.add(svc, $context_info_takeoff + $takeoff_info_disrobing), 0n);
        return NHM.ECMD_OK;
    } else if (!uwep.v && !uswapwep.v && !uquiver.v && !uamul.v && !ublindf.v && !uleft.v && !uright.v && !wearing_armor()) {
        (yield* You(__s_are_not_wearing_anything));
        return NHM.ECMD_OK;
    }
    add_valid_menu_class(0);
    if (cptr.ld1so(flags, $flag_menu_style) != NHM.MENU_TRADITIONAL || (result = (yield* ggetobj(__s_take_off, select_off, 0, 0, null))) < -1)
        void (yield* menu_remarm(result));
    if (cptr.ldI64o(svc, $context_info_takeoff)) {
        void __builtin___strncpy_chk(cptr.add(svc, $context_info_takeoff + $takeoff_info_disrobing), (((cptr.ldI64o(svc, $context_info_takeoff) & -1793n) != 0n) ? __s_disrobing : __s_disarming), 30n, __builtin_object_size(cptr.add(svc, $context_info_takeoff + $takeoff_info_disrobing), 1));
        void (yield* take_off());
    }
    return NHM.ECMD_OK;
}

/** C ref: do_wear.c:3062 @returns {CInt} */
export function* remarm_swapwep() {
    let cq = cptr.alloc(32);
    let cmdq;
    let oldbknown;
    if ((cmdq = cmdq_pop()) !== null) {
        cptr.memcpy(cq, cmdq, 32);
        cptr.free(cmdq);
    } else {
        cptr.stI32(cq, NHC.CMDQ_KEY);
        cptr.st1o(cq, $_cmd_queue_key, 0);
    }
    if (cptr.ldI32(cq) != NHC.CMDQ_KEY || cptr.ld1so(cq, $_cmd_queue_key) != 45 || !uswapwep.v)
        return NHM.ECMD_FAIL;
    oldbknown = (cptr.ldI32o(uswapwep.v, $obj_bknown) & 1);
    reset_remarm();
    cptr.stI64o(svc, $context_info_takeoff + $takeoff_info_what, cptr.stI64o(svc, $context_info_takeoff, 1024n));
    void (yield* do_takeoff());
    return (!uswapwep.v || (cptr.ldI32o(uswapwep.v, $obj_bknown) & 1) != oldbknown) ? NHM.ECMD_TIME : NHM.ECMD_OK;
}

/** C ref: do_wear.c:3090 — @param {CInt} retry @returns {CInt} */
function* menu_remarm(retry) {
    let n;
    let i = 0;
    let pick_list = cptr.box(0);
    let all_worn_categories = 1;
    if (retry) {
        all_worn_categories = schar((retry == -2));
    } else if (cptr.ld1so(flags, $flag_menu_style) == NHM.MENU_FULL) {
        all_worn_categories = 0;
        n = (yield* query_category(__s_what_type_of_things_do_you_want_to_take, cptr.ldPtro(gi, $instance_globals_i_invent), 3892, pick_list, NHM.PICK_ANY));
        if (!n)
            return 0;
        for (i = 0; i < n; i++) {
            if (cptr.ldI32o(pick_list.v, i, 24) == -2)
                all_worn_categories = 1;
            else
                add_valid_menu_class(cptr.ldI32o(pick_list.v, i, 24));
        }
        cptr.free(pick_list.v);
    } else if (cptr.ld1so(flags, $flag_menu_style) == NHM.MENU_COMBINATION) {
        let ggofeedback = cptr.box(0);
        i = (yield* ggetobj(__s_take_off, select_off, 0, 1, ggofeedback));
        if ((ggofeedback.v & NHM.ALL_FINISHED) >>> 0)
            return 0;
        all_worn_categories = schar((i == -2));
    }
    if (menu_class_present(117) || menu_class_present(66) || menu_class_present(85) || menu_class_present(67) || menu_class_present(88))
        all_worn_categories = 0;
    n = (yield* query_objlist(__s_what_do_you_want_to_take_off, cptr.add(gi, $instance_globals_i_invent), 56, pick_list, NHM.PICK_ANY, all_worn_categories ? is_worn : is_worn_by_type));
    if (n > 0) {
        for (i = 0; i < n; i++)
            void (yield* select_off(cptr.ldPtro(pick_list.v, i, 24)));
        cptr.free(pick_list.v);
    } else if (n < 0 && cptr.ld1so(flags, $flag_menu_style) != NHM.MENU_COMBINATION) {
        (yield* There(__s_is_nothing_else_you_can_remove_or));
    }
    return 0;
}

/** C ref: do_wear.c:3144 — @param {CPtr<struct obj>} wornarm */
function* wornarm_destroyed(wornarm) {
    let invobj;
    let nextobj;
    let wornoid = cptr.ldI32o(wornarm, $obj_o_id);
    if (donning(wornarm))
        cancel_don();
    if (cptr.eq(wornarm, uarmc.v))
        void (yield* Cloak_off());
    else if (cptr.eq(wornarm, uarm.v))
        void (yield* Armor_off());
    else if (cptr.eq(wornarm, uarmu.v))
        void (yield* Shirt_off());
    else if (cptr.eq(wornarm, uarmh.v))
        void (yield* Helmet_off());
    else if (cptr.eq(wornarm, uarmg.v))
        void (yield* Gloves_off());
    else if (cptr.eq(wornarm, uarmf.v))
        void (yield* Boots_off());
    else if (cptr.eq(wornarm, uarms.v))
        void (yield* Shield_off());
    for (invobj = cptr.ldPtro(gi, $instance_globals_i_invent); invobj; invobj = nextobj) {
        nextobj = cptr.ldPtr(invobj);
        if (cptr.eq(invobj, wornarm) && cptr.ldI32o(invobj, $obj_o_id) == wornoid) {
            (yield* useup(wornarm));
            break;
        }
    }
}

/** C ref: do_wear.c:3189 — @param {CPtr<struct obj>} armor @param {CPtr<struct obj>} atmp @param {CPtr<boolean>} resisted @returns {CPtr<struct obj>} */
function maybe_destroy_armor(armor, atmp, resisted) {
    if ((armor !== null) && (!atmp || cptr.eq(atmp, armor)) && ((cptr.st1(resisted, obj_resists(armor, 0, 90))) == 0)) {
        cptr.stI32o(armor, $obj_in_use, 1);
        return armor;
    }
    return null;
}

/** C ref: do_wear.c:3201 — @param {CPtr<struct obj>} atmp @returns {CInt} */
export function* disintegrate_arm(atmp) {
    let otmp = null;
    let losing_gloves = 0;
    let resisted = cptr.box(0);
    let resistedc = cptr.box(0);
    let resistedsuit = cptr.box(0);
    if ((otmp = maybe_destroy_armor(uarmc.v, atmp, resistedc)) !== null) {
        (yield* urgent_pline(__s_your_s_crumbles_and_turns_to_dust, cloak_simple_name(otmp)));
    } else if (!resistedc.v && (otmp = maybe_destroy_armor(uarm.v, atmp, resistedsuit)) !== null) {
        let suit = suit_simple_name(otmp);
        if ((cptr.ldI32o(otmp, $obj_lamplit) & 1))
            (yield* end_burn(otmp, 0));
        (yield* urgent_pline(__s_your_s_s_to_dust_and_s_to_the_s, suit, (yield* vtense(suit, __s_turn)), (yield* vtense(suit, __s_fall)), surface(cptr.ldI16(u), cptr.ldI16o(u, $you_uy))));
    } else if (!resistedc.v && !resistedsuit.v && (otmp = maybe_destroy_armor(uarmu.v, atmp, resisted)) !== null) {
        (yield* urgent_pline(__s_your_s_crumbles_into_tiny_threads_and, shirt_simple_name(otmp)));
    } else if ((otmp = maybe_destroy_armor(uarmh.v, atmp, resisted)) !== null) {
        (yield* urgent_pline(__s_your_s_turns_to_dust_and_is_blown_away, helm_simple_name(otmp)));
    } else if ((otmp = maybe_destroy_armor(uarmg.v, atmp, resisted)) !== null) {
        (yield* urgent_pline(__s_your_s_vanish, (yield* gloves_simple_name(otmp))));
        losing_gloves = 1;
    } else if ((otmp = maybe_destroy_armor(uarmf.v, atmp, resisted)) !== null) {
        (yield* urgent_pline(__s_your_s_disintegrate, (yield* boots_simple_name(otmp))));
    } else if ((otmp = maybe_destroy_armor(uarms.v, atmp, resisted)) !== null) {
        (yield* urgent_pline(__s_your_s_crumbles_away, shield_simple_name(otmp)));
    } else {
        return 0;
    }
    (yield* wornarm_destroyed(otmp));
    if (losing_gloves)
        (yield* selftouch(__s_you));
    (yield* stop_occupation());
    return 1;
}

/** C ref: do_wear.c:3260 — @param {CPtr<struct obj>} otmp @returns {CInt} */
function obj_erode_type(otmp) {
    if (is_flammable(otmp))
        return NHM.ERODE_BURN;
    else if ((((cptr.ldI32o2(objects, cptr.ldI16o(otmp, $obj_otyp), 120, $objclass_oc_material) & 31) | 0) == NHC.IRON))
        return NHM.ERODE_RUST;
    else if (is_crackable(otmp))
        return NHM.ERODE_CRACK;
    else if (is_rottable(otmp))
        return NHM.ERODE_ROT;
    else if (is_corrodeable(otmp))
        return NHM.ERODE_CORRODE;
    return -1;
}

/** C ref: do_wear.c:3278 @returns {CInt} */
export function* destroy_arm() {
    let armors = cptr.alloc(7 * 8); cptr.stPtro(armors, 0, null);
    let otmp;
    let i;
    let idx = 0;
    let hits = ((rng_log_enabled() ? (rng_log_set_caller(__s_do_wear_c, 3282, __s_destroy_arm), rn2(4)) : rn2(4)) + 1) | 0;
    let ret = 0;
    if (uarm.v)
        cptr.stPtro(armors, idx++, uarm.v, 8);
    if (uarmc.v)
        cptr.stPtro(armors, idx++, uarmc.v, 8);
    if (uarmh.v)
        cptr.stPtro(armors, idx++, uarmh.v, 8);
    if (uarms.v)
        cptr.stPtro(armors, idx++, uarms.v, 8);
    if (uarmg.v)
        cptr.stPtro(armors, idx++, uarmg.v, 8);
    if (uarmf.v)
        cptr.stPtro(armors, idx++, uarmf.v, 8);
    if (uarmu.v)
        cptr.stPtro(armors, idx++, uarmu.v, 8);
    if (!idx)
        return 0;
    for (i = 0; i < hits; i++) {
        otmp = cptr.ldPtro(armors, (rng_log_enabled() ? (rng_log_set_caller(__s_do_wear_c, 3297, __s_destroy_arm), rn2(idx)) : rn2(idx)), 8);
        if (erosion_matters(otmp) && is_damageable(otmp) && !(cptr.ldI32o(otmp, $obj_oerodeproof) & 1)) {
            let erosion = obj_erode_type(otmp);
            if (erosion != -1) {
                let r = (yield* erode_obj(otmp, (yield* xname(otmp)), erosion, 10));
                if (r != NHM.ER_NOTHING)
                    ret = 1;
                if (r == NHM.ER_DESTROYED)
                    break;
            }
        }
    }
    if (ret)
        (yield* stop_occupation());
    return ret;
}

/** C ref: do_wear.c:3319 — @param {CPtr<struct obj>} otmp @param {CInt} delta */
export function* adj_abon(otmp, delta) {
    if (uarmg.v && cptr.eq(uarmg.v, otmp) && cptr.ldI16o(otmp, $obj_otyp) == NHC.GAUNTLETS_OF_DEXTERITY) {
        if (delta) {
            (yield* discover_object((cptr.ldI16o(uarmg.v, $obj_otyp)), 1, 1, 1));
            cptr.st1o2(u, NHC.A_DEX, 1, $you_abon, cptr.ld1so2(u, NHC.A_DEX, 1, $you_abon) + (delta));
        }
        cptr.st1(disp, 1);
    }
    if (uarmh.v && cptr.eq(uarmh.v, otmp) && cptr.ldI16o(otmp, $obj_otyp) == NHC.HELM_OF_BRILLIANCE) {
        if (delta) {
            (yield* discover_object((cptr.ldI16o(uarmh.v, $obj_otyp)), 1, 1, 1));
            cptr.st1o2(u, NHC.A_INT, 1, $you_abon, cptr.ld1so2(u, NHC.A_INT, 1, $you_abon) + (delta));
            cptr.st1o2(u, NHC.A_WIS, 1, $you_abon, cptr.ld1so2(u, NHC.A_WIS, 1, $you_abon) + (delta));
        }
        cptr.st1(disp, 1);
    }
}

const __static_inaccessible_equipment_need_to_take_off_outer_armor = cptr.bytes("need to take off %s to %s %s."); /** C ref: do_wear.c:3348 — char[30] (function-static) */

/** C ref: do_wear.c:3342 — @param {CPtr<struct obj>} obj @param {CPtr<char>} verb @param {CInt} only_if_known_cursed @returns {CInt} */
export function* inaccessible_equipment(obj, verb, only_if_known_cursed) {
    let buf = new Uint8Array(256);
    let anycovering = schar((!only_if_known_cursed));
    if (!obj || !cptr.ldI64o(obj, $obj_owornmask))
        return 0;
    if (cptr.eq(obj, uarm.v) && uarmc.v && (anycovering || ((cptr.ldI32o((uarmc.v), $obj_cursed) & 1) | 0 && (cptr.ldI32o((uarmc.v), $obj_bknown) & 1) | 0))) {
        if (verb) {
            void cptr.strcpy(cptr.decay(buf), (yield* yname(uarmc.v)));
            (yield* You(cptr.decay(__static_inaccessible_equipment_need_to_take_off_outer_armor), cptr.decay(buf), verb, (yield* yname(obj))));
        }
        return 1;
    }
    if (cptr.eq(obj, uarmu.v) && ((uarm.v && (anycovering || ((cptr.ldI32o((uarm.v), $obj_cursed) & 1) | 0 && (cptr.ldI32o((uarm.v), $obj_bknown) & 1) | 0))) || (uarmc.v && (anycovering || ((cptr.ldI32o((uarmc.v), $obj_cursed) & 1) | 0 && (cptr.ldI32o((uarmc.v), $obj_bknown) & 1) | 0))))) {
        if (verb) {
            let cloaktmp = new Uint8Array(128);
            let suittmp = new Uint8Array(128);
            let sameprefix = schar((uarm.v && uarmc.v && !strcmp((yield* shk_your(cptr.decay(cloaktmp), uarmc.v)), (yield* shk_your(cptr.decay(suittmp), uarm.v))) ? 1 : 0));
            cptr.st1(cptr.decay(buf), 0);
            if (uarmc.v)
                void cptr.strcat(cptr.decay(buf), (yield* yname(uarmc.v)));
            if (uarm.v && uarmc.v)
                void cptr.strcat(cptr.decay(buf), __s_and__2);
            if (uarm.v)
                void cptr.strcat(cptr.decay(buf), sameprefix ? (yield* xname(uarm.v)) : (yield* yname(uarm.v)));
            (yield* You(cptr.decay(__static_inaccessible_equipment_need_to_take_off_outer_armor), cptr.decay(buf), verb, (yield* yname(obj))));
        }
        return 1;
    }
    if ((cptr.eq(obj, uleft.v) || cptr.eq(obj, uright.v)) && uarmg.v && (anycovering || ((cptr.ldI32o((uarmg.v), $obj_cursed) & 1) | 0 && (cptr.ldI32o((uarmg.v), $obj_bknown) & 1) | 0))) {
        if (verb) {
            void cptr.strcpy(cptr.decay(buf), (yield* yname(uarmg.v)));
            (yield* You(cptr.decay(__static_inaccessible_equipment_need_to_take_off_outer_armor), cptr.decay(buf), verb, (yield* yname(obj))));
        }
        return 1;
    }
    return 0;
}

/** C ref: do_wear.c:3404 — @param {CPtr<struct obj>} obj @param {CInt} removing @param {CInt} accessory @returns {CInt} */
function* equip_ok(obj, removing, accessory) {
    let is_worn;
    let dummymask = cptr.box(0n);
    if (!obj)
        return NHC.GETOBJ_EXCLUDE;
    is_worn = schar(((cptr.ldI64o(obj, $obj_owornmask) & 983167n) != 0n));
    if (removing ^ is_worn)
        return NHC.GETOBJ_EXCLUDE_INACCESS;
    if (cptr.ld1so(obj, $obj_oclass) != NHC.ARMOR_CLASS && cptr.ld1so(obj, $obj_oclass) != NHC.RING_CLASS && cptr.ld1so(obj, $obj_oclass) != NHC.AMULET_CLASS) {
        if (cptr.ldI16o(obj, $obj_otyp) != NHC.MEAT_RING && cptr.ldI16o(obj, $obj_otyp) != NHC.BLINDFOLD && cptr.ldI16o(obj, $obj_otyp) != NHC.TOWEL && cptr.ldI16o(obj, $obj_otyp) != NHC.LENSES)
            return NHC.GETOBJ_EXCLUDE;
    }
    if (accessory ^ (cptr.ld1so(obj, $obj_oclass) != NHC.ARMOR_CLASS))
        return NHC.GETOBJ_DOWNPLAY;
    if (cptr.ld1so(obj, $obj_oclass) == NHC.ARMOR_CLASS && !removing && !(yield* canwearobj(obj, dummymask, 0)))
        return NHC.GETOBJ_DOWNPLAY;
    if (removing && !cptr.ld1so(gi, $instance_globals_i_item_action_in_progress)) {
        if ((yield* inaccessible_equipment(obj, null, schar((cptr.ld1so(obj, $obj_oclass) == NHC.RING_CLASS)))))
            return NHC.GETOBJ_EXCLUDE_INACCESS;
    }
    return NHC.GETOBJ_SUGGEST;
}

/** C ref: do_wear.c:3451 — @param {CPtr<struct obj>} obj @returns {CInt} */
function* puton_ok(obj) {
    return (yield* equip_ok(obj, 0, 1));
}

/** C ref: do_wear.c:3458 — @param {CPtr<struct obj>} obj @returns {CInt} */
function* remove_ok(obj) {
    return (yield* equip_ok(obj, 1, 1));
}

/** C ref: do_wear.c:3465 — @param {CPtr<struct obj>} obj @returns {CInt} */
function* wear_ok(obj) {
    return (yield* equip_ok(obj, 0, 0));
}

/** C ref: do_wear.c:3472 — @param {CPtr<struct obj>} obj @returns {CInt} */
function* takeoff_ok(obj) {
    return (yield* equip_ok(obj, 1, 0));
}

/** C ref: do_wear.c:3480 — @param {CPtr<struct obj>} obj @returns {CInt} */
export function any_worn_armor_ok(obj) {
    if (obj && (cptr.ldI64o(obj, $obj_owornmask) & 127n))
        return NHC.GETOBJ_SUGGEST;
    return NHC.GETOBJ_EXCLUDE;
}

/** C ref: do_wear.c:3489 @returns {CInt} */
export function count_worn_armor() {
    let ret = 0;
    if (uarm.v)
        ret++;
    if (uarmc.v)
        ret++;
    if (uarmh.v)
        ret++;
    if (uarms.v)
        ret++;
    if (uarmg.v)
        ret++;
    if (uarmf.v)
        ret++;
    if (uarmu.v)
        ret++;
    return ret;
}

// --- BEGIN c2js reset block (tools/c2js/resetify.mjs) — do not edit ---
// 19 bindings: 0 rebound+refilled, 2 rebound, 17 refilled.
// S/P are supplied by js/generated-y/__reset.js so this module needs no new import.
let __c2js_rs = null;
export function __captureState(S) { __c2js_rs = [S(see_yourself), S(unknown_type), S(c_armor), S(c_suit), S(c_shirt), S(c_cloak), S(c_gloves), S(c_boots), S(c_helmet), S(c_shield), S(c_weapon), S(c_sword), S(c_axe), S(c_that_), S(takeoff_order), S(Narmorpieces), S(Naccessories), S(__static_armoroff_offdelaybuf), S(__static_inaccessible_equipment_need_to_take_off_outer_armor)]; }
export function __resetState(P) {
    const r = __c2js_rs;
    if (r === null) throw new Error("do_wear.js: __resetState before __captureState");
    P(r[0]);
    P(r[1]);
    P(r[2]);
    P(r[3]);
    P(r[4]);
    P(r[5]);
    P(r[6]);
    P(r[7]);
    P(r[8]);
    P(r[9]);
    P(r[10]);
    P(r[11]);
    P(r[12]);
    P(r[13]);
    P(r[14]);
    Narmorpieces = P(r[15]);
    Naccessories = P(r[16]);
    P(r[17]);
    P(r[18]);
}
// --- END c2js reset block ---
