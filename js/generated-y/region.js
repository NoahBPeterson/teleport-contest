    ...
    for (i = 0; i < cptr.ldI32o(svn, $instance_globals_saved_n_n_regions); i++) {
        ...
        if (f_indx != -1) {
            for (j = 0; j < cptr.ldI16o(cptr.ldPtro(cptr.ldPtro(gr, $instance_globals_r_regions), i, 8), $NhRegion_n_monst); j++) {
                ...
                if (!mtmp || DEADMONSTER(mtmp) || (yield* Y.icall((callbacks[f_indx])(cptr.ldPtro(cptr.ldPtro(gr, $instance_globals_r_regions), i, 8), mtmp)))) {
...
export function* create_gas_cloud(x, y, cloudsize, damage) {
    ...
    if (!cptr.ld1so(svc, $context_info_mon_moving) && u_at(x, y) && cloudsize == 1 && (!damage || (damage && (yield* m_poisongas_ok(cptr.add(gy, $instance_globals_y_youmonst))) == NHM.M_POISONGAS_OK)))
        inside_cloud = 1;
    if (cloudsize > 150) {
