// YIELDABLE BUILD — generated by tools/c2js/yieldify.mjs from js/generated/topten.js
// Do not edit by hand, and do not score from this directory: the synchronous
// build in js/generated/ is the one the judge runs. Coloured functions here
// are generators; `yield*` marks every call that can reach a keystroke read.
import * as Y from '../yield-rt.js';
// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/src/topten.c
// Input sha256: 39c15b5ce85bc5d1cb4de6510ab35e7c45b3983d0ea8c94ad2e91b029f76a6a6
// Transpiler: tools/c2js c2js emit v1+batch

import { schar } from '../cmachine.js';
import * as cptr from '../cptr.js';
import * as NHC from './nhconst.js';
import * as NHM from './nhmacro.js';
import * as FLD from './nhfield.js';
import { flags, gh, gi, gm, gt, gu, iflags, program_state, svd, svk, svm, svp, u, ubirthday, urealtime } from './decl.js';
import { impossible, raw_printf } from './pline.js';
import { an } from './objnam.js';
import { Strlen_ } from './strutil.js';
import { windowprocs } from './windows.js';
import { deepest_lev_reached, depth, init_dungeons } from './dungeon.js';
import { copynchars, digit, eos, highc, lcase, nh_snprintf, onlyspace, ordin, strNsubst, strsubst } from './hacklib.js';
import { aligns, genders, roles, str2race, str2role } from './role.js';
import { timet_to_seconds } from './allmain.js';
import { money_cnt } from './hack.js';
import { hidden_gold } from './vault.js';
import { num_genocides, sokoban_in_play } from './insight.js';
import { rank_of, rank_to_xlev } from './botl.js';
import { sysopt } from './sys.js';
import { alloc } from './alloc.js';
import { yyyymmdd } from './calendar.js';
import { fopen_datafile, lock_file, unlock_file } from './files.js';
import { free_dungeons } from './save.js';
import { rn2, rnd, rng_log_enabled, rng_log_set_caller } from './rnd.js';
import { set_corpsenm } from './mkobj.js';
import { christen_monst, oname } from './do_name.js';
import { canseemon } from './display.js';

// string literals (C char* uses decay to CPtr into these static buffers)
const __sl0 = cptr.lit("bad killer format? (%d)");
const __sl1 = cptr.lit("formatkiller");
const __sl2 = cptr.lit(", while %s");
const __sl3 = cptr.lit(", while helpless");
const __sl4 = cptr.lit("killed by ");
const __sl5 = cptr.lit("choked on ");
const __sl6 = cptr.lit("poisoned by ");
const __sl7 = cptr.lit("died of ");
const __sl8 = cptr.lit("drowned in ");
const __sl9 = cptr.lit("burned by ");
const __sl10 = cptr.lit("dissolved in ");
const __sl11 = cptr.lit("crushed to death by ");
const __sl12 = cptr.lit("petrified by ");
const __sl13 = cptr.lit("turned to slime by ");
const __sl14 = cptr.lit("");
const __sl15 = cptr.lit("\n");
const __sl16 = cptr.lit("?");
const __sl17 = cptr.lit("Mal");
const __sl18 = cptr.lit("Fem");
const __sl19 = cptr.lit("_");
const __sl20 = cptr.lit("version=%d.%d.%d");
const __sl21 = cptr.lit("%cpoints=%ld%cdeathdnum=%d%cdeathlev=%d");
const __sl22 = cptr.lit("%cmaxlvl=%d%chp=%d%cmaxhp=%d");
const __sl23 = cptr.lit("%cdeaths=%d%cdeathdate=%ld%cbirthdate=%ld%cuid=%d");
const __sl24 = cptr.lit("%s");
const __sl25 = cptr.lit("%crole=%s%crace=%s%cgender=%s%calign=%s");
const __sl26 = cptr.lit("%s%cname=%s%cdeath=%s");
const __sl27 = cptr.lit("%cwhile=%s");
const __sl28 = cptr.lit("helpless");
const __sl29 = cptr.lit("%cconduct=0x%lx%cturns=%ld%cachieve=0x%lx");
const __sl30 = cptr.lit("%cachieveX=%s");
const __sl31 = cptr.lit("%cconductX=%s");
const __sl32 = cptr.lit("%crealtime=%ld%cstarttime=%ld%cendtime=%ld");
const __sl33 = cptr.lit("%cgender0=%s%calign0=%s");
const __sl34 = cptr.lit("%cflags=0x%lx");
const __sl35 = cptr.lit("%cgold=%ld");
const __sl36 = cptr.lit("%cwish_cnt=%ld");
const __sl37 = cptr.lit("%carti_wish_cnt=%ld");
const __sl38 = cptr.lit("%cbones=%ld");
const __sl39 = cptr.lit("%crerolls=%ld");
const __sl40 = cptr.lit(",");
const __sl41 = cptr.lit("ascended");
const __sl42 = cptr.lit("entered_astral_plane");
const __sl43 = cptr.lit("entered_elemental_planes");
const __sl44 = cptr.lit("obtained_the_amulet_of_yendor");
const __sl45 = cptr.lit("performed_the_invocation_ritual");
const __sl46 = cptr.lit("obtained_the_book_of_the_dead");
const __sl47 = cptr.lit("obtained_the_bell_of_opening");
const __sl48 = cptr.lit("obtained_the_candelabrum_of_invocation");
const __sl49 = cptr.lit("entered_gehennom");
const __sl50 = cptr.lit("defeated_medusa");
const __sl51 = cptr.lit("obtained_the_luckstone_from_the_mines");
const __sl52 = cptr.lit("obtained_the_sokoban_prize");
const __sl53 = cptr.lit("consulted_the_oracle");
const __sl54 = cptr.lit("read_a_discworld_novel");
const __sl55 = cptr.lit("entered_the_gnomish_mines");
const __sl56 = cptr.lit("entered_mine_town");
const __sl57 = cptr.lit("entered_a_shop");
const __sl58 = cptr.lit("entered_a_temple");
const __sl59 = cptr.lit("entered_sokoban");
const __sl60 = cptr.lit("entered_bigroom");
const __sl61 = cptr.lit("learned_castle_drawbridge_tune");
const __sl62 = cptr.lit("attained_the_rank_of_%s");
const __sl63 = cptr.lit(" ");
const __sl64 = cptr.lit("foodless");
const __sl65 = cptr.lit("vegan");
const __sl66 = cptr.lit("vegetarian");
const __sl67 = cptr.lit("atheist");
const __sl68 = cptr.lit("weaponless");
const __sl69 = cptr.lit("pacifist");
const __sl70 = cptr.lit("illiterate");
const __sl71 = cptr.lit("polyless");
const __sl72 = cptr.lit("polyselfless");
const __sl73 = cptr.lit("wishless");
const __sl74 = cptr.lit("artiwishless");
const __sl75 = cptr.lit("genocideless");
const __sl76 = cptr.lit("sokoban");
const __sl77 = cptr.lit("blind");
const __sl78 = cptr.lit("deaf");
const __sl79 = cptr.lit("nudist");
const __sl80 = cptr.lit("pauper");
const __sl81 = cptr.lit("bonesless");
const __sl82 = cptr.lit("petless");
const __sl83 = cptr.lit("unrerolled");
const __sl84 = cptr.lit("logfile");
const __sl85 = cptr.lit("a");
const __sl86 = cptr.lit("Cannot open log file!");
const __sl87 = cptr.lit("xlogfile");
const __sl88 = cptr.lit("Cannot open extended log file!");
const __sl89 = cptr.lit("Since you were in %s mode, the score list will not be checked.");
const __sl90 = cptr.lit("wizard");
const __sl91 = cptr.lit("discover");
const __sl92 = cptr.lit("record");
const __sl93 = cptr.lit("r");
const __sl94 = cptr.lit("Cannot open record file!");
const __sl95 = cptr.lit("You didn't beat your previous score of %ld points.");
const __sl96 = cptr.lit("w");
const __sl97 = cptr.lit("Cannot write record file");
const __sl98 = cptr.lit("You made the top ten list!");
const __sl99 = cptr.lit("You reached the %d%s place on the top %d list.");
const __sl100 = cptr.lit(" No  Points     Name");
const __sl101 = cptr.lit("Hp [max]");
const __sl102 = cptr.lit("%3d");
const __sl103 = cptr.lit("   ");
const __sl104 = cptr.lit(" %10ld  %.10s");
const __sl105 = cptr.lit("-%s");
const __sl106 = cptr.lit("-%s ");
const __sl107 = cptr.lit("escaped");
const __sl108 = cptr.lit("escaped the dungeon %s[max level %d]");
const __sl109 = cptr.lit(" (");
const __sl110 = cptr.lit("ascended to demigod%s-hood");
const __sl111 = cptr.lit("dess");
const __sl112 = cptr.lit("quit");
const __sl113 = cptr.lit("died of st");
const __sl114 = cptr.lit("starved to death");
const __sl115 = cptr.lit("choked");
const __sl116 = cptr.lit("choked on h%s food");
const __sl117 = cptr.lit("er");
const __sl118 = cptr.lit("is");
const __sl119 = cptr.lit("poisoned");
const __sl120 = cptr.lit("was poisoned");
const __sl121 = cptr.lit("crushed");
const __sl122 = cptr.lit("was crushed to death");
const __sl123 = cptr.lit("turned to stone");
const __sl124 = cptr.lit("died");
const __sl125 = cptr.lit(" on the Plane of %s");
const __sl126 = cptr.lit(" on the %s Plane");
const __sl127 = cptr.lit("Astral");
const __sl128 = cptr.lit("Water");
const __sl129 = cptr.lit("Fire");
const __sl130 = cptr.lit("Air");
const __sl131 = cptr.lit("Earth");
const __sl132 = cptr.lit("Void");
const __sl133 = cptr.lit(" in %s");
const __sl134 = cptr.lit(" on level %d");
const __sl135 = cptr.lit(" [max %d]");
const __sl136 = cptr.lit("quit ");
const __sl137 = cptr.lit(".");
const __sl138 = cptr.lit("  %c%s.");
const __sl139 = cptr.lit("; the ");
const __sl140 = cptr.lit(", the ");
const __sl141 = cptr.lit("%d");
const __sl142 = cptr.lit(" [max");
const __sl143 = cptr.lit("outentry");
const __sl144 = cptr.lit("%15s %s");
const __sl145 = cptr.lit(" %s[%d]");
const __sl146 = cptr.lit("  ");
const __sl147 = cptr.lit("pru");
const __sl148 = cptr.lit("all");
const __sl149 = cptr.lit("prscore");
const __sl150 = cptr.lit("-s");
const __sl151 = cptr.lit("--scores");
const __sl152 = cptr.lit("prscore: bad arguments (%d)");
const __sl153 = cptr.lit("-v");
const __sl154 = cptr.lit("Cannot find any %sentries for ");
const __sl155 = cptr.lit("current ");
const __sl156 = cptr.lit("you");
const __sl157 = cptr.lit("any of ");
const __sl158 = cptr.lit("-u");
const __sl159 = cptr.lit("...");
const __sl160 = cptr.lit("pr");
const __sl161 = cptr.lit(":");
const __sl162 = cptr.lit("Usage: %s -s [-v] <playertypes> [maxrank] [playernames]");
const __sl163 = cptr.lit("Player types are: [-p role] [-r race]");
const __sl164 = cptr.lit("E");
const __sl165 = cptr.lit("What weird role is this? (%s)");
const __sl166 = cptr.lit("/Users/noahpeterson/Documents/Projects/teleport-contest-research/original-contest-to-fork/nethack-c/recorder/src/topten.c");
const __sl167 = cptr.lit("get_rnd_toptenentry");
const __sl168 = cptr.lit("tt_doppel");

/** C ref: topten.c:38 — struct toptenentry { tt_next, points, deathdnum, deathlev, maxlvl, hp, maxhp, deaths, ver_major, ver_minor, patchlevel, deathdate, birthdate, uid, plrole, plrace, plgend, plalign, name, death } (memory model v0.5) */

/** C ref: topten.c:56 — struct toptenentry * */
let tt_head = null;

/** C ref: topten.c:61 — struct toptenentry */
let zerott = cptr.alloc(208);

const __static_formatkiller_killed_by_prefix = cptr.alloc(16 * 8);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 0, __sl4);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 8, __sl5);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 16, __sl6);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 24, __sl7);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 32, __sl8);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 40, __sl9);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 48, __sl10);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 56, __sl11);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 64, __sl12);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 72, __sl13);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 80, __sl4);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 88, __sl14);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 96, __sl14);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 104, __sl14);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 112, __sl14);
cptr.stPtro(__static_formatkiller_killed_by_prefix, 120, __sl14); /** C ref: topten.c:96 — char *[16] (function-static) */

/** C ref: topten.c:90 — @param {CPtr} buf @param {CUInt} siz @param {CInt} how @param {CInt} incl_helpless */
export function* formatkiller(buf, siz, how, incl_helpless) {
    let l;
    let c;
    let kname = cptr.add(svk, FLD.kinfo_name);
    cptr.st1o(buf, 0, 0);
    switch (cptr.ldI32o(svk, FLD.kinfo_format)) {
        default:
        (yield* impossible(__sl0, cptr.ldI32o(svk, FLD.kinfo_format)));
        // @FallThrough
        ;
        case NHM.NO_KILLER_PREFIX:
        break;
        case NHM.KILLED_BY_AN:
        kname = (yield* an(kname));
        // @FallThrough
        ;
        case NHM.KILLED_BY:
        void __builtin___strncat_chk(buf, cptr.ldPtro(__static_formatkiller_killed_by_prefix, how, 8), BigInt(((siz - 1) >>> 0) >>> 0), __builtin_object_size(buf, 1));
        l = (yield* Strlen_(buf, __sl1, 123));
        buf = cptr.add(buf, l), siz = (siz - l) | 0;
        break;
    }
    while (--siz > 0) {
        c = cptr.ld1s(cptr.postinc(() => kname, (v) => { kname = v; }));
        if (!c)
            break;
        else if (c == 44)
            c = 59;
        else if (c == 61)
            c = 95;
        else if (c == 9)
            c = 32;
        cptr.st1(cptr.postinc(() => buf, (v) => { buf = v; }), c);
    }
    cptr.st1(buf, 0);
    if (incl_helpless && cptr.ldI64o(gm, FLD.instance_globals_m_multi) < 0n) {
        if (cptr.ldPtro(gm, FLD.instance_globals_m_multi_reason) && BigInt.asUintN(64, cptr.strlen(cptr.ldPtro(gm, FLD.instance_globals_m_multi_reason)) + 9n) <= BigInt(siz >>> 0))
            void cptr.sprintf(buf, __sl2, cptr.ldPtro(gm, FLD.instance_globals_m_multi_reason));
        else if (17n <= BigInt(siz >>> 0))
            void cptr.strcpy(buf, __sl3);
    }
}

/** C ref: topten.c:165 — @param {CPtr} x */
function* topten_print(x) {
    if (cptr.ldI32o(gt, FLD.instance_globals_t_toptenwin) == -1)
        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_raw_print))(x)));
    else
        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(cptr.ldI32o(gt, FLD.instance_globals_t_toptenwin), NHM.ATR_NONE, x)));
}

/** C ref: topten.c:174 — @param {CPtr} x */
function* topten_print_bold(x) {
    if (cptr.ldI32o(gt, FLD.instance_globals_t_toptenwin) == -1)
        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_raw_print_bold))(x)));
    else
        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(cptr.ldI32o(gt, FLD.instance_globals_t_toptenwin), NHM.ATR_BOLD, x)));
}

/** C ref: topten.c:183 — @param {CPtr} lev @returns {CInt} */
export function observable_depth(lev) {
    return depth(lev);
}

/** C ref: topten.c:208 — @param {CPtr} rfile */
function discardexcess(rfile) {
    let c;
    do {
        c = fgetc(rfile);
    } while (c != 10 && c != -1);
}

const __static_readentry_fmt = cptr.bytes("%d.%d.%d %ld %d %d %d %d %d %d %ld %ld %d "); /** C ref: topten.c:230 — char[43] (function-static) */
const __static_readentry_fmt32 = cptr.bytes("%c%c %[^,],%[^\n]%*c"); /** C ref: topten.c:231 — char[20] (function-static) */
const __static_readentry_fmt33 = cptr.bytes("%s %s %s %s %[^,],%[^\n]%*c"); /** C ref: topten.c:232 — char[27] (function-static) */

/** C ref: topten.c:220 — @param {CPtr} rfile @param {CPtr} tt */
function* readentry(rfile, tt) {
    let inbuf = new Uint8Array(129);
    let s1 = new Uint8Array(129);
    let s2 = new Uint8Array(129);
    let s3 = new Uint8Array(129);
    let s4 = new Uint8Array(129);
    let s5 = new Uint8Array(129);
    let s6 = new Uint8Array(129);
    if (fscanf(rfile, cptr.decay(__static_readentry_fmt), cptr.add(tt, FLD.toptenentry_ver_major), cptr.add(tt, FLD.toptenentry_ver_minor), cptr.add(tt, FLD.toptenentry_patchlevel), cptr.add(tt, FLD.toptenentry_points), cptr.add(tt, FLD.toptenentry_deathdnum), cptr.add(tt, FLD.toptenentry_deathlev), cptr.add(tt, FLD.toptenentry_maxlvl), cptr.add(tt, FLD.toptenentry_hp), cptr.add(tt, FLD.toptenentry_maxhp), cptr.add(tt, FLD.toptenentry_deaths), cptr.add(tt, FLD.toptenentry_deathdate), cptr.add(tt, FLD.toptenentry_birthdate), cptr.add(tt, FLD.toptenentry_uid)) != 13) {
        cptr.stI64o(tt, FLD.toptenentry_points, 0n);
        discardexcess(rfile);
    } else {
        if (!fgets(cptr.decay(inbuf), 129, rfile)) {
            cptr.st1(cptr.decay(inbuf), 0);
        } else if (!cptr.strchr(cptr.decay(inbuf), 10)) {
            void cptr.strcpy(cptr.add(cptr.decay(inbuf), 127n, 1), __sl15);
            discardexcess(rfile);
        }
        if (cptr.ldI32o(tt, FLD.toptenentry_ver_major) < 3 || (cptr.ldI32o(tt, FLD.toptenentry_ver_major) == 3 && cptr.ldI32o(tt, FLD.toptenentry_ver_minor) < 3)) {
            let i;
            if (sscanf(cptr.decay(inbuf), cptr.decay(__static_readentry_fmt32), cptr.add(tt, FLD.toptenentry_plrole), cptr.add(tt, FLD.toptenentry_plgend), cptr.decay(s1), cptr.decay(s2)) == 4) {
                cptr.st1o2(tt, 1, 1, FLD.toptenentry_plrole, cptr.st1o2(tt, 1, 1, FLD.toptenentry_plgend, 0));
                (yield* copynchars(cptr.add(tt, FLD.toptenentry_name), cptr.decay(s1), 10));
                (yield* copynchars(cptr.add(tt, FLD.toptenentry_death), cptr.decay(s2), 100));
            } else
                cptr.stI64o(tt, FLD.toptenentry_points, 0n);
            cptr.st1o2(tt, 1, 1, FLD.toptenentry_plrole, 0);
            if ((i = (yield* str2role(cptr.add(tt, FLD.toptenentry_plrole)))) >= 0)
                void cptr.strcpy(cptr.add(tt, FLD.toptenentry_plrole), cptr.ldPtro2(roles, i, 312, FLD.Role_filecode));
            void cptr.strcpy(cptr.add(tt, FLD.toptenentry_plrace), __sl16);
            void cptr.strcpy(cptr.add(tt, FLD.toptenentry_plgend), (cptr.ld1so2(tt, 0, 1, FLD.toptenentry_plgend) == 77) ? __sl17 : __sl18);
            void cptr.strcpy(cptr.add(tt, FLD.toptenentry_plalign), __sl16);
        } else if (sscanf(cptr.decay(inbuf), cptr.decay(__static_readentry_fmt33), cptr.decay(s1), cptr.decay(s2), cptr.decay(s3), cptr.decay(s4), cptr.decay(s5), cptr.decay(s6)) == 6) {
            (yield* copynchars(cptr.add(tt, FLD.toptenentry_plrole), cptr.decay(s1), 3));
            (yield* copynchars(cptr.add(tt, FLD.toptenentry_plrace), cptr.decay(s2), 3));
            (yield* copynchars(cptr.add(tt, FLD.toptenentry_plgend), cptr.decay(s3), 3));
            (yield* copynchars(cptr.add(tt, FLD.toptenentry_plalign), cptr.decay(s4), 3));
            (yield* copynchars(cptr.add(tt, FLD.toptenentry_name), cptr.decay(s5), 10));
            (yield* copynchars(cptr.add(tt, FLD.toptenentry_death), cptr.decay(s6), 100));
        } else
            cptr.stI64o(tt, FLD.toptenentry_points, 0n);
    }
    if (cptr.ldI64o(tt, FLD.toptenentry_points) > 0n) {
        if (cptr.ldI64o(tt, FLD.toptenentry_birthdate) < 19000000n)
            cptr.stI64o(tt, FLD.toptenentry_birthdate, cptr.ldI64o(tt, FLD.toptenentry_birthdate) + 19000000n);
        if (cptr.ldI64o(tt, FLD.toptenentry_deathdate) < 19000000n)
            cptr.stI64o(tt, FLD.toptenentry_deathdate, cptr.ldI64o(tt, FLD.toptenentry_deathdate) + 19000000n);
    }
}

const __static_writeentry_fmt32 = cptr.bytes("%c%c "); /** C ref: topten.c:303 — char[6] (function-static) */
const __static_writeentry_fmt33 = cptr.bytes("%s %s %s %s "); /** C ref: topten.c:304 — char[13] (function-static) */
const __static_writeentry_fmt0 = cptr.bytes("%d.%d.%d %ld %d %d %d %d %d %d %ld %ld %d "); /** C ref: topten.c:306 — char[43] (function-static) */
const __static_writeentry_fmtX = cptr.bytes("%s,%s\n"); /** C ref: topten.c:307 — char[7] (function-static) */

/** C ref: topten.c:301 — @param {CPtr} rfile @param {CPtr} tt */
function writeentry(rfile, tt) {
    void fprintf(rfile, cptr.decay(__static_writeentry_fmt0), cptr.ldI32o(tt, FLD.toptenentry_ver_major), cptr.ldI32o(tt, FLD.toptenentry_ver_minor), cptr.ldI32o(tt, FLD.toptenentry_patchlevel), cptr.ldI64o(tt, FLD.toptenentry_points), cptr.ldI32o(tt, FLD.toptenentry_deathdnum), cptr.ldI32o(tt, FLD.toptenentry_deathlev), cptr.ldI32o(tt, FLD.toptenentry_maxlvl), cptr.ldI32o(tt, FLD.toptenentry_hp), cptr.ldI32o(tt, FLD.toptenentry_maxhp), cptr.ldI32o(tt, FLD.toptenentry_deaths), cptr.ldI64o(tt, FLD.toptenentry_deathdate), cptr.ldI64o(tt, FLD.toptenentry_birthdate), cptr.ldI32o(tt, FLD.toptenentry_uid));
    if (cptr.ldI32o(tt, FLD.toptenentry_ver_major) < 3 || (cptr.ldI32o(tt, FLD.toptenentry_ver_major) == 3 && cptr.ldI32o(tt, FLD.toptenentry_ver_minor) < 3))
        void fprintf(rfile, cptr.decay(__static_writeentry_fmt32), cptr.ld1so2(tt, 0, 1, FLD.toptenentry_plrole), cptr.ld1so2(tt, 0, 1, FLD.toptenentry_plgend));
    else
        void fprintf(rfile, cptr.decay(__static_writeentry_fmt33), cptr.add(tt, FLD.toptenentry_plrole), cptr.add(tt, FLD.toptenentry_plrace), cptr.add(tt, FLD.toptenentry_plgend), cptr.add(tt, FLD.toptenentry_plalign));
    void fprintf(rfile, cptr.decay(__static_writeentry_fmtX), onlyspace(cptr.add(tt, FLD.toptenentry_name)) ? __sl19 : cptr.add(tt, FLD.toptenentry_name), cptr.add(tt, FLD.toptenentry_death));
}

/** C ref: topten.c:340 — @param {CPtr} rfile @param {CPtr} tt @param {CInt} how */
function* writexlentry(rfile, tt, how) {
    let buf = new Uint8Array(256);
    let tmpbuf = new Uint8Array(101);
    let achbuf = new Uint8Array(1280);
    void cptr.sprintf(cptr.decay(buf), __sl20, cptr.ldI32o(tt, FLD.toptenentry_ver_major), cptr.ldI32o(tt, FLD.toptenentry_ver_minor), cptr.ldI32o(tt, FLD.toptenentry_patchlevel));
    void cptr.sprintf(eos(cptr.decay(buf)), __sl21, 9, cptr.ldI64o(tt, FLD.toptenentry_points), 9, cptr.ldI32o(tt, FLD.toptenentry_deathdnum), 9, cptr.ldI32o(tt, FLD.toptenentry_deathlev));
    void cptr.sprintf(eos(cptr.decay(buf)), __sl22, 9, cptr.ldI32o(tt, FLD.toptenentry_maxlvl), 9, cptr.ldI32o(tt, FLD.toptenentry_hp), 9, cptr.ldI32o(tt, FLD.toptenentry_maxhp));
    void cptr.sprintf(eos(cptr.decay(buf)), __sl23, 9, cptr.ldI32o(tt, FLD.toptenentry_deaths), 9, cptr.ldI64o(tt, FLD.toptenentry_deathdate), 9, cptr.ldI64o(tt, FLD.toptenentry_birthdate), 9, cptr.ldI32o(tt, FLD.toptenentry_uid));
    void fprintf(rfile, __sl24, cptr.decay(buf));
    void cptr.sprintf(cptr.decay(buf), __sl25, 9, cptr.add(tt, FLD.toptenentry_plrole), 9, cptr.add(tt, FLD.toptenentry_plrace), 9, cptr.add(tt, FLD.toptenentry_plgend), 9, cptr.add(tt, FLD.toptenentry_plalign));
    (yield* formatkiller(cptr.decay(tmpbuf), 101, how, 0));
    void fprintf(rfile, __sl26, cptr.decay(buf), 9, svp, 9, cptr.decay(tmpbuf));
    if (cptr.ldI64o(gm, FLD.instance_globals_m_multi) < 0n)
        void fprintf(rfile, __sl27, 9, cptr.ldPtro(gm, FLD.instance_globals_m_multi_reason) ? cptr.ldPtro(gm, FLD.instance_globals_m_multi_reason) : __sl28);
    void fprintf(rfile, __sl29, 9, (yield* encodeconduct()), 9, cptr.ldI64o(svm, FLD.instance_globals_saved_m_moves), 9, encodeachieve(0));
    void fprintf(rfile, __sl30, 9, (yield* encode_extended_achievements(cptr.decay(achbuf))));
    void fprintf(rfile, __sl31, 9, (yield* encode_extended_conducts(cptr.decay(buf))));
    void fprintf(rfile, __sl32, 9, cptr.ldI64(urealtime), 9, timet_to_seconds(ubirthday.v), 9, timet_to_seconds(cptr.ldI64o(urealtime, FLD.u_realtime_finish_time)));
    void fprintf(rfile, __sl33, 9, cptr.ldPtro2(genders, cptr.ldI32o(flags, FLD.flag_initgend), 48, FLD.Gender_filecode), 9, cptr.ldPtro2(aligns, (1 - cptr.ld1so2(u, NHM.A_ORIGINAL, 1, FLD.you_ualignbase)) | 0, 32, FLD.Align_filecode));
    void fprintf(rfile, __sl34, 9, encodexlogflags());
    void fprintf(rfile, __sl35, 9, BigInt.asIntN(64, money_cnt(cptr.ldPtro(gi, FLD.instance_globals_i_invent)) + hidden_gold(1)));
    void fprintf(rfile, __sl36, 9, cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_wishes));
    void fprintf(rfile, __sl37, 9, cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_wisharti));
    void fprintf(rfile, __sl38, 9, cptr.ldI64o(u, FLD.you_uroleplay + FLD.u_roleplay_numbones));
    void fprintf(rfile, __sl39, 9, cptr.ldI64o(u, FLD.you_uroleplay + FLD.u_roleplay_numrerolls));
    void fprintf(rfile, __sl15);
}

/** C ref: topten.c:394 @returns {CLongLong} */
function encodexlogflags() {
    let e = 0n;
    if (cptr.ld1so(flags, FLD.flag_debug))
        e |= 1n;
    if (cptr.ld1so(flags, FLD.flag_explore))
        e |= 2n;
    if (!cptr.ldI64o(u, FLD.you_uroleplay + FLD.u_roleplay_numbones))
        e |= 4n;
    if (cptr.ld1so(u, FLD.you_uroleplay + FLD.u_roleplay_reroll))
        e |= 8n;
    return e;
}

/** C ref: topten.c:411 @returns {CLongLong} */
function* encodeconduct() {
    let e = 0n;
    if (!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_food))
        e |= 1n;
    if (!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_unvegan))
        e |= 2n;
    if (!cptr.ldI64o(u, FLD.you_uconduct))
        e |= 4n;
    if (!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_gnostic))
        e |= 8n;
    if (!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_weaphit))
        e |= 16n;
    if (!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_killer))
        e |= 32n;
    if (!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_literate))
        e |= 64n;
    if (!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_polypiles))
        e |= 128n;
    if (!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_polyselfs))
        e |= 256n;
    if (!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_wishes))
        e |= 512n;
    if (!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_wisharti))
        e |= 1024n;
    if (!(yield* num_genocides()))
        e |= 2048n;
    if (!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_sokocheat) && sokoban_in_play())
        e |= 4096n;
    if (!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_pets))
        e |= 8192n;
    return e;
}

/** C ref: topten.c:455 — @param {CInt} secondlong @returns {CLongLong} */
function encodeachieve(secondlong) {
    let i;
    let achidx;
    let offset;
    let r = 0n;
    offset = secondlong ? 31 : 0;
    for (i = 0; cptr.ld1so2(u, i, 1, FLD.you_uachieved); ++i) {
        achidx = (cptr.ld1so2(u, i, 1, FLD.you_uachieved) - offset) | 0;
        if (achidx > 0 && achidx < 32)
            r |= 1n << BigInt(((achidx - 1) | 0));
    }
    return r;
}

/** C ref: topten.c:480 — @param {CPtr} buf @param {CPtr} achievement @param {CInt} condition */
function add_achieveX(buf, achievement, condition) {
    if (condition) {
        if (cptr.ld1so(buf, 0) != 0) {
            void cptr.strcat(buf, __sl40);
        }
        void cptr.strcat(buf, achievement);
    }
}

/** C ref: topten.c:491 — @param {CPtr} buf @returns {CPtr} */
function* encode_extended_achievements(buf) {
    let rnkbuf = new Uint8Array(40);
    let achievement = null;
    let i;
    let achidx;
    let absidx;
    cptr.st1o(buf, 0, 0);
    for (i = 0; cptr.ld1so2(u, i, 1, FLD.you_uachieved); i++) {
        achidx = cptr.ld1so2(u, i, 1, FLD.you_uachieved);
        absidx = Math.abs(achidx);
        switch (absidx) {
            case NHC.ACH_UWIN:
            achievement = __sl41;
            break;
            case NHC.ACH_ASTR:
            achievement = __sl42;
            break;
            case NHC.ACH_ENDG:
            achievement = __sl43;
            break;
            case NHC.ACH_AMUL:
            achievement = __sl44;
            break;
            case NHC.ACH_INVK:
            achievement = __sl45;
            break;
            case NHC.ACH_BOOK:
            achievement = __sl46;
            break;
            case NHC.ACH_BELL:
            achievement = __sl47;
            break;
            case NHC.ACH_CNDL:
            achievement = __sl48;
            break;
            case NHC.ACH_HELL:
            achievement = __sl49;
            break;
            case NHC.ACH_MEDU:
            achievement = __sl50;
            break;
            case NHC.ACH_MINE_PRIZE:
            achievement = __sl51;
            break;
            case NHC.ACH_SOKO_PRIZE:
            achievement = __sl52;
            break;
            case NHC.ACH_ORCL:
            achievement = __sl53;
            break;
            case NHC.ACH_NOVL:
            achievement = __sl54;
            break;
            case NHC.ACH_MINE:
            achievement = __sl55;
            break;
            case NHC.ACH_TOWN:
            achievement = __sl56;
            break;
            case NHC.ACH_SHOP:
            achievement = __sl57;
            break;
            case NHC.ACH_TMPL:
            achievement = __sl58;
            break;
            case NHC.ACH_SOKO:
            achievement = __sl59;
            break;
            case NHC.ACH_BGRM:
            achievement = __sl60;
            break;
            case NHC.ACH_TUNE:
            achievement = __sl61;
            break;
            case NHC.ACH_RNK1:
            case NHC.ACH_RNK2:
            case NHC.ACH_RNK3:
            case NHC.ACH_RNK4:
            case NHC.ACH_RNK5:
            case NHC.ACH_RNK6:
            case NHC.ACH_RNK7:
            case NHC.ACH_RNK8:
            void cptr.sprintf(cptr.decay(rnkbuf), __sl62, rank_of(rank_to_xlev((absidx - ((NHC.ACH_RNK1 - 1) | 0)) | 0), (cptr.ldI16o(gu, FLD.instance_globals_u_urole + FLD.Role_mnum)), schar(((achidx < 0) ? 1 : 0))));
            (yield* strNsubst(cptr.decay(rnkbuf), __sl63, __sl19, 0));
            achievement = lcase(cptr.decay(rnkbuf));
            break;
            default:
            continue;
        }
        add_achieveX(buf, achievement, 1);
    }
    return buf;
}

/** C ref: topten.c:584 — @param {CPtr} buf @returns {CPtr} */
function* encode_extended_conducts(buf) {
    cptr.st1o(buf, 0, 0);
    add_achieveX(buf, __sl64, schar((!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_food))));
    add_achieveX(buf, __sl65, schar((!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_unvegan))));
    add_achieveX(buf, __sl66, schar((!cptr.ldI64o(u, FLD.you_uconduct))));
    add_achieveX(buf, __sl67, schar((!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_gnostic))));
    add_achieveX(buf, __sl68, schar((!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_weaphit))));
    add_achieveX(buf, __sl69, schar((!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_killer))));
    add_achieveX(buf, __sl70, schar((!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_literate))));
    add_achieveX(buf, __sl71, schar((!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_polypiles))));
    add_achieveX(buf, __sl72, schar((!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_polyselfs))));
    add_achieveX(buf, __sl73, schar((!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_wishes))));
    add_achieveX(buf, __sl74, schar((!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_wisharti))));
    add_achieveX(buf, __sl75, schar((!(yield* num_genocides()))));
    if (sokoban_in_play())
        add_achieveX(buf, __sl76, schar((!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_sokocheat))));
    add_achieveX(buf, __sl77, cptr.ld1so(u, FLD.you_uroleplay));
    add_achieveX(buf, __sl78, cptr.ld1so(u, FLD.you_uroleplay + FLD.u_roleplay_deaf));
    add_achieveX(buf, __sl79, cptr.ld1so(u, FLD.you_uroleplay + FLD.u_roleplay_nudist));
    add_achieveX(buf, __sl80, cptr.ld1so(u, FLD.you_uroleplay + FLD.u_roleplay_pauper));
    add_achieveX(buf, __sl81, schar((!cptr.ld1so(flags, FLD.flag_bones))));
    add_achieveX(buf, __sl82, schar((!cptr.ldI64o(u, FLD.you_uconduct + FLD.u_conduct_pets))));
    add_achieveX(buf, __sl83, schar((!cptr.ld1so(u, FLD.you_uroleplay + FLD.u_roleplay_reroll))));
    return buf;
}

/** C ref: topten.c:615 — @param {CPtr} tt */
function free_ttlist(tt) {
    let ttnext;
    while (cptr.ldI64o(tt, FLD.toptenentry_points) > 0n) {
        ttnext = cptr.ldPtr(tt);
        cptr.free((tt));
        tt = ttnext;
    }
    cptr.free((tt));
}

/** C ref: topten.c:628 — @param {CInt} how @param {CLongLong} when */
export function* topten(how, when) {
    let t0;
    let tprev;
    let t1;
    let rfile;
    let lfile;
    let xlfile;
    let uid = getuid() | 0;
    let rank;
    let rank0 = -1;
    let rank1 = 0;
    let occ_cnt = cptr.ldI32o(sysopt, FLD.sysopt_s_persmax);
    let flg = 0;
    let t0_used;
    let skip_scores;
    __lbl_destroywin: {
    __lbl_showwin: {
        if (cptr.ldI32o(program_state, FLD.sinfo_panicking))
            return;
        if (cptr.ld1so(iflags, FLD.instance_flags_toptenwin)) {
            cptr.stI32o(gt, FLD.instance_globals_t_toptenwin, (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_create_nhwindow))(NHM.NHW_TEXT))));
        }
        t0_used = 0;
        t0 = (yield* alloc(208));
        cptr.memcpy(t0, zerott, 208);
        cptr.stI32o(t0, FLD.toptenentry_ver_major, NHM.VERSION_MAJOR);
        cptr.stI32o(t0, FLD.toptenentry_ver_minor, NHM.VERSION_MINOR);
        cptr.stI32o(t0, FLD.toptenentry_patchlevel, NHM.PATCHLEVEL);
        cptr.stI64o(t0, FLD.toptenentry_points, cptr.ldI64o(u, FLD.you_urexp));
        cptr.stI32o(t0, FLD.toptenentry_deathdnum, cptr.ldI16o(u, FLD.you_uz));
        cptr.stI32o(t0, FLD.toptenentry_deathlev, observable_depth(cptr.add(u, FLD.you_uz)));
        cptr.stI32o(t0, FLD.toptenentry_maxlvl, deepest_lev_reached(1));
        cptr.stI32o(t0, FLD.toptenentry_hp, cptr.ldI32o(u, FLD.you_uhp));
        cptr.stI32o(t0, FLD.toptenentry_maxhp, cptr.ldI32o(u, FLD.you_uhpmax));
        cptr.stI32o(t0, FLD.toptenentry_deaths, cptr.ldI32o(u, FLD.you_umortality));
        cptr.stI32o(t0, FLD.toptenentry_uid, uid);
        (yield* copynchars(cptr.add(t0, FLD.toptenentry_plrole), cptr.ldPtro(gu, FLD.instance_globals_u_urole + FLD.Role_filecode), 3));
        (yield* copynchars(cptr.add(t0, FLD.toptenentry_plrace), cptr.ldPtro(gu, FLD.instance_globals_u_urace + FLD.Race_filecode), 3));
        (yield* copynchars(cptr.add(t0, FLD.toptenentry_plgend), cptr.ldPtro2(genders, cptr.ld1so(flags, FLD.flag_female), 48, FLD.Gender_filecode), 3));
        (yield* copynchars(cptr.add(t0, FLD.toptenentry_plalign), cptr.ldPtro2(aligns, (1 - cptr.ld1so(u, FLD.you_ualign)) | 0, 32, FLD.Align_filecode), 3));
        (yield* copynchars(cptr.add(t0, FLD.toptenentry_name), svp, 10));
        (yield* formatkiller(cptr.add(t0, FLD.toptenentry_death), 101, how, 1));
        cptr.stI64o(t0, FLD.toptenentry_birthdate, (yield* yyyymmdd(ubirthday.v)));
        cptr.stI64o(t0, FLD.toptenentry_deathdate, (yield* yyyymmdd(when)));
        cptr.stPtr(t0, null);
        if ((yield* lock_file(__sl84, NHM.SCOREPREFIX, 10))) {
            if (!(lfile = fopen_datafile(__sl84, __sl85, NHM.SCOREPREFIX))) {
                if (!cptr.ldI32o(program_state, FLD.sinfo_done_hup))
                    (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_raw_print))(__sl86)));
            } else {
                writeentry(lfile, t0);
                void fclose(lfile);
            }
            (yield* unlock_file(__sl84));
        }
        if ((yield* lock_file(__sl87, NHM.SCOREPREFIX, 10))) {
            if (!(xlfile = fopen_datafile(__sl87, __sl85, NHM.SCOREPREFIX))) {
                if (!cptr.ldI32o(program_state, FLD.sinfo_done_hup))
                    (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_raw_print))(__sl88)));
            } else {
                (yield* writexlentry(xlfile, t0, how));
                void fclose(xlfile);
            }
            (yield* unlock_file(__sl87));
        }
        if (cptr.ld1so(flags, FLD.flag_debug) || cptr.ld1so(flags, FLD.flag_explore)) {
            if (how != NHC.PANICKED)
                if (!cptr.ldI32o(program_state, FLD.sinfo_done_hup)) {
                    let pbuf = new Uint8Array(256);
                    (yield* topten_print(__sl14));
                    void cptr.sprintf(cptr.decay(pbuf), __sl89, cptr.ld1so(flags, FLD.flag_debug) ? __sl90 : __sl91);
                    (yield* topten_print(cptr.decay(pbuf)));
                }
            break __lbl_showwin;
        }
        if (!(yield* lock_file(__sl92, NHM.SCOREPREFIX, 60)))
            break __lbl_destroywin;
        rfile = fopen_datafile(__sl92, __sl93, NHM.SCOREPREFIX);
        if (!rfile) {
            if (!cptr.ldI32o(program_state, FLD.sinfo_done_hup))
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_raw_print))(__sl94)));
            (yield* unlock_file(__sl92));
            break __lbl_destroywin;
        }
        if (!cptr.ldI32o(program_state, FLD.sinfo_done_hup))
            (yield* topten_print(__sl14));
        if (cptr.ldI64o(t0, FLD.toptenentry_points) < BigInt(cptr.ldI32o(sysopt, FLD.sysopt_s_pointsmin)))
            cptr.stI64o(t0, FLD.toptenentry_points, 0n);
        t1 = (tt_head = (yield* alloc(208)));
        tprev = null;
        for (rank = 1; ; ) {
            (yield* readentry(rfile, t1));
            if (cptr.ldI64o(t1, FLD.toptenentry_points) < BigInt(cptr.ldI32o(sysopt, FLD.sysopt_s_pointsmin)))
                cptr.stI64o(t1, FLD.toptenentry_points, 0n);
            if (rank0 < 0 && cptr.ldI64o(t1, FLD.toptenentry_points) < cptr.ldI64o(t0, FLD.toptenentry_points)) {
                rank0 = rank++;
                if (tprev === null)
                    tt_head = t0;
                else
                    cptr.stPtr(tprev, t0);
                cptr.stPtr(t0, t1);
                t0_used = 1;
                occ_cnt--;
                flg++;
            } else
                tprev = t1;
            if (cptr.ldI64o(t1, FLD.toptenentry_points) == 0n)
                break;
            if ((cptr.ldI32o(sysopt, FLD.sysopt_s_pers_is_uid) ? cptr.ldI32o(t1, FLD.toptenentry_uid) == cptr.ldI32o(t0, FLD.toptenentry_uid) : cptr.strncmp(cptr.add(t1, FLD.toptenentry_name), cptr.add(t0, FLD.toptenentry_name), 10n) == 0) && !cptr.strncmp(cptr.add(t1, FLD.toptenentry_plrole), cptr.add(t0, FLD.toptenentry_plrole), 3n) && --occ_cnt <= 0) {
                if (rank0 < 0) {
                    rank0 = 0;
                    rank1 = rank;
                    if (!cptr.ldI32o(program_state, FLD.sinfo_done_hup)) {
                        let pbuf = new Uint8Array(256);
                        void cptr.sprintf(cptr.decay(pbuf), __sl95, cptr.ldI64o(t1, FLD.toptenentry_points));
                        (yield* topten_print(cptr.decay(pbuf)));
                        (yield* topten_print(__sl14));
                    }
                }
                if (occ_cnt < 0) {
                    flg++;
                    continue;
                }
            }
            if (rank <= cptr.ldI32o(sysopt, FLD.sysopt_s_entrymax)) {
                cptr.stPtr(t1, (yield* alloc(208)));
                t1 = cptr.ldPtr(t1);
                rank++;
            }
            if (rank > cptr.ldI32o(sysopt, FLD.sysopt_s_entrymax)) {
                cptr.stI64o(t1, FLD.toptenentry_points, 0n);
                break;
            }
        }
        if (flg) {
            void fclose(rfile);
            if (!(rfile = fopen_datafile(__sl92, __sl96, NHM.SCOREPREFIX))) {
                if (!cptr.ldI32o(program_state, FLD.sinfo_done_hup))
                    (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_raw_print))(__sl97)));
                (yield* unlock_file(__sl92));
                free_ttlist(tt_head);
                break __lbl_destroywin;
            }
            if (!cptr.ldI32o(program_state, FLD.sinfo_stopprint))
                if (rank0 > 0) {
                    if (rank0 <= 10) {
                        (yield* topten_print(__sl98));
                    } else {
                        let pbuf = new Uint8Array(256);
                        void cptr.sprintf(cptr.decay(pbuf), __sl99, rank0, ordin(rank0), cptr.ldI32o(sysopt, FLD.sysopt_s_entrymax));
                        (yield* topten_print(cptr.decay(pbuf)));
                    }
                    (yield* topten_print(__sl14));
                }
        }
        skip_scores = schar((!cptr.ldI32o(flags, FLD.flag_end_top) && !cptr.ldI32o(flags, FLD.flag_end_around) && !cptr.ld1so(flags, FLD.flag_end_own) ? 1 : 0));
        if (rank0 == 0)
            rank0 = rank1;
        if (rank0 <= 0)
            rank0 = rank;
        if (!skip_scores && !cptr.ldI32o(program_state, FLD.sinfo_stopprint))
            (yield* outheader());
        for (t1 = tt_head, rank = 1; cptr.ldI64o(t1, FLD.toptenentry_points) != 0n; t1 = cptr.ldPtr(t1), ++rank) {
            if (flg)
                writeentry(rfile, t1);
            if (skip_scores || cptr.ldI32o(program_state, FLD.sinfo_stopprint))
                continue;
            if (rank <= cptr.ldI32o(flags, FLD.flag_end_top) || (rank >= ((rank0 - cptr.ldI32o(flags, FLD.flag_end_around)) | 0) && rank <= ((rank0 + cptr.ldI32o(flags, FLD.flag_end_around)) | 0)) || (cptr.ld1so(flags, FLD.flag_end_own) && (cptr.ldI32o(sysopt, FLD.sysopt_s_pers_is_uid) ? cptr.ldI32o(t1, FLD.toptenentry_uid) == cptr.ldI32o(t0, FLD.toptenentry_uid) : !cptr.strncmp(cptr.add(t1, FLD.toptenentry_name), cptr.add(t0, FLD.toptenentry_name), 10n)))) {
                if (rank == ((rank0 - cptr.ldI32o(flags, FLD.flag_end_around)) | 0) && rank0 > ((((cptr.ldI32o(flags, FLD.flag_end_top) + cptr.ldI32o(flags, FLD.flag_end_around)) | 0) + 1) | 0) && !cptr.ld1so(flags, FLD.flag_end_own))
                    (yield* topten_print(__sl14));
                if (rank != rank0) {
                    (yield* outentry(rank, t1, 0));
                } else if (!rank1) {
                    (yield* outentry(rank, t1, 1));
                } else {
                    (yield* outentry(rank, t1, 1));
                    (yield* outentry(0, t0, 1));
                }
            }
        }
        if (rank0 >= rank)
            if (!skip_scores && !cptr.ldI32o(program_state, FLD.sinfo_stopprint))
                (yield* outentry(0, t0, 1));
        void fclose(rfile);
        (yield* unlock_file(__sl92));
        free_ttlist(tt_head);
    }
        if (!cptr.ldI32o(program_state, FLD.sinfo_stopprint)) {
            if (cptr.ld1so(iflags, FLD.instance_flags_toptenwin)) {
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_display_nhwindow))(cptr.ldI32o(gt, FLD.instance_globals_t_toptenwin), 1)));
            } else {
                ;
            }
        }
    }
    if (!t0_used)
        cptr.free((t0));
    if (cptr.ld1so(iflags, FLD.instance_flags_toptenwin)) {
        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_destroy_nhwindow))(cptr.ldI32o(gt, FLD.instance_globals_t_toptenwin))));
        cptr.stI32o(gt, FLD.instance_globals_t_toptenwin, -1);
    }
}

/** C ref: topten.c:929 */
function* outheader() {
    let linebuf = new Uint8Array(256);
    let bp;
    void cptr.strcpy(cptr.decay(linebuf), __sl100);
    bp = eos(cptr.decay(linebuf));
    while (cptr.cmp(bp, cptr.add(cptr.add(cptr.decay(linebuf), NHM.COLNO), -(9))) < 0)
        cptr.st1(cptr.postinc(() => bp, (v) => { bp = v; }), 32);
    void cptr.strcpy(bp, __sl101);
    (yield* topten_print(cptr.decay(linebuf)));
}

/** C ref: topten.c:946 — @param {CInt} rank @param {CPtr} t1 @param {CInt} so */
function* outentry(rank, t1, so) {
    let second_line = 1;
    let linebuf = new Uint8Array(256);
    let bp;
    let hpbuf = new Uint8Array(24);
    let linebuf3 = new Uint8Array(256);
    let hppos;
    let lngr;
    cptr.st1o(cptr.decay(linebuf), 0, 0, 1);
    if (rank)
        void cptr.sprintf(eos(cptr.decay(linebuf)), __sl102, rank);
    else
        void cptr.strcat(cptr.decay(linebuf), __sl103);
    void cptr.sprintf(eos(cptr.decay(linebuf)), __sl104, cptr.ldI64o(t1, FLD.toptenentry_points) ? cptr.ldI64o(t1, FLD.toptenentry_points) : cptr.ldI64o(u, FLD.you_urexp), cptr.add(t1, FLD.toptenentry_name));
    void cptr.sprintf(eos(cptr.decay(linebuf)), __sl105, cptr.add(t1, FLD.toptenentry_plrole));
    if (cptr.ld1so2(t1, 0, 1, FLD.toptenentry_plrace) != 63)
        void cptr.sprintf(eos(cptr.decay(linebuf)), __sl105, cptr.add(t1, FLD.toptenentry_plrace));
    void cptr.sprintf(eos(cptr.decay(linebuf)), __sl105, cptr.add(t1, FLD.toptenentry_plgend));
    if (cptr.ld1so2(t1, 0, 1, FLD.toptenentry_plalign) != 63)
        void cptr.sprintf(eos(cptr.decay(linebuf)), __sl106, cptr.add(t1, FLD.toptenentry_plalign));
    else
        void cptr.strcat(cptr.decay(linebuf), __sl63);
    if (!cptr.strncmp(__sl107, cptr.add(t1, FLD.toptenentry_death), 7n)) {
        void cptr.sprintf(eos(cptr.decay(linebuf)), __sl108, !cptr.strncmp(__sl109, cptr.add(t1, FLD.toptenentry_death + 7), 2n) ? cptr.add(t1, FLD.toptenentry_death + 7 + 2) : __sl14, cptr.ldI32o(t1, FLD.toptenentry_maxlvl));
        if ((bp = cptr.strchr(cptr.decay(linebuf), 41)) !== null)
            cptr.st1(bp, schar(((cptr.ldI32o(t1, FLD.toptenentry_deathdnum) == cptr.ldI16((cptr.add(svd, FLD.instance_globals_saved_d_dungeon_topology + FLD.dgn_topology_d_astral_level)))) ? 0 : 32)));
        second_line = 0;
    } else if (!cptr.strncmp(__sl41, cptr.add(t1, FLD.toptenentry_death), 8n)) {
        void cptr.sprintf(eos(cptr.decay(linebuf)), __sl110, (cptr.ld1so2(t1, 0, 1, FLD.toptenentry_plgend) == 70) ? __sl111 : __sl14);
        second_line = 0;
    } else {
        if (!cptr.strncmp(cptr.add(t1, FLD.toptenentry_death), __sl112, 4n)) {
            void cptr.strcat(cptr.decay(linebuf), __sl112);
            second_line = 0;
        } else if (!cptr.strncmp(cptr.add(t1, FLD.toptenentry_death), __sl113, 10n)) {
            void cptr.strcat(cptr.decay(linebuf), __sl114);
            second_line = 0;
        } else if (!cptr.strncmp(cptr.add(t1, FLD.toptenentry_death), __sl115, 6n)) {
            void cptr.sprintf(eos(cptr.decay(linebuf)), __sl116, (cptr.ld1so2(t1, 0, 1, FLD.toptenentry_plgend) == 70) ? __sl117 : __sl118);
        } else if (!cptr.strncmp(cptr.add(t1, FLD.toptenentry_death), __sl119, 8n)) {
            void cptr.strcat(cptr.decay(linebuf), __sl120);
        } else if (!cptr.strncmp(cptr.add(t1, FLD.toptenentry_death), __sl121, 7n)) {
            void cptr.strcat(cptr.decay(linebuf), __sl122);
        } else if (!cptr.strncmp(cptr.add(t1, FLD.toptenentry_death), __sl12, 13n)) {
            void cptr.strcat(cptr.decay(linebuf), __sl123);
        } else
            void cptr.strcat(cptr.decay(linebuf), __sl124);
        if (cptr.ldI32o(t1, FLD.toptenentry_deathdnum) == cptr.ldI16((cptr.add(svd, FLD.instance_globals_saved_d_dungeon_topology + FLD.dgn_topology_d_astral_level)))) {
            let arg;
            let fmt = __sl125;
            switch (cptr.ldI32o(t1, FLD.toptenentry_deathlev)) {
                case -5:
                fmt = __sl126;
                arg = __sl127;
                break;
                case -4:
                arg = __sl128;
                break;
                case -3:
                arg = __sl129;
                break;
                case -2:
                arg = __sl130;
                break;
                case -1:
                arg = __sl131;
                break;
                default:
                arg = __sl132;
                break;
            }
            void cptr.sprintf(eos(cptr.decay(linebuf)), fmt, arg);
        } else {
            void cptr.sprintf(eos(cptr.decay(linebuf)), __sl133, cptr.add(svd, cptr.ldI32o(t1, FLD.toptenentry_deathdnum), 112));
            if (cptr.ldI32o(t1, FLD.toptenentry_deathdnum) != cptr.ldI16((cptr.add(svd, FLD.instance_globals_saved_d_dungeon_topology + FLD.dgn_topology_d_knox_level))))
                void cptr.sprintf(eos(cptr.decay(linebuf)), __sl134, cptr.ldI32o(t1, FLD.toptenentry_deathlev));
            if (cptr.ldI32o(t1, FLD.toptenentry_deathlev) != cptr.ldI32o(t1, FLD.toptenentry_maxlvl))
                void cptr.sprintf(eos(cptr.decay(linebuf)), __sl135, cptr.ldI32o(t1, FLD.toptenentry_maxlvl));
        }
        if (!cptr.strncmp(cptr.add(t1, FLD.toptenentry_death), __sl136, 5n))
            void cptr.strcat(cptr.decay(linebuf), cptr.add(t1, FLD.toptenentry_death + 4));
    }
    void cptr.strcat(cptr.decay(linebuf), __sl137);
    if (second_line) {
        bp = eos(cptr.decay(linebuf));
        void cptr.sprintf(bp, __sl138, highc(cptr.ld1s((cptr.add(t1, FLD.toptenentry_death)))), cptr.add(t1, FLD.toptenentry_death + 1));
        void strsubst(bp, __sl139, __sl140);
    }
    lngr = Number(BigInt.asIntN(32, cptr.strlen(cptr.decay(linebuf))));
    if (cptr.ldI32o(t1, FLD.toptenentry_hp) <= 0)
        cptr.st1o(cptr.decay(hpbuf), 0, 45, 1), cptr.st1o(cptr.decay(hpbuf), 1, 0, 1);
    else
        void cptr.sprintf(cptr.decay(hpbuf), __sl141, cptr.ldI32o(t1, FLD.toptenentry_hp));
    hppos = 70;
    while (lngr >= hppos) {
        for (bp = eos(cptr.decay(linebuf)); !(cptr.ld1s(bp) == 32 && cptr.diff(bp, cptr.decay(linebuf)) < BigInt(hppos)); bp = cptr.add(bp, -1))
            ;
        if (cptr.cmp(cptr.add(cptr.decay(linebuf), 15), bp) >= 0)
            bp = cptr.add(cptr.add(cptr.decay(linebuf), hppos), -(1));
        if (cptr.cmp(bp, cptr.add(cptr.decay(linebuf), 5)) > 0 && !cptr.strncmp(cptr.add(bp, -(5)), __sl142, 5n))
            bp = cptr.sub(bp, 5);
        if (cptr.ld1s(bp) != 32)
            void cptr.strcpy(cptr.decay(linebuf3), bp);
        else
            void cptr.strcpy(cptr.decay(linebuf3), cptr.add(bp, 1));
        cptr.st1(bp, 0);
        if (so) {
            while (cptr.cmp(bp, cptr.add(cptr.decay(linebuf), 79)) < 0)
                cptr.st1(cptr.postinc(() => bp, (v) => { bp = v; }), 32);
            cptr.st1(bp, 0);
            (yield* topten_print_bold(cptr.decay(linebuf)));
        } else
            (yield* topten_print(cptr.decay(linebuf)));
        nh_snprintf(__sl143, 1082, cptr.decay(linebuf), 256n, __sl144, __sl14, cptr.decay(linebuf3));
        lngr = (yield* Strlen_(cptr.decay(linebuf), __sl143, 1083)) | 0;
    }
    hppos = (73 - Number(BigInt.asIntN(32, cptr.strlen(cptr.decay(hpbuf))))) | 0;
    bp = eos(cptr.decay(linebuf));
    if (cptr.cmp(bp, cptr.add(cptr.decay(linebuf), hppos)) <= 0) {
        while (cptr.cmp(bp, cptr.add(cptr.decay(linebuf), hppos)) < 0)
            cptr.st1(cptr.postinc(() => bp, (v) => { bp = v; }), 32);
        void cptr.strcpy(bp, cptr.decay(hpbuf));
        void cptr.sprintf(eos(bp), __sl145, (cptr.ldI32o(t1, FLD.toptenentry_maxhp) < 10) ? __sl146 : ((cptr.ldI32o(t1, FLD.toptenentry_maxhp) < 100) ? __sl63 : __sl14), cptr.ldI32o(t1, FLD.toptenentry_maxhp));
    }
    if (so) {
        bp = eos(cptr.decay(linebuf));
        while (cptr.cmp(bp, cptr.add(cptr.decay(linebuf), 79)) < 0)
            cptr.st1(cptr.postinc(() => bp, (v) => { bp = v; }), 32);
        cptr.st1(bp, 0);
        (yield* topten_print_bold(cptr.decay(linebuf)));
    } else
        (yield* topten_print(cptr.decay(linebuf)));
}

/** C ref: topten.c:1112 — @param {CInt} current_ver @param {CInt} rank @param {CPtr} t1 @param {CInt} playerct @param {CPtr} players @param {CInt} uid @returns {CInt} */
function* score_wanted(current_ver, rank, t1, playerct, players, uid) {
    let arg;
    let nxt;
    let i;
    if (current_ver && (cptr.ldI32o(t1, FLD.toptenentry_ver_major) != NHM.VERSION_MAJOR || cptr.ldI32o(t1, FLD.toptenentry_ver_minor) != NHM.VERSION_MINOR || cptr.ldI32o(t1, FLD.toptenentry_patchlevel) != NHM.PATCHLEVEL))
        return 0;
    if (cptr.ldI32o(sysopt, FLD.sysopt_s_pers_is_uid) && !playerct && cptr.ldI32o(t1, FLD.toptenentry_uid) == uid)
        return 1;
    for (i = 0; i < playerct; i++) {
        arg = cptr.ldPtro(players, i, 8);
        if (cptr.ld1so(arg, 0) == 45 && cptr.ld1so(arg, 1) == 117 && cptr.ld1so(arg, 2) != 0)
            arg = cptr.add(arg, 2);
        if (cptr.ld1so(arg, 0) == 45 && cptr.strchr(__sl147, cptr.ld1so(arg, 1)) && !cptr.ld1so(arg, 2) && ((i + 1) | 0) < playerct) {
            nxt = cptr.ldPtro(players, (i + 1) | 0, 8);
            if ((cptr.ld1so(arg, 1) == 112 && (yield* str2role(nxt)) == (yield* str2role(cptr.add(t1, FLD.toptenentry_plrole)))) || (cptr.ld1so(arg, 1) == 114 && (yield* str2race(nxt)) == (yield* str2race(cptr.add(t1, FLD.toptenentry_plrace)))) || (cptr.ld1so(arg, 1) == 117 && (!strcmp(nxt, __sl148) || !cptr.strncmp(cptr.add(t1, FLD.toptenentry_name), nxt, 10n))))
                return 1;
            i++;
        } else if (!strcmp(arg, __sl148) || !cptr.strncmp(cptr.add(t1, FLD.toptenentry_name), arg, 10n) || (cptr.ld1so(arg, 0) == 45 && cptr.ld1so(arg, 1) == cptr.ld1so2(t1, 0, 1, FLD.toptenentry_plrole) && !cptr.ld1so(arg, 2)) || (digit(cptr.ld1so(arg, 0)) && rank <= atoi(arg)))
            return 1;
    }
    return 0;
}

/** C ref: topten.c:1194 — @param {CInt} argc @param {CPtr} argv */
export function* prscore(argc, argv) {
    let players;
    let player0 = cptr.box(0);
    let i;
    let playerct;
    let rank;
    let t1;
    let rfile;
    let pbuf = new Uint8Array(256);
    let p;
    let ln;
    let uid = -1;
    let current_ver = 1;
    let init_done = 0;
    let match_found = 0;
    ln = (argc < 2) ? 0 : (((p = cptr.strchr(cptr.ldPtro(argv, 1, 8), 32)) !== null) ? Number(BigInt.asUintN(32, (cptr.diff(p, cptr.ldPtro(argv, 1, 8))))) : (yield* Strlen_(cptr.ldPtro(argv, 1, 8), __sl149, 1208)));
    if (ln < 2 || (cptr.strncmp(cptr.ldPtro(argv, 1, 8), __sl150, 2n) && strcmp(cptr.ldPtro(argv, 1, 8), __sl151))) {
        (yield* raw_printf(__sl152, argc));
        return;
    }
    rfile = fopen_datafile(__sl92, __sl93, NHM.SCOREPREFIX);
    if (!rfile) {
        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_raw_print))(__sl94)));
        return;
    }
    if (cptr.ldI16o((cptr.add(svd, FLD.instance_globals_saved_d_dungeon_topology + FLD.dgn_topology_d_wiz1_level)), FLD.d_level_dlevel) == 0) {
        ;
        (yield* init_dungeons());
        init_done = 1;
    }
    if (cptr.ld1so(cptr.ldPtro(argv, 1, 8), 1) == 45 || !cptr.ld1so(cptr.ldPtro(argv, 1, 8), 2)) {
        argc--;
        argv = cptr.add(argv, 1, 8);
    } else {
        cptr.stPtro(argv, 1, cptr.add(cptr.ldPtro(argv, 1, 8), 2), 8);
    }
    if (argc > 1 && !strcmp(cptr.ldPtro(argv, 1, 8), __sl153)) {
        current_ver = 0;
        argc--;
        argv = cptr.add(argv, 1, 8);
    }
    if (argc <= 1) {
        if (cptr.ldI32o(sysopt, FLD.sysopt_s_pers_is_uid)) {
            uid = getuid() | 0;
            playerct = 0;
            players = null;
        } else {
            player0.v = svp;
            if (!cptr.ld1s(player0.v))
                player0.v = __sl148;
            playerct = 1;
            players = player0;
        }
    } else {
        playerct = --argc;
        players = cptr.preinc(() => argv, (v) => { argv = v; }, 8);
    }
    (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_raw_print))(__sl14)));
    t1 = (tt_head = (yield* alloc(208)));
    for (rank = 1; ; rank++) {
        (yield* readentry(rfile, t1));
        if (cptr.ldI64o(t1, FLD.toptenentry_points) == 0n)
            break;
        if (!match_found && (yield* score_wanted(current_ver, rank, t1, playerct, players, uid)))
            match_found = 1;
        cptr.stPtr(t1, (yield* alloc(208)));
        t1 = cptr.ldPtr(t1);
    }
    void fclose(rfile);
    if (init_done) {
        (yield* free_dungeons());
        ;
    }
    if (match_found) {
        (yield* outheader());
        t1 = tt_head;
        for (rank = 1; cptr.ldI64o(t1, FLD.toptenentry_points) != 0n; rank++, t1 = cptr.ldPtr(t1)) {
            if ((yield* score_wanted(current_ver, rank, t1, playerct, players, uid)))
                void (yield* outentry(rank, t1, 0));
        }
    } else {
        void cptr.sprintf(cptr.decay(pbuf), __sl154, current_ver ? __sl155 : __sl14);
        if (playerct < 1) {
            void cptr.strcat(cptr.decay(pbuf), __sl156);
        } else {
            if (playerct > 1)
                void cptr.strcat(cptr.decay(pbuf), __sl157);
            for (i = 0; i < playerct; i++) {
                if (!cptr.strncmp(cptr.ldPtro(players, i, 8), __sl158, 2n)) {
                    if (!cptr.ld1so(cptr.ldPtro(players, i, 8), 2))
                        continue;
                    cptr.stPtro(players, i, cptr.add(cptr.ldPtro(players, i, 8), 2), 8);
                }
                if (BigInt.asUintN(64, BigInt.asUintN(64, cptr.strlen(cptr.decay(pbuf)) + cptr.strlen(cptr.ldPtro(players, i, 8))) + 2n) >= 256n) {
                    if (cptr.strlen(cptr.decay(pbuf)) < 252n)
                        void cptr.strcat(cptr.decay(pbuf), __sl159);
                    else
                        void cptr.strcpy(cptr.add(cptr.add(cptr.decay(pbuf), cptr.strlen(cptr.decay(pbuf))), -(4)), __sl159);
                    break;
                }
                void cptr.strcat(cptr.decay(pbuf), cptr.ldPtro(players, i, 8));
                if (i < ((playerct - 1) | 0)) {
                    if (cptr.ld1so(cptr.ldPtro(players, i, 8), 0) == 45 && cptr.strchr(__sl160, cptr.ld1so(cptr.ldPtro(players, i, 8), 1)) && cptr.ld1so(cptr.ldPtro(players, i, 8), 2) == 0)
                        void cptr.strcat(cptr.decay(pbuf), __sl63);
                    else
                        void cptr.strcat(cptr.decay(pbuf), __sl161);
                }
            }
        }
        if (cptr.strlen(cptr.decay(pbuf)) < 255n)
            void cptr.strcat(cptr.decay(pbuf), __sl137);
        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_raw_print))(cptr.decay(pbuf))));
        (yield* raw_printf(__sl162, cptr.ldPtr(gh)));
        (yield* raw_printf(__sl163));
    }
    free_ttlist(tt_head);
}

/** C ref: topten.c:1356 — @param {CPtr} plch @returns {CInt} */
function* classmon(plch) {
    let i;
    for (i = 0; cptr.ldPtro(roles, i, 312); i++) {
        if (!cptr.strncmp(plch, cptr.ldPtro2(roles, i, 312, FLD.Role_filecode), 3n)) {
            if (cptr.ldI16o2(roles, i, 312, FLD.Role_mnum) != NHC.NON_PM)
                return cptr.ldI16o2(roles, i, 312, FLD.Role_mnum);
            else
                return NHC.PM_HUMAN;
        }
    }
    if (!strcmp(plch, __sl164))
        return NHC.PM_RANGER;
    (yield* impossible(__sl165, plch));
    return NHC.PM_HUMAN_MUMMY;
}

let __static_get_rnd_toptenentry_tt_buf = cptr.alloc(208); /** C ref: topten.c:1386 — struct toptenentry (function-static) */

/** C ref: topten.c:1381 @returns {CPtr} */
export function* get_rnd_toptenentry() {
    let rank;
    let i;
    let rfile;
    let tt;
    rfile = fopen_datafile(__sl92, __sl93, NHM.SCOREPREFIX);
    if (!rfile) {
        (yield* impossible(__sl94));
        return null;
    }
    tt = __static_get_rnd_toptenentry_tt_buf;
    rank = (rng_log_enabled() ? (rng_log_set_caller(__sl166, 1395, __sl167), rnd(cptr.ldI32o(sysopt, FLD.sysopt_s_tt_oname_maxrank))) : rnd(cptr.ldI32o(sysopt, FLD.sysopt_s_tt_oname_maxrank)));
    __lbl_pickentry: while (true) {
        for (i = rank; i; i--) {
            (yield* readentry(rfile, tt));
            if (cptr.ldI64o(tt, FLD.toptenentry_points) == 0n)
                break;
        }
        if (cptr.ldI64o(tt, FLD.toptenentry_points) == 0n) {
            if (rank > 1) {
                rank = 1;
                rewind(rfile);
                continue __lbl_pickentry;
            }
            tt = null;
        }
        void fclose(rfile);
        return tt;
        break __lbl_pickentry;
    }
}

/** C ref: topten.c:1422 — @param {CPtr} otmp @returns {CPtr} */
export function* tt_oname(otmp) {
    let tt;
    if (!otmp)
        return null;
    tt = (yield* get_rnd_toptenentry());
    if (!tt)
        return null;
    (yield* set_corpsenm(otmp, (yield* classmon(cptr.add(tt, FLD.toptenentry_plrole)))));
    if (cptr.ld1so2(tt, 0, 1, FLD.toptenentry_plgend) == 70)
        cptr.st1o(otmp, FLD.obj_spe, NHM.CORPSTAT_FEMALE);
    else if (cptr.ld1so2(tt, 0, 1, FLD.toptenentry_plgend) == 77)
        cptr.st1o(otmp, FLD.obj_spe, NHM.CORPSTAT_MALE);
    otmp = (yield* oname(otmp, cptr.add(tt, FLD.toptenentry_name), NHM.ONAME_NO_FLAGS));
    return otmp;
}

/** C ref: topten.c:1445 — @param {CPtr} mon @returns {CInt} */
export function* tt_doppel(mon) {
    let tt = (rng_log_enabled() ? (rng_log_set_caller(__sl166, 1446, __sl168), rn2(13)) : rn2(13)) ? (yield* get_rnd_toptenentry()) : null;
    let ret;
    if (!tt)
        ret = (((rng_log_enabled() ? (rng_log_set_caller(__sl166, 1450, __sl168), rn2(((((NHC.PM_WIZARD - NHC.PM_ARCHEOLOGIST) | 0) + 1) | 0))) : rn2(((((NHC.PM_WIZARD - NHC.PM_ARCHEOLOGIST) | 0) + 1) | 0))) + NHC.PM_ARCHEOLOGIST) | 0);
    else {
        if (cptr.ld1so2(tt, 0, 1, FLD.toptenentry_plgend) == 70)
            cptr.stI32o(mon, FLD.monst_female, 1);
        else if (cptr.ld1so2(tt, 0, 1, FLD.toptenentry_plgend) == 77)
            cptr.stI32o(mon, FLD.monst_female, 0);
        ret = (yield* classmon(cptr.add(tt, FLD.toptenentry_plrole)));
        if (canseemon(mon))
            (yield* christen_monst(mon, cptr.add(tt, FLD.toptenentry_name)));
    }
    return ret;
}

// --- BEGIN c2js reset block (tools/c2js/resetify.mjs) — do not edit ---
// 11 bindings: 2 rebound+refilled, 1 rebound, 8 refilled.
// S/P are supplied by js/generated-y/__reset.js so this module needs no new import.
let __c2js_rs = null;
export function __captureState(S) { __c2js_rs = [S(tt_head), S(zerott), S(__static_formatkiller_killed_by_prefix), S(__static_readentry_fmt), S(__static_readentry_fmt32), S(__static_readentry_fmt33), S(__static_writeentry_fmt32), S(__static_writeentry_fmt33), S(__static_writeentry_fmt0), S(__static_writeentry_fmtX), S(__static_get_rnd_toptenentry_tt_buf)]; }
export function __resetState(P) {
    const r = __c2js_rs;
    if (r === null) throw new Error("topten.js: __resetState before __captureState");
    tt_head = P(r[0]);
    zerott = P(r[1]);
    P(r[2]);
    P(r[3]);
    P(r[4]);
    P(r[5]);
    P(r[6]);
    P(r[7]);
    P(r[8]);
    P(r[9]);
    __static_get_rnd_toptenentry_tt_buf = P(r[10]);
}
// --- END c2js reset block ---
