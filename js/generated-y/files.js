// YIELDABLE BUILD — generated by tools/c2js/yieldify.mjs from js/generated/files.js
// Do not edit by hand, and do not score from this directory: the synchronous
// build in js/generated/ is the one the judge runs. Coloured functions here
// are generators; `yield*` marks every call that can reach a keystroke read.
import * as Y from '../yield-rt.js';
// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/src/files.c
// Input sha256: 5295af5df75ed1f67c9898d9d56a2cad6a35da68e511b5e5ab35886db6f9f2a5
// Transpiler: tools/c2js c2js emit v1+batch

import { i16, schar, u32mod } from '../cmachine.js';
import * as cptr from '../cptr.js';
import * as NHC from './nhconst.js';
import * as NHM from './nhmacro.js';
import { You_feel, impossible, pline, raw_printf } from './pline.js';
import { alloc } from './alloc.js';
import { bclose, close_check } from './sfstruct.js';
import { c_eos, copynchars, eos, fuzzymatch, mungspaces, nh_deterministic_qsort, nh_snprintf, s_suffix, strip_newline, strncmpi } from './hacklib.js';
import { WIN_MESSAGE, flags, gb, gc, gd, gh, gi, gl, gn, gs, gu, gw, hands_obj, iflags, program_state, svc, svd, svl, svm, svn, svp, u, ubirthday } from './decl.js';
import { sethanguphandler } from './unixmain.js';
import { In_quest, Is_special, maxledgerno } from './dungeon.js';
import { sysopt } from './sys.js';
import { regularize } from './unixunix.js';
import { sfo_char } from './sfbase.js';
import { validate, version_string } from './version.js';
import { get_plname_from_file } from './restore.js';
import { done1, nh_terminate } from './end.js';
import { windowprocs } from './windows.js';
import { more } from './topl.js';
import { nh_getenv } from './options.js';
import { observe_object } from './o_init.js';
import { inv_cnt } from './hack.js';
import { addinv, merge_choice } from './invent.js';
import { add_to_migration } from './mkobj.js';
import { readobjnam } from './objnam.js';
import { wish_history_add } from './zap.js';
import { config_error_add, config_error_done, config_error_init, get_configfile, get_default_configfile, parse_conf_file, set_configfile_name } from './cfgfiles.js';
import { clear_symsetentry, proc_symset_line } from './symbols.js';
import { getnow, hhmmss, yyyymmdd } from './calendar.js';
import { freedynamicdata } from './save.js';
import { l_nhcore_done } from './nhlua.js';
import { after_opt_showpaths } from './earlyarg.js';
import { pmatch } from './strutil.js';
import { rn2, rng_log_enabled, rng_log_set_caller } from './rnd.js';
import { aligns, genders } from './role.js';
import { timet_to_seconds } from './allmain.js';

// string literals (C char* uses decay to CPtr into these static buffers)
const __sl0 = cptr.lit("%c%02X");
const __sl1 = cptr.lit("Warning - Unclosed structlevel file being reinitialized");
const __sl2 = cptr.lit("Warning - Unclosed fieldlevel file being reinitialized");
const __sl3 = cptr.lit("# closing\n");
const __sl4 = cptr.lit("# closing, not viable\n");
const __sl5 = cptr.lit(".%d");
const __sl6 = cptr.lit("Cannot create file \"%s\" for level %d (errno %d).");
const __sl7 = cptr.lit("Cannot open file \"%s\" for level %d (errno %d).");
const __sl8 = cptr.lit("bon");
const __sl9 = cptr.lit("%u");
const __sl10 = cptr.lit("%c%s");
const __sl11 = cptr.lit("0");
const __sl12 = cptr.lit(".%c");
const __sl13 = cptr.lit(".bn");
const __sl14 = cptr.lit("Cannot create bones \"%s\", id %s (errno %d).");
const __sl15 = cptr.lit("couldn't rename %s to %s.");
const __sl16 = cptr.lit("save/%d%s");
const __sl17 = cptr.lit("");
const __sl18 = cptr.lit("savefile_name");
const __sl19 = cptr.lit(".e");
const __sl20 = cptr.lit(".Z");
const __sl21 = cptr.lit("r");
const __sl22 = cptr.lit("save");
const __sl23 = cptr.lit("%d%63s");
const __sl24 = cptr.lit("freopen of %s for %scompress failed; (%d) %s\n");
const __sl25 = cptr.lit("un");
const __sl26 = cptr.lit("/usr/bin/compress");
const __sl27 = cptr.lit("-d");
const __sl28 = cptr.lit("w+");
const __sl29 = cptr.lit("Exec to %scompress %s failed.\n");
const __sl30 = cptr.lit("Fork to %scompress %s failed.");
const __sl31 = cptr.lit("(%d)");
const __sl32 = cptr.lit("Wait when %scompressing %s failed; %s.");
const __sl33 = cptr.lit("Unable to uncompress %s");
const __sl34 = cptr.lit("everything matches");
const __sl35 = cptr.lit("outdated savefile");
const __sl36 = cptr.lit("savefile critical byte-count mismatch");
const __sl37 = cptr.lit("Windows x64 savefile on x86");
const __sl38 = cptr.lit("Unix 64 savefile on x86");
const __sl39 = cptr.lit("x86 savefile on Unix 64");
const __sl40 = cptr.lit("x86 savefile on Windows x64");
const __sl41 = cptr.lit("Unix 64 savefile on Windows x64");
const __sl42 = cptr.lit("Windows x64 savefile on Unix 64");
const __sl43 = cptr.lit("generic savefile mismatch");
const __sl44 = cptr.lit("\n%s is %s %s\n");
const __sl45 = cptr.lit("an");
const __sl46 = cptr.lit("a");
const __sl47 = cptr.lit("NETHACKDIR");
const __sl48 = cptr.lit("HACKDIR");
const __sl49 = cptr.lit("/usr/games/lib/nethackdir");
const __sl50 = cptr.lit("make_converted_name");
const __sl51 = cptr.lit("%s%s%s");
const __sl52 = cptr.lit("/");
const __sl53 = cptr.lit(".exportascii");
const __sl54 = cptr.lit("TRIED TO NEST LOCKS");
const __sl55 = cptr.lit("Cannot open file %s.  Is NetHack installed correctly?");
const __sl56 = cptr.lit("Waiting for release of fcntl lock on %s.  (%d retries left.)");
const __sl57 = cptr.lit("I give up.  Sorry.");
const __sl58 = cptr.lit("Some other process has an unnatural grip on %s.");
const __sl59 = cptr.lit("Can't remove fcntl lock on %s.");
const __sl60 = cptr.lit("WIZKIT");
const __sl61 = cptr.lit("Access to %s denied (%d).");
const __sl62 = cptr.lit("Couldn't open requested wizkit file %s (%d).");
const __sl63 = cptr.lit("HOME");
const __sl64 = cptr.lit("%s/%s");
const __sl65 = cptr.lit("Couldn't open default gw.wizkit file %s (%d).");
const __sl66 = cptr.lit("Bad wizkit item: \"%.60s\"");
const __sl67 = cptr.lit("symbols");
const __sl68 = cptr.lit("Default symbols");
const __sl69 = cptr.lit(" -_");
const __sl70 = cptr.lit("default");
const __sl71 = cptr.lit("Missing finish for symset \"%s\"");
const __sl72 = cptr.lit("unknown");
const __sl73 = cptr.lit("record");
const __sl74 = cptr.lit("Warning: cannot write scoreboard file '%s'");
const __sl75 = cptr.lit("paniclog");
const __sl76 = cptr.lit("%s %08ld %06ld %d %c: %s %s\n");
const __sl77 = cptr.lit(".log");
const __sl78 = cptr.lit("%s\n%s\n");
const __sl79 = cptr.lit("NetHack");
const __sl80 = cptr.lit("%s %s%s:");
const __sl81 = cptr.lit("system configuration file");
const __sl82 = cptr.lit("sysconf");
const __sl83 = cptr.lit("    \"%s\"");
const __sl84 = cptr.lit("NOTE: The %s above is missing or inaccessible!");
const __sl85 = cptr.lit("The loadable symbols file:");
const __sl86 = cptr.lit("    \"%s/%s\"");
const __sl87 = cptr.lit("Basic data files%s are in many separate files.");
const __sl88 = cptr.lit("not supported");
const __sl89 = cptr.lit("Your personal configuration file%s:");
const __sl90 = cptr.lit("Library/Preferences/NetHack Defaults");
const __sl91 = cptr.lit(".txt");
const __sl92 = cptr.lit("/Users/noahpeterson/Documents/Projects/teleport-contest-research/original-contest-to-fork/nethack-c/recorder/src/files.c");
const __sl93 = cptr.lit("choose_passage");
const __sl94 = cptr.lit("an incomprehensible foreign translation");
const __sl95 = cptr.lit("It's %s of \"%s\"!");
const __sl96 = cptr.lit("read_tribute %s, %s, %d.");
const __sl97 = cptr.lit("tribute");
const __sl98 = cptr.lit("too overwhelmed to continue!");
const __sl99 = cptr.lit("section ");
const __sl100 = cptr.lit("title ");
const __sl101 = cptr.lit("passage ");
const __sl102 = cptr.lit("e ");
const __sl103 = cptr.lit("tribute file error: bad %% command, line %d.");
const __sl104 = cptr.lit("[%s, by Terry Pratchett]");
const __sl105 = cptr.lit("; passage #%d]");
const __sl106 = cptr.lit("It seems to be %s of \"%s\"!");
const __sl107 = cptr.lit("Death");
const __sl108 = cptr.lit("Death Quotes");
const __sl109 = cptr.lit("livelog");
const __sl110 = cptr.lit("Cannot open live log file!");
const __sl111 = cptr.lit("lltype=%ld\tname=%s\trole=%s\trace=%s\tgender=%s\talign=%s\tturns=%ld\tstarttime=%ld\tcurtime=%ld\tmessage=%s\n");

const __static_nh_basename_basebuf = new Uint8Array(80); /** C ref: files.c:202 — char[80] (function-static) */

/** C ref: files.c:199 — @param {CPtr} fname @param {CInt} keep_suffix @returns {CPtr} */
export function nh_basename(fname, keep_suffix) {
    let p;
    if ((p = cptr.strrchr(fname, 47)) !== null)
        fname = cptr.add(p, 1);
    if ((p = cptr.strrchr(fname, 46)) !== null && !keep_suffix) {
        let ln = BigInt.asUintN(64, (cptr.diff(p, fname)));
        if (ln < 80n) {
            __builtin___strncpy_chk(cptr.decay(__static_nh_basename_basebuf), fname, ln, __builtin_object_size(cptr.decay(__static_nh_basename_basebuf), 1));
            cptr.st1o(cptr.decay(__static_nh_basename_basebuf), ln, 0, 1);
            fname = cptr.decay(__static_nh_basename_basebuf);
        }
    }
    return fname;
}

const __static_fname_encode_hexdigits = cptr.bytes("0123456789ABCDEF"); /** C ref: files.c:264 — char[17] (function-static) */

/** C ref: files.c:255 — @param {CPtr} legal @param {CInt} quotechar @param {CPtr} s @param {CPtr} callerbuf @param {CInt} bufsz @returns {CPtr} */
export function* fname_encode(legal, quotechar, s, callerbuf, bufsz) {
    let sp;
    let op;
    let cnt = 0;
    sp = s;
    op = callerbuf;
    cptr.st1(op, 0);
    while (cptr.ld1s(sp)) {
        if (((bufsz - cnt) | 0) <= 4)
            return callerbuf;
        if (cptr.ld1s(sp) == quotechar) {
            void cptr.sprintf(op, __sl0, quotechar, cptr.ld1s(sp));
            op = cptr.add(op, 3);
            cnt = (cnt + 3) | 0;
        } else if ((cptr.strchr(legal, cptr.ld1s(sp)) !== null) || (cptr.strchr(cptr.decay(__static_fname_encode_hexdigits), cptr.ld1s(sp)) !== null)) {
            cptr.st1(cptr.postinc(() => op, (v) => { op = v; }), cptr.ld1s(sp));
            cptr.st1(op, 0);
            cnt++;
        } else {
            void cptr.sprintf(op, __sl0, quotechar, cptr.ld1s(sp));
            op = cptr.add(op, 3);
            cnt = (cnt + 3) | 0;
        }
        sp = cptr.add(sp, 1);
    }
    return callerbuf;
}

const __static_fname_decode_hexdigits = cptr.bytes("0123456789ABCDEF"); /** C ref: files.c:309 — char[17] (function-static) */

/** C ref: files.c:305 — @param {CInt} quotechar @param {CPtr} s @param {CPtr} callerbuf @param {CInt} bufsz @returns {CPtr} */
export function* fname_decode(quotechar, s, callerbuf, bufsz) {
    let sp;
    let op;
    let k;
    let calc;
    let cnt = 0;
    sp = s;
    op = callerbuf;
    cptr.st1(op, 0);
    while (cptr.ld1s(sp)) {
        if (((bufsz - cnt) | 0) <= 2)
            return callerbuf;
        if (cptr.ld1s(sp) == quotechar) {
            sp = cptr.add(sp, 1);
            for (k = 0; k < 16; ++k)
                if (cptr.ld1s(sp) == cptr.ld1so(cptr.decay(__static_fname_decode_hexdigits), k, 1))
                    break;
            if (k >= 16)
                return callerbuf;
            calc = k << 4;
            sp = cptr.add(sp, 1);
            for (k = 0; k < 16; ++k)
                if (cptr.ld1s(sp) == cptr.ld1so(cptr.decay(__static_fname_decode_hexdigits), k, 1))
                    break;
            if (k >= 16)
                return callerbuf;
            calc = (calc + k) | 0;
            sp = cptr.add(sp, 1);
            cptr.st1(cptr.postinc(() => op, (v) => { op = v; }), schar(calc));
            cptr.st1(op, 0);
        } else {
            cptr.st1(cptr.postinc(() => op, (v) => { op = v; }), cptr.ld1s(cptr.postinc(() => sp, (v) => { sp = v; })));
            cptr.st1(op, 0);
        }
        cnt++;
    }
    return callerbuf;
}

/** C ref: files.c:354 — @param {CPtr} basenam @param {CInt} whichprefix @param {CInt} buffnum @returns {CPtr} */
export function fqname(basenam, whichprefix, buffnum) {
    return basenam;
}

/** C ref: files.c:394 — @param {CPtr} reasonbuf @returns {CInt} */
export function validate_prefix_locations(reasonbuf) {
    if (reasonbuf)
        cptr.st1o(reasonbuf, 0, 0);
    return 1;
}

/** C ref: files.c:444 — @param {CPtr} filename @param {CPtr} mode @param {CInt} prefix @returns {CPtr} */
export function fopen_datafile(filename, mode, prefix) {
    let fp;
    filename = fqname(filename, prefix, prefix == NHM.TROUBLEPREFIX ? 3 : 0);
    fp = fopen(filename, mode);
    return fp;
}

/** C ref: files.c:457 — int */
let bei = cptr.box(1);

/** C ref: files.c:461 — @param {CPtr} nhfp */
export function* init_nhfile(nhfp) {
    if (cptr.ld1so(nhfp, 32)) {
        if (cptr.ldI32(nhfp) != -1) {
            (yield* impossible(__sl1));
            void (yield* nhclose(cptr.ldI32(nhfp)));
        }
    } else if (cptr.ldPtro(nhfp, 40)) {
        if (cptr.ldPtro(nhfp, 40)) {
            (yield* impossible(__sl2));
            void fclose(cptr.ldPtro(nhfp, 40));
        }
    }
    cptr.stI32(nhfp, -1);
    cptr.stPtro(nhfp, 40, null);
    cptr.stI32o(nhfp, 4, NHM.COUNTING);
    cptr.st1o(nhfp, 32, 1);
    cptr.st1o(nhfp, 33, 0);
    cptr.st1o(nhfp, 34, 0);
    cptr.st1o(nhfp, 36, schar(((cptr.ld1s(bei)) == 0)));
    cptr.stPtro(nhfp, 56, null);
    cptr.stPtro(nhfp, 64, null);
    cptr.stI64o(nhfp, 16, cptr.stI64o(nhfp, 24, 0n));
    cptr.st1o(nhfp, 35, 0);
    cptr.stI32o(nhfp, 12, 0);
    cptr.st1o(nhfp, 72, 0);
    cptr.st1o(nhfp, 73, 1);
    cptr.stPtro(nhfp, 80, null);
}

/** C ref: files.c:496 @returns {CPtr} */
function* new_nhfile() {
    let nhfp = (yield* alloc(88));
    __builtin___memset_chk(nhfp, 0, 88n, __builtin_object_size(nhfp, 0));
    (yield* init_nhfile(nhfp));
    return nhfp;
}

/** C ref: files.c:509 — @param {CPtr} nhfp */
function* free_nhfile(nhfp) {
    if (nhfp) {
        (yield* init_nhfile(nhfp));
        cptr.free(nhfp);
    }
}

/** C ref: files.c:518 — @param {CPtr} nhfp */
export function* close_nhfile(nhfp) {
    if (cptr.ld1so(nhfp, 32) && cptr.ldI32(nhfp) != -1)
        void (yield* nhclose(cptr.ldI32(nhfp))), cptr.stI32(nhfp, -1);
    else if (cptr.ldPtro(nhfp, 40))
        void fclose(cptr.ldPtro(nhfp, 40)), cptr.stPtro(nhfp, 40, null);
    if (cptr.ldPtro(nhfp, 56))
        void fprintf(cptr.ldPtro(nhfp, 56), __sl3);
    if (cptr.ldPtro(nhfp, 56))
        void fclose(cptr.ldPtro(nhfp, 56));
    if (cptr.ldPtro(nhfp, 64))
        void fclose(cptr.ldPtro(nhfp, 64));
    (yield* free_nhfile(nhfp));
}

/** C ref: files.c:534 — @param {CPtr} nhfp */
export function rewind_nhfile(nhfp) {
    if (cptr.ld1so(nhfp, 32)) {
        void lseek(cptr.ldI32(nhfp), 0n, 0);
    } else {
        rewind(cptr.ldPtro(nhfp, 40));
    }
}

/** C ref: files.c:549 — @param {CPtr} nhfp @returns {CPtr} */
function* viable_nhfile(nhfp) {
    if (nhfp) {
        if (((cptr.ldI32(nhfp) == -1) && !cptr.ldPtro(nhfp, 40)) || (cptr.ld1so(nhfp, 32) && cptr.ldI32(nhfp) < 0) || (cptr.ld1so(nhfp, 33) && !cptr.ldPtro(nhfp, 40))) {
            if (cptr.ld1so(nhfp, 33)) {
                if (cptr.ldPtro(nhfp, 40)) {
                    void fclose(cptr.ldPtro(nhfp, 40));
                    cptr.stPtro(nhfp, 40, null);
                }
                if (cptr.ldPtro(nhfp, 56)) {
                    void fprintf(cptr.ldPtro(nhfp, 56), __sl4);
                    void fclose(cptr.ldPtro(nhfp, 56));
                }
                if (cptr.ldPtro(nhfp, 64))
                    void fclose(cptr.ldPtro(nhfp, 64));
            }
            (yield* free_nhfile(nhfp));
            nhfp = null;
        }
    }
    return nhfp;
}

/** C ref: files.c:583 — @param {CInt} fd @returns {CInt} */
export function* nhclose(fd) {
    let retval = 0;
    if (fd >= 0) {
        if (close_check(fd))
            (yield* bclose(fd));
        else
            retval = close(fd);
    }
    return retval;
}

/** C ref: files.c:606 — @param {CPtr} file @param {CInt} lev */
export function set_levelfile_name(file, lev) {
    let tf;
    tf = cptr.strrchr(file, 46);
    if (!tf)
        tf = eos(file);
    void cptr.sprintf(tf, __sl5, lev);
    return;
}

/** C ref: files.c:621 — @param {CInt} lev @param {CPtr} errbuf @returns {CPtr} */
export function* create_levelfile(lev, errbuf) {
    let fq_lock;
    let nhfp = null;
    if (errbuf)
        cptr.st1(errbuf, 0);
    set_levelfile_name(cptr.add(gl, 16), lev);
    fq_lock = fqname(cptr.add(gl, 16), NHM.LEVELPREFIX, 0);
    nhfp = (yield* new_nhfile());
    if (nhfp) {
        cptr.stI32o(nhfp, 8, NHM.NHF_LEVELFILE);
        cptr.stI32o(nhfp, 4, NHM.WRITING);
        cptr.st1o(nhfp, 32, 1);
        cptr.st1o(nhfp, 33, 0);
        cptr.st1o(nhfp, 34, 0);
        cptr.st1o(nhfp, 72, 0);
        cptr.st1o(nhfp, 73, 1);
        cptr.stI32o(nhfp, 12, NHC.historical);
        cptr.stI32(nhfp, -1);
        cptr.stPtro(nhfp, 40, null);
        cptr.stI32(nhfp, creat(fq_lock, NHM.FCMASK));
        if (cptr.ldI32(nhfp) >= 0)
            cptr.st1o2(svl, lev, 1, 89208, cptr.ld1uo2(svl, lev, 1, 89208) | NHM.LFILE_EXISTS);
        else if (errbuf)
            void cptr.sprintf(errbuf, __sl6, cptr.add(gl, 16), lev, (cptr.ldI32(__error())));
    }
    nhfp = (yield* viable_nhfile(nhfp));
    return nhfp;
}

/** C ref: files.c:673 — @param {CInt} lev @param {CPtr} errbuf @returns {CPtr} */
export function* open_levelfile(lev, errbuf) {
    let fq_lock;
    let nhfp = null;
    if (errbuf)
        cptr.st1(errbuf, 0);
    set_levelfile_name(cptr.add(gl, 16), lev);
    fq_lock = fqname(cptr.add(gl, 16), NHM.LEVELPREFIX, 0);
    nhfp = (yield* new_nhfile());
    if (nhfp) {
        cptr.stI32o(nhfp, 4, NHM.READING);
        cptr.st1o(nhfp, 32, 1);
        cptr.st1o(nhfp, 33, 0);
        cptr.st1o(nhfp, 34, 0);
        cptr.st1o(nhfp, 72, 0);
        cptr.st1o(nhfp, 73, 1);
        cptr.stI32o(nhfp, 8, NHM.NHF_LEVELFILE);
        cptr.stI32o(nhfp, 12, NHC.historical);
        cptr.stI32(nhfp, -1);
        cptr.stPtro(nhfp, 40, null);
    }
    if (nhfp && cptr.ld1so(nhfp, 32)) {
        cptr.stI32(nhfp, open(fq_lock, 0, 0));
        if (cptr.ldI32(nhfp) < 0 && errbuf)
            void cptr.sprintf(errbuf, __sl7, cptr.add(gl, 16), lev, (cptr.ldI32(__error())));
    }
    nhfp = (yield* viable_nhfile(nhfp));
    return nhfp;
}

/** C ref: files.c:719 — @param {CInt} lev */
export function delete_levelfile(lev) {
    if (lev == 0 || (cptr.ld1uo2(svl, lev, 1, 89208) & NHM.LFILE_EXISTS)) {
        set_levelfile_name(cptr.add(gl, 16), lev);
        void unlink(fqname(cptr.add(gl, 16), NHM.LEVELPREFIX, 0));
        cptr.st1o2(svl, lev, 1, 89208, cptr.ld1uo2(svl, lev, 1, 89208) & -5);
    }
}

/** C ref: files.c:733 */
export function clearlocks() {
    let x;
    if (cptr.ldI32o(program_state, 12))
        return;
    void signal(2, 1);
    sethanguphandler(1);
    for (x = (cptr.ldI32(svn) ? maxledgerno() : 0); x >= 0; x--)
        delete_levelfile(x);
}

/** C ref: files.c:755 — @param {CPtr} p @param {CPtr} q @returns {CInt} */
function strcmp_wrap(p, q) {
    return strcmp(cptr.ldPtr(p), cptr.ldPtr(q));
}

/** C ref: files.c:769 — @param {CPtr} file @param {CPtr} lev @returns {CPtr} */
function set_bonesfile_name(file, lev) {
    let sptr;
    let dptr;
    void cptr.strcpy(file, __sl8);
    if (cptr.ldI32o(sysopt, 92) > 1) {
        let poolnum = ((cptr.ldI32o(sysopt, 92) >>> 0) < 10 ? (cptr.ldI32o(sysopt, 92) >>> 0) : 10);
        poolnum = u32mod(Number(BigInt.asUintN(32, ubirthday.v)), poolnum);
        void cptr.sprintf(eos(file), __sl9, poolnum);
    }
    dptr = eos(file);
    void cptr.sprintf(dptr, __sl10, cptr.ld1so2(svd, cptr.ldI16(lev), 112, 69), In_quest(lev) ? cptr.ldPtro(gu, 192) : __sl11);
    if ((sptr = Is_special(lev)) !== null)
        void cptr.sprintf(eos(dptr), __sl12, cptr.ld1so(sptr, 27));
    else
        void cptr.sprintf(eos(dptr), __sl5, cptr.ldI16o(lev, 2));
    return dptr;
}

/** C ref: files.c:818 @returns {CPtr} */
function set_bonestemp_name() {
    let tf;
    tf = cptr.strrchr(cptr.add(gl, 16), 46);
    if (!tf)
        tf = eos(cptr.add(gl, 16));
    void cptr.sprintf(tf, __sl13);
    return cptr.add(gl, 16);
}

/** C ref: files.c:833 — @param {CPtr} lev @param {CPtr} bonesid @param {CPtr} errbuf @returns {CPtr} */
export function* create_bonesfile(lev, bonesid, errbuf) {
    let file;
    let nhfp = null;
    let failed = 0;
    if (errbuf)
        cptr.st1(errbuf, 0);
    cptr.stPtr(bonesid, set_bonesfile_name(cptr.add(gb, 4784), lev));
    file = set_bonestemp_name();
    file = fqname(file, NHM.BONESPREFIX, 0);
    nhfp = (yield* new_nhfile());
    if (nhfp) {
        cptr.stI32o(nhfp, 8, NHM.NHF_BONESFILE);
        cptr.stI32o(nhfp, 4, NHM.WRITING);
        cptr.st1o(nhfp, 32, 1);
        cptr.st1o(nhfp, 33, 0);
        cptr.st1o(nhfp, 34, 1);
        cptr.st1o(nhfp, 72, 1);
        cptr.st1o(nhfp, 73, 1);
        cptr.stI32o(nhfp, 12, NHC.historical);
        cptr.stI32(nhfp, -1);
        cptr.stPtro(nhfp, 40, null);
        if (cptr.ldPtro(nhfp, 40)) {
        } else {
            failed = (cptr.ldI32(__error()));
        }
        if (cptr.ld1so(nhfp, 32)) {
            cptr.stI32(nhfp, creat(file, NHM.FCMASK));
            if (cptr.ldI32(nhfp) < 0)
                failed = (cptr.ldI32(__error()));
        }
        if (failed && errbuf)
            void cptr.sprintf(errbuf, __sl14, cptr.add(gl, 16), cptr.ldPtr(bonesid), (cptr.ldI32(__error())));
    }
    nhfp = (yield* viable_nhfile(nhfp));
    return nhfp;
}

/** C ref: files.c:915 — @param {CPtr} lev */
export function* commit_bonesfile(lev) {
    let fq_bones;
    let tempname;
    let ret;
    void set_bonesfile_name(cptr.add(gb, 4784), lev);
    fq_bones = fqname(cptr.add(gb, 4784), NHM.BONESPREFIX, 0);
    tempname = set_bonestemp_name();
    tempname = fqname(tempname, NHM.BONESPREFIX, 1);
    ret = rename(tempname, fq_bones);
    if (cptr.ld1so(flags, 10) && ret != 0)
        (yield* pline(__sl15, tempname, fq_bones));
}

/** C ref: files.c:940 — @param {CPtr} lev @param {CPtr} bonesid @returns {CPtr} */
export function* open_bonesfile(lev, bonesid) {
    let fq_bones;
    let nhfp = null;
    cptr.stPtr(bonesid, set_bonesfile_name(cptr.add(gb, 4784), lev));
    fq_bones = fqname(cptr.add(gb, 4784), NHM.BONESPREFIX, 0);
    (yield* nh_uncompress(fq_bones));
    nhfp = (yield* new_nhfile());
    if (nhfp) {
        cptr.st1o(nhfp, 32, 1);
        cptr.st1o(nhfp, 33, 0);
        cptr.stI32o(nhfp, 8, NHM.NHF_BONESFILE);
        cptr.stI32o(nhfp, 4, NHM.READING);
        cptr.st1o(nhfp, 34, 1);
        cptr.st1o(nhfp, 72, 1);
        cptr.st1o(nhfp, 73, schar((cptr.ldI32o2(sysopt, 0, 4, 168) != NHC.exportascii)));
        cptr.stI32o(nhfp, 12, cptr.ldI32o2(sysopt, 0, 4, 168));
        cptr.stI32(nhfp, -1);
        cptr.stPtro(nhfp, 40, null);
        if (cptr.ldPtro(nhfp, 40)) {
        }
        if (cptr.ld1so(nhfp, 32)) {
            cptr.stI32(nhfp, open(fq_bones, 0, 0));
        }
    }
    nhfp = (yield* viable_nhfile(nhfp));
    return nhfp;
}

/** C ref: files.c:993 — @param {CPtr} lev @returns {CInt} */
export function* delete_bonesfile(lev) {
    let reslt;
    void set_bonesfile_name(cptr.add(gb, 4784), lev);
    reslt = unlink(fqname(cptr.add(gb, 4784), NHM.BONESPREFIX, 0));
    (yield* delete_convertedfile(fqname(cptr.add(gb, 4784), NHM.BONESPREFIX, 0)));
    return !(reslt < 0);
}

/** C ref: files.c:1006 */
export function* compress_bonesfile() {
    nh_sfconvert(fqname(cptr.add(gb, 4784), NHM.BONESPREFIX, 0));
    (yield* nh_compress(fqname(cptr.add(gb, 4784), NHM.BONESPREFIX, 0)));
}

/** C ref: files.c:1020 — @param {CInt} regularize_it */
export function set_savefile_name(regularize_it) {
    let regoffset = 0;
    let overflow = 0;
    let indicator_spot = 0;
    let postappend = null;
    let sfindicator = null;
    void cptr.sprintf(cptr.add(gs, 884), __sl16, getuid() | 0, svp);
    regoffset = 5;
    indicator_spot = 2;
    if (regularize_it)
        regularize(cptr.add(cptr.add(gs, 884), regoffset));
    if (indicator_spot == 1 && sfindicator && !overflow) {
        if (BigInt.asUintN(64, cptr.strlen(cptr.add(gs, 884)) + cptr.strlen(sfindicator)) < 53n)
            void cptr.strcat(cptr.add(gs, 884), sfindicator);
        else
            overflow = 2;
    }
    if (cptr.strlen(__sl17) > 0n && !overflow) {
        if (BigInt.asUintN(64, cptr.strlen(cptr.add(gs, 884)) + cptr.strlen(__sl17)) < 53n) {
            void cptr.strcat(cptr.add(gs, 884), __sl17);
        } else
            overflow = 3;
    }
    if (indicator_spot == 2 && sfindicator && !overflow) {
        if (BigInt.asUintN(64, cptr.strlen(cptr.add(gs, 884)) + cptr.strlen(sfindicator)) < 53n)
            void cptr.strcat(cptr.add(gs, 884), sfindicator);
        else
            overflow = 4;
    }
    if (postappend && !overflow) {
        if (BigInt.asUintN(64, cptr.strlen(cptr.add(gs, 884)) + cptr.strlen(postappend)) < 53n)
            void cptr.strcat(cptr.add(gs, 884), postappend);
        else
            overflow = 5;
    }
}

/** C ref: files.c:1128 — @param {CPtr} nhfp */
export function* save_savefile_name(nhfp) {
    (yield* sfo_char(nhfp, cptr.add(gs, 884), __sl18, 54));
}

/** C ref: files.c:1137 */
export function set_error_savefile() {
    void cptr.strcat(cptr.add(gs, 884), __sl19);
}

/** C ref: files.c:1159 @returns {CPtr} */
export function* create_savefile() {
    let fq_save;
    let nhfp = null;
    let do_historical = 1;
    fq_save = fqname(cptr.add(gs, 884), NHM.SAVEPREFIX, 0);
    nhfp = (yield* new_nhfile());
    if (nhfp) {
        cptr.stI32o(nhfp, 8, NHM.NHF_SAVEFILE);
        cptr.stI32o(nhfp, 4, NHM.WRITING);
        if (cptr.ldI32o(program_state, 56) || do_historical) {
            (void (do_historical));
            cptr.st1o(nhfp, 32, 1);
            cptr.st1o(nhfp, 33, 0);
            cptr.st1o(nhfp, 34, 0);
            cptr.st1o(nhfp, 72, 0);
            cptr.st1o(nhfp, 73, 1);
            cptr.stI32o(nhfp, 12, NHC.historical);
            cptr.stI32(nhfp, -1);
            cptr.stPtro(nhfp, 40, null);
            cptr.stI32(nhfp, creat(fq_save, NHM.FCMASK));
        }
    }
    nhfp = (yield* viable_nhfile(nhfp));
    return nhfp;
}

/** C ref: files.c:1217 @returns {CPtr} */
export function* open_savefile() {
    let fq_save;
    let nhfp = null;
    let do_historical = 1;
    fq_save = fqname(cptr.add(gs, 884), NHM.SAVEPREFIX, 0);
    nhfp = (yield* new_nhfile());
    if (nhfp) {
        cptr.stI32o(nhfp, 8, NHM.NHF_SAVEFILE);
        cptr.stI32o(nhfp, 4, NHM.READING);
        if (cptr.ldI32o(program_state, 56) || do_historical) {
            do_historical = 1;
            (void (do_historical));
            cptr.st1o(nhfp, 32, 1);
            cptr.st1o(nhfp, 33, 0);
            cptr.st1o(nhfp, 34, 0);
            cptr.st1o(nhfp, 72, 0);
            cptr.st1o(nhfp, 73, 1);
            cptr.stI32o(nhfp, 12, NHC.historical);
            cptr.stI32(nhfp, -1);
            cptr.stPtro(nhfp, 40, null);
        }
        cptr.stI32(nhfp, open(fq_save, 0, 0));
    }
    nhfp = (yield* viable_nhfile(nhfp));
    return nhfp;
}

/** C ref: files.c:1259 @returns {CInt} */
export function* delete_savefile() {
    let sfname = fqname(cptr.add(gs, 884), NHM.SAVEPREFIX, 0);
    void unlink(sfname);
    void (yield* delete_convertedfile(sfname));
    return 0;
}

/** C ref: files.c:1270 @returns {CPtr} */
export function* restore_saved_game() {
    let fq_save;
    let nhfp = null;
    let sfstatus = 0;
    set_savefile_name(1);
    fq_save = fqname(cptr.add(gs, 884), NHM.SAVEPREFIX, 0);
    (yield* nh_uncompress(fq_save));
    if ((nhfp = (yield* open_savefile())) !== null) {
        if ((sfstatus = (yield* validate(nhfp, fq_save, 0))) != NHM.SF_UPTODATE) {
            (yield* close_nhfile(nhfp));
            nhfp = (yield* problematic_savefile(sfstatus, fq_save));
        }
    }
    return nhfp;
}

/** C ref: files.c:1299 @returns {CPtr} */
export function* get_freeing_nhfile() {
    let nhfp = null;
    nhfp = (yield* new_nhfile());
    if (nhfp) {
        cptr.stI32o(nhfp, 4, NHM.FREEING);
    }
    return nhfp;
}

/** C ref: files.c:1312 @returns {CInt} */
export function* check_panic_save() {
    let result = 0;
    let cf;
    let savef;
    set_error_savefile();
    savef = fqname(cptr.add(gs, 884), NHM.SAVEPREFIX, 0);
    let ln = Number(BigInt.asUintN(32, (BigInt.asUintN(64, cptr.strlen(savef) + cptr.strlen(__sl20)))));
    let cfn = (yield* alloc((ln + 1) >>> 0));
    void cptr.strcpy(cfn, savef);
    void cptr.strcat(cfn, __sl20);
    if (!cptr.eq((cf = fopen(cfn, __sl21)), (null))) {
        void fclose(cf);
        result = 1;
    }
    cptr.free(cfn);
    if (!result) {
        if (!cptr.eq((cf = fopen(savef, __sl21)), (null))) {
            void fclose(cf);
            result = 1;
        }
    }
    set_savefile_name(1);
    return result;
}

/** C ref: files.c:1357 — @param {CPtr} filename @param {CInt} without_wait_synch_per_file @returns {CPtr} */
export function* plname_from_file(filename, without_wait_synch_per_file) {
    let nhfp;
    let ln;
    let result = null;
    let sfstatus = 0;
    void cptr.strcpy(cptr.add(gs, 884), filename);
    {
        let sln = Number(BigInt.asIntN(32, cptr.strlen(cptr.add(gs, 884))));
        let xln = Number(BigInt.asIntN(32, cptr.strlen(__sl20)));
        if (sln > xln && !strcmp(cptr.add(cptr.add(gs, 884), (sln - xln) | 0, 1), __sl20))
            cptr.st1o2(gs, (sln - xln) | 0, 1, 884, 0);
    }
    (yield* nh_uncompress(cptr.add(gs, 884)));
    if ((nhfp = (yield* open_savefile())) !== null) {
        if ((sfstatus = (yield* validate(nhfp, filename, without_wait_synch_per_file))) == NHM.SF_UPTODATE) {
            ln = 49;
            result = __builtin___memset_chk((yield* alloc(ln)), 0, BigInt(ln >>> 0), __builtin_object_size((yield* alloc(ln)), 0));
            (yield* get_plname_from_file(nhfp, result, 0));
        }
        (yield* close_nhfile(nhfp));
    }
    (yield* nh_compress(cptr.add(gs, 884)));
    return result;
}

/** C ref: files.c:1399 @returns {CPtr} */
export function* get_saved_games() {
    let result = null;
    let n;
    let j = 0;
    let myuid = getuid() | 0;
    let dir;
    if ((dir = opendir(fqname(__sl22, NHM.SAVEPREFIX, 0)))) {
        for (n = 0; readdir(dir); n++)
            ;
        closedir(dir);
        if (n > 0) {
            let i;
            if (!(dir = opendir(fqname(__sl22, NHM.SAVEPREFIX, 0))))
                return null;
            result = (yield* alloc(Number(BigInt.asUintN(32, BigInt.asUintN(64, BigInt.asUintN(64, BigInt(((n + 1) | 0))) * 8n)))));
            void __builtin___memset_chk(result, 0, BigInt.asUintN(64, BigInt.asUintN(64, BigInt(((n + 1) | 0))) * 8n), __builtin_object_size(result, 0));
            for (i = 0, j = 0; i < n; i++) {
                let uid = cptr.box(0);
                let name = new Uint8Array(64);
                let entry = readdir(dir);
                if (!entry)
                    break;
                if (sscanf(cptr.add(entry, 21), __sl23, uid, cptr.decay(name)) == 2) {
                    if (uid.v == myuid) {
                        let filename = new Uint8Array(256);
                        let r;
                        void cptr.sprintf(cptr.decay(filename), __sl16, uid.v, cptr.decay(name));
                        r = (yield* plname_from_file(cptr.decay(filename), 0));
                        if (r)
                            cptr.stPtro(result, j++, r, 8);
                    }
                }
            }
            closedir(dir);
        }
    }
    if (j > 0) {
        if (j > 1)
            (yield* nh_deterministic_qsort((result), BigInt.asUintN(64, BigInt((j))), 8n, (strcmp_wrap)));
        cptr.stPtro(result, j, (null), 8);
    } else if (result) {
        free_saved_games(result);
        result = (null);
    }
    return result;
}

/** C ref: files.c:1543 — @param {CPtr} saved */
export function free_saved_games(saved) {
    if (saved) {
        let i;
        for (i = 0; cptr.ldPtro(saved, i, 8); ++i)
            cptr.free(cptr.ldPtro(saved, i, 8));
        cptr.free(saved);
    }
}

/** C ref: files.c:1563 — @param {CPtr} filename @param {CPtr} mode @param {CPtr} stream @param {CInt} uncomp */
function* redirect(filename, mode, stream, uncomp) {
    if (freopen(filename, mode, stream) === null) {
        let details;
        if ((details = strerror((cptr.ldI32(__error())))) === null)
            details = __sl17;
        void fprintf(__stderrp, __sl24, filename, uncomp ? __sl25 : __sl17, (cptr.ldI32(__error())), details);
        (yield* nh_terminate(1));
    }
}

/** C ref: files.c:1591 — @param {CPtr} filename @param {CInt} uncomp */
function* docompress_file(filename, uncomp) {
    let cfn = null;
    let xtra;
    let cf;
    let args = cptr.alloc(10 * 8);
    let i = 0;
    let f;
    let childstatus = cptr.box(0);
    let ln;
    let istty = schar((cptr.ldI32o(windowprocs, 8) == NHC.wp_tty));
    xtra = __sl20;
    ln = Number(BigInt.asUintN(32, (BigInt.asUintN(64, cptr.strlen(filename) + cptr.strlen(xtra)))));
    cfn = (yield* alloc((ln + 1) >>> 0));
    void cptr.strcpy(cfn, filename);
    void cptr.strcat(cfn, xtra);
    if (uncomp) {
        if ((cf = fopen(cfn, __sl21)) === null) {
            cptr.free(cfn);
            return;
        }
        void fclose(cf);
    }
    cptr.stPtro(args, 0, __sl26, 8);
    if (uncomp)
        cptr.stPtro(args, ++i, __sl27, 8);
    cptr.stPtro(args, ++i, null, 8);
    if (istty) {
        (yield* Y.icall((cptr.ldPtro(windowprocs, 208))()));
    }
    f = fork();
    if (f == 0) {
        if (istty) {
            (yield* Y.icall((cptr.ldPtro(windowprocs, 240))(__sl17)));
        }
        if (uncomp) {
            (yield* redirect(cfn, __sl21, __stdinp, uncomp));
            (yield* redirect(filename, __sl28, __stdoutp, uncomp));
        } else {
            (yield* redirect(filename, __sl21, __stdinp, uncomp));
            (yield* redirect(cfn, __sl28, __stdoutp, uncomp));
        }
        void setgid(getgid());
        void setuid(getuid());
        void execv(cptr.ldPtro(args, 0, 8), args);
        perror(null);
        void fprintf(__stderrp, __sl29, uncomp ? __sl25 : __sl17, filename);
        cptr.free(cfn);
        (yield* nh_terminate(1));
    } else if (f == -1) {
        perror(null);
        (yield* pline(__sl30, uncomp ? __sl25 : __sl17, filename));
        cptr.free(cfn);
        return;
    }
    childstatus.v = 1;
    void signal(2, 1);
    void signal(3, 1);
    cptr.stI32(__error(), 0);
    if (BigInt(wait(childstatus)) == -1n) {
        let numbuf = new Uint8Array(40);
        let details = strerror((cptr.ldI32(__error())));
        if (!details) {
            void cptr.sprintf(cptr.decay(numbuf), __sl31, (cptr.ldI32(__error())));
            details = cptr.decay(numbuf);
        }
        (yield* raw_printf(__sl32, uncomp ? __sl25 : __sl17, filename, details));
    }
    void signal(2, done1);
    if (cptr.ld1so(flags, 10))
        void signal(3, null);
    if (childstatus.v == 0) {
        if (uncomp)
            void unlink(cfn);
        else
            void unlink(filename);
    } else {
        if (uncomp) {
            (yield* raw_printf(__sl33, filename));
            void unlink(filename);
        } else {
            void unlink(cfn);
        }
        if (istty && cptr.ld1so(iflags, 81)) {
            (yield* Y.icall((cptr.ldPtro(windowprocs, 112))(WIN_MESSAGE.v)));
            (yield* more());
        }
    }
    cptr.free(cfn);
    return;
}

/** C ref: files.c:1787 — @param {CPtr} filename */
export function* nh_compress(filename) {
    (yield* docompress_file(filename, 0));
}

/** C ref: files.c:1796 — @param {CPtr} filename */
export function* nh_uncompress(filename) {
    (yield* docompress_file(filename, 1));
}

/** C ref: files.c:1997 — struct sfstatus_to_msg { sfstatus, msg } (memory model v0.5) */

/** C ref: files.c:2000 — struct sfstatus_to_msg[10] */
const sf2msg = cptr.alloc(10 * 16);
cptr.stI32o(sf2msg, 0, NHM.SF_UPTODATE);
cptr.stPtro(sf2msg, 8, __sl34);
cptr.stI32o(sf2msg, 16, NHM.SF_OUTDATED);
cptr.stPtro(sf2msg, 24, __sl35);
cptr.stI32o(sf2msg, 32, NHM.SF_CRITICAL_BYTE_COUNT_MISMATCH);
cptr.stPtro(sf2msg, 40, __sl36);
cptr.stI32o(sf2msg, 48, NHM.SF_DM_IL32LLP64_ON_ILP32LL64);
cptr.stPtro(sf2msg, 56, __sl37);
cptr.stI32o(sf2msg, 64, NHM.SF_DM_I32LP64_ON_ILP32LL64);
cptr.stPtro(sf2msg, 72, __sl38);
cptr.stI32o(sf2msg, 80, NHM.SF_DM_ILP32LL64_ON_I32LP64);
cptr.stPtro(sf2msg, 88, __sl39);
cptr.stI32o(sf2msg, 96, NHM.SF_DM_ILP32LL64_ON_IL32LLP64);
cptr.stPtro(sf2msg, 104, __sl40);
cptr.stI32o(sf2msg, 112, NHM.SF_DM_I32LP64_ON_IL32LLP64);
cptr.stPtro(sf2msg, 120, __sl41);
cptr.stI32o(sf2msg, 128, NHM.SF_DM_IL32LLP64_ON_I32LP64);
cptr.stPtro(sf2msg, 136, __sl42);
cptr.stI32o(sf2msg, 144, NHM.SF_DM_MISMATCH);
cptr.stPtro(sf2msg, 152, __sl43);

/** C ref: files.c:2015 — @param {CInt} sfstatus @param {CPtr} savefilenm @returns {CPtr} */
function* problematic_savefile(sfstatus, savefilenm) {
    let i;
    let nhfp = null;
    switch (sfstatus) {
        case NHM.SF_UPTODATE:
        break;
        case NHM.SF_DM_IL32LLP64_ON_ILP32LL64:
        case NHM.SF_DM_I32LP64_ON_ILP32LL64:
        case NHM.SF_DM_ILP32LL64_ON_I32LP64:
        case NHM.SF_DM_ILP32LL64_ON_IL32LLP64:
        case NHM.SF_DM_I32LP64_ON_IL32LLP64:
        case NHM.SF_DM_IL32LLP64_ON_I32LP64:
        // @FallThrough
        ;
        case NHM.SF_DM_MISMATCH:
        case NHM.SF_OUTDATED:
        case NHM.SF_CRITICAL_BYTE_COUNT_MISMATCH:
        default:
        for (i = 0; i < 10; ++i) {
            if (cptr.ldI32o(sf2msg, i, 16) == sfstatus) {
                (yield* raw_printf(__sl44, savefilenm, (sfstatus == NHM.SF_OUTDATED) ? __sl45 : __sl46, cptr.ldPtro2(sf2msg, i, 16, 8)));
                break;
            }
        }
    }
    return nhfp;
}

/** C ref: files.c:2053 — signed char */
let cvtinit = 0;

/** C ref: files.c:2056 — char * */
let unconverted_filename = null;

/** C ref: files.c:2056 — char * */
let converted_filename = null;

/** C ref: files.c:2062 — @param {CPtr} filename @param {CInt} sfstatus @param {CInt} unconvert @returns {CInt} */
function doconvert_file(filename, sfstatus, unconvert) {
    (void (filename));
    (void (sfstatus));
    (void (unconvert));
    return 1;
}

/** C ref: files.c:2072 — @param {CPtr} filename */
export function nh_sfconvert(filename) {
    void doconvert_file(filename, 0, 0);
}

/** C ref: files.c:2079 — @param {CPtr} filename */
export function nh_sfunconvert(filename) {
    void doconvert_file(filename, 0, 1);
}

/** C ref: files.c:2090 — @param {CPtr} filename @returns {CInt} */
function* make_converted_name(filename) {
    let ln;
    let xtra;
    let finaldirchar;
    let dir = null;
    let needsep = 0;
    if (!filename)
        return 0;
    if (unconverted_filename)
        cptr.free(unconverted_filename), unconverted_filename = null;
    if (converted_filename)
        cptr.free(converted_filename), converted_filename = null;
    ln = Number(BigInt.asUintN(32, cptr.strlen(filename)));
    if (!contains_directory(filename)) {
        dir = nh_getenv(__sl47);
        if (!dir)
            dir = nh_getenv(__sl48);
        if (!dir)
            dir = __sl49;
        if (dir) {
            finaldirchar = c_eos(dir);
            finaldirchar = cptr.add(finaldirchar, -1);
            if (!(cptr.ld1s(finaldirchar) == 47 || cptr.ld1s(finaldirchar) == 92 || cptr.ld1s(finaldirchar) == 58)) {
                needsep = 1;
                ln = (ln + 1) | 0;
            }
            ln = Number(BigInt.asUintN(32, BigInt(ln >>> 0) + cptr.strlen(dir)));
        }
    }
    unconverted_filename = (yield* alloc((ln + 1) >>> 0));
    nh_snprintf(__sl50, 2146, unconverted_filename, BigInt(((ln + 1) >>> 0) >>> 0), __sl51, dir ? dir : __sl17, (dir && needsep) ? __sl52 : __sl17, filename);
    xtra = __sl53;
    ln = (ln + Number(BigInt.asUintN(32, cptr.strlen(xtra)))) | 0;
    converted_filename = (yield* alloc((ln + 1) >>> 0));
    void cptr.strcpy(converted_filename, unconverted_filename);
    void cptr.strcat(converted_filename, xtra);
    return 1;
}

/** C ref: files.c:2157 — @param {CPtr} basefilename @returns {CInt} */
export function* delete_convertedfile(basefilename) {
    if (!converted_filename)
        (yield* make_converted_name(basefilename));
    if (converted_filename) {
        void unlink(converted_filename);
    }
    return 0;
}

/** C ref: files.c:2168 */
export function free_convert_filenames() {
    if (converted_filename)
        cptr.free(converted_filename), converted_filename = null;
    if (unconverted_filename)
        cptr.free(unconverted_filename), unconverted_filename = null;
    cvtinit = 0;
}

/** C ref: files.c:2179 — @param {CPtr} s @returns {CInt} */
export function contains_directory(s) {
    let i;
    let slen = Number(BigInt.asIntN(32, cptr.strlen(s)));
    let cp = s;
    for (i = 0; i < slen; ++i) {
        if (cptr.ld1s(cp) == 92 || cptr.ld1s(cp) == 47 || cptr.ld1s(cp) == 58)
            return 1;
        cp = cptr.add(cp, 1);
    }
    return 0;
}

/** C ref: files.c:2201 — int */
let lockfd = -1;

/** C ref: files.c:2204 — struct flock */
let sflock = cptr.alloc(24);

/** C ref: files.c:2255 — @param {CPtr} filename @param {CInt} whichprefix @param {CInt} retryct @returns {CInt} */
export function* lock_file(filename, whichprefix, retryct) {
    (cptr.stI32o(gn, 56, cptr.ldI32o(gn, 56) + 1)) - (1);
    if (cptr.ldI32o(gn, 56) > 1) {
        (yield* impossible(__sl54));
        return 1;
    }
    filename = fqname(filename, whichprefix, 0);
    lockfd = open(filename, 2);
    if (lockfd == -1) {
        if (!cptr.ldI32o(program_state, 8))
            (yield* raw_printf(__sl55, filename));
        (cptr.stI32o(gn, 56, cptr.ldI32o(gn, 56) + -1)) - (-1);
        return 0;
    }
    cptr.stI16o(sflock, 20, 3);
    cptr.stI16o(sflock, 22, 0);
    cptr.stI64(sflock, 0n);
    cptr.stI64o(sflock, 8, 0n);
    while (fcntl(lockfd, 8, sflock) == -1) {
        if (retryct--) {
            if (!cptr.ldI32o(program_state, 8))
                (yield* raw_printf(__sl56, filename, retryct));
            sleep(1);
        } else {
            if (!cptr.ldI32o(program_state, 8))
                (yield* Y.icall((cptr.ldPtro(windowprocs, 240))(__sl57)));
            if (!cptr.ldI32o(program_state, 8))
                (yield* raw_printf(__sl58, filename));
            (cptr.stI32o(gn, 56, cptr.ldI32o(gn, 56) + -1)) - (-1);
            return 0;
        }
    }
    return 1;
}

/** C ref: files.c:2416 — @param {CPtr} filename */
export function* unlock_file(filename) {
    if (cptr.ldI32o(gn, 56) == 1) {
        cptr.stI16o(sflock, 20, 2);
        if (lockfd >= 0) {
            if (fcntl(lockfd, 8, sflock) == -1)
                if (!cptr.ldI32o(program_state, 8))
                    (yield* raw_printf(__sl59, filename));
            void close(lockfd), lockfd = -1;
        }
    }
    (cptr.stI32o(gn, 56, cptr.ldI32o(gn, 56) + -1)) - (-1);
}

/** C ref: files.c:2465 @returns {CPtr} */
function* fopen_wizkit_file() {
    let fp;
    let tmp_wizkit = new Uint8Array(256);
    let envp;
    envp = nh_getenv(__sl60);
    if (envp && cptr.ld1s(envp))
        void __builtin___strncpy_chk(cptr.add(gw, 23), envp, 127n, __builtin_object_size(cptr.add(gw, 23), 1));
    if (!cptr.ld1so2(gw, 0, 1, 23))
        return null;
    if (access(cptr.add(gw, 23), 4) == -1) {
        (yield* raw_printf(__sl61, cptr.add(gw, 23), (cptr.ldI32(__error()))));
        (yield* Y.icall((cptr.ldPtro(windowprocs, 216))()));
    } else if ((fp = fopen(cptr.add(gw, 23), __sl21)) !== null) {
        return fp;
    } else {
        (yield* raw_printf(__sl62, cptr.add(gw, 23), (cptr.ldI32(__error()))));
        (yield* Y.icall((cptr.ldPtro(windowprocs, 216))()));
    }
    envp = nh_getenv(__sl63);
    if (envp)
        void cptr.sprintf(cptr.decay(tmp_wizkit), __sl64, envp, cptr.add(gw, 23));
    else
        void cptr.strcpy(cptr.decay(tmp_wizkit), cptr.add(gw, 23));
    if ((fp = fopen(cptr.decay(tmp_wizkit), __sl21)) !== null)
        return fp;
    else if ((cptr.ldI32(__error())) != 2) {
        (yield* raw_printf(__sl65, cptr.decay(tmp_wizkit), (cptr.ldI32(__error()))));
        (yield* Y.icall((cptr.ldPtro(windowprocs, 216))()));
    }
    return null;
}

/** C ref: files.c:2537 — @param {CPtr} obj */
function* wizkit_addinv(obj) {
    if (!obj || cptr.eq(obj, hands_obj))
        return;
    (yield* observe_object(obj));
    if ((cptr.ldI16o(gu, 216) == NHC.PM_CLERIC))
        cptr.stI32o(obj, 88, 1);
    if (cptr.ld1so(obj, 49) != NHC.COIN_CLASS && inv_cnt(0) >= NHC.invlet_basic && !(yield* merge_choice(cptr.ldPtro(gi, 8), obj))) {
        (yield* add_to_migration(obj));
        cptr.stI16o(obj, 28, 0);
        cptr.stI16o(obj, 30, 1);
        cptr.stI64o(obj, 192, 3081n);
    } else {
        void (yield* addinv(obj));
    }
}

/** C ref: files.c:2562 — @param {CPtr} buf @returns {CInt} */
export function* proc_wizkit_line(buf) {
    let otmp;
    if (cptr.strlen(buf) >= 256n)
        cptr.st1o(buf, 255, 0);
    otmp = (yield* readobjnam(buf, null));
    if (otmp) {
        if (!cptr.eq(otmp, hands_obj)) {
            (yield* wish_history_add(buf));
            (yield* wizkit_addinv(otmp));
        }
    } else {
        (yield* config_error_add(__sl66, buf));
        return 0;
    }
    return 1;
}

/** C ref: files.c:2584 */
export function* read_wizkit() {
    let fp;
    if (!cptr.ld1so(flags, 10) || !(fp = (yield* fopen_wizkit_file())))
        return;
    cptr.stI32o(program_state, 100, 1);
    (yield* config_error_init(1, __sl60, 0));
    (yield* parse_conf_file(fp, proc_wizkit_line));
    void fclose(fp);
    (yield* config_error_done());
    cptr.stI32o(program_state, 100, 0);
    return;
}

/** C ref: files.c:2611 @returns {CPtr} */
function fopen_sym_file() {
    let fp;
    fp = fopen_datafile(__sl67, __sl21, NHM.HACKPREFIX);
    return fp;
}

/** C ref: files.c:2631 — @param {CInt} which_set @returns {CInt} */
export function* read_sym_file(which_set) {
    let fp;
    cptr.stI32o2(gs, which_set, 48, 244, 0);
    if (!(fp = fopen_sym_file()))
        return 0;
    cptr.stI32o2(gs, which_set, 48, 244, 1);
    cptr.st1o(gc, 456, cptr.st1o(gc, 457, 0));
    cptr.stI32o(gs, 880, which_set);
    cptr.stI32o(gs, 876, 0);
    (yield* config_error_init(1, __sl67, 0));
    (yield* parse_conf_file(fp, proc_symset_line));
    void fclose(fp);
    if (!cptr.ld1so(gc, 456) && !cptr.ld1so(gc, 457)) {
        if (cptr.ldPtro2(gs, which_set, 48, 208) && ((yield* fuzzymatch(cptr.ldPtro2(gs, which_set, 48, 208), __sl68, __sl69, 1)) || !(yield* strncmpi((cptr.ldPtro2(gs, which_set, 48, 208)), (__sl70), -1))))
            clear_symsetentry(which_set, 1);
        (yield* config_error_done());
        if (cptr.ldPtro2(gs, which_set, 48, 208)) {
            cptr.stI32o2(gs, which_set, 48, 244, 0);
            return 0;
        }
        return 1;
    }
    if (!cptr.ld1so(gc, 457))
        (yield* config_error_add(__sl71, cptr.ldPtro2(gs, which_set, 48, 208) ? cptr.ldPtro2(gs, which_set, 48, 208) : __sl72));
    (yield* config_error_done());
    return 1;
}

/** C ref: files.c:2689 — @param {CPtr} dir */
export function* check_recordfile(dir) {
    let fq_record;
    let fd;
    fq_record = fqname(__sl73, NHM.SCOREPREFIX, 0);
    fd = open(fq_record, 2, 0);
    if (fd >= 0) {
        void (yield* nhclose(fd));
    } else if ((fd = open(fq_record, 514, NHM.FCMASK)) >= 0) {
        void (yield* nhclose(fd));
    } else {
        (yield* raw_printf(__sl74, fq_record));
        (yield* Y.icall((cptr.ldPtro(windowprocs, 216))()));
    }
}

/** C ref: files.c:2801 — @param {CPtr} type @param {CPtr} reason */
export function* paniclog(type, reason) {
    let lfile;
    if (!cptr.ldI32o(program_state, 96)) {
        cptr.stI32o(program_state, 96, 1);
        lfile = fopen_datafile(__sl75, __sl46, NHM.TROUBLEPREFIX);
        if (lfile) {
            let buf = new Uint8Array(256);
            let now = (yield* getnow());
            let uid = getuid() | 0;
            let playmode = schar((cptr.ld1so(flags, 10) ? 68 : (cptr.ld1so(flags, 12) ? 88 : 45)));
            void fprintf(lfile, __sl76, version_string(cptr.decay(buf), 256n), (yield* yyyymmdd(now)), (yield* hhmmss(now)), uid, playmode, type, reason);
            void fclose(lfile);
        }
        cptr.stI32o(program_state, 96, 0);
    }
    return;
}

/** C ref: files.c:2836 — @param {CPtr} filenm @param {CPtr} type @param {CPtr} reason */
export function testinglog(filenm, type, reason) {
    let lfile;
    let fnbuf = new Uint8Array(256);
    if (!filenm)
        return;
    void cptr.strcpy(cptr.decay(fnbuf), filenm);
    if (cptr.strchr(cptr.decay(fnbuf), 46) === null)
        void cptr.strcat(cptr.decay(fnbuf), __sl77);
    lfile = fopen_datafile(cptr.decay(fnbuf), __sl46, NHM.TROUBLEPREFIX);
    if (lfile) {
        void fprintf(lfile, __sl78, type, reason);
        void fclose(lfile);
    }
    return;
}

/** C ref: files.c:3090 — @param {CInt} code */
export function* do_deferred_showpaths(code) {
    cptr.st1o(gd, 146, 0);
    (yield* reveal_paths(code));
    (yield* freedynamicdata());
    ;
    (yield* l_nhcore_done());
    (yield* after_opt_showpaths(cptr.ldPtro(gd, 152)));
}

/** C ref: files.c:3126 — @param {CPtr} filename @param {CInt} wildcards @returns {CInt} */
export function* debugcore(filename, wildcards) {
    let debugfiles;
    let p;
    if (!cptr.ld1so(flags, 10))
        return 0;
    if (!filename || !cptr.ld1s(filename))
        return 0;
    debugfiles = cptr.ldPtro(sysopt, 56);
    if (!debugfiles || !cptr.ld1s(debugfiles))
        return 0;
    filename = nh_basename(filename, 1);
    if (wildcards && (yield* pmatch(debugfiles, filename)))
        return 1;
    if ((p = cptr.strstr(debugfiles, filename)) !== null) {
        let l = Number(BigInt.asIntN(32, cptr.strlen(filename)));
        if ((cptr.eq(p, debugfiles) || cptr.ld1so(p, -1) == 32 || cptr.ld1so(p, -1) == 47) && (cptr.ld1so(p, l) == 32 || cptr.ld1so(p, l) == 0))
            return 1;
    }
    return 0;
}

/** C ref: files.c:3175 — @param {CInt} code */
export function* reveal_paths(code) {
    let skip_sysopt = 0;
    let fqn;
    let nodumpreason;
    let buf = new Uint8Array(256);
    let filep;
    let gamename = (cptr.ldPtr(gh) && cptr.ld1s(cptr.ldPtr(gh))) ? cptr.ldPtr(gh) : __sl79;
    let endp;
    let envp;
    let cwdbuf = new Uint8Array(1024);
    cptr.st1o(cptr.decay(buf), 0, 0, 1);
    (yield* raw_printf(__sl80, (yield* s_suffix(gamename)), __sl81, cptr.decay(buf)));
    filep = __sl82;
    fqn = fqname(filep, NHM.SYSCONFPREFIX, 0);
    if (fqn) {
        set_configfile_name(fqn);
        filep = get_configfile();
    }
    (yield* raw_printf(__sl83, filep));
    if (code == 1) {
        (yield* raw_printf(__sl84, __sl81));
        skip_sysopt = 1;
    }
    cptr.st1o(cptr.decay(buf), 0, 0, 1);
    envp = getcwd(cptr.decay(cwdbuf), 1024n);
    if (envp) {
        (yield* Y.icall((cptr.ldPtro(windowprocs, 240))(__sl85)));
        (yield* raw_printf(__sl86, envp, __sl67));
    }
    cptr.st1o(cptr.decay(buf), 0, 0, 1);
    (yield* raw_printf(__sl87, cptr.decay(buf)));
    fqn = null;
    nodumpreason = __sl88;
    cptr.st1o(cptr.decay(buf), 0, 0, 1);
    (yield* raw_printf(__sl89, cptr.decay(buf)));
    cptr.st1o(cptr.decay(buf), 0, 0, 1);
    if ((envp = nh_getenv(__sl63)) !== null) {
        (yield* copynchars(cptr.decay(buf), envp, 254));
        void cptr.strcat(cptr.decay(buf), __sl52);
    }
    endp = eos(cptr.decay(buf));
    (yield* copynchars(endp, get_default_configfile(), Number(BigInt.asIntN(32, (BigInt.asUintN(64, 255n - cptr.strlen(cptr.decay(buf))))))));
    if (envp) {
        if (access(cptr.decay(buf), 4) == -1) {
            (yield* copynchars(endp, __sl90, Number(BigInt.asIntN(32, (BigInt.asUintN(64, 255n - cptr.strlen(cptr.decay(buf))))))));
            if (access(cptr.decay(buf), 4) == -1) {
                (yield* copynchars(eos(cptr.decay(buf)), __sl91, Number(BigInt.asIntN(32, (BigInt.asUintN(64, 255n - cptr.strlen(cptr.decay(buf))))))));
                if (access(cptr.decay(buf), 4) == -1) {
                    (yield* copynchars(endp, get_default_configfile(), Number(BigInt.asIntN(32, (BigInt.asUintN(64, 255n - cptr.strlen(cptr.decay(buf))))))));
                }
            }
        }
    }
    (yield* raw_printf(__sl83, cptr.decay(buf)));
    (yield* Y.icall((cptr.ldPtro(windowprocs, 240))(__sl17)));
    (void (skip_sysopt));
    (void (nodumpreason));
}

/** C ref: files.c:3431 — @param {CInt} passagecnt @param {CUInt} oid @returns {CInt} */
function choose_passage(passagecnt, oid) {
    let idx;
    let res;
    if (passagecnt < 1)
        return 0;
    if (oid != cptr.ldI32o(svc, 632) || cptr.ldI32o(svc, 636) == 0) {
        let i;
        let range = passagecnt;
        let limit = Number(BigInt.asIntN(32, (30n / 1n)));
        cptr.stI32o(svc, 632, oid);
        if (range <= limit) {
            cptr.stI32o(svc, 636, passagecnt);
            for (idx = 0; idx < Number(BigInt.asIntN(32, (30n / 1n))); idx++)
                cptr.st1o2(svc, idx, 1, 640, schar(i16(((idx < passagecnt) ? (idx + 1) | 0 : 0))));
        } else {
            cptr.stI32o(svc, 636, Number(BigInt.asIntN(32, (30n / 1n))));
            for (idx = (i = 0); i < passagecnt; ++i, --range)
                if (range > 0 && (rng_log_enabled() ? (rng_log_set_caller(__sl92, 3456, __sl93), rn2(range)) : rn2(range)) < limit) {
                    cptr.st1o2(svc, idx++, 1, 640, schar(i16(((i + 1) | 0))));
                    --limit;
                }
        }
    }
    idx = (rng_log_enabled() ? (rng_log_set_caller(__sl92, 3463, __sl93), rn2(cptr.ldI32o(svc, 636))) : rn2(cptr.ldI32o(svc, 636)));
    res = cptr.ld1so2(svc, idx, 1, 640);
    cptr.st1o2(svc, idx, 1, 640, cptr.ld1so2(svc, cptr.stI32o(svc, 636, cptr.ldI32o(svc, 636) + -1), 1, 640));
    return res;
}

/** C ref: files.c:3474 — @param {CPtr} tribsection @param {CPtr} tribtitle @param {CInt} tribpassage @param {CPtr} nowin_buf @param {CInt} bufsz @param {CUInt} oid @returns {CInt} */
export function* read_tribute(tribsection, tribtitle, tribpassage, nowin_buf, bufsz, oid) {
    let fp;
    let line = new Uint8Array(256);
    let lastline = new Uint8Array(256);
    let scope = 0;
    let linect = 0;
    let passagecnt = 0;
    let targetpassage = 0;
    let badtranslation = __sl94;
    let matchedsection = 0;
    let matchedtitle = 0;
    let tribwin = -1;
    let grasped = 0;
    let foundpassage = 0;
    __lbl_cleanup: {
        if (nowin_buf)
            cptr.st1(nowin_buf, 0);
        if (!tribsection || !tribtitle) {
            if (!nowin_buf)
                (yield* pline(__sl95, badtranslation, tribtitle));
            return grasped;
        }
        do {
            if ((yield* debugcore(__sl92, 1))) {
                let save_plnmsg = cptr.ldI32o(iflags, 40);
                (yield* pline(__sl96, tribsection, tribtitle, tribpassage));
                cptr.stI32o(iflags, 40, save_plnmsg);
            }
        } while (0);
        fp = fopen(__sl97, __sl21);
        if (!fp) {
            if (!nowin_buf)
                (yield* You_feel(__sl98));
            return grasped;
        }
        cptr.st1(cptr.decay(line), cptr.st1(cptr.decay(lastline), 0));
        while (fgets(cptr.decay(line), 256, fp) !== null) {
            linect++;
            void strip_newline(cptr.decay(line));
            switch (cptr.ld1so(cptr.decay(line), 0, 1)) {
                case 37:
                if (!(yield* strncmpi(cptr.add(cptr.decay(line), 1, 1), __sl99, 8))) {
                    let st = cptr.add(cptr.decay(line), 9, 1);
                    scope = 1;
                    matchedsection = schar((!(yield* strncmpi((st), (tribsection), -1)) ? 1 : 0));
                } else if (!(yield* strncmpi(cptr.add(cptr.decay(line), 1, 1), __sl100, 6))) {
                    let st = cptr.add(cptr.decay(line), 7, 1);
                    let p1;
                    let p2;
                    if ((p1 = cptr.strchr(st, 40)) !== null) {
                        cptr.st1(cptr.postinc(() => p1, (v) => { p1 = v; }), 0);
                        void (yield* mungspaces(st));
                        if ((p2 = cptr.strchr(p1, 41)) !== null) {
                            cptr.st1(p2, 0);
                            passagecnt = atoi(p1);
                            scope = 2;
                            if (matchedsection && !(yield* strncmpi((st), (tribtitle), -1))) {
                                matchedtitle = 1;
                                targetpassage = !tribpassage ? choose_passage(passagecnt, oid) : ((tribpassage <= passagecnt) ? tribpassage : 0);
                            } else {
                                matchedtitle = 0;
                            }
                        }
                    }
                } else if (!(yield* strncmpi(cptr.add(cptr.decay(line), 1, 1), __sl101, 8))) {
                    let passagenum = 0;
                    let st = cptr.add(cptr.decay(line), 9, 1);
                    (yield* mungspaces(st));
                    passagenum = atoi(st);
                    if (passagenum > 0 && passagenum <= passagecnt) {
                        scope = 3;
                        if (matchedtitle && passagenum == targetpassage) {
                            foundpassage = 1;
                            if (!nowin_buf) {
                                tribwin = (yield* Y.icall((cptr.ldPtro(windowprocs, 104))(NHM.NHW_MENU)));
                                if (tribwin == -1)
                                    break __lbl_cleanup;
                            }
                        }
                    }
                } else if (!(yield* strncmpi(cptr.add(cptr.decay(line), 1, 1), __sl102, 2))) {
                    if (foundpassage)
                        break __lbl_cleanup;
                    if (scope == 2)
                        matchedtitle = 0;
                    if (scope == 1)
                        matchedsection = 0;
                    if (scope)
                        --scope;
                } else {
                    do {
                        if ((yield* debugcore(__sl92, 1))) {
                            let save_plnmsg = cptr.ldI32o(iflags, 40);
                            (yield* pline(__sl103, linect));
                            cptr.stI32o(iflags, 40, save_plnmsg);
                        }
                    } while (0);
                }
                break;
                case 35:
                break;
                default:
                if (foundpassage) {
                    if (!nowin_buf) {
                        (yield* Y.icall((cptr.ldPtro(windowprocs, 144))(tribwin, 0, cptr.decay(line))));
                        if (cptr.ld1s(cptr.decay(line)))
                            void cptr.strcpy(cptr.decay(lastline), cptr.decay(line));
                    } else {
                        (yield* copynchars(nowin_buf, cptr.decay(line), (bufsz - 1) | 0));
                        break __lbl_cleanup;
                    }
                }
            }
        }
    }
    void fclose(fp);
    if (nowin_buf) {
        grasped = schar((cptr.ld1s(nowin_buf) ? 1 : 0));
    } else {
        if (tribwin != -1) {
            if (cptr.ld1s(cptr.decay(lastline))) {
                let p;
                (yield* Y.icall((cptr.ldPtro(windowprocs, 120))(tribwin, 0)));
                if (cptr.strchr(cptr.decay(lastline), 91))
                    (yield* mungspaces(cptr.decay(lastline)));
                else
                    void cptr.sprintf(cptr.decay(lastline), __sl104, tribtitle);
                if ((p = cptr.strrchr(cptr.decay(lastline), 93)) !== null)
                    void cptr.sprintf(p, __sl105, targetpassage);
                (yield* Y.icall((cptr.ldPtro(windowprocs, 352))(cptr.decay(lastline), 0)));
                grasped = 1;
            }
            (yield* Y.icall((cptr.ldPtro(windowprocs, 128))(tribwin)));
        }
        if (!grasped)
            (yield* pline(__sl106, badtranslation, tribtitle));
    }
    return grasped;
}

/** C ref: files.c:3648 — @param {CPtr} buf @param {CInt} bufsz @returns {CInt} */
export function* Death_quote(buf, bufsz) {
    let death_oid = 1;
    return (yield* read_tribute(__sl107, __sl108, 0, buf, bufsz, death_oid));
}

/** C ref: files.c:3667 — @param {CLongLong} ll_type @param {CPtr} str */
export function* livelog_add(ll_type, str) {
    let livelogfile;
    let now;
    let gindx;
    let aindx;
    if (!(ll_type & cptr.ldI64o(sysopt, 96)))
        return;
    if ((yield* lock_file(__sl109, NHM.SCOREPREFIX, 10))) {
        if (!(livelogfile = fopen_datafile(__sl109, __sl46, NHM.SCOREPREFIX))) {
            (yield* pline(__sl110));
            (yield* unlock_file(__sl109));
            return;
        }
        now = (yield* getnow());
        gindx = cptr.ld1so(flags, 13) ? 1 : 0;
        aindx = (1 - cptr.ld1so(u, 2172)) | 0;
        void fprintf(livelogfile, __sl111, (ll_type & cptr.ldI64o(sysopt, 96)), svp, cptr.ldPtro(gu, 192), cptr.ldPtro(gu, 344), cptr.ldPtro2(genders, gindx, 48, 32), cptr.ldPtro2(aligns, aindx, 32, 16), cptr.ldI64o(svm, 8), timet_to_seconds(ubirthday.v), timet_to_seconds(now), str);
        void fclose(livelogfile);
        (yield* unlock_file(__sl109));
    }
}

// --- BEGIN c2js reset block (tools/c2js/resetify.mjs) — do not edit ---
// 10 bindings: 2 rebound+refilled, 4 rebound, 4 refilled.
// S/P are supplied by js/generated-y/__reset.js so this module needs no new import.
let __c2js_rs = null;
export function __captureState(S) { __c2js_rs = [S(__static_nh_basename_basebuf), S(__static_fname_encode_hexdigits), S(__static_fname_decode_hexdigits), S(bei), S(sf2msg), S(cvtinit), S(unconverted_filename), S(converted_filename), S(lockfd), S(sflock)]; }
export function __resetState(P) {
    const r = __c2js_rs;
    if (r === null) throw new Error("files.js: __resetState before __captureState");
    P(r[0]);
    P(r[1]);
    P(r[2]);
    bei = P(r[3]);
    P(r[4]);
    cvtinit = P(r[5]);
    unconverted_filename = P(r[6]);
    converted_filename = P(r[7]);
    lockfd = P(r[8]);
    sflock = P(r[9]);
}
// --- END c2js reset block ---
