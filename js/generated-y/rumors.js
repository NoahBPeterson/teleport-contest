// YIELDABLE BUILD — generated by tools/c2js/yieldify.mjs from js/generated/rumors.js
// Do not edit by hand, and do not score from this directory: the synchronous
// build in js/generated/ is the one the judge runs. Coloured functions here
// are generators; `yield*` marks every call that can reach a keystroke read.
import * as Y from '../yield-rt.js';
// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/src/rumors.c
// Input sha256: 0bc8665c1bcff5b2d9d9063ff02dfd081ddcb01ba0ba5c817f0dbf857d73c255
// Transpiler: tools/c2js c2js emit v1+batch

import { schar } from '../cmachine.js';
import * as cptr from '../cptr.js';
import * as NHC from './nhconst.js';
import * as NHM from './nhmacro.js';
import * as FLD from './nhfield.js';
import { eos, lowc, xcrypt } from './hacklib.js';
import { WIN_MESSAGE, disp, flags, gf, gi, gm, go, gt, iflags, program_state, svo, u, ynchars, ynqchars } from './decl.js';
import { rn2, rnd, rng_log_enabled, rng_log_set_caller } from './rnd.js';
import { There, You, impossible, nhassert_failed, pline, verbalize } from './pline.js';
import { exercise } from './attrib.js';
import { windowprocs } from './windows.js';
import { alloc, dupstr } from './alloc.js';
import { is_fainted } from './eat.js';
import { sfi_ulong, sfi_unsigned, sfo_ulong, sfo_unsigned } from './sfbase.js';
import { money_cnt } from './hack.js';
import { Monnam, bogon_codes, bogon_is_pname } from './do_name.js';
import { currency } from './invent.js';
import { yn_function } from './cmd.js';
import { money2mon } from './shk.js';
import { record_achievement } from './insight.js';
import { more_experienced, newexplevel } from './exper.js';
import { mons } from './monst.js';
import { the_unique_pm } from './objnam.js';
import { debugcore } from './files.js';

// string literals (C char* uses decay to CPtr into these static buffers)
const __sl0 = cptr.lit("rumors");
const __sl1 = cptr.lit("r");
const __sl2 = cptr.lit("Error reading \"%.80s\".");
const __sl3 = cptr.lit("/Users/noahpeterson/Documents/Projects/teleport-contest-research/original-contest-to-fork/nethack-c/recorder/src/rumors.c");
const __sl4 = cptr.lit("getrumor");
const __sl5 = cptr.lit("strange truth value for rumor");
const __sl6 = cptr.lit("Oops...");
const __sl7 = cptr.lit("Can't find non-cookie rumor?");
const __sl8 = cptr.lit("[cookie] ");
const __sl9 = cptr.lit("T start=%06ld (%06lx), end=%06ld (%06lx), size=%06ld (%06lx)");
const __sl10 = cptr.lit("F start=%06ld (%06lx), end=%06ld (%06lx), size=%06ld (%06lx)");
const __sl11 = cptr.lit("T %06ld %s");
const __sl12 = cptr.lit("  %6s %s");
const __sl13 = cptr.lit("");
const __sl14 = cptr.lit("F %06ld %s");
const __sl15 = cptr.lit("rumors not accessible.");
const __sl16 = cptr.lit("Engravings:");
const __sl17 = cptr.lit("engrave");
const __sl18 = cptr.lit("Epitaphs:");
const __sl19 = cptr.lit("epitaph");
const __sl20 = cptr.lit("Bogus monsters:");
const __sl21 = cptr.lit("bogusmon");
const __sl22 = cptr.lit("can't create temporary window");
const __sl23 = cptr.lit("error; can't read comment line");
const __sl24 = cptr.lit("malformed; first line is not a comment line:");
const __sl25 = cptr.lit("- first line, as is");
const __sl26 = cptr.lit("- xcrypt of first line");
const __sl27 = cptr.lit("can't read first non-comment line");
const __sl28 = cptr.lit("first non-comment line is empty");
const __sl29 = cptr.lit("(no second entry)");
const __sl30 = cptr.lit("(only two entries)");
const __sl31 = cptr.lit(" ...");
const __sl32 = cptr.lit("filechunksize <= INT_MAX");
const __sl33 = cptr.lit("What a pity that you cannot read it!");
const __sl34 = cptr.lit("NetHack rumors file closed for renovation.");
const __sl35 = cptr.lit("True to her word, the Oracle %ssays: ");
const __sl36 = cptr.lit("outrumor");
const __sl37 = cptr.lit("offhandedly ");
const __sl38 = cptr.lit("casually ");
const __sl39 = cptr.lit("nonchalantly ");
const __sl40 = cptr.lit("%s");
const __sl41 = cptr.lit("It reads:");
const __sl42 = cptr.lit("%5d\n");
const __sl43 = cptr.lit("%5lx\n");
const __sl44 = cptr.lit("oracle-oracle_cnt");
const __sl45 = cptr.lit("oracle-oracle_loc");
const __sl46 = cptr.lit("oracles");
const __sl47 = cptr.lit("outoracle");
const __sl48 = cptr.lit("The Oracle scornfully takes all your gold and says:");
const __sl49 = cptr.lit("The Oracle meditates for a moment and then intones:");
const __sl50 = cptr.lit("The message reads:");
const __sl51 = cptr.lit("---\n");
const __sl52 = cptr.lit("is no one here to consult.");
const __sl53 = cptr.lit("%s is in no mood for consultations.");
const __sl54 = cptr.lit("have no gold.");
const __sl55 = cptr.lit("\"Wilt thou settle for a minor consultation?\" (%d %s)");
const __sl56 = cptr.lit("don't even have enough gold for that!");
const __sl57 = cptr.lit("\"Then dost thou desire a major one?\" (%d %s)");
const __sl58 = cptr.lit("Can't open '%s' file.");
const __sl59 = cptr.lit("CapitalMon");
const __sl60 = cptr.lit("rumors.c");
const __sl61 = cptr.lit("CapMons != 0");
const __sl62 = cptr.lit("CapMons");
const __sl63 = cptr.lit("Capitalized monster type names normally preceded by \"the\":");
const __sl64 = cptr.lit("  %.77s");

/** C ref: rumors.c:56 — unsigned int */
let CapMonstCnt = 0;

/** C ref: rumors.c:56 — unsigned int */
let CapBogonCnt = 0;

/** C ref: rumors.c:57 — unsigned int */
let CapMonSiz = 0;

/** C ref: rumors.c:58 — char ** */
let CapMons = null;

/** C ref: rumors.c:67 — @param {CPtr} line */
function unpadline(line) {
    let p = eos(line);
    if (cptr.cmp(p, line) > 0 && cptr.ld1so(p, -1) == 10)
        p = cptr.add(p, -1);
    while (cptr.cmp(p, line) > 0 && cptr.ld1so(p, -1) == 95)
        p = cptr.add(p, -1);
    cptr.st1(p, 0);
}

const __static_init_rumors_rumors_header = cptr.bytes("%d,%ld,%lx;%d,%ld,%lx;0,0,%lx\n"); /** C ref: rumors.c:87 — char[31] (function-static) */

/** C ref: rumors.c:85 — @param {CPtr} fp */
function init_rumors(fp) {
    let true_count = cptr.box(0);
    let false_count = cptr.box(0);
    let eof_offset = cptr.box(0n);
    let line = new Uint8Array(256);
    void fgets(cptr.decay(line), 256, fp);
    void fgets(cptr.decay(line), 256, fp);
    if (sscanf(cptr.decay(line), cptr.decay(__static_init_rumors_rumors_header), true_count, cptr.add(gt, FLD.instance_globals_t_true_rumor_size), cptr.add(gt, FLD.instance_globals_t_true_rumor_start), false_count, cptr.add(gf, FLD.instance_globals_f_false_rumor_size), cptr.add(gf, FLD.instance_globals_f_false_rumor_start), eof_offset) == 7 && cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_size) > 0n && cptr.ldI64o(gf, FLD.instance_globals_f_false_rumor_size) > 0n) {
        cptr.stI64o(gt, FLD.instance_globals_t_true_rumor_end, BigInt.asIntN(64, BigInt.asIntN(64, cptr.ldU64o(gt, FLD.instance_globals_t_true_rumor_start)) + cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_size)));
        cptr.stI64o(gf, FLD.instance_globals_f_false_rumor_end, BigInt.asIntN(64, BigInt.asIntN(64, cptr.ldU64o(gf, FLD.instance_globals_f_false_rumor_start)) + cptr.ldI64o(gf, FLD.instance_globals_f_false_rumor_size)));
    } else {
        cptr.stI64o(gt, FLD.instance_globals_t_true_rumor_size, -1n);
        void fclose(fp);
    }
}

let __static_getrumor_cookie_marker = __sl8; /** C ref: rumors.c:125 — char * (function-static) */

/** C ref: rumors.c:117 — @param {CInt} truth @param {CPtr} rumor_buf @param {CInt} exclude_cookie @returns {CPtr} */
export function* getrumor(truth, rumor_buf, exclude_cookie) {
    let rumors;
    let beginning;
    let ending;
    let line = new Uint8Array(256);
    let marklen = Number(BigInt.asIntN(32, cptr.strlen(__static_getrumor_cookie_marker)));
    cptr.st1o(rumor_buf, 0, 0);
    if (cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_size) < 0n)
        return rumor_buf;
    rumors = fopen(__sl0, __sl1);
    if (rumors) {
        let count = 0;
        let adjtruth;
        do {
            cptr.st1o(rumor_buf, 0, 0);
            if (cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_size) == 0n) {
                init_rumors(rumors);
                if (cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_size) < 0n) {
                    void cptr.sprintf(rumor_buf, __sl2, __sl0);
                    return rumor_buf;
                }
            }
            switch (adjtruth = (truth + (rng_log_enabled() ? (rng_log_set_caller(__sl3, 151, __sl4), rn2(2)) : rn2(2))) | 0) {
                case 2:
                case 1:
                beginning = BigInt.asIntN(64, cptr.ldU64o(gt, FLD.instance_globals_t_true_rumor_start));
                ending = cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_end);
                break;
                case 0:
                case -1:
                beginning = BigInt.asIntN(64, cptr.ldU64o(gf, FLD.instance_globals_f_false_rumor_start));
                ending = cptr.ldI64o(gf, FLD.instance_globals_f_false_rumor_end);
                break;
                default:
                (yield* impossible(__sl5));
                return cptr.strcpy(rumor_buf, __sl6);
            }
            void cptr.strcpy(rumor_buf, (yield* get_rnd_line(rumors, cptr.decay(line), 256, rn2, beginning, ending, NHM.MD_PAD_RUMORS)));
        } while (count++ < 50 && exclude_cookie && !cptr.strncmp(rumor_buf, __static_getrumor_cookie_marker, BigInt.asUintN(64, BigInt(marklen))));
        void fclose(rumors);
        if (count >= 50)
            (yield* impossible(__sl7));
        else if (!cptr.ld1so(gi, FLD.instance_globals_i_in_mklev))
            (yield* exercise(NHC.A_WIS, schar((adjtruth > 0))));
    } else {
        (yield* couldnt_open_file(__sl0));
        cptr.stI64o(gt, FLD.instance_globals_t_true_rumor_size, -1n);
    }
    if (!exclude_cookie && !cptr.strncmp(rumor_buf, __static_getrumor_cookie_marker, BigInt.asUintN(64, BigInt(marklen)))) {
        let src = cptr.add(rumor_buf, marklen);
        let dst = rumor_buf;
        for (; cptr.ld1s(src) != 0; src = cptr.add(src, 1), dst = cptr.add(dst, 1)) {
            cptr.st1(dst, cptr.ld1s(src));
        }
        cptr.st1(dst, 0);
    }
    return rumor_buf;
}

/** C ref: rumors.c:196 */
export function* rumor_check() {
    let tmpwin = cptr.box(0);
    let __go_no_rumors = false;
    __skip_no_rumors: {
        let rumors;
        tmpwin.v = -1;
        let endp;
        let line = new Uint8Array(256);
        let xbuf = new Uint8Array(256);
        let rumor_buf = new Uint8Array(256);
        rumors = (cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_size) >= 0n) ? fopen(__sl0, __sl1) : null;
        if (rumors) {
            let ftell_rumor_start = 0n;
            cptr.st1o(cptr.decay(rumor_buf), 0, 0, 1);
            if (cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_size) == 0n) {
                init_rumors(rumors);
                if (cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_size) < 0n) {
                    rumors = null;
                    { __go_no_rumors = true; break __skip_no_rumors; }
                }
            }
            tmpwin.v = (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_create_nhwindow))(NHM.NHW_TEXT)));
            void cptr.sprintf(cptr.decay(rumor_buf), __sl9, BigInt.asIntN(64, cptr.ldU64o(gt, FLD.instance_globals_t_true_rumor_start)), cptr.ldU64o(gt, FLD.instance_globals_t_true_rumor_start), cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_end), BigInt.asUintN(64, cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_end)), cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_size), BigInt.asUintN(64, cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_size)));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin.v, 0, cptr.decay(rumor_buf))));
            void cptr.sprintf(cptr.decay(rumor_buf), __sl10, BigInt.asIntN(64, cptr.ldU64o(gf, FLD.instance_globals_f_false_rumor_start)), cptr.ldU64o(gf, FLD.instance_globals_f_false_rumor_start), cptr.ldI64o(gf, FLD.instance_globals_f_false_rumor_end), BigInt.asUintN(64, cptr.ldI64o(gf, FLD.instance_globals_f_false_rumor_end)), cptr.ldI64o(gf, FLD.instance_globals_f_false_rumor_size), BigInt.asUintN(64, cptr.ldI64o(gf, FLD.instance_globals_f_false_rumor_size)));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin.v, 0, cptr.decay(rumor_buf))));
            cptr.st1o(cptr.decay(rumor_buf), 0, 0, 1);
            void fseek(rumors, BigInt.asIntN(64, cptr.ldU64o(gt, FLD.instance_globals_t_true_rumor_start)), 0);
            ftell_rumor_start = ftell(rumors);
            void fgets(cptr.decay(line), 256, rumors);
            if ((endp = cptr.strchr(cptr.decay(line), 10)) !== null)
                cptr.st1(endp, 0);
            void cptr.sprintf(cptr.decay(rumor_buf), __sl11, ftell_rumor_start, (yield* xcrypt(cptr.decay(line), cptr.decay(xbuf))));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin.v, 0, cptr.decay(rumor_buf))));
            while (fgets(cptr.decay(line), 256, rumors) && ftell(rumors) < cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_end))
                continue;
            if ((endp = cptr.strchr(cptr.decay(line), 10)) !== null)
                cptr.st1(endp, 0);
            void cptr.sprintf(cptr.decay(rumor_buf), __sl12, __sl13, (yield* xcrypt(cptr.decay(line), cptr.decay(xbuf))));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin.v, 0, cptr.decay(rumor_buf))));
            cptr.st1o(cptr.decay(rumor_buf), 0, 0, 1);
            void fseek(rumors, BigInt.asIntN(64, cptr.ldU64o(gf, FLD.instance_globals_f_false_rumor_start)), 0);
            ftell_rumor_start = ftell(rumors);
            void fgets(cptr.decay(line), 256, rumors);
            if ((endp = cptr.strchr(cptr.decay(line), 10)) !== null)
                cptr.st1(endp, 0);
            void cptr.sprintf(cptr.decay(rumor_buf), __sl14, ftell_rumor_start, (yield* xcrypt(cptr.decay(line), cptr.decay(xbuf))));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin.v, 0, cptr.decay(rumor_buf))));
            while (fgets(cptr.decay(line), 256, rumors) && ftell(rumors) < cptr.ldI64o(gf, FLD.instance_globals_f_false_rumor_end))
                continue;
            if ((endp = cptr.strchr(cptr.decay(line), 10)) !== null)
                cptr.st1(endp, 0);
            void cptr.sprintf(cptr.decay(rumor_buf), __sl12, __sl13, (yield* xcrypt(cptr.decay(line), cptr.decay(xbuf))));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin.v, 0, cptr.decay(rumor_buf))));
            void fclose(rumors);
        } else if (cptr.ldI64o(gt, FLD.instance_globals_t_true_rumor_size) < 0n) {
            __go_no_rumors = true; break __skip_no_rumors;
        } else {
            (yield* couldnt_open_file(__sl0));
            cptr.stI64o(gt, FLD.instance_globals_t_true_rumor_size, -1n);
        }
    }
    if (__go_no_rumors) {
        (yield* pline(__sl15));
        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_display_nhwindow))(WIN_MESSAGE.v, 1)));
    }
    (yield* others_check(__sl16, __sl17, tmpwin));
    (yield* others_check(__sl18, __sl19, tmpwin));
    (yield* others_check(__sl20, __sl21, tmpwin));
    if (tmpwin.v != -1) {
        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_display_nhwindow))(tmpwin.v, 1)));
        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_destroy_nhwindow))(tmpwin.v)));
    }
}

const __static_others_check_errfmt = cptr.bytes("others_check(\"%s\"): %s"); /** C ref: rumors.c:313 — char[23] (function-static) */

/** C ref: rumors.c:308 — @param {CPtr} ftype @param {CPtr} fname @param {CPtr} winptr */
function* others_check(ftype, fname, winptr) {
    let fh;
    let line = new Uint8Array(256);
    let xbuf = new Uint8Array(256);
    let endp;
    let tmpwin = cptr.ldI32(winptr);
    let entrycount = 0;
    fh = fopen(fname, __sl1);
    if (fh) {
        __lbl_closeit: {
            if (tmpwin == -1) {
                cptr.stI32(winptr, tmpwin = (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_create_nhwindow))(NHM.NHW_TEXT))));
                if (tmpwin == -1) {
                    (yield* impossible(cptr.decay(__static_others_check_errfmt), fname, __sl22));
                    break __lbl_closeit;
                }
            }
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, __sl13)));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, ftype)));
            cptr.st1(cptr.decay(line), 0);
            if (!fgets(cptr.decay(line), 256, fh)) {
                void cptr.sprintf(cptr.decay(xbuf), cptr.decay(__static_others_check_errfmt), fname, __sl23);
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, cptr.decay(xbuf))));
                break __lbl_closeit;
            }
            if (cptr.ld1s(cptr.decay(line)) != 35) {
                void cptr.sprintf(cptr.decay(xbuf), cptr.decay(__static_others_check_errfmt), fname, __sl24);
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, cptr.decay(xbuf))));
                if ((endp = cptr.strchr(cptr.decay(line), 10)) !== null)
                    cptr.st1(endp, 0);
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, __sl25)));
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, cptr.decay(line))));
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, __sl26)));
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, (yield* xcrypt(cptr.decay(line), cptr.decay(xbuf))))));
                break __lbl_closeit;
            }
            cptr.st1(cptr.decay(line), 0);
            if (!fgets(cptr.decay(line), 256, fh) || cptr.ld1s(cptr.decay(line)) == 10) {
                void cptr.sprintf(cptr.decay(xbuf), cptr.decay(__static_others_check_errfmt), fname, !cptr.ld1s(cptr.decay(line)) ? __sl27 : __sl28);
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, cptr.decay(xbuf))));
                break __lbl_closeit;
            }
            ++entrycount;
            if ((endp = cptr.strchr(cptr.decay(line), 10)) !== null)
                cptr.st1(endp, 0);
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, (yield* xcrypt(cptr.decay(line), cptr.decay(xbuf))))));
            if (!fgets(cptr.decay(line), 256, fh)) {
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, __sl29)));
            } else {
                ++entrycount;
                if ((endp = cptr.strchr(cptr.decay(line), 10)) !== null)
                    cptr.st1(endp, 0);
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, (yield* xcrypt(cptr.decay(line), cptr.decay(xbuf))))));
                while (fgets(cptr.decay(line), 256, fh)) {
                    ++entrycount;
                    if ((endp = cptr.strchr(cptr.decay(line), 10)) !== null)
                        cptr.st1(endp, 0);
                    void (yield* xcrypt(cptr.decay(line), cptr.decay(xbuf)));
                }
                if (entrycount == 2) {
                    (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, __sl30)));
                } else {
                    if (entrycount > 3)
                        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, __sl31)));
                    (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, cptr.decay(xbuf))));
                }
            }
        }
        void fclose(fh);
    } else {
        (yield* couldnt_open_file(fname));
    }
}

/** C ref: rumors.c:420 — @param {CPtr} fh @param {CPtr} buf @param {CUInt} bufsiz @param {CPtr} rng @param {CLongLong} startpos @param {CLongLong} endpos @param {CUInt} padlength @returns {CPtr} */
function* get_rnd_line(fh, buf, bufsiz, rng, startpos, endpos, padlength) {
    let newl;
    let xbufp;
    let xbuf = new Uint8Array(256);
    let filechunksize;
    let chunkoffset;
    let trylimit;
    cptr.st1(buf, 0);
    if (!endpos) {
        void fseek(fh, 0n, 2);
        endpos = ftell(fh);
    }
    filechunksize = BigInt.asIntN(64, endpos - startpos);
    if (filechunksize < 1n)
        return buf;
    void ((!!(filechunksize <= 2147483647n)) || ((yield* nhassert_failed(__sl32, __sl3, 449)), 0) ? 1 : 0);
    for (trylimit = 10; trylimit > 0; --trylimit) {
        chunkoffset = BigInt((yield* Y.icall((rng)(Number(BigInt.asIntN(32, filechunksize))))));
        void fseek(fh, BigInt.asIntN(64, startpos + chunkoffset), 0);
        void fgets(buf, bufsiz | 0, fh);
        if (!padlength || Number(BigInt.asUintN(32, cptr.strlen(buf))) <= (padlength + 1) >>> 0)
            break;
    }
    if (ftell(fh) >= endpos || !fgets(buf, bufsiz | 0, fh)) {
        void fseek(fh, startpos, 0);
        void fgets(buf, bufsiz | 0, fh);
    }
    if ((newl = cptr.strchr(buf, 10)) !== null)
        cptr.st1(newl, 0);
    xbufp = (cptr.strlen(buf) <= 255n) ? cptr.add(cptr.decay(xbuf), 0, 1) : (yield* alloc((Number(BigInt.asUintN(32, cptr.strlen(buf))) + 1) >>> 0));
    void cptr.strcpy(buf, (yield* xcrypt(buf, xbufp)));
    if (!cptr.eq(xbufp, cptr.add(cptr.decay(xbuf), 0, 1)))
        cptr.free(xbufp);
    if (padlength)
        unpadline(buf);
    return buf;
}

/** C ref: rumors.c:499 — @param {CPtr} fname @param {CPtr} buf @param {CPtr} rng @param {CUInt} padlength @returns {CPtr} */
export function* get_rnd_text(fname, buf, rng, padlength) {
    let fh = fopen(fname, __sl1);
    cptr.st1o(buf, 0, 0);
    if (fh) {
        let starttxt = 0n;
        let line = new Uint8Array(256);
        void fgets(cptr.decay(line), 256, fh);
        void fseek(fh, 0n, 1);
        starttxt = ftell(fh);
        void cptr.strcpy(buf, (yield* get_rnd_line(fh, cptr.decay(line), 256, rng, starttxt, 0n, padlength)));
        void fclose(fh);
    } else {
        (yield* couldnt_open_file(fname));
    }
    return buf;
}

const __static_outrumor_fortune_msg = cptr.bytes("This cookie has a scrap of paper inside."); /** C ref: rumors.c:533 — char[41] (function-static) */

/** C ref: rumors.c:529 — @param {CInt} truth @param {CInt} mechanism */
export function* outrumor(truth, mechanism) {
    let line;
    let buf = new Uint8Array(256);
    let reading = schar((mechanism == NHM.BY_COOKIE || mechanism == NHM.BY_PAPER ? 1 : 0));
    if (reading) {
        if (is_fainted() && mechanism == NHM.BY_COOKIE) {
            return;
        } else if (((cptr.ldI64o2(u, NHC.BLINDED, 24, FLD.you_uprops + FLD.prop_intrinsic) || cptr.ldI64o2(u, NHC.BLINDED, 24, FLD.you_uprops)) && !cptr.ldI64o2(u, NHC.BLINDED, 24, FLD.you_uprops + FLD.prop_blocked))) {
            if (mechanism == NHM.BY_COOKIE)
                (yield* pline(cptr.decay(__static_outrumor_fortune_msg)));
            (yield* pline(__sl33));
            return;
        }
    }
    line = (yield* getrumor(truth, cptr.decay(buf), schar((reading ? 0 : 1))));
    if (!cptr.ld1s(line))
        line = __sl34;
    switch (mechanism) {
        case NHM.BY_ORACLE:
        (yield* pline(__sl35, (!(rng_log_enabled() ? (rng_log_set_caller(__sl3, 558, __sl36), rn2(4)) : rn2(4)) ? __sl37 : (!(rng_log_enabled() ? (rng_log_set_caller(__sl3, 559, __sl36), rn2(3)) : rn2(3)) ? __sl38 : ((rng_log_enabled() ? (rng_log_set_caller(__sl3, 560, __sl36), rn2(2)) : rn2(2)) ? __sl39 : __sl13)))));
        ;
        (yield* verbalize(__sl40, line));
        return;
        case NHM.BY_COOKIE:
        (yield* pline(cptr.decay(__static_outrumor_fortune_msg)));
        // @FallThrough
        ;
        case NHM.BY_PAPER:
        (yield* pline(__sl41));
        break;
    }
    (yield* pline(__sl40, line));
}

/** C ref: rumors.c:577 — @param {CPtr} fp */
function* init_oracles(fp) {
    let i;
    let line = new Uint8Array(256);
    let cnt = cptr.box(0);
    void fgets(cptr.decay(line), 256, fp);
    void fgets(cptr.decay(line), 256, fp);
    if (sscanf(cptr.decay(line), __sl42, cnt) == 1 && cnt.v > 0) {
        cptr.stI32(svo, cnt.v >>> 0);
        cptr.stPtro(svo, FLD.instance_globals_saved_o_oracle_loc, (yield* alloc(Number(BigInt.asUintN(32, BigInt.asUintN(64, BigInt((cnt.v >>> 0) >>> 0) * 8n))))));
        for (i = 0; i < cnt.v; i++) {
            void fgets(cptr.decay(line), 256, fp);
            void sscanf(cptr.decay(line), __sl43, cptr.add(cptr.ldPtro(svo, FLD.instance_globals_saved_o_oracle_loc), i, 8));
        }
    }
    return;
}

/** C ref: rumors.c:598 — @param {CPtr} nhfp */
export function* save_oracles(nhfp) {
    let i;
    if ((cptr.ldI32o((nhfp), FLD.NHFILE_mode) & 3)) {
        (yield* sfo_unsigned(nhfp, svo, __sl44));
        if (cptr.ldI32(svo)) {
            for (i = 0; i >>> 0 < cptr.ldI32(svo); ++i) {
                (yield* sfo_ulong(nhfp, cptr.add(cptr.ldPtro(svo, FLD.instance_globals_saved_o_oracle_loc), i, 8), __sl45));
                ;
            }
        }
    }
    if ((cptr.ldI32o((nhfp), FLD.NHFILE_mode) & NHM.FREEING)) {
        if (cptr.ldI32(svo)) {
            cptr.stI32(svo, 0), cptr.stI32o(go, FLD.instance_globals_o_oracle_flg, 0);
        }
        if (cptr.ldPtro(svo, FLD.instance_globals_saved_o_oracle_loc)) {
            cptr.free(cptr.ldPtro(svo, FLD.instance_globals_saved_o_oracle_loc));
            cptr.stPtro(svo, FLD.instance_globals_saved_o_oracle_loc, null);
        }
    }
}

/** C ref: rumors.c:623 — @param {CPtr} nhfp */
export function* restore_oracles(nhfp) {
    let i;
    (yield* sfi_unsigned(nhfp, svo, __sl44));
    ;
    if (cptr.ldI32(svo)) {
        cptr.stPtro(svo, FLD.instance_globals_saved_o_oracle_loc, (yield* alloc(Number(BigInt.asUintN(32, BigInt.asUintN(64, BigInt(cptr.ldI32(svo) >>> 0) * 8n))))));
        for (i = 0; i >>> 0 < cptr.ldI32(svo); ++i) {
            (yield* sfi_ulong(nhfp, cptr.add(cptr.ldPtro(svo, FLD.instance_globals_saved_o_oracle_loc), i, 8), __sl45));
            ;
        }
        cptr.stI32o(go, FLD.instance_globals_o_oracle_flg, 1);
    }
}

/** C ref: rumors.c:640 — @param {CInt} special @param {CInt} delphi */
export function* outoracle(special, delphi) {
    let tmpwin;
    let oracles;
    let oracle_idx;
    let endp;
    let line = new Uint8Array(80);
    let xbuf = new Uint8Array(256);
    if (cptr.ldI32o(go, FLD.instance_globals_o_oracle_flg) < 0 || (cptr.ldI32o(go, FLD.instance_globals_o_oracle_flg) > 0 && cptr.ldI32(svo) == 0))
        return;
    oracles = fopen(__sl46, __sl1);
    if (oracles) {
        __lbl_close_oracles: {
            if (cptr.ldI32o(go, FLD.instance_globals_o_oracle_flg) == 0) {
                (yield* init_oracles(oracles));
                cptr.stI32o(go, FLD.instance_globals_o_oracle_flg, 1);
                if (cptr.ldI32(svo) == 0)
                    break __lbl_close_oracles;
            }
            if (cptr.ldI32(svo) <= 1 && !special)
                break __lbl_close_oracles;
            oracle_idx = special ? 0 : (rng_log_enabled() ? (rng_log_set_caller(__sl3, 665, __sl47), rnd(((cptr.ldI32(svo) | 0) - 1) | 0)) : rnd(((cptr.ldI32(svo) | 0) - 1) | 0));
            void fseek(oracles, BigInt.asIntN(64, cptr.ldU64o(cptr.ldPtro(svo, FLD.instance_globals_saved_o_oracle_loc), oracle_idx, 8)), 0);
            if (!special)
                cptr.stU64o(cptr.ldPtro(svo, FLD.instance_globals_saved_o_oracle_loc), oracle_idx, cptr.ldU64o(cptr.ldPtro(svo, FLD.instance_globals_saved_o_oracle_loc), cptr.stI32(svo, cptr.ldI32(svo) + -1), 8), 8);
            tmpwin = (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_create_nhwindow))(NHM.NHW_TEXT)));
            if (delphi)
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, special ? __sl48 : __sl49)));
            else
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, __sl50)));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, __sl13)));
            while (fgets(cptr.decay(line), NHM.COLNO, oracles) && strcmp(cptr.decay(line), __sl51)) {
                if ((endp = cptr.strchr(cptr.decay(line), 10)) !== null)
                    cptr.st1(endp, 0);
                (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, (yield* xcrypt(cptr.decay(line), cptr.decay(xbuf))))));
            }
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_display_nhwindow))(tmpwin, 1)));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_destroy_nhwindow))(tmpwin)));
        }
        void fclose(oracles);
    } else {
        (yield* couldnt_open_file(__sl46));
        cptr.stI32o(go, FLD.instance_globals_o_oracle_flg, -1);
    }
}

/** C ref: rumors.c:696 — @param {CPtr} oracl @returns {CInt} */
export function* doconsult(oracl) {
    let umoney;
    let u_pay;
    let minor_cost = 50;
    let major_cost = (500 + Math.imul(50, cptr.ldI32o(u, FLD.you_ulevel))) | 0;
    let add_xpts;
    let qbuf = new Uint8Array(128);
    cptr.stI64o(gm, FLD.instance_globals_m_multi, 0n);
    umoney = money_cnt(cptr.ldPtro(gi, FLD.instance_globals_i_invent));
    if (!oracl) {
        (yield* There(__sl52));
        return NHM.ECMD_OK;
    } else if (!(cptr.ldI32o(oracl, FLD.monst_mpeaceful) & 1)) {
        (yield* pline(__sl53, (yield* Monnam(oracl))));
        return NHM.ECMD_OK;
    } else if (!umoney) {
        (yield* You(__sl54));
        return NHM.ECMD_OK;
    }
    void cptr.sprintf(cptr.decay(qbuf), __sl55, minor_cost, (yield* currency(BigInt(minor_cost))));
    switch ((yield* yn_function(cptr.decay(qbuf), cptr.decay(ynqchars), 113, 1))) {
        default:
        case 113:
        return NHM.ECMD_OK;
        case 121:
        if (umoney < BigInt(minor_cost)) {
            (yield* You(__sl56));
            return NHM.ECMD_OK;
        }
        u_pay = minor_cost;
        break;
        case 110:
        if (umoney <= BigInt(minor_cost) || (cptr.ldI32(svo) == 1 || cptr.ldI32o(go, FLD.instance_globals_o_oracle_flg) < 0))
            return NHM.ECMD_OK;
        void cptr.sprintf(cptr.decay(qbuf), __sl57, major_cost, (yield* currency(BigInt(major_cost))));
        if ((yield* yn_function(cptr.decay(qbuf), cptr.decay(ynchars), 110, 1)) != 121)
            return NHM.ECMD_OK;
        u_pay = (umoney < BigInt(major_cost)) ? Number(BigInt.asIntN(32, umoney)) : major_cost;
        break;
    }
    (yield* money2mon(oracl, BigInt(u_pay)));
    cptr.st1(disp, 1);
    if (!(cptr.ldI32o(u, FLD.you_uevent + FLD.u_event_major_oracle) & 1) && !(cptr.ldI32o(u, FLD.you_uevent) & 1))
        (yield* record_achievement(NHC.ACH_ORCL));
    add_xpts = 0;
    if (u_pay == minor_cost) {
        (yield* outrumor(1, NHM.BY_ORACLE));
        if (!(cptr.ldI32o(u, FLD.you_uevent) & 1))
            add_xpts = (u_pay / ((cptr.ldI32o(u, FLD.you_uevent + FLD.u_event_major_oracle) & 1) | 0 ? 25 : 10)) | 0;
        cptr.stI32o(u, FLD.you_uevent, 1);
    } else {
        let cheapskate = schar((u_pay < major_cost));
        (yield* outoracle(cheapskate, 1));
        if (!cheapskate && !(cptr.ldI32o(u, FLD.you_uevent + FLD.u_event_major_oracle) & 1))
            add_xpts = (u_pay / ((cptr.ldI32o(u, FLD.you_uevent) & 1) | 0 ? 25 : 10)) | 0;
        cptr.stI32o(u, FLD.you_uevent + FLD.u_event_major_oracle, 1);
        (yield* exercise(NHC.A_WIS, schar((!cheapskate))));
    }
    if (add_xpts) {
        (yield* more_experienced(add_xpts, (u_pay / 50) | 0));
        (yield* newexplevel());
    }
    return NHM.ECMD_TIME;
}

/** C ref: rumors.c:770 — @param {CPtr} filename */
function* couldnt_open_file(filename) {
    let save_something = cptr.ldI32o(program_state, FLD.sinfo_something_worth_saving);
    if (!cptr.ld1so(iflags, FLD.instance_flags_debug_fuzzer))
        cptr.stI32o(program_state, FLD.sinfo_something_worth_saving, 0);
    (yield* impossible(__sl58, filename));
    cptr.stI32o(program_state, FLD.sinfo_something_worth_saving, save_something);
}

/** C ref: rumors.c:791 — @param {CPtr} word @returns {CInt} */
export function* CapitalMon(word) {
    let nam;
    let i;
    let wln;
    let nln;
    if (!word || !cptr.ld1s(word) || cptr.ld1s(word) == lowc(cptr.ld1s(word)))
        return 0;
    if (!CapMons)
        (yield* init_CapMons());
    (__builtin_expect(BigInt((!(CapMons !== null))), 0n) ? __assert_rtn(__sl59, __sl60, 803, __sl61) : void 0);
    wln = Number(BigInt.asUintN(32, cptr.strlen(word)));
    for (i = 0; i < (CapMonSiz - 1) >>> 0; ++i) {
        nam = cptr.ldPtro(CapMons, i, 8);
        nln = Number(BigInt.asUintN(32, cptr.strlen(nam)));
        if (wln < nln)
            continue;
        if (!cptr.strncmp(nam, word, BigInt(nln >>> 0)) && (!cptr.ld1so(word, nln) || cptr.ld1so(word, nln) == 32 || cptr.ld1so(word, nln) == 39))
            return 1;
    }
    return 0;
}

/** C ref: rumors.c:829 */
function* init_CapMons() {
    let pass;
    let bogonfile = fopen(__sl21, __sl1);
    if (CapMons)
        free_CapMons();
    for (pass = 1; pass <= 2; ++pass) {
        let mptr;
        let nam;
        let mndx;
        let mgend;
        CapMonstCnt = (CapBogonCnt = 0);
        for (mndx = NHC.LOW_PM; mndx < NHC.NUMMONS; ++mndx) {
            mptr = cptr.add(mons, mndx, 96);
            if ((cptr.ldU16o(mptr, FLD.permonst_geno) & NHM.G_UNIQ) != 0 && !the_unique_pm(mptr))
                continue;
            for (mgend = NHC.MALE; mgend < NHC.NUM_MGENDERS; ++mgend) {
                nam = cptr.ldPtro(mptr, mgend, 8);
                if (nam && cptr.ld1s(nam) != lowc(cptr.ld1s(nam))) {
                    if (pass == 2)
                        cptr.stPtro(CapMons, CapMonstCnt, nam, 8);
                    ++CapMonstCnt;
                }
            }
        }
        if (bogonfile) {
            let hline = new Uint8Array(256);
            let xbuf = new Uint8Array(256);
            let endp;
            let startp;
            let code;
            void fseek(bogonfile, 0n, 0);
            void fgets(cptr.decay(hline), 256, bogonfile);
            while (fgets(cptr.decay(hline), 256, bogonfile)) {
                if ((endp = cptr.strchr(cptr.decay(hline), 10)) !== null)
                    cptr.st1(endp, 0);
                void (yield* xcrypt(cptr.decay(hline), cptr.decay(xbuf)));
                unpadline(cptr.decay(xbuf));
                if (!cptr.ld1so(cptr.decay(xbuf), 0, 1) || !cptr.strchr(cptr.decay(bogon_codes), cptr.ld1so(cptr.decay(xbuf), 0, 1)))
                    code = 0, startp = cptr.add(cptr.decay(xbuf), 0, 1);
                else
                    code = cptr.ld1so(cptr.decay(xbuf), 0, 1), startp = cptr.add(cptr.decay(xbuf), 1, 1);
                if (cptr.ld1s(startp) != lowc(cptr.ld1s(startp)) && !bogon_is_pname(code)) {
                    if (pass == 2)
                        cptr.stPtro(CapMons, (CapMonstCnt + CapBogonCnt) >>> 0, (yield* dupstr(startp)), 8);
                    ++CapBogonCnt;
                }
            }
        }
        if (pass == 1) {
            CapMonSiz = (((CapMonstCnt + CapBogonCnt) >>> 0) + 1) >>> 0;
            CapMons = (yield* alloc(Number(BigInt.asUintN(32, BigInt.asUintN(64, BigInt(CapMonSiz >>> 0) * 8n)))));
        } else {
            cptr.stPtro(CapMons, (CapMonSiz - 1) >>> 0, null, 8);
            if (bogonfile)
                void fclose(bogonfile), bogonfile = null;
        }
    }
    if (cptr.ld1so(flags, FLD.flag_debug) && (yield* debugcore(__sl62, 0))) {
        let buf = new Uint8Array(256);
        let i;
        let tmpwin = (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_create_nhwindow))(NHM.NHW_TEXT)));
        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, __sl63)));
        for (i = 0; i < (CapMonSiz - 1) >>> 0; ++i) {
            void cptr.sprintf(cptr.decay(buf), __sl64, cptr.ldPtro(CapMons, i, 8));
            (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_putstr))(tmpwin, 0, cptr.decay(buf))));
        }
        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_display_nhwindow))(tmpwin, 1)));
        (yield* Y.icall((cptr.ldPtro(windowprocs, FLD.window_procs_win_destroy_nhwindow))(tmpwin)));
    }
    return;
}

/** C ref: rumors.c:939 */
export function free_CapMons() {
    if (CapMons) {
        let idx;
        for (idx = CapMonstCnt; idx < (CapMonSiz - 1) >>> 0; ++idx)
            cptr.free(cptr.ldPtro(CapMons, idx, 8));
        cptr.free(CapMons), CapMons = null;
    }
    CapMonSiz = 0;
}

// --- BEGIN c2js reset block (tools/c2js/resetify.mjs) — do not edit ---
// 8 bindings: 0 rebound+refilled, 5 rebound, 3 refilled.
// S/P are supplied by js/generated-y/__reset.js so this module needs no new import.
let __c2js_rs = null;
export function __captureState(S) { __c2js_rs = [S(CapMonstCnt), S(CapBogonCnt), S(CapMonSiz), S(CapMons), S(__static_init_rumors_rumors_header), S(__static_getrumor_cookie_marker), S(__static_others_check_errfmt), S(__static_outrumor_fortune_msg)]; }
export function __resetState(P) {
    const r = __c2js_rs;
    if (r === null) throw new Error("rumors.js: __resetState before __captureState");
    CapMonstCnt = P(r[0]);
    CapBogonCnt = P(r[1]);
    CapMonSiz = P(r[2]);
    CapMons = P(r[3]);
    P(r[4]);
    __static_getrumor_cookie_marker = P(r[5]);
    P(r[6]);
    P(r[7]);
}
// --- END c2js reset block ---
