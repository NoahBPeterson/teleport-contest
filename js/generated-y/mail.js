// YIELDABLE BUILD — generated by tools/c2js/yieldify.mjs from js/generated/mail.js
// Do not edit by hand, and do not score from this directory: the synchronous
// build in js/generated/ is the one the judge runs. Coloured functions here
// are generators; `yield*` marks every call that can reach a keystroke read.
import * as Y from '../yield-rt.js';
// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/src/mail.c
// Input sha256: 0d86390373abfbb64c21e69d5a2e0e917365c0039f9445848b87a5793d296d61
// Transpiler: tools/c2js c2js emit v1+batch

import { i16 } from '../cmachine.js';
import * as cptr from '../cptr.js';
import * as NHC from './nhconst.js';
import * as NHM from './nhmacro.js';
import { nh_getenv } from './options.js';
import { alloc, dupstr } from './alloc.js';
import { debugcore } from './files.js';
import { WIN_MESSAGE, c_common_strings, flags, gs, gv, iflags, svl, svm, svp, u } from './decl.js';
import { pline, verbalize } from './pline.js';
import { enexto } from './teleport.js';
import { dist2 } from './hacklib.js';
import { isok } from './cmd.js';
import { accessible } from './monmove.js';
import { rn2, rng_log_enabled, rng_log_set_caller } from './rnd.js';
import { mons } from './monst.js';
import { flush_screen, newsym } from './display.js';
import { place_monster } from './steed.js';
import { windowprocs } from './windows.js';
import { makemon } from './makemon.js';
import { Hello } from './role.js';
import { mksobj, new_omailcmd } from './mkobj.js';
import { oname } from './do_name.js';
import { hold_another_object } from './invent.js';
import { mongone } from './mon.js';
import { child } from './unixunix.js';
import { nh_terminate } from './end.js';

// string literals (C char* uses decay to CPtr into these static buffers)
const __sl0 = cptr.lit("MAIL");
const __sl1 = cptr.lit("/usr/mail/");
const __sl2 = cptr.lit("/Users/noahpeterson/Documents/Projects/teleport-contest-research/original-contest-to-fork/nethack-c/recorder/src/mail.c");
const __sl3 = cptr.lit("mailbox=%c%s%c");
const __sl4 = cptr.lit("null");
const __sl5 = cptr.lit("md_stop");
const __sl6 = cptr.lit("Gangway!");
const __sl7 = cptr.lit("Look out!");
const __sl8 = cptr.lit("Pardon me!");
const __sl9 = cptr.lit("%s");
const __sl10 = cptr.lit("md_rush");
const __sl11 = cptr.lit("Excuse me.");
const __sl12 = cptr.lit("This place's too crowded.  I'm outta here.");
const __sl13 = cptr.lit("%s.");
const __sl14 = cptr.lit("%s, %s!  %s.");
const __sl15 = cptr.lit("Message:  %s.");
const __sl16 = cptr.lit("Catch!");
const __sl17 = cptr.lit("Oops!");
const __sl18 = cptr.lit("Hark!  \"%s.\"");
const __sl19 = cptr.lit("I have some mail for you");
const __sl20 = cptr.lit("MAILREADER");
const __sl21 = cptr.lit("/usr/bin/mailx");

/** C ref: mail.c:69 — struct stat */
let omstat = cptr.alloc(144);

/** C ref: mail.c:69 — struct stat */
let nmstat = cptr.alloc(144);

/** C ref: mail.c:70 — char * */
let mailbox = null;

/** C ref: mail.c:71 — long */
let laststattime = 0n;

/** C ref: mail.c:90 */
export function free_maildata() {
    if (mailbox)
        cptr.free(mailbox), mailbox = null;
}

/** C ref: mail.c:97 */
export function* getmailstatus() {
    if (mailbox) {
        ;
    } else if ((mailbox = nh_getenv(__sl0)) !== null) {
        mailbox = (yield* dupstr(mailbox));
    } else {
        let pw_name = cptr.ldPtr(getpwuid(getuid()));
        mailbox = (yield* alloc(Number(BigInt.asUintN(32, (BigInt.asUintN(64, cptr.strlen(pw_name) + 11n))))));
        void cptr.strcpy(mailbox, __sl1);
        void cptr.strcat(mailbox, pw_name);
    }
    do {
        if ((yield* debugcore(__sl2, 1))) {
            let save_plnmsg = cptr.ldI32o(iflags, 40);
            (yield* pline(__sl3, mailbox ? 34 : 60, mailbox ? mailbox : __sl4, mailbox ? 34 : 62));
            cptr.stI32o(iflags, 40, save_plnmsg);
        }
    } while (0);
    if (mailbox && stat(mailbox, omstat)) {
        cptr.stI64o(omstat, 48, 0n);
    }
}

/** C ref: mail.c:149 — @param {CPtr} startp @returns {CInt} */
function* md_start(startp) {
    let testcc = cptr.alloc(4);
    let row;
    let lax;
    let dd;
    let max_distance;
    let stway = cptr.ldPtro(gs, 8);
    if (((cptr.ldI64o2(u, NHC.BLINDED, 24, 128) || cptr.ldI64o2(u, NHC.BLINDED, 24, 112)) && !cptr.ldI64o2(u, NHC.BLINDED, 24, 120)) && !(cptr.ldI64o2(u, NHC.TELEPAT, 24, 128) || cptr.ldI64o2(u, NHC.TELEPAT, 24, 112))) {
        if (!(yield* enexto(startp, cptr.ldI16(u), cptr.ldI16o(u, 2), null)))
            return 0;
        return 1;
    }
    while (stway) {
        if (cptr.ldI16o(stway, 4) == cptr.ldI16o(u, 24) && ((cptr.ld1uo(cptr.ldPtro(cptr.ldPtro(gv, 120), cptr.ldI16o(stway, 2), 8), cptr.ldI16(stway)) & NHM.COULD_SEE) != 0)) {
            cptr.stI16(startp, cptr.ldI16(stway));
            cptr.stI16o(startp, 2, cptr.ldI16o(stway, 2));
            return 1;
        }
        stway = cptr.ldPtro(stway, 16);
    }
    lax = 0;
    max_distance = -1;
    __lbl_retry: while (true) {
        for (row = 0; row < NHM.ROWNO; row++) {
            if (cptr.ldI16o(cptr.ldPtro(gv, 128), row, 2) < cptr.ldI16o(cptr.ldPtro(gv, 136), row, 2)) {
                dd = dist2((cptr.ldI16o(cptr.ldPtro(gv, 128), row, 2)), i16((row)), cptr.ldI16(u), cptr.ldI16o(u, 2));
                if (dd > max_distance) {
                    if (lax) {
                        max_distance = dd;
                        cptr.stI16o(startp, 2, i16(row));
                        cptr.stI16(startp, cptr.ldI16o(cptr.ldPtro(gv, 128), row, 2));
                    } else if ((yield* enexto(testcc, cptr.ldI16o(cptr.ldPtro(gv, 128), row, 2), i16(row), null)) && !((cptr.ld1uo(cptr.ldPtro(cptr.ldPtro(gv, 120), cptr.ldI16o(testcc, 2), 8), cptr.ldI16(testcc)) & NHM.IN_SIGHT) != 0) && ((cptr.ld1uo(cptr.ldPtro(cptr.ldPtro(gv, 120), cptr.ldI16o(testcc, 2), 8), cptr.ldI16(testcc)) & NHM.COULD_SEE) != 0)) {
                        max_distance = dd;
                        cptr.memcpy(startp, testcc, 4);
                    }
                }
                dd = dist2((cptr.ldI16o(cptr.ldPtro(gv, 136), row, 2)), i16((row)), cptr.ldI16(u), cptr.ldI16o(u, 2));
                if (dd > max_distance) {
                    if (lax) {
                        max_distance = dd;
                        cptr.stI16o(startp, 2, i16(row));
                        cptr.stI16(startp, cptr.ldI16o(cptr.ldPtro(gv, 136), row, 2));
                    } else if ((yield* enexto(testcc, cptr.ldI16o(cptr.ldPtro(gv, 136), row, 2), i16(row), null)) && !((cptr.ld1uo(cptr.ldPtro(cptr.ldPtro(gv, 120), cptr.ldI16o(testcc, 2), 8), cptr.ldI16(testcc)) & NHM.IN_SIGHT) != 0) && ((cptr.ld1uo(cptr.ldPtro(cptr.ldPtro(gv, 120), cptr.ldI16o(testcc, 2), 8), cptr.ldI16(testcc)) & NHM.COULD_SEE) != 0)) {
                        max_distance = dd;
                        cptr.memcpy(startp, testcc, 4);
                    }
                }
            }
        }
        if (max_distance < 0) {
            if (!lax) {
                lax = 1;
                continue __lbl_retry;
            }
            return 0;
        }
        return 1;
        break __lbl_retry;
    }
}

/** C ref: mail.c:248 — @param {CPtr} stopp @param {CPtr} startp @returns {CInt} */
function* md_stop(stopp, startp) {
    let x;
    let y;
    let distance;
    let min_distance = -1;
    for (x = i16(((cptr.ldI16(u) - 1) | 0)); x <= ((cptr.ldI16(u) + 1) | 0); x++)
        for (y = i16(((cptr.ldI16o(u, 2) - 1) | 0)); y <= ((cptr.ldI16o(u, 2) + 1) | 0); y++) {
            if (!isok(x, y) || ((x) == cptr.ldI16(u) && (y) == cptr.ldI16o(u, 2)))
                continue;
            if (accessible(x, y) && !(cptr.ldPtro3(svl, x, 168, y, 8, 75600) !== null)) {
                distance = i16(dist2(x, y, cptr.ldI16(startp), cptr.ldI16o(startp, 2)));
                if (min_distance < 0 || distance < min_distance || (distance == min_distance && (rng_log_enabled() ? (rng_log_set_caller(__sl2, 261, __sl5), rn2(2)) : rn2(2)))) {
                    cptr.stI16(stopp, x);
                    cptr.stI16o(stopp, 2, y);
                    min_distance = distance;
                }
            }
        }
    if (min_distance < 0 && !(yield* enexto(stopp, cptr.ldI16(u), cptr.ldI16o(u, 2), cptr.add(mons, NHC.PM_MAIL_DAEMON, 96))))
        return 0;
    return 1;
}

/** C ref: mail.c:277 — char *[3] */
const mail_text = cptr.alloc(3 * 8);
cptr.stPtro(mail_text, 0, __sl6);
cptr.stPtro(mail_text, 8, __sl7);
cptr.stPtro(mail_text, 16, __sl8);

/** C ref: mail.c:288 — @param {CPtr} md @param {CInt} tx @param {CInt} ty @returns {CInt} */
function* md_rush(md, tx, ty) {
    let mon;
    let dx;
    let dy;
    let fx = cptr.ldI16o(md, 28);
    let fy = cptr.ldI16o(md, 30);
    let nfx = fx;
    let nfy = fy;
    let d1;
    let d2;
    if (cptr.eq((cptr.ldPtro3(svl, fx, 168, fy, 8, 75600)), md)) {
        cptr.stPtro3(svl, fx, 168, fy, 8, 75600, null);
        (yield* newsym(i16(fx), i16(fy)));
    }
    while (1) {
        d1 = dist2(i16(fx), i16(fy), i16(tx), i16(ty));
        for (dx = -1; dx <= 1; dx++)
            for (dy = -1; dy <= 1; dy++)
                if ((dx || dy) && isok(i16(((fx + dx) | 0)), i16(((fy + dy) | 0))) && !((cptr.ld1so3(svl, (fx + dx) | 0, 756, (fy + dy) | 0, 36, 1684)) <= NHC.DBWALL)) {
                    d2 = dist2(i16(((fx + dx) | 0)), i16(((fy + dy) | 0)), i16(tx), i16(ty));
                    if (d2 < d1) {
                        d1 = d2;
                        nfx = (fx + dx) | 0;
                        nfy = (fy + dy) | 0;
                    }
                }
        if (nfx == fx && nfy == fy)
            break;
        fx = nfx;
        fy = nfy;
        if (fx == tx && fy == ty)
            break;
        mon = (cptr.ldPtro3(svl, fx, 168, fy, 8, 75600));
        if (!(cptr.ldI64o2(u, NHC.DEAF, 24, 128) || cptr.ldI64o2(u, NHC.DEAF, 24, 112) || cptr.ld1so(u, 2114))) {
            ;
            if (mon)
                (yield* verbalize(__sl9, (cptr.ldPtro(mail_text, (rng_log_enabled() ? (rng_log_set_caller(__sl2, 340, __sl10), rn2(3)) : rn2(3)), 8))));
            else if (((fx) == cptr.ldI16(u) && (fy) == cptr.ldI16o(u, 2)))
                (yield* verbalize(__sl11));
        }
        if (mon)
            cptr.stPtro3(svl, fx, 168, fy, 8, 75600, null);
        (yield* place_monster(md, i16(fx), i16(fy)));
        (yield* newsym(i16(fx), i16(fy)));
        (yield* flush_screen(0));
        (yield* Y.icall((cptr.ldPtro(windowprocs, 320))()));
        cptr.stPtro3(svl, fx, 168, fy, 8, 75600, null);
        if (mon) {
            if ((cptr.ldI16o(mon, 28) != fx) || (cptr.ldI16o(mon, 30) != fy))
                cptr.stPtro3(svl, fx, 168, fy, 8, 75600, mon);
            else
                (yield* place_monster(mon, i16(fx), i16(fy)));
        }
        (yield* newsym(i16(fx), i16(fy)));
    }
    if ((mon = (cptr.ldPtro3(svl, fx, 168, fy, 8, 75600))) !== null) {
        cptr.stPtro3(svl, fx, 168, fy, 8, 75600, null);
        (yield* place_monster(md, i16(fx), i16(fy)));
        (yield* newsym(i16(fx), i16(fy)));
        if (!(cptr.ldI64o2(u, NHC.DEAF, 24, 128) || cptr.ldI64o2(u, NHC.DEAF, 24, 112) || cptr.ld1so(u, 2114))) {
            ;
            (yield* verbalize(__sl12));
        } else {
            (yield* pline(__sl13, cptr.ldPtro(c_common_strings, 64)));
        }
        cptr.stPtro3(svl, fx, 168, fy, 8, 75600, null);
        if ((cptr.ldI16o(mon, 28) != fx) || (cptr.ldI16o(mon, 30) != fy))
            cptr.stPtro3(svl, fx, 168, fy, 8, 75600, mon);
        else
            (yield* place_monster(mon, i16(fx), i16(fy)));
        (yield* newsym(i16(fx), i16(fy)));
        return 0;
    }
    (yield* place_monster(md, i16(fx), i16(fy)));
    (yield* newsym(i16(fx), i16(fy)));
    (yield* flush_screen(0));
    (yield* Y.icall((cptr.ldPtro(windowprocs, 320))()));
    return 1;
}

/** C ref: mail.c:399 — @param {CPtr} info */
function* newmail(info) {
    let md;
    let start = cptr.alloc(4);
    let stop = cptr.alloc(4);
    let message_seen = 0;
    __lbl_give_up: {
    __lbl_go_back: {
        if (!(yield* md_start(start)) || !(yield* md_stop(stop, start)))
            break __lbl_give_up;
        if (!(md = (yield* makemon(cptr.add(mons, NHC.PM_MAIL_DAEMON, 96), cptr.ldI16(start), cptr.ldI16o(start, 2), NHM.NO_MM_FLAGS))))
            break __lbl_give_up;
        if (!(yield* md_rush(md, cptr.ldI16(stop), cptr.ldI16o(stop, 2))))
            break __lbl_go_back;
        message_seen = 1;
        if (!(cptr.ldI64o2(u, NHC.DEAF, 24, 128) || cptr.ldI64o2(u, NHC.DEAF, 24, 112) || cptr.ld1so(u, 2114))) {
            ;
            (yield* verbalize(__sl14, Hello(md), svp, cptr.ldPtro(info, 8)));
        } else {
            (yield* pline(__sl15, cptr.ldPtro(info, 8)));
        }
        if (cptr.ldI32(info)) {
            let obj = (yield* mksobj(NHC.SCR_MAIL, 0, 0));
            if (cptr.ldPtro(info, 16))
                obj = (yield* oname(obj, cptr.ldPtro(info, 16), NHM.ONAME_NO_FLAGS));
            if (cptr.ldPtro(info, 24))
                (yield* new_omailcmd(obj, cptr.ldPtro(info, 24)));
            if (!(dist2((cptr.ldI16o((md), 28)), (cptr.ldI16o((md), 30)), cptr.ldI16(u), cptr.ldI16o(u, 2)) <= 2)) {
                if (!(cptr.ldI64o2(u, NHC.DEAF, 24, 128) || cptr.ldI64o2(u, NHC.DEAF, 24, 112) || cptr.ld1so(u, 2114))) {
                    ;
                    (yield* verbalize(__sl16));
                } else {
                    ;
                }
            }
            (yield* Y.icall((cptr.ldPtro(windowprocs, 120))(WIN_MESSAGE.v, 0)));
            obj = (yield* hold_another_object(obj, __sl17, null, null));
            (void (obj));
        }
    }
        if (!(yield* md_rush(md, cptr.ldI16(start), cptr.ldI16o(start, 2))))
            cptr.stI16o(md, 28, cptr.stI16o(md, 30, 0));
        (yield* mongone(md));
    }
    if (!message_seen && cptr.ldI32(info) == NHM.MSG_OTHER)
        (yield* pline(__sl18, cptr.ldPtro(info, 8)));
}

let __static_ckmailstatus_deliver = cptr.alloc(32); /** C ref: mail.c:571 — struct mail_info (function-static) */
cptr.stI32(__static_ckmailstatus_deliver, NHM.MSG_MAIL);
cptr.stPtro(__static_ckmailstatus_deliver, 8, __sl19);
cptr.stPtro(__static_ckmailstatus_deliver, 16, null);
cptr.stPtro(__static_ckmailstatus_deliver, 24, null);

/** C ref: mail.c:550 */
export function* ckmailstatus() {
    ck_server_admin_msg();
    if (!mailbox || (cptr.ldI32o(u, 1848) & 1) | 0 || !cptr.ld1so(flags, 6) || cptr.ldI64o(svm, 8) < BigInt.asIntN(64, laststattime + 50n))
        return;
    laststattime = cptr.ldI64o(svm, 8);
    if (stat(mailbox, nmstat)) {
        cptr.stI64o(nmstat, 48, 0n);
    } else if (cptr.ldI64o(nmstat, 48) > cptr.ldI64o(omstat, 48)) {
        if (cptr.ldI64o(nmstat, 96)) {
            (yield* newmail(__static_ckmailstatus_deliver));
        }
        (yield* getmailstatus());
    }
}

/** C ref: mail.c:685 */
export function ck_server_admin_msg() {
}

/** C ref: mail.c:704 — @param {CPtr} otmp */
export function* readmail(otmp) {
    let mr = null;
    if (cptr.ld1so(iflags, 15))
        return;
    (yield* Y.icall((cptr.ldPtro(windowprocs, 120))(WIN_MESSAGE.v, 0)));
    if (!(mr = nh_getenv(__sl20)))
        mr = __sl21;
    if ((yield* child(1))) {
        void execl(mr, mr, null);
        (yield* nh_terminate(1));
    }
    (yield* getmailstatus());
}

// --- BEGIN c2js reset block (tools/c2js/resetify.mjs) — do not edit ---
// 6 bindings: 3 rebound+refilled, 2 rebound, 1 refilled.
// S/P are supplied by js/generated-y/__reset.js so this module needs no new import.
let __c2js_rs = null;
export function __captureState(S) { __c2js_rs = [S(omstat), S(nmstat), S(mailbox), S(laststattime), S(mail_text), S(__static_ckmailstatus_deliver)]; }
export function __resetState(P) {
    const r = __c2js_rs;
    if (r === null) throw new Error("mail.js: __resetState before __captureState");
    omstat = P(r[0]);
    nmstat = P(r[1]);
    mailbox = P(r[2]);
    laststattime = P(r[3]);
    P(r[4]);
    __static_ckmailstatus_deliver = P(r[5]);
}
// --- END c2js reset block ---
