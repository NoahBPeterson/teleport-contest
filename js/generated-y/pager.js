...
export function* mhidden_description(mon, mhid_flags, outbuf) {
    ...
    if (__go_objfrommap) {
        ...
        what = (otmp.v && cptr.ldI16o(otmp.v, 32) != NHC.STRANGE_OBJECT) ? (yield* simpleonames(otmp.v)) : cptr.ldPtro(obj_descr, NHC.STRANGE_OBJECT, 16);
...
function* look_at_monster(buf, monbuf, mtmp, x, y) {
    ...
    name = (cptr.eq(cptr.ldPtro(mtmp, 8), cptr.add(mons, NHC.PM_COYOTE, 96)) && accurate) ? (yield* coyotename(mtmp, cptr.decay(monnambuf))) : (yield* distant_monnam(mtmp, NHM.ARTICLE_NONE, cptr.decay(monnambuf)));
        ...
        if (cptr.eq(x_str, cptr.decay(__static_do_screen_description_unreconnoitered)))
            {
                if (looked) {
                    ...
                    if (found > 1 || need_to_look) {
                        ...
                        if (!strcmp(cptr.decay(__static_do_screen_description_look_buf), __sl151) && on_level(cptr.add(u, 24), cptr.add(svd, 1882)) && !(yield* ok_to_quest()))
...
export function* doidtrap() {
    ...
    for (trap = cptr.ldPtr(gf); trap; trap = cptr.ldPtr(trap))
        if (cptr.ldI16o(trap, 8) == x && cptr.ldI16o(trap, 10) == y) {
            ...
            (yield* pline(__sl238, (yield* an((yield* trapname(tt, 0)))), !(cptr.ldI32o(trap, 32) & 1) ? __sl4 : ((tt == NHC.WEB) ? __sl239 : ((tt == NHC.HOLE || tt == NHC.PIT) ? __sl240 : __sl241)), !(cptr.ldI32o(trap, 32) & 1) ? __sl4 : __sl242));
            return NHM.ECMD_OK;
        }
    (yield* pline(__sl243));
    return NHM.ECMD_OK;
