...
export function* random_engraving(outbuf, pristine_copy) {
    let rumor;
    if (!(rng_log_enabled() ? (rng_log_set_caller(__s_engrave_c, 57, __s_random_engraving), rn2(4)) : rn2(4)) || !(rumor = (yield* getrumor(0, pristine_copy, 1))) || !cptr.ld1s(rumor))
...
export function* doengrave() {
    ...
    __lbl_doengr_exit: {
        ...
        if (cptr.ld1so(de, $_doengrave_ctx_doblind) && !(yield* resists_blnd(cptr.add(gy, $instance_globals_y_youmonst)))) {
            ...
            (yield* make_blinded(BigInt((rng_log_enabled() ? (rng_log_set_caller(__s_engrave_c, 1250, __s_doengrave), rnd(50)) : rnd(50))), 0));
            if (!Blind())
