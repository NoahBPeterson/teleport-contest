...
function mkshobj_at(shp, sx, sy, mkspecl) {
    ...
    if ((rng_log_enabled() ? (rng_log_set_caller(__s_shknam_c, 470, __s_mkshobj_at), rn2(100)) : rn2(100)) < depth(cptr.add(u, $you_uz)) && !(cptr.ldPtro3(svl, sx, 168, sy, 8, $instance_globals_saved_l_level + $dlevel_t_monsters) !== null) && (ptr = mkclass(NHC.S_MIMIC, 0)) !== null && (mtmp = makemon(ptr, i16(sx), i16(sy), NHM.NO_MM_FLAGS)) !== null) {
    } else {
...
function shkinit(shp, sroom) {
    ...
    if (sh < 0) {
        if (wizard()) {
            ...
            while (j--) {
                pline(__s_door_d_d, cptr.ldI16o(cptr.ldPtro(svd, $instance_globals_saved_d_doors), sh, 4), cptr.ldI16o2(cptr.ldPtro(svd, $instance_globals_saved_d_doors), sh, 4, $nhcoord_y));
                sh++;
            }
    ...
    mkmonmoney(shk, BigInt.asIntN(64, 1000n + BigInt.asIntN(64, 30n * BigInt((rng_log_enabled() ? (rng_log_set_caller(__s_shknam_c, 682, __s_shkinit), rnd(100)) : rnd(100))))));
...
export function get_shop_item(type) {
    ...
    let i;
    let j;
    for (j = (rng_log_enabled() ? (rng_log_set_caller(__s_shknam_c, 835, __s_get_shop_item), rnd(100)) : rnd(100)), i = 0; (j = (j - cptr.ldI32o2(shp, i, 8, $shclass_iprobs)) | 0) > 0; i++)
        continue;
        ...
        if (Hallucination() && !cptr.ldI32(program_state)) {
            ...
            for (num = 0; num < 13; num++)
                if (cptr.ldI32o2(shtypes, num, 112, $shclass_prob) == 0)
                    break;
            if (num > 0) {
                nlp = cptr.ldPtro2(shtypes, (rng_log_enabled() ? (rng_log_set_caller(__s_shknam_c, 884, __s_shkname), rn2(num)) : rn2(num)), 112, $shclass_shknms);
