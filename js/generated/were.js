...
export function were_change(mon) {
    ...
    if (((cptr.ldU64o((cptr.ldPtro(mon, FLD.monst_data)), FLD.permonst_mflags2) & 8n) != 0n)) {
        if (!Protection_from_shape_changers() && !(rng_log_enabled() ? (rng_log_set_caller(__sl0, 17, __sl1), rn2(night() ? (cptr.ldI32o(flags, FLD.flag_moonphase) == NHM.FULL_MOON ? 3 : 30) : (cptr.ldI32o(flags, FLD.flag_moonphase) == NHM.FULL_MOON ? 10 : 50))) : rn2(night() ? (cptr.ldI32o(flags, FLD.flag_moonphase) == NHM.FULL_MOON ? 3 : 30) : (cptr.ldI32o(flags, FLD.flag_moonphase) == NHM.FULL_MOON ? 10 : 50)))) {
            new_were(mon);
...
export function you_unwere(purify) {
    ...
    if (!Unchanging() && ((cptr.ldU64o((cptr.ldPtro(gy, FLD.instance_globals_y_youmonst + FLD.monst_data)), FLD.permonst_mflags2) & 4n) != 0n) && !monster_nearby() && (!controllable_poly || !paranoid_query(schar((((cptr.ldI32o(flags, FLD.flag_paranoia_bits) & NHM.PARANOID_WERECHANGE) >>> 0) != 0)), __sl13)))
        rehumanize();
