...
export function create_drawbridge(x, y, dir, flag) {
    ...
    let y2;
    let horiz;
    let lava = schar((cptr.ld1so3(svl, x, $sizeof_rm_x21, y, $sizeof_rm, $instance_globals_saved_l_level + $rm_typ) == NHC.LAVAPOOL));
    x2 = x;
    y2 = y;
    switch (dir) {
        case NHM.DB_NORTH:
...
function do_entity(etmp) {
    ...
    if (e_missed(etmp, 0)) {
        ...
        if (cptr.ld1so(crm, $rm_typ) == NHC.DRAWBRIDGE_DOWN) {
            ...
            pline(__s_s_crushed_underneath_the_drawbridge, E_phrase(etmp, __s_are));
...
export function close_drawbridge(x, y) {
    ...
    do_entity(cptr.add(cptr.add(go, $instance_globals_o_occupants), 0, $sizeof_entity));
    set_entity(x2.v, y2.v, cptr.add(cptr.add(go, $instance_globals_o_occupants), 1, $sizeof_entity));
...
export function destroy_drawbridge(x, y) {
    ...
    for (i = (rng_log_enabled() ? (rng_log_set_caller(__s_dbridge_c, 949, __s_destroy_drawbridge), rn2(6)) : rn2(6)); i > 0; --i) {
        otmp = mksobj_at(NHC.IRON_CHAIN, i16(((rng_log_enabled() ? (rng_log_set_caller(__s_dbridge_c, 953, __s_destroy_drawbridge), rn2(2)) : rn2(2)) ? x : x2.v)), i16(((rng_log_enabled() ? (rng_log_set_caller(__s_dbridge_c, 953, __s_destroy_drawbridge), rn2(2)) : rn2(2)) ? y : y2.v)), 1, 0);
