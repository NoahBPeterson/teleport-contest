// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/src/mdlib.c
// Input sha256: 9c9a6197c31db73a0947da889020ab354b269be7bb3701d5aa733ecd932d6686
// Transpiler: tools/c2js c2js emit v1+batch

import * as cptr from '../cptr.js';
import * as NHC from './nhconst.js';
import * as NHM from './nhmacro.js';
import * as FLD from './nhfield.js';
import { datamodel, eos, nh_snprintf } from './hacklib.js';
import { dupstr } from './alloc.js';
import { free_nomakedefs, populate_nomakedefs } from './date.js';

// string literals (C char* uses decay to CPtr into these static buffers)
const __sl0 = cptr.lit("");
const __sl1 = cptr.lit("Excalibur");
const __sl2 = cptr.lit("Stormbringer");
const __sl3 = cptr.lit("Mjollnir");
const __sl4 = cptr.lit("Cleaver");
const __sl5 = cptr.lit("Grimtooth");
const __sl6 = cptr.lit("Orcrist");
const __sl7 = cptr.lit("Sting");
const __sl8 = cptr.lit("Magicbane");
const __sl9 = cptr.lit("Frost Brand");
const __sl10 = cptr.lit("Fire Brand");
const __sl11 = cptr.lit("Dragonbane");
const __sl12 = cptr.lit("Demonbane");
const __sl13 = cptr.lit("Werebane");
const __sl14 = cptr.lit("Grayswandir");
const __sl15 = cptr.lit("Giantslayer");
const __sl16 = cptr.lit("Ogresmasher");
const __sl17 = cptr.lit("Trollsbane");
const __sl18 = cptr.lit("Vorpal Blade");
const __sl19 = cptr.lit("Snickersnee");
const __sl20 = cptr.lit("Sunsword");
const __sl21 = cptr.lit("The Orb of Detection");
const __sl22 = cptr.lit("The Heart of Ahriman");
const __sl23 = cptr.lit("The Sceptre of Might");
const __sl24 = cptr.lit("The Staff of Aesculapius");
const __sl25 = cptr.lit("The Magic Mirror of Merlin");
const __sl26 = cptr.lit("The Eyes of the Overworld");
const __sl27 = cptr.lit("The Mitre of Holiness");
const __sl28 = cptr.lit("The Longbow of Diana");
const __sl29 = cptr.lit("The Master Key of Thievery");
const __sl30 = cptr.lit("The Tsurugi of Muramasa");
const __sl31 = cptr.lit("The Platinum Yendorian Express Card");
const __sl32 = cptr.lit("The Orb of Fate");
const __sl33 = cptr.lit("The Eye of the Aethiopica");
const __sl34 = cptr.lit("tty");
const __sl35 = cptr.lit("traditional text with optional line-drawing");
const __sl36 = cptr.lit("soundlib_nosound");
const __sl37 = cptr.lit("%d%s%d%s%d");
const __sl38 = cptr.lit("version_id_string");
const __sl39 = cptr.lit("%s NetHack%s Version %s%s - last %s %s.");
const __sl40 = cptr.lit("MacOS");
const __sl41 = cptr.lit(".");
const __sl42 = cptr.lit("revision");
const __sl43 = cptr.lit("build");
const __sl44 = cptr.lit("bannerc_string");
const __sl45 = cptr.lit("         Version %s %s%s, %s %s.");
const __sl46 = cptr.lit("revised");
const __sl47 = cptr.lit("built");
const __sl48 = cptr.lit("save and bones files accepted from version");
const __sl49 = cptr.lit(" %d.%d.%d only");
const __sl50 = cptr.lit("color");
const __sl51 = cptr.lit("data file compression");
const __sl52 = cptr.lit("deferred handling of hangup signal");
const __sl53 = cptr.lit("insurance files for recovering from crashes");
const __sl54 = cptr.lit("live logging support");
const __sl55 = cptr.lit("log file");
const __sl56 = cptr.lit("extended log file");
const __sl57 = cptr.lit("errors and warnings log file");
const __sl58 = cptr.lit("mail daemon");
const __sl59 = cptr.lit("news file");
const __sl60 = cptr.lit("internal pager used for viewing help files");
const __sl61 = cptr.lit("pattern matching via :PATMATCH:");
const __sl62 = cptr.lit("pseudo random numbers generated by ISAAC64");
const __sl63 = cptr.lit("strong PRNG seed from /dev/random");
const __sl64 = cptr.lit("restore saved games via menu");
const __sl65 = cptr.lit("screen clipping");
const __sl66 = cptr.lit("shell command");
const __sl67 = cptr.lit("traditional status display");
const __sl68 = cptr.lit("status via windowport with highlighting");
const __sl69 = cptr.lit("suspend command");
const __sl70 = cptr.lit("terminal info library");
const __sl71 = cptr.lit("system configuration at run-time");
const __sl72 = cptr.lit("show stack trace on error");
const __sl73 = cptr.lit("launch browser to report issues");
const __sl74 = cptr.lit("and basic NetHack features");
const __sl75 = cptr.lit("%s");
const __sl76 = cptr.lit(" ");
const __sl77 = cptr.lit("%sNetHack version %d.%d.%d%s\n");
const __sl78 = cptr.lit("Options compiled into this edition:");
const __sl79 = cptr.lit(" data model,");
const __sl80 = cptr.lit(",");
const __sl81 = cptr.lit("Supported windowing system%s:");
const __sl82 = cptr.lit("s");
const __sl83 = cptr.lit("\"%s\"");
const __sl84 = cptr.lit(" (%s)");
const __sl85 = cptr.lit(" and");
const __sl86 = cptr.lit(", and");
const __sl87 = cptr.lit("with a default of \"%s\".");
const __sl88 = cptr.lit("Supported soundlib%s:");
const __sl89 = cptr.lit("soundlib_");
const __sl90 = cptr.lit("NetHack 5.0.* uses the 'Lua' interpreter to process some data:");
const __sl91 = cptr.lit("    :LUACOPYRIGHT:");
const __sl92 = cptr.lit("    \"Permission is hereby granted, free of charge, to any person obtaining");
const __sl93 = cptr.lit("     a copy of this software and associated documentation files (the ");
const __sl94 = cptr.lit("     \"Software\"), to deal in the Software without restriction including");
const __sl95 = cptr.lit("     without limitation the rights to use, copy, modify, merge, publish,");
const __sl96 = cptr.lit("     distribute, sublicense, and/or sell copies of the Software, and to ");
const __sl97 = cptr.lit("     permit persons to whom the Software is furnished to do so, subject to");
const __sl98 = cptr.lit("     the following conditions:");
const __sl99 = cptr.lit("     The above copyright notice and this permission notice shall be");
const __sl100 = cptr.lit("     included in all copies or substantial portions of the Software.\"");

/** C ref: mdlib.c:10 — char *[35] */
const artifact_names = cptr.alloc(35 * 8);
cptr.stPtro(artifact_names, 0, __sl0);
cptr.stPtro(artifact_names, 8, __sl1);
cptr.stPtro(artifact_names, 16, __sl2);
cptr.stPtro(artifact_names, 24, __sl3);
cptr.stPtro(artifact_names, 32, __sl4);
cptr.stPtro(artifact_names, 40, __sl5);
cptr.stPtro(artifact_names, 48, __sl6);
cptr.stPtro(artifact_names, 56, __sl7);
cptr.stPtro(artifact_names, 64, __sl8);
cptr.stPtro(artifact_names, 72, __sl9);
cptr.stPtro(artifact_names, 80, __sl10);
cptr.stPtro(artifact_names, 88, __sl11);
cptr.stPtro(artifact_names, 96, __sl12);
cptr.stPtro(artifact_names, 104, __sl13);
cptr.stPtro(artifact_names, 112, __sl14);
cptr.stPtro(artifact_names, 120, __sl15);
cptr.stPtro(artifact_names, 128, __sl16);
cptr.stPtro(artifact_names, 136, __sl17);
cptr.stPtro(artifact_names, 144, __sl18);
cptr.stPtro(artifact_names, 152, __sl19);
cptr.stPtro(artifact_names, 160, __sl20);
cptr.stPtro(artifact_names, 168, __sl21);
cptr.stPtro(artifact_names, 176, __sl22);
cptr.stPtro(artifact_names, 184, __sl23);
cptr.stPtro(artifact_names, 192, __sl24);
cptr.stPtro(artifact_names, 200, __sl25);
cptr.stPtro(artifact_names, 208, __sl26);
cptr.stPtro(artifact_names, 216, __sl27);
cptr.stPtro(artifact_names, 224, __sl28);
cptr.stPtro(artifact_names, 232, __sl29);
cptr.stPtro(artifact_names, 240, __sl30);
cptr.stPtro(artifact_names, 248, __sl31);
cptr.stPtro(artifact_names, 256, __sl32);
cptr.stPtro(artifact_names, 264, __sl33);
cptr.stPtro(artifact_names, 272, null);

/** C ref: mdlib.c:56 — signed char */
let date_via_env = 0;

/** C ref: mdlib.c:95 — int */
let idxopttext = 0;

/** C ref: mdlib.c:95 — int */
let done_runtime_opt_init_once = 0;

/** C ref: mdlib.c:97 — char *[60] */
const opttext = cptr.alloc(60 * 8);
cptr.stPtro(opttext, 0, null);

/** C ref: mdlib.c:102 — char[256] */
const optbuf = new Uint8Array(256);

/** C ref: mdlib.c:103 — struct version_info */
let version = cptr.alloc(24);

/** C ref: mdlib.c:104 — char[5] */
const opt_indent = cptr.bytes("    ");

/** C ref: mdlib.c:106 — struct win_information { id, name, valid } (memory model v0.5) */

/** C ref: mdlib.c:112 — struct win_information[2] */
const window_opts = cptr.alloc(2 * 24);
cptr.stPtro(window_opts, 0, __sl34);
cptr.stPtro(window_opts, 0 + FLD.win_information_name, __sl35);
cptr.st1o(window_opts, 0 + FLD.win_information_valid, 1);
cptr.stPtro(window_opts, 24, null);
cptr.stPtro(window_opts, 24 + FLD.win_information_name, null);
cptr.st1o(window_opts, 24 + FLD.win_information_valid, 0);

/** C ref: mdlib.c:169 — struct soundlib_information { id, text_id, Url, valid } (memory model v0.5) */

/** C ref: mdlib.c:184 — struct soundlib_information[2] */
const soundlib_opts = cptr.alloc(2 * 32);
cptr.stI32o(soundlib_opts, 0, NHC.soundlib_nosound);
cptr.stPtro(soundlib_opts, 0 + FLD.soundlib_information_text_id, __sl36);
cptr.stPtro(soundlib_opts, 0 + FLD.soundlib_information_Url, __sl0);
cptr.st1o(soundlib_opts, 0 + FLD.soundlib_information_valid, 0);
cptr.stI32o(soundlib_opts, 32, 0);
cptr.stPtro(soundlib_opts, 32 + FLD.soundlib_information_text_id, null);
cptr.stPtro(soundlib_opts, 32 + FLD.soundlib_information_Url, null);
cptr.st1o(soundlib_opts, 32 + FLD.soundlib_information_valid, 0);

/** C ref: mdlib.c:236 @returns {CLongLong} */
export function md_ignored_features() {
    return 1074266112n;
}

/** C ref: mdlib.c:248 */
function make_version() {
    let i;
    cptr.stU64(version, 83886080n);
    cptr.stU64o(version, FLD.version_info_feature_set, 393280n);
    for (i = 1; cptr.ldPtro(artifact_names, i, 8); i++)
        continue;
    cptr.stU64o(version, FLD.version_info_entity_count, BigInt.asUintN(64, BigInt(((i - 1) | 0))));
    i = NHC.NUM_OBJECTS;
    cptr.stU64o(version, FLD.version_info_entity_count, (cptr.ldU64o(version, FLD.version_info_entity_count) << 12n) | BigInt.asUintN(64, BigInt(i)));
    i = NHC.NUMMONS;
    cptr.stU64o(version, FLD.version_info_entity_count, (cptr.ldU64o(version, FLD.version_info_entity_count) << 12n) | BigInt.asUintN(64, BigInt(i)));
    return;
}

/** C ref: mdlib.c:300 — @param {CPtr} outbuf @param {CPtr} delim @returns {CPtr} */
export function mdlib_version_string(outbuf, delim) {
    void cptr.sprintf(outbuf, __sl37, NHM.VERSION_MAJOR, delim, NHM.VERSION_MINOR, delim, NHM.PATCHLEVEL);
    return outbuf;
}

/** C ref: mdlib.c:316 — @param {CPtr} outbuf @param {CLongLong} bufsz @param {CPtr} build_date @returns {CPtr} */
export function version_id_string(outbuf, bufsz, build_date) {
    let subbuf = new Uint8Array(64);
    let versbuf = new Uint8Array(64);
    let statusbuf = new Uint8Array(64);
    cptr.st1o(cptr.decay(statusbuf), 0, 0, 1);
    cptr.st1o(cptr.decay(subbuf), 0, 0, 1);
    nh_snprintf(__sl38, 342, outbuf, bufsz, __sl39, __sl40, cptr.decay(subbuf), mdlib_version_string(cptr.decay(versbuf), __sl41), cptr.decay(statusbuf), date_via_env ? __sl42 : __sl43, build_date);
    return outbuf;
}

/** C ref: mdlib.c:349 — @param {CPtr} outbuf @param {CLongLong} bufsz @param {CPtr} build_date @returns {CPtr} */
export function bannerc_string(outbuf, bufsz, build_date) {
    let subbuf = new Uint8Array(64);
    let versbuf = new Uint8Array(64);
    cptr.st1o(cptr.decay(subbuf), 0, 0, 1);
    nh_snprintf(__sl44, 368, outbuf, bufsz, __sl45, mdlib_version_string(cptr.decay(versbuf), __sl41), __sl40, cptr.decay(subbuf), date_via_env ? __sl46 : __sl47, build_date);
    return outbuf;
}

/** C ref: mdlib.c:390 — char[256] */
const save_bones_compat_buf = new Uint8Array(256);

/** C ref: mdlib.c:393 */
function build_savebones_compat_string() {
    void cptr.strcpy(cptr.decay(save_bones_compat_buf), __sl48);
    void cptr.sprintf(eos(cptr.decay(save_bones_compat_buf)), __sl49, NHM.VERSION_MAJOR, NHM.VERSION_MINOR, NHM.PATCHLEVEL);
}

/** C ref: mdlib.c:417 — char *[26] */
const build_opts = cptr.alloc(26 * 8);
cptr.stPtro(build_opts, 0, __sl50);
cptr.stPtro(build_opts, 8, __sl51);
cptr.stPtro(build_opts, 16, __sl52);
cptr.stPtro(build_opts, 24, __sl53);
cptr.stPtro(build_opts, 32, __sl54);
cptr.stPtro(build_opts, 40, __sl55);
cptr.stPtro(build_opts, 48, __sl56);
cptr.stPtro(build_opts, 56, __sl57);
cptr.stPtro(build_opts, 64, __sl58);
cptr.stPtro(build_opts, 72, __sl59);
cptr.stPtro(build_opts, 80, __sl60);
cptr.stPtro(build_opts, 88, __sl61);
cptr.stPtro(build_opts, 96, __sl62);
cptr.stPtro(build_opts, 104, __sl63);
cptr.stPtro(build_opts, 112, __sl64);
cptr.stPtro(build_opts, 120, __sl65);
cptr.stPtro(build_opts, 128, __sl66);
cptr.stPtro(build_opts, 136, __sl67);
cptr.stPtro(build_opts, 144, __sl68);
cptr.stPtro(build_opts, 152, __sl69);
cptr.stPtro(build_opts, 160, __sl70);
cptr.stPtro(build_opts, 168, __sl71);
cptr.stPtro(build_opts, 176, __sl72);
cptr.stPtro(build_opts, 184, __sl73);
cptr.stPtro(build_opts, 192, cptr.decay(save_bones_compat_buf));
cptr.stPtro(build_opts, 200, __sl74);

/** C ref: mdlib.c:602 @returns {CInt} */
function count_and_validate_winopts() {
    let i;
    let cnt = 0;
    for (i = 0; i < ((2 - 1) | 0); i++) {
        ++cnt;
        cptr.st1o2(window_opts, i, 24, FLD.win_information_valid, 1);
    }
    return cnt;
}

/** C ref: mdlib.c:627 @returns {CInt} */
function count_and_validate_soundlibopts() {
    let i;
    let cnt = 0;
    for (i = 0; i < ((2 - 1) | 0); i++) {
        ++cnt;
        cptr.st1o2(soundlib_opts, i, 32, FLD.soundlib_information_valid, 1);
    }
    return cnt;
}

/** C ref: mdlib.c:641 — @param {CPtr} str @param {CPtr} length_p */
function opt_out_words(str, length_p) {
    let word;
    while (cptr.ld1s(str)) {
        word = cptr.strchr(str, 32);
        if (word)
            cptr.st1(word, 0);
        if (((cptr.ldI32(length_p) + Number(BigInt.asIntN(32, cptr.strlen(str)))) | 0) > 75) {
            (void ((idxopttext < 60) ? (cptr.stPtro(opttext, idxopttext++, dupstr(cptr.decay(optbuf)), 8)) : null));
            void cptr.sprintf(cptr.decay(optbuf), __sl75, cptr.decay(opt_indent)), cptr.stI32(length_p, Number(BigInt.asIntN(32, cptr.strlen(cptr.decay(opt_indent)))));
        } else {
            void cptr.sprintf(eos(cptr.decay(optbuf)), __sl76), (cptr.stI32(length_p, cptr.ldI32(length_p) + 1)) - (1);
        }
        void cptr.sprintf(eos(cptr.decay(optbuf)), __sl75, str), cptr.stI32(length_p, (cptr.ldI32(length_p) + Number(BigInt.asIntN(32, cptr.strlen(str)))) | 0);
        str = cptr.add(str, BigInt.asUintN(64, cptr.strlen(str) + BigInt.asUintN(64, BigInt((word ? 1 : 0)))));
    }
}

const __static_build_options_lua_info = cptr.alloc(15 * 8);
cptr.stPtro(__static_build_options_lua_info, 0, __sl0);
cptr.stPtro(__static_build_options_lua_info, 8, __sl90);
cptr.stPtro(__static_build_options_lua_info, 16, __sl0);
cptr.stPtro(__static_build_options_lua_info, 24, __sl91);
cptr.stPtro(__static_build_options_lua_info, 32, __sl0);
cptr.stPtro(__static_build_options_lua_info, 40, (__sl92));
cptr.stPtro(__static_build_options_lua_info, 48, __sl93);
cptr.stPtro(__static_build_options_lua_info, 56, __sl94);
cptr.stPtro(__static_build_options_lua_info, 64, __sl95);
cptr.stPtro(__static_build_options_lua_info, 72, __sl96);
cptr.stPtro(__static_build_options_lua_info, 80, __sl97);
cptr.stPtro(__static_build_options_lua_info, 88, __sl98);
cptr.stPtro(__static_build_options_lua_info, 96, __sl99);
cptr.stPtro(__static_build_options_lua_info, 104, __sl100);
cptr.stPtro(__static_build_options_lua_info, 112, null); /** C ref: mdlib.c:800 — char *[15] (function-static) */

/** C ref: mdlib.c:669 */
function build_options() {
    let buf = new Uint8Array(256);
    let i;
    let length = cptr.box(0);
    let winsyscnt;
    let cnt = 0;
    let defwinsys = __sl34;
    let soundlibcnt;
    build_savebones_compat_string();
    (void ((idxopttext < 60) ? (cptr.stPtro(opttext, idxopttext++, dupstr(cptr.decay(optbuf)), 8)) : null));
    void cptr.sprintf(cptr.decay(optbuf), __sl77, cptr.decay(opt_indent), NHM.VERSION_MAJOR, NHM.VERSION_MINOR, NHM.PATCHLEVEL, __sl0);
    (void ((idxopttext < 60) ? (cptr.stPtro(opttext, idxopttext++, dupstr(cptr.decay(optbuf)), 8)) : null));
    void cptr.sprintf(cptr.decay(optbuf), __sl78);
    (void ((idxopttext < 60) ? (cptr.stPtro(opttext, idxopttext++, dupstr(cptr.decay(optbuf)), 8)) : null));
    cptr.st1o(cptr.decay(optbuf), 0, 0, 1);
    length.v = 81;
    void cptr.strcat(cptr.strcpy(cptr.decay(buf), datamodel(0)), __sl79);
    opt_out_words(cptr.decay(buf), length);
    for (i = 0; i < 26; i++) {
        void cptr.strcat(cptr.strcpy(cptr.decay(buf), cptr.ldPtro(build_opts, i, 8)), (i < ((26 - 1) | 0)) ? __sl80 : __sl41);
        opt_out_words(cptr.decay(buf), length);
    }
    (void ((idxopttext < 60) ? (cptr.stPtro(opttext, idxopttext++, dupstr(cptr.decay(optbuf)), 8)) : null));
    cptr.st1o(cptr.decay(optbuf), 0, 0, 1);
    winsyscnt = count_and_validate_winopts();
    (void ((idxopttext < 60) ? (cptr.stPtro(opttext, idxopttext++, dupstr(cptr.decay(optbuf)), 8)) : null));
    void cptr.sprintf(cptr.decay(optbuf), __sl81, (winsyscnt > 1) ? __sl82 : __sl0);
    (void ((idxopttext < 60) ? (cptr.stPtro(opttext, idxopttext++, dupstr(cptr.decay(optbuf)), 8)) : null));
    cptr.st1o(cptr.decay(optbuf), 0, 0, 1);
    length.v = 81;
    for (i = 0; i < ((2 - 1) | 0); i++) {
        if (!cptr.ld1so2(window_opts, i, 24, FLD.win_information_valid))
            continue;
        void cptr.sprintf(cptr.decay(buf), __sl83, cptr.ldPtro(window_opts, i, 24));
        if (strcmp(cptr.ldPtro2(window_opts, i, 24, FLD.win_information_name), cptr.ldPtro(window_opts, i, 24)))
            void cptr.sprintf(eos(cptr.decay(buf)), __sl84, cptr.ldPtro2(window_opts, i, 24, FLD.win_information_name));
        void cptr.strcat(cptr.decay(buf), (winsyscnt == 1) ? __sl41 : ((winsyscnt == 2 && cnt == 0) ? __sl85 : ((cnt == ((winsyscnt - 2) | 0)) ? __sl86 : __sl80)));
        opt_out_words(cptr.decay(buf), length);
        cnt++;
    }
    if (cnt > 1) {
        void cptr.sprintf(cptr.decay(buf), __sl87, defwinsys);
        opt_out_words(cptr.decay(buf), length);
    }
    cnt = 0;
    (void ((idxopttext < 60) ? (cptr.stPtro(opttext, idxopttext++, dupstr(cptr.decay(optbuf)), 8)) : null));
    cptr.st1o(cptr.decay(optbuf), 0, 0, 1);
    soundlibcnt = count_and_validate_soundlibopts();
    (void ((idxopttext < 60) ? (cptr.stPtro(opttext, idxopttext++, dupstr(cptr.decay(optbuf)), 8)) : null));
    void cptr.sprintf(cptr.decay(optbuf), __sl88, (soundlibcnt > 1) ? __sl82 : __sl0);
    (void ((idxopttext < 60) ? (cptr.stPtro(opttext, idxopttext++, dupstr(cptr.decay(optbuf)), 8)) : null));
    cptr.st1o(cptr.decay(optbuf), 0, 0, 1);
    length.v = 81;
    for (i = 0; i < ((2 - 1) | 0); i++) {
        let soundlib;
        if (!cptr.ld1so2(soundlib_opts, i, 32, FLD.soundlib_information_valid))
            continue;
        soundlib = cptr.ldPtro2(soundlib_opts, i, 32, FLD.soundlib_information_text_id);
        if (!cptr.strncmp(soundlib, __sl89, 9n))
            soundlib = cptr.add(soundlib, 9);
        void cptr.sprintf(cptr.decay(buf), __sl83, soundlib);
        void cptr.strcat(cptr.decay(buf), (soundlibcnt == 1 || cnt == ((soundlibcnt - 1) | 0)) ? __sl41 : ((soundlibcnt == 2 && cnt == 0) ? __sl85 : ((cnt == ((soundlibcnt - 2) | 0)) ? __sl86 : __sl80)));
        opt_out_words(cptr.decay(buf), length);
        cnt++;
    }
    (void ((idxopttext < 60) ? (cptr.stPtro(opttext, idxopttext++, dupstr(cptr.decay(optbuf)), 8)) : null));
    cptr.st1o(cptr.decay(optbuf), 0, 0, 1);
    {
        for (i = 0; cptr.ldPtro(__static_build_options_lua_info, i, 8); ++i) {
            (void ((idxopttext < 60) ? (cptr.stPtro(opttext, idxopttext++, dupstr(cptr.ldPtro(__static_build_options_lua_info, i, 8)), 8)) : null));
        }
    }
    (void ((idxopttext < 60) ? (cptr.stPtro(opttext, idxopttext++, dupstr(__sl0), 8)) : null));
    return;
}

/** C ref: mdlib.c:835 */
export function runtime_info_init() {
    if (!done_runtime_opt_init_once) {
        done_runtime_opt_init_once = 1;
        build_savebones_compat_string();
        make_version();
        populate_nomakedefs(version);
        idxopttext = 0;
        build_options();
    }
}

/** C ref: mdlib.c:849 — @param {CPtr} rtcontext @returns {CPtr} */
export function do_runtime_info(rtcontext) {
    let retval = null;
    if (!done_runtime_opt_init_once)
        runtime_info_init();
    if (idxopttext && rtcontext)
        if (cptr.ldI32(rtcontext) >= 0 && cptr.ldI32(rtcontext) < 60) {
            retval = cptr.ldPtro(opttext, cptr.ldI32(rtcontext), 8);
            cptr.stI32(rtcontext, (cptr.ldI32(rtcontext) + 1) | 0);
        }
    return retval;
}

/** C ref: mdlib.c:864 */
export function release_runtime_info() {
    while (idxopttext > 0) {
        --idxopttext;
        cptr.free(cptr.ldPtro(opttext, idxopttext, 8)), cptr.stPtro(opttext, idxopttext, null, 8);
    }
    done_runtime_opt_init_once = 0;
    free_nomakedefs();
}

// --- BEGIN c2js reset block (tools/c2js/resetify.mjs) — do not edit ---
// 13 bindings: 1 rebound+refilled, 3 rebound, 9 refilled.
// S/P are supplied by js/generated/__reset.js so this module needs no new import.
let __c2js_rs = null;
export function __captureState(S) { __c2js_rs = [S(artifact_names), S(date_via_env), S(idxopttext), S(done_runtime_opt_init_once), S(opttext), S(optbuf), S(version), S(opt_indent), S(window_opts), S(soundlib_opts), S(save_bones_compat_buf), S(build_opts), S(__static_build_options_lua_info)]; }
export function __resetState(P) {
    const r = __c2js_rs;
    if (r === null) throw new Error("mdlib.js: __resetState before __captureState");
    P(r[0]);
    date_via_env = P(r[1]);
    idxopttext = P(r[2]);
    done_runtime_opt_init_once = P(r[3]);
    P(r[4]);
    P(r[5]);
    version = P(r[6]);
    P(r[7]);
    P(r[8]);
    P(r[9]);
    P(r[10]);
    P(r[11]);
    P(r[12]);
}
// --- END c2js reset block ---
