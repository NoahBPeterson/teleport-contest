// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/win/tty/topl.c
// Input sha256: d355faebfb41d9e61e784f1ef29050a55c5ca45cd066f2f47688c6b7b7cee566
// Transpiler: tools/c2js c2js emit v1+batch

import { i16, schar, uchar } from '../cmachine.js';
import * as cptr from '../cptr.js';
import * as NHM from './nhmacro.js';
import { BASE_WINDOW, defmorestr, docorner, end_glyphout, g_putch, ttyDisplay, tty_clear_nhwindow, tty_curs, wins } from './wintty.js';
import { WIN_MESSAGE, flags, gs, gt, iflags, program_state, quitchars, svx, yn_number } from './decl.js';
import { windowprocs } from './windows.js';
import { morc, xwaitforspace } from './getline.js';
import { backsp, cl_end, home, nomux_putch, standoutbeg, standoutend, tty_nhbell } from './termcap.js';
import { alloc } from './alloc.js';
import { panic } from './end.js';
import { custompline, dumplogmsg, nhassert_failed } from './pline.js';
import { digit, eos, lowc } from './hacklib.js';
import { key2txt, readchar } from './cmd.js';
import { erase_char } from './unixtty.js';

// string literals (C char* uses decay to CPtr into these static buffers)
const __sl0 = cptr.lit("Message History");
const __sl1 = cptr.lit("");
const __sl2 = cptr.lit("\x1b ");
const __sl3 = cptr.lit("You die");
const __sl4 = cptr.lit("  ");
const __sl5 = cptr.lit("Putsym window MESSAGE nonexistent");
const __sl6 = cptr.lit("ttyDisplay->curx > 0");
const __sl7 = cptr.lit("/Users/noahpeterson/Documents/Projects/teleport-contest-research/original-contest-to-fork/nethack-c/recorder/win/tty/topl.c");
const __sl8 = cptr.lit("\b \b");
const __sl9 = cptr.lit(" [%s]");
const __sl10 = cptr.lit(" (%c)");
const __sl11 = cptr.lit(" ");
const __sl12 = cptr.lit("%s");
const __sl13 = cptr.lit("%s ");
const __sl14 = cptr.lit("#");
const __sl15 = cptr.lit("#%ld");
const __sl16 = cptr.lit("%s%s");
const __sl17 = cptr.lit("ttyDisplay == NULL || ttyDisplay->toplin != TOPLINE_NEED_MORE");

/** C ref: topl.c:20 @returns {CInt} */
export function tty_doprev_message() {
    let cw = cptr.ldPtr(cptr.add(wins, WIN_MESSAGE.v, 8));
    let prevmsg_win;
    let i;
    if ((cptr.ld1s(cptr.add(iflags, 176)) != 115) && !cptr.ldI32(cptr.add(ttyDisplay, 36)) ? 1 : 0) {
        if (cptr.ld1s(cptr.add(iflags, 176)) == 102) {
            prevmsg_win = (cptr.ldPtr(cptr.add(windowprocs, 104)))(NHM.NHW_MENU);
            (cptr.ldPtr(cptr.add(windowprocs, 144)))(prevmsg_win, 0, __sl0);
            (cptr.ldPtr(cptr.add(windowprocs, 144)))(prevmsg_win, 0, __sl1);
            cptr.stI64(cptr.add(cw, 56), cptr.ldI64(cptr.add(cw, 48)));
            i = Number(BigInt.asIntN(32, cptr.ldI64(cptr.add(cw, 56))));
            do {
                if (cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), i, 8)) && strcmp(cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), i, 8)), __sl1) ? 1 : 0)
                    (cptr.ldPtr(cptr.add(windowprocs, 144)))(prevmsg_win, 0, cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), i, 8)));
                i = Number(BigInt.asIntN(32, (BigInt(((i + 1) | 0)) % cptr.ldI64(cptr.add(cw, 16)))));
            } while (BigInt(i) != cptr.ldI64(cptr.add(cw, 56)));
            (cptr.ldPtr(cptr.add(windowprocs, 144)))(prevmsg_win, 0, cptr.add(gt, 26));
            (cptr.ldPtr(cptr.add(windowprocs, 120)))(prevmsg_win, 1);
            (cptr.ldPtr(cptr.add(windowprocs, 128)))(prevmsg_win);
        } else if (cptr.ld1s(cptr.add(iflags, 176)) == 99) {
            do {
                morc.v = 0;
                if (cptr.ldI64(cptr.add(cw, 56)) == cptr.ldI64(cptr.add(cw, 48))) {
                    cptr.st1(cptr.add(ttyDisplay, 48), 16);
                    redotoplin(cptr.add(gt, 26));
                    (cptr.stI64(cptr.add(cw, 56), cptr.ldI64(cptr.add(cw, 56)) + -1n)) - (-1n);
                    if (cptr.ldI64(cptr.add(cw, 56)) < 0n)
                        cptr.stI64(cptr.add(cw, 56), BigInt.asIntN(64, cptr.ldI64(cptr.add(cw, 16)) - 1n));
                    if (!cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), cptr.ldI64(cptr.add(cw, 56)), 8)))
                        cptr.stI64(cptr.add(cw, 56), cptr.ldI64(cptr.add(cw, 48)));
                } else if (cptr.ldI64(cptr.add(cw, 56)) == (BigInt.asIntN(64, cptr.ldI64(cptr.add(cw, 48)) - 1n))) {
                    cptr.st1(cptr.add(ttyDisplay, 48), 16);
                    redotoplin(cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), cptr.ldI64(cptr.add(cw, 56)), 8)));
                    (cptr.stI64(cptr.add(cw, 56), cptr.ldI64(cptr.add(cw, 56)) + -1n)) - (-1n);
                    if (cptr.ldI64(cptr.add(cw, 56)) < 0n)
                        cptr.stI64(cptr.add(cw, 56), BigInt.asIntN(64, cptr.ldI64(cptr.add(cw, 16)) - 1n));
                    if (!cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), cptr.ldI64(cptr.add(cw, 56)), 8)))
                        cptr.stI64(cptr.add(cw, 56), cptr.ldI64(cptr.add(cw, 48)));
                } else {
                    prevmsg_win = (cptr.ldPtr(cptr.add(windowprocs, 104)))(NHM.NHW_MENU);
                    (cptr.ldPtr(cptr.add(windowprocs, 144)))(prevmsg_win, 0, __sl0);
                    (cptr.ldPtr(cptr.add(windowprocs, 144)))(prevmsg_win, 0, __sl1);
                    cptr.stI64(cptr.add(cw, 56), cptr.ldI64(cptr.add(cw, 48)));
                    i = Number(BigInt.asIntN(32, cptr.ldI64(cptr.add(cw, 56))));
                    do {
                        if (cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), i, 8)) && strcmp(cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), i, 8)), __sl1) ? 1 : 0)
                            (cptr.ldPtr(cptr.add(windowprocs, 144)))(prevmsg_win, 0, cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), i, 8)));
                        i = Number(BigInt.asIntN(32, (BigInt(((i + 1) | 0)) % cptr.ldI64(cptr.add(cw, 16)))));
                    } while (BigInt(i) != cptr.ldI64(cptr.add(cw, 56)));
                    (cptr.ldPtr(cptr.add(windowprocs, 144)))(prevmsg_win, 0, cptr.add(gt, 26));
                    (cptr.ldPtr(cptr.add(windowprocs, 120)))(prevmsg_win, 1);
                    (cptr.ldPtr(cptr.add(windowprocs, 128)))(prevmsg_win);
                }
            } while (morc.v == 16);
            cptr.st1(cptr.add(ttyDisplay, 48), 0);
        } else {
            morc.v = 0;
            prevmsg_win = (cptr.ldPtr(cptr.add(windowprocs, 104)))(NHM.NHW_MENU);
            (cptr.ldPtr(cptr.add(windowprocs, 144)))(prevmsg_win, 0, __sl0);
            (cptr.ldPtr(cptr.add(windowprocs, 144)))(prevmsg_win, 0, __sl1);
            (cptr.ldPtr(cptr.add(windowprocs, 144)))(prevmsg_win, 0, cptr.add(gt, 26));
            cptr.stI64(cptr.add(cw, 56), BigInt.asIntN(64, cptr.ldI64(cptr.add(cw, 48)) - 1n));
            if (cptr.ldI64(cptr.add(cw, 56)) < 0n)
                cptr.stI64(cptr.add(cw, 56), BigInt.asIntN(64, cptr.ldI64(cptr.add(cw, 16)) - 1n));
            do {
                (cptr.ldPtr(cptr.add(windowprocs, 144)))(prevmsg_win, 0, cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), cptr.ldI64(cptr.add(cw, 56)), 8)));
                (cptr.stI64(cptr.add(cw, 56), cptr.ldI64(cptr.add(cw, 56)) + -1n)) - (-1n);
                if (cptr.ldI64(cptr.add(cw, 56)) < 0n)
                    cptr.stI64(cptr.add(cw, 56), BigInt.asIntN(64, cptr.ldI64(cptr.add(cw, 16)) - 1n));
                if (!cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), cptr.ldI64(cptr.add(cw, 56)), 8)))
                    cptr.stI64(cptr.add(cw, 56), cptr.ldI64(cptr.add(cw, 48)));
            } while (cptr.ldI64(cptr.add(cw, 56)) != cptr.ldI64(cptr.add(cw, 48)));
            (cptr.ldPtr(cptr.add(windowprocs, 120)))(prevmsg_win, 1);
            (cptr.ldPtr(cptr.add(windowprocs, 128)))(prevmsg_win);
            cptr.stI64(cptr.add(cw, 56), cptr.ldI64(cptr.add(cw, 48)));
            cptr.st1(cptr.add(ttyDisplay, 48), 0);
        }
    } else if (cptr.ld1s(cptr.add(iflags, 176)) == 115) {
        cptr.st1(cptr.add(ttyDisplay, 48), 16);
        do {
            morc.v = 0;
            if (cptr.ldI64(cptr.add(cw, 56)) == cptr.ldI64(cptr.add(cw, 48)))
                redotoplin(cptr.add(gt, 26));
            else if (cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), cptr.ldI64(cptr.add(cw, 56)), 8)))
                redotoplin(cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), cptr.ldI64(cptr.add(cw, 56)), 8)));
            (cptr.stI64(cptr.add(cw, 56), cptr.ldI64(cptr.add(cw, 56)) + -1n)) - (-1n);
            if (cptr.ldI64(cptr.add(cw, 56)) < 0n)
                cptr.stI64(cptr.add(cw, 56), BigInt.asIntN(64, cptr.ldI64(cptr.add(cw, 16)) - 1n));
            if (!cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), cptr.ldI64(cptr.add(cw, 56)), 8)))
                cptr.stI64(cptr.add(cw, 56), cptr.ldI64(cptr.add(cw, 48)));
        } while (morc.v == 16);
        cptr.st1(cptr.add(ttyDisplay, 48), 0);
    }
    return 0;
}

/** C ref: topl.c:122 — @param {CPtr} str */
function redotoplin(str) {
    let otoplin = cptr.ldI32(cptr.add(ttyDisplay, 24));
    home();
    if (!cptr.ldI32(cptr.add(ttyDisplay, 52))) {
        if (cptr.ldI32(cptr.add(ttyDisplay, 56)) && (cptr.ld1s(str) & 128) ? 1 : 0) {
            g_putch(cptr.ld1s(cptr.postinc(() => str, (v) => { str = v; })));
            (cptr.stI16(cptr.add(ttyDisplay, 4), cptr.ldI16(cptr.add(ttyDisplay, 4)) + 1)) - (1);
        }
        end_glyphout();
    }
    putsyms(str);
    cl_end();
    cptr.stI32(cptr.add(ttyDisplay, 24), NHM.TOPLINE_NEED_MORE);
    if (cptr.ldI16(cptr.add(ttyDisplay, 6)) && otoplin != NHM.TOPLINE_SPECIAL_PROMPT ? 1 : 0)
        more();
}

/** C ref: topl.c:145 — @param {CPtr} str */
export function show_topl(str) {
    let cw = cptr.ldPtr(cptr.add(wins, WIN_MESSAGE.v, 8));
    if ((cptr.ldI32(cw) & 5) != NHM.WIN_STOP) {
        cptr.stI32(cw, cptr.ldI32(cw) & -6);
        if (cptr.ldI16(cptr.add(ttyDisplay, 6)) && cptr.ldI32(cptr.add(ttyDisplay, 24)) == NHM.TOPLINE_NON_EMPTY ? 1 : 0)
            tty_clear_nhwindow(WIN_MESSAGE.v);
        cptr.stI64(cptr.add(cw, 32), cptr.stI64(cptr.add(cw, 40), 0n));
        home();
        cl_end();
        addtopl(str);
        if (cptr.ldI16(cptr.add(ttyDisplay, 6)) && cptr.ldI32(cptr.add(ttyDisplay, 24)) != NHM.TOPLINE_SPECIAL_PROMPT ? 1 : 0)
            cptr.stI32(cptr.add(ttyDisplay, 24), NHM.TOPLINE_NON_EMPTY);
    }
}

/** C ref: topl.c:170 */
export function remember_topl() {
    let cw = cptr.ldPtr(cptr.add(wins, WIN_MESSAGE.v, 8));
    let idx = Number(BigInt.asIntN(32, cptr.ldI64(cptr.add(cw, 48))));
    let len = Number(BigInt.asUintN(32, BigInt.asUintN(64, cptr.strlen(cptr.add(gt, 26)) + 1n)));
    if ((cptr.ldI32(cw) & NHM.WIN_LOCKHISTORY) || !cptr.ld1s(cptr.add(gt, 26)) ? 1 : 0)
        return;
    if (len > cptr.ldI16(cptr.add(cptr.ldPtr(cptr.add(cw, 72)), idx, 2))) {
        if (cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), idx, 8)))
            cptr.free(cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), idx, 8)));
        len = (len + ((8 - ((len & 7) >>> 0)) >>> 0)) | 0;
        cptr.stPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), idx, 8), alloc(len));
        cptr.stI16(cptr.add(cptr.ldPtr(cptr.add(cw, 72)), idx, 2), i16(len));
    }
    void cptr.strcpy(cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), idx, 8)), cptr.add(gt, 26));
    if (!cptr.ldI32(cptr.add(program_state, 60))) {
        cptr.st1(cptr.add(gt, 26), 0);
        cptr.stI64(cptr.add(cw, 56), cptr.stI64(cptr.add(cw, 48), BigInt(((idx + 1) | 0)) % cptr.ldI64(cptr.add(cw, 16))));
    }
}

/** C ref: topl.c:194 — @param {CPtr} s */
export function addtopl(s) {
    let cw = cptr.ldPtr(cptr.add(wins, WIN_MESSAGE.v, 8));
    tty_curs(BASE_WINDOW, Number(BigInt.asIntN(32, BigInt.asIntN(64, cptr.ldI64(cptr.add(cw, 32)) + 1n))), Number(BigInt.asIntN(32, cptr.ldI64(cptr.add(cw, 40)))));
    putsyms(s);
    cl_end();
    cptr.stI32(cptr.add(ttyDisplay, 24), NHM.TOPLINE_NEED_MORE);
}

/** C ref: topl.c:205 */
export function more() {
    let cw = cptr.ldPtr(cptr.add(wins, WIN_MESSAGE.v, 8));
    if (cptr.ld1s(cptr.add(iflags, 15)))
        return;
    if (cptr.ldI32(cptr.add(ttyDisplay, 32)))
        return;
    (cptr.stI32(cptr.add(ttyDisplay, 32), cptr.ldI32(cptr.add(ttyDisplay, 32)) + 1)) - (1);
    if (cptr.ldI32(cptr.add(ttyDisplay, 24))) {
        tty_curs(BASE_WINDOW, Number(BigInt.asIntN(32, BigInt.asIntN(64, cptr.ldI64(cptr.add(cw, 32)) + 1n))), Number(BigInt.asIntN(32, cptr.ldI64(cptr.add(cw, 40)))));
        if (cptr.ldI64(cptr.add(cw, 32)) >= BigInt(((cptr.ldI32(cptr.add(gt, 356)) - 8) | 0)))
            topl_putsym(10);
    }
    if (cptr.ld1s(cptr.add(flags, 44)))
        standoutbeg();
    putsyms(cptr.decay(defmorestr));
    if (cptr.ld1s(cptr.add(flags, 44)))
        standoutend();
    xwaitforspace(__sl2);
    if (morc.v == 27) {
        if (!(cptr.ldI32(cw) & NHM.WIN_NOSTOP))
            cptr.stI32(cw, cptr.ldI32(cw) | NHM.WIN_STOP);
    }
    if (cptr.ldI32(cptr.add(ttyDisplay, 24)) && cptr.ldI64(cptr.add(cw, 40)) ? 1 : 0) {
        docorner(1, Number(BigInt.asIntN(32, BigInt.asIntN(64, cptr.ldI64(cptr.add(cw, 40)) + 1n))), 0);
        cptr.stI64(cptr.add(cw, 32), cptr.stI64(cptr.add(cw, 40), 0n));
        home();
    } else if (morc.v == 27) {
        cptr.stI64(cptr.add(cw, 32), cptr.stI64(cptr.add(cw, 40), 0n));
        home();
        cl_end();
    }
    cptr.stI32(cptr.add(ttyDisplay, 24), NHM.TOPLINE_EMPTY);
    cptr.stI32(cptr.add(ttyDisplay, 32), 0);
}

/** C ref: topl.c:251 — @param {CPtr} bp */
export function update_topl(bp) {
    let tl;
    let otl;
    let n0;
    let notdied = 1;
    let cw = cptr.ldPtr(cptr.add(wins, WIN_MESSAGE.v, 8));
    let skip = schar(((cptr.ldI32(cw) & 5) == NHM.WIN_STOP));
    n0 = Number(BigInt.asIntN(32, cptr.strlen(bp)));
    if ((((cptr.ldI32(cptr.add(ttyDisplay, 24)) == NHM.TOPLINE_NEED_MORE || skip ? 1 : 0) && cptr.ldI64(cptr.add(cw, 40)) == 0n ? 1 : 0) && ((((n0 + Number(BigInt.asIntN(32, cptr.strlen(cptr.add(gt, 26))))) | 0) + 3) | 0) < ((cptr.ldI32(cptr.add(gt, 356)) - 8) | 0) ? 1 : 0) && (notdied = cptr.strncmp(bp, __sl3, 7n)) != 0 ? 1 : 0) {
        void cptr.strcat(cptr.add(gt, 26), __sl4);
        void cptr.strcat(cptr.add(gt, 26), bp);
        cptr.stI64(cptr.add(cw, 32), cptr.ldI64(cptr.add(cw, 32)) + 2n);
        if (!skip)
            addtopl(bp);
        return;
    } else if (!skip) {
        if (cptr.ldI32(cptr.add(ttyDisplay, 24)) == NHM.TOPLINE_NEED_MORE) {
            more();
        } else if (cptr.ldI64(cptr.add(cw, 40))) {
            docorner(1, Number(BigInt.asIntN(32, BigInt.asIntN(64, cptr.ldI64(cptr.add(cw, 40)) + 1n))), 0);
            cptr.stI64(cptr.add(cw, 32), cptr.stI64(cptr.add(cw, 40), 0n));
        }
    }
    remember_topl();
    void __builtin___strncpy_chk(cptr.add(gt, 26), bp, 300n, __builtin_object_size(cptr.add(gt, 26), 1));
    cptr.st1(cptr.add(cptr.add(gt, 26), 299, 1), 0);
    for (tl = cptr.add(gt, 26); n0 >= cptr.ldI32(cptr.add(gt, 356)); ) {
        otl = tl;
        for (tl = cptr.add(tl, (cptr.ldI32(cptr.add(gt, 356)) - 1) | 0); !cptr.eq(tl, otl); tl = cptr.add(tl, -1))
            if (cptr.ld1s(tl) == 32)
                break;
        if (cptr.eq(tl, otl)) {
            tl = cptr.strchr(otl, 32);
            if (!tl)
                break;
        }
        cptr.st1(cptr.postinc(() => tl, (v) => { tl = v; }), 10);
        n0 = Number(BigInt.asIntN(32, cptr.strlen(tl)));
    }
    if (!notdied)
        cptr.stI32(cw, cptr.ldI32(cw) & -2), skip = 0;
    if (!skip)
        redotoplin(cptr.add(gt, 26));
}

/** C ref: topl.c:305 — @param {CInt} c */
function topl_putsym(c) {
    let cw = cptr.ldPtr(cptr.add(wins, WIN_MESSAGE.v, 8));
    if (cw === null)
        panic(__sl5);
    switch (c) {
        case 8:
        if (cptr.ldI16(cptr.add(ttyDisplay, 4)) == 0 && cptr.ldI16(cptr.add(ttyDisplay, 6)) > 0 ? 1 : 0)
            tty_curs(BASE_WINDOW, cptr.ldI32(cptr.add(gt, 356)), (cptr.ldI16(cptr.add(ttyDisplay, 6)) - 1) | 0);
        backsp();
        void ((!!(cptr.ldI16(cptr.add(ttyDisplay, 4)) > 0)) || (nhassert_failed(__sl6, __sl7, 317), 0) ? 1 : 0);
        (cptr.stI16(cptr.add(ttyDisplay, 4), cptr.ldI16(cptr.add(ttyDisplay, 4)) + -1)) - (-1);
        cptr.stI64(cptr.add(cw, 32), BigInt(cptr.ldI16(cptr.add(ttyDisplay, 4))));
        return;
        case 10:
        cl_end();
        cptr.stI16(cptr.add(ttyDisplay, 4), 0);
        (cptr.stI16(cptr.add(ttyDisplay, 6), cptr.ldI16(cptr.add(ttyDisplay, 6)) + 1)) - (1);
        cptr.stI64(cptr.add(cw, 40), BigInt(cptr.ldI16(cptr.add(ttyDisplay, 6))));
        break;
        default:
        if (cptr.ldI16(cptr.add(ttyDisplay, 4)) == ((cptr.ldI32(cptr.add(gt, 356)) - 1) | 0))
            topl_putsym(10);
        (cptr.stI16(cptr.add(ttyDisplay, 4), cptr.ldI16(cptr.add(ttyDisplay, 4)) + 1)) - (1);
    }
    cptr.stI64(cptr.add(cw, 32), BigInt(cptr.ldI16(cptr.add(ttyDisplay, 4))));
    if (cptr.ldI64(cptr.add(cw, 32)) == 0n)
        cl_end();
    void putchar(c);
    {
        let svx = cptr.ldI16(cptr.add(ttyDisplay, 4));
        (cptr.stI16(cptr.add(ttyDisplay, 4), cptr.ldI16(cptr.add(ttyDisplay, 4)) + -1)) - (-1);
        nomux_putch(c);
        cptr.stI16(cptr.add(ttyDisplay, 4), i16(svx));
    }
}

/** C ref: topl.c:355 — @param {CPtr} str */
export function putsyms(str) {
    while (cptr.ld1s(str))
        topl_putsym(cptr.ld1s(cptr.postinc(() => str, (v) => { str = v; })));
}

/** C ref: topl.c:362 — @param {CInt} n */
function removetopl(n) {
    while (n-- > 0)
        putsyms(__sl8);
}

/** C ref: topl.c:373 — @param {CPtr} query @param {CPtr} resp @param {CInt} def @returns {CInt} */
export function tty_yn_function(query, resp, def) {
    let q;
    let rtmp = new Uint8Array(40);
    let digit_ok;
    let allow_num;
    let preserve_case = 0;
    let cw = cptr.ldPtr(cptr.add(wins, WIN_MESSAGE.v, 8));
    let doprev = 0;
    let prompt = new Uint8Array(256);
    __lbl_clean_up: {
        yn_number.v = 0n;
        if (cptr.ldI32(cptr.add(ttyDisplay, 24)) == NHM.TOPLINE_NEED_MORE && (cptr.ldI32(cw) & 5) != NHM.WIN_STOP ? 1 : 0)
            more();
        cptr.stI32(cw, cptr.ldI32(cw) & -6);
        cptr.stI32(cptr.add(ttyDisplay, 24), NHM.TOPLINE_SPECIAL_PROMPT);
        (cptr.stI32(cptr.add(ttyDisplay, 36), cptr.ldI32(cptr.add(ttyDisplay, 36)) + 1)) - (1);
        if (resp) {
            let rb;
            let respbuf = new Uint8Array(128);
            allow_num = schar((cptr.strchr(resp, 35) !== null));
            void cptr.strcpy(cptr.decay(respbuf), resp);
            for (rb = cptr.decay(respbuf); cptr.ld1s(rb); rb = cptr.add(rb, 1))
                if (65 <= cptr.ld1s(rb) && cptr.ld1s(rb) <= 90 ? 1 : 0) {
                    preserve_case = 1;
                    break;
                }
            if ((rb = cptr.strchr(cptr.decay(respbuf), 27)) !== null)
                cptr.st1(rb, 0);
            void __builtin___strncpy_chk(cptr.decay(prompt), query, 127n, __builtin_object_size(cptr.decay(prompt), 1));
            cptr.st1(cptr.add(cptr.decay(prompt), 127, 1), 0);
            void cptr.sprintf(eos(cptr.decay(prompt)), __sl9, cptr.decay(respbuf));
            if (def)
                void cptr.sprintf(eos(cptr.decay(prompt)), __sl10, def);
            void cptr.strcat(cptr.decay(prompt), __sl11);
            custompline(6, __sl12, cptr.decay(prompt));
        } else {
            void cptr.sprintf(cptr.decay(prompt), __sl13, query);
            custompline(6, __sl12, cptr.decay(prompt));
            q = readchar();
            break __lbl_clean_up;
        }
        do {
            q = readchar();
            if (!preserve_case)
                q = lowc(q);
            if (q == 16) {
                if (cptr.ld1s(cptr.add(iflags, 176)) != 115) {
                    let sav = cptr.ldI32(cptr.add(ttyDisplay, 36));
                    cptr.stI32(cptr.add(ttyDisplay, 36), 0);
                    void tty_doprev_message();
                    cptr.stI32(cptr.add(ttyDisplay, 36), sav);
                    tty_clear_nhwindow(WIN_MESSAGE.v);
                    cptr.stI64(cptr.add(cw, 56), cptr.ldI64(cptr.add(cw, 48)));
                    addtopl(cptr.decay(prompt));
                } else {
                    if (!doprev)
                        void tty_doprev_message();
                    void tty_doprev_message();
                    doprev = 1;
                }
                q = 0;
                continue;
            } else if (doprev) {
                tty_clear_nhwindow(WIN_MESSAGE.v);
                cptr.stI64(cptr.add(cw, 56), cptr.ldI64(cptr.add(cw, 48)));
                doprev = 0;
                addtopl(cptr.decay(prompt));
                q = 0;
                continue;
            }
            digit_ok = schar((allow_num && digit(q) ? 1 : 0));
            if (q == 27) {
                if (cptr.strchr(resp, 113))
                    q = 113;
                else if (cptr.strchr(resp, 110))
                    q = 110;
                else
                    q = def;
                break;
            } else if (cptr.strchr(cptr.decay(quitchars), q)) {
                q = def;
                break;
            }
            if (!cptr.strchr(resp, q) && !digit_ok ? 1 : 0) {
                tty_nhbell();
                q = 0;
            } else if (q == 35 || digit_ok ? 1 : 0) {
                let z;
                let digit_string = new Uint8Array(2);
                let n_len = 0;
                let value = 0n;
                addtopl(__sl14), n_len++;
                cptr.st1(cptr.add(cptr.decay(digit_string), 1, 1), 0);
                if (q != 35) {
                    cptr.st1(cptr.add(cptr.decay(digit_string), 0, 1), q);
                    addtopl(cptr.decay(digit_string)), n_len++;
                    value = BigInt(((q - 48) | 0));
                    q = 35;
                }
                do {
                    z = readchar();
                    if (!preserve_case)
                        z = lowc(z);
                    if (digit(z)) {
                        let dgt = BigInt(((z - 48) | 0));
                        value = (((value) < 922337203685477580n || ((value) == 922337203685477580n && (dgt) <= 7n ? 1 : 0) ? 1 : 0) ? BigInt.asIntN(64, BigInt.asIntN(64, (value) * 10n) + (dgt)) : -1n);
                        if (value < 0n)
                            break;
                        cptr.st1(cptr.add(cptr.decay(digit_string), 0, 1), z);
                        addtopl(cptr.decay(digit_string)), n_len++;
                    } else if (z == 121 || cptr.strchr(cptr.decay(quitchars), z) ? 1 : 0) {
                        if (z == 27)
                            value = -1n;
                        z = 10;
                    } else if (z == erase_char || z == 8 ? 1 : 0) {
                        if (n_len <= 1) {
                            value = -1n;
                            break;
                        } else {
                            value /= 10n;
                            removetopl(1), n_len--;
                        }
                    } else {
                        value = -1n;
                        tty_nhbell();
                        break;
                    }
                } while (z != 10);
                if (value > 0n)
                    yn_number.v = value;
                else if (value == 0n)
                    q = 110;
                else {
                    removetopl(n_len), n_len = 0;
                    q = 0;
                }
            }
        } while (!q);
    }
    if (yn_number.v)
        void cptr.sprintf(cptr.decay(rtmp), __sl15, yn_number.v);
    else
        void key2txt(uchar(q), cptr.decay(rtmp));
    void cptr.sprintf(cptr.add(gt, 26), __sl16, cptr.decay(prompt), cptr.decay(rtmp));
    dumplogmsg(cptr.add(gt, 26));
    (cptr.stI32(cptr.add(ttyDisplay, 36), cptr.ldI32(cptr.add(ttyDisplay, 36)) + -1)) - (-1);
    cptr.stI32(cptr.add(ttyDisplay, 24), NHM.TOPLINE_NON_EMPTY);
    if (cptr.ldI32(cptr.add(ttyDisplay, 40)))
        (cptr.stI32(cptr.add(ttyDisplay, 40), cptr.ldI32(cptr.add(ttyDisplay, 40)) + -1)) - (-1);
    if (cptr.ldI64(cptr.add(cptr.ldPtr(cptr.add(wins, WIN_MESSAGE.v, 8)), 40)))
        tty_clear_nhwindow(WIN_MESSAGE.v);
    return q;
}

/** C ref: topl.c:562 — char ** */
let snapshot_mesgs = null;

/** C ref: topl.c:567 — @param {CInt} purge */
function msghistory_snapshot(purge) {
    let mesg;
    let i;
    let inidx;
    let outidx;
    let cw;
    if (WIN_MESSAGE.v == -1 || !cptr.ldPtr(cptr.add(wins, WIN_MESSAGE.v, 8)) ? 1 : 0)
        return;
    cw = cptr.ldPtr(cptr.add(wins, WIN_MESSAGE.v, 8));
    remember_topl();
    if (!purge)
        cptr.stI32(cw, cptr.ldI32(cw) | NHM.WIN_LOCKHISTORY);
    snapshot_mesgs = alloc(Number(BigInt.asUintN(32, BigInt.asUintN(64, BigInt.asUintN(64, (BigInt.asIntN(64, cptr.ldI64(cptr.add(cw, 16)) + 1n))) * 8n))));
    outidx = 0;
    inidx = Number(BigInt.asIntN(32, cptr.ldI64(cptr.add(cw, 48))));
    for (i = 0; BigInt(i) < cptr.ldI64(cptr.add(cw, 16)); ++i) {
        cptr.stPtr(cptr.add(snapshot_mesgs, i, 8), null);
        mesg = cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), inidx, 8));
        if (mesg && cptr.ld1s(mesg) ? 1 : 0) {
            cptr.stPtr(cptr.add(snapshot_mesgs, outidx++, 8), mesg);
            if (purge) {
                cptr.stPtr(cptr.add(cptr.ldPtr(cptr.add(cw, 80)), inidx, 8), null);
                cptr.stI16(cptr.add(cptr.ldPtr(cptr.add(cw, 72)), inidx, 2), 0);
            }
        }
        inidx = Number(BigInt.asIntN(32, (BigInt(((inidx + 1) | 0)) % cptr.ldI64(cptr.add(cw, 16)))));
    }
    cptr.stPtr(cptr.add(snapshot_mesgs, cptr.ldI64(cptr.add(cw, 16)), 8), null);
    if (purge)
        cptr.stI64(cptr.add(cw, 56), cptr.stI64(cptr.add(cw, 48), 0n));
}

/** C ref: topl.c:613 — @param {CInt} purged */
function free_msghistory_snapshot(purged) {
    if (snapshot_mesgs) {
        if (purged) {
            let i;
            for (i = 0; cptr.ldPtr(cptr.add(snapshot_mesgs, i, 8)); ++i)
                cptr.free(cptr.ldPtr(cptr.add(snapshot_mesgs, i, 8)));
        }
        cptr.free(snapshot_mesgs), snapshot_mesgs = null;
        if (!purged)
            cptr.stI32(cptr.ldPtr(cptr.add(wins, WIN_MESSAGE.v, 8)), cptr.ldI32(cptr.ldPtr(cptr.add(wins, WIN_MESSAGE.v, 8))) & -3);
    }
}

let __static_tty_getmsghistory_nxtidx = 0; /** C ref: topl.c:647 — int (function-static) */

/** C ref: topl.c:645 — @param {CInt} init @returns {CPtr} */
export function tty_getmsghistory(init) {
    let nextmesg;
    let result = null;
    if (init) {
        msghistory_snapshot(0);
        __static_tty_getmsghistory_nxtidx = 0;
    }
    if (snapshot_mesgs) {
        nextmesg = cptr.ldPtr(cptr.add(snapshot_mesgs, __static_tty_getmsghistory_nxtidx++, 8));
        if (nextmesg) {
            result = nextmesg;
        } else {
            free_msghistory_snapshot(0);
        }
    }
    return result;
}

let __static_tty_putmsghistory_initd = 0; /** C ref: topl.c:687 — signed char (function-static) */

/** C ref: topl.c:685 — @param {CPtr} msg @param {CInt} restoring_msghist */
export function tty_putmsghistory(msg, restoring_msghist) {
    let idx;
    if (restoring_msghist && !__static_tty_putmsghistory_initd ? 1 : 0) {
        msghistory_snapshot(1);
        __static_tty_putmsghistory_initd = 1;
        cptr.stI32(cptr.add(gs, 980), 0);
    }
    if (msg) {
        if (ttyDisplay && cptr.ldI32(cptr.add(ttyDisplay, 24)) == NHM.TOPLINE_NEED_MORE ? 1 : 0)
            cptr.stI32(cptr.add(ttyDisplay, 24), NHM.TOPLINE_NON_EMPTY);
        remember_topl();
        void cptr.strcpy(cptr.add(gt, 26), msg);
        dumplogmsg(cptr.add(gt, 26));
    } else if (snapshot_mesgs) {
        void ((!!(cptr.eq(ttyDisplay, (null)) || cptr.ldI32(cptr.add(ttyDisplay, 24)) != NHM.TOPLINE_NEED_MORE ? 1 : 0)) || (nhassert_failed(__sl17, __sl7, 720), 0) ? 1 : 0);
        for (idx = 0; cptr.ldPtr(cptr.add(snapshot_mesgs, idx, 8)); ++idx) {
            remember_topl();
            void cptr.strcpy(cptr.add(gt, 26), cptr.ldPtr(cptr.add(snapshot_mesgs, idx, 8)));
            dumplogmsg(cptr.add(gt, 26));
        }
        free_msghistory_snapshot(1);
        __static_tty_putmsghistory_initd = 0;
    }
}
