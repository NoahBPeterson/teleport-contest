...
function mkshop() {
    ...
    if (i < 0) {
        let j;
        for (j = (rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 193, __s_mkshop), rnd(100)) : rnd(100)), i = 0; (j = (j - cptr.ldI32o2(shtypes, i, 112, $shclass_prob)) | 0) > 0; i++)
            continue;
...
function pick_room(strict) {
    ...
    for (sroom = cptr.add(svr, (rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 225, __s_pick_room), rn2(cptr.ldI32o(svn, $instance_globals_saved_n_nroom))) : rn2(cptr.ldI32o(svn, $instance_globals_saved_n_nroom))), 224); i--; sroom = cptr.add(sroom, 1, 224)) {
    ...
    for (sx = cptr.ldI16(sroom); sx <= cptr.ldI16o(sroom, $mkroom_hx); sx++)
        for (sy = cptr.ldI16o(sroom, $mkroom_ly); sy <= cptr.ldI16o(sroom, $mkroom_hy); sy++) {
            ...
            switch (type) {
                ...
                goldlim = (goldlim - i) | 0;
                void mkgold(BigInt((((rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 381, __s_fill_zoo), rn2(i)) : rn2(i)) + 10) | 0)), i16(sx), i16(sy));
                break;
                case NHC.MORGUE:
                ...
                if (!(rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 386, __s_fill_zoo), rn2(10)) : rn2(10)))
                    void mksobj_at(((rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 387, __s_fill_zoo), rn2(3)) : rn2(3))) ? NHC.LARGE_BOX : NHC.CHEST, i16(sx), i16(sy), 1, 0);
                ...
                if (!(rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 398, __s_fill_zoo), rn2(20)) : rn2(20)))
                    void mksobj_at(((rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 399, __s_fill_zoo), rn2(3)) : rn2(3))) ? NHC.LARGE_BOX : NHC.CHEST, i16(sx), i16(sy), 1, 0);
                break;
                case NHC.COCKNEST:
...
export function mkundead(mm, revive_corpses, mm_flags) {
    let cnt = (((((level_difficulty() + 1) | 0) / 10) | 0) + (rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 461, __s_mkundead), rnd(5)) : rnd(5))) | 0;
    let mdat;
    let otmp;
    let cc = cptr.alloc(4);
    while (cnt--) {
...
function mkswamp() {
    ...
    let rmno;
    for (i = 0; i < 5; i++) {
        sroom = cptr.add(svr, (rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 538, __s_mkswamp), rn2(cptr.ldI32o(svn, $instance_globals_saved_n_nroom))) : rn2(cptr.ldI32o(svn, $instance_globals_saved_n_nroom))), 224);
                ...
                if (!(cptr.ldPtro3(svl, sx, 168, sy, 8, $instance_globals_saved_l_level + $dlevel_t_objects) !== null) && !(cptr.ldPtro3(svl, sx, 168, sy, 8, $instance_globals_saved_l_level + $dlevel_t_monsters) !== null) && !t_at(sx, sy) && !nexttodoor(sx, sy)) {
                    if (((sx + sy) | 0) % 2) {
                        ...
                        if (!eelct || !(rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 557, __s_mkswamp), rn2(4)) : rn2(4))) {
                            void makemon((rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 559, __s_mkswamp), rn2(5)) : rn2(5)) ? cptr.add(mons, NHC.PM_GIANT_EEL, 96) : ((rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 561, __s_mkswamp), rn2(2)) : rn2(2)) ? cptr.add(mons, NHC.PM_PIRANHA, 96) : cptr.add(mons, NHC.PM_ELECTRIC_EEL, 96)), sx, sy, NHM.NO_MM_FLAGS);
                            eelct++;
                        }
...
export function courtmon() {
    let i = ((rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 785, __s_courtmon), rn2(60)) : rn2(60)) + (rng_log_enabled() ? (rng_log_set_caller(__s_mkroom_c, 785, __s_courtmon), rn2(Math.imul(3, level_difficulty()))) : rn2(Math.imul(3, level_difficulty())))) | 0;
    if (i > 100)
