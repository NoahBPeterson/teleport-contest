...
function explosionmask(m, adtyp, olet) {
    ...
    if (cptr.eq(m, cptr.add(gy, $instance_globals_y_youmonst))) {
        ...
        switch (adtyp) {
            ...
            break;
            case NHM.AD_DISN:
            if ((olet == NHC.WAND_CLASS) ? ((is_undead(cptr.ldPtro(m, $monst_data)) || cptr.eq((cptr.ldPtro(m, $monst_data)), cptr.add(mons, NHC.PM_MANES, 96)) || (is_golem(cptr.ldPtro(m, $monst_data)) || cptr.ld1so((cptr.ldPtro(m, $monst_data)), $permonst_mlet) == NHC.S_VORTEX)) || is_demon(cptr.ldPtro(m, $monst_data)) || is_vampshifter(m) ? 1 : 0) : !!Resists_Elem(m, NHC.DISINT_RES))
                res = NHC.EXPL_MON;
            break;
            case NHM.AD_ELEC:
                ...
                if (DEADMONSTER(mtmp)) {
                    ...
                    if (!cptr.ld1so(svc, $context_info_mon_moving)) {
                        ...
                        if (cansee(cptr.ldI16o(mtmp, $monst_mx), cptr.ldI16o(mtmp, $monst_my)) || canspotmon(mtmp))
                            pline(__sl35, Monnam(mtmp), xkflg ? __sl36 : ((is_undead(cptr.ldPtro(mtmp, $monst_data)) || cptr.eq((cptr.ldPtro(mtmp, $monst_data)), cptr.add(mons, NHC.PM_MANES, 96)) || (is_golem(cptr.ldPtro(mtmp, $monst_data)) || cptr.ld1so((cptr.ldPtro(mtmp, $monst_data)), $permonst_mlet) == NHC.S_VORTEX)) ? __sl37 : __sl38));
