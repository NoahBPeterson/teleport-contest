...
export function mattacku(mtmp) {
    ...
    if (U_AP_TYPE() == NHC.M_AP_OBJECT && !range2.v && foundyou.v && !(cptr.ldI32o(u, $you_uswallow) & 1)) {
        if (!canspotmon(mtmp))
            map_invisible(cptr.ldI16o(mtmp, $monst_mx), cptr.ldI16o(mtmp, $monst_my));
        if (!youseeit.v)
            pline(__sl74, cptr.ldPtro(c_common_strings, $c_common_strings_c_Something), (likes_gold(cptr.ldPtro(mtmp, $monst_data)) && cptr.ldI32o(gy, $instance_globals_y_youmonst + $monst_mappearance) == NHC.GOLD_PIECE) ? __sl75 : __sl76);
        else
            ...
            if (!range2.v && (!(cptr.ldPtro((mtmp), $monst_mw)) || (cptr.ldI32o(mtmp, $monst_mconf) & 1) | 0 || Conflict() || !touch_petrifies(cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)))) {
                if (foundyou.v) {
                    if (tmp > (j = (rng_log_enabled() ? (rng_log_set_caller(__sl22, 806, __sl62), rnd((20 + i) | 0)) : rnd((20 + i) | 0)))) {
                        if (unsolid(cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)) && failed_grab(mtmp, cptr.add(gy, $instance_globals_y_youmonst), mattk))
                            continue;
...
function gulpmu(mtmp, mattk) {
    ...
    if (!(cptr.ldI32o(u, $you_uswallow) & 1)) {
        ...
        if (cptr.ldPtro(u, $you_usteed)) {
            ...
            urgent_pline(__sl112, Some_Monnam(mtmp), is_animal(cptr.ldPtro(mtmp, $monst_data)) ? __sl40 : (is_whirly(cptr.ldPtro(mtmp, $monst_data)) ? __sl113 : (unsolid(cptr.ldPtro(mtmp, $monst_data)) ? __sl114 : (amorphous(cptr.ldPtro(mtmp, $monst_data)) ? __sl115 : __sl116))), cptr.decay(buf));
    ...
    switch (cptr.ld1uo(mattk, $attack_adtyp)) {
        ...
        if (cptr.eq(cptr.ldPtro(mtmp, $monst_data), cptr.add(mons, NHC.PM_FOG_CLOUD, 96))) {
            You(__sl131, flaming(cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)) ? __sl132 : (Breathless() ? __sl133 : (amphibious(cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)) ? __sl134 : __sl135)));
