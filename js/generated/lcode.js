// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/lib/lua-5.4.8/src/lcode.c
// Input sha256: 2cf13c7f8205e9f07c2ccf1fedc3f0f0a05d238c194b176c72cc22b2cc96210e
// Transpiler: tools/c2js c2js emit v1+batch

import { i16, schar, uchar } from '../cmachine.js';
import * as cptr from '../cptr.js';
import * as NHC from './nhconst.js';
import { luaX_syntaxerror } from './llex.js';
import { luaP_opmodes } from './lopcodes.js';
import { luaM_growaux_ } from './lmem.js';
import { luaY_nvarstack } from './lparser.js';
import { luaH_finishset, luaH_get } from './ltable.js';
import { luaV_equalobj, luaV_flttointeger, luaV_tointegerns } from './lvm.js';
import { luaC_barrier_ } from './lgc.js';
import { luaO_ceillog2, luaO_rawarith } from './lobject.js';

// string literals (C char* uses decay to CPtr into these static buffers)
const __sl0 = cptr.lit("control structure too long");
const __sl1 = cptr.lit("lines");
const __sl2 = cptr.lit("opcodes");
const __sl3 = cptr.lit("function or expression needs too many registers");
const __sl4 = cptr.lit("constants");

/** C ref: lcode.c:47 — @param {CPtr} ls @param {CPtr} msg */
export function luaK_semerror(ls, msg) {
    cptr.stI32(cptr.add(ls, 16), 0);
    luaX_syntaxerror(ls, msg);
}

/** C ref: lcode.c:57 — @param {CPtr} e @param {CPtr} v @returns {CInt} */
function tonumeral(e, v) {
    if ((cptr.ldI32(cptr.add((e), 16)) != cptr.ldI32(cptr.add((e), 20))))
        return 0;
    switch (cptr.ldI32(e)) {
        case NHC.VKINT:
        if (v) {
            let io = (v);
            cptr.stI64(((io)), (cptr.ldI64(cptr.add(e, 8))));
            (cptr.st1(cptr.add((io), 8), 3));
        }
        ;
        return 1;
        case NHC.VKFLT:
        if (v) {
            let io = (v);
            cptr.stF64(((io)), (cptr.ldF64(cptr.add(e, 8))));
            (cptr.st1(cptr.add((io), 8), 19));
        }
        ;
        return 1;
        default:
        return 0;
    }
}

/** C ref: lcode.c:75 — @param {CPtr} fs @param {CPtr} e @returns {CPtr} */
function const2val(fs, e) {
    (void 0);
    return cptr.add(cptr.ldPtr(cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 88))), cptr.ldI32(cptr.add(e, 8)), 24);
}

/** C ref: lcode.c:85 — @param {CPtr} fs @param {CPtr} e @param {CPtr} v @returns {CInt} */
export function luaK_exp2const(fs, e, v) {
    if ((cptr.ldI32(cptr.add((e), 16)) != cptr.ldI32(cptr.add((e), 20))))
        return 0;
    switch (cptr.ldI32(e)) {
        case NHC.VFALSE:
        (cptr.st1(cptr.add((v), 8), 1));
        return 1;
        case NHC.VTRUE:
        (cptr.st1(cptr.add((v), 8), 17));
        return 1;
        case NHC.VNIL:
        (cptr.st1(cptr.add((v), 8), 0));
        return 1;
        case NHC.VKSTR:
        {
            {
                let io = (v);
                let x_ = (cptr.ldPtr(cptr.add(e, 8)));
                cptr.stPtr(((io)), ((((x_)))));
                (cptr.st1(cptr.add((io), 8), uchar((((cptr.ld1u(cptr.add(x_, 8))) | 64)))));
                (void cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 56)), (void 0));
            }
            ;
            return 1;
        }
        case NHC.VCONST:
        {
            {
                let io1 = (v);
                let io2 = (const2val(fs, e));
                cptr.memcpy(io1, io2, 8);
                (cptr.st1(cptr.add((io1), 8), (cptr.ld1u(cptr.add(io2, 8)))));
                (void cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 56)), (void 0));
                (void 0);
            }
            ;
            return 1;
        }
        default:
        return tonumeral(e, v);
    }
}

let __static_previousinstruction_invalidinstruction = cptr.box(~0); /** C ref: lcode.c:118 — unsigned int (function-static, boxed) */

/** C ref: lcode.c:117 — @param {CPtr} fs @returns {CPtr} */
function previousinstruction(fs) {
    if (cptr.ldI32(cptr.add(fs, 32)) > cptr.ldI32(cptr.add(fs, 36)))
        return cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr(fs), 64)), (cptr.ldI32(cptr.add(fs, 32)) - 1) | 0, 4);
    else
        return ((__static_previousinstruction_invalidinstruction));
}

/** C ref: lcode.c:132 — @param {CPtr} fs @param {CInt} from @param {CInt} n */
export function luaK_nil(fs, from, n) {
    let l = (((from + n) | 0) - 1) | 0;
    let previous = previousinstruction(fs);
    if (((((((cptr.ldI32(previous)) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0))) == NHC.OP_LOADNIL) {
        let pfrom = (((((((cptr.ldI32(previous)) >>> 7) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0));
        let pl = (pfrom + ((((((((cptr.ldI32(previous)) >>> 16) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)))) | 0;
        if ((pfrom <= from && from <= ((pl + 1) | 0) ? 1 : 0) || (from <= pfrom && pfrom <= ((l + 1) | 0) ? 1 : 0) ? 1 : 0) {
            if (pfrom < from)
                from = pfrom;
            if (pl > l)
                l = pl;
            (cptr.stI32(previous, (((((cptr.ldI32(previous)) & (~(((~(((~0) << 8) >>> 0)) << 7) >>> 0))) >>> 0) | ((((((from) >>> 0) << 7) >>> 0) & (((~(((~0) << 8) >>> 0)) << 7) >>> 0)) >>> 0)) >>> 0)));
            (cptr.stI32(previous, (((((cptr.ldI32(previous)) & (~(((~(((~0) << 8) >>> 0)) << 16) >>> 0))) >>> 0) | (((((((l - from) | 0) >>> 0) << 16) >>> 0) & (((~(((~0) << 8) >>> 0)) << 16) >>> 0)) >>> 0)) >>> 0)));
            return;
        }
    }
    luaK_codeABCk(fs, NHC.OP_LOADNIL, from, (n - 1) | 0, 0, 0);
}

/** C ref: lcode.c:155 — @param {CPtr} fs @param {CInt} pc @returns {CInt} */
function getjump(fs, pc) {
    let offset = (((((((((cptr.ldI32(cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr(fs), 64)), pc, 4))) >>> 7) & (((~(((~0) << 25) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)) - 16777215) | 0);
    if (offset == -1)
        return -1;
    else
        return (((pc + 1) | 0) + offset) | 0;
}

/** C ref: lcode.c:168 — @param {CPtr} fs @param {CInt} pc @param {CInt} dest */
function fixjump(fs, pc, dest) {
    let jmp = cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr(fs), 64)), pc, 4);
    let offset = (dest - ((pc + 1) | 0)) | 0;
    (void 0);
    if (!(-16777215 <= offset && offset <= 16777216 ? 1 : 0))
        luaX_syntaxerror(cptr.ldPtr(cptr.add(fs, 16)), __sl0);
    (void 0);
    (cptr.stI32(jmp, (((((cptr.ldI32(jmp)) & (~(((~(((~0) << 25) >>> 0)) << 7) >>> 0))) >>> 0) | (((((((((((offset) + 16777215) | 0)) >>> 0))) << 7) >>> 0) & (((~(((~0) << 25) >>> 0)) << 7) >>> 0)) >>> 0)) >>> 0)));
}

/** C ref: lcode.c:182 — @param {CPtr} fs @param {CPtr} l1 @param {CInt} l2 */
export function luaK_concat(fs, l1, l2) {
    if (l2 == -1)
        return;
    else if (cptr.ldI32(l1) == -1)
        cptr.stI32(l1, l2);
    else {
        let list = cptr.ldI32(l1);
        let next;
        while ((next = getjump(fs, list)) != -1)
            list = next;
        fixjump(fs, list, l2);
    }
}

/** C ref: lcode.c:200 — @param {CPtr} fs @returns {CInt} */
export function luaK_jump(fs) {
    return codesJ(fs, NHC.OP_JMP, -1, 0);
}

/** C ref: lcode.c:208 — @param {CPtr} fs @param {CInt} first @param {CInt} nret */
export function luaK_ret(fs, first, nret) {
    let op;
    switch (nret) {
        case 0:
        op = NHC.OP_RETURN0;
        break;
        case 1:
        op = NHC.OP_RETURN1;
        break;
        default:
        op = NHC.OP_RETURN;
        break;
    }
    luaK_codeABCk(fs, op, first, (nret + 1) | 0, 0, 0);
}

/** C ref: lcode.c:223 — @param {CPtr} fs @param {*} op @param {CInt} A @param {CInt} B @param {CInt} C @param {CInt} k @returns {CInt} */
function condjump(fs, op, A, B, C, k) {
    luaK_codeABCk(fs, op, A, B, C, k);
    return luaK_jump(fs);
}

/** C ref: lcode.c:233 — @param {CPtr} fs @returns {CInt} */
export function luaK_getlabel(fs) {
    cptr.stI32(cptr.add(fs, 36), cptr.ldI32(cptr.add(fs, 32)));
    return cptr.ldI32(cptr.add(fs, 32));
}

/** C ref: lcode.c:244 — @param {CPtr} fs @param {CInt} pc @returns {CPtr} */
function getjumpcontrol(fs, pc) {
    let pi = cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr(fs), 64)), pc, 4);
    if (pc >= 1 && (cptr.ld1u(cptr.add(cptr.decay(luaP_opmodes), ((((((cptr.ldI32((cptr.add(pi, -(1), 4)))) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0))), 1)) & 16) ? 1 : 0)
        return cptr.add(pi, -(1), 4);
    else
        return pi;
}

/** C ref: lcode.c:260 — @param {CPtr} fs @param {CInt} node @param {CInt} reg @returns {CInt} */
function patchtestreg(fs, node, reg) {
    let i = getjumpcontrol(fs, node);
    if (((((((cptr.ldI32(i)) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0))) != NHC.OP_TESTSET)
        return 0;
    if (reg != 255 && reg != ((((((((cptr.ldI32(i)) >>> 16) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0))) ? 1 : 0)
        (cptr.stI32(i, (((((cptr.ldI32(i)) & (~(((~(((~0) << 8) >>> 0)) << 7) >>> 0))) >>> 0) | ((((((reg) >>> 0) << 7) >>> 0) & (((~(((~0) << 8) >>> 0)) << 7) >>> 0)) >>> 0)) >>> 0)));
    else {
        cptr.stI32(i, (((((66 | ((((((((((((cptr.ldI32(i)) >>> 16) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)))) >>> 0) << 7) >>> 0)) >>> 0 | 0) >>> 0 | 0) >>> 0 | ((((((((((((cptr.ldI32(i)) >>> 15) & (((~(((~0) << 1) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)))) >>> 0) << 15) >>> 0)) >>> 0));
    }
    return 1;
}

/** C ref: lcode.c:278 — @param {CPtr} fs @param {CInt} list */
function removevalues(fs, list) {
    for (; list != -1; list = getjump(fs, list))
        patchtestreg(fs, list, 255);
}

/** C ref: lcode.c:289 — @param {CPtr} fs @param {CInt} list @param {CInt} vtarget @param {CInt} reg @param {CInt} dtarget */
function patchlistaux(fs, list, vtarget, reg, dtarget) {
    while (list != -1) {
        let next = getjump(fs, list);
        if (patchtestreg(fs, list, reg))
            fixjump(fs, list, vtarget);
        else
            fixjump(fs, list, dtarget);
        list = next;
    }
}

/** C ref: lcode.c:307 — @param {CPtr} fs @param {CInt} list @param {CInt} target */
export function luaK_patchlist(fs, list, target) {
    (void 0);
    patchlistaux(fs, list, target, 255, target);
}

/** C ref: lcode.c:313 — @param {CPtr} fs @param {CInt} list */
export function luaK_patchtohere(fs, list) {
    let hr = luaK_getlabel(fs);
    luaK_patchlist(fs, list, hr);
}

/** C ref: lcode.c:330 — @param {CPtr} fs @param {CPtr} f @param {CInt} line */
function savelineinfo(fs, f, line) {
    let linedif = (line - cptr.ldI32(cptr.add(fs, 40))) | 0;
    let pc = (cptr.ldI32(cptr.add(fs, 32)) - 1) | 0;
    if (Math.abs(linedif) >= 128 || cptr.postinc1(cptr.add(fs, 69)) >= 128 ? 1 : 0) {
        (cptr.stPtr(cptr.add(f, 96), ((luaM_growaux_(cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 56)), cptr.ldPtr(cptr.add(f, 96)), cptr.ldI32(cptr.add(fs, 52)), cptr.add(f, 40), 8, 2147483647, __sl1)))));
        cptr.stI32(cptr.add(cptr.ldPtr(cptr.add(f, 96)), cptr.ldI32(cptr.add(fs, 52)), 8), pc);
        cptr.stI32(cptr.add(cptr.add(cptr.ldPtr(cptr.add(f, 96)), (cptr.stI32(cptr.add(fs, 52), cptr.ldI32(cptr.add(fs, 52)) + 1)) - (1), 8), 4), line);
        linedif = -128;
        cptr.st1(cptr.add(fs, 69), 1);
    }
    (cptr.stPtr(cptr.add(f, 88), ((luaM_growaux_(cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 56)), cptr.ldPtr(cptr.add(f, 88)), pc, cptr.add(f, 28), 1, 2147483647, __sl2)))));
    cptr.st1(cptr.add(cptr.ldPtr(cptr.add(f, 88)), pc), schar(linedif));
    cptr.stI32(cptr.add(fs, 40), line);
}

/** C ref: lcode.c:354 — @param {CPtr} fs */
function removelastlineinfo(fs) {
    let f = cptr.ldPtr(fs);
    let pc = (cptr.ldI32(cptr.add(fs, 32)) - 1) | 0;
    if (cptr.ld1u(cptr.add(cptr.ldPtr(cptr.add(f, 88)), pc)) != -128) {
        cptr.stI32(cptr.add(fs, 40), (cptr.ldI32(cptr.add(fs, 40)) - cptr.ld1u(cptr.add(cptr.ldPtr(cptr.add(f, 88)), pc))) | 0);
        (cptr.st1(cptr.add(fs, 69), cptr.ld1u(cptr.add(fs, 69)) + -1)) - (-1);
    } else {
        (void 0);
        (cptr.stI32(cptr.add(fs, 52), cptr.ldI32(cptr.add(fs, 52)) + -1)) - (-1);
        cptr.st1(cptr.add(fs, 69), 129);
    }
}

/** C ref: lcode.c:373 — @param {CPtr} fs */
function removelastinstruction(fs) {
    removelastlineinfo(fs);
    (cptr.stI32(cptr.add(fs, 32), cptr.ldI32(cptr.add(fs, 32)) + -1)) - (-1);
}

/** C ref: lcode.c:383 — @param {CPtr} fs @param {CUInt} i @returns {CInt} */
export function luaK_code(fs, i) {
    let f = cptr.ldPtr(fs);
    (cptr.stPtr(cptr.add(f, 64), ((luaM_growaux_(cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 56)), cptr.ldPtr(cptr.add(f, 64)), cptr.ldI32(cptr.add(fs, 32)), cptr.add(f, 24), 4, 2147483647, __sl2)))));
    cptr.stI32(cptr.add(cptr.ldPtr(cptr.add(f, 64)), (cptr.stI32(cptr.add(fs, 32), cptr.ldI32(cptr.add(fs, 32)) + 1)) - (1), 4), i);
    savelineinfo(fs, f, cptr.ldI32(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 8)));
    return (cptr.ldI32(cptr.add(fs, 32)) - 1) | 0;
}

/** C ref: lcode.c:398 — @param {CPtr} fs @param {*} o @param {CInt} a @param {CInt} b @param {CInt} c @param {CInt} k @returns {CInt} */
export function luaK_codeABCk(fs, o, a, b, c, k) {
    (void 0);
    (void 0);
    return luaK_code(fs, (((((((((o)) << 0) >>> 0) | ((((a) >>> 0) << 7) >>> 0)) >>> 0 | ((((b) >>> 0) << 16) >>> 0)) >>> 0 | ((((c) >>> 0) << 24) >>> 0)) >>> 0 | ((((k) >>> 0) << 15) >>> 0)) >>> 0));
}

/** C ref: lcode.c:409 — @param {CPtr} fs @param {*} o @param {CInt} a @param {CUInt} bc @returns {CInt} */
export function luaK_codeABx(fs, o, a, bc) {
    (void 0);
    (void 0);
    return luaK_code(fs, (((((((o)) << 0) >>> 0) | ((((a) >>> 0) << 7) >>> 0)) >>> 0 | ((((bc)) << 15) >>> 0)) >>> 0));
}

/** C ref: lcode.c:419 — @param {CPtr} fs @param {*} o @param {CInt} a @param {CInt} bc @returns {CInt} */
function codeAsBx(fs, o, a, bc) {
    let b = ((bc + 65535) | 0) >>> 0;
    (void 0);
    (void 0);
    return luaK_code(fs, (((((((o)) << 0) >>> 0) | ((((a) >>> 0) << 7) >>> 0)) >>> 0 | ((((b)) << 15) >>> 0)) >>> 0));
}

/** C ref: lcode.c:430 — @param {CPtr} fs @param {*} o @param {CInt} sj @param {CInt} k @returns {CInt} */
function codesJ(fs, o, sj, k) {
    let j = ((sj + 16777215) | 0) >>> 0;
    (void 0);
    (void 0);
    return luaK_code(fs, (((((((o)) << 0) >>> 0) | ((((j)) << 7) >>> 0)) >>> 0 | ((((k) >>> 0) << 15) >>> 0)) >>> 0));
}

/** C ref: lcode.c:441 — @param {CPtr} fs @param {CInt} a @returns {CInt} */
function codeextraarg(fs, a) {
    (void 0);
    return luaK_code(fs, ((82 | ((((a) >>> 0) << 7) >>> 0)) >>> 0));
}

/** C ref: lcode.c:452 — @param {CPtr} fs @param {CInt} reg @param {CInt} k @returns {CInt} */
function luaK_codek(fs, reg, k) {
    if (k <= 131071)
        return luaK_codeABx(fs, NHC.OP_LOADK, reg, k >>> 0);
    else {
        let p = luaK_codeABx(fs, NHC.OP_LOADKX, reg, 0);
        codeextraarg(fs, k);
        return p;
    }
}

/** C ref: lcode.c:467 — @param {CPtr} fs @param {CInt} n */
export function luaK_checkstack(fs, n) {
    let newstack = (cptr.ld1u(cptr.add(fs, 68)) + n) | 0;
    if (newstack > cptr.ld1u(cptr.add(cptr.ldPtr(fs), 12))) {
        if (newstack >= 255)
            luaX_syntaxerror(cptr.ldPtr(cptr.add(fs, 16)), __sl3);
        cptr.st1(cptr.add(cptr.ldPtr(fs), 12), (uchar(((newstack)))));
    }
}

/** C ref: lcode.c:481 — @param {CPtr} fs @param {CInt} n */
export function luaK_reserveregs(fs, n) {
    luaK_checkstack(fs, n);
    cptr.st1(cptr.add(fs, 68), cptr.ld1u(cptr.add(fs, 68)) + n);
}

/** C ref: lcode.c:492 — @param {CPtr} fs @param {CInt} reg */
function freereg(fs, reg) {
    if (reg >= luaY_nvarstack(fs)) {
        (cptr.st1(cptr.add(fs, 68), cptr.ld1u(cptr.add(fs, 68)) + -1)) - (-1);
        (void 0);
    }
}

/** C ref: lcode.c:503 — @param {CPtr} fs @param {CInt} r1 @param {CInt} r2 */
function freeregs(fs, r1, r2) {
    if (r1 > r2) {
        freereg(fs, r1);
        freereg(fs, r2);
    } else {
        freereg(fs, r2);
        freereg(fs, r1);
    }
}

/** C ref: lcode.c:518 — @param {CPtr} fs @param {CPtr} e */
function freeexp(fs, e) {
    if (cptr.ldI32(e) == NHC.VNONRELOC)
        freereg(fs, cptr.ldI32(cptr.add(e, 8)));
}

/** C ref: lcode.c:528 — @param {CPtr} fs @param {CPtr} e1 @param {CPtr} e2 */
function freeexps(fs, e1, e2) {
    let r1 = (cptr.ldI32(e1) == NHC.VNONRELOC) ? cptr.ldI32(cptr.add(e1, 8)) : -1;
    let r2 = (cptr.ldI32(e2) == NHC.VNONRELOC) ? cptr.ldI32(cptr.add(e2, 8)) : -1;
    freeregs(fs, r1, r2);
}

/** C ref: lcode.c:544 — @param {CPtr} fs @param {CPtr} key @param {CPtr} v @returns {CInt} */
function addk(fs, key, v) {
    let val = cptr.alloc(16);
    let L = cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 56));
    let f = cptr.ldPtr(fs);
    let idx = luaH_get(cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 80)), key);
    let k;
    let oldsize;
    if (((cptr.ld1u(cptr.add(((idx)), 8))) == 3)) {
        k = (Number(BigInt.asIntN(32, (((cptr.ldI64(((idx)))))))));
        if ((k < cptr.ldI32(cptr.add(fs, 44)) && (((cptr.ld1u(cptr.add((cptr.add(cptr.ldPtr(cptr.add(f, 56)), k, 16)), 8)))) & 63) == (((cptr.ld1u(cptr.add((v), 8)))) & 63) ? 1 : 0) && luaV_equalobj(null, cptr.add(cptr.ldPtr(cptr.add(f, 56)), k, 16), v) ? 1 : 0)
            return k;
    }
    oldsize = cptr.ldI32(cptr.add(f, 20));
    k = cptr.ldI32(cptr.add(fs, 44));
    {
        let io = (val);
        cptr.stI64(((io)), BigInt((k)));
        (cptr.st1(cptr.add((io), 8), 3));
    }
    ;
    luaH_finishset(L, cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 80)), key, idx, val);
    (cptr.stPtr(cptr.add(f, 56), ((luaM_growaux_(L, cptr.ldPtr(cptr.add(f, 56)), k, cptr.add(f, 20), 16, 33554431, __sl4)))));
    while (oldsize < cptr.ldI32(cptr.add(f, 20)))
        (cptr.st1(cptr.add((cptr.add(cptr.ldPtr(cptr.add(f, 56)), oldsize++, 16)), 8), 0));
    {
        let io1 = (cptr.add(cptr.ldPtr(cptr.add(f, 56)), k, 16));
        let io2 = (v);
        cptr.memcpy(io1, io2, 8);
        (cptr.st1(cptr.add((io1), 8), (cptr.ld1u(cptr.add(io2, 8)))));
        (void L, (void 0));
        (void 0);
    }
    ;
    (cptr.stI32(cptr.add(fs, 44), cptr.ldI32(cptr.add(fs, 44)) + 1)) - (1);
    (((cptr.ld1u(cptr.add((v), 8))) & 64) ? ((((cptr.ld1u(cptr.add((f), 9))) & 32) && ((cptr.ld1u(cptr.add(((cptr.ldPtr(((v))))), 9))) & 24) ? 1 : 0) ? luaC_barrier_(L, ((((f)))), (((((cptr.ldPtr(((v))))))))) : (void 0)) : (void 0));
    return k;
}

/** C ref: lcode.c:576 — @param {CPtr} fs @param {CPtr} s @returns {CInt} */
function stringK(fs, s) {
    let o = cptr.alloc(16);
    {
        let io = (o);
        let x_ = (s);
        cptr.stPtr(((io)), ((((x_)))));
        (cptr.st1(cptr.add((io), 8), uchar((((cptr.ld1u(cptr.add(x_, 8))) | 64)))));
        (void cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 56)), (void 0));
    }
    ;
    return addk(fs, o, o);
}

/** C ref: lcode.c:586 — @param {CPtr} fs @param {CLongLong} n @returns {CInt} */
function luaK_intK(fs, n) {
    let o = cptr.alloc(16);
    {
        let io = (o);
        cptr.stI64(((io)), (n));
        (cptr.st1(cptr.add((io), 8), 3));
    }
    ;
    return addk(fs, o, o);
}

/** C ref: lcode.c:603 — @param {CPtr} fs @param {CDouble} r @returns {CInt} */
function luaK_numberK(fs, r) {
    let o = cptr.alloc(16);
    let ik = cptr.box(0n);
    {
        let io = (o);
        cptr.stF64(((io)), (r));
        (cptr.st1(cptr.add((io), 8), 19));
    }
    ;
    if (!luaV_flttointeger(r, ik, NHC.F2Ieq))
        return addk(fs, o, o);
    else {
        let nbm = 53;
        let q = ldexp(1, (-nbm + 1) | 0);
        let k = (ik.v == 0n) ? q : r + r * q;
        let kv = cptr.alloc(16);
        {
            let io = (kv);
            cptr.stF64(((io)), (k));
            (cptr.st1(cptr.add((io), 8), 19));
        }
        ;
        (void 0);
        return addk(fs, kv, o);
    }
}

/** C ref: lcode.c:626 — @param {CPtr} fs @returns {CInt} */
function boolF(fs) {
    let o = cptr.alloc(16);
    (cptr.st1(cptr.add((o), 8), 1));
    return addk(fs, o, o);
}

/** C ref: lcode.c:636 — @param {CPtr} fs @returns {CInt} */
function boolT(fs) {
    let o = cptr.alloc(16);
    (cptr.st1(cptr.add((o), 8), 17));
    return addk(fs, o, o);
}

/** C ref: lcode.c:646 — @param {CPtr} fs @returns {CInt} */
function nilK(fs) {
    let k = cptr.alloc(16);
    let v = cptr.alloc(16);
    (cptr.st1(cptr.add((v), 8), 0));
    {
        let io = (k);
        let x_ = (cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 80)));
        cptr.stPtr(((io)), ((((x_)))));
        (cptr.st1(cptr.add((io), 8), 69));
        (void cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 56)), (void 0));
    }
    ;
    return addk(fs, k, v);
}

/** C ref: lcode.c:660 — @param {CLongLong} i @returns {CInt} */
function fitsC(i) {
    return (BigInt.asUintN(64, (BigInt.asUintN(64, (i))) + 127n) <= 255n);
}

/** C ref: lcode.c:668 — @param {CLongLong} i @returns {CInt} */
function fitsBx(i) {
    return (-65535n <= i && i <= 65536n ? 1 : 0);
}

/** C ref: lcode.c:673 — @param {CPtr} fs @param {CInt} reg @param {CLongLong} i */
export function luaK_int(fs, reg, i) {
    if (fitsBx(i))
        codeAsBx(fs, NHC.OP_LOADI, reg, (Number(BigInt.asIntN(32, ((i))))));
    else
        luaK_codek(fs, reg, luaK_intK(fs, i));
}

/** C ref: lcode.c:681 — @param {CPtr} fs @param {CInt} reg @param {CDouble} f */
function luaK_float(fs, reg, f) {
    let fi = cptr.box(0n);
    if (luaV_flttointeger(f, fi, NHC.F2Ieq) && fitsBx(fi.v) ? 1 : 0)
        codeAsBx(fs, NHC.OP_LOADF, reg, (Number(BigInt.asIntN(32, ((fi.v))))));
    else
        luaK_codek(fs, reg, luaK_numberK(fs, f));
}

/** C ref: lcode.c:693 — @param {CPtr} v @param {CPtr} e */
function const2exp(v, e) {
    switch ((((cptr.ld1u(cptr.add((v), 8)))) & 63)) {
        case 3:
        cptr.stI32(e, NHC.VKINT);
        cptr.stI64(cptr.add(e, 8), (cptr.ldI64(((v)))));
        break;
        case 19:
        cptr.stI32(e, NHC.VKFLT);
        cptr.stF64(cptr.add(e, 8), (cptr.ldF64(((v)))));
        break;
        case 1:
        cptr.stI32(e, NHC.VFALSE);
        break;
        case 17:
        cptr.stI32(e, NHC.VTRUE);
        break;
        case 0:
        cptr.stI32(e, NHC.VNIL);
        break;
        case 4:
        case 20:
        cptr.stI32(e, NHC.VKSTR);
        cptr.stPtr(cptr.add(e, 8), ((((((cptr.ldPtr(((v))))))))));
        break;
        default:
        (void 0);
    }
}

/** C ref: lcode.c:722 — @param {CPtr} fs @param {CPtr} e @param {CInt} nresults */
export function luaK_setreturns(fs, e, nresults) {
    let pc = cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr((fs)), 64)), cptr.ldI32(cptr.add((e), 8)), 4);
    if (cptr.ldI32(e) == NHC.VCALL)
        (cptr.stI32(pc, (((((cptr.ldI32(pc)) & (~(((~(((~0) << 8) >>> 0)) << 24) >>> 0))) >>> 0) | (((((((nresults + 1) | 0) >>> 0) << 24) >>> 0) & (((~(((~0) << 8) >>> 0)) << 24) >>> 0)) >>> 0)) >>> 0)));
    else {
        (void 0);
        (cptr.stI32(pc, (((((cptr.ldI32(pc)) & (~(((~(((~0) << 8) >>> 0)) << 24) >>> 0))) >>> 0) | (((((((nresults + 1) | 0) >>> 0) << 24) >>> 0) & (((~(((~0) << 8) >>> 0)) << 24) >>> 0)) >>> 0)) >>> 0)));
        (cptr.stI32(pc, (((((cptr.ldI32(pc)) & (~(((~(((~0) << 8) >>> 0)) << 7) >>> 0))) >>> 0) | ((((((cptr.ld1u(cptr.add(fs, 68)))) << 7) >>> 0) & (((~(((~0) << 8) >>> 0)) << 7) >>> 0)) >>> 0)) >>> 0)));
        luaK_reserveregs(fs, 1);
    }
}

/** C ref: lcode.c:738 — @param {CPtr} fs @param {CPtr} e */
function str2K(fs, e) {
    (void 0);
    cptr.stI32(cptr.add(e, 8), stringK(fs, cptr.ldPtr(cptr.add(e, 8))));
    cptr.stI32(e, NHC.VK);
}

/** C ref: lcode.c:755 — @param {CPtr} fs @param {CPtr} e */
export function luaK_setoneret(fs, e) {
    if (cptr.ldI32(e) == NHC.VCALL) {
        (void 0);
        cptr.stI32(e, NHC.VNONRELOC);
        cptr.stI32(cptr.add(e, 8), ((((((((cptr.ldI32(cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr((fs)), 64)), cptr.ldI32(cptr.add((e), 8)), 4)))) >>> 7) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)));
    } else if (cptr.ldI32(e) == NHC.VVARARG) {
        (cptr.stI32(cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr((fs)), 64)), cptr.ldI32(cptr.add((e), 8)), 4), ((((((cptr.ldI32(cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr((fs)), 64)), cptr.ldI32(cptr.add((e), 8)), 4)))) & (~(((~(((~0) << 8) >>> 0)) << 24) >>> 0))) >>> 0) | ((33554432 & (((~(((~0) << 8) >>> 0)) << 24) >>> 0)) >>> 0)) >>> 0)));
        cptr.stI32(e, NHC.VRELOC);
    }
}

/** C ref: lcode.c:773 — @param {CPtr} fs @param {CPtr} e */
export function luaK_dischargevars(fs, e) {
    switch (cptr.ldI32(e)) {
        case NHC.VCONST:
        {
            const2exp(const2val(fs, e), e);
            break;
        }
        case NHC.VLOCAL:
        {
            let temp = cptr.ld1u(cptr.add(e, 8));
            cptr.stI32(cptr.add(e, 8), temp);
            cptr.stI32(e, NHC.VNONRELOC);
            break;
        }
        case NHC.VUPVAL:
        {
            cptr.stI32(cptr.add(e, 8), luaK_codeABCk(fs, NHC.OP_GETUPVAL, 0, cptr.ldI32(cptr.add(e, 8)), 0, 0));
            cptr.stI32(e, NHC.VRELOC);
            break;
        }
        case NHC.VINDEXUP:
        {
            cptr.stI32(cptr.add(e, 8), luaK_codeABCk(fs, NHC.OP_GETTABUP, 0, cptr.ld1u(cptr.add(e, 10)), cptr.ldI16(cptr.add(e, 8)), 0));
            cptr.stI32(e, NHC.VRELOC);
            break;
        }
        case NHC.VINDEXI:
        {
            freereg(fs, cptr.ld1u(cptr.add(e, 10)));
            cptr.stI32(cptr.add(e, 8), luaK_codeABCk(fs, NHC.OP_GETI, 0, cptr.ld1u(cptr.add(e, 10)), cptr.ldI16(cptr.add(e, 8)), 0));
            cptr.stI32(e, NHC.VRELOC);
            break;
        }
        case NHC.VINDEXSTR:
        {
            freereg(fs, cptr.ld1u(cptr.add(e, 10)));
            cptr.stI32(cptr.add(e, 8), luaK_codeABCk(fs, NHC.OP_GETFIELD, 0, cptr.ld1u(cptr.add(e, 10)), cptr.ldI16(cptr.add(e, 8)), 0));
            cptr.stI32(e, NHC.VRELOC);
            break;
        }
        case NHC.VINDEXED:
        {
            freeregs(fs, cptr.ld1u(cptr.add(e, 10)), cptr.ldI16(cptr.add(e, 8)));
            cptr.stI32(cptr.add(e, 8), luaK_codeABCk(fs, NHC.OP_GETTABLE, 0, cptr.ld1u(cptr.add(e, 10)), cptr.ldI16(cptr.add(e, 8)), 0));
            cptr.stI32(e, NHC.VRELOC);
            break;
        }
        case NHC.VVARARG:
        case NHC.VCALL:
        {
            luaK_setoneret(fs, e);
            break;
        }
        default:
        break;
    }
}

/** C ref: lcode.c:827 — @param {CPtr} fs @param {CPtr} e @param {CInt} reg */
function discharge2reg(fs, e, reg) {
    luaK_dischargevars(fs, e);
    switch (cptr.ldI32(e)) {
        case NHC.VNIL:
        {
            luaK_nil(fs, reg, 1);
            break;
        }
        case NHC.VFALSE:
        {
            luaK_codeABCk(fs, NHC.OP_LOADFALSE, reg, 0, 0, 0);
            break;
        }
        case NHC.VTRUE:
        {
            luaK_codeABCk(fs, NHC.OP_LOADTRUE, reg, 0, 0, 0);
            break;
        }
        case NHC.VKSTR:
        {
            str2K(fs, e);
        }
        case NHC.VK:
        {
            luaK_codek(fs, reg, cptr.ldI32(cptr.add(e, 8)));
            break;
        }
        case NHC.VKFLT:
        {
            luaK_float(fs, reg, cptr.ldF64(cptr.add(e, 8)));
            break;
        }
        case NHC.VKINT:
        {
            luaK_int(fs, reg, cptr.ldI64(cptr.add(e, 8)));
            break;
        }
        case NHC.VRELOC:
        {
            let pc = cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr((fs)), 64)), cptr.ldI32(cptr.add((e), 8)), 4);
            (cptr.stI32(pc, (((((cptr.ldI32(pc)) & (~(((~(((~0) << 8) >>> 0)) << 7) >>> 0))) >>> 0) | ((((((reg) >>> 0) << 7) >>> 0) & (((~(((~0) << 8) >>> 0)) << 7) >>> 0)) >>> 0)) >>> 0)));
            break;
        }
        case NHC.VNONRELOC:
        {
            if (reg != cptr.ldI32(cptr.add(e, 8)))
                luaK_codeABCk(fs, NHC.OP_MOVE, reg, cptr.ldI32(cptr.add(e, 8)), 0, 0);
            break;
        }
        default:
        {
            (void 0);
            return;
        }
    }
    cptr.stI32(cptr.add(e, 8), reg);
    cptr.stI32(e, NHC.VNONRELOC);
}

/** C ref: lcode.c:882 — @param {CPtr} fs @param {CPtr} e */
function discharge2anyreg(fs, e) {
    if (cptr.ldI32(e) != NHC.VNONRELOC) {
        luaK_reserveregs(fs, 1);
        discharge2reg(fs, e, (cptr.ld1u(cptr.add(fs, 68)) - 1) | 0);
    }
}

/** C ref: lcode.c:890 — @param {CPtr} fs @param {CInt} A @param {*} op @returns {CInt} */
function code_loadbool(fs, A, op) {
    luaK_getlabel(fs);
    return luaK_codeABCk(fs, op, A, 0, 0, 0);
}

/** C ref: lcode.c:900 — @param {CPtr} fs @param {CInt} list @returns {CInt} */
function need_value(fs, list) {
    for (; list != -1; list = getjump(fs, list)) {
        let i = cptr.ldI32(getjumpcontrol(fs, list));
        if (((((((i) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0))) != NHC.OP_TESTSET)
            return 1;
    }
    return 0;
}

/** C ref: lcode.c:916 — @param {CPtr} fs @param {CPtr} e @param {CInt} reg */
function exp2reg(fs, e, reg) {
    discharge2reg(fs, e, reg);
    if (cptr.ldI32(e) == NHC.VJMP)
        luaK_concat(fs, cptr.add(e, 16), cptr.ldI32(cptr.add(e, 8)));
    if ((cptr.ldI32(cptr.add((e), 16)) != cptr.ldI32(cptr.add((e), 20)))) {
        let final;
        let p_f = -1;
        let p_t = -1;
        if (need_value(fs, cptr.ldI32(cptr.add(e, 16))) || need_value(fs, cptr.ldI32(cptr.add(e, 20))) ? 1 : 0) {
            let fj = (cptr.ldI32(e) == NHC.VJMP) ? -1 : luaK_jump(fs);
            p_f = code_loadbool(fs, reg, NHC.OP_LFALSESKIP);
            p_t = code_loadbool(fs, reg, NHC.OP_LOADTRUE);
            luaK_patchtohere(fs, fj);
        }
        final = luaK_getlabel(fs);
        patchlistaux(fs, cptr.ldI32(cptr.add(e, 20)), final, reg, p_f);
        patchlistaux(fs, cptr.ldI32(cptr.add(e, 16)), final, reg, p_t);
    }
    cptr.stI32(cptr.add(e, 20), cptr.stI32(cptr.add(e, 16), -1));
    cptr.stI32(cptr.add(e, 8), reg);
    cptr.stI32(e, NHC.VNONRELOC);
}

/** C ref: lcode.c:944 — @param {CPtr} fs @param {CPtr} e */
export function luaK_exp2nextreg(fs, e) {
    luaK_dischargevars(fs, e);
    freeexp(fs, e);
    luaK_reserveregs(fs, 1);
    exp2reg(fs, e, (cptr.ld1u(cptr.add(fs, 68)) - 1) | 0);
}

/** C ref: lcode.c:956 — @param {CPtr} fs @param {CPtr} e @returns {CInt} */
export function luaK_exp2anyreg(fs, e) {
    luaK_dischargevars(fs, e);
    if (cptr.ldI32(e) == NHC.VNONRELOC) {
        if (!(cptr.ldI32(cptr.add((e), 16)) != cptr.ldI32(cptr.add((e), 20))))
            return cptr.ldI32(cptr.add(e, 8));
        if (cptr.ldI32(cptr.add(e, 8)) >= luaY_nvarstack(fs)) {
            exp2reg(fs, e, cptr.ldI32(cptr.add(e, 8)));
            return cptr.ldI32(cptr.add(e, 8));
        }
    }
    luaK_exp2nextreg(fs, e);
    return cptr.ldI32(cptr.add(e, 8));
}

/** C ref: lcode.c:978 — @param {CPtr} fs @param {CPtr} e */
export function luaK_exp2anyregup(fs, e) {
    if (cptr.ldI32(e) != NHC.VUPVAL || (cptr.ldI32(cptr.add((e), 16)) != cptr.ldI32(cptr.add((e), 20))) ? 1 : 0)
        luaK_exp2anyreg(fs, e);
}

/** C ref: lcode.c:988 — @param {CPtr} fs @param {CPtr} e */
export function luaK_exp2val(fs, e) {
    if (cptr.ldI32(e) == NHC.VJMP || (cptr.ldI32(cptr.add((e), 16)) != cptr.ldI32(cptr.add((e), 20))) ? 1 : 0)
        luaK_exp2anyreg(fs, e);
    else
        luaK_dischargevars(fs, e);
}

/** C ref: lcode.c:1000 — @param {CPtr} fs @param {CPtr} e @returns {CInt} */
function luaK_exp2K(fs, e) {
    if (!(cptr.ldI32(cptr.add((e), 16)) != cptr.ldI32(cptr.add((e), 20)))) {
        let info;
        switch (cptr.ldI32(e)) {
            case NHC.VTRUE:
            info = boolT(fs);
            break;
            case NHC.VFALSE:
            info = boolF(fs);
            break;
            case NHC.VNIL:
            info = nilK(fs);
            break;
            case NHC.VKINT:
            info = luaK_intK(fs, cptr.ldI64(cptr.add(e, 8)));
            break;
            case NHC.VKFLT:
            info = luaK_numberK(fs, cptr.ldF64(cptr.add(e, 8)));
            break;
            case NHC.VKSTR:
            info = stringK(fs, cptr.ldPtr(cptr.add(e, 8)));
            break;
            case NHC.VK:
            info = cptr.ldI32(cptr.add(e, 8));
            break;
            default:
            return 0;
        }
        if (info <= 255) {
            cptr.stI32(e, NHC.VK);
            cptr.stI32(cptr.add(e, 8), info);
            return 1;
        }
    }
    return 0;
}

/** C ref: lcode.c:1030 — @param {CPtr} fs @param {CPtr} e @returns {CInt} */
function exp2RK(fs, e) {
    if (luaK_exp2K(fs, e))
        return 1;
    else {
        luaK_exp2anyreg(fs, e);
        return 0;
    }
}

/** C ref: lcode.c:1040 — @param {CPtr} fs @param {*} o @param {CInt} a @param {CInt} b @param {CPtr} ec */
function codeABRK(fs, o, a, b, ec) {
    let k = exp2RK(fs, ec);
    luaK_codeABCk(fs, o, a, b, cptr.ldI32(cptr.add(ec, 8)), k);
}

/** C ref: lcode.c:1050 — @param {CPtr} fs @param {CPtr} var @param {CPtr} ex */
export function luaK_storevar(fs, var$, ex) {
    switch (cptr.ldI32(var$)) {
        case NHC.VLOCAL:
        {
            freeexp(fs, ex);
            exp2reg(fs, ex, cptr.ld1u(cptr.add(var$, 8)));
            return;
        }
        case NHC.VUPVAL:
        {
            let e = luaK_exp2anyreg(fs, ex);
            luaK_codeABCk(fs, NHC.OP_SETUPVAL, e, cptr.ldI32(cptr.add(var$, 8)), 0, 0);
            break;
        }
        case NHC.VINDEXUP:
        {
            codeABRK(fs, NHC.OP_SETTABUP, cptr.ld1u(cptr.add(var$, 10)), cptr.ldI16(cptr.add(var$, 8)), ex);
            break;
        }
        case NHC.VINDEXI:
        {
            codeABRK(fs, NHC.OP_SETI, cptr.ld1u(cptr.add(var$, 10)), cptr.ldI16(cptr.add(var$, 8)), ex);
            break;
        }
        case NHC.VINDEXSTR:
        {
            codeABRK(fs, NHC.OP_SETFIELD, cptr.ld1u(cptr.add(var$, 10)), cptr.ldI16(cptr.add(var$, 8)), ex);
            break;
        }
        case NHC.VINDEXED:
        {
            codeABRK(fs, NHC.OP_SETTABLE, cptr.ld1u(cptr.add(var$, 10)), cptr.ldI16(cptr.add(var$, 8)), ex);
            break;
        }
        default:
        (void 0);
    }
    freeexp(fs, ex);
}

/** C ref: lcode.c:1087 — @param {CPtr} fs @param {CPtr} e @param {CPtr} key */
export function luaK_self(fs, e, key) {
    let ereg;
    luaK_exp2anyreg(fs, e);
    ereg = cptr.ldI32(cptr.add(e, 8));
    freeexp(fs, e);
    cptr.stI32(cptr.add(e, 8), cptr.ld1u(cptr.add(fs, 68)));
    cptr.stI32(e, NHC.VNONRELOC);
    luaK_reserveregs(fs, 2);
    codeABRK(fs, NHC.OP_SELF, cptr.ldI32(cptr.add(e, 8)), ereg, key);
    freeexp(fs, key);
}

/** C ref: lcode.c:1103 — @param {CPtr} fs @param {CPtr} e */
function negatecondition(fs, e) {
    let pc = getjumpcontrol(fs, cptr.ldI32(cptr.add(e, 8)));
    (void 0);
    (cptr.stI32(pc, (((((cptr.ldI32(pc)) & (~(((~(((~0) << 1) >>> 0)) << 15) >>> 0))) >>> 0) | (((((((((((((((cptr.ldI32(pc)) >>> 15) & (((~(((~0) << 1) >>> 0)) << 0) >>> 0)) >>> 0)) | 0))) ^ 1)) >>> 0) << 15) >>> 0) & (((~(((~0) << 1) >>> 0)) << 15) >>> 0)) >>> 0)) >>> 0)));
}

/** C ref: lcode.c:1117 — @param {CPtr} fs @param {CPtr} e @param {CInt} cond @returns {CInt} */
function jumponcond(fs, e, cond) {
    if (cptr.ldI32(e) == NHC.VRELOC) {
        let ie = (cptr.ldI32(cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr((fs)), 64)), cptr.ldI32(cptr.add((e), 8)), 4)));
        if (((((((ie) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0))) == NHC.OP_NOT) {
            removelastinstruction(fs);
            return condjump(fs, NHC.OP_TEST, ((((((((ie) >>> 16) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0))), 0, 0, !cond);
        }
    }
    discharge2anyreg(fs, e);
    freeexp(fs, e);
    return condjump(fs, NHC.OP_TESTSET, 255, cptr.ldI32(cptr.add(e, 8)), 0, cond);
}

/** C ref: lcode.c:1135 — @param {CPtr} fs @param {CPtr} e */
export function luaK_goiftrue(fs, e) {
    let pc;
    luaK_dischargevars(fs, e);
    switch (cptr.ldI32(e)) {
        case NHC.VJMP:
        {
            negatecondition(fs, e);
            pc = cptr.ldI32(cptr.add(e, 8));
            break;
        }
        case NHC.VK:
        case NHC.VKFLT:
        case NHC.VKINT:
        case NHC.VKSTR:
        case NHC.VTRUE:
        {
            pc = -1;
            break;
        }
        default:
        {
            pc = jumponcond(fs, e, 0);
            break;
        }
    }
    luaK_concat(fs, cptr.add(e, 20), pc);
    luaK_patchtohere(fs, cptr.ldI32(cptr.add(e, 16)));
    cptr.stI32(cptr.add(e, 16), -1);
}

/** C ref: lcode.c:1162 — @param {CPtr} fs @param {CPtr} e */
export function luaK_goiffalse(fs, e) {
    let pc;
    luaK_dischargevars(fs, e);
    switch (cptr.ldI32(e)) {
        case NHC.VJMP:
        {
            pc = cptr.ldI32(cptr.add(e, 8));
            break;
        }
        case NHC.VNIL:
        case NHC.VFALSE:
        {
            pc = -1;
            break;
        }
        default:
        {
            pc = jumponcond(fs, e, 1);
            break;
        }
    }
    luaK_concat(fs, cptr.add(e, 16), pc);
    luaK_patchtohere(fs, cptr.ldI32(cptr.add(e, 20)));
    cptr.stI32(cptr.add(e, 20), -1);
}

/** C ref: lcode.c:1188 — @param {CPtr} fs @param {CPtr} e */
function codenot(fs, e) {
    switch (cptr.ldI32(e)) {
        case NHC.VNIL:
        case NHC.VFALSE:
        {
            cptr.stI32(e, NHC.VTRUE);
            break;
        }
        case NHC.VK:
        case NHC.VKFLT:
        case NHC.VKINT:
        case NHC.VKSTR:
        case NHC.VTRUE:
        {
            cptr.stI32(e, NHC.VFALSE);
            break;
        }
        case NHC.VJMP:
        {
            negatecondition(fs, e);
            break;
        }
        case NHC.VRELOC:
        case NHC.VNONRELOC:
        {
            discharge2anyreg(fs, e);
            freeexp(fs, e);
            cptr.stI32(cptr.add(e, 8), luaK_codeABCk(fs, NHC.OP_NOT, 0, cptr.ldI32(cptr.add(e, 8)), 0, 0));
            cptr.stI32(e, NHC.VRELOC);
            break;
        }
        default:
        (void 0);
    }
    {
        let temp = cptr.ldI32(cptr.add(e, 20));
        cptr.stI32(cptr.add(e, 20), cptr.ldI32(cptr.add(e, 16)));
        cptr.stI32(cptr.add(e, 16), temp);
    }
    removevalues(fs, cptr.ldI32(cptr.add(e, 20)));
    removevalues(fs, cptr.ldI32(cptr.add(e, 16)));
}

/** C ref: lcode.c:1222 — @param {CPtr} fs @param {CPtr} e @returns {CInt} */
function isKstr(fs, e) {
    return (((cptr.ldI32(e) == NHC.VK && !(cptr.ldI32(cptr.add((e), 16)) != cptr.ldI32(cptr.add((e), 20))) ? 1 : 0) && cptr.ldI32(cptr.add(e, 8)) <= 255 ? 1 : 0) && ((cptr.ld1u(cptr.add(((cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr(fs), 56)), cptr.ldI32(cptr.add(e, 8)), 16))), 8))) == 68) ? 1 : 0);
}

/** C ref: lcode.c:1230 — @param {CPtr} e @returns {CInt} */
function isKint(e) {
    return (cptr.ldI32(e) == NHC.VKINT && !(cptr.ldI32(cptr.add((e), 16)) != cptr.ldI32(cptr.add((e), 20))) ? 1 : 0);
}

/** C ref: lcode.c:1239 — @param {CPtr} e @returns {CInt} */
function isCint(e) {
    return isKint(e) && ((BigInt.asUintN(64, (cptr.ldI64(cptr.add(e, 8))))) <= 255n) ? 1 : 0;
}

/** C ref: lcode.c:1248 — @param {CPtr} e @returns {CInt} */
function isSCint(e) {
    return isKint(e) && fitsC(cptr.ldI64(cptr.add(e, 8))) ? 1 : 0;
}

/** C ref: lcode.c:1257 — @param {CPtr} e @param {CPtr} pi @param {CPtr} isfloat @returns {CInt} */
function isSCnumber(e, pi, isfloat) {
    let i = cptr.box(0n);
    if (cptr.ldI32(e) == NHC.VKINT)
        i.v = cptr.ldI64(cptr.add(e, 8));
    else if (cptr.ldI32(e) == NHC.VKFLT && luaV_flttointeger(cptr.ldF64(cptr.add(e, 8)), i, NHC.F2Ieq) ? 1 : 0)
        cptr.stI32(isfloat, 1);
    else
        return 0;
    if (!(cptr.ldI32(cptr.add((e), 16)) != cptr.ldI32(cptr.add((e), 20))) && fitsC(i.v) ? 1 : 0) {
        cptr.stI32(pi, ((((Number(BigInt.asIntN(32, ((i.v)))))) + 127) | 0));
        return 1;
    } else
        return 0;
}

/** C ref: lcode.c:1280 — @param {CPtr} fs @param {CPtr} t @param {CPtr} k */
export function luaK_indexed(fs, t, k) {
    if (cptr.ldI32(k) == NHC.VKSTR)
        str2K(fs, k);
    (void 0);
    if (cptr.ldI32(t) == NHC.VUPVAL && !isKstr(fs, k) ? 1 : 0)
        luaK_exp2anyreg(fs, t);
    if (cptr.ldI32(t) == NHC.VUPVAL) {
        let temp = cptr.ldI32(cptr.add(t, 8));
        (void 0);
        cptr.st1(cptr.add(t, 10), uchar(temp));
        cptr.stI16(cptr.add(t, 8), i16(cptr.ldI32(cptr.add(k, 8))));
        cptr.stI32(t, NHC.VINDEXUP);
    } else {
        cptr.st1(cptr.add(t, 10), uchar(((cptr.ldI32(t) == NHC.VLOCAL) ? cptr.ld1u(cptr.add(t, 8)) : cptr.ldI32(cptr.add(t, 8)))));
        if (isKstr(fs, k)) {
            cptr.stI16(cptr.add(t, 8), i16(cptr.ldI32(cptr.add(k, 8))));
            cptr.stI32(t, NHC.VINDEXSTR);
        } else if (isCint(k)) {
            cptr.stI16(cptr.add(t, 8), i16((Number(BigInt.asIntN(32, ((cptr.ldI64(cptr.add(k, 8)))))))));
            cptr.stI32(t, NHC.VINDEXI);
        } else {
            cptr.stI16(cptr.add(t, 8), i16(luaK_exp2anyreg(fs, k)));
            cptr.stI32(t, NHC.VINDEXED);
        }
    }
}

/** C ref: lcode.c:1318 — @param {CInt} op @param {CPtr} v1 @param {CPtr} v2 @returns {CInt} */
function validop(op, v1, v2) {
    switch (op) {
        case 7:
        case 8:
        case 9:
        case 10:
        case 11:
        case 13:
        {
            let i = cptr.box(0n);
            return (luaV_tointegerns(v1, i, NHC.F2Ieq) && luaV_tointegerns(v2, i, NHC.F2Ieq) ? 1 : 0);
        }
        case 5:
        case 6:
        case 3:
        return (((((cptr.ld1u(cptr.add(((v2)), 8))) == 3) ? (Number((((cptr.ldI64(((v2)))))))) : (cptr.ldF64(((v2)))))) != 0);
        default:
        return 1;
    }
}

/** C ref: lcode.c:1337 — @param {CPtr} fs @param {CInt} op @param {CPtr} e1 @param {CPtr} e2 @returns {CInt} */
function constfolding(fs, op, e1, e2) {
    let v1 = cptr.alloc(16);
    let v2 = cptr.alloc(16);
    let res = cptr.alloc(16);
    if ((!tonumeral(e1, v1) || !tonumeral(e2, v2) ? 1 : 0) || !validop(op, v1, v2) ? 1 : 0)
        return 0;
    luaO_rawarith(cptr.ldPtr(cptr.add(cptr.ldPtr(cptr.add(fs, 16)), 56)), op, v1, v2, res);
    if (((cptr.ld1u(cptr.add(((res)), 8))) == 3)) {
        cptr.stI32(e1, NHC.VKINT);
        cptr.stI64(cptr.add(e1, 8), (cptr.ldI64(((res)))));
    } else {
        let n = (cptr.ldF64(((res))));
        if ((!(((n)) == ((n)))) || n == 0 ? 1 : 0)
            return 0;
        cptr.stI32(e1, NHC.VKFLT);
        cptr.stF64(cptr.add(e1, 8), n);
    }
    return 1;
}

/** C ref: lcode.c:1361 — @param {*} opr @param {*} baser @param {*} base @returns {*} */
function binopr2op(opr, baser, base) {
    (void 0);
    return ((((((((opr))) - (((baser)))) | 0) + (((base)))) | 0));
}

/** C ref: lcode.c:1372 — @param {*} opr @returns {*} */
function unopr2op(opr) {
    return ((((((((opr))) - NHC.OPR_MINUS) | 0) + NHC.OP_UNM) | 0));
}

/** C ref: lcode.c:1381 — @param {*} opr @returns {*} */
function binopr2TM(opr) {
    (void 0);
    return ((((((((opr))) - NHC.OPR_ADD) | 0) + NHC.TM_ADD) | 0));
}

/** C ref: lcode.c:1392 — @param {CPtr} fs @param {*} op @param {CPtr} e @param {CInt} line */
function codeunexpval(fs, op, e, line) {
    let r = luaK_exp2anyreg(fs, e);
    freeexp(fs, e);
    cptr.stI32(cptr.add(e, 8), luaK_codeABCk(fs, op, 0, r, 0, 0));
    cptr.stI32(e, NHC.VRELOC);
    luaK_fixline(fs, line);
}

/** C ref: lcode.c:1407 — @param {CPtr} fs @param {CPtr} e1 @param {CPtr} e2 @param {*} op @param {CInt} v2 @param {CInt} flip @param {CInt} line @param {*} mmop @param {*} event */
function finishbinexpval(fs, e1, e2, op, v2, flip, line, mmop, event) {
    let v1 = luaK_exp2anyreg(fs, e1);
    let pc = luaK_codeABCk(fs, op, 0, v1, v2, 0);
    freeexps(fs, e1, e2);
    cptr.stI32(cptr.add(e1, 8), pc);
    cptr.stI32(e1, NHC.VRELOC);
    luaK_fixline(fs, line);
    luaK_codeABCk(fs, mmop, v1, v2, event, flip);
    luaK_fixline(fs, line);
}

/** C ref: lcode.c:1425 — @param {CPtr} fs @param {*} opr @param {CPtr} e1 @param {CPtr} e2 @param {CInt} line */
function codebinexpval(fs, opr, e1, e2, line) {
    let op = binopr2op(opr, NHC.OPR_ADD, NHC.OP_ADD);
    let v2 = luaK_exp2anyreg(fs, e2);
    (void 0);
    (void 0);
    finishbinexpval(fs, e1, e2, op, v2, 0, line, NHC.OP_MMBIN, binopr2TM(opr));
}

/** C ref: lcode.c:1440 — @param {CPtr} fs @param {*} op @param {CPtr} e1 @param {CPtr} e2 @param {CInt} flip @param {CInt} line @param {*} event */
function codebini(fs, op, e1, e2, flip, line, event) {
    let v2 = ((((Number(BigInt.asIntN(32, ((cptr.ldI64(cptr.add(e2, 8)))))))) + 127) | 0);
    (void 0);
    finishbinexpval(fs, e1, e2, op, v2, flip, line, NHC.OP_MMBINI, event);
}

/** C ref: lcode.c:1452 — @param {CPtr} fs @param {*} opr @param {CPtr} e1 @param {CPtr} e2 @param {CInt} flip @param {CInt} line */
function codebinK(fs, opr, e1, e2, flip, line) {
    let event = binopr2TM(opr);
    let v2 = cptr.ldI32(cptr.add(e2, 8));
    let op = binopr2op(opr, NHC.OPR_ADD, NHC.OP_ADDK);
    finishbinexpval(fs, e1, e2, op, v2, flip, line, NHC.OP_MMBINK, event);
}

/** C ref: lcode.c:1464 — @param {CPtr} fs @param {CPtr} e1 @param {CPtr} e2 @param {*} op @param {CInt} line @param {*} event @returns {CInt} */
function finishbinexpneg(fs, e1, e2, op, line, event) {
    if (!isKint(e2))
        return 0;
    else {
        let i2 = cptr.ldI64(cptr.add(e2, 8));
        if (!(fitsC(i2) && fitsC(-i2) ? 1 : 0))
            return 0;
        else {
            let v2 = (Number(BigInt.asIntN(32, ((i2)))));
            finishbinexpval(fs, e1, e2, op, (((-v2) + 127) | 0), 0, line, NHC.OP_MMBINI, event);
            (cptr.stI32(cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr(fs), 64)), (cptr.ldI32(cptr.add(fs, 32)) - 1) | 0, 4), (((((cptr.ldI32(cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr(fs), 64)), (cptr.ldI32(cptr.add(fs, 32)) - 1) | 0, 4))) & (~(((~(((~0) << 8) >>> 0)) << 16) >>> 0))) >>> 0) | (((((((((v2) + 127) | 0)) >>> 0) << 16) >>> 0) & (((~(((~0) << 8) >>> 0)) << 16) >>> 0)) >>> 0)) >>> 0)));
            return 1;
        }
    }
}

/** C ref: lcode.c:1483 — @param {CPtr} e1 @param {CPtr} e2 */
function swapexps(e1, e2) {
    let temp = cptr.alloc(24); cptr.memcpy(temp, e1, 24);
    cptr.memcpy(e1, e2, 24);
    cptr.memcpy(e2, temp, 24);
}

/** C ref: lcode.c:1491 — @param {CPtr} fs @param {*} opr @param {CPtr} e1 @param {CPtr} e2 @param {CInt} flip @param {CInt} line */
function codebinNoK(fs, opr, e1, e2, flip, line) {
    if (flip)
        swapexps(e1, e2);
    codebinexpval(fs, opr, e1, e2, line);
}

/** C ref: lcode.c:1503 — @param {CPtr} fs @param {*} opr @param {CPtr} e1 @param {CPtr} e2 @param {CInt} flip @param {CInt} line */
function codearith(fs, opr, e1, e2, flip, line) {
    if (tonumeral(e2, null) && luaK_exp2K(fs, e2) ? 1 : 0)
        codebinK(fs, opr, e1, e2, flip, line);
    else
        codebinNoK(fs, opr, e1, e2, flip, line);
}

/** C ref: lcode.c:1517 — @param {CPtr} fs @param {*} op @param {CPtr} e1 @param {CPtr} e2 @param {CInt} line */
function codecommutative(fs, op, e1, e2, line) {
    let flip = 0;
    if (tonumeral(e1, null)) {
        swapexps(e1, e2);
        flip = 1;
    }
    if (op == NHC.OPR_ADD && isSCint(e2) ? 1 : 0)
        codebini(fs, NHC.OP_ADDI, e1, e2, flip, line, NHC.TM_ADD);
    else
        codearith(fs, op, e1, e2, flip, line);
}

/** C ref: lcode.c:1535 — @param {CPtr} fs @param {*} opr @param {CPtr} e1 @param {CPtr} e2 @param {CInt} line */
function codebitwise(fs, opr, e1, e2, line) {
    let flip = 0;
    if (cptr.ldI32(e1) == NHC.VKINT) {
        swapexps(e1, e2);
        flip = 1;
    }
    if (cptr.ldI32(e2) == NHC.VKINT && luaK_exp2K(fs, e2) ? 1 : 0)
        codebinK(fs, opr, e1, e2, flip, line);
    else
        codebinNoK(fs, opr, e1, e2, flip, line);
}

/** C ref: lcode.c:1553 — @param {CPtr} fs @param {*} opr @param {CPtr} e1 @param {CPtr} e2 */
function codeorder(fs, opr, e1, e2) {
    let r1;
    let r2;
    let im = cptr.box(0);
    let isfloat = cptr.box(0);
    let op;
    if (isSCnumber(e2, im, isfloat)) {
        r1 = luaK_exp2anyreg(fs, e1);
        r2 = im.v;
        op = binopr2op(opr, NHC.OPR_LT, NHC.OP_LTI);
    } else if (isSCnumber(e1, im, isfloat)) {
        r1 = luaK_exp2anyreg(fs, e2);
        r2 = im.v;
        op = binopr2op(opr, NHC.OPR_LT, NHC.OP_GTI);
    } else {
        r1 = luaK_exp2anyreg(fs, e1);
        r2 = luaK_exp2anyreg(fs, e2);
        op = binopr2op(opr, NHC.OPR_LT, NHC.OP_LT);
    }
    freeexps(fs, e1, e2);
    cptr.stI32(cptr.add(e1, 8), condjump(fs, op, r1, r2, isfloat.v, 1));
    cptr.stI32(e1, NHC.VJMP);
}

/** C ref: lcode.c:1585 — @param {CPtr} fs @param {*} opr @param {CPtr} e1 @param {CPtr} e2 */
function codeeq(fs, opr, e1, e2) {
    let r1;
    let r2;
    let im = cptr.box(0);
    let isfloat = cptr.box(0);
    let op;
    if (cptr.ldI32(e1) != NHC.VNONRELOC) {
        (void 0);
        swapexps(e1, e2);
    }
    r1 = luaK_exp2anyreg(fs, e1);
    if (isSCnumber(e2, im, isfloat)) {
        op = NHC.OP_EQI;
        r2 = im.v;
    } else if (exp2RK(fs, e2)) {
        op = NHC.OP_EQK;
        r2 = cptr.ldI32(cptr.add(e2, 8));
    } else {
        op = NHC.OP_EQ;
        r2 = luaK_exp2anyreg(fs, e2);
    }
    freeexps(fs, e1, e2);
    cptr.stI32(cptr.add(e1, 8), condjump(fs, op, r1, r2, isfloat.v, (opr == NHC.OPR_EQ)));
    cptr.stI32(e1, NHC.VJMP);
}

let __static_luaK_prefix_ef = cptr.alloc(24); /** C ref: lcode.c:1617 — struct expdesc (function-static) */
cptr.stI32(__static_luaK_prefix_ef, NHC.VKINT);
cptr.stI64(cptr.add(__static_luaK_prefix_ef, 8), 0n);
cptr.stI32(cptr.add(__static_luaK_prefix_ef, 16), -1);
cptr.stI32(cptr.add(__static_luaK_prefix_ef, 20), -1);

/** C ref: lcode.c:1616 — @param {CPtr} fs @param {*} opr @param {CPtr} e @param {CInt} line */
export function luaK_prefix(fs, opr, e, line) {
    luaK_dischargevars(fs, e);
    switch (opr) {
        case NHC.OPR_MINUS:
        case NHC.OPR_BNOT:
        if (constfolding(fs, ((opr + 12) >>> 0) | 0, e, __static_luaK_prefix_ef))
            break;
        case NHC.OPR_LEN:
        codeunexpval(fs, unopr2op(opr), e, line);
        break;
        case NHC.OPR_NOT:
        codenot(fs, e);
        break;
        default:
        (void 0);
    }
}

/** C ref: lcode.c:1637 — @param {CPtr} fs @param {*} op @param {CPtr} v */
export function luaK_infix(fs, op, v) {
    luaK_dischargevars(fs, v);
    switch (op) {
        case NHC.OPR_AND:
        {
            luaK_goiftrue(fs, v);
            break;
        }
        case NHC.OPR_OR:
        {
            luaK_goiffalse(fs, v);
            break;
        }
        case NHC.OPR_CONCAT:
        {
            luaK_exp2nextreg(fs, v);
            break;
        }
        case NHC.OPR_ADD:
        case NHC.OPR_SUB:
        case NHC.OPR_MUL:
        case NHC.OPR_DIV:
        case NHC.OPR_IDIV:
        case NHC.OPR_MOD:
        case NHC.OPR_POW:
        case NHC.OPR_BAND:
        case NHC.OPR_BOR:
        case NHC.OPR_BXOR:
        case NHC.OPR_SHL:
        case NHC.OPR_SHR:
        {
            if (!tonumeral(v, null))
                luaK_exp2anyreg(fs, v);
            break;
        }
        case NHC.OPR_EQ:
        case NHC.OPR_NE:
        {
            if (!tonumeral(v, null))
                exp2RK(fs, v);
            break;
        }
        case NHC.OPR_LT:
        case NHC.OPR_LE:
        case NHC.OPR_GT:
        case NHC.OPR_GE:
        {
            let dummy = cptr.box(0);
            let dummy2 = cptr.box(0);
            if (!isSCnumber(v, dummy, dummy2))
                luaK_exp2anyreg(fs, v);
            break;
        }
        default:
        (void 0);
    }
}

/** C ref: lcode.c:1686 — @param {CPtr} fs @param {CPtr} e1 @param {CPtr} e2 @param {CInt} line */
function codeconcat(fs, e1, e2, line) {
    let ie2 = previousinstruction(fs);
    if (((((((cptr.ldI32(ie2)) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0))) == NHC.OP_CONCAT) {
        let n = ((((((((cptr.ldI32(ie2)) >>> 16) & (((~(((~0) << 8) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)));
        (void 0);
        freeexp(fs, e2);
        (cptr.stI32(ie2, (((((cptr.ldI32(ie2)) & (~(((~(((~0) << 8) >>> 0)) << 7) >>> 0))) >>> 0) | ((((((cptr.ldI32(cptr.add(e1, 8))) >>> 0) << 7) >>> 0) & (((~(((~0) << 8) >>> 0)) << 7) >>> 0)) >>> 0)) >>> 0)));
        (cptr.stI32(ie2, (((((cptr.ldI32(ie2)) & (~(((~(((~0) << 8) >>> 0)) << 16) >>> 0))) >>> 0) | (((((((n + 1) | 0) >>> 0) << 16) >>> 0) & (((~(((~0) << 8) >>> 0)) << 16) >>> 0)) >>> 0)) >>> 0)));
    } else {
        luaK_codeABCk(fs, NHC.OP_CONCAT, cptr.ldI32(cptr.add(e1, 8)), 2, 0, 0);
        freeexp(fs, e2);
        luaK_fixline(fs, line);
    }
}

/** C ref: lcode.c:1706 — @param {CPtr} fs @param {*} opr @param {CPtr} e1 @param {CPtr} e2 @param {CInt} line */
export function luaK_posfix(fs, opr, e1, e2, line) {
    luaK_dischargevars(fs, e2);
    if (((opr) <= NHC.OPR_SHR) && constfolding(fs, ((opr + 0) >>> 0) | 0, e1, e2) ? 1 : 0)
        return;
    switch (opr) {
        case NHC.OPR_AND:
        {
            (void 0);
            luaK_concat(fs, cptr.add(e2, 20), cptr.ldI32(cptr.add(e1, 20)));
            cptr.memcpy(e1, e2, 24);
            break;
        }
        case NHC.OPR_OR:
        {
            (void 0);
            luaK_concat(fs, cptr.add(e2, 16), cptr.ldI32(cptr.add(e1, 16)));
            cptr.memcpy(e1, e2, 24);
            break;
        }
        case NHC.OPR_CONCAT:
        {
            luaK_exp2nextreg(fs, e2);
            codeconcat(fs, e1, e2, line);
            break;
        }
        case NHC.OPR_ADD:
        case NHC.OPR_MUL:
        {
            codecommutative(fs, opr, e1, e2, line);
            break;
        }
        case NHC.OPR_SUB:
        {
            if (finishbinexpneg(fs, e1, e2, NHC.OP_ADDI, line, NHC.TM_SUB))
                break;
        }
        case NHC.OPR_DIV:
        case NHC.OPR_IDIV:
        case NHC.OPR_MOD:
        case NHC.OPR_POW:
        {
            codearith(fs, opr, e1, e2, 0, line);
            break;
        }
        case NHC.OPR_BAND:
        case NHC.OPR_BOR:
        case NHC.OPR_BXOR:
        {
            codebitwise(fs, opr, e1, e2, line);
            break;
        }
        case NHC.OPR_SHL:
        {
            if (isSCint(e1)) {
                swapexps(e1, e2);
                codebini(fs, NHC.OP_SHLI, e1, e2, 1, line, NHC.TM_SHL);
            } else if (finishbinexpneg(fs, e1, e2, NHC.OP_SHRI, line, NHC.TM_SHL)) {
                ;
            } else
                codebinexpval(fs, opr, e1, e2, line);
            break;
        }
        case NHC.OPR_SHR:
        {
            if (isSCint(e2))
                codebini(fs, NHC.OP_SHRI, e1, e2, 0, line, NHC.TM_SHR);
            else
                codebinexpval(fs, opr, e1, e2, line);
            break;
        }
        case NHC.OPR_EQ:
        case NHC.OPR_NE:
        {
            codeeq(fs, opr, e1, e2);
            break;
        }
        case NHC.OPR_GT:
        case NHC.OPR_GE:
        {
            swapexps(e1, e2);
            opr = (((((opr - NHC.OPR_GT) >>> 0) + NHC.OPR_LT) >>> 0));
        }
        case NHC.OPR_LT:
        case NHC.OPR_LE:
        {
            codeorder(fs, opr, e1, e2);
            break;
        }
        default:
        (void 0);
    }
}

/** C ref: lcode.c:1787 — @param {CPtr} fs @param {CInt} line */
export function luaK_fixline(fs, line) {
    removelastlineinfo(fs);
    savelineinfo(fs, cptr.ldPtr(fs), line);
}

/** C ref: lcode.c:1793 — @param {CPtr} fs @param {CInt} pc @param {CInt} ra @param {CInt} asize @param {CInt} hsize */
export function luaK_settablesize(fs, pc, ra, asize, hsize) {
    let inst = cptr.add(cptr.ldPtr(cptr.add(cptr.ldPtr(fs), 64)), pc, 4);
    let rb = (hsize != 0) ? (luaO_ceillog2(hsize >>> 0) + 1) | 0 : 0;
    let extra = (asize / 256) | 0;
    let rc = asize % 256;
    let k = (extra > 0);
    cptr.stI32(inst, (((((19 | ((((ra) >>> 0) << 7) >>> 0)) >>> 0 | ((((rb) >>> 0) << 16) >>> 0)) >>> 0 | ((((rc) >>> 0) << 24) >>> 0)) >>> 0 | ((((k) >>> 0) << 15) >>> 0)) >>> 0));
    cptr.stI32((cptr.add(inst, 1, 4)), ((82 | ((((extra) >>> 0) << 7) >>> 0)) >>> 0));
}

/** C ref: lcode.c:1811 — @param {CPtr} fs @param {CInt} base @param {CInt} nelems @param {CInt} tostore */
export function luaK_setlist(fs, base, nelems, tostore) {
    (void 0);
    if (tostore == -1)
        tostore = 0;
    if (nelems <= 255)
        luaK_codeABCk(fs, NHC.OP_SETLIST, base, tostore, nelems, 0);
    else {
        let extra = (nelems / 256) | 0;
        nelems %= 256;
        luaK_codeABCk(fs, NHC.OP_SETLIST, base, tostore, nelems, 1);
        codeextraarg(fs, extra);
    }
    cptr.st1(cptr.add(fs, 68), uchar(((base + 1) | 0)));
}

/** C ref: lcode.c:1830 — @param {CPtr} code @param {CInt} i @returns {CInt} */
function finaltarget(code, i) {
    let count;
    for (count = 0; count < 100; count++) {
        let pc = cptr.ldI32(cptr.add(code, i, 4));
        if (((((((pc) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0))) != NHC.OP_JMP)
            break;
        else
            i = (i + (((((((((((pc) >>> 7) & (((~(((~0) << 25) >>> 0)) << 0) >>> 0)) >>> 0)) | 0)) - 16777215) | 0) + 1) | 0)) | 0;
    }
    return i;
}

/** C ref: lcode.c:1847 — @param {CPtr} fs */
export function luaK_finish(fs) {
    let i;
    let p = cptr.ldPtr(fs);
    for (i = 0; i < cptr.ldI32(cptr.add(fs, 32)); i++) {
        let pc = cptr.add(cptr.ldPtr(cptr.add(p, 64)), i, 4);
        (void 0);
        switch (((((((cptr.ldI32(pc)) >>> 0) & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0)))) {
            case NHC.OP_RETURN0:
            case NHC.OP_RETURN1:
            {
                if (!(cptr.ld1u(cptr.add(fs, 70)) || cptr.ld1u(cptr.add(p, 11)) ? 1 : 0))
                    break;
                (cptr.stI32(pc, (((((cptr.ldI32(pc)) & (~(((~(((~0) << 7) >>> 0)) << 0) >>> 0))) >>> 0) | ((70 & (((~(((~0) << 7) >>> 0)) << 0) >>> 0)) >>> 0)) >>> 0)));
            }
            case NHC.OP_RETURN:
            case NHC.OP_TAILCALL:
            {
                if (cptr.ld1u(cptr.add(fs, 70)))
                    (cptr.stI32(pc, (((((cptr.ldI32(pc)) & (~(((~(((~0) << 1) >>> 0)) << 15) >>> 0))) >>> 0) | ((32768 & (((~(((~0) << 1) >>> 0)) << 15) >>> 0)) >>> 0)) >>> 0)));
                if (cptr.ld1u(cptr.add(p, 11)))
                    (cptr.stI32(pc, (((((cptr.ldI32(pc)) & (~(((~(((~0) << 8) >>> 0)) << 24) >>> 0))) >>> 0) | (((((((cptr.ld1u(cptr.add(p, 10)) + 1) | 0) >>> 0) << 24) >>> 0) & (((~(((~0) << 8) >>> 0)) << 24) >>> 0)) >>> 0)) >>> 0)));
                break;
            }
            case NHC.OP_JMP:
            {
                let target = finaltarget(cptr.ldPtr(cptr.add(p, 64)), i);
                fixjump(fs, i, target);
                break;
            }
            default:
            break;
        }
    }
}
