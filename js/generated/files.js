...
export function delete_bonesfile(lev) {
    ...
    delete_convertedfile(fqname(cptr.add(gb, FLD.instance_globals_b_bones), NHM.BONESPREFIX, 0));
    return !(reslt < 0);
}
...
export function compress_bonesfile() {
    ...
    nh_compress(fqname(cptr.add(gb, FLD.instance_globals_b_bones), NHM.BONESPREFIX, 0));
}
...
export function save_savefile_name(nhfp) {
    sfo_char(nhfp, cptr.add(gs, FLD.instance_globals_s_SAVEF), __sl18, 54);
}
...
export function plname_from_file(filename, without_wait_synch_per_file) {
    ...
    nh_uncompress(cptr.add(gs, FLD.instance_globals_s_SAVEF));
    if ((nhfp = open_savefile()) !== null) {
    ...
    nh_compress(cptr.add(gs, FLD.instance_globals_s_SAVEF));
    return result;
}
    ...
    switch (sfstatus) {
        ...
        for (i = 0; i < 10; ++i) {
            if (cptr.ldI32o(sf2msg, i, 16) == sfstatus) {
                raw_printf(__sl44, savefilenm, (sfstatus == NHM.SF_OUTDATED) ? __sl45 : __sl46, cptr.ldPtro2(sf2msg, i, 16, FLD.sfstatus_to_msg_msg));
                break;
            }
        }
    }
...
function wizkit_addinv(obj) {
    ...
    if (cptr.ld1so(obj, FLD.obj_oclass) != NHC.COIN_CLASS && inv_cnt(0) >= NHC.invlet_basic && !merge_choice(cptr.ldPtro(gi, FLD.instance_globals_i_invent), obj)) {
...
export function read_sym_file(which_set) {
    ...
    if (!cptr.ld1so(gc, FLD.instance_globals_c_chosen_symset_start) && !cptr.ld1so(gc, FLD.instance_globals_c_chosen_symset_end)) {
        if (cptr.ldPtro2(gs, which_set, 48, FLD.instance_globals_s_symset + FLD.symsetentry_name) && (fuzzymatch(cptr.ldPtro2(gs, which_set, 48, FLD.instance_globals_s_symset + FLD.symsetentry_name), __sl68, __sl69, 1) || !strncmpi((cptr.ldPtro2(gs, which_set, 48, FLD.instance_globals_s_symset + FLD.symsetentry_name)), (__sl70), -1)))
    ...
    if (!cptr.ld1so(gc, FLD.instance_globals_c_chosen_symset_end))
        config_error_add(__sl71, cptr.ldPtro2(gs, which_set, 48, FLD.instance_globals_s_symset + FLD.symsetentry_name) ? cptr.ldPtro2(gs, which_set, 48, FLD.instance_globals_s_symset + FLD.symsetentry_name) : __sl72);
    config_error_done();
    return 1;
}
...
export function do_deferred_showpaths(code) {
    ...
    ;
    l_nhcore_done();
    after_opt_showpaths(cptr.ldPtro(gd, FLD.instance_globals_d_deferred_showpaths_dir));
}
