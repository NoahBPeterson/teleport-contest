...
function wizkit_addinv(obj) {
    ...
    if (cptr.ld1so(obj, 49) != NHC.COIN_CLASS && inv_cnt(0) >= NHC.invlet_basic && !merge_choice(cptr.ldPtro(gi, 8), obj)) {
...
export function read_wizkit() {
    let fp;
    if (!cptr.ld1so(flags, 10) || !(fp = fopen_wizkit_file()))
        return;
...
export function read_sym_file(which_set) {
    ...
    if (!cptr.ld1so(gc, 456) && !cptr.ld1so(gc, 457)) {
        if (cptr.ldPtro2(gs, which_set, 48, 208) && (fuzzymatch(cptr.ldPtro2(gs, which_set, 48, 208), __sl68, __sl69, 1) || !strncmpi((cptr.ldPtro2(gs, which_set, 48, 208)), (__sl70), -1)))
