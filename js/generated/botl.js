// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/src/botl.c
// Input sha256: 765fe13d7ee604c606c6920bce9bcb2a8a82299dbd56073b2b90101409409360
// Transpiler: tools/c2js c2js emit v1+batch

import { i16, schar, u32div } from '../cmachine.js';
import * as cptr from '../cptr.js';
import * as NHC from './nhconst.js';
import * as NHM from './nhmacro.js';
import * as FLD from './nhfield.js';
import { bimanual, is_sword, is_weptool } from './nhmacrofn.js';
import { Blind, Deaf, Flying, Glib, HConfusion, HStun, Hallucination, Levitation, Role_switch, Sick, Slimed, Stoned, Strangled, Ugender, Underwater, Upolyd, Wounded_legs, create_nhwindow, curs, destroy_nhwindow, display_nhwindow, end_menu, putmixed, putstr, start_menu, status_enablefield, status_update, tutorial_dnum } from './nhprop.js';
import { acurr } from './attrib.js';
import { WIN_STATUS, cg, disp, flags, gb, gc, gi, gm, gn, gs, gu, gv, gy, iflags, svc, svd, svl, svm, svp, u, uamul, uarm, uarmc, uarmf, uarmg, uarmh, uarms, uarmu, uleft, uright, uswapwep, uwep } from './decl.js';
import { nul_glyphinfo, suppress_map_output } from './display.js';
import { digit, eos, fuzzymatch, highc, lowc, mungspaces, nh_deterministic_qsort, nh_snprintf, sgn, strNsubst, str_start_is, stripchars, strkitten, strncmpi, strstri, strsubst, trimspaces, upstart } from './hacklib.js';
import { pmname } from './do_name.js';
import { mons } from './monst.js';
import { classify_terrain, money_cnt, near_capacity } from './hack.js';
import { add_menu, add_menu_heading, add_menu_str, encglyph, getlin, select_menu, windowprocs } from './windows.js';
import { hu_stat } from './eat.js';
import { status_version } from './version.js';
import { panic } from './end.js';
import { roles } from './role.js';
import { Strlen_, strbuf_append } from './strutil.js';
import { In_quest, depth, dunlev, endgamelevelname, on_level } from './dungeon.js';
import { weapon_descr, weapon_type } from './weapon.js';
import { objects } from './objects.js';
import { helm_simple_name } from './objnam.js';
import { sticks } from './mondata.js';
import { unconscious } from './trap.js';
import { match_optname } from './options.js';
import { impossible, pline } from './pline.js';
import { alloc, dupstr, fmt_ptr } from './alloc.js';
import { newuexp } from './exper.js';
import { critically_low_hp } from './pray.js';
import { config_error_add } from './cfgfiles.js';
import { clr2colorname, match_str2attr, match_str2clr, query_attr, query_color } from './coloratt.js';

// struct field offsets used below, bound at module scope so V8 folds them
// (values from ./nhfield.js, which is the whole table)
const $RoleName_f = FLD.RoleName_f, $Role_mnum = FLD.Role_mnum, $Role_rank = FLD.Role_rank,
    $_cond_map_clratr = FLD._cond_map_clratr, $_status_hilite_line_str_fld = FLD._status_hilite_line_str_fld,
    $_status_hilite_line_str_hl = FLD._status_hilite_line_str_hl,
    $_status_hilite_line_str_mask = FLD._status_hilite_line_str_mask,
    $_status_hilite_line_str_next = FLD._status_hilite_line_str_next,
    $_status_hilite_line_str_str = FLD._status_hilite_line_str_str, $conditions_t_c = FLD.conditions_t_c,
    $conditions_t_mask = FLD.conditions_t_mask, $conditions_t_text = FLD.conditions_t_text,
    $condmap_bitmask = FLD.condmap_bitmask, $condtests_t_choice = FLD.condtests_t_choice,
    $condtests_t_enabled = FLD.condtests_t_enabled, $condtests_t_opt = FLD.condtests_t_opt,
    $condtests_t_test = FLD.condtests_t_test, $condtests_t_useroption = FLD.condtests_t_useroption,
    $const_globals_zeroany = FLD.const_globals_zeroany, $context_info_rndencode = FLD.context_info_rndencode,
    $d_level_dlevel = FLD.d_level_dlevel, $dgn_topology_d_astral_level = FLD.dgn_topology_d_astral_level,
    $dgn_topology_d_knox_level = FLD.dgn_topology_d_knox_level,
    $dgn_topology_d_tutorial_dnum = FLD.dgn_topology_d_tutorial_dnum,
    $display_hints_botlx = FLD.display_hints_botlx, $display_hints_time_botl = FLD.display_hints_time_botl,
    $fieldid_t_fldid = FLD.fieldid_t_fldid, $flag_armorstatus = FLD.flag_armorstatus,
    $flag_female = FLD.flag_female, $flag_showexp = FLD.flag_showexp, $flag_showscore = FLD.flag_showscore,
    $flag_showvers = FLD.flag_showvers, $flag_terrainstatus = FLD.flag_terrainstatus,
    $flag_time = FLD.flag_time, $flag_versinfo = FLD.flag_versinfo,
    $flag_weaponstatus = FLD.flag_weaponstatus, $hilite_s_anytype = FLD.hilite_s_anytype,
    $hilite_s_behavior = FLD.hilite_s_behavior, $hilite_s_coloridx = FLD.hilite_s_coloridx,
    $hilite_s_next = FLD.hilite_s_next, $hilite_s_rel = FLD.hilite_s_rel, $hilite_s_set = FLD.hilite_s_set,
    $hilite_s_textmatch = FLD.hilite_s_textmatch, $hilite_s_value = FLD.hilite_s_value,
    $instance_flags_debug_fuzzer = FLD.instance_flags_debug_fuzzer,
    $instance_flags_hilite_delta = FLD.instance_flags_hilite_delta,
    $instance_flags_in_dumplog = FLD.instance_flags_in_dumplog,
    $instance_flags_invis_goldsym = FLD.instance_flags_invis_goldsym,
    $instance_flags_status_updates = FLD.instance_flags_status_updates,
    $instance_flags_terrain_typ = FLD.instance_flags_terrain_typ,
    $instance_flags_wc2_hitpointbar = FLD.instance_flags_wc2_hitpointbar,
    $instance_globals_b_bl_hilite_moves = FLD.instance_globals_b_bl_hilite_moves,
    $instance_globals_b_blinit = FLD.instance_globals_b_blinit,
    $instance_globals_b_bot_disabled = FLD.instance_globals_b_bot_disabled,
    $instance_globals_c_cond_hilites = FLD.instance_globals_c_cond_hilites,
    $instance_globals_c_condmenu_sortorder = FLD.instance_globals_c_condmenu_sortorder,
    $instance_globals_i_invent = FLD.instance_globals_i_invent,
    $instance_globals_m_mrank_sz = FLD.instance_globals_m_mrank_sz,
    $instance_globals_m_multi = FLD.instance_globals_m_multi,
    $instance_globals_m_multi_reason = FLD.instance_globals_m_multi_reason,
    $instance_globals_n_nomovemsg = FLD.instance_globals_n_nomovemsg,
    $instance_globals_s_showsyms = FLD.instance_globals_s_showsyms,
    $instance_globals_saved_d_dungeon_topology = FLD.instance_globals_saved_d_dungeon_topology,
    $instance_globals_saved_l_level = FLD.instance_globals_saved_l_level,
    $instance_globals_saved_m_moves = FLD.instance_globals_saved_m_moves,
    $instance_globals_u_urole = FLD.instance_globals_u_urole,
    $instance_globals_y_youmonst = FLD.instance_globals_y_youmonst, $istat_s_a = FLD.istat_s_a,
    $istat_s_anytype = FLD.istat_s_anytype, $istat_s_chg = FLD.istat_s_chg, $istat_s_fld = FLD.istat_s_fld,
    $istat_s_fldfmt = FLD.istat_s_fldfmt, $istat_s_hilite_rule = FLD.istat_s_hilite_rule,
    $istat_s_idxmax = FLD.istat_s_idxmax, $istat_s_percent_matters = FLD.istat_s_percent_matters,
    $istat_s_percent_value = FLD.istat_s_percent_value, $istat_s_rawval = FLD.istat_s_rawval,
    $istat_s_thresholds = FLD.istat_s_thresholds, $istat_s_time = FLD.istat_s_time,
    $istat_s_val = FLD.istat_s_val, $istat_s_valwidth = FLD.istat_s_valwidth, $monst_data = FLD.monst_data,
    $obj_oartifact = FLD.obj_oartifact, $obj_oclass = FLD.obj_oclass, $obj_otyp = FLD.obj_otyp,
    $objclass_oc_big = FLD.objclass_oc_big, $objclass_oc_subtyp = FLD.objclass_oc_subtyp,
    $permonst_mflags1 = FLD.permonst_mflags1, $permonst_mlet = FLD.permonst_mlet,
    $permonst_mlevel = FLD.permonst_mlevel, $prop_blocked = FLD.prop_blocked,
    $prop_intrinsic = FLD.prop_intrinsic, $rm_typ = FLD.rm_typ, $sizeof_Role = FLD.sizeof_Role,
    $sizeof_RoleName = FLD.sizeof_RoleName, $sizeof__cond_map = FLD.sizeof__cond_map,
    $sizeof_conditions_t = FLD.sizeof_conditions_t, $sizeof_condmap = FLD.sizeof_condmap,
    $sizeof_condtests_t = FLD.sizeof_condtests_t, $sizeof_dungeon = FLD.sizeof_dungeon,
    $sizeof_fieldid_t = FLD.sizeof_fieldid_t, $sizeof_istat_s = FLD.sizeof_istat_s,
    $sizeof_istat_s_x27 = FLD.sizeof_istat_s_x27, $sizeof_menu_item = FLD.sizeof_menu_item,
    $sizeof_objclass = FLD.sizeof_objclass, $sizeof_permonst = FLD.sizeof_permonst,
    $sizeof_prop = FLD.sizeof_prop, $sizeof_rm = FLD.sizeof_rm, $sizeof_rm_x21 = FLD.sizeof_rm_x21,
    $u_roleplay_deaf = FLD.u_roleplay_deaf,
    $window_procs_win_create_nhwindow = FLD.window_procs_win_create_nhwindow,
    $window_procs_win_curs = FLD.window_procs_win_curs,
    $window_procs_win_destroy_nhwindow = FLD.window_procs_win_destroy_nhwindow,
    $window_procs_win_display_nhwindow = FLD.window_procs_win_display_nhwindow,
    $window_procs_win_end_menu = FLD.window_procs_win_end_menu,
    $window_procs_win_putmixed = FLD.window_procs_win_putmixed,
    $window_procs_win_putstr = FLD.window_procs_win_putstr,
    $window_procs_win_start_menu = FLD.window_procs_win_start_menu,
    $window_procs_win_status_enablefield = FLD.window_procs_win_status_enablefield,
    $window_procs_win_status_finish = FLD.window_procs_win_status_finish,
    $window_procs_win_status_init = FLD.window_procs_win_status_init,
    $window_procs_win_status_update = FLD.window_procs_win_status_update,
    $window_procs_wincap2 = FLD.window_procs_wincap2, $you_mfemale = FLD.you_mfemale, $you_mh = FLD.you_mh,
    $you_mhmax = FLD.you_mhmax, $you_twoweap = FLD.you_twoweap, $you_uac = FLD.you_uac,
    $you_ualign = FLD.you_ualign, $you_uen = FLD.you_uen, $you_uenmax = FLD.you_uenmax,
    $you_uexp = FLD.you_uexp, $you_uhp = FLD.you_uhp, $you_uhpmax = FLD.you_uhpmax, $you_uhs = FLD.you_uhs,
    $you_uinwater = FLD.you_uinwater, $you_ulevel = FLD.you_ulevel, $you_umconf = FLD.you_umconf,
    $you_umonnum = FLD.you_umonnum, $you_umonster = FLD.you_umonster, $you_uprops = FLD.you_uprops,
    $you_uroleplay = FLD.you_uroleplay, $you_usick_type = FLD.you_usick_type, $you_usleep = FLD.you_usleep,
    $you_usteed = FLD.you_usteed, $you_ustuck = FLD.you_ustuck, $you_uswallow = FLD.you_uswallow,
    $you_utrap = FLD.you_utrap, $you_utraptype = FLD.you_utraptype, $you_uy = FLD.you_uy,
    $you_uz = FLD.you_uz;

// string literals (C char* uses decay to CPtr into these static buffers)
const __s_empty = cptr.lit("");
const __s_burdened = cptr.lit("Burdened");
const __s_stressed = cptr.lit("Stressed");
const __s_strained = cptr.lit("Strained");
const __s_overtaxed = cptr.lit("Overtaxed");
const __s_overloaded = cptr.lit("Overloaded");
const __s_2d = cptr.lit("%2d");
const __s_18_02d = cptr.lit("18/%02d");
const __s_18 = cptr.lit("18/**");
const __s_1d = cptr.lit("%-1d");
const __s_the = cptr.lit(" the ");
const __s_sp2 = cptr.lit("  ");
const __s_pct_star_s = cptr.lit("%*s");
const __s_sp = cptr.lit(" ");
const __s_st_s_dx_1d_co_1d_in_1d_wi_1d_ch_1d = cptr.lit("St:%s Dx:%-1d Co:%-1d In:%-1d Wi:%-1d Ch:%-1d");
const __s_pct_s = cptr.lit("%s");
const __s_chaotic = cptr.lit("  Chaotic");
const __s_neutral = cptr.lit("  Neutral");
const __s_lawful = cptr.lit("  Lawful");
const __s_s_2ld = cptr.lit("%s:%-2ld");
const __s_dollar = cptr.lit("$");
const __s_bslash_g = cptr.lit("\\G");
const __s_hp_d_d_pw_d_d_ac_2d = cptr.lit("HP:%d(%d) Pw:%d(%d) AC:%-2d");
const __s_hd_d = cptr.lit("HD:%d");
const __s_xp_d_1ld = cptr.lit("Xp:%d/%-1ld");
const __s_xp_d = cptr.lit("Xp:%d");
const __s_t_ld = cptr.lit("T:%ld");
const __s_stone = cptr.lit(" Stone");
const __s_slime = cptr.lit(" Slime");
const __s_strngl = cptr.lit(" Strngl");
const __s_foodpois = cptr.lit(" FoodPois");
const __s_termill = cptr.lit(" TermIll");
const __s_sp_pct_s = cptr.lit(" %s");
const __s_blind = cptr.lit(" Blind");
const __s_deaf = cptr.lit(" Deaf");
const __s_stun = cptr.lit(" Stun");
const __s_conf = cptr.lit(" Conf");
const __s_hallu = cptr.lit(" Hallu");
const __s_lev = cptr.lit(" Lev");
const __s_fly = cptr.lit(" Fly");
const __s_ride = cptr.lit(" Ride");
const __s_do_statusline2 = cptr.lit("do_statusline2");
const __s_s_s_s_s_s_s = cptr.lit("%s %s %s %s %s%s");
const __s_bot2_second_status_line_exceeds_maxco_u = cptr.lit("bot2: second status line exceeds MAXCO (%u > %d)");
const __s_player = cptr.lit("Player");
const __s_title_to_mon = cptr.lit("title_to_mon");
const __s_home_d = cptr.lit("Home %d");
const __s_plane_of = cptr.lit("Plane of ");
const __s_s_2d = cptr.lit("%s:%-2d");
const __s_tutorial = cptr.lit("Tutorial");
const __s_dlvl = cptr.lit("Dlvl");
const __s_level_d = cptr.lit("level %d");
const __s_comma_sp_pct_s = cptr.lit(", %s");
const __s_the__2 = cptr.lit("The ");
const __s_the__3 = cptr.lit("the ");
const __s_empty_hnd = cptr.lit("Empty-hnd");
const __s_bare_hnds = cptr.lit("Bare-hnds");
const __s_no_weapon = cptr.lit("No-weapon");
const __s_dual_weps = cptr.lit("Dual-weps");
const __s_dual_joust = cptr.lit("Dual+joust");
const __s_joust = cptr.lit("joust");
const __s_aklys = cptr.lit("aklys");
const __s_sword = cptr.lit("sword");
const __s_staff = cptr.lit("staff");
const __s_mrng_star = cptr.lit("mrng-star");
const __s_pole = cptr.lit("pole");
const __s_unihorn = cptr.lit("unihorn");
const __s_food = cptr.lit("food");
const __s_pie = cptr.lit("pie");
const __s_two = cptr.lit("two");
const __s_2h = cptr.lit("2H-");
const __s_dash = cptr.lit("-");
const __s_naked = cptr.lit("naked");
const __s_gloves = cptr.lit("gloves");
const __s_cloak = cptr.lit("cloak");
const __s_suit = cptr.lit("suit");
const __s_shirt = cptr.lit("shirt");
const __s_boots = cptr.lit("boots");
const __s_shield = cptr.lit("shield");
const __s_title = cptr.lit("title");
const __s_strength = cptr.lit("strength");
const __s_st_s = cptr.lit(" St:%s");
const __s_dexterity = cptr.lit("dexterity");
const __s_dx_s = cptr.lit(" Dx:%s");
const __s_constitution = cptr.lit("constitution");
const __s_co_s = cptr.lit(" Co:%s");
const __s_intelligence = cptr.lit("intelligence");
const __s_in_s = cptr.lit(" In:%s");
const __s_wisdom = cptr.lit("wisdom");
const __s_wi_s = cptr.lit(" Wi:%s");
const __s_charisma = cptr.lit("charisma");
const __s_ch_s = cptr.lit(" Ch:%s");
const __s_alignment = cptr.lit("alignment");
const __s_score = cptr.lit("score");
const __s_s_s = cptr.lit(" S:%s");
const __s_carrying_capacity = cptr.lit("carrying-capacity");
const __s_gold = cptr.lit("gold");
const __s_power = cptr.lit("power");
const __s_pw_s = cptr.lit(" Pw:%s");
const __s_power_max = cptr.lit("power-max");
const __s_lparen_pct_s_rparen = cptr.lit("(%s)");
const __s_experience_level = cptr.lit("experience-level");
const __s_xp_s = cptr.lit(" Xp:%s");
const __s_armor_class = cptr.lit("armor-class");
const __s_ac_s = cptr.lit(" AC:%s");
const __s_hd = cptr.lit("HD");
const __s_hd_s = cptr.lit(" HD:%s");
const __s_time = cptr.lit("time");
const __s_t_s = cptr.lit(" T:%s");
const __s_hunger = cptr.lit("hunger");
const __s_hitpoints = cptr.lit("hitpoints");
const __s_hp_s = cptr.lit(" HP:%s");
const __s_hitpoints_max = cptr.lit("hitpoints-max");
const __s_dungeon_level = cptr.lit("dungeon-level");
const __s_experience = cptr.lit("experience");
const __s_slash_pct_s = cptr.lit("/%s");
const __s_condition = cptr.lit("condition");
const __s_version = cptr.lit("version");
const __s_weapon = cptr.lit("weapon");
const __s_armor = cptr.lit("armor");
const __s_terrain = cptr.lit("terrain");
const __s_strangled = cptr.lit("strangled");
const __s_all = cptr.lit("all");
const __s_major_troubles = cptr.lit("major_troubles");
const __s_minor_troubles = cptr.lit("minor_troubles");
const __s_movement = cptr.lit("movement");
const __s_opt_in = cptr.lit("opt_in");
const __s_bare = cptr.lit("Bare");
const __s_bar = cptr.lit("Bar");
const __s_bh = cptr.lit("Bh");
const __s_blind__2 = cptr.lit("Blind");
const __s_blnd = cptr.lit("Blnd");
const __s_bl = cptr.lit("Bl");
const __s_busy = cptr.lit("Busy");
const __s_bsy = cptr.lit("Bsy");
const __s_by = cptr.lit("By");
const __s_conf__2 = cptr.lit("Conf");
const __s_cnf = cptr.lit("Cnf");
const __s_cf = cptr.lit("Cf");
const __s_deaf__2 = cptr.lit("Deaf");
const __s_def = cptr.lit("Def");
const __s_df = cptr.lit("Df");
const __s_iron = cptr.lit("Iron");
const __s_irn = cptr.lit("Irn");
const __s_fe = cptr.lit("Fe");
const __s_fly__2 = cptr.lit("Fly");
const __s_fl = cptr.lit("Fl");
const __s_foodpois__2 = cptr.lit("FoodPois");
const __s_fpois = cptr.lit("Fpois");
const __s_poi = cptr.lit("Poi");
const __s_glow = cptr.lit("Glow");
const __s_glo = cptr.lit("Glo");
const __s_gl = cptr.lit("Gl");
const __s_grab = cptr.lit("Grab");
const __s_grb = cptr.lit("Grb");
const __s_gr = cptr.lit("Gr");
const __s_hallu__2 = cptr.lit("Hallu");
const __s_hal = cptr.lit("Hal");
const __s_hl = cptr.lit("Hl");
const __s_held = cptr.lit("Held");
const __s_hld = cptr.lit("Hld");
const __s_hd__2 = cptr.lit("Hd");
const __s_icy = cptr.lit("Icy");
const __s_ic = cptr.lit("Ic");
const __s_inlava = cptr.lit("InLava");
const __s_lav = cptr.lit("Lav");
const __s_la = cptr.lit("La");
const __s_lev__2 = cptr.lit("Lev");
const __s_lv = cptr.lit("Lv");
const __s_parlyz = cptr.lit("Parlyz");
const __s_para = cptr.lit("Para");
const __s_par = cptr.lit("Par");
const __s_ride__2 = cptr.lit("Ride");
const __s_rid = cptr.lit("Rid");
const __s_rd = cptr.lit("Rd");
const __s_zzz = cptr.lit("Zzz");
const __s_zz = cptr.lit("Zz");
const __s_slime__2 = cptr.lit("Slime");
const __s_slim = cptr.lit("Slim");
const __s_slm = cptr.lit("Slm");
const __s_slip = cptr.lit("Slip");
const __s_slp = cptr.lit("Slp");
const __s_sl = cptr.lit("Sl");
const __s_stone__2 = cptr.lit("Stone");
const __s_ston = cptr.lit("Ston");
const __s_sto = cptr.lit("Sto");
const __s_strngl__2 = cptr.lit("Strngl");
const __s_stngl = cptr.lit("Stngl");
const __s_str = cptr.lit("Str");
const __s_stun__2 = cptr.lit("Stun");
const __s_st = cptr.lit("St");
const __s_submrg = cptr.lit("Submrg");
const __s_subm = cptr.lit("Subm");
const __s_sm = cptr.lit("Sm");
const __s_termill__2 = cptr.lit("TermIll");
const __s_ill = cptr.lit("Ill");
const __s_teth = cptr.lit("Teth");
const __s_tth = cptr.lit("Tth");
const __s_te = cptr.lit("Te");
const __s_trap = cptr.lit("Trap");
const __s_trp = cptr.lit("Trp");
const __s_tr = cptr.lit("Tr");
const __s_out = cptr.lit("Out");
const __s_ko = cptr.lit("KO");
const __s_wlegs = cptr.lit("WLegs");
const __s_leg = cptr.lit("Leg");
const __s_lg = cptr.lit("Lg");
const __s_uhold = cptr.lit("UHold");
const __s_uhld = cptr.lit("UHld");
const __s_uhd = cptr.lit("UHd");
const __s_barehanded = cptr.lit("barehanded");
const __s_blind__3 = cptr.lit("blind");
const __s_busy__2 = cptr.lit("busy");
const __s_conf__3 = cptr.lit("conf");
const __s_deaf__3 = cptr.lit("deaf");
const __s_iron__2 = cptr.lit("iron");
const __s_fly__3 = cptr.lit("fly");
const __s_foodpois__3 = cptr.lit("foodPois");
const __s_glowhands = cptr.lit("glowhands");
const __s_grab__2 = cptr.lit("grab");
const __s_hallucinat = cptr.lit("hallucinat");
const __s_held__2 = cptr.lit("held");
const __s_ice = cptr.lit("ice");
const __s_lava = cptr.lit("lava");
const __s_levitate = cptr.lit("levitate");
const __s_paralyzed = cptr.lit("paralyzed");
const __s_ride__3 = cptr.lit("ride");
const __s_sleep = cptr.lit("sleep");
const __s_slime__3 = cptr.lit("slime");
const __s_slip__2 = cptr.lit("slip");
const __s_stone__3 = cptr.lit("stone");
const __s_strngl__3 = cptr.lit("strngl");
const __s_stun__3 = cptr.lit("stun");
const __s_submerged = cptr.lit("submerged");
const __s_termill__3 = cptr.lit("termIll");
const __s_tethered = cptr.lit("tethered");
const __s_trap__2 = cptr.lit("trap");
const __s_unconscious = cptr.lit("unconscious");
const __s_woundedlegs = cptr.lit("woundedlegs");
const __s_holding = cptr.lit("holding");
const __s_portcullis = cptr.lit("Portcullis");
const __s_tree = cptr.lit("Tree");
const __s_pool = cptr.lit("Pool");
const __s_moat = cptr.lit("Moat");
const __s_water = cptr.lit("Water");
const __s_gap = cptr.lit("(gap)");
const __s_lava__2 = cptr.lit("Lava");
const __s_lavawall = cptr.lit("LavaWall");
const __s_bars = cptr.lit("Bars");
const __s_doorway = cptr.lit("Doorway");
const __s_corridor = cptr.lit("Corridor");
const __s_room = cptr.lit("Room");
const __s_stairs = cptr.lit("Stairs");
const __s_ladder = cptr.lit("Ladder");
const __s_fountain = cptr.lit("Fountain");
const __s_throne = cptr.lit("Throne");
const __s_sink = cptr.lit("Sink");
const __s_grave = cptr.lit("Grave");
const __s_altar = cptr.lit("Altar");
const __s_ice__2 = cptr.lit("Ice");
const __s_bridge = cptr.lit("Bridge");
const __s_air = cptr.lit("Air");
const __s_cloud = cptr.lit("Cloud");
const __s_floor = cptr.lit("Floor");
const __s_ground = cptr.lit("Ground");
const __s_open_door = cptr.lit("Open-door");
const __s_shut_door = cptr.lit("Shut-door");
const __s_swamp = cptr.lit("Swamp");
const __s_submerged__2 = cptr.lit("Submerged");
const __s_sea = cptr.lit("Sea");
const __s_waterwall = cptr.lit("WaterWall");
const __s_bot_before_init = cptr.lit("bot before init.");
const __s_30s = cptr.lit("%-30s");
const __s_chaotic__2 = cptr.lit("Chaotic");
const __s_neutral__2 = cptr.lit("Neutral");
const __s_lawful__2 = cptr.lit("Lawful");
const __s_s_ld = cptr.lit("%s:%ld");
const __s_frozen = cptr.lit("frozen");
const __s_parse_cond_option = cptr.lit("parse_cond_option");
const __s_change_sort_order_from_s_to_s = cptr.lit("change sort order from \"%s\" to \"%s\"");
const __s_sorted_s = cptr.lit("sorted %s");
const __s_cond_14s = cptr.lit("cond_%-14s");
const __s_choose_status_conditions_to_toggle = cptr.lit("Choose status conditions to toggle");
const __s_alphabetically = cptr.lit("alphabetically");
const __s_by_ranking = cptr.lit("by ranking");
const __s_scond_s = cptr.lit("%scond_%s");
const __s_bang = cptr.lit("!");
const __s_2nd_status_initialize_with_full_init = cptr.lit("2nd status_initialize with full init.");
const __s_status_reassess_before_init = cptr.lit("status 'reassess' before init");
const __s_30_30s = cptr.lit("%-30.30s");
const __s_init_blstats_called_more_than_once = cptr.lit("init_blstats called more than once.");
const __s_compare_blstat_bad_istat_pointer_s_s = cptr.lit("compare_blstat: bad istat pointer %s, %s");
const __s_compare_blstat_invalid_pointer_s_s = cptr.lit("compare_blstat: invalid pointer %s, %s");
const __s_lu = cptr.lit("%lu");
const __s_lx = cptr.lit("%lx");
const __s_ld = cptr.lit("%ld");
const __s_pct_d = cptr.lit("%d");
const __s_pct_u = cptr.lit("%u");
const __s_percentage_bad_istat_pointer_s_s = cptr.lit("percentage: bad istat pointer %s, %s");
const __s_characteristics = cptr.lit("characteristics");
const __s_encumbrance = cptr.lit("encumbrance");
const __s_experience_points = cptr.lit("experience-points");
const __s_dx = cptr.lit("dx");
const __s_co = cptr.lit("co");
const __s_con = cptr.lit("con");
const __s_points = cptr.lit("points");
const __s_cap = cptr.lit("cap");
const __s_pw = cptr.lit("pw");
const __s_pw_max = cptr.lit("pw-max");
const __s_xl = cptr.lit("xl");
const __s_xplvl = cptr.lit("xplvl");
const __s_ac = cptr.lit("ac");
const __s_hit_dice = cptr.lit("hit-dice");
const __s_turns = cptr.lit("turns");
const __s_hp = cptr.lit("hp");
const __s_hp_max = cptr.lit("hp-max");
const __s_dgn = cptr.lit("dgn");
const __s_xp = cptr.lit("xp");
const __s_exp = cptr.lit("exp");
const __s_flags = cptr.lit("flags");
const __s_sp_dash_us = cptr.lit(" -_");
const __s_none_of_the_above = cptr.lit("none of the above");
const __s_quot_sp_dash_us = cptr.lit("\" -_");
const __s_polymorphed = cptr.lit("(polymorphed)");
const __s_quot_lparen_rparen = cptr.lit("\"()");
const __s_none_of_the_above_polymorphed = cptr.lit("none of the above (polymorphed)");
const __s_quot_sp_dash_us_lparen_rparen = cptr.lit("\" -_()");
const __s_0123456789 = cptr.lit("<>=-+0123456789%");
const __s_unknown_status_field_s = cptr.lit("Unknown status field '%s'");
const __s_always = cptr.lit("always");
const __s_up = cptr.lit("up");
const __s_down = cptr.lit("down");
const __s_changed = cptr.lit("changed");
const __s_criticalhp = cptr.lit("criticalhp");
const __s_pct_lt_gt_eq_plus = cptr.lit("%<>=+");
const __s_gt = cptr.lit(">");
const __s_gt_eq = cptr.lit(">=");
const __s_lt = cptr.lit("<");
const __s_lt_eq = cptr.lit("<=");
const __s_eq = cptr.lit("=");
const __s_s_s_d_s_s = cptr.lit("%s'%s%d%s'%s");
const __s_pct = cptr.lit("%");
const __s_s_s_ld_s = cptr.lit("%s'%s%ld'%s");
const __s_wrong_format_s_expected_a_threshold = cptr.lit("Wrong format '%s', expected a threshold number or percent");
const __s_unknown_behavior_s = cptr.lit("Unknown behavior '%s'");
const __s_field_s_does_not_support_numeric_values = cptr.lit("Field '%s' does not support numeric values");
const __s_cannot_use_percent_with_s = cptr.lit("Cannot use percent with '%s'");
const __s_hilite_status_invalid_percentage_value = cptr.lit("hilite_status: invalid percentage value '%s%d%%'");
const __s_bad_color_d_d = cptr.lit("bad color '%d %d'");
const __s_chaotic__3 = cptr.lit("chaotic");
const __s_neutral__3 = cptr.lit("neutral");
const __s_lawful__3 = cptr.lit("lawful");
const __s_satiated = cptr.lit("Satiated");
const __s_hungry = cptr.lit("Hungry");
const __s_weak = cptr.lit("Weak");
const __s_fainting = cptr.lit("Fainting");
const __s_fainted = cptr.lit("Fainted");
const __s_starved = cptr.lit("Starved");
const __s_choose_status_conditions = cptr.lit("Choose status conditions");
const __s_s_s__2 = cptr.lit("%s%s");
const __s_plus = cptr.lit("+");
const __s_unknown_condition_s = cptr.lit("Unknown condition '%s'");
const __s_missing_condition_s = cptr.lit("Missing condition(s)");
const __s_missing_color_attribute = cptr.lit("Missing color+attribute");
const __s_bad_color_d = cptr.lit("bad color %d");
const __s_normal = cptr.lit("normal");
const __s_bold = cptr.lit("+bold");
const __s_bold__2 = cptr.lit("bold");
const __s_dim = cptr.lit("+dim");
const __s_dim__2 = cptr.lit("dim");
const __s_italic = cptr.lit("+italic");
const __s_italic__2 = cptr.lit("italic");
const __s_underline = cptr.lit("+underline");
const __s_underline__2 = cptr.lit("underline");
const __s_blink = cptr.lit("+blink");
const __s_blink__2 = cptr.lit("blink");
const __s_inverse = cptr.lit("+inverse");
const __s_inverse__2 = cptr.lit("inverse");
const __s_amp_pct_s = cptr.lit("&%s");
const __s_status_hilite_linestr_gather_conditions = cptr.lit("status_hilite_linestr_gather_conditions");
const __s_condition_s_s = cptr.lit("condition/%s/%s");
const __s_s_d = cptr.lit("%s%d%%");
const __s_hl_behavior_percentage_rel_error = cptr.lit("hl->behavior=percentage, rel error");
const __s_hl_behavior_updown_rel_error = cptr.lit("hl->behavior=updown, rel error");
const __s_s_d__2 = cptr.lit("%s%d");
const __s_hl_behavior_absolute_rel_error = cptr.lit("hl->behavior=absolute, rel error");
const __s_hl_behavior_textmatch_rel_or_textmatch = cptr.lit("hl->behavior=textmatch, rel or textmatch error");
const __s_hl_behavior_condition_rel_error = cptr.lit("hl->behavior=condition, rel error");
const __s_status_hilite2str = cptr.lit("status_hilite2str");
const __s_s_s_s = cptr.lit("%s/%s/%s");
const __s_select_a_hilite_field = cptr.lit("Select a hilite field:");
const __s_always_highlight_s = cptr.lit("Always highlight %s");
const __s_bitmask_of_conditions = cptr.lit("Bitmask of conditions");
const __s_s_value_changes = cptr.lit("%s value changes");
const __s_number_threshold = cptr.lit("Number threshold");
const __s_percentage_threshold = cptr.lit("Percentage threshold");
const __s_highlight_critically_low_s = cptr.lit("Highlight critically low %s");
const __s_s_text_match = cptr.lit("%s text match");
const __s_select_s_field_hilite_behavior = cptr.lit("Select %s field hilite behavior:");
const __s_s_than_s = cptr.lit("%s than %s");
const __s_better_lower = cptr.lit("Better (lower)");
const __s_less = cptr.lit("Less");
const __s_value_goes_down = cptr.lit("Value goes down");
const __s_s_or_s = cptr.lit("%s or %s");
const __s_better_lower__2 = cptr.lit("better (lower)");
const __s_less__2 = cptr.lit("less");
const __s_exactly_s = cptr.lit("Exactly %s");
const __s_value_changes = cptr.lit("Value changes");
const __s_worse_higher = cptr.lit("worse (higher)");
const __s_more = cptr.lit("more");
const __s_worse_higher__2 = cptr.lit("Worse (higher)");
const __s_more__2 = cptr.lit("More");
const __s_value_goes_up = cptr.lit("Value goes up");
const __s_select_field_s_value = cptr.lit("Select field %s value:");
const __s_that_s_enough_tries = cptr.lit("That's enough tries.");
const __s_enter_svalue_for_s_threshold = cptr.lit("Enter %svalue for %s threshold:");
const __s_percentage = cptr.lit("percentage ");
const __s_not_expecting_a_percentage = cptr.lit("Not expecting a percentage.");
const __s_s_is_not_a_recognized_number = cptr.lit("\"%s\" is not a recognized number.");
const __s_is_that_an_invisible_number = cptr.lit("Is that an invisible number?");
const __s_field_s_does_not_support_percentage = cptr.lit("Field '%s' does not support percentage values.");
const __s_s_d_is_not_a_valid_percent_value = cptr.lit("'%s%d%%' is not a valid percent value.");
const __s_s_s_d_s = cptr.lit("%s'%s%d'%s");
const __s_choose_a_color_for_when_s_is_s_s_s = cptr.lit("Choose a color for when %s is %s%s%s:");
const __s_less_than = cptr.lit("less than ");
const __s_more_than = cptr.lit("more than ");
const __s_or_less = cptr.lit(" or less");
const __s_or_more = cptr.lit(" or more");
const __s_choose_attribute_for_when_s_is_s_s_s = cptr.lit("Choose attribute for when %s is %s%s%s:");
const __s_choose_a_color_for_when_s_s = cptr.lit("Choose a color for when %s %s:");
const __s_changes = cptr.lit("changes");
const __s_decreases = cptr.lit("decreases");
const __s_increases = cptr.lit("increases");
const __s_choose_attribute_for_when_s_s = cptr.lit("Choose attribute for when %s %s:");
const __s_status_hilite_menu_add = cptr.lit("status_hilite_menu_add");
const __s_choose_a_color_for_conditions_s = cptr.lit("Choose a color for conditions %s:");
const __s_choose_attribute_for_conditions_s = cptr.lit("Choose attribute for conditions %s:");
const __s_s_s_text_value_to_match = cptr.lit("%s %s text value to match:");
const __s_choose = cptr.lit("Choose");
const __s_enter = cptr.lit("Enter");
const __s_quot_pct_s_quot = cptr.lit("\"%s\"");
const __s_none_of_the_above_polymorphed__2 = cptr.lit("\"none of the above (polymorphed)\"");
const __s_choose_a_color_for_when_s_is_s = cptr.lit("Choose a color for when %s is '%s':");
const __s_choose_attribute_for_when_s_is_s = cptr.lit("Choose attribute for when %s is '%s':");
const __s_choose_a_color_to_always_hilite_s = cptr.lit("Choose a color to always hilite %s:");
const __s_choose_attribute_to_always_hilite_s = cptr.lit("Choose attribute to always hilite %s:");
const __s_added_hilite_condition_s_s = cptr.lit("Added hilite condition/%s/%s");
const __s_or = cptr.lit(" or ");
const __s_added_hilite_s = cptr.lit("Added hilite %s");
const __s_no_current_hilites_for_s = cptr.lit("No current hilites for %s");
const __s_remove_selected_hilites = cptr.lit("Remove selected hilites");
const __s_add_new_hilites = cptr.lit("Add new hilites");
const __s_current_s_hilites = cptr.lit("Current %s hilites:");
const __s_options_hilite_status_s = cptr.lit("OPTIONS=hilite_status: %.*s");
const __s_options_hilite_status_s__2 = cptr.lit("OPTIONS=hilite_status: %.*s\n");
const __s_view_all_hilites_in_config_format = cptr.lit("View all hilites in config format");
const __s_18s = cptr.lit("%-18s");
const __s_d_defined = cptr.lit(" (%d defined)");
const __s_status_hilites = cptr.lit("Status hilites:");

/** C ref: botl.c:11 — char *[6] */
export const enc_stat = cptr.alloc(6 * 8);
cptr.stPtro(enc_stat, 0, __s_empty);
cptr.stPtro(enc_stat, 8, __s_burdened);
cptr.stPtro(enc_stat, 16, __s_stressed);
cptr.stPtro(enc_stat, 24, __s_strained);
cptr.stPtro(enc_stat, 32, __s_overtaxed);
cptr.stPtro(enc_stat, 40, __s_overloaded);

const __static_get_strength_str_buf = new Uint8Array(32); /** C ref: botl.c:23 — char[32] (function-static) */

/** C ref: botl.c:21 @returns {CPtr<char>} */
export function get_strength_str() {
    let st = (acurr(NHC.A_STR));
    if (st > 18) {
        if (st > 118)
            void cptr.sprintf(cptr.decay(__static_get_strength_str_buf), __s_2d, (st - 100) | 0);
        else if (st < 118)
            void cptr.sprintf(cptr.decay(__static_get_strength_str_buf), __s_18_02d, (st - 18) | 0);
        else
            void cptr.sprintf(cptr.decay(__static_get_strength_str_buf), __s_18);
    } else
        void cptr.sprintf(cptr.decay(__static_get_strength_str_buf), __s_1d, st);
    return cptr.decay(__static_get_strength_str_buf);
}

/** C ref: botl.c:40 */
export function check_gold_symbol() {
    let goldch = cptr.ld1uo2(gs, ((NHC.COIN_CLASS + (((0) + NHC.MAXPCHARS) | 0)) | 0), 1, $instance_globals_s_showsyms);
    cptr.st1o(iflags, $instance_flags_invis_goldsym, schar((goldch <= 32)));
}

const __static_do_statusline1_newbot1 = new Uint8Array(256); /** C ref: botl.c:50 — char[256] (function-static) */

/** C ref: botl.c:48 @returns {CPtr<char>} */
export function do_statusline1() {
    let nb;
    let i;
    let j;
    if (suppress_map_output())
        return cptr.strcpy(cptr.decay(__static_do_statusline1_newbot1), __s_empty);
    void cptr.strcpy(cptr.decay(__static_do_statusline1_newbot1), svp);
    if (97 <= cptr.ld1so(cptr.decay(__static_do_statusline1_newbot1), 0, 1) && cptr.ld1so(cptr.decay(__static_do_statusline1_newbot1), 0, 1) <= 122)
        cptr.st1o(cptr.decay(__static_do_statusline1_newbot1), 0, cptr.ld1so(cptr.decay(__static_do_statusline1_newbot1), 0, 1) + -32, 1);
    cptr.st1o(cptr.decay(__static_do_statusline1_newbot1), NHM.BOTL_NSIZ, 0, 1);
    void cptr.sprintf(nb = eos(cptr.decay(__static_do_statusline1_newbot1)), __s_the);
    if (Upolyd()) {
        let mbot = new Uint8Array(256);
        let k = 0;
        void cptr.strcpy(cptr.decay(mbot), pmname(cptr.add(mons, cptr.ldI32o(u, $you_umonnum), $sizeof_permonst), Ugender()));
        while (cptr.ld1so(cptr.decay(mbot), k, 1) != 0) {
            if ((k == 0 || (k > 0 && cptr.ld1so(cptr.decay(mbot), (k - 1) | 0, 1) == 32)) && 97 <= cptr.ld1so(cptr.decay(mbot), k, 1) && cptr.ld1so(cptr.decay(mbot), k, 1) <= 122)
                cptr.st1o(cptr.decay(mbot), k, cptr.ld1so(cptr.decay(mbot), k, 1) + -32, 1);
            k++;
        }
        void cptr.strcpy(nb = eos(nb), cptr.decay(mbot));
    } else {
        void cptr.strcpy(nb = eos(nb), rank());
    }
    void cptr.sprintf(nb = eos(nb), __s_sp2);
    i = (cptr.ldI32o(gm, $instance_globals_m_mrank_sz) + 15) | 0;
    j = Number(BigInt.asIntN(32, (cptr.diff((cptr.add(nb, 2)), cptr.decay(__static_do_statusline1_newbot1)))));
    if (((i - j) | 0) > 0)
        void cptr.sprintf(nb = eos(nb), __s_pct_star_s, (i - j) | 0, __s_sp);
    void cptr.sprintf(nb = eos(nb), __s_st_s_dx_1d_co_1d_in_1d_wi_1d_ch_1d, get_strength_str(), (acurr(NHC.A_DEX)), (acurr(NHC.A_CON)), (acurr(NHC.A_INT)), (acurr(NHC.A_WIS)), (acurr(NHC.A_CHA)));
    void cptr.sprintf(nb = eos(nb), __s_pct_s, (cptr.ld1so(u, $you_ualign) == -1) ? __s_chaotic : ((cptr.ld1so(u, $you_ualign) == NHM.A_NEUTRAL) ? __s_neutral : __s_lawful));
    return cptr.decay(__static_do_statusline1_newbot1);
}

const __static_do_statusline2_newbot2 = new Uint8Array(256); /** C ref: botl.c:103 — char[256] (function-static) */
const __static_do_statusline2_dloc = new Uint8Array(128); /** C ref: botl.c:107 — char[128] (function-static) */
const __static_do_statusline2_hlth = new Uint8Array(128); /** C ref: botl.c:107 — char[128] (function-static) */
const __static_do_statusline2_expr = new Uint8Array(128); /** C ref: botl.c:107 — char[128] (function-static) */
const __static_do_statusline2_tmmv = new Uint8Array(128); /** C ref: botl.c:108 — char[128] (function-static) */
const __static_do_statusline2_cond = new Uint8Array(128); /** C ref: botl.c:108 — char[128] (function-static) */
const __static_do_statusline2_vers = new Uint8Array(128); /** C ref: botl.c:108 — char[128] (function-static) */

/** C ref: botl.c:101 @returns {CPtr<char>} */
export function do_statusline2() {
    let nb;
    let dln;
    let dx;
    let hln;
    let xln;
    let tln;
    let cln;
    let vrn;
    let hp;
    let hpmax;
    let cap;
    let money;
    if (suppress_map_output())
        return cptr.strcpy(cptr.decay(__static_do_statusline2_newbot2), __s_empty);
    void describe_level(cptr.decay(__static_do_statusline2_dloc), 1);
    if ((money = money_cnt(cptr.ldPtro(gi, $instance_globals_i_invent))) < 0n)
        money = 0n;
    void cptr.sprintf(eos(cptr.decay(__static_do_statusline2_dloc)), __s_s_2ld, (cptr.ld1so(iflags, $instance_flags_in_dumplog) || cptr.ld1so(iflags, $instance_flags_invis_goldsym)) ? __s_dollar : encglyph((((NHC.GOLD_PIECE) + NHC.GLYPH_OBJ_OFF) | 0)), ((money) < 999999n ? (money) : 999999n));
    dln = cptr.strlen(cptr.decay(__static_do_statusline2_dloc));
    dx = BigInt.asUintN(64, BigInt((strstri(cptr.decay(__static_do_statusline2_dloc), __s_bslash_g) ? 9 : 0)));
    hp = Upolyd() ? cptr.ldI32o(u, $you_mh) : cptr.ldI32o(u, $you_uhp);
    hpmax = Upolyd() ? cptr.ldI32o(u, $you_mhmax) : cptr.ldI32o(u, $you_uhpmax);
    if (hp < 0)
        hp = 0;
    void cptr.sprintf(cptr.decay(__static_do_statusline2_hlth), __s_hp_d_d_pw_d_d_ac_2d, ((hp) < 9999 ? (hp) : 9999), ((hpmax) < 9999 ? (hpmax) : 9999), ((cptr.ldI32o(u, $you_uen)) < 9999 ? (cptr.ldI32o(u, $you_uen)) : 9999), ((cptr.ldI32o(u, $you_uenmax)) < 9999 ? (cptr.ldI32o(u, $you_uenmax)) : 9999), cptr.ld1so(u, $you_uac));
    hln = cptr.strlen(cptr.decay(__static_do_statusline2_hlth));
    if (Upolyd())
        void cptr.sprintf(cptr.decay(__static_do_statusline2_expr), __s_hd_d, cptr.ld1so2(mons, cptr.ldI32o(u, $you_umonnum), $sizeof_permonst, $permonst_mlevel));
    else if (cptr.ld1so(flags, $flag_showexp))
        void cptr.sprintf(cptr.decay(__static_do_statusline2_expr), __s_xp_d_1ld, cptr.ldI32o(u, $you_ulevel), cptr.ldI64o(u, $you_uexp));
    else
        void cptr.sprintf(cptr.decay(__static_do_statusline2_expr), __s_xp_d, cptr.ldI32o(u, $you_ulevel));
    xln = cptr.strlen(cptr.decay(__static_do_statusline2_expr));
    if (cptr.ld1so(flags, $flag_time))
        void cptr.sprintf(cptr.decay(__static_do_statusline2_tmmv), __s_t_ld, cptr.ldI64o(svm, $instance_globals_saved_m_moves));
    else
        cptr.st1o(cptr.decay(__static_do_statusline2_tmmv), 0, 0, 1);
    tln = cptr.strlen(cptr.decay(__static_do_statusline2_tmmv));
    cptr.st1o(cptr.decay(__static_do_statusline2_cond), 0, 0, 1);
    nb = cptr.decay(__static_do_statusline2_cond);
    if (Stoned())
        void cptr.strcpy(nb = eos(nb), __s_stone);
    if (Slimed())
        void cptr.strcpy(nb = eos(nb), __s_slime);
    if (Strangled())
        void cptr.strcpy(nb = eos(nb), __s_strngl);
    if (Sick()) {
        if (((cptr.ldI32o(u, $you_usick_type) & 3) | 0) & NHM.SICK_VOMITABLE)
            void cptr.strcpy(nb = eos(nb), __s_foodpois);
        if (((cptr.ldI32o(u, $you_usick_type) & 3) | 0) & NHM.SICK_NONVOMITABLE)
            void cptr.strcpy(nb = eos(nb), __s_termill);
    }
    if (cptr.ldI32o(u, $you_uhs) != NHC.NOT_HUNGRY)
        void cptr.sprintf(nb = eos(nb), __s_sp_pct_s, cptr.ldPtro(hu_stat, cptr.ldI32o(u, $you_uhs), 8));
    if ((cap = near_capacity()) > NHC.UNENCUMBERED)
        void cptr.sprintf(nb = eos(nb), __s_sp_pct_s, cptr.ldPtro(enc_stat, cap, 8));
    if (Blind())
        void cptr.strcpy(nb = eos(nb), __s_blind);
    if (Deaf())
        void cptr.strcpy(nb = eos(nb), __s_deaf);
    if (HStun())
        void cptr.strcpy(nb = eos(nb), __s_stun);
    if (HConfusion())
        void cptr.strcpy(nb = eos(nb), __s_conf);
    if (Hallucination())
        void cptr.strcpy(nb = eos(nb), __s_hallu);
    if (Levitation())
        void cptr.strcpy(nb = eos(nb), __s_lev);
    if (Flying())
        void cptr.strcpy(nb = eos(nb), __s_fly);
    if (cptr.ldPtro(u, $you_usteed))
        void cptr.strcpy(nb = eos(nb), __s_ride);
    cln = cptr.strlen(cptr.decay(__static_do_statusline2_cond));
    if (cptr.ld1so(flags, $flag_showvers))
        void status_version(cptr.decay(__static_do_statusline2_vers), 128n, 1);
    else
        cptr.st1o(cptr.decay(__static_do_statusline2_vers), 0, 0, 1);
    vrn = cptr.strlen(cptr.decay(__static_do_statusline2_vers));
    if (BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, (BigInt.asUintN(64, dln - dx)) + 1n) + hln) + 1n) + xln) + 1n) + tln) + 1n) + cln) + vrn) <= 80n) {
        nh_snprintf(__s_do_statusline2, 229, cptr.decay(__static_do_statusline2_newbot2), 256n, __s_s_s_s_s_s_s, cptr.decay(__static_do_statusline2_dloc), cptr.decay(__static_do_statusline2_hlth), cptr.decay(__static_do_statusline2_expr), cptr.decay(__static_do_statusline2_tmmv), cptr.decay(__static_do_statusline2_cond), cptr.decay(__static_do_statusline2_vers));
    } else {
        if (BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, dln + 1n) + hln) + 1n) + xln) + 1n) + tln) + 1n) + cln) + vrn) > 200n) {
            panic(__s_bot2_second_status_line_exceeds_maxco_u, Number(BigInt.asUintN(32, (BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, dln + 1n) + hln) + 1n) + xln) + 1n) + tln) + 1n) + cln) + vrn)))), NHM.MAXCO);
        } else if (BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, (BigInt.asUintN(64, dln - dx)) + 1n) + hln) + 1n) + xln) + 1n) + cln) <= 80n) {
            nh_snprintf(__s_do_statusline2, 238, cptr.decay(__static_do_statusline2_newbot2), 256n, __s_s_s_s_s_s_s, cptr.decay(__static_do_statusline2_dloc), cptr.decay(__static_do_statusline2_hlth), cptr.decay(__static_do_statusline2_expr), cptr.decay(__static_do_statusline2_cond), cptr.decay(__static_do_statusline2_tmmv), cptr.decay(__static_do_statusline2_vers));
        } else if (BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, (BigInt.asUintN(64, dln - dx)) + 1n) + hln) + 1n) + cln) <= 80n) {
            nh_snprintf(__s_do_statusline2, 241, cptr.decay(__static_do_statusline2_newbot2), 256n, __s_s_s_s_s_s_s, cptr.decay(__static_do_statusline2_dloc), cptr.decay(__static_do_statusline2_hlth), cptr.decay(__static_do_statusline2_cond), cptr.decay(__static_do_statusline2_expr), cptr.decay(__static_do_statusline2_tmmv), cptr.decay(__static_do_statusline2_vers));
        } else {
            nh_snprintf(__s_do_statusline2, 244, cptr.decay(__static_do_statusline2_newbot2), 256n, __s_s_s_s_s_s_s, cptr.decay(__static_do_statusline2_hlth), cptr.decay(__static_do_statusline2_cond), cptr.decay(__static_do_statusline2_dloc), cptr.decay(__static_do_statusline2_expr), cptr.decay(__static_do_statusline2_tmmv), cptr.decay(__static_do_statusline2_vers));
        }
        mungspaces(cptr.decay(__static_do_statusline2_newbot2));
    }
    return cptr.decay(__static_do_statusline2_newbot2);
}

/** C ref: botl.c:253 */
export function bot() {
    if (cptr.ld1so(gb, $instance_globals_b_bot_disabled))
        return;
    if (cptr.ldI32o(u, $you_uhp) != -1 && cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data) && cptr.ld1so(iflags, $instance_flags_status_updates) && !suppress_map_output()) {
        if (((cptr.ldU64o(windowprocs, $window_procs_wincap2) & 136n) != 0n)) {
            bot_via_windowport();
        } else {
            curs()(WIN_STATUS.v, 1, 0);
            putstr()(WIN_STATUS.v, 0, do_statusline1());
            curs()(WIN_STATUS.v, 1, 1);
            putmixed()(WIN_STATUS.v, 0, do_statusline2());
        }
    }
    cptr.st1(disp, cptr.st1o(disp, $display_hints_botlx, cptr.st1o(disp, $display_hints_time_botl, 0)));
}

/** C ref: botl.c:275 */
export function timebot() {
    if (cptr.ld1so(gb, $instance_globals_b_bot_disabled))
        return;
    if (cptr.ld1so(flags, $flag_time) && cptr.ld1so(iflags, $instance_flags_status_updates) && !suppress_map_output()) {
        if (((cptr.ldU64o(windowprocs, $window_procs_wincap2) & 136n) != 0n)) {
            stat_update_time();
        } else {
            bot();
        }
    }
    cptr.st1o(disp, $display_hints_time_botl, 0);
}

/** C ref: botl.c:298 — @param {CInt} xlev @returns {CInt} */
export function xlev_to_rank(xlev) {
    return (xlev <= 2) ? 0 : ((xlev <= 30) ? ((((xlev + 2) | 0) / 4) | 0) : 8);
}

/** C ref: botl.c:315 — @param {CInt} rank @returns {CInt} */
export function rank_to_xlev(rank) {
    return (rank < 1) ? 1 : ((rank < 2) ? 3 : ((rank < 8) ? (((Math.imul(rank, 4)) - 2) | 0) : 30));
}

/** C ref: botl.c:332 — @param {CInt} lev @param {CInt} monnum @param {CInt} female @returns {CPtr<char>} */
export function rank_of(lev, monnum, female) {
    let role;
    let i;
    for (role = roles; cptr.ldPtr(role); role = cptr.add(role, 1, 312))
        if (monnum == cptr.ldI16o(role, $Role_mnum))
            break;
    if (!cptr.ldPtr(role))
        role = cptr.add(gu, $instance_globals_u_urole);
    for (i = xlev_to_rank(lev); i >= 0; i--) {
        if (female && cptr.ldPtro2(role, i, $sizeof_RoleName, $Role_rank + $RoleName_f))
            return cptr.ldPtro2(role, i, $sizeof_RoleName, $Role_rank + $RoleName_f);
        if (cptr.ldPtro2(role, i, $sizeof_RoleName, $Role_rank))
            return cptr.ldPtro2(role, i, $sizeof_RoleName, $Role_rank);
    }
    if (female && cptr.ldPtro(role, $RoleName_f))
        return cptr.ldPtro(role, $RoleName_f);
    else if (cptr.ldPtr(role))
        return cptr.ldPtr(role);
    return __s_player;
}

/** C ref: botl.c:361 @returns {CPtr<char>} */
function rank() {
    return rank_of(cptr.ldI32o(u, $you_ulevel), Role_switch(), cptr.ld1so(flags, $flag_female));
}

/** C ref: botl.c:367 — @param {CPtr<char>} str @param {CPtr<int>} rank_indx @param {CPtr<int>} title_length @returns {CInt} */
export function title_to_mon(str, rank_indx, title_length) {
    let i;
    let j;
    for (i = 0; cptr.ldPtro(roles, i, $sizeof_Role); i++) {
        for (j = 0; j < 9; j++) {
            if (cptr.ldPtro3(roles, i, $sizeof_Role, j, $sizeof_RoleName, $Role_rank) && str_start_is(str, cptr.ldPtro3(roles, i, $sizeof_Role, j, $sizeof_RoleName, $Role_rank), 1)) {
                if (rank_indx)
                    cptr.stI32(rank_indx, j);
                if (title_length)
                    cptr.stI32(title_length, Strlen_(cptr.ldPtro3(roles, i, $sizeof_Role, j, $sizeof_RoleName, $Role_rank), __s_title_to_mon, 383) | 0);
                return cptr.ldI16o2(roles, i, $sizeof_Role, $Role_mnum);
            }
            if (cptr.ldPtro3(roles, i, $sizeof_Role, j, $sizeof_RoleName, $Role_rank + $RoleName_f) && str_start_is(str, cptr.ldPtro3(roles, i, $sizeof_Role, j, $sizeof_RoleName, $Role_rank + $RoleName_f), 1)) {
                if (rank_indx)
                    cptr.stI32(rank_indx, j);
                if (title_length)
                    cptr.stI32(title_length, Strlen_(cptr.ldPtro3(roles, i, $sizeof_Role, j, $sizeof_RoleName, $Role_rank + $RoleName_f), __s_title_to_mon, 391) | 0);
                return cptr.ldI16o2(roles, i, $sizeof_Role, $Role_mnum);
            }
        }
    }
    if (title_length)
        cptr.stI32(title_length, 0);
    return NHC.NON_PM;
}

/** C ref: botl.c:402 */
export function max_rank_sz() {
    let i;
    let r;
    let maxr = 0n;
    for (i = 0; i < 9; i++) {
        if (cptr.ldPtro2(gu, i, $sizeof_RoleName, $instance_globals_u_urole + $Role_rank) && (r = cptr.strlen(cptr.ldPtro2(gu, i, $sizeof_RoleName, $instance_globals_u_urole + $Role_rank))) > maxr)
            maxr = r;
        if (cptr.ldPtro2(gu, i, $sizeof_RoleName, $instance_globals_u_urole + $Role_rank + $RoleName_f) && (r = cptr.strlen(cptr.ldPtro2(gu, i, $sizeof_RoleName, $instance_globals_u_urole + $Role_rank + $RoleName_f))) > maxr)
            maxr = r;
    }
    cptr.stI32o(gm, $instance_globals_m_mrank_sz, Number(BigInt.asIntN(32, maxr)));
    return;
}

/** C ref: botl.c:441 — @param {CPtr<char>} buf @param {CInt} dflgs @returns {CInt} */
export function describe_level(buf, dflgs) {
    let addspace = schar(((dflgs & 1) != 0));
    let addbranch = schar(((dflgs & 2) != 0));
    let ret = 1;
    if ((((cptr.ldI16o((cptr.add(svd, $instance_globals_saved_d_dungeon_topology + $dgn_topology_d_knox_level)), $d_level_dlevel) || cptr.ldI16((cptr.add(svd, $instance_globals_saved_d_dungeon_topology + $dgn_topology_d_knox_level)))) && on_level(cptr.add(u, $you_uz), cptr.add(svd, $instance_globals_saved_d_dungeon_topology + $dgn_topology_d_knox_level))))) {
        void cptr.sprintf(buf, __s_pct_s, cptr.add(svd, cptr.ldI16o(u, $you_uz), $sizeof_dungeon));
        addbranch = 0;
    } else if (In_quest(cptr.add(u, $you_uz))) {
        void cptr.sprintf(buf, __s_home_d, dunlev(cptr.add(u, $you_uz)));
    } else if ((cptr.ldI16((cptr.add(u, $you_uz))) == cptr.ldI16((cptr.add(svd, $instance_globals_saved_d_dungeon_topology + $dgn_topology_d_astral_level))))) {
        void endgamelevelname(buf, depth(cptr.add(u, $you_uz)));
        if (!addbranch)
            void strsubst(buf, __s_plane_of, __s_empty);
        addbranch = 0;
    } else {
        if (!addbranch)
            void cptr.sprintf(buf, __s_s_2d, (cptr.ldI16((cptr.add(u, $you_uz))) == tutorial_dnum()) ? __s_tutorial : __s_dlvl, depth(cptr.add(u, $you_uz)));
        else
            void cptr.sprintf(buf, __s_level_d, depth(cptr.add(u, $you_uz)));
        ret = 0;
    }
    if (addbranch) {
        void cptr.sprintf(eos(buf), __s_comma_sp_pct_s, cptr.add(svd, cptr.ldI16o(u, $you_uz), $sizeof_dungeon));
        void strsubst(buf, __s_the__2, __s_the__3);
    }
    if (addspace)
        void cptr.strcat(buf, __s_sp);
    return ret;
}

/** C ref: botl.c:481 — @param {CPtr<char>} outbuf @returns {CPtr<char>} */
export function weapon_status(outbuf) {
    let res = null;
    cptr.st1(outbuf, 0);
    if (!uwep.v) {
        res = uarmg.v ? __s_empty_hnd : (((cptr.ldU64o((cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data)), $permonst_mflags1) & 131072n) != 0n) ? __s_bare_hnds : __s_no_weapon);
    } else if (cptr.ld1so(u, $you_twoweap)) {
        res = __s_dual_weps;
        if (cptr.ldPtro(u, $you_usteed) && (weapon_type(uwep.v) == NHC.P_LANCE || weapon_type(uswapwep.v) == NHC.P_LANCE))
            res = __s_dual_joust;
    } else {
        let p;
        let skill = weapon_type(uwep.v);
        if (cptr.ldPtro(u, $you_usteed) && skill == NHC.P_LANCE) {
            res = __s_joust;
        } else if (cptr.ldI16o(uwep.v, $obj_otyp) == NHC.AKLYS) {
            res = __s_aklys;
        } else if (is_sword(uwep.v)) {
            res = __s_sword;
        } else {
            switch (skill) {
                case NHC.P_QUARTERSTAFF:
                res = __s_staff;
                break;
                case NHC.P_MORNING_STAR:
                res = __s_mrng_star;
                break;
                case NHC.P_POLEARMS:
                res = __s_pole;
                break;
                case NHC.P_UNICORN_HORN:
                res = __s_unihorn;
                break;
                default:
                res = weapon_descr(uwep.v);
                if (!strncmpi((res), (__s_food), -1) && cptr.ldI16o(uwep.v, $obj_otyp) == NHC.CREAM_PIE)
                    res = __s_pie;
                break;
            }
        }
        if ((cptr.ld1so(uwep.v, $obj_oclass) == NHC.WEAPON_CLASS || is_weptool(uwep.v)) && bimanual(uwep.v) && cptr.ld1s(res) != 50 && strncmpi(res, __s_two, 3))
            void cptr.strcat(outbuf, __s_2h);
        void cptr.strcpy(p = eos(outbuf), res), res = outbuf;
        cptr.st1(p, highc(cptr.ld1s(p)));
        void strNsubst(outbuf, __s_sp, __s_dash, 0);
    }
    return (cptr.eq(outbuf, res)) ? outbuf : cptr.strcpy(outbuf, res);
}

/** C ref: botl.c:559 — @param {CPtr<char>} armbuf @returns {CPtr<char>} */
export function armor_status(armbuf) {
    let n = (((((((((((!!uarmg.v + !!uarmc.v) | 0) + !!uarm.v) | 0) + !!uarmu.v) | 0) + !!uarmh.v) | 0) + !!uarmf.v) | 0) + !!uarms.v) | 0;
    if (n == 0) {
        void cptr.strcpy(armbuf, __s_naked);
    } else if (n == 1) {
        void cptr.strcpy(armbuf, uarmg.v ? __s_gloves : (uarmc.v ? __s_cloak : (uarm.v ? __s_suit : (uarmu.v ? __s_shirt : (uarmh.v ? helm_simple_name(uarmh.v) : (uarmf.v ? __s_boots : (uarms.v ? __s_shield : __s_empty)))))));
    } else {
        let p = armbuf;
        if (uarmg.v)
            cptr.st1(cptr.postinc(() => p, (v) => { p = v; }), 71);
        if (uarmc.v)
            cptr.st1(cptr.postinc(() => p, (v) => { p = v; }), 67);
        if (uarm.v)
            cptr.st1(cptr.postinc(() => p, (v) => { p = v; }), 65);
        if (uarmu.v)
            cptr.st1(cptr.postinc(() => p, (v) => { p = v; }), 85);
        if (uarmh.v)
            cptr.st1(cptr.postinc(() => p, (v) => { p = v; }), 72);
        if (uarmf.v)
            cptr.st1(cptr.postinc(() => p, (v) => { p = v; }), 66);
        if (uarms.v)
            cptr.st1(cptr.postinc(() => p, (v) => { p = v; }), 83);
        cptr.st1(p, 0);
    }
    if ((uright.v && cptr.ldI16o(uright.v, $obj_otyp) == NHC.RIN_PROTECTION) || (uleft.v && cptr.ldI16o(uleft.v, $obj_otyp) == NHC.RIN_PROTECTION) || (uamul.v && cptr.ldI16o(uamul.v, $obj_otyp) == NHC.AMULET_OF_GUARDING) || (uarmc.v && cptr.ldI16o(uarmc.v, $obj_otyp) == NHC.CLOAK_OF_PROTECTION) || (uarmh.v && cptr.ld1so(uarmh.v, $obj_oartifact) == NHC.ART_MITRE_OF_HOLINESS) || (uwep.v && cptr.ld1so(uwep.v, $obj_oartifact) == NHC.ART_TSURUGI_OF_MURAMASA))
        void strkitten(armbuf, 43);
    return upstart(armbuf);
}

/** C ref: botl.c:703 — struct istat_s[27] */
const initblstats = cptr.alloc(27 * $sizeof_istat_s);
cptr.stPtro(initblstats, 0, __s_title);
cptr.stPtro(initblstats, 0 + $istat_s_fldfmt, __s_pct_s);
cptr.stI64o(initblstats, 0 + $istat_s_time, 0n);
cptr.st1o(initblstats, 0 + $istat_s_chg, 0);
cptr.st1o(initblstats, 0 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 0 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 0 + $istat_s_anytype, NHC.ANY_STR);
cptr.stPtro(initblstats, 0 + $istat_s_a, null);
cptr.stPtro(initblstats, 0 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 0 + $istat_s_val, null);
cptr.stI32o(initblstats, 0 + $istat_s_valwidth, NHM.MAXVALWIDTH);
cptr.stI32o(initblstats, 0 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 0 + $istat_s_fld, NHC.BL_TITLE);
cptr.stPtro(initblstats, 0 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 0 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 88, __s_strength);
cptr.stPtro(initblstats, 88 + $istat_s_fldfmt, __s_st_s);
cptr.stI64o(initblstats, 88 + $istat_s_time, 0n);
cptr.st1o(initblstats, 88 + $istat_s_chg, 0);
cptr.st1o(initblstats, 88 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 88 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 88 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 88 + $istat_s_a, null);
cptr.stPtro(initblstats, 88 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 88 + $istat_s_val, null);
cptr.stI32o(initblstats, 88 + $istat_s_valwidth, 10);
cptr.stI32o(initblstats, 88 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 88 + $istat_s_fld, NHC.BL_STR);
cptr.stPtro(initblstats, 88 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 88 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 176, __s_dexterity);
cptr.stPtro(initblstats, 176 + $istat_s_fldfmt, __s_dx_s);
cptr.stI64o(initblstats, 176 + $istat_s_time, 0n);
cptr.st1o(initblstats, 176 + $istat_s_chg, 0);
cptr.st1o(initblstats, 176 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 176 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 176 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 176 + $istat_s_a, null);
cptr.stPtro(initblstats, 176 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 176 + $istat_s_val, null);
cptr.stI32o(initblstats, 176 + $istat_s_valwidth, 10);
cptr.stI32o(initblstats, 176 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 176 + $istat_s_fld, NHC.BL_DX);
cptr.stPtro(initblstats, 176 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 176 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 264, __s_constitution);
cptr.stPtro(initblstats, 264 + $istat_s_fldfmt, __s_co_s);
cptr.stI64o(initblstats, 264 + $istat_s_time, 0n);
cptr.st1o(initblstats, 264 + $istat_s_chg, 0);
cptr.st1o(initblstats, 264 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 264 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 264 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 264 + $istat_s_a, null);
cptr.stPtro(initblstats, 264 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 264 + $istat_s_val, null);
cptr.stI32o(initblstats, 264 + $istat_s_valwidth, 10);
cptr.stI32o(initblstats, 264 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 264 + $istat_s_fld, NHC.BL_CO);
cptr.stPtro(initblstats, 264 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 264 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 352, __s_intelligence);
cptr.stPtro(initblstats, 352 + $istat_s_fldfmt, __s_in_s);
cptr.stI64o(initblstats, 352 + $istat_s_time, 0n);
cptr.st1o(initblstats, 352 + $istat_s_chg, 0);
cptr.st1o(initblstats, 352 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 352 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 352 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 352 + $istat_s_a, null);
cptr.stPtro(initblstats, 352 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 352 + $istat_s_val, null);
cptr.stI32o(initblstats, 352 + $istat_s_valwidth, 10);
cptr.stI32o(initblstats, 352 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 352 + $istat_s_fld, NHC.BL_IN);
cptr.stPtro(initblstats, 352 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 352 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 440, __s_wisdom);
cptr.stPtro(initblstats, 440 + $istat_s_fldfmt, __s_wi_s);
cptr.stI64o(initblstats, 440 + $istat_s_time, 0n);
cptr.st1o(initblstats, 440 + $istat_s_chg, 0);
cptr.st1o(initblstats, 440 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 440 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 440 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 440 + $istat_s_a, null);
cptr.stPtro(initblstats, 440 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 440 + $istat_s_val, null);
cptr.stI32o(initblstats, 440 + $istat_s_valwidth, 10);
cptr.stI32o(initblstats, 440 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 440 + $istat_s_fld, NHC.BL_WI);
cptr.stPtro(initblstats, 440 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 440 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 528, __s_charisma);
cptr.stPtro(initblstats, 528 + $istat_s_fldfmt, __s_ch_s);
cptr.stI64o(initblstats, 528 + $istat_s_time, 0n);
cptr.st1o(initblstats, 528 + $istat_s_chg, 0);
cptr.st1o(initblstats, 528 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 528 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 528 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 528 + $istat_s_a, null);
cptr.stPtro(initblstats, 528 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 528 + $istat_s_val, null);
cptr.stI32o(initblstats, 528 + $istat_s_valwidth, 10);
cptr.stI32o(initblstats, 528 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 528 + $istat_s_fld, NHC.BL_CH);
cptr.stPtro(initblstats, 528 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 528 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 616, __s_alignment);
cptr.stPtro(initblstats, 616 + $istat_s_fldfmt, __s_sp_pct_s);
cptr.stI64o(initblstats, 616 + $istat_s_time, 0n);
cptr.st1o(initblstats, 616 + $istat_s_chg, 0);
cptr.st1o(initblstats, 616 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 616 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 616 + $istat_s_anytype, NHC.ANY_STR);
cptr.stPtro(initblstats, 616 + $istat_s_a, null);
cptr.stPtro(initblstats, 616 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 616 + $istat_s_val, null);
cptr.stI32o(initblstats, 616 + $istat_s_valwidth, 20);
cptr.stI32o(initblstats, 616 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 616 + $istat_s_fld, NHC.BL_ALIGN);
cptr.stPtro(initblstats, 616 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 616 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 704, __s_score);
cptr.stPtro(initblstats, 704 + $istat_s_fldfmt, __s_s_s);
cptr.stI64o(initblstats, 704 + $istat_s_time, 0n);
cptr.st1o(initblstats, 704 + $istat_s_chg, 0);
cptr.st1o(initblstats, 704 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 704 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 704 + $istat_s_anytype, NHC.ANY_LONG);
cptr.stPtro(initblstats, 704 + $istat_s_a, null);
cptr.stPtro(initblstats, 704 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 704 + $istat_s_val, null);
cptr.stI32o(initblstats, 704 + $istat_s_valwidth, 30);
cptr.stI32o(initblstats, 704 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 704 + $istat_s_fld, NHC.BL_SCORE);
cptr.stPtro(initblstats, 704 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 704 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 792, __s_carrying_capacity);
cptr.stPtro(initblstats, 792 + $istat_s_fldfmt, __s_sp_pct_s);
cptr.stI64o(initblstats, 792 + $istat_s_time, 0n);
cptr.st1o(initblstats, 792 + $istat_s_chg, 0);
cptr.st1o(initblstats, 792 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 792 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 792 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 792 + $istat_s_a, null);
cptr.stPtro(initblstats, 792 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 792 + $istat_s_val, null);
cptr.stI32o(initblstats, 792 + $istat_s_valwidth, 20);
cptr.stI32o(initblstats, 792 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 792 + $istat_s_fld, NHC.BL_CAP);
cptr.stPtro(initblstats, 792 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 792 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 880, __s_gold);
cptr.stPtro(initblstats, 880 + $istat_s_fldfmt, __s_sp_pct_s);
cptr.stI64o(initblstats, 880 + $istat_s_time, 0n);
cptr.st1o(initblstats, 880 + $istat_s_chg, 0);
cptr.st1o(initblstats, 880 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 880 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 880 + $istat_s_anytype, NHC.ANY_LONG);
cptr.stPtro(initblstats, 880 + $istat_s_a, null);
cptr.stPtro(initblstats, 880 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 880 + $istat_s_val, null);
cptr.stI32o(initblstats, 880 + $istat_s_valwidth, 40);
cptr.stI32o(initblstats, 880 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 880 + $istat_s_fld, NHC.BL_GOLD);
cptr.stPtro(initblstats, 880 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 880 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 968, __s_power);
cptr.stPtro(initblstats, 968 + $istat_s_fldfmt, __s_pw_s);
cptr.stI64o(initblstats, 968 + $istat_s_time, 0n);
cptr.st1o(initblstats, 968 + $istat_s_chg, 0);
cptr.st1o(initblstats, 968 + $istat_s_percent_matters, 1);
cptr.stI16o(initblstats, 968 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 968 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 968 + $istat_s_a, null);
cptr.stPtro(initblstats, 968 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 968 + $istat_s_val, null);
cptr.stI32o(initblstats, 968 + $istat_s_valwidth, 10);
cptr.stI32o(initblstats, 968 + $istat_s_idxmax, NHC.BL_ENEMAX);
cptr.stI32o(initblstats, 968 + $istat_s_fld, NHC.BL_ENE);
cptr.stPtro(initblstats, 968 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 968 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 1056, __s_power_max);
cptr.stPtro(initblstats, 1056 + $istat_s_fldfmt, __s_lparen_pct_s_rparen);
cptr.stI64o(initblstats, 1056 + $istat_s_time, 0n);
cptr.st1o(initblstats, 1056 + $istat_s_chg, 0);
cptr.st1o(initblstats, 1056 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 1056 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 1056 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 1056 + $istat_s_a, null);
cptr.stPtro(initblstats, 1056 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 1056 + $istat_s_val, null);
cptr.stI32o(initblstats, 1056 + $istat_s_valwidth, 10);
cptr.stI32o(initblstats, 1056 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 1056 + $istat_s_fld, NHC.BL_ENEMAX);
cptr.stPtro(initblstats, 1056 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 1056 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 1144, __s_experience_level);
cptr.stPtro(initblstats, 1144 + $istat_s_fldfmt, __s_xp_s);
cptr.stI64o(initblstats, 1144 + $istat_s_time, 0n);
cptr.st1o(initblstats, 1144 + $istat_s_chg, 0);
cptr.st1o(initblstats, 1144 + $istat_s_percent_matters, 1);
cptr.stI16o(initblstats, 1144 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 1144 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 1144 + $istat_s_a, null);
cptr.stPtro(initblstats, 1144 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 1144 + $istat_s_val, null);
cptr.stI32o(initblstats, 1144 + $istat_s_valwidth, 10);
cptr.stI32o(initblstats, 1144 + $istat_s_idxmax, NHC.BL_EXP);
cptr.stI32o(initblstats, 1144 + $istat_s_fld, NHC.BL_XP);
cptr.stPtro(initblstats, 1144 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 1144 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 1232, __s_armor_class);
cptr.stPtro(initblstats, 1232 + $istat_s_fldfmt, __s_ac_s);
cptr.stI64o(initblstats, 1232 + $istat_s_time, 0n);
cptr.st1o(initblstats, 1232 + $istat_s_chg, 0);
cptr.st1o(initblstats, 1232 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 1232 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 1232 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 1232 + $istat_s_a, null);
cptr.stPtro(initblstats, 1232 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 1232 + $istat_s_val, null);
cptr.stI32o(initblstats, 1232 + $istat_s_valwidth, 10);
cptr.stI32o(initblstats, 1232 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 1232 + $istat_s_fld, NHC.BL_AC);
cptr.stPtro(initblstats, 1232 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 1232 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 1320, __s_hd);
cptr.stPtro(initblstats, 1320 + $istat_s_fldfmt, __s_hd_s);
cptr.stI64o(initblstats, 1320 + $istat_s_time, 0n);
cptr.st1o(initblstats, 1320 + $istat_s_chg, 0);
cptr.st1o(initblstats, 1320 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 1320 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 1320 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 1320 + $istat_s_a, null);
cptr.stPtro(initblstats, 1320 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 1320 + $istat_s_val, null);
cptr.stI32o(initblstats, 1320 + $istat_s_valwidth, 10);
cptr.stI32o(initblstats, 1320 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 1320 + $istat_s_fld, NHC.BL_HD);
cptr.stPtro(initblstats, 1320 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 1320 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 1408, __s_time);
cptr.stPtro(initblstats, 1408 + $istat_s_fldfmt, __s_t_s);
cptr.stI64o(initblstats, 1408 + $istat_s_time, 0n);
cptr.st1o(initblstats, 1408 + $istat_s_chg, 0);
cptr.st1o(initblstats, 1408 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 1408 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 1408 + $istat_s_anytype, NHC.ANY_LONG);
cptr.stPtro(initblstats, 1408 + $istat_s_a, null);
cptr.stPtro(initblstats, 1408 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 1408 + $istat_s_val, null);
cptr.stI32o(initblstats, 1408 + $istat_s_valwidth, 30);
cptr.stI32o(initblstats, 1408 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 1408 + $istat_s_fld, NHC.BL_TIME);
cptr.stPtro(initblstats, 1408 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 1408 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 1496, __s_hunger);
cptr.stPtro(initblstats, 1496 + $istat_s_fldfmt, __s_sp_pct_s);
cptr.stI64o(initblstats, 1496 + $istat_s_time, 0n);
cptr.st1o(initblstats, 1496 + $istat_s_chg, 0);
cptr.st1o(initblstats, 1496 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 1496 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 1496 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 1496 + $istat_s_a, null);
cptr.stPtro(initblstats, 1496 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 1496 + $istat_s_val, null);
cptr.stI32o(initblstats, 1496 + $istat_s_valwidth, 20);
cptr.stI32o(initblstats, 1496 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 1496 + $istat_s_fld, NHC.BL_HUNGER);
cptr.stPtro(initblstats, 1496 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 1496 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 1584, __s_hitpoints);
cptr.stPtro(initblstats, 1584 + $istat_s_fldfmt, __s_hp_s);
cptr.stI64o(initblstats, 1584 + $istat_s_time, 0n);
cptr.st1o(initblstats, 1584 + $istat_s_chg, 0);
cptr.st1o(initblstats, 1584 + $istat_s_percent_matters, 1);
cptr.stI16o(initblstats, 1584 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 1584 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 1584 + $istat_s_a, null);
cptr.stPtro(initblstats, 1584 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 1584 + $istat_s_val, null);
cptr.stI32o(initblstats, 1584 + $istat_s_valwidth, 10);
cptr.stI32o(initblstats, 1584 + $istat_s_idxmax, NHC.BL_HPMAX);
cptr.stI32o(initblstats, 1584 + $istat_s_fld, NHC.BL_HP);
cptr.stPtro(initblstats, 1584 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 1584 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 1672, __s_hitpoints_max);
cptr.stPtro(initblstats, 1672 + $istat_s_fldfmt, __s_lparen_pct_s_rparen);
cptr.stI64o(initblstats, 1672 + $istat_s_time, 0n);
cptr.st1o(initblstats, 1672 + $istat_s_chg, 0);
cptr.st1o(initblstats, 1672 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 1672 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 1672 + $istat_s_anytype, NHC.ANY_INT);
cptr.stPtro(initblstats, 1672 + $istat_s_a, null);
cptr.stPtro(initblstats, 1672 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 1672 + $istat_s_val, null);
cptr.stI32o(initblstats, 1672 + $istat_s_valwidth, 10);
cptr.stI32o(initblstats, 1672 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 1672 + $istat_s_fld, NHC.BL_HPMAX);
cptr.stPtro(initblstats, 1672 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 1672 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 1760, __s_dungeon_level);
cptr.stPtro(initblstats, 1760 + $istat_s_fldfmt, __s_pct_s);
cptr.stI64o(initblstats, 1760 + $istat_s_time, 0n);
cptr.st1o(initblstats, 1760 + $istat_s_chg, 0);
cptr.st1o(initblstats, 1760 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 1760 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 1760 + $istat_s_anytype, NHC.ANY_STR);
cptr.stPtro(initblstats, 1760 + $istat_s_a, null);
cptr.stPtro(initblstats, 1760 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 1760 + $istat_s_val, null);
cptr.stI32o(initblstats, 1760 + $istat_s_valwidth, NHM.MAXVALWIDTH);
cptr.stI32o(initblstats, 1760 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 1760 + $istat_s_fld, NHC.BL_LEVELDESC);
cptr.stPtro(initblstats, 1760 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 1760 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 1848, __s_experience);
cptr.stPtro(initblstats, 1848 + $istat_s_fldfmt, __s_slash_pct_s);
cptr.stI64o(initblstats, 1848 + $istat_s_time, 0n);
cptr.st1o(initblstats, 1848 + $istat_s_chg, 0);
cptr.st1o(initblstats, 1848 + $istat_s_percent_matters, 1);
cptr.stI16o(initblstats, 1848 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 1848 + $istat_s_anytype, NHC.ANY_LONG);
cptr.stPtro(initblstats, 1848 + $istat_s_a, null);
cptr.stPtro(initblstats, 1848 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 1848 + $istat_s_val, null);
cptr.stI32o(initblstats, 1848 + $istat_s_valwidth, 30);
cptr.stI32o(initblstats, 1848 + $istat_s_idxmax, NHC.BL_EXP);
cptr.stI32o(initblstats, 1848 + $istat_s_fld, NHC.BL_EXP);
cptr.stPtro(initblstats, 1848 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 1848 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 1936, __s_condition);
cptr.stPtro(initblstats, 1936 + $istat_s_fldfmt, __s_pct_s);
cptr.stI64o(initblstats, 1936 + $istat_s_time, 0n);
cptr.st1o(initblstats, 1936 + $istat_s_chg, 0);
cptr.st1o(initblstats, 1936 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 1936 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 1936 + $istat_s_anytype, NHC.ANY_MASK32);
cptr.stPtro(initblstats, 1936 + $istat_s_a, null);
cptr.stPtro(initblstats, 1936 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 1936 + $istat_s_val, null);
cptr.stI32o(initblstats, 1936 + $istat_s_valwidth, 0);
cptr.stI32o(initblstats, 1936 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 1936 + $istat_s_fld, NHC.BL_CONDITION);
cptr.stPtro(initblstats, 1936 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 1936 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 2024, __s_version);
cptr.stPtro(initblstats, 2024 + $istat_s_fldfmt, __s_sp_pct_s);
cptr.stI64o(initblstats, 2024 + $istat_s_time, 0n);
cptr.st1o(initblstats, 2024 + $istat_s_chg, 0);
cptr.st1o(initblstats, 2024 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 2024 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 2024 + $istat_s_anytype, NHC.ANY_STR);
cptr.stPtro(initblstats, 2024 + $istat_s_a, null);
cptr.stPtro(initblstats, 2024 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 2024 + $istat_s_val, null);
cptr.stI32o(initblstats, 2024 + $istat_s_valwidth, NHM.MAXVALWIDTH);
cptr.stI32o(initblstats, 2024 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 2024 + $istat_s_fld, NHC.BL_VERS);
cptr.stPtro(initblstats, 2024 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 2024 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 2112, __s_weapon);
cptr.stPtro(initblstats, 2112 + $istat_s_fldfmt, __s_sp_pct_s);
cptr.stI64o(initblstats, 2112 + $istat_s_time, 0n);
cptr.st1o(initblstats, 2112 + $istat_s_chg, 0);
cptr.st1o(initblstats, 2112 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 2112 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 2112 + $istat_s_anytype, NHC.ANY_STR);
cptr.stPtro(initblstats, 2112 + $istat_s_a, null);
cptr.stPtro(initblstats, 2112 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 2112 + $istat_s_val, null);
cptr.stI32o(initblstats, 2112 + $istat_s_valwidth, 20);
cptr.stI32o(initblstats, 2112 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 2112 + $istat_s_fld, NHC.BL_WEAPON);
cptr.stPtro(initblstats, 2112 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 2112 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 2200, __s_armor);
cptr.stPtro(initblstats, 2200 + $istat_s_fldfmt, __s_sp_pct_s);
cptr.stI64o(initblstats, 2200 + $istat_s_time, 0n);
cptr.st1o(initblstats, 2200 + $istat_s_chg, 0);
cptr.st1o(initblstats, 2200 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 2200 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 2200 + $istat_s_anytype, NHC.ANY_STR);
cptr.stPtro(initblstats, 2200 + $istat_s_a, null);
cptr.stPtro(initblstats, 2200 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 2200 + $istat_s_val, null);
cptr.stI32o(initblstats, 2200 + $istat_s_valwidth, 20);
cptr.stI32o(initblstats, 2200 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 2200 + $istat_s_fld, NHC.BL_ARMOR);
cptr.stPtro(initblstats, 2200 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 2200 + $istat_s_thresholds, null);
cptr.stPtro(initblstats, 2288, __s_terrain);
cptr.stPtro(initblstats, 2288 + $istat_s_fldfmt, __s_sp_pct_s);
cptr.stI64o(initblstats, 2288 + $istat_s_time, 0n);
cptr.st1o(initblstats, 2288 + $istat_s_chg, 0);
cptr.st1o(initblstats, 2288 + $istat_s_percent_matters, 0);
cptr.stI16o(initblstats, 2288 + $istat_s_percent_value, 0);
cptr.stI32o(initblstats, 2288 + $istat_s_anytype, NHC.ANY_STR);
cptr.stPtro(initblstats, 2288 + $istat_s_a, null);
cptr.stPtro(initblstats, 2288 + $istat_s_rawval, null);
cptr.stPtro(initblstats, 2288 + $istat_s_val, null);
cptr.stI32o(initblstats, 2288 + $istat_s_valwidth, 20);
cptr.stI32o(initblstats, 2288 + $istat_s_idxmax, -1);
cptr.stI32o(initblstats, 2288 + $istat_s_fld, NHC.BL_TERRAIN);
cptr.stPtro(initblstats, 2288 + $istat_s_hilite_rule, null);
cptr.stPtro(initblstats, 2288 + $istat_s_thresholds, null);

/** C ref: botl.c:749 — struct condmap[6] */
const condition_aliases = cptr.alloc(6 * $sizeof_condmap);
cptr.stPtro(condition_aliases, 0, __s_strangled);
cptr.stU64o(condition_aliases, 0 + $condmap_bitmask, 2097152n);
cptr.stPtro(condition_aliases, 16, __s_all);
cptr.stU64o(condition_aliases, 16 + $condmap_bitmask, 1073741823n);
cptr.stPtro(condition_aliases, 32, __s_major_troubles);
cptr.stU64o(condition_aliases, 32 + $condmap_bitmask, 20193920n);
cptr.stPtro(condition_aliases, 48, __s_minor_troubles);
cptr.stU64o(condition_aliases, 48 + $condmap_bitmask, 12616730n);
cptr.stPtro(condition_aliases, 64, __s_movement);
cptr.stU64o(condition_aliases, 64 + $condmap_bitmask, 81984n);
cptr.stPtro(condition_aliases, 80, __s_opt_in);
cptr.stU64o(condition_aliases, 80 + $condmap_bitmask, 1049270533n);

/** C ref: botl.c:781 — struct conditions_t[30] */
export const conditions = cptr.alloc(30 * $sizeof_conditions_t);
cptr.stI32o(conditions, 0, 20);
cptr.stI64o(conditions, 0 + $conditions_t_mask, 1n);
cptr.stI32o(conditions, 0 + $conditions_t_c, NHC.bl_bareh);
cptr.stPtro(conditions, 0 + $conditions_t_text + 0, __s_bare);
cptr.stPtro(conditions, 0 + $conditions_t_text + 8, __s_bar);
cptr.stPtro(conditions, 0 + $conditions_t_text + 16, __s_bh);
cptr.stI32o(conditions, 48, 10);
cptr.stI64o(conditions, 48 + $conditions_t_mask, 2n);
cptr.stI32o(conditions, 48 + $conditions_t_c, NHC.bl_blind);
cptr.stPtro(conditions, 48 + $conditions_t_text + 0, __s_blind__2);
cptr.stPtro(conditions, 48 + $conditions_t_text + 8, __s_blnd);
cptr.stPtro(conditions, 48 + $conditions_t_text + 16, __s_bl);
cptr.stI32o(conditions, 96, 20);
cptr.stI64o(conditions, 96 + $conditions_t_mask, 4n);
cptr.stI32o(conditions, 96 + $conditions_t_c, NHC.bl_busy);
cptr.stPtro(conditions, 96 + $conditions_t_text + 0, __s_busy);
cptr.stPtro(conditions, 96 + $conditions_t_text + 8, __s_bsy);
cptr.stPtro(conditions, 96 + $conditions_t_text + 16, __s_by);
cptr.stI32o(conditions, 144, 10);
cptr.stI64o(conditions, 144 + $conditions_t_mask, 8n);
cptr.stI32o(conditions, 144 + $conditions_t_c, NHC.bl_conf);
cptr.stPtro(conditions, 144 + $conditions_t_text + 0, __s_conf__2);
cptr.stPtro(conditions, 144 + $conditions_t_text + 8, __s_cnf);
cptr.stPtro(conditions, 144 + $conditions_t_text + 16, __s_cf);
cptr.stI32o(conditions, 192, 10);
cptr.stI64o(conditions, 192 + $conditions_t_mask, 16n);
cptr.stI32o(conditions, 192 + $conditions_t_c, NHC.bl_deaf);
cptr.stPtro(conditions, 192 + $conditions_t_text + 0, __s_deaf__2);
cptr.stPtro(conditions, 192 + $conditions_t_text + 8, __s_def);
cptr.stPtro(conditions, 192 + $conditions_t_text + 16, __s_df);
cptr.stI32o(conditions, 240, 15);
cptr.stI64o(conditions, 240 + $conditions_t_mask, 32n);
cptr.stI32o(conditions, 240 + $conditions_t_c, NHC.bl_elf_iron);
cptr.stPtro(conditions, 240 + $conditions_t_text + 0, __s_iron);
cptr.stPtro(conditions, 240 + $conditions_t_text + 8, __s_irn);
cptr.stPtro(conditions, 240 + $conditions_t_text + 16, __s_fe);
cptr.stI32o(conditions, 288, 10);
cptr.stI64o(conditions, 288 + $conditions_t_mask, 64n);
cptr.stI32o(conditions, 288 + $conditions_t_c, NHC.bl_fly);
cptr.stPtro(conditions, 288 + $conditions_t_text + 0, __s_fly__2);
cptr.stPtro(conditions, 288 + $conditions_t_text + 8, __s_fly__2);
cptr.stPtro(conditions, 288 + $conditions_t_text + 16, __s_fl);
cptr.stI32o(conditions, 336, 6);
cptr.stI64o(conditions, 336 + $conditions_t_mask, 128n);
cptr.stI32o(conditions, 336 + $conditions_t_c, NHC.bl_foodpois);
cptr.stPtro(conditions, 336 + $conditions_t_text + 0, __s_foodpois__2);
cptr.stPtro(conditions, 336 + $conditions_t_text + 8, __s_fpois);
cptr.stPtro(conditions, 336 + $conditions_t_text + 16, __s_poi);
cptr.stI32o(conditions, 384, 20);
cptr.stI64o(conditions, 384 + $conditions_t_mask, 256n);
cptr.stI32o(conditions, 384 + $conditions_t_c, NHC.bl_glowhands);
cptr.stPtro(conditions, 384 + $conditions_t_text + 0, __s_glow);
cptr.stPtro(conditions, 384 + $conditions_t_text + 8, __s_glo);
cptr.stPtro(conditions, 384 + $conditions_t_text + 16, __s_gl);
cptr.stI32o(conditions, 432, 2);
cptr.stI64o(conditions, 432 + $conditions_t_mask, 512n);
cptr.stI32o(conditions, 432 + $conditions_t_c, NHC.bl_grab);
cptr.stPtro(conditions, 432 + $conditions_t_text + 0, __s_grab);
cptr.stPtro(conditions, 432 + $conditions_t_text + 8, __s_grb);
cptr.stPtro(conditions, 432 + $conditions_t_text + 16, __s_gr);
cptr.stI32o(conditions, 480, 10);
cptr.stI64o(conditions, 480 + $conditions_t_mask, 1024n);
cptr.stI32o(conditions, 480 + $conditions_t_c, NHC.bl_hallu);
cptr.stPtro(conditions, 480 + $conditions_t_text + 0, __s_hallu__2);
cptr.stPtro(conditions, 480 + $conditions_t_text + 8, __s_hal);
cptr.stPtro(conditions, 480 + $conditions_t_text + 16, __s_hl);
cptr.stI32o(conditions, 528, 20);
cptr.stI64o(conditions, 528 + $conditions_t_mask, 2048n);
cptr.stI32o(conditions, 528 + $conditions_t_c, NHC.bl_held);
cptr.stPtro(conditions, 528 + $conditions_t_text + 0, __s_held);
cptr.stPtro(conditions, 528 + $conditions_t_text + 8, __s_hld);
cptr.stPtro(conditions, 528 + $conditions_t_text + 16, __s_hd__2);
cptr.stI32o(conditions, 576, 20);
cptr.stI64o(conditions, 576 + $conditions_t_mask, 4096n);
cptr.stI32o(conditions, 576 + $conditions_t_c, NHC.bl_icy);
cptr.stPtro(conditions, 576 + $conditions_t_text + 0, __s_icy);
cptr.stPtro(conditions, 576 + $conditions_t_text + 8, __s_icy);
cptr.stPtro(conditions, 576 + $conditions_t_text + 16, __s_ic);
cptr.stI32o(conditions, 624, 8);
cptr.stI64o(conditions, 624 + $conditions_t_mask, 8192n);
cptr.stI32o(conditions, 624 + $conditions_t_c, NHC.bl_inlava);
cptr.stPtro(conditions, 624 + $conditions_t_text + 0, __s_inlava);
cptr.stPtro(conditions, 624 + $conditions_t_text + 8, __s_lav);
cptr.stPtro(conditions, 624 + $conditions_t_text + 16, __s_la);
cptr.stI32o(conditions, 672, 10);
cptr.stI64o(conditions, 672 + $conditions_t_mask, 16384n);
cptr.stI32o(conditions, 672 + $conditions_t_c, NHC.bl_lev);
cptr.stPtro(conditions, 672 + $conditions_t_text + 0, __s_lev__2);
cptr.stPtro(conditions, 672 + $conditions_t_text + 8, __s_lev__2);
cptr.stPtro(conditions, 672 + $conditions_t_text + 16, __s_lv);
cptr.stI32o(conditions, 720, 20);
cptr.stI64o(conditions, 720 + $conditions_t_mask, 32768n);
cptr.stI32o(conditions, 720 + $conditions_t_c, NHC.bl_parlyz);
cptr.stPtro(conditions, 720 + $conditions_t_text + 0, __s_parlyz);
cptr.stPtro(conditions, 720 + $conditions_t_text + 8, __s_para);
cptr.stPtro(conditions, 720 + $conditions_t_text + 16, __s_par);
cptr.stI32o(conditions, 768, 10);
cptr.stI64o(conditions, 768 + $conditions_t_mask, 65536n);
cptr.stI32o(conditions, 768 + $conditions_t_c, NHC.bl_ride);
cptr.stPtro(conditions, 768 + $conditions_t_text + 0, __s_ride__2);
cptr.stPtro(conditions, 768 + $conditions_t_text + 8, __s_rid);
cptr.stPtro(conditions, 768 + $conditions_t_text + 16, __s_rd);
cptr.stI32o(conditions, 816, 20);
cptr.stI64o(conditions, 816 + $conditions_t_mask, 131072n);
cptr.stI32o(conditions, 816 + $conditions_t_c, NHC.bl_sleeping);
cptr.stPtro(conditions, 816 + $conditions_t_text + 0, __s_zzz);
cptr.stPtro(conditions, 816 + $conditions_t_text + 8, __s_zzz);
cptr.stPtro(conditions, 816 + $conditions_t_text + 16, __s_zz);
cptr.stI32o(conditions, 864, 6);
cptr.stI64o(conditions, 864 + $conditions_t_mask, 262144n);
cptr.stI32o(conditions, 864 + $conditions_t_c, NHC.bl_slime);
cptr.stPtro(conditions, 864 + $conditions_t_text + 0, __s_slime__2);
cptr.stPtro(conditions, 864 + $conditions_t_text + 8, __s_slim);
cptr.stPtro(conditions, 864 + $conditions_t_text + 16, __s_slm);
cptr.stI32o(conditions, 912, 20);
cptr.stI64o(conditions, 912 + $conditions_t_mask, 524288n);
cptr.stI32o(conditions, 912 + $conditions_t_c, NHC.bl_slippery);
cptr.stPtro(conditions, 912 + $conditions_t_text + 0, __s_slip);
cptr.stPtro(conditions, 912 + $conditions_t_text + 8, __s_slp);
cptr.stPtro(conditions, 912 + $conditions_t_text + 16, __s_sl);
cptr.stI32o(conditions, 960, 6);
cptr.stI64o(conditions, 960 + $conditions_t_mask, 1048576n);
cptr.stI32o(conditions, 960 + $conditions_t_c, NHC.bl_stone);
cptr.stPtro(conditions, 960 + $conditions_t_text + 0, __s_stone__2);
cptr.stPtro(conditions, 960 + $conditions_t_text + 8, __s_ston);
cptr.stPtro(conditions, 960 + $conditions_t_text + 16, __s_sto);
cptr.stI32o(conditions, 1008, 4);
cptr.stI64o(conditions, 1008 + $conditions_t_mask, 2097152n);
cptr.stI32o(conditions, 1008 + $conditions_t_c, NHC.bl_strngl);
cptr.stPtro(conditions, 1008 + $conditions_t_text + 0, __s_strngl__2);
cptr.stPtro(conditions, 1008 + $conditions_t_text + 8, __s_stngl);
cptr.stPtro(conditions, 1008 + $conditions_t_text + 16, __s_str);
cptr.stI32o(conditions, 1056, 10);
cptr.stI64o(conditions, 1056 + $conditions_t_mask, 4194304n);
cptr.stI32o(conditions, 1056 + $conditions_t_c, NHC.bl_stun);
cptr.stPtro(conditions, 1056 + $conditions_t_text + 0, __s_stun__2);
cptr.stPtro(conditions, 1056 + $conditions_t_text + 8, __s_stun__2);
cptr.stPtro(conditions, 1056 + $conditions_t_text + 16, __s_st);
cptr.stI32o(conditions, 1104, 15);
cptr.stI64o(conditions, 1104 + $conditions_t_mask, 8388608n);
cptr.stI32o(conditions, 1104 + $conditions_t_c, NHC.bl_submerged);
cptr.stPtro(conditions, 1104 + $conditions_t_text + 0, __s_submrg);
cptr.stPtro(conditions, 1104 + $conditions_t_text + 8, __s_subm);
cptr.stPtro(conditions, 1104 + $conditions_t_text + 16, __s_sm);
cptr.stI32o(conditions, 1152, 6);
cptr.stI64o(conditions, 1152 + $conditions_t_mask, 16777216n);
cptr.stI32o(conditions, 1152 + $conditions_t_c, NHC.bl_termill);
cptr.stPtro(conditions, 1152 + $conditions_t_text + 0, __s_termill__2);
cptr.stPtro(conditions, 1152 + $conditions_t_text + 8, __s_ill);
cptr.stPtro(conditions, 1152 + $conditions_t_text + 16, __s_ill);
cptr.stI32o(conditions, 1200, 20);
cptr.stI64o(conditions, 1200 + $conditions_t_mask, 33554432n);
cptr.stI32o(conditions, 1200 + $conditions_t_c, NHC.bl_tethered);
cptr.stPtro(conditions, 1200 + $conditions_t_text + 0, __s_teth);
cptr.stPtro(conditions, 1200 + $conditions_t_text + 8, __s_tth);
cptr.stPtro(conditions, 1200 + $conditions_t_text + 16, __s_te);
cptr.stI32o(conditions, 1248, 20);
cptr.stI64o(conditions, 1248 + $conditions_t_mask, 67108864n);
cptr.stI32o(conditions, 1248 + $conditions_t_c, NHC.bl_trapped);
cptr.stPtro(conditions, 1248 + $conditions_t_text + 0, __s_trap);
cptr.stPtro(conditions, 1248 + $conditions_t_text + 8, __s_trp);
cptr.stPtro(conditions, 1248 + $conditions_t_text + 16, __s_tr);
cptr.stI32o(conditions, 1296, 20);
cptr.stI64o(conditions, 1296 + $conditions_t_mask, 134217728n);
cptr.stI32o(conditions, 1296 + $conditions_t_c, NHC.bl_unconsc);
cptr.stPtro(conditions, 1296 + $conditions_t_text + 0, __s_out);
cptr.stPtro(conditions, 1296 + $conditions_t_text + 8, __s_out);
cptr.stPtro(conditions, 1296 + $conditions_t_text + 16, __s_ko);
cptr.stI32o(conditions, 1344, 20);
cptr.stI64o(conditions, 1344 + $conditions_t_mask, 268435456n);
cptr.stI32o(conditions, 1344 + $conditions_t_c, NHC.bl_woundedl);
cptr.stPtro(conditions, 1344 + $conditions_t_text + 0, __s_wlegs);
cptr.stPtro(conditions, 1344 + $conditions_t_text + 8, __s_leg);
cptr.stPtro(conditions, 1344 + $conditions_t_text + 16, __s_lg);
cptr.stI32o(conditions, 1392, 20);
cptr.stI64o(conditions, 1392 + $conditions_t_mask, 536870912n);
cptr.stI32o(conditions, 1392 + $conditions_t_c, NHC.bl_holding);
cptr.stPtro(conditions, 1392 + $conditions_t_text + 0, __s_uhold);
cptr.stPtro(conditions, 1392 + $conditions_t_text + 8, __s_uhld);
cptr.stPtro(conditions, 1392 + $conditions_t_text + 16, __s_uhd);

/** C ref: botl.c:817 — struct condtests_t[30] */
export const condtests = cptr.alloc(30 * $sizeof_condtests_t);
cptr.stI32o(condtests, 0, NHC.bl_bareh);
cptr.stPtro(condtests, 0 + $condtests_t_useroption, __s_barehanded);
cptr.stI32o(condtests, 0 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 0 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 0 + $condtests_t_choice, 0);
cptr.st1o(condtests, 0 + $condtests_t_test, 0);
cptr.stI32o(condtests, 24, NHC.bl_blind);
cptr.stPtro(condtests, 24 + $condtests_t_useroption, __s_blind__3);
cptr.stI32o(condtests, 24 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 24 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 24 + $condtests_t_choice, 0);
cptr.st1o(condtests, 24 + $condtests_t_test, 0);
cptr.stI32o(condtests, 48, NHC.bl_busy);
cptr.stPtro(condtests, 48 + $condtests_t_useroption, __s_busy__2);
cptr.stI32o(condtests, 48 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 48 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 48 + $condtests_t_choice, 0);
cptr.st1o(condtests, 48 + $condtests_t_test, 0);
cptr.stI32o(condtests, 72, NHC.bl_conf);
cptr.stPtro(condtests, 72 + $condtests_t_useroption, __s_conf__3);
cptr.stI32o(condtests, 72 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 72 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 72 + $condtests_t_choice, 0);
cptr.st1o(condtests, 72 + $condtests_t_test, 0);
cptr.stI32o(condtests, 96, NHC.bl_deaf);
cptr.stPtro(condtests, 96 + $condtests_t_useroption, __s_deaf__3);
cptr.stI32o(condtests, 96 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 96 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 96 + $condtests_t_choice, 0);
cptr.st1o(condtests, 96 + $condtests_t_test, 0);
cptr.stI32o(condtests, 120, NHC.bl_elf_iron);
cptr.stPtro(condtests, 120 + $condtests_t_useroption, __s_iron__2);
cptr.stI32o(condtests, 120 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 120 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 120 + $condtests_t_choice, 0);
cptr.st1o(condtests, 120 + $condtests_t_test, 0);
cptr.stI32o(condtests, 144, NHC.bl_fly);
cptr.stPtro(condtests, 144 + $condtests_t_useroption, __s_fly__3);
cptr.stI32o(condtests, 144 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 144 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 144 + $condtests_t_choice, 0);
cptr.st1o(condtests, 144 + $condtests_t_test, 0);
cptr.stI32o(condtests, 168, NHC.bl_foodpois);
cptr.stPtro(condtests, 168 + $condtests_t_useroption, __s_foodpois__3);
cptr.stI32o(condtests, 168 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 168 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 168 + $condtests_t_choice, 0);
cptr.st1o(condtests, 168 + $condtests_t_test, 0);
cptr.stI32o(condtests, 192, NHC.bl_glowhands);
cptr.stPtro(condtests, 192 + $condtests_t_useroption, __s_glowhands);
cptr.stI32o(condtests, 192 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 192 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 192 + $condtests_t_choice, 0);
cptr.st1o(condtests, 192 + $condtests_t_test, 0);
cptr.stI32o(condtests, 216, NHC.bl_grab);
cptr.stPtro(condtests, 216 + $condtests_t_useroption, __s_grab__2);
cptr.stI32o(condtests, 216 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 216 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 216 + $condtests_t_choice, 0);
cptr.st1o(condtests, 216 + $condtests_t_test, 0);
cptr.stI32o(condtests, 240, NHC.bl_hallu);
cptr.stPtro(condtests, 240 + $condtests_t_useroption, __s_hallucinat);
cptr.stI32o(condtests, 240 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 240 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 240 + $condtests_t_choice, 0);
cptr.st1o(condtests, 240 + $condtests_t_test, 0);
cptr.stI32o(condtests, 264, NHC.bl_held);
cptr.stPtro(condtests, 264 + $condtests_t_useroption, __s_held__2);
cptr.stI32o(condtests, 264 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 264 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 264 + $condtests_t_choice, 0);
cptr.st1o(condtests, 264 + $condtests_t_test, 0);
cptr.stI32o(condtests, 288, NHC.bl_icy);
cptr.stPtro(condtests, 288 + $condtests_t_useroption, __s_ice);
cptr.stI32o(condtests, 288 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 288 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 288 + $condtests_t_choice, 0);
cptr.st1o(condtests, 288 + $condtests_t_test, 0);
cptr.stI32o(condtests, 312, NHC.bl_inlava);
cptr.stPtro(condtests, 312 + $condtests_t_useroption, __s_lava);
cptr.stI32o(condtests, 312 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 312 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 312 + $condtests_t_choice, 0);
cptr.st1o(condtests, 312 + $condtests_t_test, 0);
cptr.stI32o(condtests, 336, NHC.bl_lev);
cptr.stPtro(condtests, 336 + $condtests_t_useroption, __s_levitate);
cptr.stI32o(condtests, 336 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 336 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 336 + $condtests_t_choice, 0);
cptr.st1o(condtests, 336 + $condtests_t_test, 0);
cptr.stI32o(condtests, 360, NHC.bl_parlyz);
cptr.stPtro(condtests, 360 + $condtests_t_useroption, __s_paralyzed);
cptr.stI32o(condtests, 360 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 360 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 360 + $condtests_t_choice, 0);
cptr.st1o(condtests, 360 + $condtests_t_test, 0);
cptr.stI32o(condtests, 384, NHC.bl_ride);
cptr.stPtro(condtests, 384 + $condtests_t_useroption, __s_ride__3);
cptr.stI32o(condtests, 384 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 384 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 384 + $condtests_t_choice, 0);
cptr.st1o(condtests, 384 + $condtests_t_test, 0);
cptr.stI32o(condtests, 408, NHC.bl_sleeping);
cptr.stPtro(condtests, 408 + $condtests_t_useroption, __s_sleep);
cptr.stI32o(condtests, 408 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 408 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 408 + $condtests_t_choice, 0);
cptr.st1o(condtests, 408 + $condtests_t_test, 0);
cptr.stI32o(condtests, 432, NHC.bl_slime);
cptr.stPtro(condtests, 432 + $condtests_t_useroption, __s_slime__3);
cptr.stI32o(condtests, 432 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 432 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 432 + $condtests_t_choice, 0);
cptr.st1o(condtests, 432 + $condtests_t_test, 0);
cptr.stI32o(condtests, 456, NHC.bl_slippery);
cptr.stPtro(condtests, 456 + $condtests_t_useroption, __s_slip__2);
cptr.stI32o(condtests, 456 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 456 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 456 + $condtests_t_choice, 0);
cptr.st1o(condtests, 456 + $condtests_t_test, 0);
cptr.stI32o(condtests, 480, NHC.bl_stone);
cptr.stPtro(condtests, 480 + $condtests_t_useroption, __s_stone__3);
cptr.stI32o(condtests, 480 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 480 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 480 + $condtests_t_choice, 0);
cptr.st1o(condtests, 480 + $condtests_t_test, 0);
cptr.stI32o(condtests, 504, NHC.bl_strngl);
cptr.stPtro(condtests, 504 + $condtests_t_useroption, __s_strngl__3);
cptr.stI32o(condtests, 504 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 504 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 504 + $condtests_t_choice, 0);
cptr.st1o(condtests, 504 + $condtests_t_test, 0);
cptr.stI32o(condtests, 528, NHC.bl_stun);
cptr.stPtro(condtests, 528 + $condtests_t_useroption, __s_stun__3);
cptr.stI32o(condtests, 528 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 528 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 528 + $condtests_t_choice, 0);
cptr.st1o(condtests, 528 + $condtests_t_test, 0);
cptr.stI32o(condtests, 552, NHC.bl_submerged);
cptr.stPtro(condtests, 552 + $condtests_t_useroption, __s_submerged);
cptr.stI32o(condtests, 552 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 552 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 552 + $condtests_t_choice, 0);
cptr.st1o(condtests, 552 + $condtests_t_test, 0);
cptr.stI32o(condtests, 576, NHC.bl_termill);
cptr.stPtro(condtests, 576 + $condtests_t_useroption, __s_termill__3);
cptr.stI32o(condtests, 576 + $condtests_t_opt, NHC.opt_out);
cptr.st1o(condtests, 576 + $condtests_t_enabled, 1);
cptr.st1o(condtests, 576 + $condtests_t_choice, 0);
cptr.st1o(condtests, 576 + $condtests_t_test, 0);
cptr.stI32o(condtests, 600, NHC.bl_tethered);
cptr.stPtro(condtests, 600 + $condtests_t_useroption, __s_tethered);
cptr.stI32o(condtests, 600 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 600 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 600 + $condtests_t_choice, 0);
cptr.st1o(condtests, 600 + $condtests_t_test, 0);
cptr.stI32o(condtests, 624, NHC.bl_trapped);
cptr.stPtro(condtests, 624 + $condtests_t_useroption, __s_trap__2);
cptr.stI32o(condtests, 624 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 624 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 624 + $condtests_t_choice, 0);
cptr.st1o(condtests, 624 + $condtests_t_test, 0);
cptr.stI32o(condtests, 648, NHC.bl_unconsc);
cptr.stPtro(condtests, 648 + $condtests_t_useroption, __s_unconscious);
cptr.stI32o(condtests, 648 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 648 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 648 + $condtests_t_choice, 0);
cptr.st1o(condtests, 648 + $condtests_t_test, 0);
cptr.stI32o(condtests, 672, NHC.bl_woundedl);
cptr.stPtro(condtests, 672 + $condtests_t_useroption, __s_woundedlegs);
cptr.stI32o(condtests, 672 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 672 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 672 + $condtests_t_choice, 0);
cptr.st1o(condtests, 672 + $condtests_t_test, 0);
cptr.stI32o(condtests, 696, NHC.bl_holding);
cptr.stPtro(condtests, 696 + $condtests_t_useroption, __s_holding);
cptr.stI32o(condtests, 696 + $condtests_t_opt, NHC.opt_in);
cptr.st1o(condtests, 696 + $condtests_t_enabled, 0);
cptr.st1o(condtests, 696 + $condtests_t_choice, 0);
cptr.st1o(condtests, 696 + $condtests_t_test, 0);

/** C ref: botl.c:852 — int[30] */
export const cond_idx = cptr.alloc(30 * 4);
cptr.stI32o(cond_idx, 0, 0);

/** C ref: botl.c:854 — char[5] */
const c_Wall = cptr.bytes("Wall");

/** C ref: botl.c:863 — char *[47] */
export const terrain_descr = cptr.alloc(47 * 8);
cptr.stPtro(terrain_descr, 0, __s_stone__2);
cptr.stPtro(terrain_descr, 8, cptr.decay(c_Wall));
cptr.stPtro(terrain_descr, 16, cptr.decay(c_Wall));
cptr.stPtro(terrain_descr, 24, cptr.decay(c_Wall));
cptr.stPtro(terrain_descr, 32, cptr.decay(c_Wall));
cptr.stPtro(terrain_descr, 40, cptr.decay(c_Wall));
cptr.stPtro(terrain_descr, 48, cptr.decay(c_Wall));
cptr.stPtro(terrain_descr, 56, cptr.decay(c_Wall));
cptr.stPtro(terrain_descr, 64, cptr.decay(c_Wall));
cptr.stPtro(terrain_descr, 72, cptr.decay(c_Wall));
cptr.stPtro(terrain_descr, 80, cptr.decay(c_Wall));
cptr.stPtro(terrain_descr, 88, cptr.decay(c_Wall));
cptr.stPtro(terrain_descr, 96, __s_portcullis);
cptr.stPtro(terrain_descr, 104, __s_tree);
cptr.stPtro(terrain_descr, 112, cptr.decay(c_Wall));
cptr.stPtro(terrain_descr, 120, __s_stone__2);
cptr.stPtro(terrain_descr, 128, __s_pool);
cptr.stPtro(terrain_descr, 136, __s_moat);
cptr.stPtro(terrain_descr, 144, __s_water);
cptr.stPtro(terrain_descr, 152, __s_gap);
cptr.stPtro(terrain_descr, 160, __s_lava__2);
cptr.stPtro(terrain_descr, 168, __s_lavawall);
cptr.stPtro(terrain_descr, 176, __s_bars);
cptr.stPtro(terrain_descr, 184, __s_doorway);
cptr.stPtro(terrain_descr, 192, __s_corridor);
cptr.stPtro(terrain_descr, 200, __s_room);
cptr.stPtro(terrain_descr, 208, __s_stairs);
cptr.stPtro(terrain_descr, 216, __s_ladder);
cptr.stPtro(terrain_descr, 224, __s_fountain);
cptr.stPtro(terrain_descr, 232, __s_throne);
cptr.stPtro(terrain_descr, 240, __s_sink);
cptr.stPtro(terrain_descr, 248, __s_grave);
cptr.stPtro(terrain_descr, 256, __s_altar);
cptr.stPtro(terrain_descr, 264, __s_ice__2);
cptr.stPtro(terrain_descr, 272, __s_bridge);
cptr.stPtro(terrain_descr, 280, __s_air);
cptr.stPtro(terrain_descr, 288, __s_cloud);
cptr.stPtro(terrain_descr, 296, __s_empty);
cptr.stPtro(terrain_descr, 304, cptr.decay(c_Wall));
cptr.stPtro(terrain_descr, 312, __s_floor);
cptr.stPtro(terrain_descr, 320, __s_ground);
cptr.stPtro(terrain_descr, 328, __s_open_door);
cptr.stPtro(terrain_descr, 336, __s_shut_door);
cptr.stPtro(terrain_descr, 344, __s_swamp);
cptr.stPtro(terrain_descr, 352, __s_submerged__2);
cptr.stPtro(terrain_descr, 360, __s_sea);
cptr.stPtro(terrain_descr, 368, __s_waterwall);

/** C ref: botl.c:919 — boolean[3] */
const cache_avail = [0, 0, 0];

/** C ref: botl.c:920 — boolean[3] */
const cache_reslt = [0, 0, 0];

/** C ref: botl.c:921 — char * */
let cache_nomovemsg = null;

/** C ref: botl.c:921 — char * */
let cache_multi_reason = null;

/** C ref: botl.c:962 */
function bot_via_windowport() {
    let buf = new Uint8Array(256);
    let titl;
    let nb;
    let i;
    let idx;
    let cap;
    let money;
    if (!cptr.ld1so(gb, $instance_globals_b_blinit))
        panic(__s_bot_before_init);
    idx = (1 - cptr.ldI32(gn)) | 0;
    cptr.stI32(gn, idx);
    void __builtin___memset_chk(gv, 0, 27n, __builtin_object_size(gv, 0));
    void cptr.strcpy(nb = cptr.decay(buf), svp);
    cptr.st1o(nb, 0, highc(cptr.ld1so(nb, 0)));
    titl = !Upolyd() ? rank() : pmname(cptr.add(mons, cptr.ldI32o(u, $you_umonnum), $sizeof_permonst), Ugender());
    i = Number(BigInt.asIntN(32, (BigInt.asUintN(64, BigInt.asUintN(64, BigInt.asUintN(64, cptr.strlen(cptr.decay(buf)) + 6n) + cptr.strlen(titl)) - 1n))));
    if (i > 30) {
        i = (30 - Number(BigInt.asIntN(32, (BigInt.asUintN(64, BigInt.asUintN(64, 6n + cptr.strlen(titl)) - 1n))))) | 0;
        cptr.st1o(nb, ((i) > NHM.BOTL_NSIZ ? (i) : NHM.BOTL_NSIZ), 0);
    }
    void cptr.strcpy(nb = eos(nb), __s_the);
    void cptr.strcpy(nb = eos(nb), titl);
    if (Upolyd()) {
        for (i = 0; cptr.ld1so(nb, i); i++)
            if (i == 0 || cptr.ld1so(nb, (i - 1) | 0) == 32)
                cptr.st1o(nb, i, highc(cptr.ld1so(nb, i)));
    }
    void cptr.sprintf(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_TITLE, $sizeof_istat_s, $istat_s_val), __s_30s, cptr.decay(buf));
    cptr.st1o(gv, NHC.BL_TITLE, 1, 1);
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_STR, $sizeof_istat_s, $istat_s_a, (acurr(NHC.A_STR)));
    void cptr.strcpy(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_STR, $sizeof_istat_s, $istat_s_val), get_strength_str());
    cptr.st1o(gv, NHC.BL_STR, 1, 1);
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_DX, $sizeof_istat_s, $istat_s_a, (acurr(NHC.A_DEX)));
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_CO, $sizeof_istat_s, $istat_s_a, (acurr(NHC.A_CON)));
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_IN, $sizeof_istat_s, $istat_s_a, (acurr(NHC.A_INT)));
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_WI, $sizeof_istat_s, $istat_s_a, (acurr(NHC.A_WIS)));
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_CH, $sizeof_istat_s, $istat_s_a, (acurr(NHC.A_CHA)));
    void cptr.strcpy(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_ALIGN, $sizeof_istat_s, $istat_s_val), (cptr.ld1so(u, $you_ualign) == -1) ? __s_chaotic__2 : ((cptr.ld1so(u, $you_ualign) == NHM.A_NEUTRAL) ? __s_neutral__2 : __s_lawful__2));
    cptr.stI64o2(cptr.add(gb, idx, $sizeof_istat_s_x27), NHC.BL_SCORE, $sizeof_istat_s, $istat_s_a, 0n);
    i = Upolyd() ? cptr.ldI32o(u, $you_mh) : cptr.ldI32o(u, $you_uhp);
    if (i < 0)
        i = 0;
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_HP, $sizeof_istat_s, $istat_s_rawval, i);
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_HP, $sizeof_istat_s, $istat_s_a, ((i) < 9999 ? (i) : 9999));
    i = Upolyd() ? cptr.ldI32o(u, $you_mhmax) : cptr.ldI32o(u, $you_uhpmax);
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_HPMAX, $sizeof_istat_s, $istat_s_rawval, i);
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_HPMAX, $sizeof_istat_s, $istat_s_a, ((i) < 9999 ? (i) : 9999));
    void describe_level(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_LEVELDESC, $sizeof_istat_s, $istat_s_val), 1);
    cptr.st1o(gv, NHC.BL_LEVELDESC, 1, 1);
    if ((money = money_cnt(cptr.ldPtro(gi, $instance_globals_i_invent))) < 0n)
        money = 0n;
    cptr.stI64o2(cptr.add(gb, idx, $sizeof_istat_s_x27), NHC.BL_GOLD, $sizeof_istat_s, $istat_s_rawval, money);
    cptr.stI64o2(cptr.add(gb, idx, $sizeof_istat_s_x27), NHC.BL_GOLD, $sizeof_istat_s, $istat_s_a, ((money) < 999999n ? (money) : 999999n));
    void cptr.sprintf(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_GOLD, $sizeof_istat_s, $istat_s_val), __s_s_ld, (cptr.ld1so(iflags, $instance_flags_in_dumplog) || cptr.ld1so(iflags, $instance_flags_invis_goldsym)) ? __s_dollar : encglyph((((NHC.GOLD_PIECE) + NHC.GLYPH_OBJ_OFF) | 0)), cptr.ldI64o2(cptr.add(gb, idx, $sizeof_istat_s_x27), NHC.BL_GOLD, $sizeof_istat_s, $istat_s_a));
    cptr.st1o(gv, NHC.BL_GOLD, 1, 1);
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_ENE, $sizeof_istat_s, $istat_s_rawval, cptr.ldI32o(u, $you_uen));
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_ENE, $sizeof_istat_s, $istat_s_a, ((cptr.ldI32o(u, $you_uen)) < 9999 ? (cptr.ldI32o(u, $you_uen)) : 9999));
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_ENEMAX, $sizeof_istat_s, $istat_s_rawval, cptr.ldI32o(u, $you_uenmax));
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_ENEMAX, $sizeof_istat_s, $istat_s_a, ((cptr.ldI32o(u, $you_uenmax)) < 9999 ? (cptr.ldI32o(u, $you_uenmax)) : 9999));
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_AC, $sizeof_istat_s, $istat_s_a, cptr.ld1so(u, $you_uac));
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_HD, $sizeof_istat_s, $istat_s_a, Upolyd() ? cptr.ld1so2(mons, cptr.ldI32o(u, $you_umonnum), $sizeof_permonst, $permonst_mlevel) : 0);
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_XP, $sizeof_istat_s, $istat_s_a, cptr.ldI32o(u, $you_ulevel));
    cptr.stI64o2(cptr.add(gb, idx, $sizeof_istat_s_x27), NHC.BL_EXP, $sizeof_istat_s, $istat_s_a, cptr.ldI64o(u, $you_uexp));
    cptr.stI64o2(cptr.add(gb, idx, $sizeof_istat_s_x27), NHC.BL_TIME, $sizeof_istat_s, $istat_s_a, cptr.ldI64o(svm, $instance_globals_saved_m_moves));
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_HUNGER, $sizeof_istat_s, $istat_s_a, cptr.ldI32o(u, $you_uhs) | 0);
    void cptr.strcpy(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_HUNGER, $sizeof_istat_s, $istat_s_val), (cptr.ldI32o(u, $you_uhs) != NHC.NOT_HUNGRY) ? cptr.ldPtro(hu_stat, cptr.ldI32o(u, $you_uhs), 8) : __s_empty);
    cptr.st1o(gv, NHC.BL_HUNGER, 1, 1);
    cap = near_capacity();
    cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_CAP, $sizeof_istat_s, $istat_s_a, cap);
    void cptr.strcpy(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_CAP, $sizeof_istat_s, $istat_s_val), (cap > NHC.UNENCUMBERED) ? cptr.ldPtro(enc_stat, cap, 8) : __s_empty);
    cptr.st1o(gv, NHC.BL_CAP, 1, 1);
    if (cptr.ldI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_VERS, $sizeof_istat_s, $istat_s_a) != (cptr.ldI32o(flags, $flag_versinfo) | 0)) {
        cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_VERS, $sizeof_istat_s, $istat_s_a, cptr.ldI32o(flags, $flag_versinfo) | 0);
        cptr.st1o(gv, NHC.BL_VERS, 0, 1);
    }
    if (!cptr.ld1so(gv, NHC.BL_VERS, 1)) {
        void status_version(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_VERS, $sizeof_istat_s, $istat_s_val), BigInt.asUintN(64, BigInt(cptr.ldI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_VERS, $sizeof_istat_s, $istat_s_valwidth))), 0);
        cptr.st1o(gv, NHC.BL_VERS, 1, 1);
    }
    cptr.stU64o2(cptr.add(gb, idx, $sizeof_istat_s_x27), NHC.BL_CONDITION, $sizeof_istat_s, $istat_s_a, 0n);
    cptr.st1o2(condtests, NHC.bl_foodpois, $sizeof_condtests_t, $condtests_t_test, cptr.st1o2(condtests, NHC.bl_termill, $sizeof_condtests_t, $condtests_t_test, 0));
    if (Sick()) {
        if (cptr.ld1so2(condtests, NHC.bl_foodpois, $sizeof_condtests_t, $condtests_t_enabled))
            cptr.st1o2(condtests, NHC.bl_foodpois, $sizeof_condtests_t, $condtests_t_test, schar(((((cptr.ldI32o(u, $you_usick_type) & 3) | 0) & NHM.SICK_VOMITABLE) != 0)));
        if (cptr.ld1so2(condtests, NHC.bl_termill, $sizeof_condtests_t, $condtests_t_enabled))
            cptr.st1o2(condtests, NHC.bl_termill, $sizeof_condtests_t, $condtests_t_test, schar(((((cptr.ldI32o(u, $you_usick_type) & 3) | 0) & NHM.SICK_NONVOMITABLE) != 0)));
    }
    cptr.st1o2(condtests, NHC.bl_inlava, $sizeof_condtests_t, $condtests_t_test, cptr.st1o2(condtests, NHC.bl_tethered, $sizeof_condtests_t, $condtests_t_test, cptr.st1o2(condtests, NHC.bl_trapped, $sizeof_condtests_t, $condtests_t_test, 0)));
    if (cptr.ldI32o(u, $you_utrap)) {
        if (cptr.ld1so2(condtests, NHC.bl_inlava, $sizeof_condtests_t, $condtests_t_enabled))
            cptr.st1o2(condtests, NHC.bl_inlava, $sizeof_condtests_t, $condtests_t_test, schar((cptr.ldI32o(u, $you_utraptype) == NHC.TT_LAVA)));
        if (cptr.ld1so2(condtests, NHC.bl_tethered, $sizeof_condtests_t, $condtests_t_enabled))
            cptr.st1o2(condtests, NHC.bl_tethered, $sizeof_condtests_t, $condtests_t_test, schar((cptr.ldI32o(u, $you_utraptype) == NHC.TT_BURIEDBALL)));
        if (cptr.ld1so2(condtests, NHC.bl_trapped, $sizeof_condtests_t, $condtests_t_enabled))
            cptr.st1o2(condtests, NHC.bl_trapped, $sizeof_condtests_t, $condtests_t_test, schar((!cptr.ld1so2(condtests, NHC.bl_inlava, $sizeof_condtests_t, $condtests_t_test) && !cptr.ld1so2(condtests, NHC.bl_tethered, $sizeof_condtests_t, $condtests_t_test) ? 1 : 0)));
    }
    cptr.st1o2(condtests, NHC.bl_grab, $sizeof_condtests_t, $condtests_t_test, cptr.st1o2(condtests, NHC.bl_held, $sizeof_condtests_t, $condtests_t_test, cptr.st1o2(condtests, NHC.bl_holding, $sizeof_condtests_t, $condtests_t_test, 0)));
    if (cptr.ldPtro(u, $you_ustuck)) {
        if ((cptr.ldI32o(u, $you_uswallow) & 1)) {
            if (cptr.ld1so2(condtests, NHC.bl_held, $sizeof_condtests_t, $condtests_t_enabled))
                cptr.st1o2(condtests, NHC.bl_held, $sizeof_condtests_t, $condtests_t_test, 1);
        } else if (Upolyd() && sticks(cptr.ldPtro(gy, $instance_globals_y_youmonst + $monst_data))) {
            if (cptr.ld1so2(condtests, NHC.bl_holding, $sizeof_condtests_t, $condtests_t_enabled))
                cptr.st1o2(condtests, NHC.bl_holding, $sizeof_condtests_t, $condtests_t_test, 1);
        } else {
            if (cptr.ld1so2(condtests, NHC.bl_grab, $sizeof_condtests_t, $condtests_t_enabled))
                cptr.st1o2(condtests, NHC.bl_grab, $sizeof_condtests_t, $condtests_t_test, schar((cptr.ld1so(cptr.ldPtro(cptr.ldPtro(u, $you_ustuck), $monst_data), $permonst_mlet) == NHC.S_EEL)));
            if (cptr.ld1so2(condtests, NHC.bl_held, $sizeof_condtests_t, $condtests_t_enabled))
                cptr.st1o2(condtests, NHC.bl_held, $sizeof_condtests_t, $condtests_t_test, schar((!cptr.ld1so2(condtests, NHC.bl_grab, $sizeof_condtests_t, $condtests_t_test))));
        }
    }
    cptr.st1o2(condtests, NHC.bl_blind, $sizeof_condtests_t, $condtests_t_test, schar(((Blind()) ? 1 : 0)));
    cptr.st1o2(condtests, NHC.bl_conf, $sizeof_condtests_t, $condtests_t_test, schar(((HConfusion()) ? 1 : 0)));
    cptr.st1o2(condtests, NHC.bl_deaf, $sizeof_condtests_t, $condtests_t_test, schar(((Deaf()) ? 1 : 0)));
    cptr.st1o2(condtests, NHC.bl_fly, $sizeof_condtests_t, $condtests_t_test, schar(((Flying()) ? 1 : 0)));
    cptr.st1o2(condtests, NHC.bl_glowhands, $sizeof_condtests_t, $condtests_t_test, schar(((cptr.ldI32o(u, $you_umconf)) ? 1 : 0)));
    cptr.st1o2(condtests, NHC.bl_hallu, $sizeof_condtests_t, $condtests_t_test, schar(((Hallucination()) ? 1 : 0)));
    cptr.st1o2(condtests, NHC.bl_lev, $sizeof_condtests_t, $condtests_t_test, schar(((Levitation()) ? 1 : 0)));
    cptr.st1o2(condtests, NHC.bl_ride, $sizeof_condtests_t, $condtests_t_test, schar(((cptr.ldPtro(u, $you_usteed)) ? 1 : 0)));
    cptr.st1o2(condtests, NHC.bl_slime, $sizeof_condtests_t, $condtests_t_test, schar(((Slimed()) ? 1 : 0)));
    cptr.st1o2(condtests, NHC.bl_stone, $sizeof_condtests_t, $condtests_t_test, schar(((Stoned()) ? 1 : 0)));
    cptr.st1o2(condtests, NHC.bl_strngl, $sizeof_condtests_t, $condtests_t_test, schar(((Strangled()) ? 1 : 0)));
    cptr.st1o2(condtests, NHC.bl_stun, $sizeof_condtests_t, $condtests_t_test, schar(((HStun()) ? 1 : 0)));
    cptr.st1o2(condtests, NHC.bl_submerged, $sizeof_condtests_t, $condtests_t_test, schar(((Underwater()) | 0 ? 1 : 0)));
    if (cptr.ld1so2(condtests, NHC.bl_elf_iron, $sizeof_condtests_t, $condtests_t_enabled))
        cptr.st1o2(condtests, NHC.bl_elf_iron, $sizeof_condtests_t, $condtests_t_test, 0);
    if (cptr.ld1so2(condtests, NHC.bl_bareh, $sizeof_condtests_t, $condtests_t_enabled))
        cptr.st1o2(condtests, NHC.bl_bareh, $sizeof_condtests_t, $condtests_t_test, schar((!uarmg.v && !uwep.v ? 1 : 0)));
    if (cptr.ld1so2(condtests, NHC.bl_icy, $sizeof_condtests_t, $condtests_t_enabled))
        cptr.st1o2(condtests, NHC.bl_icy, $sizeof_condtests_t, $condtests_t_test, schar((cptr.ld1so3(svl, cptr.ldI16(u), $sizeof_rm_x21, cptr.ldI16o(u, $you_uy), $sizeof_rm, $instance_globals_saved_l_level + $rm_typ) == NHC.ICE)));
    if (cptr.ld1so2(condtests, NHC.bl_slippery, $sizeof_condtests_t, $condtests_t_enabled))
        cptr.st1o2(condtests, NHC.bl_slippery, $sizeof_condtests_t, $condtests_t_test, schar(((Glib()) ? 1 : 0)));
    if (cptr.ld1so2(condtests, NHC.bl_woundedl, $sizeof_condtests_t, $condtests_t_enabled))
        cptr.st1o2(condtests, NHC.bl_woundedl, $sizeof_condtests_t, $condtests_t_test, schar(((Wounded_legs()) ? 1 : 0)));
    if (cptr.ldI64o(gm, $instance_globals_m_multi) < 0n) {
        {
            let clear_cache = 0;
            let refresh_cache = 0;
            if (cptr.ldI64o(gm, $instance_globals_m_multi) < 0n) {
                if (cptr.ldPtro(gn, $instance_globals_n_nomovemsg) || cptr.ldPtro(gm, $instance_globals_m_multi_reason)) {
                    if (!cptr.eq(cache_nomovemsg, cptr.ldPtro(gn, $instance_globals_n_nomovemsg)))
                        refresh_cache = 1;
                    if (!cptr.eq(cache_multi_reason, cptr.ldPtro(gm, $instance_globals_m_multi_reason)))
                        refresh_cache = 1;
                } else {
                    clear_cache = 1;
                }
            } else {
                clear_cache = 1;
            }
            if (clear_cache) {
                cache_nomovemsg = null;
                cache_multi_reason = null;
            }
            if (refresh_cache) {
                cache_nomovemsg = cptr.ldPtro(gn, $instance_globals_n_nomovemsg);
                cache_multi_reason = cptr.ldPtro(gm, $instance_globals_m_multi_reason);
            }
            if (clear_cache || refresh_cache) {
                cptr.st1o(cptr.decay(cache_reslt), 0, cptr.st1o(cptr.decay(cache_avail), 0, 0, 1), 1);
                cptr.st1o(cptr.decay(cache_reslt), 1, cptr.st1o(cptr.decay(cache_avail), 1, 0, 1), 1);
            }
        }
        if (cptr.ld1so2(condtests, NHC.bl_unconsc, $sizeof_condtests_t, $condtests_t_enabled) && cache_nomovemsg && !cptr.ld1so(cptr.decay(cache_avail), 0, 1)) {
            cptr.st1o(cptr.decay(cache_reslt), 0, schar((!cptr.ldI64o(u, $you_usleep) && unconscious() ? 1 : 0)), 1);
            cptr.st1o(cptr.decay(cache_avail), 0, 1, 1);
        }
        if (cptr.ld1so2(condtests, NHC.bl_parlyz, $sizeof_condtests_t, $condtests_t_enabled) && cache_multi_reason && !cptr.ld1so(cptr.decay(cache_avail), 1, 1)) {
            cptr.st1o(cptr.decay(cache_reslt), 1, schar((!cptr.strncmp(cache_multi_reason, __s_paralyzed, 9n) || !cptr.strncmp(cache_multi_reason, __s_frozen, 6n) ? 1 : 0)), 1);
            cptr.st1o(cptr.decay(cache_avail), 1, 1, 1);
        }
        if (cptr.ld1so(cptr.decay(cache_avail), 0, 1) && cptr.ld1so(cptr.decay(cache_reslt), 0, 1)) {
            cptr.st1o2(condtests, NHC.bl_unconsc, $sizeof_condtests_t, $condtests_t_test, cptr.ld1so(cptr.decay(cache_reslt), 0, 1));
        } else if (cptr.ld1so(cptr.decay(cache_avail), 1, 1) && cptr.ld1so(cptr.decay(cache_reslt), 1, 1)) {
            cptr.st1o2(condtests, NHC.bl_parlyz, $sizeof_condtests_t, $condtests_t_test, cptr.ld1so(cptr.decay(cache_reslt), 1, 1));
        } else if (cptr.ld1so2(condtests, NHC.bl_sleeping, $sizeof_condtests_t, $condtests_t_enabled) && cptr.ldI64o(u, $you_usleep)) {
            cptr.st1o2(condtests, NHC.bl_sleeping, $sizeof_condtests_t, $condtests_t_test, 1);
        } else if (cptr.ld1so2(condtests, NHC.bl_busy, $sizeof_condtests_t, $condtests_t_enabled)) {
            cptr.st1o2(condtests, NHC.bl_busy, $sizeof_condtests_t, $condtests_t_test, 1);
        }
    } else {
        cptr.st1o2(condtests, NHC.bl_unconsc, $sizeof_condtests_t, $condtests_t_test, cptr.st1o2(condtests, NHC.bl_parlyz, $sizeof_condtests_t, $condtests_t_test, cptr.st1o2(condtests, NHC.bl_sleeping, $sizeof_condtests_t, $condtests_t_test, cptr.st1o2(condtests, NHC.bl_busy, $sizeof_condtests_t, $condtests_t_test, 0))));
    }
    for (i = 0; i < NHC.CONDITION_COUNT; ++i) {
        if (cptr.ld1so2(condtests, i, $sizeof_condtests_t, $condtests_t_enabled) && cptr.ld1so2(condtests, i, $sizeof_condtests_t, $condtests_t_test))
            cptr.stU64o2(cptr.add(gb, idx, $sizeof_istat_s_x27), NHC.BL_CONDITION, $sizeof_istat_s, $istat_s_a, cptr.ldU64o2(cptr.add(gb, idx, $sizeof_istat_s_x27), NHC.BL_CONDITION, $sizeof_istat_s, $istat_s_a) | BigInt.asUintN(64, cptr.ldI64o2(conditions, (i), $sizeof_conditions_t, $conditions_t_mask)));
    }
    if (cptr.ld1so(flags, $flag_weaponstatus))
        void weapon_status(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_WEAPON, $sizeof_istat_s, $istat_s_val));
    else
        cptr.st1(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_WEAPON, $sizeof_istat_s, $istat_s_val), 0);
    if (cptr.ld1so(flags, $flag_armorstatus))
        void armor_status(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_ARMOR, $sizeof_istat_s, $istat_s_val));
    else
        cptr.st1(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_ARMOR, $sizeof_istat_s, $istat_s_val), 0);
    if (cptr.ld1so(flags, $flag_terrainstatus)) {
        if (cptr.ldI32o(iflags, $instance_flags_terrain_typ) == NHC.MAX_TYPE)
            classify_terrain();
        i = cptr.ldI32o(iflags, $instance_flags_terrain_typ);
        if (cptr.ldI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_TERRAIN, $sizeof_istat_s, $istat_s_a) != i) {
            void cptr.strcpy(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_TERRAIN, $sizeof_istat_s, $istat_s_val), cptr.ldPtro(terrain_descr, i, 8));
            cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_TERRAIN, $sizeof_istat_s, $istat_s_a, i);
        }
    } else {
        cptr.st1(cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, NHC.BL_TERRAIN, $sizeof_istat_s, $istat_s_val), 0);
        cptr.stI32o3(gb, idx, $sizeof_istat_s_x27, NHC.BL_TERRAIN, $sizeof_istat_s, $istat_s_a, NHC.MAX_TYPE);
    }
    cptr.st1o(gv, NHC.BL_TERRAIN, 1, 1);
    evaluate_and_notify_windowport(gv, idx);
}

/** C ref: botl.c:1285 */
function stat_update_time() {
    let idx = cptr.ldI32(gn);
    let fld = NHC.BL_TIME;
    cptr.stI64o2(cptr.add(gb, idx, $sizeof_istat_s_x27), fld, $sizeof_istat_s, $istat_s_a, cptr.ldI64o(svm, $instance_globals_saved_m_moves));
    cptr.st1o(gv, fld, 0, 1);
    eval_notify_windowport_field(fld, gv, idx);
    if ((cptr.ldU64o(windowprocs, $window_procs_wincap2) & 128n) != 0n)
        status_update()(NHC.BL_FLUSH, null, 0, 0, NHM.NO_COLOR, null);
    return;
}

/** C ref: botl.c:1303 — @param {CInt} idx @param {CPtr<boolean>} addr @param {CInt} negated */
export function condopt(idx, addr, negated) {
    let i;
    if ((idx < 0 || idx >= NHC.CONDITION_COUNT) || (addr && !cptr.eq(addr, cptr.add(cptr.add(condtests, idx, $sizeof_condtests_t), $condtests_t_choice))))
        return;
    if (!addr) {
        cptr.stI32o(gc, $instance_globals_c_condmenu_sortorder, 0);
        for (i = 0; i < NHC.CONDITION_COUNT; ++i) {
            cptr.stI32o(cond_idx, i, i, 4);
            cptr.st1o2(condtests, i, $sizeof_condtests_t, $condtests_t_choice, cptr.ld1so2(condtests, i, $sizeof_condtests_t, $condtests_t_enabled));
        }
        nh_deterministic_qsort((cond_idx), 30n, 4n, (cond_cmp));
    } else {
        cptr.st1o2(condtests, idx, $sizeof_condtests_t, $condtests_t_enabled, schar((negated ? 0 : 1)));
        cptr.st1o2(condtests, idx, $sizeof_condtests_t, $condtests_t_choice, cptr.ld1so2(condtests, idx, $sizeof_condtests_t, $condtests_t_enabled));
        cptr.st1o2(condtests, idx, $sizeof_condtests_t, $condtests_t_test, 0);
    }
}

/** C ref: botl.c:1333 — @param {CPtr<void>} vptr1 @param {CPtr<void>} vptr2 @returns {CInt} */
function cond_cmp(vptr1, vptr2) {
    let indx1 = cptr.ldI32(vptr1);
    let indx2 = cptr.ldI32(vptr2);
    let c1 = cptr.ldI32o(conditions, indx1, $sizeof_conditions_t);
    let c2 = cptr.ldI32o(conditions, indx2, $sizeof_conditions_t);
    if (c1 != c2)
        return (c1 - c2) | 0;
    return strncmpi((cptr.ldPtro2(condtests, indx1, $sizeof_condtests_t, $condtests_t_useroption)), (cptr.ldPtro2(condtests, indx2, $sizeof_condtests_t, $condtests_t_useroption)), -1);
}

/** C ref: botl.c:1346 — @param {CPtr<void>} vptr1 @param {CPtr<void>} vptr2 @returns {CInt} */
function menualpha_cmp(vptr1, vptr2) {
    let indx1 = cptr.ldI32(vptr1);
    let indx2 = cptr.ldI32(vptr2);
    return strncmpi((cptr.ldPtro2(condtests, indx1, $sizeof_condtests_t, $condtests_t_useroption)), (cptr.ldPtro2(condtests, indx2, $sizeof_condtests_t, $condtests_t_useroption)), -1);
}

/** C ref: botl.c:1354 — @param {CInt} negated @param {CPtr<char>} opts @returns {CInt} */
export function parse_cond_option(negated, opts) {
    let i;
    let sl;
    let compareto;
    let uniqpart;
    let prefix = cptr.bytes("cond_");
    if (!opts || cptr.strlen(opts) <= 5n)
        return 2;
    uniqpart = cptr.add(opts, 5n);
    for (i = 0; i < NHC.CONDITION_COUNT; ++i) {
        compareto = cptr.ldPtro2(condtests, i, $sizeof_condtests_t, $condtests_t_useroption);
        sl = Strlen_(compareto, __s_parse_cond_option, 1364) | 0;
        if (match_optname(uniqpart, compareto, (sl >= 4) ? 4 : sl, 0)) {
            condopt(i, cptr.add(cptr.add(condtests, i, $sizeof_condtests_t), $condtests_t_choice), negated);
            return 0;
        }
    }
    return 1;
}

const __static_cond_menu_menutitle = cptr.alloc(2 * 8);
cptr.stPtro(__static_cond_menu_menutitle, 0, __s_alphabetically);
cptr.stPtro(__static_cond_menu_menutitle, 8, __s_by_ranking); /** C ref: botl.c:1378 — char *[2] (function-static) */

/** C ref: botl.c:1376 @returns {CInt} */
export function cond_menu() {
    let i;
    let res;
    let idx = 0;
    let sequence = cptr.alloc(30 * 4);
    let tmpwin;
    let any = cptr.alloc(8);
    let picks = cptr.box(null);
    let mbuf = new Uint8Array(128);
    let showmenu = 1;
    let clr = NHM.NO_COLOR;
    let changed = 0;
    do {
        for (i = 0; i < NHC.CONDITION_COUNT; ++i) {
            cptr.stI32o(sequence, i, i, 4);
        }
        nh_deterministic_qsort((sequence), 30n, 4n, ((cptr.ldI32o(gc, $instance_globals_c_condmenu_sortorder)) ? cond_cmp : menualpha_cmp));
        tmpwin = create_nhwindow()(NHM.NHW_MENU);
        start_menu()(tmpwin, 0n);
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, 1);
        void cptr.sprintf(cptr.decay(mbuf), __s_change_sort_order_from_s_to_s, cptr.ldPtro(__static_cond_menu_menutitle, cptr.ldI32o(gc, $instance_globals_c_condmenu_sortorder), 8), cptr.ldPtro(__static_cond_menu_menutitle, (1 - cptr.ldI32o(gc, $instance_globals_c_condmenu_sortorder)) | 0, 8));
        add_menu(tmpwin, nul_glyphinfo.v, any, 83, 0, NHM.ATR_NONE, clr, cptr.decay(mbuf), NHM.MENU_ITEMFLAGS_SKIPINVERT);
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        void cptr.sprintf(cptr.decay(mbuf), __s_sorted_s, cptr.ldPtro(__static_cond_menu_menutitle, cptr.ldI32o(gc, $instance_globals_c_condmenu_sortorder), 8));
        add_menu_heading(tmpwin, cptr.decay(mbuf));
        for (i = 0; i < 30; i++) {
            idx = cptr.ldI32o(sequence, i, 4);
            void cptr.sprintf(cptr.decay(mbuf), __s_cond_14s, cptr.ldPtro2(condtests, idx, $sizeof_condtests_t, $condtests_t_useroption));
            cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
            cptr.stI32(any, (idx + 2) | 0);
            cptr.st1o2(condtests, idx, $sizeof_condtests_t, $condtests_t_choice, 0);
            add_menu(tmpwin, nul_glyphinfo.v, any, 0, 0, NHM.ATR_NONE, clr, cptr.decay(mbuf), cptr.ld1so2(condtests, idx, $sizeof_condtests_t, $condtests_t_enabled) ? NHM.MENU_ITEMFLAGS_SELECTED : NHM.MENU_ITEMFLAGS_NONE);
        }
        end_menu()(tmpwin, __s_choose_status_conditions_to_toggle);
        res = select_menu(tmpwin, NHM.PICK_ANY, picks);
        destroy_nhwindow()(tmpwin);
        showmenu = 0;
        if (res > 0) {
            for (i = 0; i < res; i++) {
                idx = cptr.ldI32o(picks.v, i, $sizeof_menu_item);
                if (idx == 1) {
                    cptr.stI32o(gc, $instance_globals_c_condmenu_sortorder, (1 - cptr.ldI32o(gc, $instance_globals_c_condmenu_sortorder)) | 0);
                    showmenu = 1;
                    break;
                } else {
                    idx = (idx - 2) | 0;
                    cptr.st1o2(condtests, idx, $sizeof_condtests_t, $condtests_t_choice, 1);
                }
            }
            cptr.free(picks.v);
        }
    } while (showmenu);
    if (res >= 0) {
        for (i = 0; i < NHC.CONDITION_COUNT; ++i)
            if (cptr.ld1so2(condtests, i, $sizeof_condtests_t, $condtests_t_enabled) != cptr.ld1so2(condtests, i, $sizeof_condtests_t, $condtests_t_choice)) {
                cptr.st1o2(condtests, i, $sizeof_condtests_t, $condtests_t_enabled, cptr.ld1so2(condtests, i, $sizeof_condtests_t, $condtests_t_choice));
                cptr.st1o2(condtests, idx, $sizeof_condtests_t, $condtests_t_test, 0);
                cptr.st1(disp, changed = 1);
            }
    }
    return changed;
}

/** C ref: botl.c:1460 — @param {CInt} indx @param {CPtr<char>} outbuf @returns {CInt} */
export function opt_next_cond(indx, outbuf) {
    cptr.st1(outbuf, 0);
    if (indx >= NHC.CONDITION_COUNT)
        return 0;
    if ((cptr.ldI32o2(condtests, indx, $sizeof_condtests_t, $condtests_t_opt) == NHC.opt_in && cptr.ld1so2(condtests, indx, $sizeof_condtests_t, $condtests_t_enabled)) || (cptr.ldI32o2(condtests, indx, $sizeof_condtests_t, $condtests_t_opt) == NHC.opt_out && !cptr.ld1so2(condtests, indx, $sizeof_condtests_t, $condtests_t_enabled))) {
        void cptr.sprintf(outbuf, __s_scond_s, cptr.ld1so2(condtests, indx, $sizeof_condtests_t, $condtests_t_enabled) ? __s_empty : __s_bang, cptr.ldPtro2(condtests, indx, $sizeof_condtests_t, $condtests_t_useroption));
    }
    return 1;
}

let __static_eval_notify_windowport_field_oldrndencode = 0; /** C ref: botl.c:1498 — int (function-static) */
let __static_eval_notify_windowport_field_oldgoldsym = 0; /** C ref: botl.c:1499 — unsigned char (function-static) */

/** C ref: botl.c:1493 — @param {CInt} fld @param {CPtr<boolean>} valsetlist @param {CInt} idx @returns {CInt} */
function eval_notify_windowport_field(fld, valsetlist, idx) {
    let pc;
    let chg;
    let color = cptr.box(NHM.NO_COLOR);
    let anytype;
    let updated = 0;
    let reset;
    let curr;
    let prev;
    let fldmax;
    anytype = cptr.ldI32o3(gb, idx, $sizeof_istat_s_x27, fld, $sizeof_istat_s, $istat_s_anytype);
    curr = cptr.add(cptr.add(gb, idx, $sizeof_istat_s_x27), fld, $sizeof_istat_s);
    prev = cptr.add(cptr.add(gb, (1 - idx) | 0, $sizeof_istat_s_x27), fld, $sizeof_istat_s);
    color.v = NHM.NO_COLOR;
    chg = cptr.ld1s(gu) ? 0 : compare_blstats(prev, curr);
    if (((chg || cptr.ld1s(gu) || fld == NHC.BL_XP) && cptr.ld1so(curr, $istat_s_percent_matters) && cptr.ldPtro(curr, $istat_s_thresholds)) || (fld == NHC.BL_HP && cptr.ld1so(iflags, $instance_flags_wc2_hitpointbar))) {
        fldmax = cptr.ldI32o(curr, $istat_s_idxmax);
        pc = (fldmax == NHC.BL_EXP) ? exp_percentage() : ((fldmax >= 0 && fldmax < NHC.MAXBLSTATS) ? percentage(curr, cptr.add(cptr.add(gb, idx, $sizeof_istat_s_x27), fldmax, $sizeof_istat_s)) : 0);
        if (pc != cptr.ldI16o(prev, $istat_s_percent_value))
            chg = (pc < cptr.ldI16o(prev, $istat_s_percent_value)) ? -1 : 1;
        cptr.stI16o(curr, $istat_s_percent_value, i16(pc));
    } else {
        pc = 0;
    }
    if (fld == NHC.BL_GOLD && (cptr.ldI32o(svc, $context_info_rndencode) != __static_eval_notify_windowport_field_oldrndencode || cptr.ld1uo2(gs, ((NHC.COIN_CLASS + (((0) + NHC.MAXPCHARS) | 0)) | 0), 1, $instance_globals_s_showsyms) != __static_eval_notify_windowport_field_oldgoldsym)) {
        cptr.st1(gu, 1);
        __static_eval_notify_windowport_field_oldrndencode = cptr.ldI32o(svc, $context_info_rndencode);
        __static_eval_notify_windowport_field_oldgoldsym = cptr.ld1uo2(gs, ((NHC.COIN_CLASS + (((0) + NHC.MAXPCHARS) | 0)) | 0), 1, $instance_globals_s_showsyms);
    }
    reset = 0;
    if (cptr.ld1s(gu)) {
        chg = 0;
        cptr.stI64o(curr, $istat_s_time, cptr.stI64o(prev, $istat_s_time, 0n));
    } else if (!chg && cptr.ldI64o(curr, $istat_s_time)) {
        reset = hilite_reset_needed(prev, cptr.ldI64o(gb, $instance_globals_b_bl_hilite_moves));
        if (reset)
            cptr.stI64o(curr, $istat_s_time, cptr.stI64o(prev, $istat_s_time, 0n));
    }
    if (cptr.ld1s(gu) || chg || reset) {
        if (!cptr.ld1so(valsetlist, fld))
            void anything_to_s(cptr.ldPtro(curr, $istat_s_val), cptr.add(curr, $istat_s_a), anytype | 0);
        if (anytype != NHC.ANY_MASK32) {
            if (chg || cptr.ld1s(cptr.ldPtro(curr, $istat_s_val))) {
                if (chg == 1 && fld == NHC.BL_XP)
                    chg = compare_blstats(prev, curr);
                cptr.stPtro(curr, $istat_s_hilite_rule, get_hilite(idx, fld, cptr.add(curr, $istat_s_a), chg, pc, color));
                cptr.stPtro(prev, $istat_s_hilite_rule, cptr.ldPtro(curr, $istat_s_hilite_rule));
                if (chg == 2) {
                    color.v = NHM.NO_COLOR;
                    chg = 0;
                }
            }
            status_update()(fld, cptr.ldPtro(curr, $istat_s_val), chg, pc, color.v, null);
        } else {
            status_update()(fld, cptr.add(curr, $istat_s_a), chg, pc, color.v, cptr.add(gc, $instance_globals_c_cond_hilites));
        }
        cptr.st1o(curr, $istat_s_chg, cptr.st1o(prev, $istat_s_chg, 1));
        updated = 1;
    }
    return updated;
}

/** C ref: botl.c:1621 — @param {CPtr<boolean>} valsetlist @param {CInt} idx */
function evaluate_and_notify_windowport(valsetlist, idx) {
    let i;
    let fld;
    let updated = 0;
    for (i = 0; i < NHC.MAXBLSTATS; i++) {
        fld = cptr.ldI32o2(initblstats, i, $sizeof_istat_s, $istat_s_fld);
        if (((fld == NHC.BL_SCORE) && !cptr.ld1so(flags, $flag_showscore)) || ((fld == NHC.BL_EXP) && !cptr.ld1so(flags, $flag_showexp)) || ((fld == NHC.BL_TIME) && !cptr.ld1so(flags, $flag_time)) || ((fld == NHC.BL_HD) && !Upolyd()) || ((fld == NHC.BL_XP || fld == NHC.BL_EXP) && Upolyd()) || ((fld == NHC.BL_VERS) && !cptr.ld1so(flags, $flag_showvers)) || ((fld == NHC.BL_TERRAIN) && !cptr.ld1so(flags, $flag_terrainstatus)) || ((fld == NHC.BL_WEAPON) && !cptr.ld1so(flags, $flag_weaponstatus)) || ((fld == NHC.BL_ARMOR) && !cptr.ld1so(flags, $flag_armorstatus))) {
            continue;
        }
        if (eval_notify_windowport_field(fld, valsetlist, idx))
            updated++;
    }
    if (cptr.ld1so(disp, $display_hints_botlx) && (cptr.ldU64o(windowprocs, $window_procs_wincap2) & 256n) != 0n)
        status_update()(NHC.BL_RESET, null, 0, 0, NHM.NO_COLOR, null);
    else if ((updated || cptr.ld1so(disp, $display_hints_botlx)) && (cptr.ldU64o(windowprocs, $window_procs_wincap2) & 128n) != 0n)
        status_update()(NHC.BL_FLUSH, null, 0, 0, NHM.NO_COLOR, null);
    cptr.st1(disp, cptr.st1o(disp, $display_hints_botlx, cptr.st1o(disp, $display_hints_time_botl, 0)));
    cptr.st1(gu, 0);
}

/** C ref: botl.c:1683 — @param {CInt} reassessment */
export function status_initialize(reassessment) {
    let fld;
    let fldenabl;
    let i;
    let fieldfmt;
    let fieldname;
    if (!reassessment) {
        if (cptr.ld1so(gb, $instance_globals_b_blinit))
            impossible(__s_2nd_status_initialize_with_full_init);
        init_blstats();
        (cptr.ldPtro(windowprocs, $window_procs_win_status_init))();
        cptr.st1o(gb, $instance_globals_b_blinit, 1);
    } else if (!cptr.ld1so(gb, $instance_globals_b_blinit)) {
        panic(__s_status_reassess_before_init);
    }
    for (i = 0; i < NHC.MAXBLSTATS; ++i) {
        fld = cptr.ldI32o2(initblstats, i, $sizeof_istat_s, $istat_s_fld);
        fldenabl = schar(((fld == NHC.BL_SCORE) ? cptr.ld1so(flags, $flag_showscore) : ((fld == NHC.BL_TIME) ? cptr.ld1so(flags, $flag_time) : ((fld == NHC.BL_EXP) ? schar((cptr.ld1so(flags, $flag_showexp) && !Upolyd() ? 1 : 0)) : ((fld == NHC.BL_XP) ? schar((!Upolyd())) : ((fld == NHC.BL_HD) ? schar(Upolyd()) : ((fld == NHC.BL_VERS) ? cptr.ld1so(flags, $flag_showvers) : ((fld == NHC.BL_WEAPON) ? cptr.ld1so(flags, $flag_weaponstatus) : ((fld == NHC.BL_ARMOR) ? cptr.ld1so(flags, $flag_armorstatus) : ((fld == NHC.BL_TERRAIN) ? cptr.ld1so(flags, $flag_terrainstatus) : 1))))))))));
        fieldname = cptr.ldPtro(initblstats, i, $sizeof_istat_s);
        fieldfmt = (fld == NHC.BL_TITLE && cptr.ld1so(iflags, $instance_flags_wc2_hitpointbar)) ? __s_30_30s : cptr.ldPtro2(initblstats, i, $sizeof_istat_s, $istat_s_fldfmt);
        status_enablefield()(fld, fieldname, fieldfmt, fldenabl);
    }
    cptr.st1(gu, 1);
    cptr.st1o(disp, $display_hints_botlx, 1);
}

/** C ref: botl.c:1723 */
export function status_finish() {
    let i;
    if (cptr.ldPtro(windowprocs, $window_procs_win_status_finish))
        (cptr.ldPtro(windowprocs, $window_procs_win_status_finish))();
    for (i = 0; i < NHC.MAXBLSTATS; ++i) {
        if (cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_val))
            cptr.free(cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_val)), cptr.stPtro3(gb, 0, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_val, (null));
        if (cptr.ldPtro3(gb, 1, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_val))
            cptr.free(cptr.ldPtro3(gb, 1, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_val)), cptr.stPtro3(gb, 1, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_val, (null));
        cptr.stPtro3(gb, 0, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_hilite_rule, cptr.stPtro3(gb, 1, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_hilite_rule, null));
        if (cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_thresholds)) {
            let temp;
            let next;
            for (temp = cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_thresholds); temp; temp = next) {
                next = cptr.ldPtro(temp, $hilite_s_next);
                cptr.free(temp);
            }
            cptr.stPtro3(gb, 0, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_thresholds, cptr.stPtro3(gb, 1, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_thresholds, (null)));
        }
    }
}

let __static_init_blstats_initalready = 0; /** C ref: botl.c:1761 — signed char (function-static) */

/** C ref: botl.c:1759 */
function init_blstats() {
    let i;
    let j;
    if (__static_init_blstats_initalready) {
        impossible(__s_init_blstats_called_more_than_once);
        return;
    }
    for (i = 0; i <= 1; ++i) {
        for (j = 0; j < NHC.MAXBLSTATS; ++j) {
            let keep_hilite_chain = cptr.ldPtro3(gb, i, $sizeof_istat_s_x27, j, $sizeof_istat_s, $istat_s_thresholds);
            cptr.memcpy(cptr.add(cptr.add(gb, i, $sizeof_istat_s_x27), j, $sizeof_istat_s), cptr.add(initblstats, j, $sizeof_istat_s), 88);
            cptr.memcpy(cptr.add(cptr.add(cptr.add(gb, i, $sizeof_istat_s_x27), j, $sizeof_istat_s), $istat_s_a), cptr.add(cg, $const_globals_zeroany), 8);
            if (cptr.ldI32o3(gb, i, $sizeof_istat_s_x27, j, $sizeof_istat_s, $istat_s_valwidth)) {
                cptr.stPtro3(gb, i, $sizeof_istat_s_x27, j, $sizeof_istat_s, $istat_s_val, alloc(cptr.ldI32o3(gb, i, $sizeof_istat_s_x27, j, $sizeof_istat_s, $istat_s_valwidth) >>> 0));
                cptr.st1o(cptr.ldPtro3(gb, i, $sizeof_istat_s_x27, j, $sizeof_istat_s, $istat_s_val), 0, 0);
            } else
                cptr.stPtro3(gb, i, $sizeof_istat_s_x27, j, $sizeof_istat_s, $istat_s_val, null);
            cptr.stPtro3(gb, i, $sizeof_istat_s_x27, j, $sizeof_istat_s, $istat_s_thresholds, keep_hilite_chain);
        }
    }
    __static_init_blstats_initalready = 1;
}

/** C ref: botl.c:1809 — @param {CPtr<struct istat_s>} bl1 @param {CPtr<struct istat_s>} bl2 @returns {CInt} */
function compare_blstats(bl1, bl2) {
    let a1;
    let a2;
    let use_rawval;
    let anytype;
    let fld;
    let result = 0;
    if (!bl1 || !bl2) {
        panic(__s_compare_blstat_bad_istat_pointer_s_s, fmt_ptr(bl1), fmt_ptr(bl2));
    }
    anytype = cptr.ldI32o(bl1, $istat_s_anytype);
    if ((!cptr.ldPtro(bl1, $istat_s_a) || !cptr.ldPtro(bl2, $istat_s_a)) && (anytype == NHC.ANY_IPTR || anytype == NHC.ANY_UPTR || anytype == NHC.ANY_LPTR || anytype == NHC.ANY_ULPTR)) {
        panic(__s_compare_blstat_invalid_pointer_s_s, fmt_ptr(cptr.ldPtro(bl1, $istat_s_a)), fmt_ptr(cptr.ldPtro(bl2, $istat_s_a)));
    }
    if (cptr.ldI32o(bl1, $istat_s_fld) == NHC.BL_TERRAIN)
        anytype = NHC.ANY_INT;
    fld = cptr.ldI32o(bl1, $istat_s_fld);
    use_rawval = schar((fld == NHC.BL_HP || fld == NHC.BL_HPMAX || fld == NHC.BL_ENE || fld == NHC.BL_ENEMAX || fld == NHC.BL_GOLD ? 1 : 0));
    a1 = use_rawval ? cptr.add(bl1, $istat_s_rawval) : cptr.add(bl1, $istat_s_a);
    a2 = use_rawval ? cptr.add(bl2, $istat_s_rawval) : cptr.add(bl2, $istat_s_a);
    switch (anytype) {
        case NHC.ANY_INT:
        result = (cptr.ldI32(a1) < cptr.ldI32(a2)) ? 1 : ((cptr.ldI32(a1) > cptr.ldI32(a2)) ? -1 : 0);
        break;
        case NHC.ANY_IPTR:
        result = (cptr.ldI32(cptr.ldPtr(a1)) < cptr.ldI32(cptr.ldPtr(a2))) ? 1 : ((cptr.ldI32(cptr.ldPtr(a1)) > cptr.ldI32(cptr.ldPtr(a2))) ? -1 : 0);
        break;
        case NHC.ANY_LONG:
        result = (cptr.ldI64(a1) < cptr.ldI64(a2)) ? 1 : ((cptr.ldI64(a1) > cptr.ldI64(a2)) ? -1 : 0);
        break;
        case NHC.ANY_LPTR:
        result = (cptr.ldI64(cptr.ldPtr(a1)) < cptr.ldI64(cptr.ldPtr(a2))) ? 1 : ((cptr.ldI64(cptr.ldPtr(a1)) > cptr.ldI64(cptr.ldPtr(a2))) ? -1 : 0);
        break;
        case NHC.ANY_UINT:
        result = (cptr.ldI32(a1) < cptr.ldI32(a2)) ? 1 : ((cptr.ldI32(a1) > cptr.ldI32(a2)) ? -1 : 0);
        break;
        case NHC.ANY_UPTR:
        result = (cptr.ldI32(cptr.ldPtr(a1)) < cptr.ldI32(cptr.ldPtr(a2))) ? 1 : ((cptr.ldI32(cptr.ldPtr(a1)) > cptr.ldI32(cptr.ldPtr(a2))) ? -1 : 0);
        break;
        case NHC.ANY_ULONG:
        result = (cptr.ldU64(a1) < cptr.ldU64(a2)) ? 1 : ((cptr.ldU64(a1) > cptr.ldU64(a2)) ? -1 : 0);
        break;
        case NHC.ANY_ULPTR:
        result = (cptr.ldU64(cptr.ldPtr(a1)) < cptr.ldU64(cptr.ldPtr(a2))) ? 1 : ((cptr.ldU64(cptr.ldPtr(a1)) > cptr.ldU64(cptr.ldPtr(a2))) ? -1 : 0);
        break;
        case NHC.ANY_STR:
        result = sgn(strcmp(cptr.ldPtro(bl1, $istat_s_val), cptr.ldPtro(bl2, $istat_s_val)));
        break;
        case NHC.ANY_MASK32:
        result = (cptr.ldU64(a1) != cptr.ldU64(a2));
        break;
        default:
        result = 1;
    }
    return result;
}

/** C ref: botl.c:1886 — @param {CPtr<char>} buf @param {CPtr<anything>} a @param {CInt} anytype @returns {CPtr<char>} */
function anything_to_s(buf, a, anytype) {
    if (!buf)
        return null;
    switch (anytype) {
        case NHC.ANY_ULONG:
        void cptr.sprintf(buf, __s_lu, cptr.ldU64(a));
        break;
        case NHC.ANY_MASK32:
        void cptr.sprintf(buf, __s_lx, cptr.ldU64(a));
        break;
        case NHC.ANY_LONG:
        void cptr.sprintf(buf, __s_ld, cptr.ldI64(a));
        break;
        case NHC.ANY_INT:
        void cptr.sprintf(buf, __s_pct_d, cptr.ldI32(a));
        break;
        case NHC.ANY_UINT:
        void cptr.sprintf(buf, __s_pct_u, cptr.ldI32(a));
        break;
        case NHC.ANY_IPTR:
        void cptr.sprintf(buf, __s_pct_d, cptr.ldI32(cptr.ldPtr(a)));
        break;
        case NHC.ANY_LPTR:
        void cptr.sprintf(buf, __s_ld, cptr.ldI64(cptr.ldPtr(a)));
        break;
        case NHC.ANY_ULPTR:
        void cptr.sprintf(buf, __s_lu, cptr.ldU64(cptr.ldPtr(a)));
        break;
        case NHC.ANY_UPTR:
        void cptr.sprintf(buf, __s_pct_u, cptr.ldI32(cptr.ldPtr(a)));
        break;
        case NHC.ANY_STR:
        ;
        break;
        default:
        cptr.st1o(buf, 0, 0);
    }
    return buf;
}

/** C ref: botl.c:1930 — @param {CPtr<anything>} a @param {CPtr<char>} buf @param {CInt} anytype */
function s_to_anything(a, buf, anytype) {
    if (!buf || !a)
        return;
    switch (anytype) {
        case NHC.ANY_LONG:
        cptr.stI64(a, atol(buf));
        break;
        case NHC.ANY_INT:
        cptr.stI32(a, atoi(buf));
        break;
        case NHC.ANY_UINT:
        cptr.stI32(a, atoi(buf) >>> 0);
        break;
        case NHC.ANY_ULONG:
        cptr.stU64(a, BigInt.asUintN(64, atol(buf)));
        break;
        case NHC.ANY_IPTR:
        if (cptr.ldPtr(a))
            cptr.stI32(cptr.ldPtr(a), atoi(buf));
        break;
        case NHC.ANY_UPTR:
        if (cptr.ldPtr(a))
            cptr.stI32(cptr.ldPtr(a), atoi(buf) >>> 0);
        break;
        case NHC.ANY_LPTR:
        if (cptr.ldPtr(a))
            cptr.stI64(cptr.ldPtr(a), atol(buf));
        break;
        case NHC.ANY_ULPTR:
        if (cptr.ldPtr(a))
            cptr.stU64(cptr.ldPtr(a), BigInt.asUintN(64, atol(buf)));
        break;
        case NHC.ANY_MASK32:
        cptr.stU64(a, BigInt.asUintN(64, atol(buf)));
        break;
        default:
        cptr.stPtr(a, null);
        break;
    }
    return;
}

/** C ref: botl.c:1977 — @param {CPtr<struct istat_s>} bl @param {CPtr<struct istat_s>} maxbl @returns {CInt} */
function percentage(bl, maxbl) {
    let result = 0;
    let anytype;
    let ival;
    let mval;
    let lval;
    let uval;
    let ulval;
    let fld;
    let use_rawval;
    if (!bl || !maxbl) {
        impossible(__s_percentage_bad_istat_pointer_s_s, fmt_ptr(bl), fmt_ptr(maxbl));
        return 0;
    }
    fld = cptr.ldI32o(bl, $istat_s_fld);
    use_rawval = schar((fld == NHC.BL_HP || fld == NHC.BL_ENE ? 1 : 0));
    ival = 0, lval = 0n, uval = 0, ulval = 0n;
    anytype = cptr.ldI32o(bl, $istat_s_anytype);
    if (cptr.ldPtro(maxbl, $istat_s_a)) {
        switch (anytype) {
            case NHC.ANY_INT:
            ival = use_rawval ? cptr.ldI32o(bl, $istat_s_rawval) : cptr.ldI32o(bl, $istat_s_a);
            mval = use_rawval ? cptr.ldI32o(maxbl, $istat_s_rawval) : cptr.ldI32o(maxbl, $istat_s_a);
            result = (((Math.imul(100, ival)) / mval) | 0);
            break;
            case NHC.ANY_LONG:
            lval = cptr.ldI64o(bl, $istat_s_a);
            result = Number(BigInt.asIntN(32, ((BigInt.asIntN(64, 100n * lval)) / cptr.ldI64o(maxbl, $istat_s_a))));
            break;
            case NHC.ANY_UINT:
            uval = cptr.ldI32o(bl, $istat_s_a);
            result = (u32div((Math.imul(100, uval) >>> 0), cptr.ldI32o(maxbl, $istat_s_a))) | 0;
            break;
            case NHC.ANY_ULONG:
            ulval = cptr.ldU64o(bl, $istat_s_a);
            result = Number(BigInt.asIntN(32, ((BigInt.asUintN(64, 100n * ulval)) / cptr.ldU64o(maxbl, $istat_s_a))));
            break;
            case NHC.ANY_IPTR:
            ival = cptr.ldI32(cptr.ldPtro(bl, $istat_s_a));
            result = (((Math.imul(100, ival)) / (cptr.ldI32(cptr.ldPtro(maxbl, $istat_s_a)))) | 0);
            break;
            case NHC.ANY_LPTR:
            lval = cptr.ldI64(cptr.ldPtro(bl, $istat_s_a));
            result = Number(BigInt.asIntN(32, ((BigInt.asIntN(64, 100n * lval)) / (cptr.ldI64(cptr.ldPtro(maxbl, $istat_s_a))))));
            break;
            case NHC.ANY_UPTR:
            uval = cptr.ldI32(cptr.ldPtro(bl, $istat_s_a));
            result = (u32div((Math.imul(100, uval) >>> 0), (cptr.ldI32(cptr.ldPtro(maxbl, $istat_s_a))))) | 0;
            break;
            case NHC.ANY_ULPTR:
            ulval = cptr.ldU64(cptr.ldPtro(bl, $istat_s_a));
            result = Number(BigInt.asIntN(32, ((BigInt.asUintN(64, 100n * ulval)) / (cptr.ldU64(cptr.ldPtro(maxbl, $istat_s_a))))));
            break;
        }
    }
    if (result == 0 && (ival != 0 || lval != 0n || uval != 0 || ulval != 0n))
        result = 1;
    return result;
}

/** C ref: botl.c:2052 @returns {CInt} */
function exp_percentage() {
    let res = 0;
    if (cptr.ldI32o(u, $you_ulevel) < 30) {
        let exp_val;
        let nxt_exp_val;
        let curlvlstart;
        curlvlstart = newuexp((cptr.ldI32o(u, $you_ulevel) - 1) | 0);
        exp_val = BigInt.asIntN(64, cptr.ldI64o(u, $you_uexp) - curlvlstart);
        nxt_exp_val = BigInt.asIntN(64, newuexp(cptr.ldI32o(u, $you_ulevel)) - curlvlstart);
        if (exp_val == BigInt.asIntN(64, nxt_exp_val - 1n)) {
            res = 100;
        } else {
            let curval = cptr.alloc(88);
            let maxval = cptr.alloc(88);
            cptr.stI32o(curval, $istat_s_anytype, cptr.stI32o(maxval, $istat_s_anytype, NHC.ANY_LONG));
            cptr.memcpy(cptr.add(curval, $istat_s_a), cptr.memcpy(cptr.add(maxval, $istat_s_a), cptr.add(cg, $const_globals_zeroany), 8), 8);
            cptr.stI64o(curval, $istat_s_a, exp_val);
            cptr.stI64o(maxval, $istat_s_a, nxt_exp_val);
            cptr.stI32o(curval, $istat_s_fld, cptr.stI32o(maxval, $istat_s_fld, NHC.BL_EXP));
            res = percentage(curval, maxval);
        }
    }
    return res;
}

/** C ref: botl.c:2090 @returns {CInt} */
export function exp_percent_changing() {
    let pc;
    let a = cptr.alloc(8);
    let color_dummy = cptr.box(0);
    let rule;
    let curr;
    if (!cptr.ld1s(disp)) {
        curr = cptr.add(cptr.add(gb, cptr.ldI32(gn), $sizeof_istat_s_x27), NHC.BL_XP, $sizeof_istat_s);
        if (cptr.ld1so(curr, $istat_s_percent_matters) && cptr.ldPtro(curr, $istat_s_thresholds) && (pc = exp_percentage()) != cptr.ldI16o(curr, $istat_s_percent_value)) {
            cptr.memcpy(a, cptr.add(cg, $const_globals_zeroany), 8);
            cptr.stI32(a, cptr.ldI32o(u, $you_ulevel));
            rule = get_hilite(cptr.ldI32(gn), NHC.BL_XP, a, 0, pc, color_dummy);
            if (!cptr.eq(rule, cptr.ldPtro(curr, $istat_s_hilite_rule)))
                return 1;
        }
    }
    return 0;
}

/** C ref: botl.c:2131 @returns {CInt} */
export function stat_cap_indx() {
    let cap;
    cap = cptr.ldI32o3(gb, cptr.ldI32(gn), $sizeof_istat_s_x27, NHC.BL_CAP, $sizeof_istat_s, $istat_s_a);
    return cap;
}

/** C ref: botl.c:2146 @returns {CInt} */
export function stat_hunger_indx() {
    let uhs;
    uhs = cptr.ldI32o3(gb, cptr.ldI32(gn), $sizeof_istat_s_x27, NHC.BL_HUNGER, $sizeof_istat_s, $istat_s_a);
    return uhs;
}

/** C ref: botl.c:2160 — @param {CInt} idx @returns {CPtr<char>} */
export function bl_idx_to_fldname(idx) {
    if (idx >= 0 && idx < NHC.MAXBLSTATS)
        return cptr.ldPtro(initblstats, idx, $sizeof_istat_s);
    return null;
}

/** C ref: botl.c:2170 — @param {CPtr<char>} inoutbuf */
export function repad_with_dashes(inoutbuf) {
    let p = eos(inoutbuf);
    while (cptr.cmp(p, cptr.add(inoutbuf, 2)) >= 0 && cptr.ld1so(p, -1) == 32 && cptr.ld1so(p, -2) == 32) {
        cptr.st1o(p, -1, 45);
        p = cptr.sub(p, 2);
    }
}

/** C ref: botl.c:2186 — struct fieldid_t { fieldname, fldid } (memory model v0.5) */

/** C ref: botl.c:2189 — struct fieldid_t[22] */
const fieldids_alias = cptr.alloc(22 * $sizeof_fieldid_t);
cptr.stPtro(fieldids_alias, 0, __s_characteristics);
cptr.stI32o(fieldids_alias, 0 + $fieldid_t_fldid, NHC.BL_CHARACTERISTICS);
cptr.stPtro(fieldids_alias, 16, __s_encumbrance);
cptr.stI32o(fieldids_alias, 16 + $fieldid_t_fldid, NHC.BL_CAP);
cptr.stPtro(fieldids_alias, 32, __s_experience_points);
cptr.stI32o(fieldids_alias, 32 + $fieldid_t_fldid, NHC.BL_EXP);
cptr.stPtro(fieldids_alias, 48, __s_dx);
cptr.stI32o(fieldids_alias, 48 + $fieldid_t_fldid, NHC.BL_DX);
cptr.stPtro(fieldids_alias, 64, __s_co);
cptr.stI32o(fieldids_alias, 64 + $fieldid_t_fldid, NHC.BL_CO);
cptr.stPtro(fieldids_alias, 80, __s_con);
cptr.stI32o(fieldids_alias, 80 + $fieldid_t_fldid, NHC.BL_CO);
cptr.stPtro(fieldids_alias, 96, __s_points);
cptr.stI32o(fieldids_alias, 96 + $fieldid_t_fldid, NHC.BL_SCORE);
cptr.stPtro(fieldids_alias, 112, __s_cap);
cptr.stI32o(fieldids_alias, 112 + $fieldid_t_fldid, NHC.BL_CAP);
cptr.stPtro(fieldids_alias, 128, __s_pw);
cptr.stI32o(fieldids_alias, 128 + $fieldid_t_fldid, NHC.BL_ENE);
cptr.stPtro(fieldids_alias, 144, __s_pw_max);
cptr.stI32o(fieldids_alias, 144 + $fieldid_t_fldid, NHC.BL_ENEMAX);
cptr.stPtro(fieldids_alias, 160, __s_xl);
cptr.stI32o(fieldids_alias, 160 + $fieldid_t_fldid, NHC.BL_XP);
cptr.stPtro(fieldids_alias, 176, __s_xplvl);
cptr.stI32o(fieldids_alias, 176 + $fieldid_t_fldid, NHC.BL_XP);
cptr.stPtro(fieldids_alias, 192, __s_ac);
cptr.stI32o(fieldids_alias, 192 + $fieldid_t_fldid, NHC.BL_AC);
cptr.stPtro(fieldids_alias, 208, __s_hit_dice);
cptr.stI32o(fieldids_alias, 208 + $fieldid_t_fldid, NHC.BL_HD);
cptr.stPtro(fieldids_alias, 224, __s_turns);
cptr.stI32o(fieldids_alias, 224 + $fieldid_t_fldid, NHC.BL_TIME);
cptr.stPtro(fieldids_alias, 240, __s_hp);
cptr.stI32o(fieldids_alias, 240 + $fieldid_t_fldid, NHC.BL_HP);
cptr.stPtro(fieldids_alias, 256, __s_hp_max);
cptr.stI32o(fieldids_alias, 256 + $fieldid_t_fldid, NHC.BL_HPMAX);
cptr.stPtro(fieldids_alias, 272, __s_dgn);
cptr.stI32o(fieldids_alias, 272 + $fieldid_t_fldid, NHC.BL_LEVELDESC);
cptr.stPtro(fieldids_alias, 288, __s_xp);
cptr.stI32o(fieldids_alias, 288 + $fieldid_t_fldid, NHC.BL_EXP);
cptr.stPtro(fieldids_alias, 304, __s_exp);
cptr.stI32o(fieldids_alias, 304 + $fieldid_t_fldid, NHC.BL_EXP);
cptr.stPtro(fieldids_alias, 320, __s_flags);
cptr.stI32o(fieldids_alias, 320 + $fieldid_t_fldid, NHC.BL_CONDITION);
cptr.stPtro(fieldids_alias, 336, null);
cptr.stI32o(fieldids_alias, 336 + $fieldid_t_fldid, NHC.BL_FLUSH);

/** C ref: botl.c:2215 — char[25] */
const threshold_value = cptr.bytes("hilite_status threshold ");

/** C ref: botl.c:2216 — char[17] */
const is_out_of_range = cptr.bytes(" is out of range");

/** C ref: botl.c:2221 — @param {CPtr<char>} name @returns {*} */
function fldname_to_bl_indx(name) {
    let i;
    let nmatches = 0;
    let fld = 0;
    if (name && cptr.ld1s(name)) {
        for (i = 0; i < 27; i++)
            if (fuzzymatch(cptr.ldPtro(initblstats, i, $sizeof_istat_s), name, __s_sp_dash_us, 1)) {
                fld = cptr.ldI32o2(initblstats, i, $sizeof_istat_s, $istat_s_fld);
                nmatches++;
            }
        if (!nmatches) {
            for (i = 0; cptr.ldPtro(fieldids_alias, i, $sizeof_fieldid_t); i++)
                if (fuzzymatch(cptr.ldPtro(fieldids_alias, i, $sizeof_fieldid_t), name, __s_sp_dash_us, 1)) {
                    fld = cptr.ldI32o2(fieldids_alias, i, $sizeof_fieldid_t, $fieldid_t_fldid);
                    nmatches++;
                }
        }
        if (!nmatches) {
            let len = Number(BigInt.asIntN(32, cptr.strlen(name)));
            for (i = 0; i < 27; i++)
                if (!strncmpi(name, cptr.ldPtro(initblstats, i, $sizeof_istat_s), len)) {
                    fld = cptr.ldI32o2(initblstats, i, $sizeof_istat_s, $istat_s_fld);
                    nmatches++;
                }
        }
    }
    return (nmatches == 1) ? fld : NHC.BL_FLUSH;
}

/** C ref: botl.c:2257 — @param {CPtr<struct istat_s>} bl_p @param {CLongLong} augmented_time @returns {CInt} */
function hilite_reset_needed(bl_p, augmented_time) {
    if (cptr.ldI64o(gm, $instance_globals_m_multi))
        return 0;
    if (!((cptr.ldPtro(bl_p, $istat_s_hilite_rule)) && cptr.ldI32o((cptr.ldPtro(bl_p, $istat_s_hilite_rule)), $hilite_s_behavior) == NHM.BL_TH_UPDOWN))
        return 0;
    if (cptr.ldI64o(bl_p, $istat_s_time) == 0n || cptr.ldI64o(bl_p, $istat_s_time) >= augmented_time)
        return 0;
    return 1;
}

/** C ref: botl.c:2279 */
export function status_eval_next_unhilite() {
    let i;
    let curr;
    let next_unhilite;
    let this_unhilite;
    cptr.stI64o(gb, $instance_globals_b_bl_hilite_moves, cptr.ldI64o(svm, $instance_globals_saved_m_moves));
    next_unhilite = 0n;
    for (i = 0; i < NHC.MAXBLSTATS; ++i) {
        curr = cptr.add(cptr.add(gb, 0, $sizeof_istat_s_x27), i, $sizeof_istat_s);
        if (cptr.ld1so(curr, $istat_s_chg)) {
            let prev = cptr.add(cptr.add(gb, 1, $sizeof_istat_s_x27), i, $sizeof_istat_s);
            if (((cptr.ldPtro(curr, $istat_s_hilite_rule)) && cptr.ldI32o((cptr.ldPtro(curr, $istat_s_hilite_rule)), $hilite_s_behavior) == NHM.BL_TH_UPDOWN))
                cptr.stI64o(curr, $istat_s_time, (BigInt.asIntN(64, cptr.ldI64o(gb, $instance_globals_b_bl_hilite_moves) + cptr.ldI64o(iflags, $instance_flags_hilite_delta))));
            else
                cptr.stI64o(curr, $istat_s_time, 0n);
            cptr.stI64o(prev, $istat_s_time, cptr.ldI64o(curr, $istat_s_time));
            cptr.st1o(curr, $istat_s_chg, cptr.st1o(prev, $istat_s_chg, 0));
            cptr.st1(disp, 1);
        }
        if (cptr.ld1s(disp))
            continue;
        this_unhilite = cptr.ldI64o(curr, $istat_s_time);
        if (this_unhilite > 0n && (next_unhilite == 0n || this_unhilite < next_unhilite) && hilite_reset_needed(curr, BigInt.asIntN(64, this_unhilite + 1n))) {
            next_unhilite = this_unhilite;
            if (next_unhilite < cptr.ldI64o(gb, $instance_globals_b_bl_hilite_moves))
                cptr.st1(disp, 1);
        }
    }
}

/** C ref: botl.c:2321 */
export function reset_status_hilites() {
    if (cptr.ldI64o(iflags, $instance_flags_hilite_delta)) {
        let i;
        for (i = 0; i < NHC.MAXBLSTATS; ++i)
            cptr.stI64o2(cptr.add(gb, 0, $sizeof_istat_s_x27), i, $sizeof_istat_s, $istat_s_time, cptr.stI64o2(cptr.add(gb, 1, $sizeof_istat_s_x27), i, $sizeof_istat_s, $istat_s_time, 0n));
        cptr.st1(gu, 1);
    }
    cptr.st1o(disp, $display_hints_botlx, 1);
}

/** C ref: botl.c:2336 — @param {CPtr<char>} hl_text @returns {CInt} */
function noneoftheabove(hl_text) {
    if (fuzzymatch(hl_text, __s_none_of_the_above, __s_quot_sp_dash_us, 1) || fuzzymatch(hl_text, __s_polymorphed, __s_quot_lparen_rparen, 1) || fuzzymatch(hl_text, __s_none_of_the_above_polymorphed, __s_quot_sp_dash_us_lparen_rparen, 1))
        return 1;
    return 0;
}

/** C ref: botl.c:2364 — @param {CInt} idx @param {CInt} fldidx @param {CPtr} vp @param {CInt} chg @param {CInt} pc @param {CPtr<int>} colorptr @returns {CPtr<struct hilite_s>} */
function get_hilite(idx, fldidx, vp, chg, pc, colorptr) {
    let hl;
    let rule = null;
    let value = vp;
    let txtstr;
    if (fldidx < 0 || fldidx >= NHC.MAXBLSTATS)
        return null;
    if ((cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, (fldidx), $sizeof_istat_s, $istat_s_thresholds))) {
        let dt;
        let max_pc = -1;
        let min_pc = 101;
        let max_ival = -32767;
        let min_ival = NHM.LARGEST_INT;
        let max_lval = -9223372036854775807n;
        let min_lval = 9223372036854775807n;
        let exactmatch = 0;
        let updown = 0;
        let changed = 0;
        let perc_or_abs = 0;
        let crit_hp = 0;
        for (hl = cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, fldidx, $sizeof_istat_s, $istat_s_thresholds); hl; hl = cptr.ldPtro(hl, $hilite_s_next)) {
            dt = cptr.ldI32o2(initblstats, fldidx, $sizeof_istat_s, $istat_s_anytype);
            if (crit_hp && cptr.ldI32o(hl, $hilite_s_behavior) != NHM.BL_TH_CRITICALHP)
                continue;
            if ((updown || changed) && cptr.ldI32o(hl, $hilite_s_behavior) != NHM.BL_TH_UPDOWN)
                continue;
            if (perc_or_abs && cptr.ldI32o(hl, $hilite_s_behavior) == NHM.BL_TH_ALWAYS_HILITE)
                continue;
            switch (cptr.ldI32o(hl, $hilite_s_behavior)) {
                case NHM.BL_TH_VAL_PERCENTAGE:
                if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.EQ_VALUE && pc == cptr.ldI32o(hl, $hilite_s_value)) {
                    rule = hl;
                    min_pc = (max_pc = cptr.ldI32o(hl, $hilite_s_value));
                    exactmatch = (perc_or_abs = 1);
                } else if (exactmatch) {
                    ;
                } else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.LT_VALUE && (pc < cptr.ldI32o(hl, $hilite_s_value)) && (cptr.ldI32o(hl, $hilite_s_value) <= min_pc)) {
                    rule = hl;
                    min_pc = cptr.ldI32o(hl, $hilite_s_value);
                    perc_or_abs = 1;
                } else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.LE_VALUE && (pc <= cptr.ldI32o(hl, $hilite_s_value)) && (cptr.ldI32o(hl, $hilite_s_value) <= min_pc)) {
                    rule = hl;
                    min_pc = cptr.ldI32o(hl, $hilite_s_value);
                    perc_or_abs = 1;
                } else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.GT_VALUE && (pc > cptr.ldI32o(hl, $hilite_s_value)) && (cptr.ldI32o(hl, $hilite_s_value) >= max_pc)) {
                    rule = hl;
                    max_pc = cptr.ldI32o(hl, $hilite_s_value);
                    perc_or_abs = 1;
                } else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.GE_VALUE && (pc >= cptr.ldI32o(hl, $hilite_s_value)) && (cptr.ldI32o(hl, $hilite_s_value) >= max_pc)) {
                    rule = hl;
                    max_pc = cptr.ldI32o(hl, $hilite_s_value);
                    perc_or_abs = 1;
                }
                break;
                case NHM.BL_TH_UPDOWN:
                if (chg < 0 && cptr.ldI32o(hl, $hilite_s_rel) == NHC.LT_VALUE) {
                    rule = hl;
                    updown = 1;
                } else if (chg > 0 && cptr.ldI32o(hl, $hilite_s_rel) == NHC.GT_VALUE) {
                    rule = hl;
                    updown = 1;
                } else if (chg != 0 && cptr.ldI32o(hl, $hilite_s_rel) == NHC.EQ_VALUE && !updown) {
                    rule = hl;
                    changed = 1;
                }
                break;
                case NHM.BL_TH_VAL_ABSOLUTE:
                if (dt == NHC.ANY_INT) {
                    if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.EQ_VALUE && cptr.ldI32o(hl, $hilite_s_value) == cptr.ldI32(value)) {
                        rule = hl;
                        min_ival = (max_ival = cptr.ldI32o(hl, $hilite_s_value));
                        exactmatch = (perc_or_abs = 1);
                    } else if (exactmatch) {
                        ;
                    } else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.LT_VALUE && (cptr.ldI32(value) < cptr.ldI32o(hl, $hilite_s_value)) && (cptr.ldI32o(hl, $hilite_s_value) <= min_ival)) {
                        rule = hl;
                        min_ival = cptr.ldI32o(hl, $hilite_s_value);
                        perc_or_abs = 1;
                    } else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.LE_VALUE && (cptr.ldI32(value) <= cptr.ldI32o(hl, $hilite_s_value)) && (cptr.ldI32o(hl, $hilite_s_value) <= min_ival)) {
                        rule = hl;
                        min_ival = cptr.ldI32o(hl, $hilite_s_value);
                        perc_or_abs = 1;
                    } else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.GT_VALUE && (cptr.ldI32(value) > cptr.ldI32o(hl, $hilite_s_value)) && (cptr.ldI32o(hl, $hilite_s_value) >= max_ival)) {
                        rule = hl;
                        max_ival = cptr.ldI32o(hl, $hilite_s_value);
                        perc_or_abs = 1;
                    } else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.GE_VALUE && (cptr.ldI32(value) >= cptr.ldI32o(hl, $hilite_s_value)) && (cptr.ldI32o(hl, $hilite_s_value) >= max_ival)) {
                        rule = hl;
                        max_ival = cptr.ldI32o(hl, $hilite_s_value);
                        perc_or_abs = 1;
                    }
                } else {
                    if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.EQ_VALUE && cptr.ldI64o(hl, $hilite_s_value) == cptr.ldI64(value)) {
                        rule = hl;
                        min_lval = (max_lval = cptr.ldI64o(hl, $hilite_s_value));
                        exactmatch = (perc_or_abs = 1);
                    } else if (exactmatch) {
                        ;
                    } else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.LT_VALUE && (cptr.ldI64(value) < cptr.ldI64o(hl, $hilite_s_value)) && (cptr.ldI64o(hl, $hilite_s_value) <= min_lval)) {
                        rule = hl;
                        min_lval = cptr.ldI64o(hl, $hilite_s_value);
                        perc_or_abs = 1;
                    } else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.LE_VALUE && (cptr.ldI64(value) <= cptr.ldI64o(hl, $hilite_s_value)) && (cptr.ldI64o(hl, $hilite_s_value) <= min_lval)) {
                        rule = hl;
                        min_lval = cptr.ldI64o(hl, $hilite_s_value);
                        perc_or_abs = 1;
                    } else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.GT_VALUE && (cptr.ldI64(value) > cptr.ldI64o(hl, $hilite_s_value)) && (cptr.ldI64o(hl, $hilite_s_value) >= max_lval)) {
                        rule = hl;
                        max_lval = cptr.ldI64o(hl, $hilite_s_value);
                        perc_or_abs = 1;
                    } else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.GE_VALUE && (cptr.ldI64(value) >= cptr.ldI64o(hl, $hilite_s_value)) && (cptr.ldI64o(hl, $hilite_s_value) >= max_lval)) {
                        rule = hl;
                        max_lval = cptr.ldI64o(hl, $hilite_s_value);
                        perc_or_abs = 1;
                    }
                }
                break;
                case NHM.BL_TH_TEXTMATCH:
                txtstr = cptr.ldPtro3(gb, idx, $sizeof_istat_s_x27, fldidx, $sizeof_istat_s, $istat_s_val);
                if (fldidx == NHC.BL_TITLE)
                    txtstr = cptr.add(txtstr, BigInt.asUintN(64, BigInt.asUintN(64, cptr.strlen(svp) + 6n) - 1n));
                if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.TXT_VALUE && cptr.ld1so2(hl, 0, 1, $hilite_s_textmatch)) {
                    if (fuzzymatch(cptr.add(hl, $hilite_s_textmatch), txtstr, __s_quot_sp_dash_us, 1)) {
                        rule = hl;
                        exactmatch = 1;
                    } else if (exactmatch) {
                        ;
                    } else if (fldidx == NHC.BL_TITLE && Upolyd() && noneoftheabove(cptr.add(hl, $hilite_s_textmatch))) {
                        rule = hl;
                    }
                }
                break;
                case NHM.BL_TH_ALWAYS_HILITE:
                rule = hl;
                break;
                case NHM.BL_TH_CRITICALHP:
                if (fldidx == NHC.BL_HP && critically_low_hp(0)) {
                    rule = hl;
                    crit_hp = 1;
                    updown = (changed = (perc_or_abs = 0));
                }
                break;
                case NHM.BL_TH_NONE:
                break;
                default:
                break;
            }
        }
    }
    cptr.stI32(colorptr, rule ? cptr.ldI32o(rule, $hilite_s_coloridx) : NHM.NO_COLOR);
    return rule;
}

/** C ref: botl.c:2576 — @param {CInt} idx @param {CPtr<int>} coloridx @param {CPtr<int>} attrib */
function split_clridx(idx, coloridx, attrib) {
    if (coloridx)
        cptr.stI32(coloridx, idx & 255);
    if (attrib)
        cptr.stI32(attrib, (idx >> 8) & 255);
}

/** C ref: botl.c:2593 — @param {CPtr<char>} op @param {CInt} from_configfile @returns {CInt} */
export function parse_status_hl1(op, from_configfile) {
    let hsbuf = (function () { const flat = new Uint8Array(21 * (128 * 1)); const a = []; for (let r = 0; r < 21; r++) a.push(flat.subarray(r * (128 * 1), (r + 1) * (128 * 1))); a.buf = flat; return a; })();
    let rslt;
    let badopt = 0;
    let i;
    let fldnum;
    let ccount = 0;
    let c;
    fldnum = 0;
    for (i = 0; i < 21; ++i) {
        cptr.st1o(cptr.decay(hsbuf[i]), 0, 0, 1);
    }
    while (cptr.ld1s(op) && fldnum < 21 && ccount < 126) {
        c = lowc(cptr.ld1s(op));
        if (c == 32) {
            if (fldnum >= 1) {
                if (fldnum == 1 && strncmpi(cptr.decay((hsbuf[0])), (__s_title), -1) == 0) {
                    cptr.st1o(cptr.decay(hsbuf[fldnum]), ccount++, c, 1);
                    cptr.st1o(cptr.decay(hsbuf[fldnum]), ccount, 0, 1);
                    op = cptr.add(op, 1);
                    continue;
                }
                rslt = parse_status_hl2(cptr.decay(hsbuf), from_configfile);
                if (!rslt) {
                    badopt = 1;
                    break;
                }
            }
            for (i = 0; i < 21; ++i) {
                cptr.st1o(cptr.decay(hsbuf[i]), 0, 0, 1);
            }
            fldnum = 0;
            ccount = 0;
        } else if (c == 47) {
            fldnum++;
            ccount = 0;
        } else {
            cptr.st1o(cptr.decay(hsbuf[fldnum]), ccount++, c, 1);
            cptr.st1o(cptr.decay(hsbuf[fldnum]), ccount, 0, 1);
        }
        op = cptr.add(op, 1);
    }
    if (fldnum >= 1 && !badopt) {
        rslt = parse_status_hl2(cptr.decay(hsbuf), from_configfile);
        if (!rslt)
            badopt = 1;
    }
    if (badopt)
        return 0;
    if (!cptr.ldI64o(iflags, $instance_flags_hilite_delta))
        cptr.stI64o(iflags, $instance_flags_hilite_delta, 3n);
    return 1;
}

/** C ref: botl.c:2652 — @param {CPtr<char>} str @returns {CInt} */
function is_ltgt_percentnumber(str) {
    let s = str;
    if (cptr.ld1s(s) == 60 || cptr.ld1s(s) == 62)
        s = cptr.add(s, 1);
    if (cptr.ld1s(s) == 61)
        s = cptr.add(s, 1);
    if (cptr.ld1s(s) == 45 || cptr.ld1s(s) == 43)
        s = cptr.add(s, 1);
    if (!digit(cptr.ld1s(s)))
        return 0;
    while (digit(cptr.ld1s(s)))
        s = cptr.add(s, 1);
    if (cptr.ld1s(s) == 37)
        s = cptr.add(s, 1);
    return schar((cptr.ld1s(s) == 0));
}

/** C ref: botl.c:2673 — @param {CPtr<char>} str @returns {CInt} */
function has_ltgt_percentnumber(str) {
    let s = str;
    while (cptr.ld1s(s)) {
        if (!cptr.strchr(__s_0123456789, cptr.ld1s(s)))
            return 0;
        s = cptr.add(s, 1);
    }
    return 1;
}

const __static_splitsubfields_subfields = cptr.alloc(16 * 8); /** C ref: botl.c:2691 — char *[16] (function-static) */

/** C ref: botl.c:2688 — @param {CPtr<char>} str @param {CPtr<char **>} sfarr @param {CInt} maxsf @returns {CInt} */
function splitsubfields(str, sfarr, maxsf) {
    let st = null;
    let sf = 0;
    if (!str)
        return 0;
    for (sf = 0; sf < 16; ++sf)
        cptr.stPtro(__static_splitsubfields_subfields, sf, null, 8);
    maxsf = (maxsf == 0) ? 16 : ((maxsf) < 16 ? (maxsf) : 16);
    if (cptr.strchr(str, 43) || cptr.strchr(str, 38)) {
        let c = str;
        sf = 0;
        st = c;
        while (cptr.ld1s(c) && sf < maxsf) {
            if (cptr.ld1s(c) == 38 || cptr.ld1s(c) == 43) {
                cptr.st1(c, 0);
                cptr.stPtro(__static_splitsubfields_subfields, sf, st, 8);
                st = cptr.add(c, 1);
                sf++;
            }
            c = cptr.add(c, 1);
        }
        if (sf >= ((maxsf - 1) | 0))
            return -1;
        if (!cptr.ld1s(c) && !cptr.eq(c, st))
            cptr.stPtro(__static_splitsubfields_subfields, sf++, st, 8);
    } else {
        sf = 1;
        cptr.stPtro(__static_splitsubfields_subfields, 0, str, 8);
    }
    cptr.stPtr(sfarr, __static_splitsubfields_subfields);
    return sf;
}

/** C ref: botl.c:2730 — @param {CPtr<char>} str @param {CPtr<char *>} arr @param {CInt} arrmin @param {CInt} arrmax @param {CPtr<int>} retidx @returns {CInt} */
function is_fld_arrayvalues(str, arr, arrmin, arrmax, retidx) {
    let i;
    for (i = arrmin; i < arrmax; i++)
        if (!strncmpi((str), (cptr.ldPtro(arr, i, 8)), -1)) {
            cptr.stI32(retidx, i);
            return 1;
        }
    return 0;
}

/** C ref: botl.c:2747 — @param {CPtr<char>} querystr @param {CPtr<char *>} arr @param {CInt} arrmin @param {CInt} arrmax @returns {CInt} */
function query_arrayvalue(querystr, arr, arrmin, arrmax) {
    let i;
    let res;
    let ret = (arrmin - 1) | 0;
    let tmpwin;
    let any = cptr.alloc(8);
    let picks = cptr.box(null);
    let adj = (arrmin > 0) ? 1 : arrmax;
    let clr = NHM.NO_COLOR;
    tmpwin = create_nhwindow()(NHM.NHW_MENU);
    start_menu()(tmpwin, 0n);
    for (i = arrmin; i < arrmax; i++) {
        if (!cptr.ldPtro(arr, i, 8))
            continue;
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, (i + adj) | 0);
        add_menu(tmpwin, nul_glyphinfo.v, any, 0, 0, NHM.ATR_NONE, clr, cptr.ldPtro(arr, i, 8), NHM.MENU_ITEMFLAGS_NONE);
    }
    end_menu()(tmpwin, querystr);
    res = select_menu(tmpwin, NHM.PICK_ONE, picks);
    destroy_nhwindow()(tmpwin);
    if (res > 0) {
        ret = (cptr.ldI32(picks.v) - adj) | 0;
        cptr.free(picks.v);
    }
    return ret;
}

/** C ref: botl.c:2784 — @param {CInt} fld @param {CPtr<struct hilite_s>} hilite */
function status_hilite_add_threshold(fld, hilite) {
    let new_hilite;
    let old_hilite;
    if (!hilite)
        return;
    new_hilite = alloc(128);
    cptr.memcpy(new_hilite, hilite, 128);
    cptr.st1o(new_hilite, $hilite_s_set, 1);
    cptr.stI32(new_hilite, fld);
    cptr.stPtro(new_hilite, $hilite_s_next, null);
    if (!cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, fld, $sizeof_istat_s, $istat_s_thresholds)) {
        cptr.stPtro3(gb, 0, $sizeof_istat_s_x27, fld, $sizeof_istat_s, $istat_s_thresholds, new_hilite);
    } else {
        for (old_hilite = cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, fld, $sizeof_istat_s, $istat_s_thresholds); cptr.ldPtro(old_hilite, $hilite_s_next); old_hilite = cptr.ldPtro(old_hilite, $hilite_s_next))
            continue;
        cptr.stPtro(old_hilite, $hilite_s_next, new_hilite);
    }
    cptr.stPtro3(gb, 1, $sizeof_istat_s_x27, fld, $sizeof_istat_s, $istat_s_thresholds, cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, fld, $sizeof_istat_s, $istat_s_thresholds));
}

const __static_parse_status_hl2_aligntxt = cptr.alloc(3 * 8);
cptr.stPtro(__static_parse_status_hl2_aligntxt, 0, __s_chaotic__3);
cptr.stPtro(__static_parse_status_hl2_aligntxt, 8, __s_neutral__3);
cptr.stPtro(__static_parse_status_hl2_aligntxt, 16, __s_lawful__3); /** C ref: botl.c:2816 — char *[3] (function-static) */
const __static_parse_status_hl2_hutxt = cptr.alloc(7 * 8);
cptr.stPtro(__static_parse_status_hl2_hutxt, 0, __s_satiated);
cptr.stPtro(__static_parse_status_hl2_hutxt, 8, __s_empty);
cptr.stPtro(__static_parse_status_hl2_hutxt, 16, __s_hungry);
cptr.stPtro(__static_parse_status_hl2_hutxt, 24, __s_weak);
cptr.stPtro(__static_parse_status_hl2_hutxt, 32, __s_fainting);
cptr.stPtro(__static_parse_status_hl2_hutxt, 40, __s_fainted);
cptr.stPtro(__static_parse_status_hl2_hutxt, 48, __s_starved); /** C ref: botl.c:2819 — char *[7] (function-static) */

/** C ref: botl.c:2814 — @param {CPtr<char [128]>} s @param {CInt} from_configfile @returns {CInt} */
function parse_status_hl2(s, from_configfile) {
    let tmp;
    let how;
    let sidx = 0;
    let i = -1;
    let dt = NHC.ANY_INVALID;
    let coloridx = -1;
    let successes = 0;
    let disp_attrib = 0;
    let percent;
    let changed;
    let numeric;
    let down;
    let up;
    let grt;
    let lt;
    let gte;
    let le;
    let eq;
    let txtval;
    let always;
    let criticalhp;
    let txt;
    let fld = NHC.BL_FLUSH;
    let hilite = cptr.alloc(128);
    let tmpbuf = new Uint8Array(256);
    fld = fldname_to_bl_indx(cptr.add(s, sidx, 128));
    if (fld == NHC.BL_CHARACTERISTICS) {
        let res = 0;
        for (fld = NHC.BL_STR; fld <= NHC.BL_CH; fld++) {
            void cptr.strcpy(cptr.add(s, sidx, 128), cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
            res = parse_status_hl2(s, from_configfile);
            if (!res)
                return 0;
        }
        return 1;
    }
    if (fld == NHC.BL_FLUSH) {
        config_error_add(__s_unknown_status_field_s, cptr.add(s, sidx, 128));
        return 0;
    }
    if (fld == NHC.BL_CONDITION)
        return parse_condition(s, sidx);
    ++sidx;
    while (cptr.ld1so3(s, sidx, 128, 0, 1, 0)) {
        let buf = new Uint8Array(256);
        let subfields = cptr.box(0);
        let sf = 0;
        let kidx = cptr.box(0);
        txt = null;
        percent = (numeric = (always = 0));
        down = (up = (changed = 0));
        criticalhp = 0;
        grt = (gte = (eq = (le = (lt = (txtval = 0)))));
        __builtin___memset_chk(hilite, 0, 128n, __builtin_object_size(hilite, 0));
        cptr.st1o(hilite, $hilite_s_set, 0);
        cptr.stI32(hilite, fld);
        if (cptr.ld1so(s, (sidx + 1) | 0, 128) == 0 || !strncmpi((cptr.add(s, sidx, 128)), (__s_always), -1)) {
            always = 1;
            if (cptr.ld1so(s, (sidx + 1) | 0, 128) == 0)
                sidx--;
        } else if (!strncmpi((cptr.add(s, sidx, 128)), (__s_up), -1) || !strncmpi((cptr.add(s, sidx, 128)), (__s_down), -1)) {
            if (cptr.ldI32o2(initblstats, fld, $sizeof_istat_s, $istat_s_anytype) == NHC.ANY_STR)
                ;
            else if (!strncmpi((cptr.add(s, sidx, 128)), (__s_down), -1))
                down = 1;
            else
                up = 1;
            changed = 1;
        } else if (fld == NHC.BL_CAP && is_fld_arrayvalues(cptr.add(s, sidx, 128), enc_stat, NHC.SLT_ENCUMBER, ((NHC.OVERLOADED + 1) | 0), kidx)) {
            txt = cptr.ldPtro(enc_stat, kidx.v, 8);
            txtval = 1;
        } else if (fld == NHC.BL_ALIGN && is_fld_arrayvalues(cptr.add(s, sidx, 128), __static_parse_status_hl2_aligntxt, 0, 3, kidx)) {
            txt = cptr.ldPtro(__static_parse_status_hl2_aligntxt, kidx.v, 8);
            txtval = 1;
        } else if (fld == NHC.BL_HUNGER && is_fld_arrayvalues(cptr.add(s, sidx, 128), __static_parse_status_hl2_hutxt, NHC.SATIATED, ((NHC.STARVED + 1) | 0), kidx)) {
            txt = cptr.ldPtro(hu_stat, kidx.v, 8);
            txtval = 1;
        } else if (!strncmpi((cptr.add(s, sidx, 128)), (__s_changed), -1)) {
            changed = 1;
        } else if (fld == NHC.BL_HP && !strncmpi((cptr.add(s, sidx, 128)), (__s_criticalhp), -1)) {
            criticalhp = 1;
        } else if (is_ltgt_percentnumber(cptr.add(s, sidx, 128))) {
            let op;
            tmp = cptr.add(s, sidx, 128);
            if (cptr.strchr(tmp, 37))
                percent = 1;
            if (cptr.ld1s(tmp) == 60) {
                if (cptr.ld1so(tmp, 1) == 61)
                    le = 1;
                else
                    lt = 1;
            } else if (cptr.ld1s(tmp) == 62) {
                if (cptr.ld1so(tmp, 1) == 61)
                    gte = 1;
                else
                    grt = 1;
            }
            tmp = stripchars(cptr.decay(tmpbuf), __s_pct_lt_gt_eq_plus, tmp);
            numeric = 1;
            dt = (percent ? NHC.ANY_INT : cptr.ldI32o2(initblstats, fld, $sizeof_istat_s, $istat_s_anytype)) | 0;
            void s_to_anything(cptr.add(hilite, $hilite_s_value), tmp, dt);
            op = grt ? __s_gt : (gte ? __s_gt_eq : (lt ? __s_lt : (le ? __s_lt_eq : __s_eq)));
            if (dt == NHC.ANY_INT && (cptr.ldI32o(hilite, $hilite_s_value) < ((fld == NHC.BL_AC) ? -128 : (grt ? -1 : (lt ? 1 : 0))) || cptr.ldI32o(hilite, $hilite_s_value) > (percent ? (lt ? 101 : 100) : NHM.LARGEST_INT))) {
                config_error_add(__s_s_s_d_s_s, cptr.decay(threshold_value), op, cptr.ldI32o(hilite, $hilite_s_value), percent ? __s_pct : __s_empty, cptr.decay(is_out_of_range));
                return 0;
            } else if (dt == NHC.ANY_LONG && cptr.ldI64o(hilite, $hilite_s_value) < (grt ? -1n : (lt ? 1n : 0n))) {
                config_error_add(__s_s_s_ld_s, cptr.decay(threshold_value), op, cptr.ldI64o(hilite, $hilite_s_value), cptr.decay(is_out_of_range));
                return 0;
            }
        } else if (cptr.ldI32o2(initblstats, fld, $sizeof_istat_s, $istat_s_anytype) == NHC.ANY_STR) {
            txt = cptr.add(s, sidx, 128);
            txtval = 1;
        } else {
            config_error_add(has_ltgt_percentnumber(cptr.add(s, sidx, 128)) ? __s_wrong_format_s_expected_a_threshold : __s_unknown_behavior_s, cptr.add(s, sidx, 128));
            return 0;
        }
        if (grt || up)
            cptr.stI32o(hilite, $hilite_s_rel, NHC.GT_VALUE);
        else if (lt || down)
            cptr.stI32o(hilite, $hilite_s_rel, NHC.LT_VALUE);
        else if (gte)
            cptr.stI32o(hilite, $hilite_s_rel, NHC.GE_VALUE);
        else if (le)
            cptr.stI32o(hilite, $hilite_s_rel, NHC.LE_VALUE);
        else if (eq || percent || numeric || changed)
            cptr.stI32o(hilite, $hilite_s_rel, NHC.EQ_VALUE);
        else if (txtval)
            cptr.stI32o(hilite, $hilite_s_rel, NHC.TXT_VALUE);
        else
            cptr.stI32o(hilite, $hilite_s_rel, NHC.LT_VALUE);
        if (cptr.ldI32o2(initblstats, fld, $sizeof_istat_s, $istat_s_anytype) == NHC.ANY_STR && (percent || numeric)) {
            config_error_add(__s_field_s_does_not_support_numeric_values, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
            return 0;
        }
        if (percent) {
            if (cptr.ldI32o2(initblstats, fld, $sizeof_istat_s, $istat_s_idxmax) < 0) {
                config_error_add(__s_cannot_use_percent_with_s, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
                return 0;
            } else if ((cptr.ldI32o(hilite, $hilite_s_value) < -1) || (cptr.ldI32o(hilite, $hilite_s_value) == -1 && cptr.ldI32o(hilite, $hilite_s_value) != NHC.GT_VALUE) || (cptr.ldI32o(hilite, $hilite_s_value) == 0 && cptr.ldI32o(hilite, $hilite_s_rel) == NHC.LT_VALUE) || (cptr.ldI32o(hilite, $hilite_s_value) == 100 && cptr.ldI32o(hilite, $hilite_s_rel) == NHC.GT_VALUE) || (cptr.ldI32o(hilite, $hilite_s_value) == 101 && cptr.ldI32o(hilite, $hilite_s_value) != NHC.LT_VALUE) || (cptr.ldI32o(hilite, $hilite_s_value) > 101)) {
                config_error_add(__s_hilite_status_invalid_percentage_value, (cptr.ldI32o(hilite, $hilite_s_rel) == NHC.LT_VALUE) ? __s_lt : ((cptr.ldI32o(hilite, $hilite_s_rel) == NHC.LE_VALUE) ? __s_lt_eq : ((cptr.ldI32o(hilite, $hilite_s_rel) == NHC.GT_VALUE) ? __s_gt : ((cptr.ldI32o(hilite, $hilite_s_rel) == NHC.GE_VALUE) ? __s_gt_eq : __s_eq))), cptr.ldI32o(hilite, $hilite_s_value));
                return 0;
            }
        }
        sidx++;
        how = cptr.add(s, sidx, 128);
        if (!how) {
            if (!successes)
                return 0;
        }
        coloridx = -1;
        void cptr.strcpy(cptr.decay(buf), how);
        sf = splitsubfields(cptr.decay(buf), subfields, 0);
        if (sf < 1)
            return 0;
        disp_attrib = NHC.HL_UNDEF;
        for (i = 0; i < sf; ++i) {
            let a = match_str2attr(cptr.ldPtro(subfields.v, i, 8), 0);
            if (a == NHM.ATR_BOLD)
                disp_attrib |= NHC.HL_BOLD;
            else if (a == NHM.ATR_DIM)
                disp_attrib |= NHC.HL_DIM;
            else if (a == NHM.ATR_ITALIC)
                disp_attrib |= NHC.HL_ITALIC;
            else if (a == NHM.ATR_ULINE)
                disp_attrib |= NHC.HL_ULINE;
            else if (a == NHM.ATR_BLINK)
                disp_attrib |= NHC.HL_BLINK;
            else if (a == NHM.ATR_INVERSE)
                disp_attrib |= NHC.HL_INVERSE;
            else if (a == NHM.ATR_NONE)
                disp_attrib = NHC.HL_NONE;
            else {
                let c = match_str2clr(cptr.ldPtro(subfields.v, i, 8), 0);
                if (c >= NHM.CLR_MAX || coloridx != -1) {
                    config_error_add(__s_bad_color_d_d, c, coloridx);
                    return 0;
                }
                coloridx = c;
            }
        }
        if (coloridx == -1)
            coloridx = NHM.NO_COLOR;
        cptr.stI32o(hilite, $hilite_s_coloridx, coloridx | (disp_attrib << 8));
        if (always)
            cptr.stI32o(hilite, $hilite_s_behavior, NHM.BL_TH_ALWAYS_HILITE);
        else if (percent)
            cptr.stI32o(hilite, $hilite_s_behavior, NHM.BL_TH_VAL_PERCENTAGE);
        else if (changed)
            cptr.stI32o(hilite, $hilite_s_behavior, NHM.BL_TH_UPDOWN);
        else if (numeric)
            cptr.stI32o(hilite, $hilite_s_behavior, NHM.BL_TH_VAL_ABSOLUTE);
        else if (txtval)
            cptr.stI32o(hilite, $hilite_s_behavior, NHM.BL_TH_TEXTMATCH);
        else if (cptr.ldPtro(hilite, $hilite_s_value))
            cptr.stI32o(hilite, $hilite_s_behavior, NHM.BL_TH_VAL_ABSOLUTE);
        else if (criticalhp)
            cptr.stI32o(hilite, $hilite_s_behavior, NHM.BL_TH_CRITICALHP);
        else
            cptr.stI32o(hilite, $hilite_s_behavior, NHM.BL_TH_NONE);
        cptr.stI32o(hilite, $hilite_s_anytype, dt);
        if (cptr.ldI32o(hilite, $hilite_s_behavior) == NHM.BL_TH_TEXTMATCH && txt) {
            void __builtin___strncpy_chk(cptr.add(hilite, $hilite_s_textmatch), txt, 80n, __builtin_object_size(cptr.add(hilite, $hilite_s_textmatch), 1));
            cptr.st1o2(hilite, 79n, 1, $hilite_s_textmatch, 0);
            void trimspaces(cptr.add(hilite, $hilite_s_textmatch));
        }
        status_hilite_add_threshold(fld, hilite);
        successes++;
        sidx++;
    }
    return schar((successes > 0));
}

/** C ref: botl.c:3109 @returns {CLongLong} */
function query_conditions() {
    let i;
    let res;
    let ret = 0n;
    let tmpwin;
    let any = cptr.alloc(8);
    let picks = cptr.box(null);
    let clr = NHM.NO_COLOR;
    tmpwin = create_nhwindow()(NHM.NHW_MENU);
    start_menu()(tmpwin, 0n);
    for (i = 0; i < 30; i++) {
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stU64(any, BigInt.asUintN(64, cptr.ldI64o2(conditions, i, $sizeof_conditions_t, $conditions_t_mask)));
        add_menu(tmpwin, nul_glyphinfo.v, any, 0, 0, NHM.ATR_NONE, clr, cptr.ldPtro3(conditions, i, $sizeof_conditions_t, 0, 8, $conditions_t_text), NHM.MENU_ITEMFLAGS_NONE);
    }
    end_menu()(tmpwin, __s_choose_status_conditions);
    res = select_menu(tmpwin, NHM.PICK_ANY, picks);
    destroy_nhwindow()(tmpwin);
    if (res > 0) {
        for (i = 0; i < res; i++)
            ret |= cptr.ldU64o(picks.v, i, $sizeof_menu_item);
        cptr.free(picks.v);
    }
    return ret;
}

const __static_conditionbitmask2str_buf = new Uint8Array(256); /** C ref: botl.c:3143 — char[256] (function-static) */

/** C ref: botl.c:3141 — @param {CLongLong} ul @returns {CPtr<char>} */
function conditionbitmask2str(ul) {
    let i;
    let first = 1;
    let alias = null;
    cptr.st1o(cptr.decay(__static_conditionbitmask2str_buf), 0, 0, 1);
    if (!ul)
        return cptr.decay(__static_conditionbitmask2str_buf);
    for (i = 1; i < 6; i++)
        if (cptr.ldU64o2(condition_aliases, i, $sizeof_condmap, $condmap_bitmask) == ul)
            alias = cptr.ldPtro(condition_aliases, i, $sizeof_condmap);
    for (i = 0; i < 30; i++)
        if ((BigInt.asUintN(64, cptr.ldI64o2(conditions, i, $sizeof_conditions_t, $conditions_t_mask)) & ul) != 0n) {
            void cptr.sprintf(eos(cptr.decay(__static_conditionbitmask2str_buf)), __s_s_s__2, (first) ? __s_empty : __s_plus, cptr.ldPtro3(conditions, i, $sizeof_conditions_t, 0, 8, $conditions_t_text));
            first = 0;
        }
    if (!first && alias)
        void cptr.sprintf(cptr.decay(__static_conditionbitmask2str_buf), __s_pct_s, alias);
    return cptr.decay(__static_conditionbitmask2str_buf);
}

/** C ref: botl.c:3171 — @param {CPtr<char>} str @returns {CLongLong} */
function match_str2conditionbitmask(str) {
    let i;
    let nmatches = 0;
    let mask = 0n;
    if (str && cptr.ld1s(str)) {
        for (i = 0; i < 30; i++)
            if (fuzzymatch(cptr.ldPtro3(conditions, i, $sizeof_conditions_t, 0, 8, $conditions_t_text), str, __s_sp_dash_us, 1)) {
                mask |= BigInt.asUintN(64, cptr.ldI64o2(conditions, i, $sizeof_conditions_t, $conditions_t_mask));
                nmatches++;
            }
        if (!nmatches) {
            for (i = 0; i < 6; i++)
                if (fuzzymatch(cptr.ldPtro(condition_aliases, i, $sizeof_condmap), str, __s_sp_dash_us, 1)) {
                    mask |= cptr.ldU64o2(condition_aliases, i, $sizeof_condmap, $condmap_bitmask);
                    nmatches++;
                }
        }
        if (!nmatches) {
            let len = Number(BigInt.asIntN(32, cptr.strlen(str)));
            for (i = 0; i < 6; i++)
                if (!strncmpi(str, cptr.ldPtro(condition_aliases, i, $sizeof_condmap), len)) {
                    mask |= cptr.ldU64o2(condition_aliases, i, $sizeof_condmap, $condmap_bitmask);
                    nmatches++;
                }
        }
    }
    return mask;
}

/** C ref: botl.c:3209 — @param {CPtr<char>} str @returns {CLongLong} */
function str2conditionbitmask(str) {
    let conditions_bitmask = 0n;
    let subfields = cptr.box(0);
    let i;
    let sf;
    sf = splitsubfields(str, subfields, 30);
    if (sf < 1)
        return 0n;
    for (i = 0; i < sf; ++i) {
        let bm = match_str2conditionbitmask(cptr.ldPtro(subfields.v, i, 8));
        if (!bm) {
            config_error_add(__s_unknown_condition_s, cptr.ldPtro(subfields.v, i, 8));
            return 0n;
        }
        conditions_bitmask |= bm;
    }
    return conditions_bitmask;
}

/** C ref: botl.c:3233 — @param {CPtr<char [128]>} s @param {CInt} sidx @returns {CInt} */
function parse_condition(s, sidx) {
    let i;
    let coloridx = NHM.NO_COLOR;
    let tmp;
    let how;
    let conditions_bitmask = 0n;
    let result = 0;
    if (!s)
        return 0;
    sidx++;
    if (!cptr.ld1so3(s, sidx, 128, 0, 1, 0)) {
        config_error_add(__s_missing_condition_s);
        return 0;
    }
    while (cptr.ld1so3(s, sidx, 128, 0, 1, 0)) {
        let sf = 0;
        let buf = new Uint8Array(256);
        let subfields = cptr.box(0);
        tmp = cptr.add(s, sidx, 128);
        void cptr.strcpy(cptr.decay(buf), tmp);
        conditions_bitmask = str2conditionbitmask(cptr.decay(buf));
        if (!conditions_bitmask)
            return 0;
        sidx++;
        how = cptr.add(s, sidx, 128);
        if (!how || !cptr.ld1s(how)) {
            config_error_add(__s_missing_color_attribute);
            return 0;
        }
        void cptr.strcpy(cptr.decay(buf), how);
        sf = splitsubfields(cptr.decay(buf), subfields, 0);
        for (i = 0; i < sf; ++i) {
            let a = match_str2attr(cptr.ldPtro(subfields.v, i, 8), 0);
            if (a == NHM.ATR_BOLD)
                cptr.stU64o2(gc, 18, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 18, 8, $instance_globals_c_cond_hilites) | conditions_bitmask);
            else if (a == NHM.ATR_DIM)
                cptr.stU64o2(gc, 19, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 19, 8, $instance_globals_c_cond_hilites) | conditions_bitmask);
            else if (a == NHM.ATR_ITALIC)
                cptr.stU64o2(gc, 20, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 20, 8, $instance_globals_c_cond_hilites) | conditions_bitmask);
            else if (a == NHM.ATR_ULINE)
                cptr.stU64o2(gc, 21, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 21, 8, $instance_globals_c_cond_hilites) | conditions_bitmask);
            else if (a == NHM.ATR_BLINK)
                cptr.stU64o2(gc, 22, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 22, 8, $instance_globals_c_cond_hilites) | conditions_bitmask);
            else if (a == NHM.ATR_INVERSE)
                cptr.stU64o2(gc, 23, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 23, 8, $instance_globals_c_cond_hilites) | conditions_bitmask);
            else if (a == NHM.ATR_NONE) {
                cptr.stU64o2(gc, 18, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 18, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~conditions_bitmask));
                cptr.stU64o2(gc, 19, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 19, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~conditions_bitmask));
                cptr.stU64o2(gc, 20, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 20, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~conditions_bitmask));
                cptr.stU64o2(gc, 21, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 21, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~conditions_bitmask));
                cptr.stU64o2(gc, 22, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 22, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~conditions_bitmask));
                cptr.stU64o2(gc, 23, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 23, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~conditions_bitmask));
            } else {
                let k = match_str2clr(cptr.ldPtro(subfields.v, i, 8), 0);
                if (k >= NHM.CLR_MAX) {
                    config_error_add(__s_bad_color_d, k);
                    return 0;
                }
                coloridx = k;
            }
        }
        cptr.stU64o2(gc, coloridx, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, coloridx, 8, $instance_globals_c_cond_hilites) | conditions_bitmask);
        result = 1;
        sidx++;
    }
    return result;
}

/** C ref: botl.c:3351 */
export function clear_status_hilites() {
    let i;
    for (i = 0; i < NHC.MAXBLSTATS; ++i) {
        let temp;
        let next;
        for (temp = cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_thresholds); temp; temp = next) {
            next = cptr.ldPtro(temp, $hilite_s_next);
            cptr.free(temp);
        }
        cptr.stPtro3(gb, 0, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_thresholds, cptr.stPtro3(gb, 1, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_thresholds, null));
        cptr.stPtro3(gb, 0, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_hilite_rule, cptr.stPtro3(gb, 1, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_hilite_rule, null));
    }
}

/** C ref: botl.c:3369 — @param {CInt} attrib @param {CPtr<char>} buf @param {CLongLong} bufsz @returns {CPtr<char>} */
function hlattr2attrname(attrib, buf, bufsz) {
    if (attrib && buf) {
        let attbuf = new Uint8Array(256);
        let first = 0;
        let k;
        cptr.st1o(cptr.decay(attbuf), 0, 0, 1);
        if (attrib == NHC.HL_NONE) {
            void cptr.strcpy(buf, __s_normal);
            return buf;
        }
        if (attrib & NHC.HL_BOLD)
            void cptr.strcat(cptr.decay(attbuf), first++ ? __s_bold : __s_bold__2);
        if (attrib & NHC.HL_DIM)
            void cptr.strcat(cptr.decay(attbuf), first++ ? __s_dim : __s_dim__2);
        if (attrib & NHC.HL_ITALIC)
            void cptr.strcat(cptr.decay(attbuf), first++ ? __s_italic : __s_italic__2);
        if (attrib & NHC.HL_ULINE)
            void cptr.strcat(cptr.decay(attbuf), first++ ? __s_underline : __s_underline__2);
        if (attrib & NHC.HL_BLINK)
            void cptr.strcat(cptr.decay(attbuf), first++ ? __s_blink : __s_blink__2);
        if (attrib & NHC.HL_INVERSE)
            void cptr.strcat(cptr.decay(attbuf), first++ ? __s_inverse : __s_inverse__2);
        k = cptr.strlen(cptr.decay(attbuf));
        if (k < (BigInt.asUintN(64, bufsz - 1n)))
            void cptr.strcpy(buf, cptr.decay(attbuf));
        return buf;
    }
    return null;
}

/** C ref: botl.c:3403 — struct _status_hilite_line_str { id, fld, hl, mask, str, next } (memory model v0.5) */

/** C ref: botl.c:3413 — struct _status_hilite_line_str * */
let status_hilite_str = null;

/** C ref: botl.c:3414 — int */
let status_hilite_str_id = 0;

/** C ref: botl.c:3417 — @param {CInt} fld @param {CPtr<struct hilite_s>} hl @param {CLongLong} mask @param {CPtr<char>} str */
function status_hilite_linestr_add(fld, hl, mask, str) {
    let tmp;
    let nxt;
    tmp = alloc(288);
    void __builtin___memset_chk(tmp, 0, 288n, __builtin_object_size(tmp, 0));
    cptr.stPtro(tmp, $_status_hilite_line_str_next, null);
    cptr.stI32(tmp, ++status_hilite_str_id);
    cptr.stI32o(tmp, $_status_hilite_line_str_fld, fld);
    cptr.stPtro(tmp, $_status_hilite_line_str_hl, hl);
    cptr.stU64o(tmp, $_status_hilite_line_str_mask, mask);
    if (fld == NHC.BL_TITLE)
        void cptr.strcpy(cptr.add(tmp, $_status_hilite_line_str_str), str);
    else
        void stripchars(cptr.add(tmp, $_status_hilite_line_str_str), __s_sp, str);
    if ((nxt = status_hilite_str) !== null) {
        while (cptr.ldPtro(nxt, $_status_hilite_line_str_next))
            nxt = cptr.ldPtro(nxt, $_status_hilite_line_str_next);
        cptr.stPtro(nxt, $_status_hilite_line_str_next, tmp);
    } else {
        status_hilite_str = tmp;
    }
}

/** C ref: botl.c:3448 */
function status_hilite_linestr_done() {
    let nxt;
    let tmp = status_hilite_str;
    while (tmp) {
        nxt = cptr.ldPtro(tmp, $_status_hilite_line_str_next);
        cptr.free(tmp);
        tmp = nxt;
    }
    status_hilite_str = null;
    status_hilite_str_id = 0;
}

/** C ref: botl.c:3462 — @param {CInt} fld @returns {CInt} */
function status_hilite_linestr_countfield(fld) {
    let tmp;
    let countall = schar((fld == NHC.BL_FLUSH));
    let count = 0;
    for (tmp = status_hilite_str; tmp; tmp = cptr.ldPtro(tmp, $_status_hilite_line_str_next)) {
        if (countall || cptr.ldI32o(tmp, $_status_hilite_line_str_fld) == fld)
            count++;
    }
    return count;
}

/** C ref: botl.c:3477 @returns {CInt} */
export function count_status_hilites() {
    let count;
    status_hilite_linestr_gather();
    count = status_hilite_linestr_countfield(NHC.BL_FLUSH);
    status_hilite_linestr_done();
    return count;
}

/** C ref: botl.c:3488 */
function status_hilite_linestr_gather_conditions() {
    let i;
    let cond_maps = cptr.alloc(30 * $sizeof__cond_map);
    void __builtin___memset_chk(cond_maps, 0, BigInt.asUintN(64, BigInt.asUintN(64, BigInt(30)) * 16n), __builtin_object_size(cond_maps, 0));
    for (i = 0; i < 30; i++) {
        let clr = cptr.box(NHM.NO_COLOR);
        let atr = cptr.box(NHC.HL_NONE);
        let j;
        for (j = 0; j < NHM.CLR_MAX; j++)
            if (cptr.ldU64o2(gc, j, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, cptr.ldI64o2(conditions, i, $sizeof_conditions_t, $conditions_t_mask))) {
                clr.v = j;
                break;
            }
        if (cptr.ldU64o2(gc, 18, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, cptr.ldI64o2(conditions, i, $sizeof_conditions_t, $conditions_t_mask)))
            atr.v |= NHC.HL_BOLD;
        if (cptr.ldU64o2(gc, 19, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, cptr.ldI64o2(conditions, i, $sizeof_conditions_t, $conditions_t_mask)))
            atr.v |= NHC.HL_DIM;
        if (cptr.ldU64o2(gc, 20, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, cptr.ldI64o2(conditions, i, $sizeof_conditions_t, $conditions_t_mask)))
            atr.v |= NHC.HL_ITALIC;
        if (cptr.ldU64o2(gc, 21, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, cptr.ldI64o2(conditions, i, $sizeof_conditions_t, $conditions_t_mask)))
            atr.v |= NHC.HL_ULINE;
        if (cptr.ldU64o2(gc, 22, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, cptr.ldI64o2(conditions, i, $sizeof_conditions_t, $conditions_t_mask)))
            atr.v |= NHC.HL_BLINK;
        if (cptr.ldU64o2(gc, 23, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, cptr.ldI64o2(conditions, i, $sizeof_conditions_t, $conditions_t_mask)))
            atr.v |= NHC.HL_INVERSE;
        if (atr.v != NHC.HL_NONE)
            atr.v &= -2;
        if (clr.v != NHM.NO_COLOR || atr.v != NHC.HL_NONE) {
            let ca = (clr.v | (atr.v << 8)) >>> 0;
            let added_condmap = 0;
            for (j = 0; j < 30; j++)
                if (cptr.ldI32o2(cond_maps, j, $sizeof__cond_map, $_cond_map_clratr) == ca) {
                    cptr.stU64o(cond_maps, j, cptr.ldU64o(cond_maps, j, $sizeof__cond_map) | BigInt.asUintN(64, cptr.ldI64o2(conditions, i, $sizeof_conditions_t, $conditions_t_mask)), $sizeof__cond_map);
                    added_condmap = 1;
                    break;
                }
            if (!added_condmap) {
                for (j = 0; j < 30; j++)
                    if (!cptr.ldU64o(cond_maps, j, $sizeof__cond_map)) {
                        cptr.stU64o(cond_maps, j, BigInt.asUintN(64, cptr.ldI64o2(conditions, i, $sizeof_conditions_t, $conditions_t_mask)), $sizeof__cond_map);
                        cptr.stI32o2(cond_maps, j, $sizeof__cond_map, $_cond_map_clratr, ca);
                        break;
                    }
            }
        }
    }
    for (i = 0; i < 30; i++)
        if (cptr.ldU64o(cond_maps, i, $sizeof__cond_map)) {
            let clr = cptr.box(NHM.NO_COLOR);
            let atr = cptr.box(NHC.HL_NONE);
            split_clridx(cptr.ldI32o2(cond_maps, i, $sizeof__cond_map, $_cond_map_clratr) | 0, clr, atr);
            if (clr.v != NHM.NO_COLOR || atr.v != NHC.HL_NONE) {
                let clrbuf = new Uint8Array(256);
                let attrbuf = new Uint8Array(256);
                let condbuf = new Uint8Array(256);
                let tmpattr;
                void strNsubst(cptr.strcpy(cptr.decay(clrbuf), clr2colorname(clr.v)), __s_sp, __s_dash, 0);
                tmpattr = hlattr2attrname(atr.v, cptr.decay(attrbuf), 256n);
                if (tmpattr)
                    void cptr.sprintf(eos(cptr.decay(clrbuf)), __s_amp_pct_s, tmpattr);
                nh_snprintf(__s_status_hilite_linestr_gather_conditions, 3562, cptr.decay(condbuf), 256n, __s_condition_s_s, conditionbitmask2str(cptr.ldU64o(cond_maps, i, $sizeof__cond_map)), cptr.decay(clrbuf));
                status_hilite_linestr_add(NHC.BL_CONDITION, null, cptr.ldU64o(cond_maps, i, $sizeof__cond_map), cptr.decay(condbuf));
            }
        }
}

/** C ref: botl.c:3570 */
function status_hilite_linestr_gather() {
    let i;
    let hl;
    status_hilite_linestr_done();
    for (i = 0; i < NHC.MAXBLSTATS; i++) {
        hl = cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, i, $sizeof_istat_s, $istat_s_thresholds);
        while (hl) {
            status_hilite_linestr_add(i, hl, 0n, status_hilite2str(hl));
            hl = cptr.ldPtro(hl, $hilite_s_next);
        }
    }
    status_hilite_linestr_gather_conditions();
}

const __static_status_hilite2str_buf = new Uint8Array(256); /** C ref: botl.c:3592 — char[256] (function-static) */

/** C ref: botl.c:3590 — @param {CPtr<struct hilite_s>} hl @returns {CPtr<char>} */
function status_hilite2str(hl) {
    let clr = cptr.box(NHM.NO_COLOR);
    let attr = cptr.box(NHM.ATR_NONE);
    let behavebuf = new Uint8Array(256);
    let clrbuf = new Uint8Array(256);
    let attrbuf = new Uint8Array(256);
    let tmpattr;
    let op;
    if (!hl)
        return null;
    cptr.st1o(cptr.decay(behavebuf), 0, 0, 1);
    cptr.st1o(cptr.decay(clrbuf), 0, 0, 1);
    op = (cptr.ldI32o(hl, $hilite_s_rel) == NHC.LT_VALUE) ? __s_lt : ((cptr.ldI32o(hl, $hilite_s_rel) == NHC.LE_VALUE) ? __s_lt_eq : ((cptr.ldI32o(hl, $hilite_s_rel) == NHC.GT_VALUE) ? __s_gt : ((cptr.ldI32o(hl, $hilite_s_rel) == NHC.GE_VALUE) ? __s_gt_eq : ((cptr.ldI32o(hl, $hilite_s_rel) == NHC.EQ_VALUE) ? __s_eq : null))));
    switch (cptr.ldI32o(hl, $hilite_s_behavior)) {
        case NHM.BL_TH_VAL_PERCENTAGE:
        if (op)
            void cptr.sprintf(cptr.decay(behavebuf), __s_s_d, op, cptr.ldI32o(hl, $hilite_s_value));
        else
            impossible(__s_hl_behavior_percentage_rel_error);
        break;
        case NHM.BL_TH_UPDOWN:
        if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.LT_VALUE)
            void cptr.sprintf(cptr.decay(behavebuf), __s_down);
        else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.GT_VALUE)
            void cptr.sprintf(cptr.decay(behavebuf), __s_up);
        else if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.EQ_VALUE)
            void cptr.sprintf(cptr.decay(behavebuf), __s_changed);
        else
            impossible(__s_hl_behavior_updown_rel_error);
        break;
        case NHM.BL_TH_VAL_ABSOLUTE:
        if (op)
            void cptr.sprintf(cptr.decay(behavebuf), __s_s_d__2, op, cptr.ldI32o(hl, $hilite_s_value));
        else
            impossible(__s_hl_behavior_absolute_rel_error);
        break;
        case NHM.BL_TH_TEXTMATCH:
        if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.TXT_VALUE && cptr.ld1so2(hl, 0, 1, $hilite_s_textmatch))
            void cptr.sprintf(cptr.decay(behavebuf), __s_pct_s, cptr.add(hl, $hilite_s_textmatch));
        else
            impossible(__s_hl_behavior_textmatch_rel_or_textmatch);
        break;
        case NHM.BL_TH_CONDITION:
        if (cptr.ldI32o(hl, $hilite_s_rel) == NHC.EQ_VALUE)
            void cptr.sprintf(cptr.decay(behavebuf), __s_pct_s, conditionbitmask2str(cptr.ldU64o(hl, $hilite_s_value)));
        else
            impossible(__s_hl_behavior_condition_rel_error);
        break;
        case NHM.BL_TH_ALWAYS_HILITE:
        void cptr.sprintf(cptr.decay(behavebuf), __s_always);
        break;
        case NHM.BL_TH_CRITICALHP:
        void cptr.sprintf(cptr.decay(behavebuf), __s_criticalhp);
        break;
        case NHM.BL_TH_NONE:
        break;
        default:
        break;
    }
    split_clridx(cptr.ldI32o(hl, $hilite_s_coloridx), clr, attr);
    void strNsubst(cptr.strcpy(cptr.decay(clrbuf), clr2colorname(clr.v)), __s_sp, __s_dash, 0);
    if (attr.v != NHC.HL_UNDEF) {
        if ((tmpattr = hlattr2attrname(attr.v, cptr.decay(attrbuf), 256n)) !== null)
            void cptr.sprintf(eos(cptr.decay(clrbuf)), __s_amp_pct_s, tmpattr);
    }
    nh_snprintf(__s_status_hilite2str, 3666, cptr.decay(__static_status_hilite2str_buf), 256n, __s_s_s_s, cptr.ldPtro(initblstats, cptr.ldI32(hl), $sizeof_istat_s), cptr.decay(behavebuf), cptr.decay(clrbuf));
    return cptr.decay(__static_status_hilite2str_buf);
}

/** C ref: botl.c:3672 @returns {CInt} */
function status_hilite_menu_choose_field() {
    let tmpwin;
    let i;
    let res;
    let fld = NHC.BL_FLUSH;
    let any = cptr.alloc(8);
    let picks = cptr.box(null);
    let clr = NHM.NO_COLOR;
    tmpwin = create_nhwindow()(NHM.NHW_MENU);
    start_menu()(tmpwin, 0n);
    for (i = 0; i < NHC.MAXBLSTATS; i++) {
        if (cptr.ldI32o2(initblstats, i, $sizeof_istat_s, $istat_s_fld) == NHC.BL_SCORE && !cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, NHC.BL_SCORE, $sizeof_istat_s, $istat_s_thresholds))
            continue;
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, ((i + 1) | 0));
        add_menu(tmpwin, nul_glyphinfo.v, any, 0, 0, NHM.ATR_NONE, clr, cptr.ldPtro(initblstats, i, $sizeof_istat_s), NHM.MENU_ITEMFLAGS_NONE);
    }
    end_menu()(tmpwin, __s_select_a_hilite_field);
    res = select_menu(tmpwin, NHM.PICK_ONE, picks);
    destroy_nhwindow()(tmpwin);
    if (res > 0) {
        fld = (cptr.ldI32(picks.v) - 1) | 0;
        cptr.free(picks.v);
    }
    return fld;
}

/** C ref: botl.c:3707 — @param {CInt} fld @returns {CInt} */
function status_hilite_menu_choose_behavior(fld) {
    let tmpwin;
    let res = 0;
    let beh = -1;
    let any = cptr.alloc(8);
    let picks = cptr.box(null);
    let buf = new Uint8Array(256);
    let at;
    let onlybeh = NHM.BL_TH_NONE;
    let nopts = 0;
    let clr = NHM.NO_COLOR;
    if (fld < 0 || fld >= NHC.MAXBLSTATS)
        return NHM.BL_TH_NONE;
    at = cptr.ldI32o2(initblstats, fld, $sizeof_istat_s, $istat_s_anytype);
    tmpwin = create_nhwindow()(NHM.NHW_MENU);
    start_menu()(tmpwin, 0n);
    if (fld != NHC.BL_CONDITION) {
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, onlybeh = NHM.BL_TH_ALWAYS_HILITE);
        void cptr.sprintf(cptr.decay(buf), __s_always_highlight_s, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
        add_menu(tmpwin, nul_glyphinfo.v, any, 97, 0, NHM.ATR_NONE, clr, cptr.decay(buf), NHM.MENU_ITEMFLAGS_NONE);
        nopts++;
    }
    if (fld == NHC.BL_CONDITION) {
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, onlybeh = NHM.BL_TH_CONDITION);
        add_menu(tmpwin, nul_glyphinfo.v, any, 98, 0, NHM.ATR_NONE, clr, __s_bitmask_of_conditions, NHM.MENU_ITEMFLAGS_NONE);
        nopts++;
    }
    if (fld != NHC.BL_CONDITION && fld != NHC.BL_VERS) {
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, onlybeh = NHM.BL_TH_UPDOWN);
        void cptr.sprintf(cptr.decay(buf), __s_s_value_changes, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
        add_menu(tmpwin, nul_glyphinfo.v, any, 99, 0, NHM.ATR_NONE, clr, cptr.decay(buf), NHM.MENU_ITEMFLAGS_NONE);
        nopts++;
    }
    if (fld != NHC.BL_CAP && fld != NHC.BL_HUNGER && (at == NHC.ANY_INT || at == NHC.ANY_LONG)) {
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, onlybeh = NHM.BL_TH_VAL_ABSOLUTE);
        add_menu(tmpwin, nul_glyphinfo.v, any, 110, 0, NHM.ATR_NONE, clr, __s_number_threshold, NHM.MENU_ITEMFLAGS_NONE);
        nopts++;
    }
    if (cptr.ldI32o2(initblstats, fld, $sizeof_istat_s, $istat_s_idxmax) >= 0) {
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, onlybeh = NHM.BL_TH_VAL_PERCENTAGE);
        add_menu(tmpwin, nul_glyphinfo.v, any, 112, 0, NHM.ATR_NONE, clr, __s_percentage_threshold, NHM.MENU_ITEMFLAGS_NONE);
        nopts++;
    }
    if (fld == NHC.BL_HP) {
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, onlybeh = NHM.BL_TH_CRITICALHP);
        void cptr.sprintf(cptr.decay(buf), __s_highlight_critically_low_s, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
        add_menu(tmpwin, nul_glyphinfo.v, any, 67, 0, NHM.ATR_NONE, clr, cptr.decay(buf), NHM.MENU_ITEMFLAGS_NONE);
        nopts++;
    }
    if (cptr.ldI32o2(initblstats, fld, $sizeof_istat_s, $istat_s_anytype) == NHC.ANY_STR || fld == NHC.BL_CAP || fld == NHC.BL_HUNGER) {
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, onlybeh = NHM.BL_TH_TEXTMATCH);
        void cptr.sprintf(cptr.decay(buf), __s_s_text_match, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
        add_menu(tmpwin, nul_glyphinfo.v, any, 116, 0, NHM.ATR_NONE, clr, cptr.decay(buf), NHM.MENU_ITEMFLAGS_NONE);
        nopts++;
    }
    void cptr.sprintf(cptr.decay(buf), __s_select_s_field_hilite_behavior, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
    end_menu()(tmpwin, cptr.decay(buf));
    if (nopts > 1) {
        res = select_menu(tmpwin, NHM.PICK_ONE, picks);
        if (res == 0)
            beh = NHM.BL_TH_NONE;
        else if (res == -1)
            beh = -1;
    } else if (onlybeh != NHM.BL_TH_NONE) {
        beh = onlybeh;
    }
    destroy_nhwindow()(tmpwin);
    if (res > 0) {
        beh = cptr.ldI32(picks.v);
        cptr.free(picks.v);
    }
    return beh;
}

/** C ref: botl.c:3811 — @param {CInt} fld @param {CPtr<char>} str @param {CInt} ltok @param {CInt} gtok @returns {CInt} */
function status_hilite_menu_choose_updownboth(fld, str, ltok, gtok) {
    let res;
    let ret = NHC.NO_LTEQGT;
    let tmpwin;
    let buf = new Uint8Array(256);
    let any = cptr.alloc(8);
    let picks = cptr.box(null);
    let clr = NHM.NO_COLOR;
    tmpwin = create_nhwindow()(NHM.NHW_MENU);
    start_menu()(tmpwin, 0n);
    if (ltok) {
        if (str)
            void cptr.sprintf(cptr.decay(buf), __s_s_than_s, (fld == NHC.BL_AC) ? __s_better_lower : __s_less, str);
        else
            void cptr.sprintf(cptr.decay(buf), __s_value_goes_down);
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, ((10 + NHC.LT_VALUE) | 0));
        add_menu(tmpwin, nul_glyphinfo.v, any, 0, 0, NHM.ATR_NONE, clr, cptr.decay(buf), NHM.MENU_ITEMFLAGS_NONE);
        if (str) {
            void cptr.sprintf(cptr.decay(buf), __s_s_or_s, str, (fld == NHC.BL_AC) ? __s_better_lower__2 : __s_less__2);
            cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
            cptr.stI32(any, ((10 + NHC.LE_VALUE) | 0));
            add_menu(tmpwin, nul_glyphinfo.v, any, 0, 0, NHM.ATR_NONE, clr, cptr.decay(buf), NHM.MENU_ITEMFLAGS_NONE);
        }
    }
    if (str)
        void cptr.sprintf(cptr.decay(buf), __s_exactly_s, str);
    else
        void cptr.sprintf(cptr.decay(buf), __s_value_changes);
    cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
    cptr.stI32(any, ((10 + NHC.EQ_VALUE) | 0));
    add_menu(tmpwin, nul_glyphinfo.v, any, 0, 0, NHM.ATR_NONE, clr, cptr.decay(buf), NHM.MENU_ITEMFLAGS_NONE);
    if (gtok) {
        if (str) {
            void cptr.sprintf(cptr.decay(buf), __s_s_or_s, str, (fld == NHC.BL_AC) ? __s_worse_higher : __s_more);
            cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
            cptr.stI32(any, ((10 + NHC.GE_VALUE) | 0));
            add_menu(tmpwin, nul_glyphinfo.v, any, 0, 0, NHM.ATR_NONE, clr, cptr.decay(buf), NHM.MENU_ITEMFLAGS_NONE);
        }
        if (str)
            void cptr.sprintf(cptr.decay(buf), __s_s_than_s, (fld == NHC.BL_AC) ? __s_worse_higher__2 : __s_more__2, str);
        else
            void cptr.sprintf(cptr.decay(buf), __s_value_goes_up);
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, ((10 + NHC.GT_VALUE) | 0));
        add_menu(tmpwin, nul_glyphinfo.v, any, 0, 0, NHM.ATR_NONE, clr, cptr.decay(buf), NHM.MENU_ITEMFLAGS_NONE);
    }
    void cptr.sprintf(cptr.decay(buf), __s_select_field_s_value, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
    end_menu()(tmpwin, cptr.decay(buf));
    res = select_menu(tmpwin, NHM.PICK_ONE, picks);
    destroy_nhwindow()(tmpwin);
    if (res > 0) {
        ret = (cptr.ldI32(picks.v) - 10) | 0;
        cptr.free(picks.v);
    }
    return ret;
}

const __static_status_hilite_menu_add_aligntxt = cptr.alloc(3 * 8);
cptr.stPtro(__static_status_hilite_menu_add_aligntxt, 0, __s_chaotic__3);
cptr.stPtro(__static_status_hilite_menu_add_aligntxt, 8, __s_neutral__3);
cptr.stPtro(__static_status_hilite_menu_add_aligntxt, 16, __s_lawful__3); /** C ref: botl.c:4145 — char *[3] (function-static) */
const __static_status_hilite_menu_add_hutxt = cptr.alloc(7 * 8);
cptr.stPtro(__static_status_hilite_menu_add_hutxt, 0, __s_satiated);
cptr.stPtro(__static_status_hilite_menu_add_hutxt, 8, null);
cptr.stPtro(__static_status_hilite_menu_add_hutxt, 16, __s_hungry);
cptr.stPtro(__static_status_hilite_menu_add_hutxt, 24, __s_weak);
cptr.stPtro(__static_status_hilite_menu_add_hutxt, 32, __s_fainting);
cptr.stPtro(__static_status_hilite_menu_add_hutxt, 40, __s_fainted);
cptr.stPtro(__static_status_hilite_menu_add_hutxt, 48, __s_starved); /** C ref: botl.c:4157 — char *[7] (function-static) */

/** C ref: botl.c:3890 — @param {CInt} origfld @returns {CInt} */
function status_hilite_menu_add(origfld) {
    let fld;
    let behavior;
    let lt_gt_eq;
    let clr = NHM.NO_COLOR;
    let atr = NHC.HL_UNDEF;
    let hilite = cptr.alloc(128);
    let cond = 0n;
    let colorqry = new Uint8Array(256);
    let attrqry = new Uint8Array(256);
    let retry = 0;
    __lbl_choose_field: while (true) {
        fld = origfld;
        if (fld == NHC.BL_FLUSH) {
            fld = status_hilite_menu_choose_field();
            if (fld == NHC.BL_FLUSH)
                return 0;
        }
        if (fld == NHC.BL_FLUSH)
            return 0;
        cptr.st1o(cptr.decay(colorqry), 0, 0, 1);
        cptr.st1o(cptr.decay(attrqry), 0, 0, 1);
        __builtin___memset_chk(hilite, 0, 128n, __builtin_object_size(hilite, 0));
        cptr.stPtro(hilite, $hilite_s_next, null);
        cptr.st1o(hilite, $hilite_s_set, 0);
        cptr.stI32(hilite, fld);
        __lbl_choose_behavior: while (true) {
            behavior = status_hilite_menu_choose_behavior(fld);
            if (behavior == -1) {
                return 0;
            } else if (behavior == NHM.BL_TH_NONE) {
                if (origfld == NHC.BL_FLUSH)
                    continue __lbl_choose_field;
                return 0;
            }
            cptr.stI32o(hilite, $hilite_s_behavior, behavior);
            __lbl_choose_value: while (true) {
                if (retry++ > 5) {
                    pline(__s_that_s_enough_tries);
                    return 0;
                }
                if (behavior == NHM.BL_TH_VAL_PERCENTAGE || behavior == NHM.BL_TH_VAL_ABSOLUTE) {
                    let inbuf = new Uint8Array(256);
                    let buf = new Uint8Array(256);
                    let aval = cptr.alloc(8);
                    let val;
                    let dt;
                    let gotnum = 0;
                    let percent = schar((behavior == NHM.BL_TH_VAL_PERCENTAGE));
                    let inp;
                    let numstart;
                    let op;
                    lt_gt_eq = NHC.NO_LTEQGT;
                    cptr.st1o(cptr.decay(inbuf), 0, 0, 1);
                    void cptr.sprintf(cptr.decay(buf), __s_enter_svalue_for_s_threshold, percent ? __s_percentage : __s_empty, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
                    getlin(cptr.decay(buf), cptr.decay(inbuf));
                    if (cptr.ld1so(cptr.decay(inbuf), 0, 1) == 0 || cptr.ld1so(cptr.decay(inbuf), 0, 1) == 27)
                        continue __lbl_choose_behavior;
                    inp = (numstart = trimspaces(cptr.decay(inbuf)));
                    if (!cptr.ld1s(inp))
                        continue __lbl_choose_behavior;
                    if (cptr.ld1s(inp) == 62 || cptr.ld1s(inp) == 60 || cptr.ld1s(inp) == 61) {
                        lt_gt_eq = (cptr.ld1s(inp) == 62) ? ((cptr.ld1so(inp, 1) == 61) ? NHC.GE_VALUE : NHC.GT_VALUE) : ((cptr.ld1s(inp) == 60) ? ((cptr.ld1so(inp, 1) == 61) ? NHC.LE_VALUE : NHC.LT_VALUE) : NHC.EQ_VALUE);
                        cptr.st1(cptr.postinc(() => inp, (v) => { inp = v; }), 32);
                        numstart = cptr.add(numstart, 1);
                        if (lt_gt_eq == NHC.GE_VALUE || lt_gt_eq == NHC.LE_VALUE) {
                            cptr.st1(cptr.postinc(() => inp, (v) => { inp = v; }), 32);
                            numstart = cptr.add(numstart, 1);
                        }
                    }
                    if (cptr.ld1s(inp) == 45) {
                        inp = cptr.add(inp, 1);
                    } else if (cptr.ld1s(inp) == 43) {
                        cptr.st1(cptr.postinc(() => inp, (v) => { inp = v; }), 32);
                        numstart = cptr.add(numstart, 1);
                    }
                    while (digit(cptr.ld1s(inp))) {
                        inp = cptr.add(inp, 1);
                        gotnum = 1;
                    }
                    if (cptr.ld1s(inp) == 37) {
                        if (!percent) {
                            pline(__s_not_expecting_a_percentage);
                            continue __lbl_choose_behavior;
                        }
                        cptr.st1(inp, 0);
                    } else if (cptr.ld1s(inp)) {
                        pline(__s_s_is_not_a_recognized_number, inp);
                        continue __lbl_choose_value;
                    }
                    if (!gotnum) {
                        pline(__s_is_that_an_invisible_number);
                        continue __lbl_choose_value;
                    }
                    op = (lt_gt_eq == NHC.LT_VALUE) ? __s_lt : ((lt_gt_eq == NHC.LE_VALUE) ? __s_lt_eq : ((lt_gt_eq == NHC.GT_VALUE) ? __s_gt : ((lt_gt_eq == NHC.GE_VALUE) ? __s_gt_eq : ((lt_gt_eq == NHC.EQ_VALUE) ? __s_eq : __s_empty))));
                    cptr.memcpy(aval, cptr.add(cg, $const_globals_zeroany), 8);
                    dt = (percent ? NHC.ANY_INT : cptr.ldI32o2(initblstats, fld, $sizeof_istat_s, $istat_s_anytype)) | 0;
                    void s_to_anything(aval, numstart, dt);
                    if (percent) {
                        val = cptr.ldI32(aval);
                        if (cptr.ldI32o2(initblstats, fld, $sizeof_istat_s, $istat_s_idxmax) == -1) {
                            pline(__s_field_s_does_not_support_percentage, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
                            behavior = NHM.BL_TH_VAL_ABSOLUTE;
                            continue __lbl_choose_value;
                        }
                        if ((val < 0 && (val != -1 || lt_gt_eq != NHC.GT_VALUE)) || (val == 0 && lt_gt_eq == NHC.LT_VALUE) || (val == 100 && lt_gt_eq == NHC.GT_VALUE) || (val > 100 && (val != 101 || lt_gt_eq != NHC.LT_VALUE))) {
                            pline(__s_s_d_is_not_a_valid_percent_value, op, val);
                            continue __lbl_choose_value;
                        }
                        if (!cptr.strchr(numstart, 37))
                            void cptr.strcat(numstart, __s_pct);
                    } else if (dt == NHC.ANY_INT && (cptr.ldI32(aval) < ((fld == NHC.BL_AC) ? -128 : ((lt_gt_eq == NHC.GT_VALUE) ? -1 : ((lt_gt_eq == NHC.LT_VALUE) ? 1 : 0))))) {
                        pline(__s_s_s_d_s, cptr.decay(threshold_value), op, cptr.ldI32(aval), cptr.decay(is_out_of_range));
                        continue __lbl_choose_value;
                    } else if (dt == NHC.ANY_LONG && (cptr.ldI64(aval) < ((lt_gt_eq == NHC.GT_VALUE) ? -1n : ((lt_gt_eq == NHC.LT_VALUE) ? 1n : 0n)))) {
                        pline(__s_s_s_ld_s, cptr.decay(threshold_value), op, cptr.ldI64(aval), cptr.decay(is_out_of_range));
                        continue __lbl_choose_value;
                    }
                    if (lt_gt_eq == NHC.NO_LTEQGT) {
                        let ltok = schar(((dt == NHC.ANY_INT) ? (cptr.ldI32(aval) > 0 || fld == NHC.BL_AC ? 1 : 0) : (cptr.ldI64(aval) > 0n)));
                        let gtok = schar((!percent || cptr.ldI64(aval) < 100n ? 1 : 0));
                        lt_gt_eq = status_hilite_menu_choose_updownboth(fld, cptr.decay(inbuf), ltok, gtok);
                        if (lt_gt_eq == NHC.NO_LTEQGT)
                            continue __lbl_choose_value;
                    }
                    void cptr.sprintf(cptr.decay(colorqry), __s_choose_a_color_for_when_s_is_s_s_s, cptr.ldPtro(initblstats, fld, $sizeof_istat_s), (lt_gt_eq == NHC.LT_VALUE) ? __s_less_than : ((lt_gt_eq == NHC.GT_VALUE) ? __s_more_than : __s_empty), numstart, (lt_gt_eq == NHC.LE_VALUE) ? __s_or_less : ((lt_gt_eq == NHC.GE_VALUE) ? __s_or_more : __s_empty));
                    void cptr.sprintf(cptr.decay(attrqry), __s_choose_attribute_for_when_s_is_s_s_s, cptr.ldPtro(initblstats, fld, $sizeof_istat_s), (lt_gt_eq == NHC.LT_VALUE) ? __s_less_than : ((lt_gt_eq == NHC.GT_VALUE) ? __s_more_than : __s_empty), numstart, (lt_gt_eq == NHC.LE_VALUE) ? __s_or_less : ((lt_gt_eq == NHC.GE_VALUE) ? __s_or_more : __s_empty));
                    cptr.stI32o(hilite, $hilite_s_rel, lt_gt_eq);
                    cptr.memcpy(cptr.add(hilite, $hilite_s_value), aval, 8);
                } else if (behavior == NHM.BL_TH_UPDOWN) {
                    if (cptr.ldI32o2(initblstats, fld, $sizeof_istat_s, $istat_s_anytype) != NHC.ANY_STR) {
                        let ltok = schar((fld != NHC.BL_TIME));
                        let gtok = 1;
                        lt_gt_eq = status_hilite_menu_choose_updownboth(fld, null, ltok, gtok);
                        if (lt_gt_eq == NHC.NO_LTEQGT)
                            continue __lbl_choose_behavior;
                    } else {
                        lt_gt_eq = NHC.EQ_VALUE;
                    }
                    void cptr.sprintf(cptr.decay(colorqry), __s_choose_a_color_for_when_s_s, cptr.ldPtro(initblstats, fld, $sizeof_istat_s), (lt_gt_eq == NHC.EQ_VALUE) ? __s_changes : ((lt_gt_eq == NHC.LT_VALUE) ? __s_decreases : __s_increases));
                    void cptr.sprintf(cptr.decay(attrqry), __s_choose_attribute_for_when_s_s, cptr.ldPtro(initblstats, fld, $sizeof_istat_s), (lt_gt_eq == NHC.EQ_VALUE) ? __s_changes : ((lt_gt_eq == NHC.LT_VALUE) ? __s_decreases : __s_increases));
                    cptr.stI32o(hilite, $hilite_s_rel, lt_gt_eq);
                } else if (behavior == NHM.BL_TH_CONDITION) {
                    cond = query_conditions();
                    if (!cond) {
                        if (origfld == NHC.BL_FLUSH)
                            continue __lbl_choose_field;
                        return 0;
                    }
                    nh_snprintf(__s_status_hilite_menu_add, 4121, cptr.decay(colorqry), 256n, __s_choose_a_color_for_conditions_s, conditionbitmask2str(cond));
                    nh_snprintf(__s_status_hilite_menu_add, 4124, cptr.decay(attrqry), 256n, __s_choose_attribute_for_conditions_s, conditionbitmask2str(cond));
                } else if (behavior == NHM.BL_TH_TEXTMATCH) {
                    let qry_buf = new Uint8Array(256);
                    void cptr.sprintf(cptr.decay(qry_buf), __s_s_s_text_value_to_match, (fld == NHC.BL_CAP || fld == NHC.BL_ALIGN || fld == NHC.BL_HUNGER || fld == NHC.BL_TITLE) ? __s_choose : __s_enter, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
                    if (fld == NHC.BL_CAP) {
                        let rv = query_arrayvalue(cptr.decay(qry_buf), enc_stat, NHC.SLT_ENCUMBER, ((NHC.OVERLOADED + 1) | 0));
                        if (rv < NHC.SLT_ENCUMBER)
                            continue __lbl_choose_behavior;
                        cptr.stI32o(hilite, $hilite_s_rel, NHC.TXT_VALUE);
                        void cptr.strcpy(cptr.add(hilite, $hilite_s_textmatch), cptr.ldPtro(enc_stat, rv, 8));
                    } else if (fld == NHC.BL_ALIGN) {
                        let rv = query_arrayvalue(cptr.decay(qry_buf), __static_status_hilite_menu_add_aligntxt, 0, 3);
                        if (rv < 0)
                            continue __lbl_choose_behavior;
                        cptr.stI32o(hilite, $hilite_s_rel, NHC.TXT_VALUE);
                        void cptr.strcpy(cptr.add(hilite, $hilite_s_textmatch), cptr.ldPtro(__static_status_hilite_menu_add_aligntxt, rv, 8));
                    } else if (fld == NHC.BL_HUNGER) {
                        let rv = query_arrayvalue(cptr.decay(qry_buf), __static_status_hilite_menu_add_hutxt, NHC.SATIATED, ((NHC.STARVED + 1) | 0));
                        if (rv < NHC.SATIATED)
                            continue __lbl_choose_behavior;
                        cptr.stI32o(hilite, $hilite_s_rel, NHC.TXT_VALUE);
                        void cptr.strcpy(cptr.add(hilite, $hilite_s_textmatch), cptr.ldPtro(__static_status_hilite_menu_add_hutxt, rv, 8));
                    } else if (fld == NHC.BL_TITLE) {
                        let rolelist = cptr.alloc(28 * 8);
                        let mbuf = new Uint8Array(80);
                        let fbuf = new Uint8Array(80);
                        let obuf = new Uint8Array(80);
                        let i;
                        let j;
                        let rv;
                        for (i = (j = 0); i < 9; i++) {
                            void cptr.sprintf(cptr.decay(mbuf), __s_quot_pct_s_quot, cptr.ldPtro2(gu, i, $sizeof_RoleName, $instance_globals_u_urole + $Role_rank));
                            if (cptr.ldPtro2(gu, i, $sizeof_RoleName, $instance_globals_u_urole + $Role_rank + $RoleName_f)) {
                                void cptr.sprintf(cptr.decay(fbuf), __s_quot_pct_s_quot, cptr.ldPtro2(gu, i, $sizeof_RoleName, $instance_globals_u_urole + $Role_rank + $RoleName_f));
                                nh_snprintf(__s_status_hilite_menu_add, 4179, cptr.decay(obuf), 80n, __s_s_or_s, cptr.ld1so(flags, $flag_female) ? cptr.decay(fbuf) : cptr.decay(mbuf), cptr.ld1so(flags, $flag_female) ? cptr.decay(mbuf) : cptr.decay(fbuf));
                            } else {
                                cptr.st1o(cptr.decay(fbuf), 0, cptr.st1o(cptr.decay(obuf), 0, 0, 1), 1);
                            }
                            if (cptr.ld1so(flags, $flag_female)) {
                                if (cptr.ld1s(cptr.decay(fbuf)))
                                    cptr.stPtro(rolelist, j++, dupstr(cptr.decay(fbuf)), 8);
                                cptr.stPtro(rolelist, j++, dupstr(cptr.decay(mbuf)), 8);
                                if (cptr.ld1s(cptr.decay(obuf)))
                                    cptr.stPtro(rolelist, j++, dupstr(cptr.decay(obuf)), 8);
                            } else {
                                cptr.stPtro(rolelist, j++, dupstr(cptr.decay(mbuf)), 8);
                                if (cptr.ld1s(cptr.decay(fbuf)))
                                    cptr.stPtro(rolelist, j++, dupstr(cptr.decay(fbuf)), 8);
                                if (cptr.ld1s(cptr.decay(obuf)))
                                    cptr.stPtro(rolelist, j++, dupstr(cptr.decay(obuf)), 8);
                            }
                        }
                        cptr.stPtro(rolelist, j++, dupstr(__s_none_of_the_above_polymorphed__2), 8);
                        rv = query_arrayvalue(cptr.decay(qry_buf), rolelist, 0, j);
                        if (rv >= 0) {
                            cptr.stI32o(hilite, $hilite_s_rel, NHC.TXT_VALUE);
                            void cptr.strcpy(cptr.add(hilite, $hilite_s_textmatch), cptr.ldPtro(rolelist, rv, 8));
                        }
                        for (i = 0; i < j; i++)
                            cptr.free(cptr.ldPtro(rolelist, i, 8)), cptr.stPtro(rolelist, i, null, 8);
                        if (rv < 0)
                            continue __lbl_choose_behavior;
                    } else {
                        let inbuf = new Uint8Array(256);
                        cptr.st1o(cptr.decay(inbuf), 0, 0, 1);
                        getlin(cptr.decay(qry_buf), cptr.decay(inbuf));
                        if (cptr.ld1so(cptr.decay(inbuf), 0, 1) == 0 || cptr.ld1so(cptr.decay(inbuf), 0, 1) == 27)
                            continue __lbl_choose_behavior;
                        cptr.stI32o(hilite, $hilite_s_rel, NHC.TXT_VALUE);
                        if (cptr.strlen(cptr.decay(inbuf)) < 80n)
                            void cptr.strcpy(cptr.add(hilite, $hilite_s_textmatch), cptr.decay(inbuf));
                        else
                            return 0;
                    }
                    void cptr.sprintf(cptr.decay(colorqry), __s_choose_a_color_for_when_s_is_s, cptr.ldPtro(initblstats, fld, $sizeof_istat_s), cptr.add(hilite, $hilite_s_textmatch));
                    void cptr.sprintf(cptr.decay(attrqry), __s_choose_attribute_for_when_s_is_s, cptr.ldPtro(initblstats, fld, $sizeof_istat_s), cptr.add(hilite, $hilite_s_textmatch));
                } else if (behavior == NHM.BL_TH_ALWAYS_HILITE) {
                    void cptr.sprintf(cptr.decay(colorqry), __s_choose_a_color_to_always_hilite_s, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
                    void cptr.sprintf(cptr.decay(attrqry), __s_choose_attribute_to_always_hilite_s, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
                }
                __lbl_choose_color: while (true) {
                    clr = query_color(cptr.decay(colorqry), NHM.NO_COLOR);
                    if (clr == -1) {
                        if (behavior != NHM.BL_TH_ALWAYS_HILITE)
                            continue __lbl_choose_value;
                        else
                            continue __lbl_choose_behavior;
                    }
                    atr = query_attr(cptr.decay(attrqry), NHM.ATR_NONE);
                    if (atr == -1)
                        continue __lbl_choose_color;
                    if (behavior == NHM.BL_TH_CONDITION) {
                        let clrbuf = new Uint8Array(256);
                        let attrbuf = new Uint8Array(256);
                        let tmpattr;
                        if (atr & NHC.HL_BOLD)
                            cptr.stU64o2(gc, 18, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 18, 8, $instance_globals_c_cond_hilites) | cond);
                        if (atr & NHC.HL_DIM)
                            cptr.stU64o2(gc, 19, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 19, 8, $instance_globals_c_cond_hilites) | cond);
                        if (atr & NHC.HL_ITALIC)
                            cptr.stU64o2(gc, 20, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 20, 8, $instance_globals_c_cond_hilites) | cond);
                        if (atr & NHC.HL_ULINE)
                            cptr.stU64o2(gc, 21, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 21, 8, $instance_globals_c_cond_hilites) | cond);
                        if (atr & NHC.HL_BLINK)
                            cptr.stU64o2(gc, 22, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 22, 8, $instance_globals_c_cond_hilites) | cond);
                        if (atr & NHC.HL_INVERSE)
                            cptr.stU64o2(gc, 23, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 23, 8, $instance_globals_c_cond_hilites) | cond);
                        if (atr == NHC.HL_NONE) {
                            cptr.stU64o2(gc, 18, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 18, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~cond));
                            cptr.stU64o2(gc, 19, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 19, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~cond));
                            cptr.stU64o2(gc, 20, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 20, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~cond));
                            cptr.stU64o2(gc, 21, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 21, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~cond));
                            cptr.stU64o2(gc, 22, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 22, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~cond));
                            cptr.stU64o2(gc, 23, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 23, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~cond));
                        }
                        cptr.stU64o2(gc, clr, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, clr, 8, $instance_globals_c_cond_hilites) | cond);
                        void strNsubst(cptr.strcpy(cptr.decay(clrbuf), clr2colorname(clr)), __s_sp, __s_dash, 0);
                        tmpattr = hlattr2attrname(atr, cptr.decay(attrbuf), 256n);
                        if (tmpattr)
                            void cptr.sprintf(eos(cptr.decay(clrbuf)), __s_amp_pct_s, tmpattr);
                        pline(__s_added_hilite_condition_s_s, conditionbitmask2str(cond), cptr.decay(clrbuf));
                    } else {
                        let p;
                        let q;
                        cptr.stI32o(hilite, $hilite_s_coloridx, clr | (atr << 8));
                        cptr.stI32o(hilite, $hilite_s_anytype, cptr.ldI32o2(initblstats, fld, $sizeof_istat_s, $istat_s_anytype));
                        if (fld == NHC.BL_TITLE && (p = strstri(cptr.add(hilite, $hilite_s_textmatch), __s_or)) !== null) {
                            cptr.st1(p, 0);
                            status_hilite_add_threshold(fld, hilite);
                            pline(__s_added_hilite_s, status_hilite2str(hilite));
                            p = cptr.add(p, 4n);
                            q = cptr.add(hilite, $hilite_s_textmatch);
                            while ((cptr.st1(cptr.postinc(() => q, (v) => { q = v; }), cptr.ld1s(cptr.postinc(() => p, (v) => { p = v; })))) != 0)
                                continue;
                        }
                        status_hilite_add_threshold(fld, hilite);
                        pline(__s_added_hilite_s, status_hilite2str(hilite));
                    }
                    reset_status_hilites();
                    return 1;
                }
                break __lbl_choose_value;
            }
            break __lbl_choose_behavior;
        }
        break __lbl_choose_field;
    }
}

/** C ref: botl.c:4305 — @param {CInt} id @returns {CInt} */
function status_hilite_remove(id) {
    let hlstr = status_hilite_str;
    while (hlstr && cptr.ldI32(hlstr) != id) {
        hlstr = cptr.ldPtro(hlstr, $_status_hilite_line_str_next);
    }
    if (!hlstr)
        return 0;
    if (cptr.ldI32o(hlstr, $_status_hilite_line_str_fld) == NHC.BL_CONDITION) {
        let i;
        for (i = 0; i < NHM.CLR_MAX; i++)
            cptr.stU64o2(gc, i, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, i, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~cptr.ldU64o(hlstr, $_status_hilite_line_str_mask)));
        cptr.stU64o2(gc, 18, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 18, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~cptr.ldU64o(hlstr, $_status_hilite_line_str_mask)));
        cptr.stU64o2(gc, 19, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 19, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~cptr.ldU64o(hlstr, $_status_hilite_line_str_mask)));
        cptr.stU64o2(gc, 20, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 20, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~cptr.ldU64o(hlstr, $_status_hilite_line_str_mask)));
        cptr.stU64o2(gc, 21, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 21, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~cptr.ldU64o(hlstr, $_status_hilite_line_str_mask)));
        cptr.stU64o2(gc, 22, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 22, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~cptr.ldU64o(hlstr, $_status_hilite_line_str_mask)));
        cptr.stU64o2(gc, 23, 8, $instance_globals_c_cond_hilites, cptr.ldU64o2(gc, 23, 8, $instance_globals_c_cond_hilites) & BigInt.asUintN(64, ~cptr.ldU64o(hlstr, $_status_hilite_line_str_mask)));
        return 1;
    } else {
        let fld = cptr.ldI32o(hlstr, $_status_hilite_line_str_fld);
        let hl;
        let hlprev = null;
        for (hl = cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, fld, $sizeof_istat_s, $istat_s_thresholds); hl; hl = cptr.ldPtro(hl, $hilite_s_next)) {
            if (cptr.eq(cptr.ldPtro(hlstr, $_status_hilite_line_str_hl), hl)) {
                if (hlprev) {
                    cptr.stPtro(hlprev, $hilite_s_next, cptr.ldPtro(hl, $hilite_s_next));
                } else {
                    cptr.stPtro3(gb, 0, $sizeof_istat_s_x27, fld, $sizeof_istat_s, $istat_s_thresholds, cptr.ldPtro(hl, $hilite_s_next));
                    cptr.stPtro3(gb, 1, $sizeof_istat_s_x27, fld, $sizeof_istat_s, $istat_s_thresholds, cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, fld, $sizeof_istat_s, $istat_s_thresholds));
                }
                if (cptr.eq(cptr.ldPtro3(gb, 0, $sizeof_istat_s_x27, fld, $sizeof_istat_s, $istat_s_hilite_rule), hl)) {
                    cptr.stPtro3(gb, 0, $sizeof_istat_s_x27, fld, $sizeof_istat_s, $istat_s_hilite_rule, cptr.stPtro3(gb, 1, $sizeof_istat_s_x27, fld, $sizeof_istat_s, $istat_s_hilite_rule, null));
                    cptr.stI64o2(cptr.add(gb, 0, $sizeof_istat_s_x27), fld, $sizeof_istat_s, $istat_s_time, cptr.stI64o2(cptr.add(gb, 1, $sizeof_istat_s_x27), fld, $sizeof_istat_s, $istat_s_time, 0n));
                }
                cptr.free(hl);
                return 1;
            }
            hlprev = hl;
        }
    }
    return 0;
}

/** C ref: botl.c:4357 — @param {CInt} fld @returns {CInt} */
function status_hilite_menu_fld(fld) {
    let tmpwin;
    let i;
    let res;
    let picks = cptr.box(null);
    let any = cptr.alloc(8);
    let count = status_hilite_linestr_countfield(fld);
    let hlstr;
    let buf = new Uint8Array(256);
    let acted;
    let clr = NHM.NO_COLOR;
    if (!count) {
        if (status_hilite_menu_add(fld)) {
            status_hilite_linestr_done();
            status_hilite_linestr_gather();
            count = status_hilite_linestr_countfield(fld);
        } else
            return 0;
    }
    tmpwin = create_nhwindow()(NHM.NHW_MENU);
    start_menu()(tmpwin, 0n);
    if (count) {
        hlstr = status_hilite_str;
        while (hlstr) {
            if (cptr.ldI32o(hlstr, $_status_hilite_line_str_fld) == fld) {
                cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
                cptr.stI32(any, cptr.ldI32(hlstr));
                add_menu(tmpwin, nul_glyphinfo.v, any, 0, 0, NHM.ATR_NONE, clr, cptr.add(hlstr, $_status_hilite_line_str_str), NHM.MENU_ITEMFLAGS_NONE);
            }
            hlstr = cptr.ldPtro(hlstr, $_status_hilite_line_str_next);
        }
    } else {
        void cptr.sprintf(cptr.decay(buf), __s_no_current_hilites_for_s, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
        add_menu_str(tmpwin, cptr.decay(buf));
    }
    add_menu_str(tmpwin, __s_empty);
    if (count) {
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, -1);
        add_menu(tmpwin, nul_glyphinfo.v, any, 88, 0, NHM.ATR_NONE, clr, __s_remove_selected_hilites, NHM.MENU_ITEMFLAGS_NONE);
    }
    if (fld == NHC.BL_SCORE) {
        ;
    } else {
        cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
        cptr.stI32(any, -2);
        add_menu(tmpwin, nul_glyphinfo.v, any, 90, 0, NHM.ATR_NONE, clr, __s_add_new_hilites, NHM.MENU_ITEMFLAGS_NONE);
    }
    void cptr.sprintf(cptr.decay(buf), __s_current_s_hilites, cptr.ldPtro(initblstats, fld, $sizeof_istat_s));
    end_menu()(tmpwin, cptr.decay(buf));
    acted = 0;
    if ((res = select_menu(tmpwin, NHM.PICK_ANY, picks)) > 0) {
        let idx;
        let mode = 0;
        for (i = 0; i < res; i++) {
            idx = cptr.ldI32o(picks.v, i, $sizeof_menu_item);
            if (idx == -1)
                mode |= 1;
            else if (idx == -2)
                mode |= 2;
        }
        if ((mode & 1) >>> 0) {
            for (i = 0; i < res; i++) {
                idx = cptr.ldI32o(picks.v, i, $sizeof_menu_item);
                if (idx > 0 && status_hilite_remove(idx))
                    acted = 1;
            }
        }
        if ((mode & 2) >>> 0) {
            while (status_hilite_menu_add(fld))
                acted = 1;
        }
        cptr.free(picks.v), picks.v = null;
    }
    destroy_nhwindow()(tmpwin);
    return acted;
}

/** C ref: botl.c:4456 */
function status_hilites_viewall() {
    let datawin;
    let hlstr = status_hilite_str;
    let buf = new Uint8Array(256);
    datawin = create_nhwindow()(NHM.NHW_TEXT);
    while (hlstr) {
        void cptr.sprintf(cptr.decay(buf), __s_options_hilite_status_s, 231, cptr.add(hlstr, $_status_hilite_line_str_str));
        putstr()(datawin, 0, cptr.decay(buf));
        hlstr = cptr.ldPtro(hlstr, $_status_hilite_line_str_next);
    }
    display_nhwindow()(datawin, 0);
    destroy_nhwindow()(datawin);
}

/** C ref: botl.c:4477 — @param {CPtr<strbuf_t>} sbuf */
export function all_options_statushilites(sbuf) {
    let hlstr;
    let buf = new Uint8Array(256);
    status_hilite_linestr_done();
    status_hilite_linestr_gather();
    hlstr = status_hilite_str;
    while (hlstr) {
        void cptr.sprintf(cptr.decay(buf), __s_options_hilite_status_s__2, 230, cptr.add(hlstr, $_status_hilite_line_str_str));
        strbuf_append(sbuf, cptr.decay(buf));
        hlstr = cptr.ldPtro(hlstr, $_status_hilite_line_str_next);
    }
    status_hilite_linestr_done();
}

/** C ref: botl.c:4498 @returns {CInt} */
export function status_hilite_menu() {
    let tmpwin;
    let i;
    let fld;
    let res;
    let picks = cptr.box(null);
    let any = cptr.alloc(8);
    let redo;
    let countall;
    let clr = NHM.NO_COLOR;
    __lbl_shlmenu_redo: while (true) {
        redo = 0;
        tmpwin = create_nhwindow()(NHM.NHW_MENU);
        start_menu()(tmpwin, 0n);
        status_hilite_linestr_gather();
        countall = status_hilite_linestr_countfield(NHC.BL_FLUSH);
        if (countall) {
            cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
            cptr.stI32(any, -1);
            add_menu(tmpwin, nul_glyphinfo.v, any, 0, 0, NHM.ATR_NONE, clr, __s_view_all_hilites_in_config_format, NHM.MENU_ITEMFLAGS_NONE);
            add_menu_str(tmpwin, __s_empty);
        }
        for (i = 0; i < NHC.MAXBLSTATS; i++) {
            let count;
            let buf = new Uint8Array(256);
            fld = cptr.ldI32o2(initblstats, i, $sizeof_istat_s, $istat_s_fld);
            count = status_hilite_linestr_countfield(fld);
            if (fld == NHC.BL_SCORE && !count)
                continue;
            cptr.memcpy(any, cptr.add(cg, $const_globals_zeroany), 8);
            cptr.stI32(any, (fld + 1) | 0);
            void cptr.sprintf(cptr.decay(buf), __s_18s, cptr.ldPtro(initblstats, i, $sizeof_istat_s));
            if (count)
                void cptr.sprintf(eos(cptr.decay(buf)), __s_d_defined, count);
            add_menu(tmpwin, nul_glyphinfo.v, any, 0, 0, NHM.ATR_NONE, clr, cptr.decay(buf), NHM.MENU_ITEMFLAGS_NONE);
        }
        end_menu()(tmpwin, __s_status_hilites);
        if ((res = select_menu(tmpwin, NHM.PICK_ONE, picks)) > 0) {
            fld = (cptr.ldI32(picks.v) - 1) | 0;
            if (fld < 0) {
                status_hilites_viewall();
            } else {
                if (status_hilite_menu_fld(fld))
                    reset_status_hilites();
            }
            cptr.free(picks.v), picks.v = null;
            redo = 1;
        }
        destroy_nhwindow()(tmpwin);
        countall = status_hilite_linestr_countfield(NHC.BL_FLUSH);
        status_hilite_linestr_done();
        if (redo && !cptr.ld1so(iflags, $instance_flags_debug_fuzzer))
            continue __lbl_shlmenu_redo;
        if (countall > 0 && !cptr.ldI64o(iflags, $instance_flags_hilite_delta))
            cptr.stI64o(iflags, $instance_flags_hilite_delta, 3n);
        return 1;
    }
}

// --- BEGIN c2js reset block (tools/c2js/resetify.mjs) — do not edit ---
// 37 bindings: 0 rebound+refilled, 7 rebound, 30 refilled.
// S/P are supplied by js/generated/__reset.js so this module needs no new import.
let __c2js_rs = null;
export function __captureState(S) { __c2js_rs = [S(enc_stat), S(__static_get_strength_str_buf), S(__static_do_statusline1_newbot1), S(__static_do_statusline2_newbot2), S(__static_do_statusline2_dloc), S(__static_do_statusline2_hlth), S(__static_do_statusline2_expr), S(__static_do_statusline2_tmmv), S(__static_do_statusline2_cond), S(__static_do_statusline2_vers), S(initblstats), S(condition_aliases), S(conditions), S(condtests), S(cond_idx), S(c_Wall), S(terrain_descr), S(cache_avail), S(cache_reslt), S(cache_nomovemsg), S(cache_multi_reason), S(__static_cond_menu_menutitle), S(__static_eval_notify_windowport_field_oldrndencode), S(__static_eval_notify_windowport_field_oldgoldsym), S(__static_init_blstats_initalready), S(fieldids_alias), S(threshold_value), S(is_out_of_range), S(__static_splitsubfields_subfields), S(__static_parse_status_hl2_aligntxt), S(__static_parse_status_hl2_hutxt), S(__static_conditionbitmask2str_buf), S(status_hilite_str), S(status_hilite_str_id), S(__static_status_hilite2str_buf), S(__static_status_hilite_menu_add_aligntxt), S(__static_status_hilite_menu_add_hutxt)]; }
export function __resetState(P) {
    const r = __c2js_rs;
    if (r === null) throw new Error("botl.js: __resetState before __captureState");
    P(r[0]);
    P(r[1]);
    P(r[2]);
    P(r[3]);
    P(r[4]);
    P(r[5]);
    P(r[6]);
    P(r[7]);
    P(r[8]);
    P(r[9]);
    P(r[10]);
    P(r[11]);
    P(r[12]);
    P(r[13]);
    P(r[14]);
    P(r[15]);
    P(r[16]);
    P(r[17]);
    P(r[18]);
    cache_nomovemsg = P(r[19]);
    cache_multi_reason = P(r[20]);
    P(r[21]);
    __static_eval_notify_windowport_field_oldrndencode = P(r[22]);
    __static_eval_notify_windowport_field_oldgoldsym = P(r[23]);
    __static_init_blstats_initalready = P(r[24]);
    P(r[25]);
    P(r[26]);
    P(r[27]);
    P(r[28]);
    P(r[29]);
    P(r[30]);
    P(r[31]);
    status_hilite_str = P(r[32]);
    status_hilite_str_id = P(r[33]);
    P(r[34]);
    P(r[35]);
    P(r[36]);
}
// --- END c2js reset block ---
