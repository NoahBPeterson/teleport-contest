...
function get_rnd_line(fh, buf, bufsiz, rng, startpos, endpos, padlength) {
    ...
    if (filechunksize < 1n)
        return buf;
    void ((!!(filechunksize <= 2147483647n)) || (nhassert_failed(__s_filechunksize_int_max, __s_rumors_c, 449), 0) ? 1 : 0);
...
export function outrumor(truth, mechanism) {
    ...
    switch (mechanism) {
        case NHM.BY_ORACLE:
        pline(__s_true_to_her_word_the_oracle_ssays, (!(rng_log_enabled() ? (rng_log_set_caller(__s_rumors_c, 558, __s_outrumor), rn2(4)) : rn2(4)) ? __s_offhandedly : (!(rng_log_enabled() ? (rng_log_set_caller(__s_rumors_c, 559, __s_outrumor), rn2(3)) : rn2(3)) ? __s_casually : ((rng_log_enabled() ? (rng_log_set_caller(__s_rumors_c, 560, __s_outrumor), rn2(2)) : rn2(2)) ? __s_nonchalantly : __s_empty))));
        ;
