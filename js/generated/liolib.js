...
function readdigits(rn, hex) {
    let count = 0;
    while ((hex ? isxdigit(cptr.ldI32o(rn, FLD.RN_c)) : isdigit(cptr.ldI32o(rn, FLD.RN_c))) && nextc(rn))
        count++;
    return count;
}
...
function read_number(L, f) {
    ...
    if ((__builtin_expect(BigInt(((lua_stringtonumber(L, cptr.add(rn, FLD.RN_buff))) != 0n)), 1n)))
        return 1;
    else {
        lua_pushnil(L);
        return 0;
...
function read_line(L, f, chop) {
    ...
    if (!chop && c == 10)
        (void (cptr.ldU64o((b), FLD.luaL_Buffer_n) < cptr.ldU64o((b), FLD.luaL_Buffer_size) || luaL_prepbuffsize((b), 1n) ? 1 : 0), (cptr.st1o(cptr.ldPtr((b)), (cptr.stU64o((b), FLD.luaL_Buffer_n, cptr.ldU64o((b), FLD.luaL_Buffer_n) + 1n)) - (1n), schar((c)))));
    luaL_pushresult(b);
