// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/lib/lua-5.4.8/src/llex.c
// Input sha256: 5cfbaabf3dc0d75835aaafc37fcd87a92f3dba451e002732747b602fda122958
// Transpiler: tools/c2js c2js emit v1+batch

import { schar, uchar } from '../cmachine.js';
import * as cptr from '../cptr.js';
import { luaM_saferealloc_ } from './lmem.js';
import { luaS_new, luaS_newlstr } from './lstring.js';
import { luaC_fix, luaC_step } from './lgc.js';
import { luai_ctype_ } from './lctype.js';
import { luaO_hexavalue, luaO_pushfstring, luaO_str2num, luaO_utf8esc } from './lobject.js';
import { luaG_addinfo } from './ldebug.js';
import { luaD_throw } from './ldo.js';
import { luaH_finishset, luaH_getstr } from './ltable.js';
import { luaZ_fill } from './lzio.js';

// string literals (C char* uses decay to CPtr into these static buffers)
const __sl0 = cptr.lit("and");
const __sl1 = cptr.lit("break");
const __sl2 = cptr.lit("do");
const __sl3 = cptr.lit("else");
const __sl4 = cptr.lit("elseif");
const __sl5 = cptr.lit("end");
const __sl6 = cptr.lit("false");
const __sl7 = cptr.lit("for");
const __sl8 = cptr.lit("function");
const __sl9 = cptr.lit("goto");
const __sl10 = cptr.lit("if");
const __sl11 = cptr.lit("in");
const __sl12 = cptr.lit("local");
const __sl13 = cptr.lit("nil");
const __sl14 = cptr.lit("not");
const __sl15 = cptr.lit("or");
const __sl16 = cptr.lit("repeat");
const __sl17 = cptr.lit("return");
const __sl18 = cptr.lit("then");
const __sl19 = cptr.lit("true");
const __sl20 = cptr.lit("until");
const __sl21 = cptr.lit("while");
const __sl22 = cptr.lit("//");
const __sl23 = cptr.lit("..");
const __sl24 = cptr.lit("...");
const __sl25 = cptr.lit("==");
const __sl26 = cptr.lit(">=");
const __sl27 = cptr.lit("<=");
const __sl28 = cptr.lit("~=");
const __sl29 = cptr.lit("<<");
const __sl30 = cptr.lit(">>");
const __sl31 = cptr.lit("::");
const __sl32 = cptr.lit("<eof>");
const __sl33 = cptr.lit("<number>");
const __sl34 = cptr.lit("<integer>");
const __sl35 = cptr.lit("<name>");
const __sl36 = cptr.lit("<string>");
const __sl37 = cptr.lit("lexical element too long");
const __sl38 = cptr.lit("_ENV");
const __sl39 = cptr.lit("'%c'");
const __sl40 = cptr.lit("'<\\%d>'");
const __sl41 = cptr.lit("'%s'");
const __sl42 = cptr.lit("%s near %s");
const __sl43 = cptr.lit("chunk has too many lines");
const __sl44 = cptr.lit("Ee");
const __sl45 = cptr.lit("xX");
const __sl46 = cptr.lit("Pp");
const __sl47 = cptr.lit("-+");
const __sl48 = cptr.lit("malformed number");
const __sl49 = cptr.lit("string");
const __sl50 = cptr.lit("comment");
const __sl51 = cptr.lit("unfinished long %s (starting at line %d)");
const __sl52 = cptr.lit("hexadecimal digit expected");
const __sl53 = cptr.lit("missing '{'");
const __sl54 = cptr.lit("UTF-8 value too large");
const __sl55 = cptr.lit("missing '}'");
const __sl56 = cptr.lit("decimal escape too large");
const __sl57 = cptr.lit("unfinished string");
const __sl58 = cptr.lit("invalid escape sequence");
const __sl59 = cptr.lit("invalid long string delimiter");

/** C ref: llex.c:40 — char *[37] */
const luaX_tokens = [__sl0, __sl1, __sl2, __sl3, __sl4, __sl5, __sl6, __sl7, __sl8, __sl9, __sl10, __sl11, __sl12, __sl13, __sl14, __sl15, __sl16, __sl17, __sl18, __sl19, __sl20, __sl21, __sl22, __sl23, __sl24, __sl25, __sl26, __sl27, __sl28, __sl29, __sl30, __sl31, __sl32, __sl33, __sl34, __sl35, __sl36];

/** C ref: llex.c:57 — @param {CPtr} ls @param {CInt} c */
function save(ls, c) {
    let b = cptr.ldPtr(cptr.add(ls, 72));
    if ((cptr.ldU64(cptr.add((b), 8))) + 1n > (cptr.ldU64(cptr.add((b), 16)))) {
        let newsize;
        if ((cptr.ldU64(cptr.add((b), 16))) >= (8n < 8n ? ((~0n)) : 9223372036854775807n) / 2n)
            lexerror(ls, __sl37, 0);
        newsize = (cptr.ldU64(cptr.add((b), 16))) * 2n;
        (cptr.stPtr((b), (((luaM_saferealloc_(cptr.ldPtr(cptr.add(ls, 56)), (cptr.ldPtr((b))), (cptr.ldU64(cptr.add((b), 16))) * 1n, (newsize) * 1n))))), cptr.stU64(cptr.add((b), 16), newsize));
    }
    cptr.st1(cptr.add(cptr.ldPtr(b), (cptr.ldU64(cptr.add((b), 8)))++), (schar(((c)))));
}

/** C ref: llex.c:70 — @param {CPtr} L */
export function luaX_init(L) {
    let i;
    let e = (luaS_newlstr(L, __sl38, (5n / 1n) - 1n));
    luaC_fix(L, ((((e)))));
    for (i = 0; i < (((((((TK_WHILE - ((((Math.imul(127, 2) + 1) | 0) + 1) | 0)) | 0) + 1) | 0)))); i++) {
        let ts = luaS_new(L, cptr.ldPtr(cptr.add(cptr.decay(luaX_tokens), i)));
        luaC_fix(L, ((((ts)))));
        cptr.st1(cptr.add(ts, 10), (uchar((((i + 1) | 0)))));
    }
}

/** C ref: llex.c:82 — @param {CPtr} ls @param {CInt} token @returns {CPtr} */
export function luaX_token2str(ls, token) {
    if (token < ((((Math.imul(127, 2) + 1) | 0) + 1) | 0)) {
        if ((cptr.ld1u(cptr.add(cptr.decay(luai_ctype_), ((token) + 1) | 0)) & ((1 << (2)))))
            return luaO_pushfstring(cptr.ldPtr(cptr.add(ls, 56)), __sl39, token);
        else
            return luaO_pushfstring(cptr.ldPtr(cptr.add(ls, 56)), __sl40, token);
    } else {
        let s = cptr.ldPtr(cptr.add(cptr.decay(luaX_tokens), (token - ((((Math.imul(127, 2) + 1) | 0) + 1) | 0)) | 0));
        if (token < TK_EOS)
            return luaO_pushfstring(cptr.ldPtr(cptr.add(ls, 56)), __sl41, s);
        else
            return s;
    }
}

/** C ref: llex.c:99 — @param {CPtr} ls @param {CInt} token @returns {CPtr} */
function txtToken(ls, token) {
    switch (token) {
        case TK_NAME:
        case TK_STRING:
        case TK_FLT:
        case TK_INT:
        save(ls, 0);
        return luaO_pushfstring(cptr.ldPtr(cptr.add(ls, 56)), __sl41, (cptr.ldPtr((cptr.ldPtr(cptr.add(ls, 72))))));
        default:
        return luaX_token2str(ls, token);
    }
}

/** C ref: llex.c:111 — @param {CPtr} ls @param {CPtr} msg @param {CInt} token */
function lexerror(ls, msg, token) {
    msg = luaG_addinfo(cptr.ldPtr(cptr.add(ls, 56)), msg, cptr.ldPtr(cptr.add(ls, 96)), cptr.ldI32(cptr.add(ls, 4)));
    if (token)
        luaO_pushfstring(cptr.ldPtr(cptr.add(ls, 56)), __sl42, msg, txtToken(ls, token));
    luaD_throw(cptr.ldPtr(cptr.add(ls, 56)), 3);
}

/** C ref: llex.c:119 — @param {CPtr} ls @param {CPtr} msg */
export function luaX_syntaxerror(ls, msg) {
    lexerror(ls, msg, cptr.ldI32(cptr.add(ls, 16)));
}

/** C ref: llex.c:134 — @param {CPtr} ls @param {CPtr} str @param {CLongLong} l @returns {CPtr} */
export function luaX_newstring(ls, str, l) {
    let L = cptr.ldPtr(cptr.add(ls, 56));
    let ts = luaS_newlstr(L, str, l);
    let o = luaH_getstr(cptr.ldPtr(cptr.add(ls, 80)), ts);
    if (!(((((cptr.ld1u(cptr.add(((o)), 8)))) & 15)) == (0)))
        ts = ((((((cptr.ldPtr((cptr.add(((((o)))), 16)))))))));
    else {
        let stv = ((cptr.postinc(() => cptr.ldPtr(cptr.add(L, 16)), (v) => { cptr.stPtr(cptr.add(L, 16), v); }, 16)));
        {
            let io = (stv);
            let x_ = (ts);
            cptr.stPtr(((io)), ((((x_)))));
            (cptr.st1(cptr.add((io), 8), uchar((((cptr.ld1u(cptr.add(x_, 8))) | (1 << 6))))));
            (void L, (void 0));
        }
        ;
        luaH_finishset(L, cptr.ldPtr(cptr.add(ls, 80)), stv, o, stv);
        {
            if (cptr.ldI64(cptr.add((cptr.ldPtr(cptr.add(L, 24))), 24)) > 0n) {
                void 0;
                luaC_step(L);
                void 0;
            }
            ;
            (void 0);
        }
        ;
        cptr.postdec(() => cptr.ldPtr(cptr.add(L, 16)), (v) => { cptr.stPtr(cptr.add(L, 16), v); }, 16);
    }
    return ts;
}

/** C ref: llex.c:156 — @param {CPtr} ls */
function inclinenumber(ls) {
    let old = cptr.ldI32(ls);
    (void 0);
    (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
    if ((cptr.ldI32(ls) == 10 || cptr.ldI32(ls) == 13) && cptr.ldI32(ls) != old)
        (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
    if (++cptr.ldI32(cptr.add(ls, 4)) >= 2147483647)
        lexerror(ls, __sl43, 0);
}

/** C ref: llex.c:167 — @param {CPtr} L @param {CPtr} ls @param {CPtr} z @param {CPtr} source @param {CInt} firstchar */
export function luaX_setinput(L, ls, z, source, firstchar) {
    cptr.stI32(cptr.add(ls, 16), 0);
    cptr.stPtr(cptr.add(ls, 56), L);
    cptr.stI32(ls, firstchar);
    cptr.stI32(cptr.add(ls, 32), TK_EOS);
    cptr.stPtr(cptr.add(ls, 64), z);
    cptr.stPtr(cptr.add(ls, 48), null);
    cptr.stI32(cptr.add(ls, 4), 1);
    cptr.stI32(cptr.add(ls, 8), 1);
    cptr.stPtr(cptr.add(ls, 96), source);
    cptr.stPtr(cptr.add(ls, 104), (luaS_newlstr(L, __sl38, (5n / 1n) - 1n)));
    (cptr.stPtr((cptr.ldPtr(cptr.add(ls, 72))), (((luaM_saferealloc_(cptr.ldPtr(cptr.add(ls, 56)), (cptr.ldPtr((cptr.ldPtr(cptr.add(ls, 72))))), (cptr.ldU64(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 16))) * 1n, 32n * 1n))))), cptr.stU64(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 16), 32n));
}

/** C ref: llex.c:191 — @param {CPtr} ls @param {CInt} c @returns {CInt} */
function check_next1(ls, c) {
    if (cptr.ldI32(ls) == c) {
        (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
        return 1;
    } else
        return 0;
}

/** C ref: llex.c:204 — @param {CPtr} ls @param {CPtr} set @returns {CInt} */
function check_next2(ls, set) {
    (void 0);
    if (cptr.ldI32(ls) == cptr.ld1s(cptr.add(set, 0)) || cptr.ldI32(ls) == cptr.ld1s(cptr.add(set, 1))) {
        (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
        return 1;
    } else
        return 0;
}

/** C ref: llex.c:227 — @param {CPtr} ls @param {CPtr} seminfo @returns {CInt} */
function read_numeral(ls, seminfo) {
    let obj = cptr.alloc(16);
    let expo = __sl44;
    let first = cptr.ldI32(ls);
    (void 0);
    (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
    if (first == 48 && check_next2(ls, __sl45))
        expo = __sl46;
    for (; ; ) {
        if (check_next2(ls, expo))
            check_next2(ls, __sl47);
        else if ((cptr.ld1u(cptr.add(cptr.decay(luai_ctype_), ((cptr.ldI32(ls)) + 1) | 0)) & ((1 << (4)))) || cptr.ldI32(ls) == 46)
            (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
        else
            break;
    }
    if ((cptr.ld1u(cptr.add(cptr.decay(luai_ctype_), ((cptr.ldI32(ls)) + 1) | 0)) & ((1 << (0)))))
        (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
    save(ls, 0);
    if (luaO_str2num((cptr.ldPtr((cptr.ldPtr(cptr.add(ls, 72))))), obj) == 0n)
        lexerror(ls, __sl48, TK_FLT);
    if (((cptr.ld1u(cptr.add(((obj)), 8))) == (((3) | ((0) << 4))))) {
        cptr.stU64(seminfo, (cptr.ldI64(((obj)))));
        return TK_INT;
    } else {
        (void 0);
        cptr.stF64(seminfo, (cptr.ldF64(((obj)))));
        return TK_FLT;
    }
}

/** C ref: llex.c:265 — @param {CPtr} ls @returns {*} */
function skip_sep(ls) {
    let count = 0n;
    let s = cptr.ldI32(ls);
    (void 0);
    (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
    while (cptr.ldI32(ls) == 61) {
        (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
        count++;
    }
    return (cptr.ldI32(ls) == s) ? count + 2n : BigInt.asUintN(64, BigInt(((count == 0n) ? 1 : 0)));
}

/** C ref: llex.c:280 — @param {CPtr} ls @param {CPtr} seminfo @param {CLongLong} sep */
function read_long_string(ls, seminfo, sep) {
    __lbl_endloop: {
        let line = cptr.ldI32(cptr.add(ls, 4));
        (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
        if ((cptr.ldI32(ls) == 10 || cptr.ldI32(ls) == 13))
            inclinenumber(ls);
        for (; ; ) {
            switch (cptr.ldI32(ls)) {
                case (-1):
                {
                    let what = (seminfo ? __sl49 : __sl50);
                    let msg = luaO_pushfstring(cptr.ldPtr(cptr.add(ls, 56)), __sl51, what, line);
                    lexerror(ls, msg, TK_EOS);
                    break;
                }
                case 93:
                {
                    if (skip_sep(ls) == sep) {
                        (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
                        break __lbl_endloop;
                    }
                    break;
                }
                case 10:
                case 13:
                {
                    save(ls, 10);
                    inclinenumber(ls);
                    if (!seminfo)
                        (cptr.stU64(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8), 0n));
                    break;
                }
                default:
                {
                    if (seminfo)
                        (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
                    else
                        (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                }
            }
        }
    }
    if (seminfo)
        cptr.stPtr(seminfo, luaX_newstring(ls, cptr.add((cptr.ldPtr((cptr.ldPtr(cptr.add(ls, 72))))), sep), (cptr.ldU64(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8))) - 2n * sep));
}

/** C ref: llex.c:319 — @param {CPtr} ls @param {CInt} c @param {CPtr} msg */
function esccheck(ls, c, msg) {
    if (!c) {
        if (cptr.ldI32(ls) != (-1))
            (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
        lexerror(ls, msg, TK_STRING);
    }
}

/** C ref: llex.c:328 — @param {CPtr} ls @returns {CInt} */
function gethexa(ls) {
    (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
    esccheck(ls, (cptr.ld1u(cptr.add(cptr.decay(luai_ctype_), ((cptr.ldI32(ls)) + 1) | 0)) & ((1 << (4)))), __sl52);
    return luaO_hexavalue(cptr.ldI32(ls));
}

/** C ref: llex.c:335 — @param {CPtr} ls @returns {CInt} */
function readhexaesc(ls) {
    let r = gethexa(ls);
    r = ((r << 4) + gethexa(ls)) | 0;
    (cptr.st1(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8), cptr.ld1u(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8)) - 2n));
    return r;
}

/** C ref: llex.c:343 — @param {CPtr} ls @returns {CLongLong} */
function readutf8esc(ls) {
    let r;
    let i = 4;
    (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
    esccheck(ls, cptr.ldI32(ls) == 123, __sl53);
    r = BigInt.asUintN(64, BigInt(gethexa(ls)));
    while ((void (((save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))))))), (cptr.ld1u(cptr.add(cptr.decay(luai_ctype_), ((cptr.ldI32(ls)) + 1) | 0)) & ((1 << (4))))) {
        i++;
        esccheck(ls, r <= BigInt((2147483647 >>> 4) >>> 0), __sl54);
        r = (r << 4n) + BigInt.asUintN(64, BigInt(luaO_hexavalue(cptr.ldI32(ls))));
    }
    esccheck(ls, cptr.ldI32(ls) == 125, __sl55);
    (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
    (cptr.st1(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8), cptr.ld1u(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8)) - BigInt.asUintN(64, BigInt((i)))));
    return r;
}

/** C ref: llex.c:361 — @param {CPtr} ls */
function utf8esc(ls) {
    let buff = new Uint8Array(8);
    let n = luaO_utf8esc(cptr.decay(buff), readutf8esc(ls));
    for (; n > 0; n--)
        save(ls, cptr.ld1s(cptr.add(cptr.decay(buff), (8 - n) | 0)));
}

/** C ref: llex.c:369 — @param {CPtr} ls @returns {CInt} */
function readdecesc(ls) {
    let i;
    let r = 0;
    for (i = 0; i < 3 && (cptr.ld1u(cptr.add(cptr.decay(luai_ctype_), ((cptr.ldI32(ls)) + 1) | 0)) & ((1 << (1)))); i++) {
        r = (((Math.imul(10, r) + cptr.ldI32(ls)) | 0) - 48) | 0;
        (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
    }
    esccheck(ls, r <= ((Math.imul(127, 2) + 1) | 0), __sl56);
    (cptr.st1(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8), cptr.ld1u(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8)) - BigInt.asUintN(64, BigInt((i)))));
    return r;
}

/** C ref: llex.c:382 — @param {CPtr} ls @param {CInt} del @param {CPtr} seminfo */
function read_string(ls, del, seminfo) {
    (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
    while (cptr.ldI32(ls) != del) {
        switch (cptr.ldI32(ls)) {
            case (-1):
            lexerror(ls, __sl57, TK_EOS);
            break;
            case 10:
            case 13:
            lexerror(ls, __sl57, TK_STRING);
            break;
            case 92:
            {
                __lbl_no_save: {
                __lbl_only_save: {
                __lbl_read_save: {
                    let c;
                    (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
                    switch (cptr.ldI32(ls)) {
                        case 97:
                        c = 7;
                        break __lbl_read_save;
                        case 98:
                        c = 8;
                        break __lbl_read_save;
                        case 102:
                        c = 12;
                        break __lbl_read_save;
                        case 110:
                        c = 10;
                        break __lbl_read_save;
                        case 114:
                        c = 13;
                        break __lbl_read_save;
                        case 116:
                        c = 9;
                        break __lbl_read_save;
                        case 118:
                        c = 11;
                        break __lbl_read_save;
                        case 120:
                        c = readhexaesc(ls);
                        break __lbl_read_save;
                        case 117:
                        utf8esc(ls);
                        break __lbl_no_save;
                        case 10:
                        case 13:
                        inclinenumber(ls);
                        c = 10;
                        break __lbl_only_save;
                        case 92:
                        case 34:
                        case 39:
                        c = cptr.ldI32(ls);
                        break __lbl_read_save;
                        case (-1):
                        break __lbl_no_save;
                        case 122:
                        {
                            (cptr.st1(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8), cptr.ld1u(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8)) - 1n));
                            (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                            while ((cptr.ld1u(cptr.add(cptr.decay(luai_ctype_), ((cptr.ldI32(ls)) + 1) | 0)) & ((1 << (3))))) {
                                if ((cptr.ldI32(ls) == 10 || cptr.ldI32(ls) == 13))
                                    inclinenumber(ls);
                                else
                                    (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                            }
                            break __lbl_no_save;
                        }
                        default:
                        {
                            esccheck(ls, (cptr.ld1u(cptr.add(cptr.decay(luai_ctype_), ((cptr.ldI32(ls)) + 1) | 0)) & ((1 << (1)))), __sl58);
                            c = readdecesc(ls);
                            break __lbl_only_save;
                        }
                    }
                }
                    (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                }
                    (cptr.st1(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8), cptr.ld1u(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8)) - 1n));
                    save(ls, c);
                }
                break;
            }
            default:
            (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
        }
    }
    (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
    cptr.stPtr(seminfo, luaX_newstring(ls, cptr.add((cptr.ldPtr((cptr.ldPtr(cptr.add(ls, 72))))), 1), (cptr.ldU64(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8))) - 2n));
}

/** C ref: llex.c:445 — @param {CPtr} ls @param {CPtr} seminfo @returns {CInt} */
function llex(ls, seminfo) {
    (cptr.stU64(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8), 0n));
    for (; ; ) {
        switch (cptr.ldI32(ls)) {
            case 10:
            case 13:
            {
                inclinenumber(ls);
                break;
            }
            case 32:
            case 12:
            case 9:
            case 11:
            {
                (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                break;
            }
            case 45:
            {
                (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                if (cptr.ldI32(ls) != 45)
                    return 45;
                (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                if (cptr.ldI32(ls) == 91) {
                    let sep = skip_sep(ls);
                    (cptr.stU64(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8), 0n));
                    if (sep >= 2n) {
                        read_long_string(ls, null, sep);
                        (cptr.stU64(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8), 0n));
                        break;
                    }
                }
                while (!(cptr.ldI32(ls) == 10 || cptr.ldI32(ls) == 13) && cptr.ldI32(ls) != (-1))
                    (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                break;
            }
            case 91:
            {
                let sep = skip_sep(ls);
                if (sep >= 2n) {
                    read_long_string(ls, seminfo, sep);
                    return TK_STRING;
                } else if (sep == 0n)
                    lexerror(ls, __sl59, TK_STRING);
                return 91;
            }
            case 61:
            {
                (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                if (check_next1(ls, 61))
                    return TK_EQ;
                else
                    return 61;
            }
            case 60:
            {
                (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                if (check_next1(ls, 61))
                    return TK_LE;
                else if (check_next1(ls, 60))
                    return TK_SHL;
                else
                    return 60;
            }
            case 62:
            {
                (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                if (check_next1(ls, 61))
                    return TK_GE;
                else if (check_next1(ls, 62))
                    return TK_SHR;
                else
                    return 62;
            }
            case 47:
            {
                (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                if (check_next1(ls, 47))
                    return TK_IDIV;
                else
                    return 47;
            }
            case 126:
            {
                (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                if (check_next1(ls, 61))
                    return TK_NE;
                else
                    return 126;
            }
            case 58:
            {
                (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                if (check_next1(ls, 58))
                    return TK_DBCOLON;
                else
                    return 58;
            }
            case 34:
            case 39:
            {
                read_string(ls, cptr.ldI32(ls), seminfo);
                return TK_STRING;
            }
            case 46:
            {
                (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
                if (check_next1(ls, 46)) {
                    if (check_next1(ls, 46))
                        return TK_DOTS;
                    else
                        return TK_CONCAT;
                } else if (!(cptr.ld1u(cptr.add(cptr.decay(luai_ctype_), ((cptr.ldI32(ls)) + 1) | 0)) & ((1 << (1)))))
                    return 46;
                else
                    return read_numeral(ls, seminfo);
            }
            case 48:
            case 49:
            case 50:
            case 51:
            case 52:
            case 53:
            case 54:
            case 55:
            case 56:
            case 57:
            {
                return read_numeral(ls, seminfo);
            }
            case (-1):
            {
                return TK_EOS;
            }
            default:
            {
                if ((cptr.ld1u(cptr.add(cptr.decay(luai_ctype_), ((cptr.ldI32(ls)) + 1) | 0)) & ((1 << (0))))) {
                    let ts;
                    do {
                        (save(ls, cptr.ldI32(ls)), (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64)))))));
                    } while ((cptr.ld1u(cptr.add(cptr.decay(luai_ctype_), ((cptr.ldI32(ls)) + 1) | 0)) & (((1 << (0)) | (1 << (1))))));
                    ts = luaX_newstring(ls, (cptr.ldPtr((cptr.ldPtr(cptr.add(ls, 72))))), (cptr.ldU64(cptr.add((cptr.ldPtr(cptr.add(ls, 72))), 8))));
                    cptr.stPtr(seminfo, ts);
                    if ((cptr.ld1u(cptr.add((ts), 8)) == ((4) | ((0) << 4)) && cptr.ld1u(cptr.add((ts), 10)) > 0))
                        return (((cptr.ld1u(cptr.add(ts, 10)) - 1) | 0) + ((((Math.imul(127, 2) + 1) | 0) + 1) | 0)) | 0;
                    else {
                        return TK_NAME;
                    }
                } else {
                    let c = cptr.ldI32(ls);
                    (cptr.stI32(ls, ((cptr.ldU64((cptr.ldPtr(cptr.add(ls, 64))))--) > 0n ? (uchar(((cptr.ld1s(cptr.postinc(() => cptr.ldPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8)), (v) => { cptr.stPtr(cptr.add((cptr.ldPtr(cptr.add(ls, 64))), 8), v); })))))) : luaZ_fill(cptr.ldPtr(cptr.add(ls, 64))))));
                    return c;
                }
            }
        }
    }
}

/** C ref: llex.c:565 — @param {CPtr} ls */
export function luaX_next(ls) {
    cptr.stI32(cptr.add(ls, 8), cptr.ldI32(cptr.add(ls, 4)));
    if (cptr.ldI32(cptr.add(ls, 32)) != TK_EOS) {
        cptr.memcpy(cptr.add(ls, 16), cptr.add(ls, 32), 16);
        cptr.stI32(cptr.add(ls, 32), TK_EOS);
    } else
        cptr.stI32(cptr.add(ls, 16), llex(ls, cptr.add(cptr.add(ls, 16), 8)));
}

/** C ref: llex.c:576 — @param {CPtr} ls @returns {CInt} */
export function luaX_lookahead(ls) {
    (void 0);
    cptr.stI32(cptr.add(ls, 32), llex(ls, cptr.add(cptr.add(ls, 32), 8)));
    return cptr.ldI32(cptr.add(ls, 32));
}
