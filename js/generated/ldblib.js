// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/lib/lua-5.4.8/src/ldblib.c
// Input sha256: 349afd63c8be2cb8a522e4a04e9f444b65ed1fc7f4dabba5b4b61fd8c11d0c42
// Transpiler: tools/c2js c2js emit v1+batch

import * as cptr from '../cptr.js';
import * as FLD from './nhfield.js';
import { lua_callk, lua_checkstack, lua_createtable, lua_getfield, lua_getiuservalue, lua_getmetatable, lua_getupvalue, lua_iscfunction, lua_pcallk, lua_pushboolean, lua_pushfstring, lua_pushinteger, lua_pushlightuserdata, lua_pushlstring, lua_pushnil, lua_pushstring, lua_pushthread, lua_pushvalue, lua_rawget, lua_rawset, lua_rotate, lua_setfield, lua_setiuservalue, lua_setmetatable, lua_settop, lua_setupvalue, lua_tolstring, lua_tothread, lua_type, lua_upvalueid, lua_upvaluejoin, lua_xmove } from './lapi.js';
import { luaL_argerror, luaL_checkany, luaL_checkinteger, luaL_checklstring, luaL_checktype, luaL_checkversion_, luaL_error, luaL_getsubtable, luaL_loadbufferx, luaL_optinteger, luaL_optlstring, luaL_setfuncs, luaL_tolstring, luaL_traceback, luaL_typeerror } from './lauxlib.js';
import { lua_gethook, lua_gethookcount, lua_gethookmask, lua_getinfo, lua_getlocal, lua_getstack, lua_sethook, lua_setlocal } from './ldebug.js';
import { lua_setcstacklimit } from './lstate.js';

// string literals (C char* uses decay to CPtr into these static buffers)
const __sl0 = cptr.lit("_HOOKKEY");
const __sl1 = cptr.lit("stack overflow");
const __sl2 = cptr.lit("nil or table");
const __sl3 = cptr.lit("flnSrtu");
const __sl4 = cptr.lit("invalid option '>'");
const __sl5 = cptr.lit(">%s");
const __sl6 = cptr.lit("invalid option");
const __sl7 = cptr.lit("source");
const __sl8 = cptr.lit("short_src");
const __sl9 = cptr.lit("linedefined");
const __sl10 = cptr.lit("lastlinedefined");
const __sl11 = cptr.lit("what");
const __sl12 = cptr.lit("currentline");
const __sl13 = cptr.lit("nups");
const __sl14 = cptr.lit("nparams");
const __sl15 = cptr.lit("isvararg");
const __sl16 = cptr.lit("name");
const __sl17 = cptr.lit("namewhat");
const __sl18 = cptr.lit("ftransfer");
const __sl19 = cptr.lit("ntransfer");
const __sl20 = cptr.lit("istailcall");
const __sl21 = cptr.lit("activelines");
const __sl22 = cptr.lit("func");
const __sl23 = cptr.lit("level out of range");
const __sl24 = cptr.lit("invalid upvalue index");
const __sl25 = cptr.lit("Lua function expected");
const __sl26 = cptr.lit("call");
const __sl27 = cptr.lit("return");
const __sl28 = cptr.lit("line");
const __sl29 = cptr.lit("count");
const __sl30 = cptr.lit("tail call");
const __sl31 = cptr.lit("k");
const __sl32 = cptr.lit("__mode");
const __sl33 = cptr.lit("external hook");
const __sl34 = cptr.lit("%s");
const __sl35 = cptr.lit("lua_debug> ");
const __sl36 = cptr.lit("cont\n");
const __sl37 = cptr.lit("=(debug command)");
const __sl38 = cptr.lit("%s\n");
const __sl39 = cptr.lit("debug");
const __sl40 = cptr.lit("getuservalue");
const __sl41 = cptr.lit("gethook");
const __sl42 = cptr.lit("getinfo");
const __sl43 = cptr.lit("getlocal");
const __sl44 = cptr.lit("getregistry");
const __sl45 = cptr.lit("getmetatable");
const __sl46 = cptr.lit("getupvalue");
const __sl47 = cptr.lit("upvaluejoin");
const __sl48 = cptr.lit("upvalueid");
const __sl49 = cptr.lit("setuservalue");
const __sl50 = cptr.lit("sethook");
const __sl51 = cptr.lit("setlocal");
const __sl52 = cptr.lit("setmetatable");
const __sl53 = cptr.lit("setupvalue");
const __sl54 = cptr.lit("traceback");
const __sl55 = cptr.lit("setcstacklimit");

/** C ref: ldblib.c:27 — char * */
let HOOKKEY = __sl0;

/** C ref: ldblib.c:35 — @param {CPtr} L @param {CPtr} L1 @param {CInt} n */
function checkstack(L, L1, n) {
    if ((__builtin_expect(BigInt(((!cptr.eq(L, L1) && !lua_checkstack(L1, n) ? 1 : 0) != 0)), 0n)))
        luaL_error(L, __sl1);
}

/** C ref: ldblib.c:41 — @param {CPtr} L @returns {CInt} */
function db_getregistry(L) {
    lua_pushvalue(L, -1001000);
    return 1;
}

/** C ref: ldblib.c:47 — @param {CPtr} L @returns {CInt} */
function db_getmetatable(L) {
    luaL_checkany(L, 1);
    if (!lua_getmetatable(L, 1)) {
        lua_pushnil(L);
    }
    return 1;
}

/** C ref: ldblib.c:56 — @param {CPtr} L @returns {CInt} */
function db_setmetatable(L) {
    let t = lua_type(L, 2);
    (void ((__builtin_expect(BigInt(((t == 0 || t == 5 ? 1 : 0) != 0)), 1n)) || luaL_typeerror(L, 2, (__sl2)) ? 1 : 0));
    lua_settop(L, 2);
    lua_setmetatable(L, 1);
    return 1;
}

/** C ref: ldblib.c:65 — @param {CPtr} L @returns {CInt} */
function db_getuservalue(L) {
    let n = Number(BigInt.asIntN(32, luaL_optinteger(L, 2, 1n)));
    if (lua_type(L, 1) != 7)
        lua_pushnil(L);
    else if (lua_getiuservalue(L, 1, n) != -1) {
        lua_pushboolean(L, 1);
        return 2;
    }
    return 1;
}

/** C ref: ldblib.c:77 — @param {CPtr} L @returns {CInt} */
function db_setuservalue(L) {
    let n = Number(BigInt.asIntN(32, luaL_optinteger(L, 3, 1n)));
    luaL_checktype(L, 1, 7);
    luaL_checkany(L, 2);
    lua_settop(L, 2);
    if (!lua_setiuservalue(L, 1, n))
        lua_pushnil(L);
    return 1;
}

/** C ref: ldblib.c:94 — @param {CPtr} L @param {CPtr} arg @returns {CPtr} */
function getthread(L, arg) {
    if ((lua_type(L, 1) == 8)) {
        cptr.stI32(arg, 1);
        return lua_tothread(L, 1);
    } else {
        cptr.stI32(arg, 0);
        return L;
    }
}

/** C ref: ldblib.c:111 — @param {CPtr} L @param {CPtr} k @param {CPtr} v */
function settabss(L, k, v) {
    lua_pushstring(L, v);
    lua_setfield(L, -2, k);
}

/** C ref: ldblib.c:116 — @param {CPtr} L @param {CPtr} k @param {CInt} v */
function settabsi(L, k, v) {
    lua_pushinteger(L, BigInt(v));
    lua_setfield(L, -2, k);
}

/** C ref: ldblib.c:121 — @param {CPtr} L @param {CPtr} k @param {CInt} v */
function settabsb(L, k, v) {
    lua_pushboolean(L, v);
    lua_setfield(L, -2, k);
}

/** C ref: ldblib.c:134 — @param {CPtr} L @param {CPtr} L1 @param {CPtr} fname */
function treatstackoption(L, L1, fname) {
    if (cptr.eq(L, L1))
        lua_rotate(L, -2, 1);
    else
        lua_xmove(L1, L, 1);
    lua_setfield(L, -2, fname);
}

/** C ref: ldblib.c:149 — @param {CPtr} L @returns {CInt} */
function db_getinfo(L) {
    let ar = cptr.alloc(136);
    let arg = cptr.box(0);
    let L1 = getthread(L, arg);
    let options = (luaL_optlstring(L, ((arg.v + 2) | 0), (__sl3), null));
    checkstack(L, L1, 3);
    (void ((__builtin_expect(BigInt(((cptr.ld1so(options, 0) != 62) != 0)), 1n)) || luaL_argerror(L, ((arg.v + 2) | 0), (__sl4)) ? 1 : 0));
    if ((lua_type(L, ((arg.v + 1) | 0)) == 6)) {
        options = lua_pushfstring(L, __sl5, options);
        lua_pushvalue(L, (arg.v + 1) | 0);
        lua_xmove(L, L1, 1);
    } else {
        if (!lua_getstack(L1, Number(BigInt.asIntN(32, luaL_checkinteger(L, (arg.v + 1) | 0))), ar)) {
            lua_pushnil(L);
            return 1;
        }
    }
    if (!lua_getinfo(L1, options, ar))
        return luaL_argerror(L, (arg.v + 2) | 0, __sl6);
    lua_createtable(L, 0, 0);
    if (cptr.strchr(options, 83)) {
        lua_pushlstring(L, cptr.ldPtro(ar, FLD.lua_Debug_source), cptr.ldU64o(ar, FLD.lua_Debug_srclen));
        lua_setfield(L, -2, __sl7);
        settabss(L, __sl8, cptr.add(ar, FLD.lua_Debug_short_src));
        settabsi(L, __sl9, cptr.ldI32o(ar, FLD.lua_Debug_linedefined));
        settabsi(L, __sl10, cptr.ldI32o(ar, FLD.lua_Debug_lastlinedefined));
        settabss(L, __sl11, cptr.ldPtro(ar, FLD.lua_Debug_what));
    }
    if (cptr.strchr(options, 108))
        settabsi(L, __sl12, cptr.ldI32o(ar, FLD.lua_Debug_currentline));
    if (cptr.strchr(options, 117)) {
        settabsi(L, __sl13, cptr.ld1uo(ar, FLD.lua_Debug_nups));
        settabsi(L, __sl14, cptr.ld1uo(ar, FLD.lua_Debug_nparams));
        settabsb(L, __sl15, cptr.ld1so(ar, FLD.lua_Debug_isvararg));
    }
    if (cptr.strchr(options, 110)) {
        settabss(L, __sl16, cptr.ldPtro(ar, FLD.lua_Debug_name));
        settabss(L, __sl17, cptr.ldPtro(ar, FLD.lua_Debug_namewhat));
    }
    if (cptr.strchr(options, 114)) {
        settabsi(L, __sl18, cptr.ldU16o(ar, FLD.lua_Debug_ftransfer));
        settabsi(L, __sl19, cptr.ldU16o(ar, FLD.lua_Debug_ntransfer));
    }
    if (cptr.strchr(options, 116))
        settabsb(L, __sl20, cptr.ld1so(ar, FLD.lua_Debug_istailcall));
    if (cptr.strchr(options, 76))
        treatstackoption(L, L1, __sl21);
    if (cptr.strchr(options, 102))
        treatstackoption(L, L1, __sl22);
    return 1;
}

/** C ref: ldblib.c:203 — @param {CPtr} L @returns {CInt} */
function db_getlocal(L) {
    let arg = cptr.box(0);
    let L1 = getthread(L, arg);
    let nvar = Number(BigInt.asIntN(32, luaL_checkinteger(L, (arg.v + 2) | 0)));
    if ((lua_type(L, ((arg.v + 1) | 0)) == 6)) {
        lua_pushvalue(L, (arg.v + 1) | 0);
        lua_pushstring(L, lua_getlocal(L, null, nvar));
        return 1;
    } else {
        let ar = cptr.alloc(136);
        let name;
        let level = Number(BigInt.asIntN(32, luaL_checkinteger(L, (arg.v + 1) | 0)));
        if ((__builtin_expect(BigInt(((!lua_getstack(L1, level, ar)) != 0)), 0n)))
            return luaL_argerror(L, (arg.v + 1) | 0, __sl23);
        checkstack(L, L1, 1);
        name = lua_getlocal(L1, ar, nvar);
        if (name) {
            lua_xmove(L1, L, 1);
            lua_pushstring(L, name);
            lua_rotate(L, -2, 1);
            return 2;
        } else {
            lua_pushnil(L);
            return 1;
        }
    }
}

/** C ref: ldblib.c:234 — @param {CPtr} L @returns {CInt} */
function db_setlocal(L) {
    let arg = cptr.box(0);
    let name;
    let L1 = getthread(L, arg);
    let ar = cptr.alloc(136);
    let level = Number(BigInt.asIntN(32, luaL_checkinteger(L, (arg.v + 1) | 0)));
    let nvar = Number(BigInt.asIntN(32, luaL_checkinteger(L, (arg.v + 2) | 0)));
    if ((__builtin_expect(BigInt(((!lua_getstack(L1, level, ar)) != 0)), 0n)))
        return luaL_argerror(L, (arg.v + 1) | 0, __sl23);
    luaL_checkany(L, (arg.v + 3) | 0);
    lua_settop(L, (arg.v + 3) | 0);
    checkstack(L, L1, 1);
    lua_xmove(L, L1, 1);
    name = lua_setlocal(L1, ar, nvar);
    if (cptr.eq(name, (null)))
        lua_settop(L1, -2);
    lua_pushstring(L, name);
    return 1;
}

/** C ref: ldblib.c:258 — @param {CPtr} L @param {CInt} get @returns {CInt} */
function auxupvalue(L, get) {
    let name;
    let n = Number(BigInt.asIntN(32, luaL_checkinteger(L, 2)));
    luaL_checktype(L, 1, 6);
    name = get ? lua_getupvalue(L, 1, n) : lua_setupvalue(L, 1, n);
    if (cptr.eq(name, (null)))
        return 0;
    lua_pushstring(L, name);
    lua_rotate(L, (-((get + 1) | 0)), 1);
    return (get + 1) | 0;
}

/** C ref: ldblib.c:270 — @param {CPtr} L @returns {CInt} */
function db_getupvalue(L) {
    return auxupvalue(L, 1);
}

/** C ref: ldblib.c:275 — @param {CPtr} L @returns {CInt} */
function db_setupvalue(L) {
    luaL_checkany(L, 3);
    return auxupvalue(L, 0);
}

/** C ref: ldblib.c:285 — @param {CPtr} L @param {CInt} argf @param {CInt} argnup @param {CPtr} pnup @returns {CPtr} */
function checkupval(L, argf, argnup, pnup) {
    let id;
    let nup = Number(BigInt.asIntN(32, luaL_checkinteger(L, argnup)));
    luaL_checktype(L, argf, 6);
    id = lua_upvalueid(L, argf, nup);
    if (pnup) {
        (void ((__builtin_expect(BigInt(((!cptr.eq(id, (null))) != 0)), 1n)) || luaL_argerror(L, (argnup), (__sl24)) ? 1 : 0));
        cptr.stI32(pnup, nup);
    }
    return id;
}

/** C ref: ldblib.c:298 — @param {CPtr} L @returns {CInt} */
function db_upvalueid(L) {
    let id = checkupval(L, 1, 2, null);
    if (!cptr.eq(id, (null)))
        lua_pushlightuserdata(L, id);
    else
        lua_pushnil(L);
    return 1;
}

/** C ref: ldblib.c:308 — @param {CPtr} L @returns {CInt} */
function db_upvaluejoin(L) {
    let n1 = cptr.box(0);
    let n2 = cptr.box(0);
    checkupval(L, 1, 2, n1);
    checkupval(L, 3, 4, n2);
    (void ((__builtin_expect(BigInt(((!lua_iscfunction(L, 1)) != 0)), 1n)) || luaL_argerror(L, 1, (__sl25)) ? 1 : 0));
    (void ((__builtin_expect(BigInt(((!lua_iscfunction(L, 3)) != 0)), 1n)) || luaL_argerror(L, 3, (__sl25)) ? 1 : 0));
    lua_upvaluejoin(L, 1, n1.v, 3, n2.v);
    return 0;
}

const __static_hookf_hooknames = cptr.alloc(5 * 8);
cptr.stPtro(__static_hookf_hooknames, 0, __sl26);
cptr.stPtro(__static_hookf_hooknames, 8, __sl27);
cptr.stPtro(__static_hookf_hooknames, 16, __sl28);
cptr.stPtro(__static_hookf_hooknames, 24, __sl29);
cptr.stPtro(__static_hookf_hooknames, 32, __sl30); /** C ref: ldblib.c:324 — char *[5] (function-static) */

/** C ref: ldblib.c:323 — @param {CPtr} L @param {CPtr} ar */
function hookf(L, ar) {
    lua_getfield(L, -1001000, HOOKKEY);
    lua_pushthread(L);
    if (lua_rawget(L, -2) == 6) {
        lua_pushstring(L, cptr.ldPtro(__static_hookf_hooknames, cptr.ldI32(ar), 8));
        if (cptr.ldI32o(ar, FLD.lua_Debug_currentline) >= 0)
            lua_pushinteger(L, BigInt(cptr.ldI32o(ar, FLD.lua_Debug_currentline)));
        else
            lua_pushnil(L);
        (void 0);
        lua_callk(L, 2, 0, 0n, null);
    }
}

/** C ref: ldblib.c:342 — @param {CPtr} smask @param {CInt} count @returns {CInt} */
function makemask(smask, count) {
    let mask = 0;
    if (cptr.strchr(smask, 99))
        mask |= 1;
    if (cptr.strchr(smask, 114))
        mask |= 2;
    if (cptr.strchr(smask, 108))
        mask |= 4;
    if (count > 0)
        mask |= 8;
    return mask;
}

/** C ref: ldblib.c:355 — @param {CInt} mask @param {CPtr} smask @returns {CPtr} */
function unmakemask(mask, smask) {
    let i = 0;
    if (mask & 1)
        cptr.st1o(smask, i++, 99);
    if (mask & 2)
        cptr.st1o(smask, i++, 114);
    if (mask & 4)
        cptr.st1o(smask, i++, 108);
    cptr.st1o(smask, i, 0);
    return smask;
}

/** C ref: ldblib.c:365 — @param {CPtr} L @returns {CInt} */
function db_sethook(L) {
    let arg = cptr.box(0);
    let mask;
    let count;
    let func;
    let L1 = getthread(L, arg);
    if ((lua_type(L, ((arg.v + 1) | 0)) <= 0)) {
        lua_settop(L, (arg.v + 1) | 0);
        func = null;
        mask = 0;
        count = 0;
    } else {
        let smask = (luaL_checklstring(L, ((arg.v + 2) | 0), null));
        luaL_checktype(L, (arg.v + 1) | 0, 6);
        count = Number(BigInt.asIntN(32, luaL_optinteger(L, (arg.v + 3) | 0, 0n)));
        func = hookf;
        mask = makemask(smask, count);
    }
    if (!luaL_getsubtable(L, -1001000, HOOKKEY)) {
        lua_pushstring(L, __sl31);
        lua_setfield(L, -2, __sl32);
        lua_pushvalue(L, -1);
        lua_setmetatable(L, -2);
    }
    checkstack(L, L1, 1);
    lua_pushthread(L1);
    lua_xmove(L1, L, 1);
    lua_pushvalue(L, (arg.v + 1) | 0);
    lua_rawset(L, -3);
    lua_sethook(L1, func, mask, count);
    return 0;
}

/** C ref: ldblib.c:395 — @param {CPtr} L @returns {CInt} */
function db_gethook(L) {
    let arg = cptr.box(0);
    let L1 = getthread(L, arg);
    let buff = new Uint8Array(5);
    let mask = lua_gethookmask(L1);
    let hook = lua_gethook(L1);
    if (hook === (null)) {
        lua_pushnil(L);
        return 1;
    } else if (hook !== hookf)
        lua_pushstring(L, __sl33);
    else {
        lua_getfield(L, -1001000, HOOKKEY);
        checkstack(L, L1, 1);
        lua_pushthread(L1);
        lua_xmove(L1, L, 1);
        lua_rawget(L, -2);
        (lua_rotate(L, -2, -1), lua_settop(L, -2));
    }
    lua_pushstring(L, unmakemask(mask, cptr.decay(buff)));
    lua_pushinteger(L, BigInt(lua_gethookcount(L1)));
    return 3;
}

/** C ref: ldblib.c:420 — @param {CPtr} L @returns {CInt} */
function db_debug(L) {
    for (; ; ) {
        let buffer = new Uint8Array(250);
        (fprintf(__stderrp, (__sl34), (__sl35)), fflush(__stderrp));
        if (cptr.eq(fgets(cptr.decay(buffer), 250, __stdinp), (null)) || strcmp(cptr.decay(buffer), __sl36) == 0)
            return 0;
        if (luaL_loadbufferx(L, cptr.decay(buffer), cptr.strlen(cptr.decay(buffer)), __sl37, null) || lua_pcallk(L, 0, 0, 0, 0n, null))
            (fprintf(__stderrp, (__sl38), (luaL_tolstring(L, -1, null))), fflush(__stderrp));
        lua_settop(L, 0);
    }
}

/** C ref: ldblib.c:435 — @param {CPtr} L @returns {CInt} */
function db_traceback(L) {
    let arg = cptr.box(0);
    let L1 = getthread(L, arg);
    let msg = lua_tolstring(L, ((arg.v + 1) | 0), null);
    if (cptr.eq(msg, (null)) && !(lua_type(L, ((arg.v + 1) | 0)) <= 0))
        lua_pushvalue(L, (arg.v + 1) | 0);
    else {
        let level = Number(BigInt.asIntN(32, luaL_optinteger(L, (arg.v + 2) | 0, BigInt(((cptr.eq(L, L1)) ? 1 : 0)))));
        luaL_traceback(L, L1, msg, level);
    }
    return 1;
}

/** C ref: ldblib.c:449 — @param {CPtr} L @returns {CInt} */
function db_setcstacklimit(L) {
    let limit = Number(BigInt.asIntN(32, luaL_checkinteger(L, 1)));
    let res = lua_setcstacklimit(L, limit >>> 0);
    lua_pushinteger(L, BigInt(res));
    return 1;
}

/** C ref: ldblib.c:457 — luaL_Reg[18] */
const dblib = cptr.alloc(18 * 16);
cptr.stPtro(dblib, 0, __sl39);
cptr.stPtro(dblib, 0 + FLD.luaL_Reg_func, db_debug);
cptr.stPtro(dblib, 16, __sl40);
cptr.stPtro(dblib, 16 + FLD.luaL_Reg_func, db_getuservalue);
cptr.stPtro(dblib, 32, __sl41);
cptr.stPtro(dblib, 32 + FLD.luaL_Reg_func, db_gethook);
cptr.stPtro(dblib, 48, __sl42);
cptr.stPtro(dblib, 48 + FLD.luaL_Reg_func, db_getinfo);
cptr.stPtro(dblib, 64, __sl43);
cptr.stPtro(dblib, 64 + FLD.luaL_Reg_func, db_getlocal);
cptr.stPtro(dblib, 80, __sl44);
cptr.stPtro(dblib, 80 + FLD.luaL_Reg_func, db_getregistry);
cptr.stPtro(dblib, 96, __sl45);
cptr.stPtro(dblib, 96 + FLD.luaL_Reg_func, db_getmetatable);
cptr.stPtro(dblib, 112, __sl46);
cptr.stPtro(dblib, 112 + FLD.luaL_Reg_func, db_getupvalue);
cptr.stPtro(dblib, 128, __sl47);
cptr.stPtro(dblib, 128 + FLD.luaL_Reg_func, db_upvaluejoin);
cptr.stPtro(dblib, 144, __sl48);
cptr.stPtro(dblib, 144 + FLD.luaL_Reg_func, db_upvalueid);
cptr.stPtro(dblib, 160, __sl49);
cptr.stPtro(dblib, 160 + FLD.luaL_Reg_func, db_setuservalue);
cptr.stPtro(dblib, 176, __sl50);
cptr.stPtro(dblib, 176 + FLD.luaL_Reg_func, db_sethook);
cptr.stPtro(dblib, 192, __sl51);
cptr.stPtro(dblib, 192 + FLD.luaL_Reg_func, db_setlocal);
cptr.stPtro(dblib, 208, __sl52);
cptr.stPtro(dblib, 208 + FLD.luaL_Reg_func, db_setmetatable);
cptr.stPtro(dblib, 224, __sl53);
cptr.stPtro(dblib, 224 + FLD.luaL_Reg_func, db_setupvalue);
cptr.stPtro(dblib, 240, __sl54);
cptr.stPtro(dblib, 240 + FLD.luaL_Reg_func, db_traceback);
cptr.stPtro(dblib, 256, __sl55);
cptr.stPtro(dblib, 256 + FLD.luaL_Reg_func, db_setcstacklimit);
cptr.stPtro(dblib, 272, null);
cptr.stPtro(dblib, 272 + FLD.luaL_Reg_func, null);

/** C ref: ldblib.c:479 — @param {CPtr} L @returns {CInt} */
export function luaopen_debug(L) {
    (luaL_checkversion_(L, 504, 136n), lua_createtable(L, 0, Number(BigInt.asIntN(32, BigInt.asUintN(64, 288n / 16n - 1n)))), luaL_setfuncs(L, dblib, 0));
    return 1;
}

// --- BEGIN c2js reset block (tools/c2js/resetify.mjs) — do not edit ---
// 3 bindings: 0 rebound+refilled, 1 rebound, 2 refilled.
// S/P are supplied by js/generated/__reset.js so this module needs no new import.
let __c2js_rs = null;
export function __captureState(S) { __c2js_rs = [S(HOOKKEY), S(__static_hookf_hooknames), S(dblib)]; }
export function __resetState(P) {
    const r = __c2js_rs;
    if (r === null) throw new Error("ldblib.js: __resetState before __captureState");
    HOOKKEY = P(r[0]);
    P(r[1]);
    P(r[2]);
}
// --- END c2js reset block ---
