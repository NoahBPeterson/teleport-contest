// Generated by tools/c2js — do not edit by hand
// Input: nethack-c/recorder/src/report.c
// Input sha256: 816c57e507cf128fc7f5313c7f0c50a2183c9101764a9494704ec425bbef55a2
// Transpiler: tools/c2js c2js emit v1+batch

import { u32mod } from '../cmachine.js';
import * as cptr from '../cptr.js';
import * as NHM from './nhmacro.js';
import * as FLD from './nhfield.js';
import { windowprocs } from './windows.js';
import { ARGV0, gc, gs } from './decl.js';
import { sysopt } from './sys.js';
import { getversionstring, version_string } from './version.js';
import { copynchars, nh_snprintf, strsubst } from './hacklib.js';
import { pline, raw_printf } from './pline.js';
import { NH_abort } from './end.js';

// string literals (C char* uses decay to CPtr into these static buffers)
const __sl0 = cptr.lit("unknown");
const __sl1 = cptr.lit("_-.~");
const __sl2 = cptr.lit("%%%02X");
const __sl3 = cptr.lit("?cos=%d&v=1");
const __sl4 = cptr.lit("&subject=");
const __sl5 = cptr.lit("%.40s report for NetHack %.40s");
const __sl6 = cptr.lit("&gitver=");
const __sl7 = cptr.lit("&name=");
const __sl8 = cptr.lit("&email=");
const __sl9 = cptr.lit("&details=");
const __sl10 = cptr.lit("\n");
const __sl11 = cptr.lit("bid: ");
const __sl12 = cptr.lit("        ");
const __sl13 = cptr.lit("");
const __sl14 = cptr.lit("Latest messages:\n");
const __sl15 = cptr.lit("&detailrows=");
const __sl16 = cptr.lit("%d");
const __sl17 = cptr.lit("/usr/bin/open");
const __sl18 = cptr.lit("Can't start /usr/bin/open: %.*s");
const __sl19 = cptr.lit("#bugreport command");
const __sl20 = cptr.lit("Unable to send bug report.  Please visit %s instead.");
const __sl21 = cptr.lit("https://www.nethack.org/");
const __sl22 = cptr.lit("  Generating more information you may report:\n");
const __sl23 = cptr.lit("[%02lu] %s");
const __sl24 = cptr.lit("NH_panictrace_gdb");
const __sl25 = cptr.lit("%s -n -q %s %d 2>&1 | %s '^#'");
const __sl26 = cptr.lit("w");
const __sl27 = cptr.lit("bt\nquit\ny");
const __sl28 = cptr.lit("\nSignal received.\n");

/** C ref: report.c:109 — char[40] */
const bid = new Uint8Array(40);

let __static_crashreport_init_once = 0; /** C ref: report.c:115 — int (function-static) */
const __static_crashreport_init_hexdigits = cptr.bytes("0123456789abcdef"); /** C ref: report.c:148 — char[17] (function-static) */

/** C ref: report.c:113 — @param {CInt} argc @param {CPtr} argv */
export function crashreport_init(argc, argv) {
    __lbl_skip: {
        if (__static_crashreport_init_once++)
            return;
        let binfile = cptr.ldPtro(argv, 0, 8);
        ;
        ;
        let tmp = new Uint8Array(16);
        let ctxp_ = cptr.alloc(160);
        let ctxp = ctxp_;
        if (!CC_MD4_Init(ctxp))
            break __lbl_skip;
        if (!binfile || !cptr.ld1s(binfile)) {
            break __lbl_skip;
        }
        ;
        let fd = open(binfile, 0, 0);
        if (fd == -1) {
            break __lbl_skip;
        }
        let segsize;
        let segment = new Uint8Array(4096);
        while (0 < (segsize = Number(BigInt.asIntN(32, cptr.read(fd, cptr.decay(segment), 4096n))))) {
            if (!CC_MD4_Update(ctxp, cptr.decay(segment), segsize >>> 0))
                break __lbl_skip;
        }
        if (segsize < 0) {
            close(fd);
            break __lbl_skip;
        }
        if (!CC_MD4_Final(cptr.decay(tmp), ctxp))
            break __lbl_skip;
        close(fd);
        let p = cptr.decay(bid);
        let in$ = cptr.box(0);
        cptr.stPtr(in$, ctxp);
        let cnt = 16;
        if (cnt >= 20)
            cnt = 19;
        while (cnt) {
            cptr.st1(cptr.postinc(() => p, (v) => { p = v; }), cptr.ld1so(cptr.decay(__static_crashreport_init_hexdigits), (cptr.ld1u(in$.v) >> 4) & 15, 1));
            cptr.st1(cptr.postinc(() => p, (v) => { p = v; }), cptr.ld1so(cptr.decay(__static_crashreport_init_hexdigits), cptr.ld1u(cptr.postinc(() => in$.v, (v) => { in$.v = v; })) & 15, 1));
            --cnt;
        }
        cptr.st1(p, 0);
        ;
        return;
    }
    void cptr.strcpy(cptr.decay(bid), __sl0);
    ;
    ;
    (void (argc));
    (void (argv));
}

/** C ref: report.c:189 */
export function crashreport_bidshow() {
    {
        (cptr.ldPtro(windowprocs, FLD.window_procs_win_raw_print))(cptr.decay(bid));
    }
}

/** C ref: report.c:237 — @param {CPtr} in @param {CPtr} out @param {CPtr} remaining @param {CPtr} markp @returns {CInt} */
export function swr_add_uricoded(in$, out, remaining, markp) {
    while (cptr.ld1s(in$)) {
        if (isalnum(cptr.ld1s(in$)) || cptr.strchr(__sl1, cptr.ld1s(in$))) {
            cptr.st1(cptr.ldPtr(out), cptr.ld1s(in$));
            cptr.postinc(() => cptr.ldPtr(out), (v) => { cptr.stPtr(out, v); });
            (cptr.stI32(remaining, cptr.ldI32(remaining) + -1)) - (-1);
        } else if (cptr.ld1s(in$) == 32) {
            cptr.st1(cptr.ldPtr(out), 43);
            cptr.postinc(() => cptr.ldPtr(out), (v) => { cptr.stPtr(out, v); });
            (cptr.stI32(remaining, cptr.ldI32(remaining) + -1)) - (-1);
        } else {
            if (cptr.ldI32(remaining) <= 3) {
                if (markp)
                    cptr.stPtr(out, markp), cptr.stI32(remaining, 0);
                cptr.st1(cptr.ldPtr(out), 0);
                return 1;
            } else {
                let chr = new Uint8Array(40);
                let x;
                void cptr.sprintf(cptr.decay(chr), __sl2, cptr.ld1s(in$));
                x = Number(BigInt.asIntN(32, cptr.strlen(cptr.decay(chr))));
                if (x <= cptr.ldI32(remaining)) {
                    void cptr.strcpy(cptr.ldPtr(out), cptr.decay(chr));
                    cptr.stPtr(out, cptr.add(cptr.ldPtr(out), x));
                    cptr.stI32(remaining, (cptr.ldI32(remaining) - x) | 0);
                }
            }
        }
        in$ = cptr.add(in$, 1);
        if (!cptr.ldI32(remaining)) {
            if (markp)
                cptr.stPtr(out, markp), cptr.stI32(remaining, 0);
            cptr.st1(cptr.ldPtr(out), 0);
            return 1;
        }
        cptr.st1(cptr.ldPtr(out), 0);
    }
    return 0;
}

/** C ref: report.c:283 — char[8192] */
const url = new Uint8Array(8192);

/** C ref: report.c:284 — int */
let urem = cptr.box(8192);

/** C ref: report.c:285 — char * */
let uend = cptr.box(cptr.decay(url));

/** C ref: report.c:286 — int */
let utmp = 0;

/** C ref: report.c:287 — char * */
let mark = null;

/** C ref: report.c:290 — @param {CInt} cos @param {CPtr} msg @param {CPtr} why @returns {CInt} */
export function submit_web_report(cos, msg, why) {
    __lbl_full: {
        urem.v = (cptr.ldI32o(gc, FLD.instance_globals_c_crash_urlmax) < 0 || cptr.ldI32o(gc, FLD.instance_globals_c_crash_urlmax) > 8192) ? 8192 : (8192 < (cptr.ldI32o(gc, FLD.instance_globals_c_crash_urlmax)) ? 8192 : (cptr.ldI32o(gc, FLD.instance_globals_c_crash_urlmax)));
        let temp = new Uint8Array(200);
        let temp2 = new Uint8Array(200);
        let countpp = 0;
        if (!cptr.ldPtro(sysopt, FLD.sysopt_s_crashreporturl))
            return 0;
        utmp = Number(BigInt.asIntN(32, cptr.strlen(cptr.ldPtro(sysopt, FLD.sysopt_s_crashreporturl))));
        mark = uend.v;
        if (utmp >= urem.v)
            break __lbl_full;
        cptr.memcpy(uend.v, cptr.ldPtro(sysopt, FLD.sysopt_s_crashreporturl), BigInt.asUintN(64, BigInt(utmp)));
        uend.v = cptr.add(uend.v, utmp);
        urem.v = (urem.v - utmp) | 0;
        cptr.st1(uend.v, 0);
        ;
        void cptr.sprintf(cptr.decay(temp), __sl3, cos);
        utmp = Number(BigInt.asIntN(32, cptr.strlen(cptr.decay(temp))));
        mark = uend.v;
        if (utmp >= urem.v)
            break __lbl_full;
        cptr.memcpy(uend.v, cptr.decay(temp), BigInt.asUintN(64, BigInt(utmp)));
        uend.v = cptr.add(uend.v, utmp);
        urem.v = (urem.v - utmp) | 0;
        cptr.st1(uend.v, 0);
        ;
        if (msg) {
            utmp = Number(BigInt.asIntN(32, cptr.strlen(__sl4)));
            mark = uend.v;
            if (utmp >= urem.v)
                break __lbl_full;
            cptr.memcpy(uend.v, __sl4, BigInt.asUintN(64, BigInt(utmp)));
            uend.v = cptr.add(uend.v, utmp);
            urem.v = (urem.v - utmp) | 0;
            cptr.st1(uend.v, 0);
            ;
            void cptr.sprintf(cptr.decay(temp), __sl5, msg, version_string(cptr.decay(temp2), 200n));
            if (swr_add_uricoded(cptr.decay(temp), uend, urem, mark))
                break __lbl_full;
            ;
        }
        utmp = Number(BigInt.asIntN(32, cptr.strlen(__sl6)));
        mark = uend.v;
        if (utmp >= urem.v)
            break __lbl_full;
        cptr.memcpy(uend.v, __sl6, BigInt.asUintN(64, BigInt(utmp)));
        uend.v = cptr.add(uend.v, utmp);
        urem.v = (urem.v - utmp) | 0;
        cptr.st1(uend.v, 0);
        ;
        if (swr_add_uricoded(getversionstring(cptr.decay(temp2), 200n), uend, urem, mark))
            break __lbl_full;
        ;
        if (cptr.ldPtro(gc, FLD.instance_globals_c_crash_name)) {
            utmp = Number(BigInt.asIntN(32, cptr.strlen(__sl7)));
            mark = uend.v;
            if (utmp >= urem.v)
                break __lbl_full;
            cptr.memcpy(uend.v, __sl7, BigInt.asUintN(64, BigInt(utmp)));
            uend.v = cptr.add(uend.v, utmp);
            urem.v = (urem.v - utmp) | 0;
            cptr.st1(uend.v, 0);
            ;
            if (swr_add_uricoded(cptr.ldPtro(gc, FLD.instance_globals_c_crash_name), uend, urem, mark))
                break __lbl_full;
            ;
        }
        if (cptr.ldPtro(gc, FLD.instance_globals_c_crash_email)) {
            utmp = Number(BigInt.asIntN(32, cptr.strlen(__sl8)));
            mark = uend.v;
            if (utmp >= urem.v)
                break __lbl_full;
            cptr.memcpy(uend.v, __sl8, BigInt.asUintN(64, BigInt(utmp)));
            uend.v = cptr.add(uend.v, utmp);
            urem.v = (urem.v - utmp) | 0;
            cptr.st1(uend.v, 0);
            ;
            if (swr_add_uricoded(cptr.ldPtro(gc, FLD.instance_globals_c_crash_email), uend, urem, mark))
                break __lbl_full;
            ;
        }
        utmp = Number(BigInt.asIntN(32, cptr.strlen(__sl9)));
        mark = uend.v;
        if (utmp >= urem.v)
            break __lbl_full;
        cptr.memcpy(uend.v, __sl9, BigInt.asUintN(64, BigInt(utmp)));
        uend.v = cptr.add(uend.v, utmp);
        urem.v = (urem.v - utmp) | 0;
        cptr.st1(uend.v, 0);
        ;
        if (why) {
            if (swr_add_uricoded(why, uend, urem, mark))
                break __lbl_full;
            ;
            if (swr_add_uricoded(__sl10, uend, urem, mark))
                break __lbl_full;
            ;
            mark = uend.v;
            countpp++;
        }
        if (swr_add_uricoded(__sl11, uend, urem, mark))
            break __lbl_full;
        ;
        if (swr_add_uricoded(cptr.decay(bid), uend, urem, mark))
            break __lbl_full;
        ;
        if (swr_add_uricoded(__sl10, uend, urem, mark))
            break __lbl_full;
        ;
        mark = uend.v;
        countpp++;
        let count = 0;
        if (cos == 1) {
            let bt = cptr.alloc(20 * 8);
            let x;
            let info;
            count = backtrace(bt, 20);
            info = backtrace_symbols(bt, count);
            for (x = 0; x < count; x++) {
                copynchars(cptr.decay(temp), cptr.ldPtro(info, x, 8), 198);
                void strsubst(cptr.decay(temp), __sl12, __sl13);
                void strsubst(cptr.decay(temp), __sl12, __sl13);
                void __builtin___strncat_chk(cptr.decay(temp), __sl10, 199n, __builtin_object_size(cptr.decay(temp), 1));
                if (swr_add_uricoded(cptr.decay(temp), uend, urem, mark))
                    break __lbl_full;
                ;
                mark = uend.v;
            }
        }
        if (cos == 1) {
            let k;
            if (swr_add_uricoded(__sl14, uend, urem, mark))
                break __lbl_full;
            ;
            mark = uend.v;
            countpp++;
            for (k = 0; k < 5; k++) {
                let line = get_saved_pline(k);
                if (!line)
                    break;
                if (swr_add_uricoded(line, uend, urem, mark))
                    break __lbl_full;
                ;
                if (swr_add_uricoded(__sl10, uend, urem, mark))
                    break __lbl_full;
                ;
                countpp++;
                mark = uend.v;
            }
        }
        utmp = Number(BigInt.asIntN(32, cptr.strlen(__sl15)));
        mark = uend.v;
        if (utmp >= urem.v)
            break __lbl_full;
        cptr.memcpy(uend.v, __sl15, BigInt.asUintN(64, BigInt(utmp)));
        uend.v = cptr.add(uend.v, utmp);
        urem.v = (urem.v - utmp) | 0;
        cptr.st1(uend.v, 0);
        ;
        void cptr.sprintf(cptr.decay(temp), __sl16, (((count + countpp) | 0) < 30 ? ((count + countpp) | 0) : 30));
        if (swr_add_uricoded(cptr.decay(temp), uend, urem, mark))
            break __lbl_full;
        ;
    }
    ;
    let xargv = cptr.alloc(3 * 8); cptr.stPtro(xargv, 0, __sl17); cptr.stPtro(xargv, 8, cptr.decay(url)); cptr.stPtro(xargv, 16, null);
    let pid = fork();
    if (pid == 0) {
        let err = new Uint8Array(400);
        void execve(__sl17, xargv, environ);
        void cptr.sprintf(cptr.decay(err), __sl18, 372, strerror((cptr.ldI32(__error()))));
        (cptr.ldPtro(windowprocs, FLD.window_procs_win_raw_print))(cptr.decay(err));
        exit(1);
    } else {
        let status = cptr.box(0);
        cptr.stI32(__error(), 0);
        void waitpid(pid, status, 0);
        if (status.v) {
            return 0;
        }
    }
    return 1;
}

/** C ref: report.c:461 @returns {CInt} */
export function dobugreport() {
    if (!submit_web_report(2, null, __sl19)) {
        pline(__sl20, (cptr.ldPtro(sysopt, FLD.sysopt_s_crashreporturl) && cptr.ld1s(cptr.ldPtro(sysopt, FLD.sysopt_s_crashreporturl))) ? cptr.ldPtro(sysopt, FLD.sysopt_s_crashreporturl) : __sl21);
    }
    return NHM.ECMD_OK;
}

/** C ref: report.c:485 @returns {CInt} */
export function NH_panictrace_libc() {
    let bt = cptr.alloc(20 * 8);
    let count;
    let x;
    let info;
    let buf = new Uint8Array(256);
    (cptr.ldPtro(windowprocs, FLD.window_procs_win_raw_print))(__sl22);
    count = backtrace(bt, 20);
    info = backtrace_symbols(bt, count);
    for (x = 0; x < count; x++) {
        copynchars(cptr.decay(buf), cptr.ldPtro(info, x, 8), 255);
        void strsubst(cptr.decay(buf), __sl12, __sl13);
        void strsubst(cptr.decay(buf), __sl12, __sl13);
        raw_printf(__sl23, BigInt.asUintN(64, BigInt(x)), cptr.decay(buf));
    }
    return 1;
}

/** C ref: report.c:529 @returns {CInt} */
export function NH_panictrace_gdb() {
    let gdbpath = cptr.ldPtro(sysopt, FLD.sysopt_s_gdbpath);
    let greppath = cptr.ldPtro(sysopt, FLD.sysopt_s_greppath);
    let buf = new Uint8Array(256);
    let gdb;
    if (cptr.eq(gdbpath, (null)) || cptr.ld1so(gdbpath, 0) == 0)
        return 0;
    if (cptr.eq(greppath, (null)) || cptr.ld1so(greppath, 0) == 0)
        return 0;
    nh_snprintf(__sl24, 545, cptr.decay(buf), 256n, __sl25, gdbpath, ARGV0.v, getpid(), greppath);
    gdb = popen(cptr.decay(buf), __sl26);
    if (gdb) {
        (cptr.ldPtro(windowprocs, FLD.window_procs_win_raw_print))(__sl22);
        void fprintf(gdb, __sl27);
        void fflush(gdb);
        sleep(4);
        void pclose(gdb);
        return 1;
    } else {
        return 0;
    }
}

/** C ref: report.c:571 — @param {CInt} lineno @returns {CPtr} */
export function get_saved_pline(lineno) {
    let p;
    let limit = NHM.DUMPLOG_MSG_COUNT;
    if (lineno >= NHM.DUMPLOG_MSG_COUNT)
        return null;
    p = u32mod(((cptr.ldI32o(gs, FLD.instance_globals_s_saved_pline_index) - 1) >>> 0), NHM.DUMPLOG_MSG_COUNT) | 0;
    while (limit--) {
        if (cptr.ldPtro2(gs, p, 8, FLD.instance_globals_s_saved_plines)) {
            if (lineno--) {
                p = ((((p - 1) | 0) + NHM.DUMPLOG_MSG_COUNT) | 0) % NHM.DUMPLOG_MSG_COUNT;
            } else {
                return cptr.ldPtro2(gs, p, 8, FLD.instance_globals_s_saved_plines);
            }
        }
    }
    return null;
}

/** C ref: report.c:600 — @param {CInt} sig_unused */
export function panictrace_handler(sig_unused) {
    let f2;
    f2 = Number(BigInt.asIntN(32, cptr.write(2, __sl28, 18n)));
    (void (f2));
    NH_abort(null);
}

/** C ref: report.c:625 — @param {CInt} set */
export function panictrace_setsignals(set) {
    void signal(4, set ? panictrace_handler : null);
    ;
    void signal(5, set ? panictrace_handler : null);
    ;
    void signal(6, set ? panictrace_handler : null);
    ;
    void signal(10, set ? panictrace_handler : null);
    ;
    void signal(8, set ? panictrace_handler : null);
    ;
    void signal(11, set ? panictrace_handler : null);
    ;
    void signal(12, set ? panictrace_handler : null);
    ;
    void signal(7, set ? panictrace_handler : null);
    ;
}

// --- BEGIN c2js reset block (tools/c2js/resetify.mjs) — do not edit ---
// 8 bindings: 2 rebound+refilled, 3 rebound, 3 refilled.
// S/P are supplied by js/generated/__reset.js so this module needs no new import.
let __c2js_rs = null;
export function __captureState(S) { __c2js_rs = [S(bid), S(__static_crashreport_init_once), S(__static_crashreport_init_hexdigits), S(url), S(urem), S(uend), S(utmp), S(mark)]; }
export function __resetState(P) {
    const r = __c2js_rs;
    if (r === null) throw new Error("report.js: __resetState before __captureState");
    P(r[0]);
    __static_crashreport_init_once = P(r[1]);
    P(r[2]);
    P(r[3]);
    urem = P(r[4]);
    uend = P(r[5]);
    utmp = P(r[6]);
    mark = P(r[7]);
}
// --- END c2js reset block ---
