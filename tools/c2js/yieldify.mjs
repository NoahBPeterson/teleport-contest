...
// a meaningless parity pass.
{
  ...
  const patches = [
    // 1. boot the yieldable module graph — as ONE module, not 180.
    //
    // tools/c2js/bundle.mjs scope-hoists the whole yieldable tree into
    // js/generated-y/__bundle.js in ESM evaluation order, and exports the two
    // names this harness reaches for. The tree is still emitted and is still
    // what every build tool reads; this is the only rung that loads the
    // bundle instead, because this is the rung a slow link paints from and the
    // mirror charges 12-16 ms per request whatever the request is
    // (docs/NOTES-startup.md §6.3, §8). Both patches land on the same
    // specifier, so the second import is a module-map hit.
    ["await import('../generated/unixmain.js')", `await import('../${path.basename(OUT_DIR)}/__bundle.js')`],
    ["await import('../generated/rnd.js')", `await import('../${path.basename(OUT_DIR)}/__bundle.js')`],
