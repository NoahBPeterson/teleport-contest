...
// a meaningless parity pass.
{
  ...
  const patches = [
    ...
    ['um.main(1, argv);', 'Y.trampoline(um.main(1, argv), __yieldKey);'],
  ...
  const preamble = [
    ...
    '// Do not edit by hand; re-run the tool. Six mechanical patches, each asserted',
    '// to match exactly once: two import redirects, the main() trampoline, and the',
    '// three lines that turn g.getchar from a blocking call into a yielding one.',
    "import * as Y from '../yield-rt.js';",
    '',
    '// Supplies a key when the engine parks. Left throwing for replay, where the',
    '// whole move string is queued up front and getchar never reaches the yield —',
    '// which is exactly what makes the corpus run a test of the TRANSFORM rather',
    '// than of the trampoline. runBootGame({ waitForKey }) installs a real source.',
    "const __yieldKey = () => { throw new Error('harness-y: engine parked with no key source'); };",
    '',
  ].join('\n');
  fs.writeFileSync(path.join(repoRoot, 'js/boot/harness-y.mjs'), preamble + h);
  console.log('yieldify -> js/boot/harness-y.mjs (6 patches applied)');
}
