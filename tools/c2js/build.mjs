...
function writeFieldModule(values) {
  ...
  const lines = [
    ...
    '// and the size of each record the port measures, named `sizeof_<record>` —',
    '// `sizeof_<record>_x<count>` being an array of them (see sizeofCode()).',
    `// Transpiler: tools/c2js ${TRANSPILER_VERSION}`,
    '//',
...
function writeRngModule() {
  ...
  const lines = [
    ...
    '',
    `import { ${[...called, 'rng_log_enabled', 'rng_log_set_caller'].sort().join(', ')} } from './rnd.js';`,
    '',
    ...bodies,
    '',
  ];
