...
| 1.10 | Lua→JS script port | 🔄 | Architecture landed + PoC. Ported scripts are readable JS driving the transpiled Lua C API on a port-owned lua_State (the lspo_* bindings are unmodified); interception is at the harness VFS (fopen swaps in a same-length comment stub, fclose runs the port), inert with `C2JS_LUA_PORT=0`. `oracle.lua` ported: corpus 69/69 both configs, `tools/lua-oracle.mjs` shows rng 54924/54924 + screens 17829/17829 + level fingerprint MATCH, negative control FAILs. 130 scripts left, ≈26 agent-legs staged S1–S7; biggest risk is reachability (half the corpus is behind quest/Sokoban/Gehennom gameplay no session performs). docs/NOTES-lua-port.md |
