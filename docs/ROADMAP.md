...
| 1.4 | Browser playability | 🔄 | Judge check = Chromium, 5 ms/move, fails on ANY console line, and gives a session ~3 s to paint its FIRST frame. Crawl reported 0 moves in 88 sessions with browser_ok:true — two causes found and fixed (docs/NOTES-transport-ladder.md): (a) the serial transport ladder paid every hanging rung's timeout before the fallback booted (staged with --hang-sw: first frame 11,481 ms → 1,879 ms), now a **race** — transports prepared in parallel, replay fallback booting alongside, first frame = min(transport, fallback), and a late transport is swapped in mid-game (byte-equal screens); (b) the judge's pages install a `process` stub claiming versions.node, so all three env checks took Node paths in a browser (--judge-stub: replay + console.error → xhr, 0 console; viewer 1/3 → 3/3 sessions). first_frame_ms now 1.0–1.9 s in every staged mode, 0 console lines, 1.19–2.67 ms/move. Await crawl verdict. |
...
| 1.10 | Lua→JS script port | ✅ | **All 131 scripts ported, 100% of the corpus by bytes**, on branch `lua-port` (not yet merged). Ported scripts are readable JS driving the transpiled Lua C API on a port-owned lua_State (the lspo_* bindings are unmodified); interception is at the harness VFS (fopen swaps in a same-length comment stub, fclose runs the port), inert with `C2JS_LUA_PORT=0`. A game with every port live **parses zero bytes of Lua source** (census over all 69 sessions: 954 loads intercepted, 0 parsed). Refreshed onto main@8c7b833: the ports share the resettable realm, thirteen hand-written bindings reset, reset-diff 12/12 via runSegment and 0/3 with that reset disabled; the yieldable build keeps the interpreter by construction (yieldify patch 8). Gates: corpus 69/69 reset ×2, fork ×1, ports-off ×1; marathon 54924/54924 + 17829/17829; judge-sim PASS; strict-score 503 files / 0 violations; reset-census js/lua-js 46 signed / 0 unclassified. docs/NOTES-lua-port.md §16 |
...
| 1.24 | Emit hygiene: `__FILE__`, `do{}while(0)`, function-like macros | 🔄 | On `emit-hygiene`, not merged. (a) **`__FILE__` leaked an absolute machine path** into 94 literals + 18 comments — a privacy leak, a break of the Phase-2 byte-reproducibility property, and a live parity risk (these strings are `nhalloc`/`panic`/`impossible`/`nhassert_failed`'s `file` argument, so a held-out session that panics would print a path the C recorder never held). Ground truth read out of the built recorder, not guessed: `src/Makefile:459-460` compiles from `src/` passing `$<`, and `strings src/nethack` has 98 bare basenames, exactly two slashed paths (`../win/tty/{topl,wintty}.c`) and zero absolute paths. Rule = re-relativize to the compile cwd; all 94 verified present in the binary's own string table. New build gate: no emitted module may carry a home directory or this checkout's root (360 modules clean) — *not* "no absolute path", since `loslib.c`'s `/tmp/lua_XXXXXX` is genuine. (b) **421 `do{}while(0)` flattened, 0 refused**, on an escape analysis over the *emitted text* (it sees goto-lowering splices the AST does not); 18 unit cases. (c) **Function-like macro tier** (readability §5's deferred class 4): 356 helpers, 6,659 sites, 6,619 case (a) / 40 case (b), 48 repeated call-arguments now evaluated once — gated by a new transitive purity analysis (819/6,586 functions) that is cross-checked against the emitted JS by `tools/c2js/purity-audit.mjs` (0 disagreements; it already forced one retraction). `js/generated` **shrinks 2.9%**. docs/NOTES-emit-hygiene.md |
